...
class Array : public ArrayBase {
 public:
  ...
  // When <src>T</src> is a class type like <src>casacore::Complex</src> or <src>std::string</src>,
  // elements are initialized. This inconsistent behavior confuses programmers and make it hard to
  // write efficient and generic code using template. Especially when <src>T</src> is of type
  // <src>Complex</src> or <src>DComplex</src> and it is unnecessary to initialize, provide
  // initPolicy with value <src>NO_INIT</src> to skip the initialization. Therefore, it is strongly
  // recommended to explicitly provide initPolicy parameter,
  ...
  static struct uninitializedType {
  } uninitialized;
  ...
  // If <src>policy</src> is <src>COPY</src>, storage of a new copy is allocated by
  // <src>DefaultAllocator<T></src>. If <src>policy</src> is <src>TAKE_OVER</src>,
  // <src>storage</src> will be destructed and released by the specified allocator. <srcblock>
  //   FILE *fp = ...;
  ...
  Array<T>& operator=(const Array<T>& other) { return assign_conforming(other); }

  // Calls assign_conforming().
  Array<T>& operator=(const MaskedArray<T>& marray) { return assign_conforming(marray); }
  ...
  bool reformOrResize(const IPosition& newShape, size_t resizePercentage = 0,
  ...
  bool adjustLastAxis(const IPosition& newShape, size_t resizePercentage = 0,
                      bool resizeIfNeeded = true);
  ...
  const Array<T> operator()(const IPosition& start, const IPosition& end,
  ...
  // <group>
  bool conform(const Array<T>& other) const { return conform2(other); }
  ...
  // <group>
  T* data() { return begin_p; }
  const T* data() const { return begin_p; }
  // </group>
  ...
  // <group>
  // If <src>policy</src> is <src>COPY</src>, storage of a new copy is allocated by
  // <src>allocator</src>. If <src>policy</src> is <src>TAKE_OVER</src>, <src>storage</src> will be
  // destructed and released by <src>allocator</src>.
  ...
  class BaseIteratorSTL {
   public:
    ...
    bool operator==(const BaseIteratorSTL& other) const { return itsPos == other.itsPos; }

    bool operator!=(const BaseIteratorSTL& other) const { return itsPos != other.itsPos; }

    T* getPos() { return const_cast<T*>(itsPos); }
  ...
  class ConstIteratorSTL : public BaseIteratorSTL {
   public:
    ...
    const T& operator*() const { return *this->itsPos; }
    const T* operator->() { return this->itsPos; }

    const T* pos() const { return this->itsPos; }
  };
  // </group>
  ...
  iterator end() { return iterator(end_p); }
  const_iterator end() const { return const_iterator(end_p); }
  // </group>

  // Get the begin iterator object for a contiguous array.
  // <group>
  contiter cbegin() { return begin_p; }
  const_contiter cbegin() const { return begin_p; }
  contiter cend() { return end_p; }
  const_contiter cend() const { return end_p; }
  // </group>
 ...
 private:
  ...
  bool isUnique() const { return !data_p->is_shared() && nrefs() == 1; }
    ...
    if (fixedDimensionality() != 0 && newShape.size() != fixedDimensionality())
      throw(ArrayNDimError(fixedDimensionality(), newShape.size(),
                           std::string("Invalid size given to ") + typeid(*this).name() +
                               ...
                               std::to_string(fixedDimensionality())));
  }
