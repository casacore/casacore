...
template <typename T>
Array<T>::Array() : data_p(new arrays_internal::Storage<T>(0)), begin_p(nullptr), end_p(nullptr) {
  assert(ok());
}
...
// </thrown>
template <typename T>
Array<T>::Array(const IPosition &shape, const T &initialValue)
    : ArrayBase(shape),
...
Array<T>::Array(const Array<T> &other)
    : ArrayBase(other), data_p(other.data_p), begin_p(other.begin_p), end_p(other.end_p) {
  assert(ok());
}

template <typename T>
Array<T>::Array(Array<T> &&source) noexcept
...
template <typename T>
Array<T>::Array(Array<T> &&source, const IPosition &shapeForSource) noexcept
...
template <class T>
Array<T>::Array(const IPosition &shape, T *storage, StorageInitPolicy policy)
    : ArrayBase(shape), data_p(), begin_p(nullptr), end_p(nullptr) {
...
Array<T> &Array<T>::operator=(Array<T> &&source) {
  bool swapWouldClashRequirements = source.fixedDimensionality() != 0 &&
                                    fixedDimensionality() != 0 &&
...
template <class T>
void Array<T>::assignBase(const ArrayBase &other, bool checkType) {
  assert(ok());
...
template <class T>
void Array<T>::reference(const Array<T> &other) {
  assert(ok());
  ...
  if (other.ndim() < fixedDimensionality()) {
    ...
    for (size_t i = 0; i != other.ndim(); ++i) newShape[i] = other.shape()[i];
    const int newValue = (other.nelements() == 0) ? 0 : 1;
    for (size_t i = other.ndim(); i != fixedDimensionality(); ++i) newShape[i] = newValue;
    Array<T> tmp;
    tmp.reference(other);
...
template <class T>
void Array<T>::copyToContiguousStorage(T *storage, Array<T> const &src, std::true_type) {
  if (src.contiguousStorage()) {
    ...
    while (!ai.pastEnd()) {
      index = ai.pos();
      size_t offset =
          ArrayIndexOffset(src.ndim(), src.originalLength_p.storage(), src.inc_p.storage(), index);
      copy_n_with_stride(src.begin_p + offset, size, storage + count * size, 1U, src.inc_p(0));
      ai.next();
      count++;
    }
  }
...
Array<T> Array<T>::copy() const {
  ...
  Array<T> vp(shape());
  if (ndim() != 0) copyToContiguousStorage(vp.begin_p, *this);
...
Array<T> &Array<T>::assign_conforming_implementation(const Array<T> &other, std::true_type) {
  ...
  if (Conform == true) {  // Copy in place
    if (ndim() == 0) {
      ...
      // Special case which can be quite common (e.g. row in a matrix).
      copy_n_with_stride(other.begin_p, length_p(1), begin_p, originalLength_p(0) * inc_p(1),
      ...
      while (!ai.pastEnd()) {
        index = ai.pos();
        offset = ArrayIndexOffset(ndim(), originalLength_p.storage(), inc_p.storage(), index);
        offset2 = ArrayIndexOffset(other.ndim(), other.originalLength_p.storage(),
                                   other.inc_p.storage(), index);
        copy_n_with_stride(other.begin_p + offset2, length_p(0), begin_p + offset, inc_p(0),
                           other.inc_p(0));
        ai.next();
      }
    }
...
Array<T> &Array<T>::assign_conforming(const MaskedArray<T> &marray) {
  ...
  bool deleteMask;
  const LogicalArrayElem *maskStorage = marray.getMaskStorage(deleteMask);
...
void Array<T>::set(const T &Value) {
  ...
  if (ndim() == 0) {
    ...
    // Special case which can be quite common (e.g. row in a matrix).
    fill_n_with_stride(begin_p, length_p(1), Value, originalLength_p(0) * inc_p(1));
  } else if (length_p(0) <= 25) {
    // If not many elements on a line, it's better to use this loop.
    iterator iterend = end();
    for (iterator iter = begin(); iter != iterend; ++iter) {
...
template <typename T>
bool Array<T>::adjustLastAxis(const IPosition &newShape, size_t resizePercentage,
                              bool resizeIfNeeded) {
  assert(ok());

  IPosition currentShape = shape();
  if (newShape.size() ==
      currentShape.size()) {  // Let base method handle attempt dimensionality changes
    for (size_t i = 0; i < newShape.size() - 1; i++) {
      if (currentShape(i) != newShape(i)) {
        std::string message =
            "Array<T>::extend - New shape can only change last dimension:"
            " current=" +
            currentShape.toString() + ", new=" + newShape.toString();
  ...
  bool resetEnd = ArrayBase::reformOrResize(newShape, resizeIfNeeded, data_p.use_count(),
                                            data_p->size(), true, resizePercentage);
...
template <class T>
bool Array<T>::reformOrResize(const IPosition &newShape, size_t resizePercentage,
                              bool resizeIfNeeded) {
  assert(ok());
  ...
  bool resetEnd = ArrayBase::reformOrResize(newShape, resizeIfNeeded, data_p.use_count(),
                                            data_p->size(), false, resizePercentage);
...
template <class T>
void Array<T>::nonDegenerate(const Array<T> &other, size_t startingAxis, bool throwIfError) {
...
template <class T>
void Array<T>::doNonDegenerate(const Array<T> &other, const IPosition &ignoreAxes) {
  assert(ok());
...
template <class T>
bool Array<T>::conform(const MaskedArray<T> &other) const {
...
template <class T>
void Array<T>::resize(const IPosition &len, bool copyValues) {
  assert(ok());
...
// </thrown>
template <typename T>
Array<T> Array<T>::operator()(const IPosition &b, const IPosition &e, const IPosition &i) {
  assert(ok());
  Array<T> tmp(*this);
...
template <class T>
const Array<T> Array<T>::operator()(const IPosition &b, const IPosition &e,
                                    const IPosition &i) const {
  return const_cast<Array<T> *>(this)->operator()(b, e, i);
}

template <typename T>
Array<T> Array<T>::operator()(const IPosition &b, const IPosition &e) {
  IPosition i(e.nelements());
  i = 1;
  return (*this)(b, e, i);
}
template <class T>
const Array<T> Array<T>::operator()(const IPosition &b, const IPosition &e) const {
  return const_cast<Array<T> *>(this)->operator()(b, e);
}

template <typename T>
Array<T> Array<T>::operator()(const Slicer &slicer) {
  if (slicer.isFixed()) {
...
template <typename T>
Array<T> Array<T>::operator[](size_t i) const {
  assert(ok());
  size_t nd = ndim();
  IPosition s(nd, 0);
  IPosition e(shape() - 1);
...
bool Array<T>::ok() const {
  ...
  if (data_p == nullptr) return false;
  if (nelements() > 0 && (begin_p == nullptr || data_p == nullptr)) return false;
...
T *Array<T>::getStorage(bool &deleteIt) {
  ...
  try {
    for (size_t i = 0; i != n; ++i) new (&storage[i]) T();
...
template <class T>
void Array<T>::putStorage(T *&storage, bool deleteAndCopy) {
  assert(ok());
  ...
  if (ndim() == 1) {
    ...
    // Special case which can be quite common (e.g. row in a matrix).
    move_n_with_stride(storage, length_p(1), begin_p, originalLength_p(0) * inc_p(1), 1U);
  } else if (length_p(0) <= 25) {
    // If not many elements on a line, it's better to use this loop.
    T *ptr = storage;
    iterator iterend = end();
    ...
    while (!ai.pastEnd()) {
      index = ai.pos();
      offset = ArrayIndexOffset(ndim(), originalLength_p.storage(), inc_p.storage(), index);
      move_n_with_stride(storage + count * length_p(0), length_p(0), begin_p + offset, inc_p(0),
                         1U);
      ai.next();
      count++;
    }
...
template <class T>
void Array<T>::takeStorage(const IPosition &shape, T *storage, StorageInitPolicy policy) {
  ...
  if (policy == TAKE_OVER) {
    ...
    for (size_t i = 0; i != new_nels; ++i) storage[new_nels - i - 1].~T();
...
Array<T>::BaseIteratorSTL::BaseIteratorSTL(const Array<T> &arr)
  ...
  if (arr.nelements() == 0) {
    ...
    if (!itsContig) {
      itsLineAxis = 0;
      while (itsLineAxis < arr.ndim() - 1 && itsLastPos(itsLineAxis) == 0) {
        itsLineAxis++;
      }
