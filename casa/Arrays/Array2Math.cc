...
void ComplexToReal(Array<float> &rarray, const Array<std::complex<float>> &carray) {
  if (rarray.nelements() != 2 * carray.nelements()) {
    throw(
        ArrayError("::ComplexToReal(Array<float> &rarray, const "
                   "Array<std::complex<float>> &carray) - rarray.nelements() != "
                   "2*carray.nelements()"));
  }
  if (rarray.contiguousStorage() && carray.contiguousStorage()) {
    memcpy(const_cast<float *>(rarray.data()), carray.data(), rarray.nelements() * sizeof(float));
  } else {
    Array<std::complex<float>>::const_iterator citer = carray.begin();
    Array<float>::iterator rend = rarray.end();
    for (Array<float>::iterator riter = rarray.begin(); riter != rend; ++riter, ++citer) {
      *riter = real(*citer);
      ++riter;
      *riter = imag(*citer);
    }
...
void ComplexToReal(Array<double> &rarray, const Array<std::complex<double>> &carray) {
  if (rarray.nelements() != 2 * carray.nelements()) {
    throw(
        ArrayError("::ComplexToReal(Array<double> &rarray, const "
                   "Array<std::complex<double>> &carray) - rarray.nelements() != "
                   "2*carray.nelements()"));
  }
  if (rarray.contiguousStorage() && carray.contiguousStorage()) {
    memcpy(const_cast<double *>(rarray.data()), carray.data(), rarray.nelements() * sizeof(double));
  } else {
    Array<std::complex<double>>::const_iterator citer = carray.begin();
    Array<double>::iterator rend = rarray.end();
    for (Array<double>::iterator riter = rarray.begin(); riter != rend; ++riter, ++citer) {
      *riter = real(*citer);
      ++riter;
      *riter = imag(*citer);
    }
...
void RealToComplex(Array<std::complex<float>> &carray, const Array<float> &rarray) {
  if (rarray.nelements() != 2 * carray.nelements()) {
    throw(
        ArrayError("::RealToComplex(Array<std::complex<float>> &carray, const "
                   "Array<float> &rarray) - rarray.nelements() != "
                   "2*carray.nelements()"));
  }
  if (rarray.contiguousStorage() && carray.contiguousStorage()) {
    std::copy_n(rarray.data(), rarray.nelements(),
                reinterpret_cast<float *>(const_cast<std::complex<float> *>(carray.data())));
  } else {
    Array<std::complex<float>>::iterator citer = carray.begin();
    Array<float>::const_iterator rend = rarray.end();
    for (Array<float>::const_iterator riter = rarray.begin(); riter != rend; ++riter, ++citer) {
      float r = *riter;
      ++riter;
...
void RealToComplex(Array<std::complex<double>> &carray, const Array<double> &rarray) {
  if (rarray.nelements() != 2 * carray.nelements()) {
    throw(
        ArrayError("::RealToComplex(Array<std::complex<double>> &carray, const "
                   "Array<double> &rarray) - rarray.nelements() != "
                   "2*carray.nelements()"));
  }
  if (rarray.contiguousStorage() && carray.contiguousStorage()) {
    std::copy_n(rarray.data(), rarray.nelements(),
                reinterpret_cast<double *>(const_cast<std::complex<double> *>(carray.data())));
  } else {
    Array<std::complex<double>>::iterator citer = carray.begin();
    Array<double>::const_iterator rend = rarray.end();
    for (Array<double>::const_iterator riter = rarray.begin(); riter != rend; ++riter, ++citer) {
      double r = *riter;
      ++riter;
...
Array<std::complex<float>> RealToComplex(const Array<float> &rarray) {
  ...
  if (shape(0) % 2 == 1) {  // Odd size
    throw(
        ArrayError("Array<std::complex<float>> RealToComplex(const Array<float> &"
...
Array<std::complex<double>> RealToComplex(const Array<double> &rarray) {
  ...
  if (shape(0) % 2 == 1) {  // Odd size
    throw(
        ArrayError("Array<std::complex<double>> RealToComplex(const Array<double> &"
                   "rarray) - rarray.shape()(0) not even"));
  }
  shape(0) /= 2;
...
IPosition checkExpandArray(IPosition &mult, IPosition &inshp, const IPosition &inShape,
  ...
  for (size_t i = 0; i < outShape.size(); ++i) {
    ...
    if (inshp[i] <= 0 || inshp[i] > outShape[i] || outShape[i] % inshp[i] != 0) {
      throw ArrayError(
          "expandArray: length of each input array axis must "
