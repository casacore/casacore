...
// <reviewed reviewer="Ger van Diepen" date="2002/12/01" tests="tArrayAccessor"
// demos="dArrayAccessor">
// </reviewed>
// <prerequisite>
...
class ArrayAccessor<T, Axis<U>> : public ArrayBaseAccessor<T> {
 public:
  // Constructors
  // <group>
  ...
  // at (0,0,...)).
  explicit ArrayAccessor(const Array<T> &arr) : ArrayBaseAccessor<T>(arr) { initStep(); }
  // Construct from an ArrayAccessor along same axis. The accessor will point
  // at the same element as the originator.
  ArrayAccessor(const ArrayAccessor<T, Axis<U>> &other) : ArrayBaseAccessor<T>(other) { ; }
  // Construct from accessor along another (or run-time) axis.
  // The accessor will point to the same element (but will be oriented
  // along another axis).
  // <group>
  template <size_t X>
  explicit ArrayAccessor(const ArrayAccessor<T, Axis<X>> &other) : ArrayBaseAccessor<T>(other) {
    initStep();
  }
  explicit ArrayAccessor(const ArrayAccessor<T, AxisN> &other) : ArrayBaseAccessor<T>(other) {
    initStep();
  }
  // </group>
  ...
  ArrayAccessor &operator=(const ArrayAccessor<T, Axis<U>> &other) {
    if (&other != this) {
      ...
      this->step_p = other.step_p;
      this->begin_p = other.begin_p;
  ...
  // <group>
  const T &next(const AxisN ax) const { return *(this->ptr_p + this->arrayPtr_p->steps()[ax.N]); }
  T &next(const AxisN ax) { return *(this->ptr_p + this->arrayPtr_p->steps()[ax.N]); }
  const T &prev(const AxisN ax) const { return *(this->ptr_p - this->arrayPtr_p->steps()[ax.N]); }
  T &prev(const AxisN ax) { return *(this->ptr_p - this->arrayPtr_p->steps()[ax.N]); }
  // </group>
 ...
 private:
  ...
  void initStep() {
    ...
    this->begin_p = this->end_p = this->ptr_p - initOff(this->ptr_p - this->arrayPtr_p->data(),
                                                        this->arrayPtr_p->ndim() - 1) *
                                                    this->step_p;
...
//
template <class T>
class ArrayAccessor<T, AxisN> : public ArrayBaseAccessor<T> {
 public:
  // Constructors
  // <group>
  explicit ArrayAccessor(const AxisN ax = AxisN(0)) : ArrayBaseAccessor<T>() {
    this->axis_p = ax.N;
  }
  explicit ArrayAccessor(Array<T> &arr, const AxisN ax = AxisN(0))
      : ArrayBaseAccessor<T>(arr, ax.N) {
    initStep();
  }
  ArrayAccessor(ArrayAccessor<T, AxisN> &other) : ArrayBaseAccessor<T>(other) { ; }
  explicit ArrayAccessor(ArrayAccessor<T, AxisN> &other, const AxisN ax)
      : ArrayBaseAccessor<T>(other, ax.N) {
    initStep();
  }
  template <size_t X>
  explicit ArrayAccessor(ArrayAccessor<T, Axis<X>> &other, const AxisN ax = AxisN(0))
      : ArrayBaseAccessor<T>(other, ax.N) {
    initStep();
  }
  ArrayAccessor &operator=(const ArrayAccessor<T, AxisN> &other) {
  ...
  void init(const Array<T> &arr, const AxisN ax) {
    ArrayBaseAccessor<T>::init(arr, ax.N);
    initStep();
  }
  void init(const AxisN ax) { ArrayBaseAccessor<T>::init(ax.N); }
  // </group>
  ...
  // Comparisons
  // <group>
  bool operator==(const ArrayAccessor<T, AxisN> &other) const { return this->ptr_p == other.ptr_p; }
  bool operator!=(const ArrayAccessor<T, AxisN> &other) const { return this->ptr_p != other.ptr_p; }
  bool operator==(const T *other) const { return this->ptr_p == other; }
  bool operator!=(const T *other) const { return this->ptr_p != other; }
  // </group>
