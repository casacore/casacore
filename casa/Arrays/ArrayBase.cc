...
ArrayBase::ArrayBase() noexcept : nels_p(0), ndimen_p(0), contiguous_p(true) {}
...
void ArrayBase::baseReform(ArrayBase& tmp, const IPosition& len, bool strict) const {
  ...
  if (strict && prod != (long long)(nelements())) {
    throw(
        ArrayConformanceError("ArrayBase::reform() - "
                              "total elements differ: " +
                              to_string(len) + " vs " + to_string(shape())));
  }
...
void ArrayBase::baseNonDegenerate(const ArrayBase& other, const IPosition& ignoreAxes) {
  assert(this != &other);
  assert(other.ndim() > 0);
...
bool ArrayBase::reformOrResize(const IPosition& newShape, bool resizeIfNeeded, size_t nReferences,
                               long long nElementsAllocated, bool copyDataIfNeeded,
  ...
  if (resizeNeeded && !resizeIfNeeded) {
    ...
    std::string message =
        "ArrayBase::reformOrResize() - insufficient storage for reform: "
        "nElementInAllocation=" +
        std::to_string(nElementsAllocated) +
...
//     <item> ArrayError
// </thrown>
size_t ArrayBase::makeSubset(ArrayBase& out, const IPosition& b, const IPosition& e,
                             const IPosition& i) {
  if (b.nelements() != ndim() || e.nelements() != ndim() || i.nelements() != ndim()) {
    std::ostringstream os;
    os << "ArrayBase::operator()(b,e,i) - ndim() b: " << b.nelements() << " e: " << e.nelements()
       << " i: " << i.nelements() << " differs from the array ndim " << ndim();
    throw(ArrayError(os.str()));
  }
  size_t j;
  for (j = 0; j < ndim(); j++) {
    if (b(j) < 0 || b(j) > e(j) + 1 || e(j) >= length_p(j) || i(j) < 1) {
      std::ostringstream os;
...
void ArrayBase::validateConformance(const ArrayBase& other) const {
  ...
  if (!conform2(other)) {
    if (ndim() != other.ndim()) {
      throw(ArrayNDimError(ndim(), other.ndim(), "ArrayBase::validateConformance"));
    } else {
      throw(ArrayShapeError(shape(), other.shape(), "ArrayBase::validateConformance"));
    }
  }
}
...
void ArrayBase::validateIndex(size_t index) const { validateIndex(IPosition(1, index)); }
...
bool ArrayBase::ok() const {
  ...
  for (i = 0; i < ndim(); i++) {
    ...
    if (length_p(i) < 0 || inc_p(i) < 1 || originalLength_p(i) < length_p(i)) {
      return false;
    }
    count *= length_p(i);
    if (length_p(i) > 1) {
      pos(i) = 1;
      size_t off = ArrayIndexOffset(ndim(), originalLength_p.storage(), inc_p.storage(), pos);
      pos(i) = 0;
    ...
    for (size_t i = 0; i < ndim(); ++i) {
      if (length_p[i] == 1) {
        ...
        if (length_p[i] != int(nels_p)) {
          throw(ArrayNDimError(1, ndim(), "Vector<T>: ndim of other array > 1"));
        }
...
void ArrayBase::checkMatrixShape() {
  if (ndim() > 2) {
    throw(ArrayNDimError(2, ndim(), "Matrix<T>: ndim of other array > 2"));
  }
  if (ndim() < 2) {
...
void ArrayBase::checkCubeShape() {
  if (ndim() > 3) {
    throw(ArrayNDimError(3, ndim(), "Cube<T>: ndim of other array > 3"));
  }
...
void* ArrayBase::getVStorage(bool&) { throw ArrayError("ArrayBase::getVStorage cannot be used"); }
...
void throwArrayShapes(const IPosition& shape1, const IPosition& shape2, const char* name) {
  throw ArrayConformanceError("ArrayMath/Logical function " + std::string(name) +
                              ": array shapes " + shape1.toString() + " and " + shape2.toString() +
                              " differ");
}
