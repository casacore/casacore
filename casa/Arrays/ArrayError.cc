...
ArrayIndexError::ArrayIndexError(const IPosition &in, const IPosition &sh, const char *m)
    : ArrayError(m), i(in), l(sh) {
  // Nothing
}

ArrayIndexError::~ArrayIndexError() noexcept {}

IPosition ArrayIndexError::index() const { return i; }

IPosition ArrayIndexError::shape() const { return l; }

ArrayConformanceError::ArrayConformanceError() : ArrayError("ArrayConformanceError") {
  // Nothing
}
...
ArrayShapeError::ArrayShapeError(const IPosition &s1, const IPosition &s2, const char *m)
    : ArrayConformanceError(m + std::string(" shape ") + s1.toString() + " differs from " +
                            s2.toString()),
      sh1(s1),
      sh2(s2) {
  // Nothing
...
ArraySlicerError::ArraySlicerError(const std::string &m) : ArrayError("Slicer error:" + m) {}
