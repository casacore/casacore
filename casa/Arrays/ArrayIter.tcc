...
template <typename T>
ArrayIterator<T>::ArrayIterator(const Array<T>& a, size_t byDim)
    : ArrayPositionIterator(a.shape(), byDim), ap_p(), pOriginalArray_p() {
  init(a);
}

template <typename T>
ArrayIterator<T>::ArrayIterator(const Array<T>& a, const IPosition& axes, bool axesAreCursor)
    : ArrayPositionIterator(a.shape(), axes, axesAreCursor), ap_p(), pOriginalArray_p() {
  init(a);
}
...
// </thrown>
template <typename T>
void ArrayIterator<T>::init(const Array<T>& a) {
  pOriginalArray_p.reference(a);
  dataPtr_p = pOriginalArray_p.begin_p;

  if (dimIter() < 1)
    throw(
        ArrayIteratorError("ArrayIterator<T>::ArrayIterator<T> - "
...
template <typename T>
void ArrayIterator<T>::set(const IPosition& cursorPos) {
...
template <typename T>
ArrayBase& ArrayIterator<T>::getArray() {
  return array();
}
