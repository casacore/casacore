...
LogicalArray near(const Array<T> &l, const Array<T> &r, double tol) {
  ...
  LogicalArray result(l.shape());
  arrayContTransform(l, r, result,
                     [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
  return result;
}
...
LogicalArray nearAbs(const Array<T> &l, const Array<T> &r, double tol) {
  ...
  LogicalArray result(l.shape());
  arrayContTransform(l, r, result,
                     [tol](T left, T right) { return arrays_internal::nearAbs(left, right, tol); });
  return result;
}
...
LogicalArray near(const Array<T> &array, const T &val, double tol) {
  LogicalArray result(array.shape());
  arrayContTransform(array, val, result,
                     [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
  return result;
}
...
LogicalArray near(const T &val, const Array<T> &array, double tol) {
  LogicalArray result(array.shape());
  arrayContTransform(val, array, result,
                     [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
  return result;
}
...
bool allNear(const Array<T> &l, const Array<T> &r, double tol) {
  return arrayCompareAll(
      l, r, [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
}

template <class T>
bool allNear(const Array<T> &array, const T &val, double tol) {
  return arrayCompareAll(
      array, val, [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
}

template <class T>
bool allNear(const T &val, const Array<T> &array, double tol) {
  return arrayCompareAll(
      val, array, [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
}

template <class T>
bool allNearAbs(const Array<T> &l, const Array<T> &r, double tol) {
  return arrayCompareAll(
      l, r, [tol](T left, T right) { return arrays_internal::nearAbs(left, right, tol); });
}

template <class T>
bool allNearAbs(const Array<T> &array, const T &val, double tol) {
  return arrayCompareAll(
      array, val, [tol](T left, T right) { return arrays_internal::nearAbs(left, right, tol); });
}

template <class T>
bool allNearAbs(const T &val, const Array<T> &array, double tol) {
  return arrayCompareAll(
      val, array, [tol](T left, T right) { return arrays_internal::nearAbs(left, right, tol); });
}

template <class T>
bool anyNear(const Array<T> &l, const Array<T> &r, double tol) {
  return arrayCompareAny(
      l, r, [tol](T left, T right) { return arrays_internal::near(left, right, tol); });
}
...
size_t nfalse(const Array<T> &array) {
  return (array.contiguousStorage() ? std::count(array.cbegin(), array.cend(), T())
