...
void myltransform(_InputIterator1 __first1, _InputIterator1 __last1, _OutputIterator __result,
                  T left, _BinaryOperation __binary_op) {
  for (; __first1 != __last1; ++__first1, ++__result) *__result = __binary_op(left, *__first1);
}
// sequence = sequence OP scalar
template <typename _InputIterator1, typename T, typename _OutputIterator, typename _BinaryOperation>
void myrtransform(_InputIterator1 __first1, _InputIterator1 __last1, _OutputIterator __result,
                  T right, _BinaryOperation __binary_op) {
  for (; __first1 != __last1; ++__first1, ++__result) *__result = __binary_op(*__first1, right);
}
// sequence OP= scalar
template <typename _InputIterator1, typename T, typename _BinaryOperation>
void myiptransform(_InputIterator1 __first1, _InputIterator1 __last1, T right,
                   _BinaryOperation __binary_op) {
  for (; __first1 != __last1; ++__first1) *__first1 = __binary_op(*__first1, right);
}
// </group>
...
inline void arrayContTransform(const Array<L> &left, const Array<R> &right, Array<RES> &result,
                               BinaryOperator op) {
  assert(result.contiguousStorage());
  if (left.contiguousStorage() && right.contiguousStorage()) {
    std::transform(left.cbegin(), left.cend(), right.cbegin(), result.cbegin(), op);
  } else {
    std::transform(left.begin(), left.end(), right.begin(), result.cbegin(), op);
  }
}
...
// Result MUST be a contiguous array.
template <typename L, typename R, typename RES, typename BinaryOperator>
inline void arrayContTransform(const Array<L> &left, R right, Array<RES> &result,
                               BinaryOperator op) {
  assert(result.contiguousStorage());
  if (left.contiguousStorage()) {
    myrtransform(left.cbegin(), left.cend(), result.cbegin(), right, op);
    ////    std::transform (left.cbegin(), left.cend(),
    ////                    result.cbegin(), bind2nd(op, right));
  } else {
    myrtransform(left.begin(), left.end(), result.cbegin(), right, op);
...
// Result MUST be a contiguous array.
template <typename L, typename R, typename RES, typename BinaryOperator>
inline void arrayContTransform(L left, const Array<R> &right, Array<RES> &result,
                               BinaryOperator op) {
  assert(result.contiguousStorage());
  if (right.contiguousStorage()) {
    myltransform(right.cbegin(), right.cend(), result.cbegin(), left, op);
    ////    std::transform (right.cbegin(), right.cend(),
    ////                    result.cbegin(), bind1st(op, left));
  } else {
    myltransform(right.begin(), right.end(), result.cbegin(), left, op);
...
inline void arrayContTransform(const Array<T> &arr, Array<RES> &result, UnaryOperator op) {
  assert(result.contiguousStorage());
...
template <typename L, typename R, typename RES, typename BinaryOperator>
void arrayTransform(const Array<L> &left, const Array<R> &right, Array<RES> &result,
                    BinaryOperator op);
...
inline void arrayTransformInPlace(Array<L> &left, const Array<R> &right, BinaryOperator op) {
  if (left.contiguousStorage() && right.contiguousStorage()) {
...
void minMax(ScalarType &minVal, ScalarType &maxVal, IPosition &minPos, IPosition &maxPos,
...
template <typename ScalarType>
void minMax(ScalarType &minVal, ScalarType &maxVal, IPosition &minPos, IPosition &maxPos,
            const Array<ScalarType> &array, const Array<bool> &mask, bool valid = true);
// The array * weight is searched
template <typename ScalarType>
void minMaxMasked(ScalarType &minVal, ScalarType &maxVal, IPosition &minPos, IPosition &maxPos,
                  const Array<ScalarType> &array, const Array<ScalarType> &weight);
// </group>
...
template <typename T>
inline void max(Array<T> &result, const T &a, const Array<T> &b) {
  max(result, b, a);
}
// "result" contains the minimum of "a" and "b" at each position. "result",
// and "a" must be conformant.
...
template <typename T>
inline void min(Array<T> &result, const T &a, const Array<T> &b) {
  min(result, b, a);
}
// Return an array that contains the maximum of "a" and "b" at each position.
template <typename T>
...
template <typename T>
inline Array<T> max(const T &a, const Array<T> &b) {
  return max(b, a);
}
// Return an array that contains the minimum of "a" and "b" at each position.
template <typename T>
...
template <typename T>
inline Array<T> min(const T &a, const Array<T> &b) {
  return min(b, a);
}
// </group>
...
// most rapidly.
template <typename T>
inline void indgen(Array<T> &a, T start) {
  indgen(a, start, T(1));
}
...
template <typename T>
inline Vector<T> indgen(size_t length, T start, T inc) {
  Vector<T> x(length);
  indgen(x, start, inc);
  return x;
}
...
template <typename T>
T median(const Array<T> &a, std::vector<T> &scratch, bool sorted, bool takeEvenMean,
         bool inPlace = false);
// TODO shouldn't take a const Array for in place sorting
template <typename T>
...
template <typename T>
inline T median(const Array<T> &a, bool sorted) {
  return median(a, sorted, (a.nelements() <= 100), false);
}
template <typename T>
inline T median(const Array<T> &a) {
...
// TODO shouldn't take a const Array for in place sorting
template <typename T>
T madfm(const Array<T> &a, std::vector<T> &tmp, bool sorted, bool takeEvenMean,
        bool inPlace = false);
// TODO shouldn't take a const Array for in place sorting
template <typename T>
...
template <typename T>
inline T madfm(const Array<T> &a, bool sorted) {
  return madfm(a, sorted, (a.nelements() <= 100), false);
}
template <typename T>
inline T madfm(const Array<T> &a) {
  return madfm(a, false, (a.nelements() <= 100), false);
}
// TODO shouldn't take a const Array for in place sorting
template <typename T>
inline T madfmInPlace(const Array<T> &a, bool sorted = false) {
...
// TODO shouldn't take a const Array for in place sorting
template <typename T>
T fractile(const Array<T> &a, std::vector<T> &tmp, float fraction, bool sorted = false,
           bool inPlace = false);
// TODO shouldn't take a const Array for in place sorting
template <typename T>
...
// TODO shouldn't take a const Array for in place sorting
template <typename T>
T interFractileRange(const Array<T> &a, std::vector<T> &tmp, float fraction, bool sorted = false,
                     bool inPlace = false);
// TODO shouldn't take a const Array for in place sorting
template <typename T>
...
// TODO shouldn't take a const Array for in place sorting
template <typename T>
T interHexileRange(const Array<T> &a, std::vector<T> &tmp, bool sorted = false,
                   bool inPlace = false) {
...
// TODO shouldn't take a const Array for in place sorting
template <typename T>
T interQuartileRange(const Array<T> &a, std::vector<T> &tmp, bool sorted = false,
                     bool inPlace = false) {
...
void operator*=(Array<std::complex<T>> &left, const Array<T> &other) {
  checkArrayShapes(left, other, "*=");
  arrayTransformInPlace(left, other, [](std::complex<T> left, T right) { return left * right; });
}
...
Array<std::complex<T>> operator/(const Array<std::complex<T>> &left, const Array<T> &right) {
  checkArrayShapes(left, right, "/");
  Array<std::complex<T>> result(left.shape());
  arrayContTransform(left, right, result, [](std::complex<T> l, T r) { return l / r; });
  return result;
}
template <typename T>
Array<std::complex<T>> operator/(const Array<std::complex<T>> &left, const T &other) {
...
// Note that std::complex<float> and std::complex<double> are simply typedefs for
// std::complex<float> and std::complex<double>, so the result is in fact one of these types.
// <group>
template <typename T>
...
template <typename T>
T *expandRecursive(int axis, const IPosition &shp, const IPosition &mult, const IPosition &inSteps,
    ...
    if (alternate[axis]) {
      for (ssize_t j = 0; j < mult[axis]; ++j) {
        ...
        for (ssize_t i = 0; i < shp[axis]; ++i) {
          out = expandRecursive(axis - 1, shp, mult, inSteps, pin, out, alternate);
          pin += inSteps[axis];
        }
      }
    } else {
      for (ssize_t i = 0; i < shp[axis]; ++i) {
        for (ssize_t j = 0; j < mult[axis]; ++j) {
          out = expandRecursive(axis - 1, shp, mult, inSteps, in, out, alternate);
        }
        in += inSteps[axis];
      }
    }
...
void expandArray(Array<T> &out, const Array<T> &in, const IPosition &alternate = IPosition()) {
  ...
  IPosition alt = checkExpandArray(mult, inshp, in.shape(), out.shape(), alternate);
  Array<T> incp(in);
  ...
  expandRecursive(out.ndim() - 1, inshp, mult, incp.steps(), incp.data(), outPtr, alt);
...
IPosition checkExpandArray(IPosition &mult, IPosition &newInShape, const IPosition &inShape,
                           const IPosition &outShape, const IPosition &alternate);
