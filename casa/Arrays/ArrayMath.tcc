...
void arrayTransform(const Array<L> &left, const Array<R> &right, Array<RES> &result,
  ...
  if (result.contiguousStorage()) {
    ...
    if (left.contiguousStorage() && right.contiguousStorage()) {
      ...
      std::transform(left.begin(), left.end(), right.begin(), result.begin(), op);
    }
  }
}
...
// </thrown>
template <typename T>
void minMax(T &minVal, T &maxVal, IPosition &minPos, IPosition &maxPos, const Array<T> &array) {
  size_t n = array.nelements();
  if (n == 0) {
    throw(
        ArrayError("void minMax(T &min, T &max, IPosition &minPos,"
...
// </thrown>
template <typename T>
void minMaxMasked(T &minVal, T &maxVal, IPosition &minPos, IPosition &maxPos, const Array<T> &array,
  ...
  if (!array.shape().isEqual(weight.shape())) {
    throw(
        ArrayConformanceError("void minMaxMasked(T &min, T &max,"
...
template <typename T>
void minMax(T &minVal, T &maxVal, IPosition &minPos, IPosition &maxPos, const Array<T> &array,
            const Array<bool> &mask, bool valid) {
  size_t n = array.nelements();
  if (n == 0) {
    throw(
        ArrayError("void minMax(T &min, T &max, IPosition &minPos,"
  ...
  if (!array.shape().isEqual(mask.shape())) {
    throw(
        ArrayConformanceError("void minMax(T &min, T &max,"
                              "IPosition &minPos, IPosition &maxPos,"
                              "const Array<T> &array, "
                              "const Array<bool> &mask) - "
  ...
  if (minp == n) {
    throw(
        std::runtime_error("void minMax(T &min, T &max,"
                           "IPosition &minPos, IPosition &maxPos,"
                           "const Array<T> &array, "
...
template <typename T>
Array<T> operator~(const Array<T> &a) {
  return arrayTransformResult(a,
                              [](T val) { return ~val; });  // bit_not would be nicer, but is C++14
}
...
// </thrown>
template <typename T>
void minMax(T &minVal, T &maxVal, const Array<T> &array) {
  if (array.nelements() == 0) {
    throw(
        ArrayError("void minMax(T &min, T &max, const Array<T> &array) - "
                   "Array has no elements"));
  }
  if (array.contiguousStorage()) {
    ...
    for (typename Array<T>::const_contiter iter = array.cbegin(); iter != iterEnd; ++iter) {
      if (*iter < minv) {
        minv = *iter;
      }
    ...
    for (typename Array<T>::const_iterator iter = array.begin(); iter != iterEnd; ++iter) {
      if (*iter < minv) {
        minv = *iter;
      }
      if (*iter > maxv) {
...
template <typename T>
Array<T> square(const Array<T> &a) {
  return arrayTransformResult(a, [](T a) { return a * a; });
}

template <typename T>
Array<T> cube(const Array<T> &a) {
...
// </thrown>
template <typename T>
Array<T> atan2(const Array<T> &a, const Array<T> &b) {
  checkArrayShapes(a, b, "atan2");
  return arrayTransformResult(a, b, [](T l, T r) { return std::atan2(l, r); });
}

template <typename T>
Array<T> atan2(const T &a, const Array<T> &b) {
  return arrayTransformResult(a, b, [](T l, T r) { return std::atan2(l, r); });
}

template <typename T>
Array<T> atan2(const Array<T> &a, const T &b) {
  return arrayTransformResult(a, b, [](T l, T r) { return std::atan2(l, r); });
}
...
T sum(const Array<T> &a) {
  return a.contiguousStorage() ? std::accumulate(a.cbegin(), a.cend(), T(), std::plus<T>())
...
T sumsqr(const Array<T> &a) {
  ...
  return a.contiguousStorage() ? std::accumulate(a.cbegin(), a.cend(), T(), sumsqr)
...
template <typename T>
T pvariance(const Array<T> &a, T mean, size_t ddof) {
  if (a.nelements() < ddof + 1) {
    throw(ArrayError("::variance(const Array<T> &) - Need at least " + std::to_string(ddof + 1) +
                     " elements"));
  }
...
template <typename T>
T pvariance(const Array<T> &a, size_t ddof) {
...
// </thrown>
template <typename T>
T pstddev(const Array<T> &a, T mean, size_t ddof) {
  if (a.nelements() < ddof + 1) {
...
template <typename T>
T pstddev(const Array<T> &a, size_t ddof) {
...
T avdev(const Array<T> &a) {
  if (a.nelements() < 1) {
    throw(
        ArrayError("::avdev(const Array<T> &,) - Need at least 1 "
                   "element"));
  }
...
T avdev(const Array<T> &a, T mean) {
  if (a.nelements() < 1) {
    throw(
        ArrayError("::avdev(const Array<T> &,T) - Need at least 1 "
                   "element"));
  }
  auto sumabsdiff = [mean](T left, T right) { return left + std::abs(right - mean); };
  T sum = a.contiguousStorage() ? std::accumulate(a.cbegin(), a.cend(), T(), sumabsdiff)
...
T rms(const Array<T> &a) {
  if (a.nelements() < 1) {
    throw(
        ArrayError("::rms(const Array<T> &) - Need at least 1 "
                   "element"));
  }
  auto sumsqr = [](T left, T right) { return left + right * right; };
  T sum = a.contiguousStorage() ? std::accumulate(a.cbegin(), a.cend(), T(), sumsqr)
                                : std::accumulate(a.begin(), a.end(), T(), sumsqr);
...
// </thrown>
template <typename T>
T median(const Array<T> &a, std::vector<T> &scratch, bool sorted, bool takeEvenMean, bool inPlace) {
  T medval = T();
...
// </thrown>
template <typename T>
T madfm(const Array<T> &a, std::vector<T> &scratch, bool sorted, bool takeEvenMean, bool inPlace) {
...
// </thrown>
template <typename T>
T fractile(const Array<T> &a, std::vector<T> &scratch, float fraction, bool sorted, bool inPlace) {
  ...
  if (nelem < 1) {
    throw(
        ArrayError("::fractile(const Array<T>&) - Need at least 1 "
                   "elements"));
  }
  // A copy is needed if not contiguous or if not in place.
...
// </thrown>
template <typename T>
T interFractileRange(const Array<T> &a, std::vector<T> &scratch, float fraction, bool sorted,
                     bool inPlace) {
...
Array<std::complex<T>> makeComplex(const Array<T> &left, const Array<T> &right) {
  ...
  arrayContTransform(left, right, res, [](T r, T i) { return std::complex<T>(r, i); });
  return res;
}
...
void setReal(Array<C> &carray, const Array<R> &rarray) {
  ...
  arrayTransform(rarray, carray, carray, [](R l, C r) -> C { return C(l, std::imag(r)); });
}

template <typename C, typename R>
void setImag(Array<C> &carray, const Array<R> &rarray) {
  checkArrayShapes(carray, rarray, "setImag");
  arrayTransformInPlace(carray, rarray, [](C l, R r) -> C { return C(std::real(l), r); });
}
...
void convertArray(Array<T> &to, const Array<U> &from) {
  ...
  if (to.shape() != from.shape()) {
    throw(
        ArrayConformanceError("void ::convertArray(Array<T> &to, "
                              "const Array<U> &from)"
  ...
  if (to.contiguousStorage() && from.contiguousStorage()) {
    ...
    for (typename Array<T>::contiter iterTo = to.cbegin(); iterFrom != endFrom;
         ++iterFrom, ++iterTo) {
    ...
    for (typename Array<T>::iterator iterTo = to.begin(); iterFrom != endFrom;
         ++iterFrom, ++iterTo) {
      arrays_internal::convertScalar(*iterTo, *iterFrom);
    }
  }
