...
LogicalArray reformedMask(const Array<T>& data, const T truthvalue, const IPosition& desiredform) {
  if (data.shape().nelements() == desiredform.nelements() && data.shape() == desiredform) {
    ...
    if (rightExpandableToLeft(data.shape(), desiredform)) {
      ...
      ReadOnlyArrayIterator<T> data_cursor(
          data, IPosition::otherAxes(n_data_dim, IPosition::makeAxisPath(n_desired_dim)));
      IPosition collapsedPos;

      // Go through each position in the new array,
      for (ArrayPositionIterator positer(desiredform, 0); !positer.pastEnd(); positer.next()) {
      ...
      os << "reformedMask(): Could not reconcile the input shape (" << data.shape() << ")\n"
...
void binOpExpandInPlace(Array<L>& leftarr, const Array<R>& rightarr, BinaryOperator op) {
  ...
  if (leftShape.nelements() == rightShape.nelements() && leftShape == rightShape) {
    ...
    if (rightExpandableToLeft(leftShape, rightShape)) {
      IPosition iteraxes(
          IPosition::otherAxes(leftShape.nelements(), IPosition::makeAxisPath(n_right_dim)));
      ArrayIterator<L> left_cursor(leftarr, iteraxes);
      IPosition rightPos;

      // Go through each position in the new array,
      for (ArrayPositionIterator positer(rightShape, 0); !positer.pastEnd(); positer.next()) {
