...
ArrayPositionIterator::ArrayPositionIterator(const IPosition &shape, const IPosition &origin,
                                             size_t byDim)
    : Start(origin), Shape(shape), atOrBeyondEnd(false) {
  setup(byDim);
}

ArrayPositionIterator::ArrayPositionIterator(const IPosition &shape, size_t byDim)
    : Start(shape.nelements(), 0), Shape(shape), atOrBeyondEnd(false) {
  setup(byDim);
}

ArrayPositionIterator::ArrayPositionIterator(const IPosition &shape, const IPosition &iterAxes,
                                             bool axesAreCursor)
    : Start(shape.nelements(), 0), Shape(shape), atOrBeyondEnd(false) {
...
void ArrayPositionIterator::setup(size_t byDim) {
  if (byDim > ndim()) {
    throw(
        ArrayIteratorError("ArrayPositionIterator::ArrayPositionIterator"
...
void ArrayPositionIterator::setup(const IPosition &axes, bool axesAreCursor) {
...
void ArrayPositionIterator::set(const IPosition &cursorPos) {
  ...
  if (cursorPos.nelements() != iterationAxes.nelements()) {
    ...
    if (cursorPos.nelements() != ndim()) {
      throw ArrayIteratorError(
          "ArrayPositionIterator::set - "
