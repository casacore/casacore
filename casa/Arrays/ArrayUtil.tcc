...
template <class T>
Array<T> reorderArray(const Array<T>& array, const IPosition& newAxisOrder, bool alwaysCopy) {
  const IPosition& shape = array.shape();
  IPosition newShape, incr;
  size_t contAxes = reorderArrayHelper(newShape, incr, shape, newAxisOrder);
...
Array<T> reverseArray(const Array<T>& array, size_t axis, bool alwaysCopy) {
  ...
  for (size_t j = 0; j < outerProduct; ++j) {
    ...
    for (int k = 0; k < shape[axis]; ++k) {
      std::copy_n(indata + idx + innerProduct * k, innerProduct,
...
template <class T>
Array<T> reverseArray(const Array<T>& array, const IPosition& reversedAxes, bool alwaysCopy) {
  const IPosition& shape = array.shape();
  bool nothingToDo = true;
  for (size_t i = 0; i < reversedAxes.size(); i++) {
    if (reversedAxes[i] >= int(shape.size())) {
      throw ArrayError(std::string(__FUNCTION__) + ": axis number " +
