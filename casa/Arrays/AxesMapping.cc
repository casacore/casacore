...
AxesMapping::AxesMapping() : itsRemoved(false), itsReordered(false) {}
...
AxesMapping::AxesMapping(const IPosition& oldToNew)
  ...
  for (int i = 0; i < naxes; i++) {
    if (itsToNew(i) < 0) {
      ...
      if (itsToNew(i) >= naxes) throw std::runtime_error("itsToNew(i) >= naxes");
      itsToOld(itsToNew(i)) = i;
      nnew++;
    }
  }
  for (size_t i = 0; i < nnew; i++) {
    if (itsToOld(i) < 0) throw std::runtime_error("itsToOld(i)<0");
...
IPosition AxesMapping::shapeToNew(const IPosition& shape) const {
  ...
  for (size_t i = 0; i < naxes; i++) {
    if (itsToNew(i) < 0) {
      if (shape(i) != 1) throw std::runtime_error("shape(i)!=1");
    } else {
...
Slicer AxesMapping::slicerToNew(const Slicer& slicer) const {
  return Slicer(posToNew(slicer.start()), shapeToNew(slicer.length()), shapeToNew(slicer.stride()));
}

Slicer AxesMapping::slicerToOld(const Slicer& slicer) const {
  return Slicer(posToOld(slicer.start()), shapeToOld(slicer.length()), shapeToOld(slicer.stride()));
}
