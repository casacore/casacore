...
AxesSpecifier::AxesSpecifier() : itsKeep(true) {}

AxesSpecifier::AxesSpecifier(bool keepDegenerate) : itsKeep(keepDegenerate) {}

AxesSpecifier::AxesSpecifier(bool keepDegenerate, const IPosition& axisPath)
    : itsPath(axisPath), itsKeep(keepDegenerate) {}

AxesSpecifier::AxesSpecifier(const IPosition& keepAxes) : itsAxes(keepAxes), itsKeep(false) {}

AxesSpecifier::AxesSpecifier(const IPosition& keepAxes, const IPosition& axisPath)
    : itsAxes(keepAxes), itsPath(axisPath), itsKeep(false) {}
...
AxesMapping AxesSpecifier::apply(const IPosition& shape) const {
  ...
  if (itsKeep) {
    ...
    for (size_t i = 0; i < itsAxes.nelements(); i++) {
      if (itsAxes(i) >= naxes) throw std::runtime_error("itsAxes(i) >= naxes");
