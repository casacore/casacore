...
template <typename T>
Cube<T>::Cube() : Array<T>(IPosition(3, 0)) {
  assert(ok());
}

template <typename T>
Cube<T>::Cube(size_t l1, size_t l2, size_t l3, const T &initialValue)
    : Array<T>(IPosition(3, l1, l2, l3), initialValue) {
  assert(ok());
}

template <typename T>
Cube<T>::Cube(size_t l1, size_t l2, size_t l3, typename Array<T>::uninitializedType)
    : Array<T>(IPosition(3, l1, l2, l3), Array<T>::uninitialized) {
  assert(ok());
}

template <typename T>
Cube<T>::Cube(const IPosition &length, const T &initialValue) : Array<T>(length, initialValue) {
  this->checkBeforeResize(length);
}

template <typename T>
Cube<T>::Cube(const IPosition &length, typename Array<T>::uninitializedType)
    : Array<T>(length, Array<T>::uninitialized) {
  this->checkBeforeResize(length);
}

template <typename T>
Cube<T>::Cube(const Cube<T> &source) : Array<T>(source) {
  assert(ok());
}

template <typename T>
Cube<T>::Cube(Cube<T> &&source) : Array<T>(std::move(source), IPosition(3, 0)) {
  assert(ok());
}
...
// </thrown>
template <typename T>
Cube<T>::Cube(const Array<T> &source) : Array<T>(source) {
  this->checkCubeShape();
  assert(ok());
}

template <typename T>
Cube<T>::Cube(Array<T> &&source) : Array<T>(std::move(source)) {
  this->checkCubeShape();
  assert(ok());
}
...
// </thrown>
template <typename T>
Cube<T> Cube<T>::operator()(const Slice &sliceX, const Slice &sliceY, const Slice &sliceZ) {
  assert(ok());
  ...
  if (s1 < 1 || s2 < 1 || s3 < 1) {
    ...
    throw(
        ArrayError("Cube<T>::operator()(Slice,Slice,Slice) : "
...
template <typename T>
const Cube<T> Cube<T>::operator()(const Slice &sliceX, const Slice &sliceY,
                                  const Slice &sliceZ) const {
  return const_cast<Cube<T> *>(this)->operator()(sliceX, sliceY, sliceZ);
}

template <typename T>
void Cube<T>::doNonDegenerate(const Array<T> &other, const IPosition &ignoreAxes) {
  Array<T> tmp(*this);
  tmp.nonDegenerate(other, ignoreAxes);
  if (tmp.ndim() != 3) {
    throw(
        ArrayError("Cube::nonDegenerate (other, ignoreAxes) - "
...
// </thrown>
template <typename T>
Matrix<T> Cube<T>::xyPlane(size_t which) {
  assert(ok());
...
template <typename T>
const Matrix<T> Cube<T>::xyPlane(size_t which) const {
...
template <typename T>
Matrix<T> Cube<T>::xzPlane(size_t which) {
  assert(ok());
...
template <typename T>
const Matrix<T> Cube<T>::xzPlane(size_t which) const {
  Cube<T> *This = const_cast<Cube<T> *>(this);
  // Cast away constness, but the return type is a const Matrix<T>, so
  // this should still be safe.
...
template <typename T>
Matrix<T> Cube<T>::yzPlane(size_t which) {
  assert(ok());
...
template <typename T>
const Matrix<T> Cube<T>::yzPlane(size_t which) const {
  Cube<T> *This = const_cast<Cube<T> *>(this);
  // Cast away constness, but the return type is a const Matrix<T>, so
  // this should still be safe.
...
template <typename T>
bool Cube<T>::ok() const {
#ifndef NDEBUG
  if (this->ndim() != 3) throw std::runtime_error("ndim() == " + std::to_string(this->ndim()));
#endif
  return this->ndim() == 3 && Array<T>::ok();
}

template <typename T>
Cube<T>::Cube(const IPosition &shape, T *storage, StorageInitPolicy policy)
    : Array<T>(shape, storage, policy) {
  if (shape.nelements() != 3) throw ArrayError("len.nelements() != 3");
}

template <typename T>
Cube<T>::Cube(const IPosition &shape, const T *storage) : Array<T>(shape, storage) {
  if (shape.nelements() != 3) throw ArrayError("len.nelements() != 3");
}
