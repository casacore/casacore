...
IPosition::IPosition(size_t length) : size_p(length), data_p(buffer_p) {
  if (length > BufferLength) {
    allocateBuffer();
  }
}

IPosition::IPosition(std::initializer_list<ssize_t> list) : size_p(list.size()), data_p(buffer_p) {
...
IPosition::IPosition(const Array<int>& other) : size_p(0), data_p(buffer_p) {
  if (other.size() > 0) {
    if (other.ndim() != 1) {
      throw(
          ArrayError("IPosition::IPosition(const Array<int> &other) - "
...
IPosition::IPosition(size_t length, ssize_t val) : size_p(length), data_p(buffer_p) {
  if (size_p > BufferLength) {
    allocateBuffer();
  }
...
IPosition::IPosition(const IPosition& other) : size_p(other.size_p), data_p(buffer_p) {
  if (size_p > BufferLength) {
    allocateBuffer();
  }
...
IPosition::IPosition(IPosition&& source) noexcept
    : size_p(source.size_p), data_p(size_p > BufferLength ? source.data_p : buffer_p) {
...
IPosition::IPosition(const std::vector<int>& other) : size_p(0), data_p(buffer_p) {
  fill(other.size(), other.begin());
  assert(ok());
}
...
IPosition IPosition::nonDegenerate(const IPosition& ignoreAxes) const {
  ...
  for (i = 0; i < ignoreAxes.nelements(); i++) {
    if (ignoreAxes(i) >= ssize_t(size_p))
      throw std::runtime_error("ignoreAxes(i) >= ssize_t(size_p)");
...
IPosition IPosition::operator()(const IPosition& axes) const {
  ...
  size_t i = 0;
  for (IPosition::const_iterator iter = axes.begin(); iter != axes.end(); ++iter, ++i) {
    if (*iter >= int(size_p)) {
      throw std::runtime_error(
          "IPosition::operator()(const IPosition&): "
...
void IPosition::operator+=(const IPosition& other) {
  ...
  if (!conform(other)) {
    throw(
        ArrayConformanceError("IPosition::operator += "
                              "(const IPosition&) - "
...
bool IPosition::isEqual(const IPosition& other) const { return isEqual(other, nelements()); }

bool IPosition::isEqual(const IPosition& other, bool skipDegeneratedAxes) const {
...
IPosition operator+(const IPosition& left, const IPosition& right) {
  if (!left.conform(right)) {
    throw(
        ArrayConformanceError("::operator + "
...
std::string IPosition::toString() const { return to_string(*this); }
...
bool IPosition::ok() const {
  ...
  bool retval = true;
  if (size_p <= BufferLength && data_p != &buffer_p[0]) {
    retval = false;
  }
  if (data_p == nullptr) {
    retval = false;
...
bool isInsideArray(const IPosition& iposition, const IPosition& shape) {
  if (!(iposition.conform(shape))) {
    throw(
        ArrayConformanceError("bool ::isInsideArray (const IPosition& iposition,"
                              " const IPosition& shape)"
                              " - IPositions do not conform"));
  }
