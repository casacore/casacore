...
#define MARRLOGI_B_ALLFUNC_MA(ALLFUNC, OP, STRALLFUNC)                                   \
  ...
  bool ALLFUNC(const MaskedArray<T> &left, const Array<T> &right) {                      \
    if (left.conform(right) == false) {                                                  \
      throw(ArrayConformanceError("bool ::" STRALLFUNC                                   \
    ...
    right.freeStorage(rightStorage, rightDelete);                                        \
                                                                                         \
    if (!foundOne) {                                                                     \
      throw(ArrayError("bool ::" STRALLFUNC "(const MaskedArray<T> &, const Array<T> &)" \
...
#define MARRLOGI_B_ALLFUNC_AM(ALLFUNC, OP, STRALLFUNC)                                        \
  ...
  bool ALLFUNC(const Array<T> &left, const MaskedArray<T> &right) {                           \
    if (left.conform(right) == false) {                                                       \
      throw(ArrayConformanceError("bool ::" STRALLFUNC "(Array<T> &, const MaskedArray<T> &)" \
                                  " - arrays do not conform"));                               \
    }                                                                                         \
                                                                                              \
    bool leftDelete;                                                                          \
    ...
    right.freeMaskStorage(rightmaskStorage, rightmaskDelete);                                 \
                                                                                              \
    if (!foundOne) {                                                                          \
      throw(ArrayError("bool ::" STRALLFUNC "(const Array<T> &, const MaskedArray<T> &)"      \
                       " - Need at least 1 unmasked element"));                               \
    }                                                                                         \
                                                                                              \
    return retval;                                                                            \
...
#define MARRLOGI_B_ALLFUNC_MM(ALLFUNC, OP, STRALLFUNC)                                \
  ...
  bool ALLFUNC(const MaskedArray<T> &left, const MaskedArray<T> &right) {             \
    if (left.conform(right) == false) {                                               \
      throw(ArrayConformanceError("bool ::" STRALLFUNC "(const MaskedArray<T> &,"     \
                                  " const MaskedArray<T> &)"                          \
                                  " - arrays do not conform"));                       \
    }                                                                                 \
                                                                                      \
    ...
    right.freeMaskStorage(rightmaskStorage, rightmaskDelete);                         \
                                                                                      \
    if (!foundOne) {                                                                  \
      throw(ArrayError("bool ::" STRALLFUNC "(const MaskedArray<T> &,"                \
                       " const MaskedArray<T> &)"                                     \
...
#define MARRLOGI_B_ANYFUNC_MA(ANYFUNC, OP, STRANYFUNC)                                   \
  ...
  bool ANYFUNC(const MaskedArray<T> &left, const Array<T> &right) {                      \
    if (left.conform(right) == false) {                                                  \
      throw(ArrayConformanceError("bool ::" STRANYFUNC                                   \
                                  "(const MaskedArray<T> &, const Array<T> &)"           \
                                  " - arrays do not conform"));                          \
    }                                                                                    \
                                                                                         \
    ...
    right.freeStorage(rightStorage, rightDelete);                                        \
                                                                                         \
    if (!foundOne) {                                                                     \
      throw(ArrayError("bool ::" STRANYFUNC "(const MaskedArray<T> &, const Array<T> &)" \
                       " - Need at least 1 unmasked element"));                          \
    }                                                                                    \
                                                                                         \
    return retval;                                                                       \
...
#define MARRLOGI_B_ANYFUNC_AM(ANYFUNC, OP, STRANYFUNC)                                        \
  ...
  bool ANYFUNC(const Array<T> &left, const MaskedArray<T> &right) {                           \
    if (left.conform(right) == false) {                                                       \
      throw(ArrayConformanceError("bool ::" STRANYFUNC "(Array<T> &, const MaskedArray<T> &)" \
                                  " - arrays do not conform"));                               \
    }                                                                                         \
                                                                                              \
    bool leftDelete;                                                                          \
    ...
    right.freeMaskStorage(rightmaskStorage, rightmaskDelete);                                 \
                                                                                              \
    if (!foundOne) {                                                                          \
      throw(ArrayError("bool ::" STRANYFUNC "(const Array<T> &, const MaskedArray<T> &)"      \
                       " - Need at least 1 unmasked element"));                               \
    }                                                                                         \
                                                                                              \
    return retval;                                                                            \
...
#define MARRLOGI_B_ANYFUNC_MM(ANYFUNC, OP, STRANYFUNC)                                \
  ...
  bool ANYFUNC(const MaskedArray<T> &left, const MaskedArray<T> &right) {             \
    if (left.conform(right) == false) {                                               \
      throw(ArrayConformanceError("bool ::" STRANYFUNC "(const MaskedArray<T> &,"     \
                                  " const MaskedArray<T> &)"                          \
                                  " - arrays do not conform"));                       \
    }                                                                                 \
                                                                                      \
    ...
    right.freeMaskStorage(rightmaskStorage, rightmaskDelete);                         \
                                                                                      \
    if (!foundOne) {                                                                  \
      throw(ArrayError("bool ::" STRANYFUNC "(const MaskedArray<T> &,"                \
                       " const MaskedArray<T> &)"                                     \
                       " - AND of MaskedArray masks must have at least 1 element"));  \
    }                                                                                 \
                                                                                      \
...
#define MARRLOGI_MLA_OP_MA(OP, STROP)                                                    \
  template <class T>                                                                     \
  MaskedLogicalArray operator OP(const MaskedArray<T> &left, const Array<T> &right) {    \
    if (left.conform(right) == false) {                                                  \
      throw(ArrayConformanceError("MaskedLogicalArray ::" STROP                          \
                                  "(const MaskedArray<T> &, const Array<T> &)"           \
                                  " - arrays do not conform"));                          \
    }                                                                                    \
                                                                                         \
    ...
    LogicalArrayElem *resultarrStorage = result.getRWArrayStorage(resultarrDelete);      \
...
#define MARRLOGI_MLA_OP_AM(OP, STROP)                                                    \
  template <class T>                                                                     \
  MaskedLogicalArray operator OP(const Array<T> &left, const MaskedArray<T> &right) {    \
    if (left.conform(right) == false) {                                                  \
      throw(ArrayConformanceError("MaskLogicalArray ::" STROP                            \
...
#define MARRLOGI_MLA_OP_MM(OP, STROP)                                                       \
  template <class T>                                                                        \
  MaskedLogicalArray operator OP(const MaskedArray<T> &left, const MaskedArray<T> &right) { \
    if (left.conform(right) == false) {                                                     \
      throw(ArrayConformanceError("MaskLogicalArray ::" STROP                               \
    ...
    LogicalArray resultarr(left.shape());                                                   \
    resultarr = false;                                                                      \
    MaskedLogicalArray result(resultarr, (left.getMask() && right.getMask()));              \
                                                                                            \
    bool resultarrDelete;                                                                   \
    LogicalArrayElem *resultarrStorage = result.getRWArrayStorage(resultarrDelete);         \
    LogicalArrayElem *resultarrS = resultarrStorage;                                        \
...
#define MARRLOGI_B_ALLFUNC_MS(ALLFUNC, OP, STRALLFUNC)                             \
  ...
  bool ALLFUNC(const MaskedArray<T> &left, const T &right) {                       \
    ...
    left.freeMaskStorage(leftmaskStorage, leftmaskDelete);                         \
                                                                                   \
    if (!foundOne) {                                                               \
      throw(ArrayError("bool ::" STRALLFUNC "(const MaskedArray<T> &, const T)"    \
                       " - Need at least 1 unmasked element"));                    \
    }                                                                              \
                                                                                   \
    return retval;                                                                 \
...
#define MARRLOGI_B_ALLFUNC_SM(ALLFUNC, OP, STRALLFUNC)                                \
  ...
  bool ALLFUNC(const T &left, const MaskedArray<T> &right) {                          \
    ...
    right.freeMaskStorage(rightmaskStorage, rightmaskDelete);                         \
                                                                                      \
    if (!foundOne) {                                                                  \
      throw(ArrayError("bool ::" STRALLFUNC "(const T, const MaskedArray<T> &)"       \
                       " - Need at least 1 unmasked element"));                       \
    }                                                                                 \
                                                                                      \
    return retval;                                                                    \
...
bool allAND(const MaskedArray<T> &marray, const T &val) {
  if (!val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::allAND(const MaskedArray<T> &, const T)"
                     " - Need at least 1 unmasked element"));
    }
...
bool allAND(const T &val, const MaskedArray<T> &marray) {
  if (!val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::allAND(const T, const MaskedArray<T> &)"
                     " - Need at least 1 unmasked element"));
    }
...
bool allOR(const MaskedArray<T> &marray, const T &val) {
  if (val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::allOR(const MaskedArray<T> &, const T)"
                     " - Need at least 1 unmasked element"));
    }
...
bool allOR(const T &val, const MaskedArray<T> &marray) {
  if (val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::allOR(const T, const MaskedArray<T> &)"
                     " - Need at least 1 unmasked element"));
    }
...
#define MARRLOGI_B_ANYFUNC_MS(ANYFUNC, OP, STRANYFUNC)                             \
  ...
  bool ANYFUNC(const MaskedArray<T> &left, const T &right) {                       \
    ...
    left.freeMaskStorage(leftmaskStorage, leftmaskDelete);                         \
                                                                                   \
    if (!foundOne) {                                                               \
      throw(ArrayError("bool ::" STRANYFUNC "(const MaskedArray<T> &, const T)"    \
                       " - Need at least 1 unmasked element"));                    \
    }                                                                              \
                                                                                   \
    return retval;                                                                 \
...
#define MARRLOGI_B_ANYFUNC_SM(ANYFUNC, OP, STRANYFUNC)                                \
  ...
  bool ANYFUNC(const T &left, const MaskedArray<T> &right) {                          \
    ...
    right.freeMaskStorage(rightmaskStorage, rightmaskDelete);                         \
                                                                                      \
    if (!foundOne) {                                                                  \
      throw(ArrayError("bool ::" STRANYFUNC "(const T, const MaskedArray<T> &)"       \
                       " - Need at least 1 unmasked element"));                       \
    }                                                                                 \
                                                                                      \
    return retval;                                                                    \
...
bool anyAND(const MaskedArray<T> &marray, const T &val) {
  if (!val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::anyAND(const MaskedArray<T> &, const T)"
                     " - Need at least 1 unmasked element"));
    }
...
bool anyAND(const T &val, const MaskedArray<T> &marray) {
  if (!val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::anyAND(const T, const MaskedArray<T> &)"
                     " - Need at least 1 unmasked element"));
    }
...
bool anyOR(const MaskedArray<T> &marray, const T &val) {
  if (val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::anyOR(const MaskedArray<T> &, const T)"
                     " - Need at least 1 unmasked element"));
    }
...
bool anyOR(const T &val, const MaskedArray<T> &marray) {
  if (val) {
    ...
    if (!foundOne) {
      throw(
          ArrayError("bool ::anyOR(const T, const MaskedArray<T> &)"
                     " - Need at least 1 unmasked element"));
    }
...
#define MARRLOGI_MLA_OP_MS(OP)                                                           \
  template <class T>                                                                     \
  MaskedLogicalArray operator OP(const MaskedArray<T> &left, const T &right) {           \
    LogicalArray resultarr(left.shape());                                                \
    resultarr = false;                                                                   \
    MaskedLogicalArray result(resultarr, left.getMask());                                \
                                                                                         \
...
#define MARRLOGI_MLA_OP_SM(OP)                                                           \
  template <class T>                                                                     \
  MaskedLogicalArray operator OP(const T &left, const MaskedArray<T> &right) {           \
...
template <class T>
MaskedLogicalArray operator&&(const MaskedArray<T> &marray, const T &val) {
...
template <class T>
MaskedLogicalArray operator&&(const T &val, const MaskedArray<T> &marray) {
  LogicalArray resultarr(marray.shape());
  resultarr = false;
  MaskedLogicalArray result(resultarr, marray.getMask());
