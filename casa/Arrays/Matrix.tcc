...
template <typename T>
Matrix<T>::Matrix() : Array<T>(IPosition(2, 0)) {
  assert(ok());
}

template <typename T>
Matrix<T>::Matrix(const IPosition &length, const T &initialValue) : Array<T>(length, initialValue) {
  Array<T>::checkBeforeResize(length);
}

template <typename T>
Matrix<T>::Matrix(const IPosition &length, typename Array<T>::uninitializedType)
    : Array<T>(length, Array<T>::uninitialized) {
  Array<T>::checkBeforeResize(length);
}

template <typename T>
Matrix<T>::Matrix(size_t l1, size_t l2, const T &initialValue)
    : Array<T>(IPosition(2, l1, l2), initialValue) {
  assert(ok());
}

template <typename T>
Matrix<T>::Matrix(size_t l1, size_t l2, typename Array<T>::uninitializedType)
    : Array<T>(IPosition(2, l1, l2), Array<T>::uninitialized) {
  assert(ok());
}

template <typename T>
Matrix<T>::Matrix(const Matrix<T> &source) : Array<T>(source) {
  assert(ok());
}

template <typename T>
Matrix<T>::Matrix(Matrix<T> &&source) : Array<T>(std::move(source), IPosition(2, 0)) {
  assert(ok());
}
...
// </thrown>
template <typename T>
Matrix<T>::Matrix(const Array<T> &source) : Array<T>(source) {
  this->checkMatrixShape();
  assert(ok());
}

template <typename T>
Matrix<T>::Matrix(Array<T> &&source) : Array<T>(std::move(source)) {
  this->checkMatrixShape();
  assert(ok());
}

template <typename T>
Matrix<T>::Matrix(const IPosition &shape, T *storage, StorageInitPolicy policy)
    : Array<T>(shape, storage, policy) {
  Array<T>::checkBeforeResize(shape);
}

template <typename T>
Matrix<T>::Matrix(const IPosition &shape, const T *storage) : Array<T>(shape, storage) {
  Array<T>::checkBeforeResize(shape);
}

template <typename T>
void Matrix<T>::resize(size_t nx, size_t ny, bool copyValues) {
...
// </thrown>
template <typename T>
Matrix<T> Matrix<T>::operator()(const Slice &sliceX, const Slice &sliceY) {
  assert(ok());
  ...
  if (s1 < 1 || s2 < 1) {
    ...
    throw(
        ArrayError("Matrix<T>::operator()(Slice,Slice): desired slice"
                   " extends beyond the end of the array"));
  } else if (b1 < 0 || b2 < 0) {
    throw(
        ArrayError("Matrix<T>::operator()(Slice,Slice) : start of slice "
...
template <typename T>
const Matrix<T> Matrix<T>::operator()(const Slice &sliceX, const Slice &sliceY) const {
...
// </thrown>
template <typename T>
Vector<T> Matrix<T>::row(size_t n) {
  assert(ok());
...
// </thrown>
template <typename T>
Vector<T> Matrix<T>::column(size_t n) {
  assert(ok());
...
// </thrown>
template <typename T>
Vector<T> Matrix<T>::diagonal(long long n) {
  assert(ok());
  Matrix<T> tmp(*this);
...
template <typename T>
const Vector<T> Matrix<T>::row(size_t n) const {
  assert(ok());
...
template <typename T>
const Vector<T> Matrix<T>::column(size_t n) const {
  assert(ok());
  // Cast away constness of this so we do not have to duplicate code.
  // Because the return type is const we are not actually violating
  // constness.
...
template <typename T>
const Vector<T> Matrix<T>::diagonal(long long n) const {
  assert(ok());
  // Cast away constness of this so we do not have to duplicate code.
  // Because the return type is const we are not actually violating
  // constness.
...
template <typename T>
void Matrix<T>::doNonDegenerate(const Array<T> &other, const IPosition &ignoreAxes) {
  Array<T> tmp(*this);
  tmp.nonDegenerate(other, ignoreAxes);
  if (tmp.ndim() != 2) {
    throw(
        ArrayError("Matrix::nonDegenerate (other, ignoreAxes) - "
...
void Matrix<T>::preTakeStorage(const IPosition &shape) {
  ...
  if (shape.nelements() != 2) throw ArrayError("shape.nelements() != 2");
}
