...
Matrix<double> Rot3D(int axis, double angle) {
  if (axis < 0 || axis > 2)
    throw(
        ArrayError("Rot3D(axis, angle): axis has to be 0 (x),"
                   " 1 (y) or 2 (z)."));
...
std::complex<float> innerProduct(const Vector<std::complex<float>> &A,
                                 const Vector<std::complex<float>> &B) {
  // check for correct dimensions
  if (!(A.conform(B))) throw(ArrayConformanceError("innerProduct - conform() error."));
  std::complex<float> scalar = 0;
  for (size_t i = 0; i < A.nelements(); i++) scalar += A(i) * conj(B(i));
  return scalar;
}

Matrix<std::complex<float>> conjugate(const Matrix<std::complex<float>> &A) { return conj(A); }
...
// the vector dot/scalar/inner product
std::complex<double> innerProduct(const Vector<std::complex<double>> &A,
...
Matrix<std::complex<double>> conjugate(const Matrix<std::complex<double>> &A) { return conj(A); }
...
int norm(const Vector<int> &A) { return int(std::sqrt(double(innerProduct(A, A)))); }

// float Vector magnitude/norm
float norm(const Vector<float> &A) { return std::sqrt(innerProduct(A, A)); }

// double Vector magnitude/norm
double norm(const Vector<double> &A) { return std::sqrt(innerProduct(A, A)); }

// std::complex<float> vector magnitude/norm
float norm(const Vector<std::complex<float>> &A) { return std::sqrt(real(innerProduct(A, A))); }

// std::complex<double> vector magnitude/norm
double norm(const Vector<std::complex<double>> &A) { return std::sqrt(real(innerProduct(A, A))); }
...
Vector<float> rproduct(const Matrix<std::complex<float>> &A, const Vector<std::complex<float>> &x) {
  ...
  for (size_t i = 0; i < A.nrow(); i++) {
    result(i) = 0.0;
    for (size_t k = 0; k < A.ncolumn(); k++) result(i) += real(A(i, k) * x(k));
  }
  return result;
}
...
Matrix<int> adjoint(const Matrix<int> &A) { return transpose(A); }
Matrix<float> adjoint(const Matrix<float> &A) { return transpose(A); }
Matrix<double> adjoint(const Matrix<double> &A) { return transpose(A); }
