...
T innerProduct(const Vector<T> &A, const Vector<T> &B) {
  ...
  T scalar = 0;
  for (size_t i = 0; i < A.nelements(); i++) scalar += A(i) * B(i);
  return scalar;
}
...
Vector<T> crossProduct(const Vector<T> &A, const Vector<T> &B) {
  ...
  if (!A.conform(B)) {
    ...
    if (A.nelements() != 3) throw(ArrayConformanceError("crossProduct - Vector not in 3-space"));
  }
  Vector<T> result(3);
...
T crossProduct2D(const Vector<T> &A, const Vector<T> &B) {
  ...
  if (!A.conform(B)) {
    ...
    if (A.nelements() != 2) throw(ArrayConformanceError("crossProduct2D - Vector not in 2-space"));
  }
