...
Slicer::Slicer(const IPosition& bl, const IPosition& tr, const IPosition& in, LengthOrLast lol)
    : asEnd_p(lol), start_p(bl), end_p(tr), stride_p(in), len_p(tr) {
  fillEndLen();
}

Slicer::Slicer(const IPosition& bl, const IPosition& tr, LengthOrLast lol)
    : asEnd_p(lol), start_p(bl), end_p(tr), stride_p(bl.nelements(), 1), len_p(tr) {
  fillEndLen();
}
...
Slicer::Slicer(const Slice& x, const Slice& y, const Slice& z, LengthOrLast lol)
    : asEnd_p(lol),
...
Slicer::Slicer(const Slice& x, const Slice& y, LengthOrLast lol)
    : asEnd_p(lol),
...
Slicer::Slicer(const Slice& x, LengthOrLast lol)
    : asEnd_p(lol),
      start_p(1, MimicSource),
      end_p(1, MimicSource),
      stride_p(1, 1),
...
bool Slicer::operator==(const Slicer& that) const {
  return this->len_p.isEqual(that.len_p) && this->start_p.isEqual(that.start_p) &&
         this->end_p.isEqual(that.end_p) && this->stride_p.isEqual(that.stride_p) &&
         this->asEnd_p == that.asEnd_p && this->fixed_p == that.fixed_p;
}

void Slicer::fillEndLen() {
  // # First check if all positions have same length.
  if (start_p.nelements() != end_p.nelements() || start_p.nelements() != stride_p.nelements()) {
...
void Slicer::fillSlice(const Slice& sl, ssize_t& start, ssize_t& len, ssize_t& stride) {
...
IPosition Slicer::inferShapeFromSource(const IPosition& shp, IPosition& start, IPosition& end,
                                       IPosition& stride) const {
  // # Check if length of shape conforms the Slicer.
  if (shp.nelements() != start_p.nelements()) {
    throw(ArraySlicerError("Shape IPosition-lengths differ from ndim()"));
  }
...
std::ostream& operator<<(std::ostream& stream, const Slicer& slicer) {
  stream << slicer.start() << " to " << slicer.end() << " with stride " << slicer.stride()
         << ", length " << slicer.length();
