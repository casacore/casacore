...
class Storage {
 public:
  ...
  Storage() : _data(nullptr), _end(nullptr), _isShared(false) {}

  // Construct Storage with a given size.
  // The elements will be default constructed
  Storage(std::size_t n) : _data(construct(n)), _end(_data + n), _isShared(false) {}

  // Construct Storage with a given size.
  // The elements will be copy constructed from the given value
  Storage(std::size_t n, const T& val)
      : _data(construct(n, val)), _end(_data + n), _isShared(false) {}
  ...
  Storage(InputIterator startIter, InputIterator endIter)
      : Storage(startIter, endIter,
                disjunction<std::is_integral<InputIterator>,
                            conjunction<std::is_same<InputIterator, const char*>,
  ...
  static std::unique_ptr<Storage<T>> MakeFromMove(T* startIter, T* endIter) {
    return std::unique_ptr<Storage<T>>(
  ...
  ~Storage() noexcept {
    if (size() && !_isShared) {
      for (size_t i = 0; i != size(); ++i) _data[size() - i - 1].~T();
