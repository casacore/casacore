...
class Vector : public Array<T> {
 public:
  ...
  Vector<T> &operator=(const Vector<T> &source) { return assign_conforming(source); }
  Vector<T> &operator=(Vector<T> &&source) { return assign_conforming(std::move(source)); }
  Vector<T> &operator=(const Array<T> &source) {
    assign_conforming(source);
    return *this;
  }
  Vector<T> &operator=(Array<T> &&source) {
    assign_conforming(std::move(source));
    return *this;
  }
  ...
  void shape(int &Shape) const { Shape = this->length_p(0); }
