...
template <typename T>
Vector<T>::Vector() : Array<T>(IPosition(1, 0)) {
  assert(ok());
}

template <typename T>
Vector<T>::Vector(size_t Length) : Array<T>(IPosition(1, Length)) {
  assert(ok());
}

template <typename T>
Vector<T>::Vector(const IPosition &len) : Array<T>(len) {
  Array<T>::checkBeforeResize(len);
}

template <typename T>
Vector<T>::Vector(size_t length, const T &initialValue)
    : Array<T>(IPosition(1, length), initialValue) {
  assert(ok());
}

template <typename T>
Vector<T>::Vector(const IPosition &length, const T &initialValue) : Array<T>(length, initialValue) {
...
Vector<T>::Vector(InputIterator startIter, InputIterator endIter)
    : Vector<T>(startIter, endIter, std::is_integral<InputIterator>()) {}
...
template <typename T>
Vector<T>::Vector(std::initializer_list<T> list) : Array<T>(list) {
  assert(ok());
}

template <typename T>
Vector<T>::Vector(const Vector<T> &other) : Array<T>(other) {
  assert(ok());
}

template <typename T>
Vector<T>::Vector(Vector<T> &&source) noexcept : Array<T>(std::move(source), IPosition(1, 0)) {
  assert(ok());
}
...
// </thrown>
template <typename T>
Vector<T>::Vector(const Array<T> &other) : Array<T>(other) {
...
template <typename T>
Vector<T>::Vector(const IPosition &shape, T *storage, StorageInitPolicy policy)
    : Array<T>(shape, storage, policy) {}

template <typename T>
Vector<T>::Vector(const IPosition &shape, const T *storage) : Array<T>(shape, storage) {}
...
void Vector<T>::initVector(const std::vector<T> &other, long long nr) {
  ...
  if (n > other.size())
    throw(
        ArrayError("Vector<T>::initVector(const Block<T> &other"
...
// </thrown>
template <typename T>
void Vector<T>::resize(const IPosition &l, bool copyValues) {
  assert(ok());
  if (copyValues) {
    Vector<T> oldref(*this);
    Array<T>::resize(l, false);
    size_t minNels = std::min(this->nelements(), oldref.nelements());
    move_n_with_stride(oldref.begin_p, minNels, this->begin_p, this->inc_p(0), oldref.inc_p(0));
  } else {
    Array<T>::resize(l, false);
  }
  assert(ok());
...
template <typename T>
Vector<T> &Vector<T>::assign_conforming(const Array<T> &a) {
  assert(ok());
  Vector<T> tmp(a);
...
template <typename T>
Vector<T> &Vector<T>::assign_conforming(Array<T> &&a) {
  assert(ok());
...
template <typename T>
Vector<T> &Vector<T>::assign_conforming_implementation(const Vector<T> &,
                                                       std::false_type /*movable?*/) {
  throw std::runtime_error(
      "assign called for which a copy is required, while element type is not copyable");
}

template <typename T>
Vector<T> &Vector<T>::assign_conforming_implementation(const Vector<T> &other,
                                                       std::true_type /*movable?*/) {
  assert(ok());
  if (this != &other) {
    ...
    this->setEndIter();
    copy_n_with_stride(other.begin_p, this->nels_p, this->begin_p, this->inc_p(0), other.inc_p(0));
  }
  return *this;
}
...
Vector<T> &Vector<T>::assign_conforming(Vector<T> &&source) {
  assert(ok());
  if (this->nrefs() > 1 || source.nrefs() > 1 || this->data_p->is_shared() ||
...
Vector<T> Vector<T>::operator()(const Slice &slice) {
  ...
  if (s < 1) {
    ...
    throw(
        ArrayError("Vector<T>::operator()(Slice) : Desired slice extends"
                   " beyond the end of the array"));
  } else if (b < 0) {
    throw(
        ArrayError("Vector<T>::operator()(Slice) : start of slice before "
                   "beginning of vector"));
  }
...
template <typename T>
const Vector<T> Vector<T>::operator()(const Slice &slice) const {
  return const_cast<Vector<T> *>(this)->operator()(slice);
}

template <typename T>
void Vector<T>::doNonDegenerate(const Array<T> &other, const IPosition &ignoreAxes) {
  Array<T> tmp(*this);
