...
class Pool {
 public:
  Pool(size_t poolSize = 1024) : _size(poolSize), _position(0), _nAlloc(0), _nDealloc(0) {
  ...
  ~Pool() noexcept(false) {
    if (_nAlloc != _nDealloc)
      throw std::runtime_error("Memory leak: nalloc=" + std::to_string(_nAlloc) +
                               ", ndealloc=" + std::to_string(_nDealloc));
    delete[] _pool;
  }
  ...
  char* allocate(size_t n) {
    ...
    _position += allocsize;
    if (_position > _size) throw std::runtime_error("Pool is full");
    ++_nAlloc;
  ...
  void deallocate(char* data, size_t n) {
    // Generally, deallocate(nullptr) is not allowed for all allocators, so this function throws
    // when doing so
    auto iter = _chunks.find(reinterpret_cast<char*>(data));
    if (iter == _chunks.end())
      throw std::runtime_error("Can't deallocate chunk at " + std::to_string((size_t)data) +
    ...
    if (iter->second.first != n)
      throw std::runtime_error("Can't deallocate chunk at " + std::to_string((size_t)data) +
                               ": invalid size specified, expected " +
                               std::to_string(iter->second.first) + ", was " + std::to_string(n));
...
class PoolAllocator {
 public:
  ...
  // Rebind constructor
  template <typename U>
  PoolAllocator(const PoolAllocator<U>& source) : _pool(source._pool) {}
  ...
  void deallocate(T* data, size_t n) {
    _pool->deallocate(reinterpret_cast<char*>(data), n * sizeof(T));
  }
...
BOOST_AUTO_TEST_CASE(poolallocator) {
  ...
  BOOST_CHECK_NE(a, c);
  a[0] = 1982;
  for (size_t i = 0; i != 5; ++i) b[i] = 2020 + i;
  c[0] = 37;
  BOOST_CHECK_EQUAL(a[0], 1982);
  for (size_t i = 0; i != 5; ++i) BOOST_CHECK_EQUAL(b[i], 2020 + i);
  ...
  BOOST_CHECK_EQUAL(a[0], 1982);
  BOOST_CHECK_EQUAL(c[0], 37);
  for (size_t i = 0; i != 3; ++i) BOOST_CHECK_EQUAL(d[i], 8 + i);
