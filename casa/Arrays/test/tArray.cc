...
static int zero(int) { return 0; }

static int minusone(const int &) { return -1; }
...
BOOST_AUTO_TEST_CASE(unique) {
  ...
  *a3 = 11.0;
  ...
  Array<float> a4(a3->operator()(IPosition(a3->ndim(), 0), a3->shape() - 1, IPosition(1, 2)));
...
BOOST_AUTO_TEST_CASE(reform) {
  ...
  indgen(ab1);
  Array<float> ab2(ab1(IPosition(4, 1, 2, 1, 3), IPosition(4, 2, 2, 5, 7), IPosition(4, 1, 1, 2, 3))
                       .reform(IPosition(3, 2, 3, 2)));
...
BOOST_AUTO_TEST_CASE(slicer1) {
  Array<float> ab1(IPosition(4, 5, 6, 7, 8));
  indgen(ab1);
  Slicer sl(IPosition(4, 1, 2, 1, 3), IPosition(4, 2, 2, 5, 5), IPosition(4, 1, 1, 2, 3),
            Slicer::endIsLast);
...
BOOST_AUTO_TEST_CASE(slicer2) {
  Array<float> ab1(IPosition(4, 5, 6, 7, 8));
  indgen(ab1);
  Slicer sl(IPosition(4, 1, 2, 1, 3), IPosition(4, 2, 2, Slicer::MimicSource, 7),
...
static Array<int> reformArray() {
  ...
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 4; c++) a0(IPosition(2, r, c)) = r * 10 + c;
  return a0;
}
...
struct LifecycleChecker {
  ...
  LifecycleChecker(LifecycleChecker &&src) : LifecycleChecker(src) {}
  ...
  LifecycleChecker &operator=(LifecycleChecker &&rhs) { return operator=(rhs); }
  static void clear() {
...
BOOST_AUTO_TEST_CASE(array_pos_iterator) {
  ...
  size_t count1 = 0;
  for (ArrayPositionIterator iter1(shape, 1); !iter1.pastEnd(); iter1.next()) ++count1;
  BOOST_CHECK_EQUAL(count1, 3);
  size_t count2 = 0;
  for (ArrayPositionIterator iter2(shape, 0); !iter2.pastEnd(); iter2.next()) ++count2;
...
BOOST_AUTO_TEST_CASE(zero_dimensional_arrays) {
  ...
  ai = 999;
  BOOST_CHECK(ai.ndim() == 0 && ai2.ndim() == 0 && ai.nelements() == 0);
}
