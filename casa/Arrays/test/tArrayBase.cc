...
void checkValues(const ArrayBase& arr, int st, size_t nr) {
  ...
  for (size_t i = 0; i < arri.size(); ++i) {
    ...
    if (arri.data()[i] != int(st + i)) break;  // stop to prevent dozens of warnings
  }
}
...
BOOST_AUTO_TEST_CASE(array_section) {
  ...
  checkValues(*arr2->getSection(Slicer(IPosition(3, 0), IPosition(3, 5, 6, 10))), 0, 300);
  checkValues(*arr2->getSection(Slicer(IPosition(3, 0, 0, 2), IPosition(3, 5, 6, 4))), 60, 120);
}
