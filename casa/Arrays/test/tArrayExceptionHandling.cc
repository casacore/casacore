...
class ExceptionThrower {
 public:
  explicit ExceptionThrower(bool someValue = false) {
    if (someValue || _count == _throwIndex) throw std::exception();
    ++_count;
    _maxCount = std::max(_count, _maxCount);
  }
  ExceptionThrower(const ExceptionThrower&) {
    if (_count == _throwIndex) throw std::exception();
    ++_count;
    _maxCount = std::max(_count, _maxCount);
  }
  ...
  static void Reset(size_t throwIndex) {
    _count = 0;
    _maxCount = 0;
...
BOOST_AUTO_TEST_CASE(default_construct) {
  ...
  BOOST_CHECK_THROW(a.reset(new Array<ExceptionThrower>(IPosition(1, 3))), std::exception);
...
BOOST_AUTO_TEST_CASE(value_construct) {
  ...
  BOOST_CHECK_THROW(a.reset(new Array<ExceptionThrower>(IPosition(1, 3), value)), std::exception);
...
BOOST_AUTO_TEST_CASE(range_construct) {
  ...
  BOOST_CHECK_THROW(a.reset(new Array<ExceptionThrower>(IPosition(1, 3), values[0])),
                    std::exception);
