...
void testSub(Array<int>& arr1, const IPosition& blc, const IPosition& trc, const IPosition& inc) {
...
void checkEmpty(Array<int> earr) {
  ...
  for (Array<int>::iterator itera1 = earr.begin(); itera1 != earr.end(); itera1++) {
    BOOST_CHECK(false);  // should be empty
  }
  for (Array<int>::iterator itera2 = earr.begin(); itera2 != earr.end(); itera2++) {
    BOOST_CHECK(false);  // should be empty
  }
  for (Array<int>::contiter itera3 = earr.cbegin(); itera3 != earr.cend(); itera3++) {
    BOOST_CHECK(false);  // should be empty
  }
  for (Array<int>::contiter itera4 = earr.cbegin(); itera4 != earr.cend(); itera4++) {
    BOOST_CHECK(false);  // should be empty
  }
  BOOST_CHECK(true);
}
