...
BOOST_AUTO_TEST_CASE(math_floor) {
  ...
  for (int i = 0; i < 5; i++) BOOST_CHECK_EQUAL(y(i), 0.0);
}
...
BOOST_AUTO_TEST_CASE(math_pow) {
  ...
  for (int i = 0; i < 5; i++) BOOST_CHECK_CLOSE_FRACTION(z(i), 1.0, 1e-6);
}
...
BOOST_AUTO_TEST_CASE(vector_indgen) {
  ...
  for (int i = 0; i < 5; i++) BOOST_CHECK_EQUAL(x(i), i);
}
...
BOOST_AUTO_TEST_CASE(vector_comparison) {
  ...
  indgen(x);
  indgen(y, 1);
  BOOST_CHECK(anyLE(x, y) && anyLE(x, y - 1) && !anyLE(x, y - 2));
  BOOST_CHECK(anyLT(x, y) && !anyLT(x, y - 1) && !anyLT(x, y - 2));
  BOOST_CHECK(!anyGE(x, y) && anyGE(x, y - 1) && anyGE(x, y - 2));
  BOOST_CHECK(!anyGT(x, y) && !anyGT(x, y - 1) && anyGT(x, y - 2));
  BOOST_CHECK(!anyEQ(x, y) && anyEQ(x, y - 1) && !anyEQ(x, y - 2));
  BOOST_CHECK(anyNE(x, y) && !anyNE(x, y - 1) && anyNE(x, y - 2));
...
BOOST_AUTO_TEST_CASE(median_and_fractile) {
  ...
  BOOST_CHECK_EQUAL(interHexileRange(x4), fractile(x4, 5. / 6.) - fractile(x4, 1. / 6.));
  // Make sure x4 is not changed.
  BOOST_CHECK(allEQ(x4, x5));
}
