...
BOOST_AUTO_TEST_CASE(reorder_array_4d) {
  ...
  BOOST_CHECK_THROW(reorderArray(Array<int>(IPosition(2, 3, 4)), IPosition(2, 1, 1)),
                    std::runtime_error);
  BOOST_CHECK_THROW(reorderArray(Array<int>(IPosition(2, 3, 4)), IPosition(2, 1, 2)),
                    std::runtime_error);
}
...
BOOST_AUTO_TEST_CASE(reverse_array) {
  ...
  for (int i = 0; i < shape[2]; i++) {
    for (int j = 0; j < shape[1]; j++) {
      for (int k = 0; k < shape[0]; k++) {
        BOOST_CHECK_EQUAL(arr(IPosition(3, k, j, i)), rev(IPosition(3, shape[0] - 1 - k, j, i)));
      }
    }
  }
  ...
  for (size_t x = 0; x < 2; x++) {
    ...
    for (int i = 0; i < shape[2]; i++) {
      for (int j = 0; j < shape[1]; j++) {
        for (int k = 0; k < shape[0]; k++) {
          BOOST_CHECK_EQUAL(arr(IPosition(3, k, j, i)), rev(IPosition(3, k, shape[1] - 1 - j, i)));
        }
      }
    }
  }
  ...
  for (size_t x = 0; x < 2; x++) {
    ...
    for (int i = 0; i < shape[2]; i++) {
      for (int j = 0; j < shape[1]; j++) {
        for (int k = 0; k < shape[0]; k++) {
          BOOST_CHECK_EQUAL(arr(IPosition(3, k, j, i)), rev(IPosition(3, k, j, shape[2] - 1 - i)));
        }
      }
    }
  }
      ...
      for (int j = 0; j < shape[1]; j++) {
        for (int k = 0; k < shape[0]; k++) {
          BOOST_CHECK_EQUAL(arr(IPosition(3, k, j, i)),
                            rev(IPosition(3, shape[0] - 1 - k, shape[1] - 1 - j, i)));
        }
      }
    }
  }
  ...
  for (size_t x = 0; x < 2; x++) {
    rev = (x == 0 ? reverseArray(reverseArray(arr, 0), 2) : reverseArray(arr, IPosition(2, 0, 2)));
    for (int i = 0; i < shape[2]; i++) {
      for (int j = 0; j < shape[1]; j++) {
        for (int k = 0; k < shape[0]; k++) {
          BOOST_CHECK_EQUAL(arr(IPosition(3, k, j, i)),
                            rev(IPosition(3, shape[0] - 1 - k, j, shape[2] - 1 - i)));
        }
      }
    }
  }
  ...
  for (size_t x = 0; x < 2; x++) {
    rev = (x == 0 ? reverseArray(reverseArray(arr, 1), 2) : reverseArray(arr, IPosition(2, 1, 2)));
    for (int i = 0; i < shape[2]; i++) {
      for (int j = 0; j < shape[1]; j++) {
        for (int k = 0; k < shape[0]; k++) {
          BOOST_CHECK_EQUAL(arr(IPosition(3, k, j, i)),
                            rev(IPosition(3, k, shape[1] - 1 - j, shape[2] - 1 - i)));
        }
      }
    }
  }
      ...
      for (int j = 0; j < shape[1]; j++) {
        for (int k = 0; k < shape[0]; k++) {
          BOOST_CHECK_EQUAL(
              ...
              rev(IPosition(3, shape[0] - 1 - k, shape[1] - 1 - j, shape[2] - 1 - i)));
        }
      }
    }
  }
