...
void checkLarge(const IPosition& ip) {
  ...
  for (size_t i = 0; i != 6; ++i) BOOST_CHECK_EQUAL(ip[i], i + 2);
}
...
BOOST_AUTO_TEST_CASE(array_move_assignment) {
  ...
  BOOST_CHECK_THROW(
      c = std::move(b),
      std::runtime_error);  // TODO it is questionable whether we would want this to throw!
...
BOOST_AUTO_TEST_CASE(array_can_hold_noncopyable_objects) {
  ...
  // a = Array<std::unique_ptr<int>>(IPosition{2,3}, nullptr); // TODO this could be made possible
  // but isn't yet
...
BOOST_AUTO_TEST_CASE(masked_array_move_constructor) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(ma2.getArray().begin(), ma2.getArray().end(), arrReference.begin(),
                                arrReference.end());

  std::vector<bool> maskReference{false, false, true, false, false, false};
  BOOST_CHECK_EQUAL_COLLECTIONS(ma2.getMask().begin(), ma2.getMask().end(), maskReference.begin(),
                                maskReference.end());
}
...
BOOST_AUTO_TEST_CASE(masked_array_move_assignment) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(ma2.getArray().begin(), ma2.getArray().end(),
                                ma3.getArray().begin(), ma3.getArray().end());
  BOOST_CHECK_EQUAL_COLLECTIONS(ma2.getMask().begin(), ma2.getMask().end(), ma3.getMask().begin(),
                                ma3.getMask().end());
}
...
BOOST_AUTO_TEST_CASE(pointer_vector) {
  ...
  // Vector<ptr>(3, 0) was allowed, but is no longer, because 0 is not implicitly convertable to a
  // pointer Hence now we need this:
  Vector<Array<int>*> ptrArray(3, nullptr);
  for (auto iter = ptrArray.begin(); iter != ptrArray.end(); ++iter) BOOST_CHECK(*iter == nullptr);
}
