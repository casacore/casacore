...
BOOST_AUTO_TEST_CASE(initialize) {
  ...
  for (int k = 0; k <= 2; k++)
    for (int j = 0; j <= 2; j++)
      for (int i = 0; i <= 2; i++) BOOST_CHECK(c(i, j, k) == 3);

  for (int k = 0; k <= 2; k++) BOOST_CHECK(allEQ(c.xyPlane(k), 3));
}
...
BOOST_AUTO_TEST_CASE(slice1) {
  ...
  // slice
  BOOST_CHECK(allEQ(c3(Slice(0, 2), Slice(1, 2), 1), c(Slice(0, 2), Slice(1, 2), 1)));
}
