...
void fullCompare(const Array<int>& darr, const Array<int>& arr, const std::vector<int>& values) {
  ...
  std::vector<int> arrDiagValues{arr(IPosition(4, 0, 0, 1, 0)), arr(IPosition(4, 1, 0, 1, 0)),
                                 arr(IPosition(4, 0, 1, 2, 0)), arr(IPosition(4, 1, 1, 2, 0)),
                                 arr(IPosition(4, 0, 0, 1, 1)), arr(IPosition(4, 1, 0, 1, 1)),
                                 arr(IPosition(4, 0, 1, 2, 1)), arr(IPosition(4, 1, 1, 2, 1))};
  getDiagRes.assign(getDiag(arr, 1, 1));
  BOOST_CHECK_EQUAL_COLLECTIONS(getDiagRes.begin(), getDiagRes.end(), arrDiagValues.begin(),
                                arrDiagValues.end());
  BOOST_CHECK_EQUAL_COLLECTIONS(getDiagRes.begin(), getDiagRes.end(), valIter, valIter + 8);
  valIter += 8;

  arrDiagValues = std::vector<int>{arr(IPosition(4, 0, 1, 0, 0)), arr(IPosition(4, 1, 1, 0, 0)),
                                   arr(IPosition(4, 0, 2, 1, 0)), arr(IPosition(4, 1, 2, 1, 0)),
                                   arr(IPosition(4, 0, 1, 0, 1)), arr(IPosition(4, 1, 1, 0, 1)),
                                   arr(IPosition(4, 0, 2, 1, 1)), arr(IPosition(4, 1, 2, 1, 1))};
  getDiagRes.assign(getDiag(arr, 1, -1));
  BOOST_CHECK_EQUAL_COLLECTIONS(getDiagRes.begin(), getDiagRes.end(), arrDiagValues.begin(),
                                arrDiagValues.end());
  BOOST_CHECK_EQUAL_COLLECTIONS(getDiagRes.begin(), getDiagRes.end(), valIter, valIter + 8);
  ...
  arrDiagValues = std::vector<int>{arr(IPosition(4, 0, 0, 2, 0)), arr(IPosition(4, 1, 0, 2, 0)),
                                   arr(IPosition(4, 0, 0, 2, 1)), arr(IPosition(4, 1, 0, 2, 1))};
  ...
  arrDiagValues = std::vector<int>{arr(IPosition(4, 0, 2, 0, 0)), arr(IPosition(4, 1, 2, 0, 0)),
                                   arr(IPosition(4, 0, 2, 0, 1)), arr(IPosition(4, 1, 2, 0, 1))};
  getDiagRes.assign(getDiag(arr, 1, -2));
  BOOST_CHECK_EQUAL_COLLECTIONS(getDiagRes.begin(), getDiagRes.end(), arrDiagValues.begin(),
                                arrDiagValues.end());
  BOOST_CHECK_EQUAL_COLLECTIONS(getDiagRes.begin(), getDiagRes.end(), valIter, valIter + 4);
...
BOOST_AUTO_TEST_CASE(full) {
  ...
  fullCompare(darr, arr, std::vector<int>{0,  1,  8,  9,  16, 17, 18, 19, 26, 27, 34, 35,
                                          6,  7,  14, 15, 24, 25, 32, 33, 2,  3,  10, 11,
                                          20, 21, 28, 29, 12, 13, 30, 31, 4,  5,  22, 23});
...
BOOST_AUTO_TEST_CASE(subset) {
  ...
  indgen(farr);
  Array<int> arr(
      farr(IPosition(4, 1, 1, 1, 3), IPosition(4, 4, 3, 7, 5), IPosition(4, 2, 1, 3, 2)));
  ...
  fullCompare(darr, arr, std::vector<int>{1339, 1341, 1453, 1455, 1567, 1569, 2203, 2205, 2317,
                                          2319, 2431, 2433, 1447, 1449, 1561, 1563, 2311, 2313,
                                          2425, 2427, 1345, 1347, 1459, 1461, 2209, 2211, 2323,
                                          2325, 1555, 1557, 2419, 2421, 1351, 1353, 2215, 2217});
  checkDiagonals(arr, 1);
  // Now test if for a subset where all axes have the same length, so
  // we can get the diagonals for all axes.
  Array<int> arr2(
      farr(IPosition(4, 1, 0, 2, 3), IPosition(4, 5, 4, 10, 7), IPosition(4, 1, 1, 2, 1)));
  indgen(arr2);
  for (int ax = 0; ax < 3; ++ax) {
    checkDiagonals(arr2, ax);
  }
...
BOOST_AUTO_TEST_CASE(diag_subset) {
  ...
  for (int ax1 = 0; ax1 < 4; ++ax1) {
    ...
    Array<int> arr4(
        arr3(IPosition(4, 1, 1, 1, 1), IPosition(4, 3, 3, 3, 3), IPosition(4, 2, 2, 2, 2)));
    for (int ax = 0; ax < 3; ++ax) {
