...
BOOST_AUTO_TEST_CASE(construct_from_int_array) {
  ...
  for (int i = 0; i != 7; ++i) BOOST_CHECK(bigPos[i] == (9 - i));
}

BOOST_AUTO_TEST_CASE(construct_from_std_vector) {
  std::vector<int> emptyVec;
  IPosition emptyPos(emptyVec);
  BOOST_CHECK(emptyPos.nelements() == 0);

  std::vector<int> filledVec{19, 82};
  IPosition filledPos(filledVec);
  BOOST_CHECK(filledPos.nelements() == 2);
  BOOST_CHECK(filledPos[0] == 19);
  BOOST_CHECK(filledPos[1] == 82);

  std::vector<long long> bigVec{9, 8, 7, 6, 5, 4, 3};
  IPosition bigPos(bigVec);
  BOOST_CHECK(bigPos.nelements() == 7);
  for (int i = 0; i != 7; ++i) BOOST_CHECK(bigPos[i] == (9 - i));
}
...
BOOST_AUTO_TEST_CASE(make) {
  ...
  for (int i = 0; i != 7; ++i) BOOST_CHECK_EQUAL(l[i], 9 - i);
}
...
BOOST_AUTO_TEST_CASE(move_exhaustively) {
  ...
  BOOST_CHECK(IPosition(6, 2, 1, 1, 3, 1, 4).isEqual(IPosition(3, 2, 3, 4), true));
  BOOST_CHECK(IPosition(6, 2, 1, 1, 3, 1, 4).isEqual(IPosition(5, 1, 1, 2, 3, 4), true));
  BOOST_CHECK(IPosition(6, 2, 1, 1, 3, 1, 4).isEqual(IPosition(5, 2, 3, 4, 1, 1), true));
  BOOST_CHECK(IPosition(3, 2, 3, 4).isEqual(IPosition(5, 2, 1, 3, 1, 4), true));

  BOOST_CHECK(IPosition(3, 2, 3, 4).nonDegenerate().isEqual(IPosition(3, 2, 3, 4)));
  BOOST_CHECK(IPosition(4, 1, 2, 3, 4).nonDegenerate().isEqual(IPosition(3, 2, 3, 4)));
  BOOST_CHECK(IPosition(5, 1, 1, 2, 3, 4).nonDegenerate().isEqual(IPosition(3, 2, 3, 4)));
  BOOST_CHECK(IPosition(6, 1, 1, 2, 3, 1, 4).nonDegenerate().isEqual(IPosition(3, 2, 3, 4)));
  BOOST_CHECK(IPosition(6, 1, 1, 2, 3, 1, 4).nonDegenerate(2).isEqual(IPosition(5, 1, 1, 2, 3, 4)));
}
...
BOOST_AUTO_TEST_CASE(as_vector) {
  ...
  vi.resize(6);
  vi = ip2.asVector();
  BOOST_CHECK(vi(0) == 1 && vi(1) == 2 && vi(2) == 3 && vi(3) == 4 && vi(4) == 5 && vi(5) == 6);
  IPosition ip3(vi);
  BOOST_CHECK(ip3(0) == 1 && ip3(1) == 2 && ip3(2) == 3 && ip3(3) == 4 && ip3(4) == 5 &&
              ip3(5) == 6);
