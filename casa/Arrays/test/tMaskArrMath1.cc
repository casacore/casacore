...
struct Fixture {
  ...
  Vector<bool> b;
  Fixture() : df(10), dg(10), dh(10), b(10) {
    indgen(df);
    indgen(dg, 2.0);
    dh = 2.0;
  }
...
BOOST_FIXTURE_TEST_CASE(atan2_func3, Fixture) {
  dh.assign_conforming(atan2(dg((df > 3.5) && (df < 7.5)), df((df > 2.5) && (df < 6.5))));
...
BOOST_FIXTURE_TEST_CASE(pow_func3, Fixture) {
  dh.assign_conforming(pow(dg((df > 3.5) && (df < 7.5)), df((df > 2.5) && (df < 6.5))));
...
BOOST_FIXTURE_TEST_CASE(median_even2_func, Fixture) {
  ...
  dfunsort(5) = tmp;
  double result(-1.0);
  result = median(dfunsort((dfunsort > 2.5) && (dfunsort < 6.5)));
...
BOOST_FIXTURE_TEST_CASE(median_odd2_func, Fixture) {
  ...
  dfunsort(5) = tmp;
  double result(-1.0);
  result = median(dfunsort((dfunsort > 2.5) && (dfunsort < 7.5)));
...
BOOST_AUTO_TEST_CASE(complex_variance_func) {
  ...
  vecb[2] = vecb[5] = true;
  BOOST_CHECK(arrays_internal::near(pvariance(vecc(vecb)),
                                    (pvariance(real(vecc(vecb))) + pvariance(imag(vecc(vecb))))));
}
