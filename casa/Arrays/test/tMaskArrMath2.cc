...
struct Fixture {
  ...
  Fixture() : df(10), dh(10), b(10), dk({0., 1., 5., 3., 9., 2., 7., 7., 4., 3.}) { indgen(df); }
};

BOOST_FIXTURE_TEST_CASE(fixture, Fixture) { check(dk, {0., 1., 5., 3., 9., 2., 7., 7., 4., 3.}); }

BOOST_FIXTURE_TEST_CASE(min_ma, Fixture) {
  BOOST_CHECK_CLOSE_FRACTION(min(dk((dk > 2.5) && (dk < 7.5))), 3, 1e-6);
}

BOOST_FIXTURE_TEST_CASE(max_ma, Fixture) {
  BOOST_CHECK_CLOSE_FRACTION(max(dk((dk > 2.5) && (dk < 7.5))), 7, 1e-6);
}
...
BOOST_FIXTURE_TEST_CASE(minmax_ma1, Fixture) {
  ...
  minMax(minVal, maxVal, minPos, maxPos, dk((dk > 2.5) && (dk < 7.5)));
...
BOOST_FIXTURE_TEST_CASE(minmax_ma2, Fixture) {
  ...
  minMax(minVal, maxVal, dk((dk > 2.5) && (dk < 7.5)));
...
BOOST_FIXTURE_TEST_CASE(min_ma_2pars_c, Fixture) {
  ...
  dh = 2.0;
  dh.assign_conforming(min(dk((dk > 2.5) && (dk < 7.5)), df((df > 2.5) && (df < 7.5))));
  check(dh, {2., 2., 2., 3., 2., 2., 6., 7., 2., 2.});
}

BOOST_FIXTURE_TEST_CASE(max_ma_2pars_c, Fixture) {
  dh = 2.0;
  dh.assign_conforming(max(dk((dk > 2.5) && (dk < 7.5)), df((df > 2.5) && (df < 7.5))));
