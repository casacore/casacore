...
BOOST_AUTO_TEST_CASE(vector_init) {
  Vector<int> x(10);
  x = 5;
  for (int i = 0; i < 10; i++) BOOST_CHECK_EQUAL(x(i), 5);
}
...
BOOST_AUTO_TEST_CASE(copy_constructor) {
  ...
  for (int i = 0; i < 10; i++) BOOST_CHECK_EQUAL(y(i), 5);
}
...
BOOST_AUTO_TEST_CASE(tostdvector) {
  ...
  for (size_t i = 0; i != 10; ++i) BOOST_CHECK_EQUAL(stdvec[i], -13);
}
...
BOOST_AUTO_TEST_CASE(vector_various) {
  ...
  Array<int> arr3(arr(IPosition(4, 1, 2, 3, 4), IPosition(4, 19, 18, 20, 22), IPosition(4, 2)));
  IPosition shp3 = arr3.shape();
  arr2.reference(
      arr3(IPosition(4, 1, 3, 1, 2), IPosition(4, 1, 3, shp3(2) - 1, 2), IPosition(4, 1, 1, 2, 1)));
