...
inline void transformInPlace(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
                             BinaryOperator op) {
...
inline void transformInPlace(InputIterator1 first1, InputIterator1 last1, UnaryOperator op) {
...
inline Accum accumulateTrue(InputIterator first, InputIterator last, MaskIterator mask, Accum acc,
...
// The default accumulation is addition.
template <typename InputIterator, typename MaskIterator, typename Accum, typename BinaryOperator>
inline Accum accumulateFalse(InputIterator first, InputIterator last, MaskIterator mask, Accum acc,
                             BinaryOperator op = std::plus<Accum>()) {
  for (; first != last; ++first, ++mask) {
    if (!*mask) acc = op(acc, *first);
  }
...
inline bool compareAll(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
                       CompareOperator op) {
  for (; first1 != last1; ++first1, ++first2) {
...
inline bool compareAllLeft(InputIterator1 first1, InputIterator1 last1, T left,
                           CompareOperator op) {
  for (; first1 != last1; ++first1) {
...
// (see ArrayMath.h).
template <typename InputIterator1, typename T, typename CompareOperator>
inline bool compareAllRight(InputIterator1 first1, InputIterator1 last1, T right,
                            CompareOperator op) {
  for (; first1 != last1; ++first1) {
...
// <group>
template <typename InputIterator1, typename InputIterator2, typename CompareOperator>
inline bool compareAny(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
                       CompareOperator op) {
  for (; first1 != last1; ++first1, ++first2) {
...
// (see ArrayMath.h).
template <typename InputIterator1, typename T, typename CompareOperator>
inline bool compareAnyLeft(InputIterator1 first1, InputIterator1 last1, T left,
                           CompareOperator op) {
  for (; first1 != last1; ++first1) {
...
// (see ArrayMath.h).
template <typename InputIterator1, typename T, typename CompareOperator>
inline bool compareAnyRight(InputIterator1 first1, InputIterator1 last1, T right,
                            CompareOperator op) {
  for (; first1 != last1; ++first1) {
...
struct Plus {
  RES operator()(const L& x, const R& y) const { return RES(x) + y; }
};
...
template <typename L, typename R = L, typename RES = L>
struct FloorMod {
  RES operator()(const L& x, const R& y) const { return floormod(RES(x), RES(y)); }
};
...
template <typename T>
struct IsNaN {
  bool operator()(T value) const { return isNaN(value); }
};

// Functor to test for infinity.
template <typename T>
struct IsInf {
  bool operator()(T value) const { return isInf(value); }
};

// Functor to test for finiteness.
template <typename T>
struct IsFinite {
  bool operator()(T value) const { return isFinite(value); }
};
...
struct Near {
  explicit Near(double tolerance = 1e-5) : itsTolerance(tolerance) {}
  bool operator()(L left, R right) const { return near(left, L(right), itsTolerance); }
...
template <typename L, typename R = L>
struct NearAbs {
  explicit NearAbs(double tolerance = 1e-13) : itsTolerance(tolerance) {}
  bool operator()(L left, R right) const { return nearAbs(left, L(right), itsTolerance); }
...
struct Sin {
  RES operator()(T value) const { return RES(sin(value)); }
};

// Functor to apply sinh.
template <typename T, typename RES = T>
struct Sinh {
  RES operator()(T value) const { return RES(sinh(value)); }
};

// Functor to apply asin.
template <typename T, typename RES = T>
struct Asin {
  RES operator()(T value) const { return RES(asin(value)); }
};

// Functor to apply cos.
template <typename T, typename RES = T>
struct Cos {
  RES operator()(T value) const { return RES(cos(value)); }
};

// Functor to apply cosh.
template <typename T, typename RES = T>
struct Cosh {
  RES operator()(T value) const { return RES(cosh(value)); }
};

// Functor to apply acos.
template <typename T, typename RES = T>
struct Acos {
  RES operator()(T value) const { return RES(acos(value)); }
};

// Functor to apply tan.
template <typename T, typename RES = T>
struct Tan {
  RES operator()(T value) const { return RES(tan(value)); }
};

// Functor to apply tanh.
template <typename T, typename RES = T>
struct Tanh {
  RES operator()(T value) const { return RES(tanh(value)); }
};

// Functor to apply atan.
template <typename T, typename RES = T>
struct Atan {
  RES operator()(T value) const { return RES(atan(value)); }
};

// Functor to apply atan2.
template <typename L, typename R = L, typename RES = L>
struct Atan2 {
  RES operator()(L left, R right) const { return RES(atan2(left, L(right))); }
};

// Functor to apply sqr (power of 2).
template <typename T, typename RES = T>
struct Sqr {
  RES operator()(T value) const { return RES(value * value); }
};

// Functor to apply a power of 3.
template <typename T, typename RES = T>
struct Pow3 {
  RES operator()(T value) const { return RES(value * value * value); }
};

// Functor to apply sqrt.
template <typename T, typename RES = T>
struct Sqrt {
  RES operator()(T value) const { return RES(sqrt(value)); }
};

// Functor to apply exp.
template <typename T, typename RES = T>
struct Exp {
  RES operator()(T value) const { return RES(exp(value)); }
};

// Functor to apply log.
template <typename T, typename RES = T>
struct Log {
  RES operator()(T value) const { return RES(log(value)); }
};

// Functor to apply log10.
template <typename T, typename RES = T>
struct Log10 {
  RES operator()(T value) const { return RES(log10(value)); }
};

// Functor to apply abs.
template <typename T, typename RES = T>
struct Abs {
  RES operator()(T value) const { return RES(abs(value)); }
};

// Functor to apply floor.
template <typename T, typename RES = T>
struct Floor {
  RES operator()(T value) const { return RES(floor(value)); }
};

// Functor to apply ceil.
template <typename T, typename RES = T>
struct Ceil {
  RES operator()(T value) const { return RES(ceil(value)); }
};

// Functor to apply round (e.g. -3.7 gets -4).
template <typename T, typename RES = T>
struct Round {
  RES operator()(T value) const { return RES(value < 0 ? ceil(value - 0.5) : floor(value + 0.5)); }
};

// Functor to apply sign (result is -1, 0, or 1).
template <typename T, typename RES = T>
struct Sign {
  RES operator()(T value) const { return (value < 0 ? -1 : (value > 0 ? 1 : 0)); }
};

// Functor to form a complex number from the left and right value.
template <typename L, typename R, typename RES>
struct MakeComplex {
  RES operator()(L l, R r) const { return RES(l, r); }
};
...
struct MakeComplexReal {
  RES operator()(L l, R r) const { return RES(real(l), r); }
};
...
struct MakeComplexImag {
  RES operator()(L l, R r) const { return RES(l, imag(r)); }
};
...
struct MakeComplexRealImag {
  RES operator()(L l, R r) const { return RES(real(l), imag(r)); }
};

// Functor to apply complex function conj.
template <typename T, typename RES = T>
struct Conj {
  RES operator()(T value) const { return RES(conj(value)); }
};

// Functor to apply complex function real.
template <typename T, typename RES>
struct Real {
  RES operator()(T value) const { return RES(real(value)); }
};

// Functor to apply complex function imag.
template <typename T, typename RES>
struct Imag {
  RES operator()(T value) const { return RES(imag(value)); }
};

// Functor to apply complex function arg.
template <typename T, typename RES>
struct CArg {
  RES operator()(T value) const { return RES(arg(value)); }
};

// Functor to apply complex function fabs.
template <typename T, typename RES>
struct CAbs {
  RES operator()(T value) const { return RES(fabs(value)); }
};

// Functor to apply pow.
template <typename T, typename E = T, typename RES = T>
struct Pow {
  RES operator()(T left, E exponent) const { return RES(pow(left, exponent)); }
};

// Functor to apply fmod.
template <typename L, typename R = L, typename RES = L>
struct Fmod {
  RES operator()(R left, L right) const { return RES(fmod(left, L(right))); }
};

// Functor to get minimum of two values.
template <typename L, typename R = L, typename RES = L>
struct Min {
  RES operator()(L left, R right) const { return RES(left < right ? left : right); }
};

// Functor to get maximum of two values.
template <typename L, typename R = L, typename RES = L>
struct Max {
  RES operator()(L left, R right) const { return RES(left < right ? right : left); }
};

// Functor to add square of right to left.
template <typename T, typename Accum = T>
struct SumSqr {
  Accum operator()(Accum left, T right) const { return left + Accum(right) * Accum(right); }
};
...
struct SumAbsDiff {
  ...
  Accum operator()(Accum left, T right) const { return left + abs((right - itsBase)); }
...
struct Downcase {
  String operator()(const std::string& value) const { return downcase(value); }
};

// Functor to upcase a std::string. The result is a casacore::String.
struct Upcase {
  String operator()(const std::string& value) const { return upcase(value); }
};

// Functor to capitalize a std::string. The result is a casacore::String.
struct Capitalize {
  String operator()(const std::string& value) const { return capitalize(value); }
};

// Functor to trim a std::string. The result is a casacore::String.
// Leading and trailing whitespace is removed.
struct Trim {
  String operator()(const std::string& value) const { return trim(value); }
};
