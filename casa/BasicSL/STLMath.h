...
inline void checkContainerSizes(const CONTAINER& left, const CONTAINER& right, const char* name) {
...
std::vector<T> operator/(const std::vector<T>& left, const T& right) {
  std::vector<T> result(left.size());
  std::transform(left.begin(), left.end(), result.begin(), [right](T x) { return x / right; });
  return result;
}
