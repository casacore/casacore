...
void String::trim() { TrimInPlace(*this); }

void String::trim(char c[], unsigned n) { TrimInPlace(*this, std::string_view(c, n)); }

void String::ltrim(char c) { LTrimInPlace(*this, c); }

void String::rtrim(char c) { RTrimInPlace(*this, c); }

// Obtain a (separate) 'sub'-string
SubString String::at(size_type pos, size_type len) { return _substr(pos, len); }
...
SubString String::from(size_type pos) { return _substr(pos, length() - pos); }
...
SubString String::after(size_type pos) { return _substr(pos + 1, length() - (pos + 1)); }
...
// Member utilities
void String::reverse() { std::reverse(begin(), end()); }
...
void String::upcase() { std::transform(begin(), end(), begin(), ToUpper); }

void String::downcase() { std::transform(begin(), end(), begin(), ToLower); }

void String::capitalize() { CapitalizeStringInPlace(*this); }
...
bool String::matches(const std::string &str, int pos) const {
  ...
  if (pos < 0) {
    ...
    if (length() != 0 && str.length() != 0 && length() == pos + str.length() &&
...
String common_prefix(const std::string &x, const std::string &y, int startpos) {
  if (static_cast<String::size_type>(startpos) == String::npos ||
      ...
      static_cast<String::size_type>(startpos) >= y.length())
    return String();
