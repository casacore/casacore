...
// startpos < 0. Note that a startpos of -1 indicates searching at the second to last character.
// This is in line with the old index() function, but rather confusing, so do not use for new code.
inline std::string::size_type IndexString(std::string_view str, std::string_view pattern,
                                          int startpos = 0) {
  if (startpos >= 0) {
...
// Replacement of old member function "matches", which has somewhat confusing semantics: two empty
// strings do not match, and an empty pattern does also not match. Do not use this for new code and
// try to rewrite old code.
inline bool EqualStringsAndNotEmpty(std::string_view str1, std::string_view str2,
                                    int position = 0) {
  if (str1.empty() || str2.empty()) {
    return false;
  } else if (position < 0) {
    return std::string_view(str1.begin(),
                            str1.begin() + std::min<size_t>(-position, str1.size())) == str2;
  } else {
    return std::string_view(str1.begin() + std::min<size_t>(position, str1.size()), str1.end()) ==
           str2;
  }
}
  ...
  if (is.fail() || !is.eof()) {
    if (throw_on_error) {
      if (is.fail()) {
        ...
        throw std::runtime_error("Extra characters after parsing string '" + str +
                                 "' in StringToValue(): '" + extra + "'");
      }
    }
    return false;
  }
...
class SubString {
 ...
 private:
  ...
  SubString(const std::string &str, std::string::size_type pos, std::string::size_type len);
  // # Data
  //  Referenced string
...
class String : public std::string {
 public:
  ...
  String &operator=(const SubString &str) { return (*this = String(str)); }
  String &operator=(const char *s) { return static_cast<String &>(std::string::operator=(s)); }
  String &operator=(char c) { return static_cast<String &>(std::string::operator=(c)); }
  // </group>
  ...
  // <group>
  DEPRECATED("Use at()")
  const_reference elem(size_type pos) const { return std::string::at(pos); }
  ...
  std::string &append(char c) { return std::string::append(1, c); }
  using std::string::append;

  // ** Casacore addition
  DEPRECATED("Use assign(1, c)")
  String &assign(char c) { return static_cast<String &>(std::string::assign(1, c)); }
  ...
  DEPRECATED(
      "Use std::format if possible, or FormatString() if the format string can't be changed (e.g. "
      "runtime determined)")
  ...
  bool startsWith(const std::string &beginString) const { return starts_with(beginString); }
  ...
  bool contains(char c, int pos) const { return IndexString(*this, c, pos) != npos; }
  ...
  bool contains(const char *s, int pos) const { return (IndexString(*this, s, pos) != npos); }
  DEPRECATED("Use std::regex_search(str.substr(pos), regex);")
  bool contains(const Regex &r, int pos) const { return (index(r, pos) != npos); }
  // </group>
  ...
  // BUG: When pos < 0, it does not behave as advertised; e.g. "xyxy" does not match with "x" when
  // pos=-1 is specified. <group name=matches>
  DEPRECATED(
      "Use substr(pos) == str if possible, consider ends_with() / starts_with() for "
      "positive/negative pos or use EqualStringsAndNotEmpty() if direct replacement is necessary")
  ...
  DEPRECATED(
      "Use substr(pos) == str if possible, consider ends_with() / starts_with() for "
      "positive/negative pos or use EqualStringsAndNotEmpty() if direct replacement is necessary")
  bool matches(const char *s, int pos = 0) const { return EqualStringsAndNotEmpty(*this, s, pos); }
  DEPRECATED("Use RegexMatches()")
  bool matches(const Regex &r, int pos = 0) const { return RegexMatches(*this, r, pos); }
  // </group>
  ...
  size_type index(char c, int startpos = 0) const {
    return ((startpos >= 0) ? find(c, startpos) : rfind(c, length() + startpos - 1));
  }
  DEPRECATED("Use find() or rfind(), or IndexString() if the search direction is runtime-dependent")
  size_type index(const std::string &str, int startpos = 0) const {
    return ((startpos >= 0) ? find(str, startpos) : rfind(str, length() + startpos - str.length()));
  }
  DEPRECATED("Use find() or rfind(), or IndexString() if the search direction is runtime-dependent")
  ...
  DEPRECATED("Use substr()")
  String at(size_type pos, size_type len) const { return String(*this, pos, len); }
  ...
  DEPRECATED("Use substr()")
  SubString at(int pos, size_type len) { return at(static_cast<size_type>(pos), len); };
  DEPRECATED("Use substr()")
  String at(int pos, size_type len) const { return at(static_cast<size_type>(pos), len); };
  using std::string::at;
  // </group>
  ...
  SubString before(int pos) { return before(static_cast<size_type>(pos)); };
  // </group>
  ...
  // Next one for overloading reasons
  DEPRECATED("Use substr(0, pos+1)")
  SubString through(int pos) { return through(static_cast<size_type>(pos)); }
  // </group>
  ...
  // Next one for overloading reasons
  DEPRECATED("Use substr(pos)")
  SubString from(int pos) { return from(static_cast<size_type>(pos)); };
  // </group>
  ...
  // Next one for overloading reasons
  DEPRECATED("Use substr(pos + 1)")
  SubString after(int pos) { return after(static_cast<size_type>(pos)); };
  // </group>
  ...
  // Overload problem
  DEPRECATED("Use erase()")
  void del(int pos, int len) { del(static_cast<size_type>(pos), static_cast<size_type>(len)); }
  //</group>
  ...
  int gsub(const Regex &pat, const std::string &repl) { return RegexReplaceAll(*this, pat, repl); }
  //</group>

 private:
  // Helper functions for at, before etc
  // <group>
  SubString _substr(size_type first, size_type l) const { return SubString(*this, first, l); }
  // </group>
...
// <group name=split>
int split(const std::string &str, std::string res[], int maxn, const std::string &sep);
int split(const std::string &str, std::string res[], int maxn, const char sep);
int split(const std::string &str, std::string res[], int maxn, const Regex &sep);
//</group>

// <summary> Some general functions </summary>
// Functions to find special patterns, join and replicate
// <group name=common>
String common_prefix(const std::string &x, const std::string &y, int startpos = 0);
String common_suffix(const std::string &x, const std::string &y, int startpos = -1);
...
inline SubString::SubString(const std::string &str, std::string::size_type pos,
    ...
    : ref_p(str),
      pos_p((pos > str.length()) ? str.length() : pos),
      len_p((len == std::string::npos || pos_p + len > str.length()) ? str.length() - pos_p : len) {
}

inline SubString String::operator()(size_type pos, size_type len) { return at(pos, len); }
inline const char *SubString::chars() const { return String(*this).c_str(); }
...
inline std::string_view GetStringUpToExcluding(std::string_view input, std::string_view pattern,
...
// Rather specific function -- this is a replacement for String:through() with string argument, but
// maybe String::through() is rarely used, in which case this function should be removed.
inline std::string_view GetStringUpToIncluding(std::string_view input, std::string_view pattern,
                                               size_t start_position = 0) {
  assert(start_position <= input.size());
...
// Get a string starting at a specific pattern that is found in the string. The @p start_position
// can be used to start the search at a later position.
inline std::string_view GetStringFrom(std::string_view input, std::string_view pattern,
                                      size_t start_position = 0) {
  assert(start_position <= input.size());
  std::size_t start = input.find(pattern, start_position);
  if (start == std::string_view::npos) start = input.size();
  // input.subview() is only available from C++26, so construct manually:
  return std::string_view(input.begin() + start, input.end());
}

// Rather specific function -- this is a replacement for String:after(), but maybe String::after()
// is rarely used, in which case this function should be removed.
inline std::string_view GetStringAfter(std::string_view input, std::string_view pattern,
                                       size_t start_position = 0) {
  assert(start_position <= input.size());
  std::size_t start = input.find(pattern, start_position);
  ...
  while (p != str.end()) {
    ...
    if (std::islower(static_cast<unsigned char>(*p))) {
      *p = std::toupper(static_cast<unsigned char>(*p));
      ...
      at_word = std::isupper(static_cast<unsigned char>(*p)) ||
...
inline void EraseStringFrom(std::string &str, std::string_view pattern, size_t start_position = 0) {
  ...
  if (start != std::string::npos) str.erase(start, pattern.length());
}

// Replace every occurence of @c pattern in @c str by @c replacement.
inline size_t ReplaceAllInPlace(std::string &str, std::string_view pattern,
