...
#define TESTOP(NAME, Z, FAILVAR)                                              \
  if (!near(casacore::NAME(Z), std::NAME(Z), 1e-5)) {                         \
    cout << "for z=" << Z << ", casacore::" #NAME "(z)=" << casacore::NAME(Z) \
         << ", std::" #NAME "(z)=" << std::NAME(Z) << '\n';                   \
    FAILVAR = true;                                                           \
  }
...
int main() {
  ...
  cout << "pow(" << f1 << "," << i1 << " = " << pow(f1, Complex(i1.real(), i1.imag())) << endl;
  ...
  c1 = Complex(0.0, c1.imag());
  c2 = DComplex(c2.real(), 0.0);
  AlwaysAssertExit(isNaN(c1) && isNaN(c2));
  c1 = Complex(0.0);
  c2 = DComplex(0.0);
  AlwaysAssertExit((!isNaN(c1)) && (!isNaN(c2)));
