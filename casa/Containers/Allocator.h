...
struct ArrayInitPolicies {
  // Don't initialize elements in the array. (The array will be explicitly filled with values other
...
struct new_del_allocator : public std11_allocator<T> {
  ...
  new_del_allocator(const new_del_allocator &other) noexcept : Super(other) {}
  ...
  void deallocate(pointer ptr, size_type) { delete[] ptr; }
...
template <typename T>
inline bool operator==(const new_del_allocator<T> &, const new_del_allocator<T> &) {
  return true;
}
...
class Allocator_private {
  ...
  struct BulkAllocatorImpl : public BulkAllocator<typename Allocator::value_type> {
    ...
    virtual std::type_info const &allocator_typeid() const override { return typeid(Allocator); }
  ...
  static BulkAllocatorImpl<Allocator> *get_allocator_raw() {
    // Because this function gets called from destructors of statically allocated objects that get
    // destructed after the program finishes, the allocator is constructed in a static storage space
    // and is never destructed.
    alignas(BulkAllocatorImpl<Allocator>) static std::byte
        storage[sizeof(BulkAllocatorImpl<Allocator>)];
    static BulkAllocatorImpl<Allocator> *ptr =
        new (reinterpret_cast<BulkAllocatorImpl<Allocator> *>(&storage))
            BulkAllocatorImpl<Allocator>();
    return ptr;
  }
...
// Because it is impossible to decouple construction/destruction from allocation/deallocation with
// this allocator, it is discouraged to use this allocator. Use <src>DefaultAllocator<T></src> or
// <src>AlignedAllocator<T, ALIGNMENT></src> as possible. This allocator is provided only for
// compatibility for calling <src>Array::takeStorage(), Block::replaceStorage(), Block(size_t, T *&,
// Bool)</src>  etc. with a storage allocated by operator new[].
template <typename T>
