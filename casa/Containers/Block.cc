...
void BlockTrace::doTraceAlloc(const void* addr, size_t nelem, DataType type, size_t sz) {
  traceMemoryAlloc(addr, nelem * sz, "Block " << type << ' ' << sz);
}

void BlockTrace::doTraceFree(const void* addr, size_t nelem, DataType type, size_t sz) {
