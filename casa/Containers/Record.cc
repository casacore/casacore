...
Record::Record(const RecordInterface& other)
  ...
  for (uInt i = 0; i < n; i++) {
    ...
    if (dtype == TpRecord) {
      ...
      defineRecord(i, Record(subrec), subrec.isFixed() ? Fixed : Variable);
    } else {
...
RecordInterface* Record::clone() const { return new Record(*this); }
...
void Record::defineRecord(const RecordFieldId& id, const RecordInterface& value, RecordType type) {
...
void Record::putRecord(AipsIO& os) const { ref().putRecord(os, recordType()); }
