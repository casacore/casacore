// # RecordInterface.cc: Abstract base class for a hierarchical collection of named fields of
// various types # Copyright (C) 1996,1998,1999,2000,2001 # Associated Universities, Inc. Washington
// DC, USA.
// #
...
RecordInterface::RecordInterface(RecordType type, CheckFieldFunction* funcPtr,
    ...
    : checkFunction_p(funcPtr), checkArgument_p(checkArgument), type_p(type) {}
...
DComplex RecordInterface::asDComplex(const RecordFieldId& id) const {
  ...
  switch (dataType) {
    ...
    case TpDouble:
      return *(const double*)get_pointer(whichField, TpDouble);
    case TpComplex: {
      const Complex* val = (const Complex*)get_pointer(whichField, TpComplex);
