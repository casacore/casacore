...
// Tweaked for SUN NTV compiler.  Added the const_cast<void*> to give a hint to the Solaris
// compiler.
...
void RecordRep::addDataField(const String& name, DataType type, const IPosition& shape,
  ...
  AlwaysAssert(type == TpBool || type == TpArrayBool || type == TpUChar || type == TpArrayUChar ||
                   ...
                   type == TpArrayInt64 || type == TpFloat || type == TpArrayFloat ||
                   type == TpDouble || type == TpArrayDouble || type == TpComplex ||
                   type == TpArrayComplex || type == TpDComplex || type == TpArrayDComplex ||
...
void RecordRep::addField(const String& name, const Record& rec, RecordInterface::RecordType type) {
...
void RecordRep::checkShape(DataType type, const IPosition& shape, const void* value,
  ...
  if (!shape.isEqual(arrShape)) {
    throw(ArrayConformanceError("Record::define - fixed array conformance error for field " +
                                fieldName));
  }
}
...
void RecordRep::defineDataField(Int whichField, DataType type, const void* value) {
  ...
  if (type == descDtype) {
    if (type == TpRecord) {
      *static_cast<Record*>(data_p[whichField]) = *static_cast<const Record*>(value);
    } else {
    ...
    throw(
        AipsError("RecordRep::defineDataField - "
...
void* RecordRep::createDataField(DataType type, const IPosition& shape) {
  ...
  switch (type) {
    ...
    default:
      throw(
          AipsError("RecordRep::createDataField: unknown data type " + std::to_string(int(type))));
  }
}
...
void RecordRep::makeDataVec(Int whichField, DataType type) {
  ...
  switch (type) {
    ...
    case TpFloat:
      datavec_p[whichField] =
          new Array<float>(shape, static_cast<float*>(data_p[whichField]), SHARE);
      break;
    case TpDouble:
      datavec_p[whichField] =
          new Array<double>(shape, static_cast<double*>(data_p[whichField]), SHARE);
      break;
    case TpComplex:
      datavec_p[whichField] =
          new Array<Complex>(shape, static_cast<Complex*>(data_p[whichField]), SHARE);
      break;
    case TpDComplex:
      datavec_p[whichField] =
          new Array<DComplex>(shape, static_cast<DComplex*>(data_p[whichField]), SHARE);
      break;
    case TpString:
      datavec_p[whichField] =
          new Array<String>(shape, static_cast<String*>(data_p[whichField]), SHARE);
      break;
    default:
...
void RecordRep::copyDataField(DataType type, void* ptr, const void* that) const {
  switch (type) {
    ...
    case TpArrayFloat:
      ...
      *static_cast<Array<float>*>(ptr) = *static_cast<const Array<float>*>(that);
      break;
    case TpArrayDouble:
      static_cast<Array<double>*>(ptr)->resize(static_cast<const Array<double>*>(that)->shape());
      *static_cast<Array<double>*>(ptr) = *static_cast<const Array<double>*>(that);
      break;
    case TpArrayComplex:
      static_cast<Array<Complex>*>(ptr)->resize(static_cast<const Array<Complex>*>(that)->shape());
      *static_cast<Array<Complex>*>(ptr) = *static_cast<const Array<Complex>*>(that);
      break;
    case TpArrayDComplex:
      static_cast<Array<DComplex>*>(ptr)->resize(
          static_cast<const Array<DComplex>*>(that)->shape());
      *static_cast<Array<DComplex>*>(ptr) = *static_cast<const Array<DComplex>*>(that);
      break;
    case TpArrayString:
      static_cast<Array<String>*>(ptr)->resize(static_cast<const Array<String>*>(that)->shape());
      *static_cast<Array<String>*>(ptr) = *static_cast<const Array<String>*>(that);
      break;
    default:
...
void* RecordRep::get_pointer(Int whichField, DataType type) const {
  ...
  if (!(isArray(type) && asScalar(type) == descDtype)) {
    throw(
        AipsError("RecordRep::get_pointer - "
                  "incorrect data type " +
...
void RecordRep::printDataField(std::ostream& os, DataType type, const String& indent,
  ...
  switch (type) {
    ...
    case TpArrayDouble:
      ...
      break;
    case TpArrayComplex:
      os << "Complex array with shape " << static_cast<const Array<Complex>*>(ptr)->shape();
      if (maxNrValues != 0) {
      ...
      break;
    case TpArrayDComplex:
      os << "DComplex array with shape " << static_cast<const Array<DComplex>*>(ptr)->shape();
      if (maxNrValues != 0) {
      ...
      break;
    case TpArrayString:
      os << "String array with shape " << static_cast<const Array<String>*>(ptr)->shape();
      if (maxNrValues != 0) {
...
void RecordRep::print(std::ostream& os, Int maxNrValues, const String& indent) const {
  for (uInt i = 0; i < nused_p; i++) {
    ...
    if (desc_p.type(i) == TpRecord) {
      os << '{' << endl;
      static_cast<const Record*>(data_p[i])->print(os, maxNrValues, indent + "  ");
      os << indent << '}' << endl;
    } else {
      printDataField(os, desc_p.type(i), indent, maxNrValues, data_p[i]);
      os << endl;
    }
  }
}
...
void RecordRep::putDataField(AipsIO& os, DataType type, const void* ptr) const {
  switch (type) {
    ...
    case TpArrayFloat:
      ...
      break;
    case TpArrayDouble:
      putArray(os, *static_cast<const Array<double>*>(ptr), "Array<double>");
      break;
    case TpArrayComplex:
      putArray(os, *static_cast<const Array<Complex>*>(ptr), "Array<void>");
      break;
    case TpArrayDComplex:
      putArray(os, *static_cast<const Array<DComplex>*>(ptr), "Array<void>");
      break;
    case TpArrayString:
      putArray(os, *static_cast<const Array<String>*>(ptr), "Array<String>");
      break;
    default:
