...
void doDefineAssign(const Record& inrecord) {
  ...
  AlwaysAssertExit(allEQ(vec, stringToVector("abcd,ghi,jklmn")));
  record.get(record.fieldNumber("TpArrayString3"), vec);
  AlwaysAssertExit(allEQ(vec, stringToVector("abc,dghij,klmn")));
  try {
  ...
  vec.resize(1);
  record.get(record.fieldNumber("TpArrayString3"), vec);
  AlwaysAssertExit(allEQ(vec, stringToVector("a")));

  *rfstr2 = stringToVector("a,b,c");
  *rfstr3 = stringToVector("d");
  record.get(record.fieldNumber("TpArrayString3"), vec);
  AlwaysAssertExit(allEQ(vec, stringToVector("d")));
  vec.resize(3);
  record.get(record.fieldNumber("TpArrayString2"), vec);
  AlwaysAssertExit(allEQ(vec, stringToVector("a,b,c")));

  rfstr2.define(stringToVector("g,h,i"));
  record.define(record.fieldNumber("TpArrayString3"), stringToVector("j,k,l"));
  record.get(record.fieldNumber("TpArrayString2"), vec);
  AlwaysAssertExit(allEQ(vec, stringToVector("g,h,i")));
  record.get(record.fieldNumber("TpArrayString3"), vec);
...
void doIt(Bool doExcp) {
  ...
  AlwaysAssertExit(record.nfields() == rd.nfields() && record.nfields() == 24);
  ...
  if (doExcp) {
    ...
    try {
      ...
      cout << x.what() << endl;  // invalid field type
    }
  }
  AlwaysAssertExit(record.nfields() == rd.nfields() && record.nfields() == 24);
  AlwaysAssertExit(record.description() == rd);
  ...
  record4 = record3;
  AlwaysAssertExit(rd == record2.description() && rd == record3.description() &&
  ...
  *arraystringField = stringToVector("Hello,Goodbye");
  ...
  if (doExcp) {
    try {
      ...
      cout << x.what() << endl;  // fixed -> add not possible
    }
  }
  ...
  *fldm = -1234555;
  ...
  if (doExcp) {
    ...
    try {
      record.mergeField(record5, "TpBool", RecordInterface::ThrowOnDuplicates);
    } catch (std::exception& x) {
    ...
    try {
      record2a.mergeField(record5, "TpBool", RecordInterface::ThrowOnDuplicates);
    } catch (std::exception& x) {
      cout << x.what() << endl;  // fixed structure
    }
  }
...
void check(const Record& record, Int intValue, uInt nrField) {
  ...
  AlwaysAssertExit(allEQ(record.toArrayDComplex(19), DComplex(*complexField)));
  AlwaysAssertExit(allEQ(record.toArrayDComplex(20), *dcomplexField));
  AlwaysAssertExit(
      allEQ(record.toArrayComplex(20), Complex((*dcomplexField).real(), (*dcomplexField).imag())));
  ...
  AlwaysAssertExit(allEQ(*arraystringField, stringToVector("Hello,Goodbye")));
