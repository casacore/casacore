...
void doPos(T v, U, DataType dt) {
  ...
  AlwaysAssertExit(veccomplex.size() == 1 && veccomplex.data()[0] == Complex(float(v), 0));
  Vector<DComplex> vecdcomplex = vh.asArrayDComplex();
  AlwaysAssertExit(vecdcomplex.size() == 1 && vecdcomplex.data()[0] == DComplex(double(v), 0));
  Record rec;
  vh.toRecord(rec, "a");
  ValueHolder vhc;
  AlwaysAssertExit(vhc.isNull());
