...
String AipsError::getStackTrace() const { return stackTrace; }
void AipsError::addStackTrace() {}
void AipsError::getLastInfo(String&, String&) {}
String AipsError::getLastMessage() { return String(); }
String AipsError::getLastStackTrace() { return String(); }
...
String AipsError::getLastMessage() { return lastMessage; }
...
void AipsError::throwIfError(int errorCode, const String& prefix, const char* file, Int line,
  ...
  if (errorCode != 0) {
    AipsError e(FormatString("Exception: %s.\n...Thrown by %s\n...(errno=%d): %s", prefix.c_str(),
                             func, errorCode, strerror(errorCode)),
                file, line);
    throw e;
  }
}
...
String strerror_overload(int err) { return "errno " + std::to_string(err); }
String strerror_overload(char* err) { return err; }
...
AbortError::AbortError(const String& str, Category c) : AipsError(str, c) {
  cerr << "An unrecoverable error occurred: " << endl;
  cerr << str << endl;
#ifndef CASACORE_NOEXIT
  exit(1);
