...
HDF5DataSet::HDF5DataSet(const HDF5Object& parentHid, const String& name, const IPosition& shape,
                         const IPosition& tileShape, const Complex* type)
    : itsDataType(type) {
  create(parentHid, name, shape, tileShape);
}

HDF5DataSet::HDF5DataSet(const HDF5Object& parentHid, const String& name, const IPosition& shape,
                         const IPosition& tileShape, const DComplex* type)
    : itsDataType(type) {
  create(parentHid, name, shape, tileShape);
}

HDF5DataSet::HDF5DataSet(const HDF5Object& parentHid, const String& name, const IPosition& shape,
                         const IPosition& tileShape, const HDF5DataType& type)
    : itsDataType(type) {
  create(parentHid, name, shape, tileShape);
}
...
HDF5DataSet::HDF5DataSet(const HDF5Object& parentHid, const String& name, const Complex* type)
    : itsDataType(type) {
  open(parentHid, name);
}

HDF5DataSet::HDF5DataSet(const HDF5Object& parentHid, const String& name, const DComplex* type)
    : itsDataType(type) {
  open(parentHid, name);
}

HDF5DataSet::HDF5DataSet(const HDF5Object& parentHid, const String& name, const HDF5DataType& type)
    : itsDataType(type) {
  open(parentHid, name);
}

HDF5DataSet::~HDF5DataSet() { close(); }

#ifdef HAVE_HDF5

void HDF5DataSet::create(const HDF5Object& parentHid, const String& name, const IPosition& shape,
  ...
  // Create the data set.
  setHid(H5Dcreate2(parentHid, name.c_str(), itsDataType.getHidFile(), itsDSid, 0, itsPLid, 0));
  if (!isValid()) {
...
void HDF5DataSet::get(const Slicer& section, void* buf) {
  ...
  if (H5Sselect_hyperslab(itsDSid, H5S_SELECT_SET, offset.storage(), stride.storage(),
                          count.storage(), NULL) < 0) {
    throw HDF5Error("invalid array get selection for dataset " + getName());
  }
  // Define a data space for the memory buffer.
  HDF5HidDataSpace memspace(H5Screate_simple(count.size(), count.storage(), NULL));
  // Define memory selection.
  offset = 0;
  if (H5Sselect_hyperslab(memspace, H5S_SELECT_SET, offset.storage(), NULL, count.storage(), NULL) <
      0) {
    throw HDF5Error("setting slab of memory buffer for dataset " + getName());
  }
  // Read the data.
  if (H5Dread(getHid(), itsDataType.getHidMem(), memspace, itsDSid, H5P_DEFAULT, buf) < 0) {
...
void HDF5DataSet::put(const Slicer& section, const void* buf) {
  ...
  // Write the data.
  if (H5Dwrite(getHid(), itsDataType.getHidMem(), memspace, itsDSid, H5P_DEFAULT, buf) < 0) {
...
void HDF5DataSet::create(const HDF5Object&, const String&, const IPosition&, const IPosition&) {
  HDF5Object::throwNoHDF5();
}

void HDF5DataSet::open(const HDF5Object&, const String&) { HDF5Object::throwNoHDF5(); }
