...
HDF5DataType::HDF5DataType(const String*) : itsSize(0) {
  itsHidMem = H5Tcopy(H5T_C_S1);
...
Bool HDF5DataType::isEmptyArray(hid_t dtid) {
  ...
  if (H5Tget_class(dtid) == H5T_COMPOUND && H5Tget_nmembers(dtid) == 3) {
    ...
    res = (strcmp(f0, "emptyarray") == 0 && strcmp(f1, "rank") == 0 && strcmp(f2, "casatype") == 0);
    H5free_memory(f0);
    H5free_memory(f1);
    H5free_memory(f2);
  }
...
HDF5DataType::HDF5DataType(const DComplex*) { HDF5Object::throwNoHDF5(); }
...
HDF5DataType::HDF5DataType(const HDF5DataType&, const IPosition&) { HDF5Object::throwNoHDF5(); }
