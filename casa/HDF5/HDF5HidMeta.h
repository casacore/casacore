...
class HDF5HidProperty {
 public:
  ...
  HDF5HidProperty() : itsHid(-1) {}
  // Construct from given hid.
  HDF5HidProperty(hid_t hid) : itsHid(hid) {}
  // The destructor closes the hid.
  ~HDF5HidProperty() { close(); }
  ...
  // Get the hid.
  hid_t getHid() const { return itsHid; }
  // Convert automatically to hid_t.
  operator hid_t() const { return itsHid; }
...
class HDF5HidDataType {
 public:
  // Default constructor sets hid to invalid.
  HDF5HidDataType() : itsHid(-1) {}
  // Construct from given hid.
  HDF5HidDataType(hid_t hid) : itsHid(hid) {}
  // Copy constructor makes a deep copy.
  HDF5HidDataType(const HDF5HidDataType& that);
  // The destructor closes the hid.
  ~HDF5HidDataType() { close(); }
...
class HDF5HidDataSpace {
 public:
  // Default constructor sets hid to invalid.
  HDF5HidDataSpace() : itsHid(-1) {}
  // Construct from given hid.
  HDF5HidDataSpace(hid_t hid) : itsHid(hid) {}
  // The destructor closes the hid.
  ~HDF5HidDataSpace() { close(); }
  // Copy constructor cannot be used.
...
class HDF5HidAttribute {
 public:
  // Default constructor sets hid to invalid.
  HDF5HidAttribute() : itsHid(-1) {}
  // Construct from given hid.
  HDF5HidAttribute(hid_t hid) : itsHid(hid) {}
  // The destructor closes the hid.
  ~HDF5HidAttribute() { close(); }
  // Copy constructor cannot be used.
