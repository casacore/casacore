...
herr_t readSubRecord(hid_t groupHid, const char* name, const H5L_info_t*, void* voidRec) {
...
void HDF5Record::remove(const HDF5Object& parentHid, const String& recordName) {
  HDF5Group::remove(parentHid, recordName);
}

Record HDF5Record::readRecord(const HDF5Object& parentHid, const String& recordName) {
  try {
...
Record HDF5Record::doReadRecord(hid_t groupHid) {
  ...
  hsize_t idx = 0;
  H5Literate(groupHid, H5_INDEX_NAME, H5_ITER_NATIVE, &idx, &readSubRecord, &rec);
  return rec;
}
...
void HDF5Record::read(hid_t attrId, void* value, const HDF5DataType& dtype) {
...
void HDF5Record::readScaString(hid_t attrId, Int sz, const String& name, RecordInterface& rec) {
  ...
  if (sz > 0) {
    ...
    AlwaysAssert(H5Aread(attrId, dtype.getHidMem(), const_cast<char*>(value.c_str())) >= 0,
                 AipsError);
    if (value == "__empty__") {
      value = String();
    }
...
void HDF5Record::readArrString(hid_t attrId, const IPosition& shape, const String& name,
                               RecordInterface& rec) {
  ...
  for (std::vector<char*>::const_iterator viter = ptrs.begin(); viter != ptrs.end();
       ++aiter, ++viter) {
    *aiter = String(*viter);
    ::free(*viter);
  }
...
void HDF5Record::writeRecord(const HDF5Object& parentHid, const String& recordName,
...
void HDF5Record::writeScalar(hid_t groupHid, const String& name, const void* value,
...
void HDF5Record::writeScaString(hid_t groupHid, const String& name, const String& value) {
...
void HDF5Record::writeArray(hid_t groupHid, const String& name, const void* value,
                            const IPosition& shape, const HDF5DataType& dtype) {
  int rank = shape.nelements();
  if (shape.product() == 0) {
    writeEmptyArray(groupHid, name, rank, HDF5DataType::getDataType(dtype.getHidMem()));
    return;
  }
...
void HDF5Record::writeArrString(hid_t groupHid, const String& name, const Array<String>& value) {
  ...
  for (std::vector<char*>::iterator viter = ptrs.begin(); viter != ptrs.end(); ++aiter, ++viter) {
  ...
  HDF5HidAttribute id(
      H5Acreate2(groupHid, name.c_str(), dtype.getHidFile(), dsid, H5P_DEFAULT, H5P_DEFAULT));
  AlwaysAssert(id.getHid() >= 0, AipsError);
  AlwaysAssert(H5Awrite(id, dtype.getHidMem(), &(ptrs[0])) >= 0, AipsError);
}

#else

void HDF5Record::remove(const HDF5Object&, const String&) { HDF5Object::throwNoHDF5(); }

Record HDF5Record::readRecord(const HDF5Object&, const String&) {
...
void HDF5Record::readScalar(hid_t, hid_t, const String&, RecordInterface&) {
  HDF5Object::throwNoHDF5();
}

void HDF5Record::readArray(hid_t, hid_t, const IPosition&, const String&, RecordInterface&) {
  HDF5Object::throwNoHDF5();
}

void HDF5Record::readEmptyArray(hid_t, const String&, RecordInterface&) {
  HDF5Object::throwNoHDF5();
}
...
void HDF5Record::readArrString(hid_t, const IPosition&, const String&, RecordInterface&) {
  HDF5Object::throwNoHDF5();
}

void HDF5Record::writeRecord(const HDF5Object&, const String&, const RecordInterface&) {
  HDF5Object::throwNoHDF5();
}

void HDF5Record::doWriteRecord(const HDF5Object&, const RecordInterface&) {
  HDF5Object::throwNoHDF5();
}

void HDF5Record::writeScalar(hid_t, const String&, const void*, const HDF5DataType&) {
  HDF5Object::throwNoHDF5();
}

void HDF5Record::writeScaString(hid_t, const String&, const String&) { HDF5Object::throwNoHDF5(); }

void HDF5Record::writeArray(hid_t, const String&, const void*, const IPosition&,
                            const HDF5DataType&) {
  HDF5Object::throwNoHDF5();
}
