...
void createFile() {
  ...
  // Create the file.
  fileId = H5Fcreate("tHDF5_tmp.dat", H5F_ACC_TRUNC, create_plist, access_plist);
...
void createDataSet(int nx, int ny, int nz, int ntx, int nty, int ntz) {
  ...
  // Create the data set.
  setId = H5Dcreate2(fileId, "dataset", typeIdFile, dsId, 0, plId, 0);
...
void setCacheSize(int nchunks, int nbytes) {
  ...
  cout << "setting cache to " << nchunks << " chunks (" << nbytes << " bytes) with " << nhash
       << " slots" << endl;
  // Setting the cache size takes only effect when opening the dataset.
  // So close it first.
  H5Dclose(setId);
  // Use LRU caching (4th argument is 0).
  check(H5Pset_chunk_cache(daplId, nhash, nbytes, 0.), "Failed to set chunk cache");
...
void get(int stx, int sty, int stz, int nx, int ny, int nz, void* buf) {
  ...
  count[2] = nx;
  stride[2] = 1;
  check(H5Sselect_hyperslab(dsId, H5S_SELECT_SET, offset, stride, count, NULL),
  ...
  offset[1] = 0;
  offset[2] = 0;
  check(H5Sselect_hyperslab(dsmemid, H5S_SELECT_SET, offset, NULL, count, NULL),
        "Failed to set slab of memory buffer");
  // Read the data.
  check(H5Dread(setId, typeIdMem, dsmemid, dsId, H5P_DEFAULT, buf),
...
void put(int stx, int sty, int stz, int nx, int ny, int nz, const void* buf) {
  ...
  check(H5Sselect_hyperslab(dsmemid, H5S_SELECT_SET, offset, NULL, count, NULL),
        "Failed to set slab of memory buffer");
  // Write the data.
  check(H5Dwrite(setId, typeIdMem, dsmemid, dsId, H5P_DEFAULT, buf),
...
void readback(int nx, int ny, int nz, int ntx, int nty, int ntz, char type, bool checkv) {
  openFile();
  openDataSet();
  if (type == 'x') {
...
int main(int argc, char* argv[]) {
  ...
  if (argc <= 7) {
    cout << "Creating tHDF5_tmp.dat with shape [" << nx << ',' << ny << ',' << nz
         << "] and chunk shape [" << ntx << ',' << nty << ',' << ntz << ']' << endl;
