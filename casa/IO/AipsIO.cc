...
const String& AipsIO::getNextType() {
  if (opened_p == 0 || swget_p < 0 || swput_p > 0) {
    ...
    throw(AipsError("AipsIO::getNextType: " + message + "not opened or not readable"));
  }
  if (hasCachedType_p) {
    return objectType_p;
  }
  ...
  if (level_p == 0) {
    ...
    if (mval != magicval_p) {
      String message;
      if (file_p) message = file_p->fileName() + " - ";
      throw(AipsError("AipsIO::getNextType: " + message + "no magic value found"));
    }
  }
  level_p++;
  if (level_p >= maxlev_p) {
...
void AipsIO::testputerr() { throw(AipsError("AipsIO: no putstart done")); }
void AipsIO::testgeterr() { throw(AipsError("AipsIO: no getstart done")); }
void AipsIO::testgeterrLength() { throw(AipsError("AipsIO: read beyond end of object")); }
