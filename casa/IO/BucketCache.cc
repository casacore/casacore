...
char* BucketCache::getBucket(uInt bucketNr) {
  ...
  if (bucketNr < its_CurNrOfBuckets) {
    ...
    if (!its_file->isWritable()) {
      throw AipsError("BucketCache::getBucket: bucket " + std::to_string(bucketNr) +
...
void BucketCache::showStatistics(ostream& os) const {
  os << "cacheSize: " << its_CacheSize << " (*" << its_BucketSize << ")" << endl;
  ...
  if (naccess_p > 0) {
    os << "        hit-rate:  " << 100 * float(naccess_p - nread_p - ninit_p) / float(naccess_p)
       << "%";
  }
  cout << endl;
}
