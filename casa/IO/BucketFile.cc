...
void BucketFile::open() {
  if (!file_p) {
    if (mfile_p) {
      file_p.reset(new MFFileIO(mfile_p, name_p, isWritable_p ? ByteIO::Update : ByteIO::Old));
    } else {
...
void BucketFile::fsync() { file_p->fsync(); }
...
uInt BucketFile::read(void* buffer, uInt length) { return file_p->read(length, buffer); }
