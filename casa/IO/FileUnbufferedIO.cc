...
FileUnbufferedIO::FileUnbufferedIO(const RegularFile& fileName, ByteIO::OpenOption option,
                                   Bool useODirect)
    : itsUseODirect(useODirect) {
  attach(RegularFileIO::openCreate(fileName, option, useODirect), fileName.path().originalName());
}
...
void FileUnbufferedIO::reopenRW() {
  ...
  int newfd = RegularFileIO::openCreate(fileName(), ByteIO::Update, itsUseODirect);
