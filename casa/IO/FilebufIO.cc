...
String FilebufIO::fileName() const { return ""; }
...
void FilebufIO::writeBuffer(Int64 offset, const char* buf, Int64 size) {
  if (size > 0) {
    ...
    if (::traceWRITE(itsFile, const_cast<char*>(buf), size) != size) {
      int error = errno;
      itsSeekOffset = -1;
      throw AipsError(String("FilebufIO: write error for file ") + fileName() + ": " +
                      strerror(error));
    }
    itsSeekOffset += size;
  }
  ...
  if (st < end) {
    ...
    if (stoff <= itsBufOffset && endoff >= itsBufOffset + itsBufSize) {
      if (stoff < itsBufOffset) {
        ...
        AlwaysAssert(readBuffer(stoff, bufp, sz, throwException) == sz, AipsError);
        bufp += sz;
      }
...
Int64 FilebufIO::readBlock(Int64 size, char* buf, Bool throwException) {
  ...
  if (st + size > itsBufLen) {
    if (throwException) {
      throw AipsError(
          "FilebufIO::readBlock - incorrect number of bytes"
          " read for file " +
          fileName());
    }
    if (itsBufLen > st) {
