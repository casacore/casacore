...
Int64 FiledesIO::read(Int64 size, void* buf, Bool throwException) {
  ...
  if (bytesRead != size && throwException == True) {
    if (bytesRead < 0) {
      ...
      throw AipsError("FiledesIO::read - incorrect number of bytes (" + std::to_string(bytesRead) +
...
Int64 FiledesIO::pread(Int64 size, Int64 offset, void* buf, Bool throwException) {
  ...
  if (bytesRead != size && throwException == True) {
    if (bytesRead < 0) {
      ...
      throw AipsError("FiledesIO::pread - incorrect number of bytes (" + std::to_string(bytesRead) +
                      " out of " + std::to_string(size) + ") read for file " +
                      std::string(itsFileName));
    }
  }
...
int FiledesIO::open(const Char* name, Bool writable, Bool throwExcp) {
  ...
  int error = errno;
  if (throwExcp && fd == -1) {
    throw AipsError("FiledesIO: file " + String(name) + " could not be opened: " + strerror(error));
  }
  return fd;
}
...
void FiledesIO::close(int fd) {
  if (fd >= 0) {
    if (::traceCLOSE(fd) == -1) {
      int error = errno;
      throw AipsError(String("FiledesIO: file could not be closed: ") + strerror(error));
    }
  }
}
