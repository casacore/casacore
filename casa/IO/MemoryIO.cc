...
// #        Intgernet email: casa-feedback@nrao.edu.
  ...
  if (size <= bytesLeft) {
    ...
    if (bytesLeft >= 0) {
      ...
      if (throwException) {
        ...
        oss << "MemoryIO::read - incorrect number of bytes read:  " << std::endl
            << "  size=" << size << ", used=" << itsUsed << ", pos=" << itsPosition
            << ", left=" << bytesLeft;
        throw AipsError(oss.str());
      }
    } else {
      std::ostringstream oss;
      oss << "MemoryIO::read - buffer position is invalid:" << std::endl
          << "  size=" << size << ", used=" << itsUsed << ", pos=" << itsPosition
          << ", left=" << bytesLeft;
      throw AipsError(oss.str());
    }
...
Int64 MemoryIO::doSeek(Int64 offset, ByteIO::SeekOption dir) {
  ...
  if (newPos > itsUsed) {
    ...
    if (!itsWritable) {
      throw(
          AipsError("MemoryIO::seek - cannot seek past the end "
