...
void MultiFile::doFlushFile() { itsIO->flush(); }
...
void MultiFile::readHeaderVersion2(std::vector<char>& buf) {
  ...
  if (version != 2) {
    throw AipsError(
        "This version of Casacore supports up to MultiFile "
        "version 2, not version " +
...
void MultiFile::getInfoVersion2(Int64 contBlockNr, CanonicalIO& cio) {
  ...
  itsHdrContInx = 0;
  if (!itsHdrCont[1].blockNrs.empty() && contBlockNr == itsHdrCont[1].blockNrs[0]) {
    itsHdrContInx = 1;
  }
}
...
void MultiFile::doTruncateFile(MultiFileInfo& info, uInt64 nrblk) {
  if (nrblk < info.blockNrs.size()) {
    ...
    genSort(itsFreeBlocks.data(), itsFreeBlocks.size(), Sort::Descending, Sort::QuickSort);
...
void MultiFile::show(std::ostream& os) const {
  os << fileName() << ": blocksize=" << blockSize() << "  nfile=" << nfile()
     << "  nblock=" << nblock() << "  nCRC=" << itsCRC.size() << endl
     << "  ncont=" << itsNrContUsed[0] << ',' << itsNrContUsed[1] << "  cont=" << itsHdrContInx
