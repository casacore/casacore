...
ostream& operator<<(ostream& ios, const MultiFileInfo& info) {
  ios << info.name << ' ' << info.blockNrs << endl
      << "  fsize=" << info.fsize << " cur=" << info.curBlock << " nest=" << info.nested
  ...
  while (done < szdo) {
    ...
    if (blknr == info.curBlock) {
      ...
      if (todo == itsBlockSize &&
          (!itsUseODirect || ((uintptr_t)buffer & (uintptr_t)(mfb_od_align - 1)) == 0)) {
...
Int MultiFileBase::addFile(const String& fname) {
  ...
  for (std::vector<MultiFileInfo>::iterator iter = itsInfo.begin(); iter != itsInfo.end();
    ...
    if (iter->name.empty()) {
      ...
      throw AipsError("MultiFileBase::addFile - logical file name " + bname +
...
Int MultiFileBase::fileId(const String& fname, Bool throwExcp) const {
  ...
  if (throwExcp) {
    throw AipsError("MultiFileBase::fileId - logical file name " + fname + " is unknown in " +
                    itsName);
  }
  return -1;
}
...
MultiFileBuffer::MultiFileBuffer(size_t bufSize, Bool useODirect) : itsData(0) {
  ...
  if (bufSize > 0) {
    if (useODirect && bufSize % align != 0) {
      throw AipsError("MultiFile bufsize " + std::to_string(bufSize) + " must be a multiple of " +
                      std::to_string(mfb_od_align) + " when using O_DIRECT");
    }
    ...
    if (useODirect) {
      if (posix_memalign(&ptr, mfb_od_align, bufSize) != 0) {
        throw AllocError("MultiFileBuffer: failed to allocate aligned buffer", bufSize);
      }
    } else {
      ptr = malloc(bufSize);
      if (!ptr) {
