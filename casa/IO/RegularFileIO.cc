...
int RegularFileIO::openCreate(const RegularFile& file, ByteIO::OpenOption option, Bool useODirect) {
  ...
  switch (option) {
    ...
    case ByteIO::NewNoReplace:
      if (file.exists()) {
        throw(AipsError("RegularFileIO: new file " + name + " already exists"));
      }
  ...
  if (fd < 0) {
    throw(AipsError("RegularFileIO: error in open or create of file " + name + ": " +
                    strerror(errno)));
  }
  return fd;
}
...
void RegularFileIO::reopenRW() {
  ...
  if (file < 0) {
    throw(
        AipsError("RegularFileIO::reopenRW "
...
String RegularFileIO::fileName() const { return itsRegularFile.path().expandedName(); }
