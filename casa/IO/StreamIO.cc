...
StreamIO::StreamIO(const String& hostname, uShort portNumber) : ByteIO(), itsSockDesc(-1) {
  ...
  if (itsSockDesc < 0) {
    throw(AipsError("StreamIO::StreamIO - cannot attach a socket to host " + std::string(hostname) +
                    " on port " + std::to_string(portNumber)));
  }
  if (connect(itsSockDesc, reinterpret_cast<sockaddr*>(&serverInfo), sizeof(serverInfo)) < 0) {
    throw(AipsError("StreamIO::StreamIO - cannot connect to host " + std::string(hostname) +
                    " on port " + std::to_string(portNumber)));
  }
#endif
}
...
Int64 StreamIO::read(Int64 size, void* buf, Bool throwException) {
  ...
  while (bytesToRead > 0) {
    ...
    if (bytesRead < 0) {
      throw(AipsError(String("StreamIO::read - ") + String("Error returned by system call")));
    }
