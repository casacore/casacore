...
TypeIO::TypeIO(const std::shared_ptr<ByteIO>& byteIO) : itsByteIO(byteIO) {}

TypeIO::TypeIO(const TypeIO& that) : itsByteIO(that.itsByteIO) {}
...
const ByteIO& TypeIO::byteIO() const { return *itsByteIO; }
