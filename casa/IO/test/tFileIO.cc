...
int main(int argc, const char* argv[]) {
  ...
  cout << "tFileIO  nrrec=" << nr << " reclength=" << leng << " buffersize=" << size
       << " seek=" << seek << endl;
  ...
  {
    RegularFileIO file1(RegularFile("tFileIO_tmp.dat1"), ByteIO::New, size);
    Timer timer;
    for (i = 0; i < nr; i++) {
      if (seek && i % 3 == 0) {
  ...
  {
    RegularFileIO file1(RegularFile("tFileIO_tmp.dat1"), ByteIO::Old, size);
    Timer timer;
    for (i = 0; i < nr; i++) {
      if (seek && i % 3 == 0) {
        file1.seek(i * leng, ByteIO::Begin);
  ...
  {
    RegularFileIO file1(RegularFile("tFileIO_tmp.dat2"), ByteIO::New, size);
    Timer timer;
    for (i = 0; i < nr; i++) {
      if (seek && i % 3 == 0) {
        file1.seek(i * leng, ByteIO::Begin);
  ...
  {
    RegularFileIO file1(RegularFile("tFileIO_tmp.dat2"), ByteIO::Old, size);
    Timer timer;
    for (i = 0; i < nr; i++) {
      if (seek && i % 3 == 0) {
        file1.seek(i * leng, ByteIO::Begin);
