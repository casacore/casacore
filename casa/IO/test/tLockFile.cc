  ...
  while (True) {
    ...
    while (True) {
      ...
      if (op == 1 || op == 3) {
        ...
        cout << " Last errno " << lockp->lastError() << ": " << lockp->lastMessage() << endl;
...
int main(int argc, const char* argv[]) {
  try {
    ...
    if (argc > 1) {
      ...
      doTest();
      cout << "Run as:   tLockFile <fileName> [inspectionInterval]" << endl;
      cout << "for a manual control of acquiring and releasing locks." << endl;
