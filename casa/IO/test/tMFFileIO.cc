...
void showMultiFile(MultiFileBase& mfile) {
  cout << mfile.fileName() << ' ' << mfile.blockSize() << ' ' << mfile.nfile() << ' '
       << mfile.nblock() << ' ' << mfile.freeBlocks() << endl;
}

void showMFFile(MFFileIO& mfile) {
  cout << mfile.fileName() << ' ' << mfile.length() << ' ' << mfile.seek(0, ByteIO::Current)
       << endl;
}
...
void testTruncate(const std::shared_ptr<MultiFileBase>& parent) {
  ...
  std::shared_ptr<MultiFileBase> mfile1(parent->makeNested(parent, "mfile1", ByteIO::Update, 0));
  ...
  try {
    {
      ...
      std::shared_ptr<MultiFileBase> mfile(new MultiFile("tMFFileIO_tmp.dat1", ByteIO::New, 512));
      makeFile(mfile);
      writeFiles1(mfile);
      checkFiles1(mfile);
    ...
    if (HDF5Object::hasHDF5Support()) {
      ...
      std::shared_ptr<MultiFileBase> mfile(new MultiHDF5("tMFFileIO_tmp.dat2", ByteIO::New, 512));
      makeFile(mfile);
      writeFiles1(mfile);
      checkFiles1(mfile);
      // Test nested with parent still open and reopen of parent.
      ...
      std::shared_ptr<MultiFileBase> parent(new MultiHDF5("tMFFileIO_tmp.dat4", ByteIO::Update));
      testTruncate(parent);
    }
