...
int main(int argc, const char* argv[]) {
  try {
    ...
    inputs.create("bytes", "32768000", "Number of bytes to transfer", "Int", "1-10000000000");
    inputs.create("record", "32768", "Number of bytes in each record", "Int", "1-262144");
    inputs.create("check", "False", "Verify if the data read is correct", "Bool");
    ...
    {  // Do the write test
      ...
      cout << "It took " << elapsedTime << " seconds to write " << totalBytes / 1024
           << " kbytes using a record size of " << recordSize << " bytes" << endl;
      cout << "Transfer rate is: " << totalBytes / elapsedTime / 1024 << " kbytes/sec" << endl;
    }
    ...
    {  // Do the read test
      ...
      Double elapsedTime = clock.real();
      cout << "It took " << elapsedTime << " seconds to read " << totalBytes / 1024
           << " kbytes using a record size of " << recordSize << " bytes" << endl;
      cout << "Transfer rate is: " << totalBytes / elapsedTime / 1024 << " kbytes/sec" << endl;
      tape.rewind();
      TapeIO::close(fd);
