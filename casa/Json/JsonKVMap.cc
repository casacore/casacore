...
JsonKVMap::JsonKVMap(const JsonKVMap& that) : map<String, JsonValue>(that) {}
...
DComplex JsonKVMap::getDComplex(const String& name, const DComplex& defVal) const {
  const_iterator value = find(name);
  if (value == end()) {
    return defVal;
  }
...
const String& JsonKVMap::getString(const String& name, const String& defVal) const {
  const_iterator value = find(name);
  if (value == end()) {
    return defVal;
  }
...
ostream& operator<<(ostream& os, const JsonKVMap& param) {
  for (JsonKVMap::const_iterator iter = param.begin(); iter != param.end(); ++iter) {
    if (iter != param.begin()) {
      os << ", ";
    }
