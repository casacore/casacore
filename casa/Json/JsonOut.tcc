...
template <typename T>
inline void JsonOut::write(const String& name, T value, const String& comment) {
...
template <typename T>
inline void JsonOut::putArray(const Array<T>& arr, const String& indent, Bool firstLine) {
  putArray(arr, indent, firstLine, False);
}
inline void JsonOut::putArray(const Array<String>& arr, const String& indent, Bool firstLine) {
  putArray(arr, indent, firstLine, True);
}

template <typename T>
void JsonOut::putArray(const Array<T>& arr, const String& indent, Bool firstLine, Bool valueEndl) {
  ...
  if (arr.ndim() <= 1) {
    ...
    for (typename Array<T>::const_iterator iter = arr.begin(); iter != iterEnd; ++iter) {
      if (first) {
        first = False;
      } else if (!valueEndl) {
        itsStream << ", ";
