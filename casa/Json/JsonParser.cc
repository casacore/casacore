      ...
      if (i < leng) {
        switch (in[i]) {
          ...
          case 'u': {
            ...
            if (!ok) {
              throw JsonError("Invalid escaped control character " + in.substr(i - 1, 6));
            }
          } break;
          default:
            out += in[i];
...
void JsonGramerror(const char*) {
  ...
  String s2(os2.str());
  throw JsonError("Json parse error at position " + s2 + " (at or near '" + String(JsonGramtext) +
                  "')");
}
