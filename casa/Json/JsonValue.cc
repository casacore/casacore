...
JsonValue::JsonValue(const DComplex& value)
    : itsDataType(TpDComplex), itsValuePtr(new DComplex(value)) {}

JsonValue::JsonValue(const char* value) : itsDataType(TpString), itsValuePtr(new String(value)) {}

JsonValue::JsonValue(const String& value) : itsDataType(TpString), itsValuePtr(new String(value)) {}

JsonValue::JsonValue(const vector<JsonValue>& value)
    : itsDataType(TpOther), itsValuePtr(new vector<JsonValue>(value)) {}
...
DataType JsonValue::vectorDataType(const vector<JsonValue>& vec) const {
  ...
  for (vector<JsonValue>::const_iterator iter = vec.begin(); iter != vec.end(); ++iter) {
    ...
    if (dt == TpRecord) {
      ...
      if (dt == TpBool || dt == TpString || vdt == TpBool || vdt == TpString) {
        return TpRecord;
      }
