...
LogMessage::LogMessage(Priority prio) : priority_p(prio) {
  // Nothing
}
...
LogMessage::LogMessage(const String &message, const LogOrigin &sourceLocation, Priority priority)
    : origin_p(sourceLocation), priority_p(priority) {
...
LogMessage::LogMessage(const LogMessage &other) { copy_other(other); }
...
const LogOrigin &LogMessage::origin() const { return origin_p; }
...
LogMessage::Priority LogMessage::priority() const { return priority_p; }
...
const Time &LogMessage::messageTime() const { return time_p; }
...
const String &LogMessage::toString(Priority which) {
  static String names[11] = {"DEBUGGING", "DEBUG2", "DEBUG1", "INFO5", "INFO4", "INFO3",
                             "INFO2",     "INFO1",  "INFO",   "WARN",  "SEVERE"};
