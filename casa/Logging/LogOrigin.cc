...
LogOrigin::LogOrigin() : node_p(getNode()) {}

LogOrigin::LogOrigin(const String &globalFunctionName)
    : function_p(globalFunctionName), node_p(getNode()) {}
...
LogOrigin::LogOrigin(const String &className, const String &memberFuncName)
    : function_p(memberFuncName), class_p(className), node_p(getNode()) {}
...
LogOrigin::LogOrigin(const String &className, const String &memberFuncName, const ObjectID &id)
    : function_p(memberFuncName), class_p(className), id_p(id), node_p(getNode()) {}
...
const String &LogOrigin::taskName() const { return task_p; }
...
const String &LogOrigin::functionName() const { return function_p; }
...
const String &LogOrigin::className() const { return class_p; }
...
const ObjectID &LogOrigin::objectID() const { return id_p; }
...
const String &LogOrigin::fileName() const { return file_p; }
...
String LogOrigin::fullName() const {
  String nameTag("");
  if (task_p.length()) nameTag = task_p + "::";
  nameTag += className() + "::" + functionName();
  if (node_p.length()) nameTag += "::" + node_p;
  return nameTag;
}
