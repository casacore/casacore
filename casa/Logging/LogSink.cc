...
String LogSink::localId() { return String("LogSink"); }
...
void LogSink::preparePostThenThrow(const LogMessage &message, const AipsError &x) {
...
void LogSink::createGlobalSink() {
  global_sink_p = std::make_shared<LsiIntermediate>(new StreamLogSink(LogMessage::NORMAL, &cerr));
}
