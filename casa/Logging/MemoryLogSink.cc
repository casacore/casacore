...
String MemoryLogSink::localId() { return String("MemoryLogSink"); }

String MemoryLogSink::id() const { return String("MemoryLogSink"); }

MemoryLogSink::MemoryLogSink() : LogSinkInterface(), nmsg_p(0) {}

MemoryLogSink::MemoryLogSink(LogMessage::Priority filter)
    : LogSinkInterface(LogFilter(filter)), nmsg_p(0) {}

MemoryLogSink::MemoryLogSink(const LogFilterInterface& filter)
    : LogSinkInterface(filter), nmsg_p(0) {}

MemoryLogSink::MemoryLogSink(const MemoryLogSink& other) : LogSinkInterface() { copy_other(other); }
