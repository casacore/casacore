...
size_t CanonicalConversion::toLocalChar(void* to, const void* from, size_t nr) {
  assert(sizeof(char) == SIZE_CAN_CHAR);
  memcpy(to, from, nr);
  return nr * SIZE_CAN_CHAR;
}

size_t CanonicalConversion::fromLocalChar(void* to, const void* from, size_t nr) {
  assert(sizeof(char) == SIZE_CAN_CHAR);
  memcpy(to, from, nr);
  return nr * SIZE_CAN_CHAR;
}

void* CanonicalConversion::byteToLocalChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(char) == SIZE_CAN_CHAR);
  memcpy(to, from, nrbytes);
  return to;
}

void* CanonicalConversion::byteFromLocalChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(char) == SIZE_CAN_CHAR);
  memcpy(to, from, nrbytes);
  return to;
}
...
size_t CanonicalConversion::toLocalUChar(void* to, const void* from, size_t nr) {
  assert(sizeof(unsigned char) == SIZE_CAN_UCHAR);
  memcpy(to, from, nr);
  return nr * SIZE_CAN_UCHAR;
}

size_t CanonicalConversion::fromLocalUChar(void* to, const void* from, size_t nr) {
  assert(sizeof(unsigned char) == SIZE_CAN_UCHAR);
  memcpy(to, from, nr);
  return nr * SIZE_CAN_UCHAR;
}

void* CanonicalConversion::byteToLocalUChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(unsigned char) == SIZE_CAN_UCHAR);
  memcpy(to, from, nrbytes);
  return to;
}

void* CanonicalConversion::byteFromLocalUChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(unsigned char) == SIZE_CAN_UCHAR);
  memcpy(to, from, nrbytes);
  return to;
}

Conversion::ByteFunction* CanonicalConversion::getByteToLocal(const unsigned char*) {
  assert(sizeof(unsigned char) == SIZE_CAN_UCHAR);
  return Conversion::getmemcpy();
}

Conversion::ByteFunction* CanonicalConversion::getByteFromLocal(const unsigned char*) {
  assert(sizeof(unsigned char) == SIZE_CAN_UCHAR);
  return Conversion::getmemcpy();
}

#define CANONICALCONVERSION_DO(CONVERT, SIZE, TOLOCAL, FROMLOCAL, BYTETO, BYTEFROM, T) \
  size_t CanonicalConversion::TOLOCAL(void* to, const void* from, size_t nr) {         \
  ...
  size_t CanonicalConversion::FROMLOCAL(void* to, const void* from, size_t nr) {       \
    /* Use memcpy if no conversion is needed. */                                       \
    if (CONVERT == 0) {                                                                \
      assert(sizeof(T) == SIZE);                                                       \
      memcpy(to, from, nr * SIZE);                                                     \
  ...
  void* CanonicalConversion::BYTETO(void* to, const void* from, size_t nrbytes) {      \
    TOLOCAL(to, from, nrbytes / sizeof(T));                                            \
    return to;                                                                         \
  }                                                                                    \
  void* CanonicalConversion::BYTEFROM(void* to, const void* from, size_t nrbytes) {    \
...
CANONICALCONVERSION_DO(CONVERT_CAN_SHORT, SIZE_CAN_SHORT, toLocalShort, fromLocalShort,
                       byteToLocalShort, byteFromLocalShort, short)
CANONICALCONVERSION_DO(CONVERT_CAN_USHORT, SIZE_CAN_USHORT, toLocalUShort, fromLocalUShort,
                       byteToLocalUShort, byteFromLocalUShort, unsigned short)
CANONICALCONVERSION_DO(CONVERT_CAN_INT, SIZE_CAN_INT, toLocalInt, fromLocalInt, byteToLocalInt,
                       byteFromLocalInt, int)
