...
size_t Conversion::boolToBit(void* to, const void* from, size_t nvalues) {
...
void Conversion::boolToBit(void* to, const void* from, size_t startBit, size_t nvalues) {
  const Bool* data = (const Bool*)from;
  unsigned char* bits = (unsigned char*)to;
...
size_t Conversion::bitToBool_(void* to, const void* from, size_t nvalues) {
  Bool* data = (Bool*)to;
...
static m64d_t conv_tab[256]
    __attribute__((aligned(8))) = {{{False, False, False, False, False, False, False, False}},
...
size_t Conversion::bitToBool(void* to, const void* from, size_t nvalues) {
...
#else
  uint64_t* __attribute__((aligned(8))) data = (uint64_t __attribute__((aligned(8)))*)to;
#endif
  const uint8_t* bits = (const uint8_t*)from;
  const size_t bits_per_loop = 8;
  const size_t nwords = nvalues / bits_per_loop;
#ifdef _OPENMP
  size_t nthr = std::max((size_t)1, std::min((size_t)omp_get_max_threads(), nwords / (16 * 1024)));
...
void Conversion::bitToBool(void* to, const void* from, size_t startBit, size_t nvalues) {
  Bool* data = (Bool*)to;
  const unsigned char* bits = (const unsigned char*)from;
...
size_t Conversion::valueCopy(void* to, const void* from, size_t nbytes) {
