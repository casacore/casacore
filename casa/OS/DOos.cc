      ...
      if (check) {
        ...
        if (checkType) {
          if (!((takeRegular && file.isRegular(follow)) ||
                (takeDirectory && file.isDirectory(follow)) || (takeSymLink && file.isSymLink()))) {
            continue;
          }
        }
        if (checkAcc) {
          if (!((takeReadable && file.isReadable()) || (takeWritable && file.isWritable()) ||
...
Vector<Double> DOos::freeSpace(const Vector<String>& fileName, Bool follow) {
  ...
  for (uInt i = 0; i < fileName.nelements(); i++) {
    ...
    if (file.isDirectory(follow)) {
      ...
      throw(AipsError("DOos::freeSpace - file " + fileName(i) + " does not exist"));
    }
  }
  return result;
}
...
void DOos::copy(const String& to, const String& from, Bool overwrite, Bool follow) {
  ...
  if (file.isRegular(follow)) {
    ...
    throw(AipsError("DOos::copy - file " + from +
...
void DOos::move(const String& to, const String& from, Bool overwrite, Bool follow) {
  ...
  if (file.isRegular(follow)) {
    ...
    throw(AipsError("DOos::move - file " + from +
                    " is not a regular file "
                    "directory, nor symlink"));
  }
}
...
void DOos::remove(const Vector<String>& fileNames, Bool recursive, Bool mustExist, Bool follow) {
  ...
  if (mustExist) {
    for (i = 0; i < fileNames.nelements(); i++) {
      ...
      if (!file.exists()) {
        throw(AipsError("DOos::remove - file " + fileNames(i) + " does not exist"));
      }
    }
  }
  for (i = 0; i < fileNames.nelements(); i++) {
