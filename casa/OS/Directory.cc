...
Directory::Directory(const Path& name) : File(name) { checkPath(); }

Directory::Directory(const String& name) : File(name) { checkPath(); }

Directory::Directory(const File& name) : File(name) { checkPath(); }

Directory::Directory(const Directory& directory) : File(directory), itsFile(directory.itsFile) {}
...
void Directory::checkPath() {
  ...
  if (exists()) {
    if (isSymLink()) {
      ...
      throw(AipsError("Directory: " + path().expandedName() + " exists, but is no directory"));
    }
  } else {
    if (!canCreate()) {
      throw(AipsError("Directory: " + path().expandedName() +
...
Double Directory::freeSpace() const {
  ...
  struct statfs buf;
#if defined(AIPS_IRIX)
  if (statfs(itsFile.path().expandedName().c_str(), &buf, sizeof(buf), 0) < 0) {
#else
...
void Directory::remove() {
  ...
  if (!isEmpty()) {
    throw(AipsError("Directory::remove: " + path().expandedName() + " is not empty"));
  }
  if (isSymLink()) {
    removeSymLinks();
  }
...
void Directory::copy(const Path& target, Bool overwrite, Bool setUserWritePermission) const {
  ...
  if (result != 0) {
    throw AipsError("Executing cp command returned an error. Command was: " + command);
  }
  ...
  if (setUserWritePermission) {
    ...
    int result = system(command.c_str());
    if (result != 0)
      throw AipsError("Executing chmod command returned an error. Command was: " + command);
  }
#endif
}
...
void Directory::move(const Path& target, Bool overwrite) {
  ...
  if (errno != EXDEV) {
    throw(AipsError("Directory::move error on " + path().expandedName() + " to " +
