...
#if defined(AIPS_CRAY_PGI)
  itsNameList = 0;
  itsNrNames = scandir(itsDirectory.path().expandedName().c_str(), &itsNameList, 0, alphasort);
  if (itsNrNames < 0) {
...
void DirectoryIterator::operator++() {
  if (itsEnd) {
    throw(AipsError("DirectoryIterator++ past end on " + itsDirectory.path().expandedName()));
  }
  ...
  } while (name == "." || name == ".." || RegexMatches(name, itsExpression) == 0);
}
...
String DirectoryIterator::name() const {
  if (itsEnd) {
    throw(AipsError("DirectoryIterator::name past end on " + itsDirectory.path().expandedName()));
  }
...
Bool DirectoryIterator::pastEnd() const { return (itsEnd == True); }
