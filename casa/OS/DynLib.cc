...
std::string DynLib::tryOpen(const std::string& library, const std::string& dir,
...
void DynLib::attach(const std::string& library, const std::string& prefix,
                    const std::string& version, const std::string& funcName) {
  ...
  os << LogIO::NORMAL3 << "Loaded shared library " << fullName << LogIO::POST;
  if (itsHandle && !funcName.empty()) {
    ...
    if (!ptrCast.ptr) {
      close();
      throw AipsError("Found dynamic library " + fullName + ", but not its " + funcName +
                      " function\n  " + itsError);
    }
    ...
    ptrCast.funcPtr();
    os << LogIO::NORMAL3 << "Executed " << funcName << " in shared library " << fullName
       << LogIO::POST;
  }
}

std::string DynLib::tryCasacorePath(const std::string& library, const std::string& prefix,
