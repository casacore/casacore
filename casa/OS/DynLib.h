...
class DynLib {
 public:
  ...
  void* getHandle() const { return itsHandle; }

  // Get the possible error.
  const std::string& getError() const { return itsError; }
 ...
 private:
  ...
  void attach(const std::string& name, const std::string& prefix, const std::string& version,
              const std::string& funcName);

  // Try to open the library with some prefixes, suffixes and versions
  // If successful, itsHandle is filled and the full library name is
  // returned. Otherwise an empty name is returned.
  std::string tryOpen(const std::string& name, const std::string& libdir, const std::string& prefix,
