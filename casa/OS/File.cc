...
File::File(const Path& path) : itsPath(path) {}

File::File(const String& string) : itsPath(string) {}

File::File(const File& that) : itsPath(that.itsPath) {}
...
Bool File::exists() const {
  ...
  if (status != 0 && errno != ENOENT) {
    ...
    logIo << LogIO::WARN;
    logIo << "lstat failed for " << itsPath.expandedName() << ": errno=" << errno << "'"
          << strerror(errno) << "'\n";
    logIo << LogIO::POST;
  }
  return status == 0;
...
int File::mylstat(const char* path, void* buf) const {
  return fileLSTAT(const_cast<char*>(path), static_cast<struct fileSTAT*>(buf));
}

void File::getstat(const File& file, void* buf) const {
  if (mylstat(file.path().expandedName().c_str(), buf) < 0) {
    throw(
        AipsError("File::getstat error on " + file.path().expandedName() + ": " + strerror(errno)));
  }
}
...
void File::checkTarget(Path& targetName, Bool overwrite, Bool forDirectory) const {
  ...
  if (!target.isWritable()) {
    throw(AipsError("RegularFile::copy/move: target directory " + targetDir.originalName() +
                    " is not writable"));
  }
  ...
  if (target.exists()) {
    if (!overwrite) {
      throw(AipsError("RegularFile::copy/move: target file " + targetName.originalName() +
                      " already exists"));
    }
    if (!target.isWritable()) {
      throw(AipsError("RegularFile::copy/move: target file " + targetName.originalName() +
...
#else
  if (statbuf.f_type == 0x0BD00BD0) rstat = "Lustre";
#endif
  return rstat;
}
