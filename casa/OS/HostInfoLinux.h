...
// returns std::numeric_limits<uint64_t>::max() on error
...
void HostMachineInfo::update_info( )
{
	...
	uint64_t proc_mem_max = get_cgroup_limit("memory", "memory.limit_in_bytes") / 1024;
	/* usage_in_bytes also includes cache so use memory.stat */
	uint64_t proc_mem_used = get_cgroup_limit("memory", "memory.stat", "total_rss") / 1024;

	/* set HostInfo memoryTotal() */
	memory_total = std::min((uint64_t)sys_mem_total, proc_mem_max);
	...
	if (proc_mem_max <= sys_mem_total && proc_mem_used <= proc_mem_max) {
	    ...
	    memory_free = std::min((uint64_t)sys_mem_avail, (uint64_t)memory_total);
	}
	...
	uint64_t proc_swap_max = get_cgroup_limit("memory", "memory.memsw.limit_in_bytes") / 1024;
	uint64_t proc_swap_used = get_cgroup_limit("memory", "memory.stat", "total_swap") / 1024;
	/* limit is mem + swap */
	...
	/* set swapTotal() */
	swap_total = std::min((uint64_t)sys_swap_total, proc_swap_max);

	if (proc_swap_max <= (uint64_t)swap_total && proc_swap_used <= proc_swap_max) {
