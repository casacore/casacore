...
size_t LECanonicalConversion::toLocalChar(void* to, const void* from, size_t nr) {
  assert(sizeof(char) == SIZE_LECAN_CHAR);
  memcpy(to, from, nr);
  return nr * SIZE_LECAN_CHAR;
}

size_t LECanonicalConversion::fromLocalChar(void* to, const void* from, size_t nr) {
  assert(sizeof(char) == SIZE_LECAN_CHAR);
  memcpy(to, from, nr);
  return nr * SIZE_LECAN_CHAR;
}

void* LECanonicalConversion::byteToLocalChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(char) == SIZE_LECAN_CHAR);
  memcpy(to, from, nrbytes);
  return to;
}

void* LECanonicalConversion::byteFromLocalChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(char) == SIZE_LECAN_CHAR);
  memcpy(to, from, nrbytes);
  return to;
}
...
size_t LECanonicalConversion::toLocalUChar(void* to, const void* from, size_t nr) {
  assert(sizeof(unsigned char) == SIZE_LECAN_UCHAR);
  memcpy(to, from, nr);
  return nr * SIZE_LECAN_UCHAR;
}

size_t LECanonicalConversion::fromLocalUChar(void* to, const void* from, size_t nr) {
  assert(sizeof(unsigned char) == SIZE_LECAN_UCHAR);
  memcpy(to, from, nr);
  return nr * SIZE_LECAN_UCHAR;
}

void* LECanonicalConversion::byteToLocalUChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(unsigned char) == SIZE_LECAN_UCHAR);
  memcpy(to, from, nrbytes);
  return to;
}

void* LECanonicalConversion::byteFromLocalUChar(void* to, const void* from, size_t nrbytes) {
  assert(sizeof(unsigned char) == SIZE_LECAN_UCHAR);
  memcpy(to, from, nrbytes);
  return to;
}

Conversion::ByteFunction* LECanonicalConversion::getByteToLocal(const unsigned char*) {
  assert(sizeof(unsigned char) == SIZE_LECAN_UCHAR);
  return Conversion::getmemcpy();
}

Conversion::ByteFunction* LECanonicalConversion::getByteFromLocal(const unsigned char*) {
  assert(sizeof(unsigned char) == SIZE_LECAN_UCHAR);
  return Conversion::getmemcpy();
}

#define LECANONICALCONVERSION_DO(CONVERT, SIZE, TOLOCAL, FROMLOCAL, BYTETO, BYTEFROM, T) \
  size_t LECanonicalConversion::TOLOCAL(void* to, const void* from, size_t nr) {         \
  ...
  size_t LECanonicalConversion::FROMLOCAL(void* to, const void* from, size_t nr) {       \
    /* Use memcpy if no conversion is needed. */                                         \
    if (CONVERT == 0) {                                                                  \
      assert(sizeof(T) == SIZE);                                                         \
      memcpy(to, from, nr * SIZE);                                                       \
  ...
  void* LECanonicalConversion::BYTETO(void* to, const void* from, size_t nrbytes) {      \
    TOLOCAL(to, from, nrbytes / sizeof(T));                                              \
    return to;                                                                           \
  }                                                                                      \
  void* LECanonicalConversion::BYTEFROM(void* to, const void* from, size_t nrbytes) {    \
...
LECANONICALCONVERSION_DO(CONVERT_LECAN_SHORT, SIZE_LECAN_SHORT, toLocalShort, fromLocalShort,
                         byteToLocalShort, byteFromLocalShort, short)
LECANONICALCONVERSION_DO(CONVERT_LECAN_USHORT, SIZE_LECAN_USHORT, toLocalUShort, fromLocalUShort,
                         byteToLocalUShort, byteFromLocalUShort, unsigned short)
LECANONICALCONVERSION_DO(CONVERT_LECAN_INT, SIZE_LECAN_INT, toLocalInt, fromLocalInt,
