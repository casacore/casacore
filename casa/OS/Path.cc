...
Path::Path(const String& pathName) : itsOriginalPathName(pathName) {
  if (itsOriginalPathName.empty()) {
    itsOriginalPathName = ".";
  }
}

Path::Path(const Path& that) : itsOriginalPathName(that.itsOriginalPathName) {}
...
void Path::append(const String& string) {
  if (!string.empty()) {
    if (itsOriginalPathName.back() != '/' && string.front() != '/') {
      itsOriginalPathName += "/";
    }
