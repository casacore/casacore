...
#define RAWDATACONVERSION_DOIT(T)                                                 \
  size_t RawDataConversion::toLocal(T& to, const void* from) const {              \
    memcpy(&to, from, sizeof(T));                                                 \
    return sizeof(T);                                                             \
  }                                                                               \
  size_t RawDataConversion::toLocal(T* to, const void* from, size_t nr) const {   \
  ...
  size_t RawDataConversion::fromLocal(void* to, const T* from, size_t nr) const { \
    memcpy(to, from, nr * sizeof(T));                                             \
    return nr * sizeof(T);                                                        \
  }                                                                               \
