...
RegularFile::RegularFile(const Path& path) : File(path) { checkPath(); }

RegularFile::RegularFile(const String& path) : File(path) { checkPath(); }

RegularFile::RegularFile(const char* path) : File(String(path)) { checkPath(); }

RegularFile::RegularFile(const File& file) : File(file) { checkPath(); }

RegularFile::RegularFile(const RegularFile& that) : File(that), itsFile(that.itsFile) {}
...
void RegularFile::checkPath() {
  ...
  if (exists()) {
    if (isSymLink()) {
      ...
      throw(AipsError("RegularFile: " + path().expandedName() + " exists, but is no regular file"));
    }
  } else {
    if (!canCreate()) {
      throw(AipsError("RegularFile: " + path().expandedName() +
...
void RegularFile::create(Bool overwrite) {
  ...
  if (exists()) {
    ...
    if (!overwrite) {
      throw(AipsError("RegularFile::create: " + itsFile.path().expandedName() + " already exists"));
    }
  }
  int fd = ::creat(itsFile.path().expandedName().c_str(), 0666);
  if (fd < 0) {
    throw(AipsError("RegularFile::create error on " + itsFile.path().expandedName() + ": " +
                    strerror(errno)));
  }
  ::close(fd);
}
...
void RegularFile::copy(const Path& target, Bool overwrite, Bool setUserWritePermission) const {
...
void RegularFile::move(const Path& target, Bool overwrite) {
  ...
  if (rename(path().expandedName().c_str(), targetPath.expandedName().c_str()) == 0) {
    return;
  }
  ...
  if (errno != EXDEV) {
    throw(AipsError("RegularFile::move error on " + path().expandedName() + " to " +
