...
SymLink::SymLink(const Path& name) : File(name) { checkPath(); }

SymLink::SymLink(const String& name) : File(name) { checkPath(); }

SymLink::SymLink(const File& name) : File(name) { checkPath(); }

SymLink::SymLink(const SymLink& that) : File(that) {}
...
void SymLink::checkPath() const {
  ...
  if (exists()) {
    if (!isSymLink()) {
      throw(AipsError("SymLink: " + path().expandedName() + " exists, but is no symbolic link"));
    }
  } else {
    if (!canCreate()) {
      throw(
          AipsError("SymLink: " + path().expandedName() + " does not exist and cannot be created"));
    }
  }
}
...
void SymLink::create(const Path& target, Bool overwrite) {
  ...
  if (exists()) {
    ...
    if (!overwrite) {
      throw(AipsError("SymLink::create: " + path().expandedName() + " already exists"));
    }
    // Remove an existing symlink, otherwise symlink fails.
    remove();
  }
  if (symlink(target.expandedName().c_str(), path().expandedName().c_str()) < 0) {
    throw(AipsError("SymLink::create error on " + target.expandedName() + ": " + strerror(errno)));
  }
}
...
String SymLink::getSymLink() const {
  ...
  if (length <= 0) {
    throw(AipsError("SymLink: " + path().expandedName() + " does not exist"));
  }
...
Path SymLink::readSymLink() const {
  ...
  if (result.originalName().front() != '/') {
    result = Path(path().dirName() + "/" + result.originalName());
  }
  return result;
}
...
Path SymLink::followSymLink() const {
  ...
  do {
    if (++count > 25) {
      throw(
          AipsError("SymLink: resolving " + path().expandedName() + " results in an endless loop"));
    }
    SymLink link(file);
