...
void VAXConversion::toLocal(float* to, const void* from, size_t nr) {
...
void VAXConversion::toLocal(double* to, const void* from, size_t nr) {
  assert(sizeof(unsigned int) == 4);
  ...
  while (to < last) {
    ...
    if (exponent == 0) {
      ...
      value = (value & 0x80000000) | exponent | ((value >> 3) & 0x000fffff);
    }
...
void VAXConversion::fromLocal(void* to, const float* from, size_t nr) {
  assert(sizeof(unsigned int) == 4);
  assert(sizeof(float) == 4);
  char* data = (char*)to;
...
void VAXConversion::fromLocal(void* to, const double* from, size_t nr) {
  assert(sizeof(unsigned int) == 4);
  assert(sizeof(double) == 8);
  char* data = (char*)to;
  ...
  while (from < last) {
    ...
    if (exponent <= 0) {
      ...
      if (exponent > 255) {
        ...
        value =
            (value & 0x80000000) | (exponent << 23) | ((value << 3) & 0x007fffff) | (rest >> 29);
        rest <<= 3;
      }
    }
    moveFloat(data, &value);
