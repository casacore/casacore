...
static FORCEINLINE void pthread_release_lock(MLOCK_T* sl) {
  ...
  if (--sl->c == 0) {
    ...
    int prev = 0;
    int ret;
    __asm__ __volatile__("lock; xchgl %0, %1" : "=r"(ret) : "m"(*(lp)), "0"(prev) : "memory");
  }
}
...
static void init_malloc_global_mutex() {
  for (;;) {
    ...
    if (stat == 0 && interlockedcompareexchange(&malloc_global_mutex_status, -1, 0) == 0) {
...
static int init_mparams(void) {
  ...
  if (mparams.magic == 0) {
...
#else  /* WIN32 */
    {
      ...
      gsize =
          ((DEFAULT_GRANULARITY != 0) ? DEFAULT_GRANULARITY : system_info.dwAllocationGranularity);
    }
#endif /* WIN32 */
  ...
  */
  ...
  if (tbase != CMFAIL) {
    ...
    if (!is_initialized(m)) { /* first-time initialization */
      ...
      while (sp != 0 && tbase != sp->base + sp->size) sp = (NO_SEGMENT_TRAVERSAL) ? 0 : sp->next;
      if (sp != 0 && !is_extern_segment(sp) && (sp->sflags & USE_MMAP_BIT) == mmap_flag &&
        ...
        sp = &m->seg;
        while (sp != 0 && sp->base != tbase + tsize) sp = (NO_SEGMENT_TRAVERSAL) ? 0 : sp->next;
        if (sp != 0 && !is_extern_segment(sp) && (sp->sflags & USE_MMAP_BIT) == mmap_flag) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    if (mem != 0) {
      ...
      if ((((size_t)(mem)) % alignment) != 0) { /* misaligned */
        ...
        */
        char* br =
            (char*)mem2chunk((size_t)(((size_t)(mem + alignment - SIZE_T_ONE)) & -alignment));
