...
void checkFlags(int& error) {
  ...
  if (((char*)(&val))[0] != 1) {
    cout << "AIPS_LITTLE_ENDIAN should not be defined" << endl;
    error = 1;
  }
#else
  if (((char*)(&val))[0] != 0) {
    cout << "AIPS_LITTLE_ENDIAN should be defined" << endl;
    error = 1;
  }
#endif
  ...
  if ((flag ^ CONVERT_CAN_CHAR) != 0) {
    cout << "invalid CONVERT_CAN_CHAR definition" << endl;
    error = 1;
  }
  flag = sizeof(unsigned char) != SIZE_CAN_UCHAR;
  if ((flag ^ CONVERT_CAN_UCHAR) != 0) {
    cout << "invalid CONVERT_CAN_UCHAR definition" << endl;
    error = 1;
  }
  if (sizeof(char) != 1) {
  ...
  if ((flag ^ CONVERT_CAN_SHORT) != 0) {
    cout << "invalid CONVERT_CAN_SHORT definition" << endl;
    error = 1;
  }
#if !defined(AIPS_LITTLE_ENDIAN)
  ...
  if ((flag ^ CONVERT_CAN_USHORT) != 0) {
    cout << "invalid CONVERT_CAN_USHORT definition" << endl;
    error = 1;
  }
  if (sizeof(short) < 2) {
  ...
  if ((flag ^ CONVERT_CAN_INT) != 0) {
    cout << "invalid CONVERT_CAN_INT definition" << endl;
    error = 1;
  }
#if !defined(AIPS_LITTLE_ENDIAN)
  ...
  if ((flag ^ CONVERT_CAN_UINT) != 0) {
    cout << "invalid CONVERT_CAN_UINT definition" << endl;
    error = 1;
  }
  if (sizeof(int) < 4) {
  ...
  if ((flag ^ CONVERT_CAN_INT64) != 0) {
    cout << "invalid CONVERT_CAN_INT64 definition" << endl;
    error = 1;
  }
#if !defined(AIPS_LITTLE_ENDIAN)
  ...
  if ((flag ^ CONVERT_CAN_UINT64) != 0) {
    cout << "invalid CONVERT_CAN_UINT64 definition" << endl;
    error = 1;
  }
  if (sizeof(Int64) < 8) {
  ...
  if ((flag ^ CONVERT_CAN_FLOAT) != 0) {
    cout << "invalid CONVERT_CAN_FLOAT definition" << endl;
    error = 1;
  }
#if !defined(AIPS_LITTLE_ENDIAN)
  ...
  if ((flag ^ CONVERT_CAN_DOUBLE) != 0) {
    cout << "invalid CONVERT_CAN_DOUBLE definition" << endl;
    error = 1;
  }
  if (sizeof(float) != 4) {
...
void checkConversion(int& error) {
  ...
  }
  {
    ...
    CanonicalConversion::fromLocal(out + 1, &result, 1);
    if (out[1] != val[1] || out[2] != val[2] || out[3] != val[3] || out[4] != val[4]) {
  ...
  }
  {
    ...
    CanonicalConversion::fromLocal(out + 1, &result, 1);
    if (out[1] != val[1] || out[2] != val[2] || out[3] != val[3] || out[4] != val[4] ||
        out[5] != val[5] || out[6] != val[6] || out[7] != val[7] || out[8] != val[8]) {
  ...
  }
  {
    ...
    if (result != float(v)) {
      ...
      cout << hex;
      cout << int(cv[0]) << ' ' << int(cv[1]) << ' ' << int(cv[2]) << ' ' << int(cv[3]) << endl;
      cv = (unsigned char*)&v;
      cout << int(cv[0]) << ' ' << int(cv[1]) << ' ' << int(cv[2]) << ' ' << int(cv[3]) << endl;
  ...
  }
  {
    ...
    float v = d;
    if (result != v) {
      cout << "invalid float to conversion " << setprecision(20) << result << " "
           << setprecision(20) << v << endl;
      unsigned char* cv = (unsigned char*)&result;
      cout << hex;
      cout << int(cv[0]) << ' ' << int(cv[1]) << ' ' << int(cv[2]) << ' ' << int(cv[3]) << endl;
      cv = (unsigned char*)&v;
  ...
  }
  {
    ...
    v *= double(16 + 15) / 32 + double(1) / (32 * 256) + double(2) / (32 * 256 * 256) +
         double(3) / (32 * 256 * 256 * 256) + double(4) / (32 * double(256 * 256 * 256) * 256) +
         double(5) / (32 * double(256 * 256 * 256) * 256 * 256);
    if (result != v) {
      cout << "invalid double to conversion " << result << " " << v << endl;
      unsigned char* cv = (unsigned char*)&result;
      cout << hex;
      cout << int(cv[0]) << ' ' << int(cv[1]) << ' ' << int(cv[2]) << ' ' << int(cv[3]) << ' '
           << int(cv[4]) << ' ' << int(cv[5]) << ' ' << int(cv[6]) << ' ' << int(cv[7]) << endl;
      cv = (unsigned char*)&v;
      cout << int(cv[0]) << ' ' << int(cv[1]) << ' ' << int(cv[2]) << ' ' << int(cv[3]) << ' '
           << int(cv[4]) << ' ' << int(cv[5]) << ' ' << int(cv[6]) << ' ' << int(cv[7]) << endl;
      cv = (unsigned char*)&result;
