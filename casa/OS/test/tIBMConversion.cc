...
void checkConversion(int& error) {
  ...
  char out[5];
  {
    ...
    if (result[0] != 'a' || result[1] != ' ' || result[2] != 'd' || result[3] != 's') {
      cout << "invalid char to conversion " << result[0] << result[1] << result[2] << result[3]
           << endl;
      error = 1;
    }
  ...
  for (uInt i = 0; i < 2; i++) {
    for (uInt j = 35; j < 97; j++) {
      ...
      for (uInt k = 1; k < 256; k++) {
        ...
        if (f1 != v1) {
          cout << i << " " << j << " " << k << " " << f1 << " " << v1 << " float to" << endl;
          error = 1;
        }
        ...
        if (k > 15) {
          ...
          if (out[1] != val[1] || out[2] != val[2] || out[3] != val[3] || out[4] != val[4]) {
            cout << i << " " << j << " " << k << " " << f1 << " " << v1 << " float from" << endl;
            error = 1;
          }
        }
      }
      ...
      for (uInt k = 1; k < 256; k++) {
        ...
        for (uInt l = 0; l < 32; l++) {
          ...
          if (k > 15) {
            IBMConversion::fromLocal(out + 1, &f1, 1);
            if (j == 0) {
              if (out[1] != 0 || out[2] != 0 || out[3] != 0 || out[4] != 0 || out[5] != 0 ||
