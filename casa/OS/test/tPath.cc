...
void check(const String& path, const String& expanded, const String& absolute, Bool& success) {
...
void checkDirBase(const String& path, const String& dir, const String& base, Bool& success) {
...
void doIt(Bool doExcp, Bool& success) {
  ...
  check("~" + user + "/test/test2", home + "/test/test2", home + "/test/test2", success);
  check("$tPath_Env_User/test", home + "/test", home + "/test", success);
  check("${tPath_Env_User}/test", home + "/test", home + "/test", success);
  check("/aa$HOME/test", "/aa" + home + "/test", "/aa" + home + "/test", success);
  check("/aa${HOME}bb/test", "/aa" + home + "bb/test", "/aa" + home + "bb/test", success);
  ...
  check("$tPath_Env_Test1/$HOME", home + "/" + home, home + home, success);

  // Check resolvedName.
  String tpDir = Path("$tPath_Env_Curr/tPath_tmpdir").absoluteName();
  AlwaysAssertExit(Path("$tPath_Env_Curr/tPath_tmpdir//d1/../d1/.//d2/").resolvedName() ==
                   tpDir + "/d1/d2");
  if (doExcp) {
    Bool ok = True;
    try {
  ...
  AlwaysAssertExit(Path::stripDirectory("././abc", "././././abc/de") == "de/.");
  AlwaysAssertExit(Path::stripDirectory("././abc", "././././abd/de") == "abc");
  AlwaysAssertExit(Path::stripDirectory("abc", "abc/de") == "de/.");
  AlwaysAssertExit(Path::stripDirectory("././abc/de/ef", "././././abc/de") == "././ef");
  const std::string result = Path::stripDirectory("abc/ef", "abc/de");
  AlwaysAssertExit(Path::stripDirectory("abc/ef", "abc/de") == "./ef");
  AlwaysAssertExit(Path::stripDirectory("ef", "de") == "./ef");
  AlwaysAssertExit(Path::stripDirectory("$HOME/ef", "de") == "$HOME/ef");
  AlwaysAssertExit(Path::stripDirectory("/de/abc/ab/ef", "/de/abc/de") == "./ab/ef");
  AlwaysAssertExit(Path::stripDirectory("/de/abc/ab/ef", "/de/abc/ab") == "././ef");
  AlwaysAssertExit(Path::addDirectory("de/ab", "/ghi/a") == "de/ab");
  AlwaysAssertExit(Path::addDirectory("./de/ab", "/ghi/a") == "/ghi/de/ab");
  AlwaysAssertExit(Path::addDirectory("././de/ab", "/ghi/a") == "/ghi/a/de/ab");
}
