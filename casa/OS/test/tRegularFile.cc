...
void doIt(Bool doExcp) {
  ...
  AlwaysAssertExit(rFile5.path().originalName() == "tRegularFile_tmp/isLink1");
  if (doExcp) {
    try {
  ...
  AlwaysAssertExit(risLink1.path().originalName() == "tRegularFile_tmp/isLink1");
  rFile5 = risFile;
  AlwaysAssertExit(rFile5.path().originalName() == "tRegularFile_tmp/isFile");
