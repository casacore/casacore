...
void doIt(Bool doExcp) {
  ...
  AlwaysAssertExit(newLink.readSymLink().originalName() == Path("$HOME").expandedName());
  AlwaysAssertExit(newLink2.readSymLink().originalName() == "tSymLink_tmp/isDir/../E");
  if (doExcp) {
    try {
      linkA.followSymLink();
    } catch (std::exception& x) {
  ...
  AlwaysAssertExit(linkII.readSymLink().originalName() == "tSymLink_tmp/B");
  ...
  AlwaysAssertExit(linkI.isSymLink());
  AlwaysAssertExit(linkIII.readSymLink().originalName() == "tSymLink_tmp/isDir/B");
  ...
  AlwaysAssertExit(SymLink(linkisFile).readSymLink().originalName() == "tSymLink_tmp/B");
  ...
  AlwaysAssertExit(!linkII.exists());
  AlwaysAssertExit(linkIIa.readSymLink().originalName() == "tSymLink_tmp/isDir/B");
