...
Euler::Euler(const Euler &other) : euler(3), axes(3) {
  euler = other.euler;
  axes = other.axes;
}
...
Euler::Euler(const Quantity &in0) : euler(3), axes(3) {
...
Euler::Euler(const Quantity &in0, const Quantity &in1) : euler(3), axes(3) {
  euler(0) = Euler::makeRad(in0);
...
Euler::Euler(const Quantity &in0, const Quantity &in1, const Quantity &in2) : euler(3), axes(3) {
  euler(0) = Euler::makeRad(in0);
  euler(1) = Euler::makeRad(in1);
...
Euler &Euler::operator+=(const Euler &right) {
  DebugAssert(axes(0) == right.axes(0) && axes(1) == right.axes(1) && axes(2) == right.axes(2),
              AipsError);
  euler += right.euler;
  return *this;
}
