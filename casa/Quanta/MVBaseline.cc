...
// # Constructors
MVBaseline::MVBaseline() : MVPosition() {}

MVBaseline::MVBaseline(Double in) : MVPosition(in) {}

MVBaseline::MVBaseline(const Quantity &l) : MVPosition(l) {}

MVBaseline::MVBaseline(Double in0, Double in1, Double in2) : MVPosition(in0, in1, in2) {}

MVBaseline::MVBaseline(const Quantity &l, Double angle0, Double angle1)
    : MVPosition(l, angle0, angle1) {}

MVBaseline::MVBaseline(const Quantity &l, const Quantity &angle0, const Quantity &angle1)
    : MVPosition(l, angle0, angle1) {}

MVBaseline::MVBaseline(const Quantum<Vector<Double>> &angle) : MVPosition(angle) {}

MVBaseline::MVBaseline(const Quantity &l, const Quantum<Vector<Double>> &angle)
    : MVPosition(l, angle) {}

MVBaseline::MVBaseline(const Vector<Double> &other) : MVPosition(other) {}

MVBaseline::MVBaseline(const Vector<Quantity> &other) : MVPosition() {
  if (!putValue(other)) {
    throw(AipsError("Illegal quantum vector in MVBaseline constructor"));
  }
}

MVBaseline::MVBaseline(const MVPosition &pos, const MVPosition &base) : MVPosition(pos) {
  xyz -= base.getValue();
}
...
Double MVBaseline::operator*(const MVBaseline &other) const {
  Double tmp = 0.0;
  for (Int i = 0; i < 3; i++) {
...
Double MVBaseline::radius() { return (std::sqrt(operator*(*this))); }
...
const Vector<Double> &MVBaseline::getValue() const { return xyz; }
...
Quantity MVBaseline::getLength(const Unit &unit) const { return getLength().get(unit); }
...
Quantity MVBaseline::BaselineAngle(const MVBaseline &other, const Unit &unit) const {
...
Quantity MVBaseline::separation(const MVBaseline &other, const Unit &unit) const {
...
void MVBaseline::print(ostream &os) const { os << getValue(); }

MeasValue *MVBaseline::clone() const { return (new MVBaseline(*this)); }

Vector<Double> MVBaseline::getVector() const { return xyz; }
