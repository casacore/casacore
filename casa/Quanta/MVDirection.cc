...
// # Constructors
MVDirection::MVDirection() : MVPosition() { xyz(2) = Double(1.0); }

MVDirection::MVDirection(const MVPosition &other) : MVPosition(other) {}

MVDirection::MVDirection(Double in0, Double in1, Double in2) : MVPosition(in0, in1, in2) {
  adjust();
}

MVDirection::MVDirection(Double in0) : MVPosition() {
  xyz(0) = std::cos(in0);
  xyz(1) = std::sin(in0);
}

MVDirection::MVDirection(Double angle0, Double angle1) : MVPosition() {
...
MVDirection::MVDirection(const Quantity &angle0) : MVPosition() {
  xyz(0) = ((cos(angle0)).getValue());
  xyz(1) = ((sin(angle0)).getValue());
  xyz(2) = 0;
}

MVDirection::MVDirection(const Quantity &angle0, const Quantity &angle1) : MVPosition() {
...
MVDirection::MVDirection(const Quantum<Vector<Double>> &angle) : MVPosition(angle) { adjust(); }

MVDirection::MVDirection(const Vector<Double> &angle) : MVPosition(angle) { adjust(); }

MVDirection::MVDirection(const Vector<Quantity> &angle) : MVPosition() {
  if (!putValue(angle)) {
...
Double MVDirection::getLat() const { return MVPosition::getLat(1.0); }
...
Quantity MVDirection::positionAngle(const MVPosition &other, const Unit &unit) const {
  return Quantity(positionAngle(other), "rad").get(unit);
}

Quantity MVDirection::positionAngle(const MVDirection &other, const Unit &unit) const {
  return Quantity(positionAngle(other), "rad").get(unit);
}
...
Double MVDirection::separation(const MVPosition &other) const {
  ...
  l2 = l2 > 0 ? l2 : 1.0;
  Double d1 = std::sqrt(square(xyz(0) - otherxyz(0) / l2) + square(xyz(1) - otherxyz(1) / l2) +
                        square(xyz(2) - otherxyz(2) / l2)) /
              2.0;
  return 2 * std::asin(d1 < 1.0 ? d1 : 1.0);
}

Double MVDirection::separation(const MVDirection &other) const {
  Double d1 = std::sqrt(square(xyz(0) - other.xyz(0)) + square(xyz(1) - other.xyz(1)) +
                        square(xyz(2) - other.xyz(2))) /
              2.0;
  return 2 * std::asin(d1 < 1.0 ? d1 : 1.0);
}

Quantity MVDirection::separation(const MVPosition &other, const Unit &unit) const {
  return Quantity(separation(other), "rad").get(unit);
}

Quantity MVDirection::separation(const MVDirection &other, const Unit &unit) const {
  return Quantity(separation(other), "rad").get(unit);
}
...
void MVDirection::shiftAngle(const Quantum<Double> &off, const Quantum<Double> &pa) {
...
void MVDirection::shiftAngle(Double off, Double pa) {
  ...
  x = get();
  Double nlat =
      std::asin(std::cos(off) * std::sin(x(1)) + std::sin(off) * std::cos(x(1)) * std::cos(pa));
...
MeasValue *MVDirection::clone() const { return (new MVDirection(*this)); }
