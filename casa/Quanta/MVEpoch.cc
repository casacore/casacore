...
// # Constructors
MVEpoch::MVEpoch() : wday(0), frday(0) {}

MVEpoch::MVEpoch(const MVEpoch &other) : MeasValue(), wday(other.wday), frday(other.frday) {
  adjust();
}

MVEpoch::MVEpoch(Double inday, Double infrac) : wday(0), frday(0) {
  addTime(inday);
  addTime(infrac);
  adjust();
}

MVEpoch::MVEpoch(const Quantity &in) : wday(0), frday(0) { addTime(makeDay(in)); }

MVEpoch::MVEpoch(const Quantity &in1, const Quantity &in2) : wday(0), frday(0) {
  addTime(makeDay(in1));
  addTime(makeDay(in2));
  adjust();
}

MVEpoch::MVEpoch(const Quantum<Vector<Double>> &in) : wday(0), frday(0) {
...
MVEpoch::MVEpoch(const Vector<Double> &inday) : wday(0), frday(0) {
  for (uInt i = 0; i < inday.nelements(); i++) addTime(inday(i));
  adjust();
}

MVEpoch::MVEpoch(const Vector<Quantity> &in) : wday(0), frday(0) {
  if (!putValue(in)) {
...
Double MVEpoch::get() const { return (wday + frday); }

Quantity MVEpoch::getTime() const { return (Quantity(get(), unitDay)); }

Quantity MVEpoch::getTime(const Unit &unit) const { return (getTime().get(unit)); }

Double MVEpoch::getDay() const { return wday; }

Double MVEpoch::getDayFraction() const { return frday; }
...
void MVEpoch::print(ostream &os) const {
  ...
  os << setw(os.precision() + 3) << s << setprecision(prec);
  os.setf(oldb, ios::floatfield);
  os.fill(fill);
}

MeasValue *MVEpoch::clone() const { return (new MVEpoch(*this)); }
