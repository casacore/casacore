...
// # Constructors
MVPosition::MVPosition() : xyz(3) { xyz = Double(0.0); }

MVPosition::MVPosition(const MVPosition &other) : MeasValue(), xyz(3) { xyz = other.xyz; }
...
MVPosition::MVPosition(Double in) : xyz(3) {
  xyz = Double(0.0);
  xyz(2) = in;
}

MVPosition::MVPosition(const Quantity &l) : xyz(3) {
  xyz = Double(0.0);
  l.assure(UnitVal::LENGTH);
  xyz(2) = l.getBaseValue();
}

MVPosition::MVPosition(Double in0, Double in1, Double in2) : xyz(3) {
  xyz(0) = in0;
  xyz(1) = in1;
  xyz(2) = in2;
}

MVPosition::MVPosition(const Quantity &l, Double angle0, Double angle1) : xyz(3) {
  ...
  if (t < 0 && t > -7.0e6)
    t = t / 1.0e7 + hiLimit;
  else if (t > loLimit && t < hiLimit)
    t += 0.001;
  readjust(t);
}

MVPosition::MVPosition(const Quantity &l, const Quantity &angle0, const Quantity &angle1) : xyz(3) {
...
MVPosition::MVPosition(const Quantum<Vector<Double>> &angle) : xyz(3) {
  uInt i;
...
MVPosition::MVPosition(const Quantity &l, const Quantum<Vector<Double>> &angle) : xyz(3) {
  uInt i;
  i = angle.getValue().nelements();
  if (i > 3) {
...
MVPosition::MVPosition(const Vector<Double> &other) : xyz(3) {
  uInt i;
  i = other.nelements();
  if (i > 3) {
    throw(AipsError("Illegal vector length in MVPosition constructor"));
...
MVPosition::MVPosition(const Vector<Quantity> &other) : xyz(3) {
  if (!putValue(other)) {
...
Double MVPosition::operator*(const MVPosition &other) const {
  Double tmp = 0.0;
  for (Int i = 0; i < 3; i++) {
...
Double MVPosition::radius() { return (std::sqrt(operator*(*this))); }
...
const Vector<Double> &MVPosition::getValue() const { return xyz; }
...
Double MVPosition::getLong() const {
  return ((xyz(0) != 0 || xyz(1) != 0) ? std::atan2(xyz(1), xyz(0)) : 0.0);
}
...
Double MVPosition::getLat() const { return getLat(norm(xyz)); }

Double MVPosition::getLat(Double ln) const { return std::asin(xyz(2) / ((ln == 0) ? 1.0 : ln)); }
...
Quantity MVPosition::getLength(const Unit &unit) const { return getLength().get(unit); }
...
Quantity MVPosition::positionAngle(const MVPosition &other, const Unit &unit) const {
...
Double MVPosition::separation(const MVPosition &other) const {
  ...
  l2 = l2 > 0 ? l2 : 1.0;
  Double d1 =
      std::sqrt(square(xyz(0) / l1 - other.xyz(0) / l2) + square(xyz(1) / l1 - other.xyz(1) / l2) +
                square(xyz(2) / l1 - other.xyz(2) / l2)) /
      2.0;
  return 2 * std::asin(d1 < 1.0 ? d1 : 1.0);
}

Quantity MVPosition::separation(const MVPosition &other, const Unit &unit) const {
...
void MVPosition::print(ostream &os) const { os << getValue(); }

MeasValue *MVPosition::clone() const { return (new MVPosition(*this)); }

Vector<Double> MVPosition::getVector() const { return xyz; }
