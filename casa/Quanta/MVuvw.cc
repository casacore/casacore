...
// # Constructors
MVuvw::MVuvw() : MVPosition() {}

MVuvw::MVuvw(Double in) : MVPosition(in) {}

MVuvw::MVuvw(const Quantity &l) : MVPosition(l) {}

MVuvw::MVuvw(Double in0, Double in1, Double in2) : MVPosition(in0, in1, in2) {}

MVuvw::MVuvw(const Quantity &l, Double angle0, Double angle1) : MVPosition(l, angle0, angle1) {}

MVuvw::MVuvw(const Quantity &l, const Quantity &angle0, const Quantity &angle1)
    : MVPosition(l, angle0, angle1) {}

MVuvw::MVuvw(const Quantum<Vector<Double>> &angle) : MVPosition(angle) {}

MVuvw::MVuvw(const Quantity &l, const Quantum<Vector<Double>> &angle) : MVPosition(l, angle) {}

MVuvw::MVuvw(const Vector<Double> &other) : MVPosition(other) {}

MVuvw::MVuvw(const Vector<Quantity> &other) : MVPosition() {
  if (!putValue(other)) {
...
MVuvw::MVuvw(const MVBaseline &pos, const MVDirection &dr, Bool ew) : MVPosition() {
  ...
  MVDirection dr1(dr);
  dr1.adjust();
  RotMatrix x(Euler(dr1.getLat() - M_PI_2, 1u, -dr1.getLong() - M_PI_2, 3u));
  if (ew) {
  }
  xyz = (x * pos).getValue();
}
...
Double MVuvw::operator*(const MVuvw &other) const {
  Double tmp = 0.0;
  for (Int i = 0; i < 3; i++) {
...
Double MVuvw::radius() { return (std::sqrt(operator*(*this))); }
...
const Vector<Double> &MVuvw::getValue() const { return xyz; }
...
Quantum<Vector<Double>> MVuvw::getAngle(const Unit &unit) const { return getAngle().get(unit); }
...
Quantity MVuvw::getLength(const Unit &unit) const { return getLength().get(unit); }
...
Quantity MVuvw::uvwAngle(const MVuvw &other, const Unit &unit) const {
...
Quantity MVuvw::separation(const MVuvw &other, const Unit &unit) const {
...
void MVuvw::print(ostream &os) const { os << getValue(); }

MeasValue *MVuvw::clone() const { return (new MVuvw(*this)); }

Vector<Double> MVuvw::getVector() const { return xyz; }
