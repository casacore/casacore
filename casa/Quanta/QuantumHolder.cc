...
QuantumHolder::QuantumHolder(const QBase &in) : hold_p(in.clone()) {}

QuantumHolder::QuantumHolder(const QuantumHolder &other) : RecordTransformable(), hold_p() {
  if (other.hold_p) {
...
const Quantum<Vector<Complex>> &QuantumHolder::asQuantumVectorComplex() {
  ...
  if (isArray()) {
    ...
    if (ndim() != 1) {
      (static_cast<Quantum<Array<Complex>> *>(hold_p.get()))
          ->getValue()
          .reform(IPosition(1, nelements()));
    }
  } else {
...
const Quantum<Vector<DComplex>> &QuantumHolder::asQuantumVectorDComplex() {
  ...
  if (isArray()) {
    ...
    if (ndim() != 1) {
      (static_cast<Quantum<Array<DComplex>> *>(hold_p.get()))
          ->getValue()
          .reform(IPosition(1, nelements()));
    }
  } else {
...
void QuantumHolder::toArray() { toVector(); }
