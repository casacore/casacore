...
Bool UnitMap::getUnit(const String& s, UnitName& name, UMaps* maps) {
  ...
  if ((pos = mapUser.find(s)) != mapUser.end() || (pos = mapCust.find(s)) != mapCust.end() ||
...
void UnitMap::putUser(const String& s, const UnitVal& val, const String& name) {
...
void UnitMap::putUser(const UnitName& name) {
  ...
  if ((pos = mapUser.find(name.getName())) != mapUser.end() ||
      ...
      (pos = mapSI.find(name.getName())) != mapSI.end())
    clearCache();
...
void UnitMap::clearCache() { getMapCache().clear(); }

void UnitMap::listPref() { listPref(cout); }

void UnitMap::listPref(ostream& os) {
  map<String, UnitName>& mapPref = getMaps().mapPref;
  for (map<String, UnitName>::iterator i = mapPref.begin(); i != mapPref.end(); ++i) {
    os << "    " << i->second << endl;
  }
}

void UnitMap::listDef() { listDef(cout); }

void UnitMap::listDef(ostream& os) {
  map<String, UnitName>& mapDef = getMaps().mapDef;
  for (map<String, UnitName>::iterator i = mapDef.begin(); i != mapDef.end(); ++i) {
    os << "    " << i->second << endl;
  }
}

void UnitMap::listSI() { listSI(cout); }

void UnitMap::listSI(ostream& os) {
  map<String, UnitName>& mapSI = getMaps().mapSI;
  for (map<String, UnitName>::iterator i = mapSI.begin(); i != mapSI.end(); ++i) {
    os << "    " << i->second << endl;
  }
}

void UnitMap::listCust() { listCust(cout); }

void UnitMap::listCust(ostream& os) {
  map<String, UnitName>& mapCust = getMaps().mapCust;
  for (map<String, UnitName>::iterator i = mapCust.begin(); i != mapCust.end(); ++i) {
    os << "    " << i->second << endl;
  }
}

void UnitMap::listUser() { listUser(cout); }

void UnitMap::listUser(ostream& os) {
  map<String, UnitName>& mapUser = getMaps().mapUser;
  for (map<String, UnitName>::iterator i = mapUser.begin(); i != mapUser.end(); ++i) {
    os << "    " << i->second << endl;
  }
}

void UnitMap::list() { list(cout); }
...
void UnitMap::listCache() { listCache(cout); }

void UnitMap::listCache(ostream& os) {
  map<String, UnitVal>& mapCache = getMapCache();
  os << "Cached unit table (" << mapCache.size() << "):" << endl;
  for (map<String, UnitVal>::iterator i = mapCache.begin(); i != mapCache.end(); ++i) {
    os << "    " << UnitName(i->first, i->second) << endl;
  }
}

const map<String, UnitName>& UnitMap::givePref() { return getMaps().mapPref; }

const map<String, UnitName>& UnitMap::giveDef() { return getMaps().mapDef; }
const map<String, UnitName>& UnitMap::giveSI() { return getMaps().mapSI; }

const map<String, UnitName>& UnitMap::giveCust() { return getMaps().mapCust; }

const map<String, UnitName>& UnitMap::giveUser() { return getMaps().mapUser; }

const map<String, UnitVal>& UnitMap::giveCache() { return getMapCache(); }
