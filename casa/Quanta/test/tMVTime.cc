...
void showTime (MVTime time, uint32_t format, uint32_t prec)
{
  cout << MVTime::Format(format,prec) << time << endl;
}

void checkTime (const String& str, uint32_t yy, uint32_t mm, uint32_t dd,
                uint32_t h, uint32_t m, uint32_t s, double ss, bool chk=true)
{
  Quantity q;
  AlwaysAssertExit (MVTime::read(q, str, chk));
  MVTime mvtm(q);
  Time tm = mvtm.getTime();
  AlwaysAssertExit (mvtm.year() == int32_t(yy));
