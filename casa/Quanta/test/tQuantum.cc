...
int main() {
  ...
  try {
    ...
    cout << "sqrt(Jy2)^2 = " << sqrt(Quantity(2, "Jy2")) * sqrt(Quantity(2, "Jy2")) << endl;
    cout << "B==A        = " << (B == A) << endl;
  ...
  AlwaysAssert(nearAbs(Quantity(4, "km"), Quantity(4000.001, "m"), Quantity(1, "cm")), AipsError);
  AlwaysAssert(nearAbs(Quantity(4.00001, "km"), Quantity(4000, "m"), Quantity(1, "cm")), AipsError);
  AlwaysAssert(!nearAbs(Quantity(4, "km"), Quantity(4000.001, "m"), Quantity(0.5, "mm")),
               AipsError);
  AlwaysAssert(!nearAbs(Quantity(4.00001, "km"), Quantity(4000, "m"), Quantity(0.5, "mm")),
               AipsError);
  {
    // getValue()
    Bool thrown = False;
