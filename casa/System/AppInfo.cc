...
String AppInfo::workDirectory(uInt minimumFreeSpaceInMB) {
  ...
  if (candidates.nelements() == 0) {
    ...
    os << LogIO::SEVERE << "No work directory with at least " << minimumFreeSpaceInMB
       << "MB free can be found." << endl
       << "Check aipsrc variable user.directories.work." << LogIO::EXCEPTION;
  }
