...
void Casarc::setDefaultPath(const std::string &path) { default_path = new std::string(path); }
...
void Casarc::put(const std::string &keyword, const std::string &value) {
  ...
  if (mapping == rcmap.end()) {
    ...
    if (meta == rcmetamap.end()) throw("Casarc::put, internal inconsistency between data & meta");

    strftime(buf, 512, "# modified %F %T\n", localtime(&tv.tv_sec));

    off_t start =
        (meta->second.time_length() ? meta->second.time_offset() : meta->second.key_offset());
    ...
    int copylen = sizeof(char) * (start + (buflen + keyword.length() + value.length() + 5) +
...
bool Casarc::get(const std::string &keyword, std::string &value) {
  ...
  if (iter == rcmap.end()) return false;
  value = iter->second;
  return true;
}
...
std::string Casarc::get(const std::string &keyword) {
  sync();
  std::map<std::string, std::string>::iterator iter = rcmap.find(keyword);
  if (iter == rcmap.end()) return std::string("");
  return iter->second;
}

size_t Casarc::size() const { return rcmap.size(); }
...
int Casarc::lock(lock_mode mode) {
  ...
  if (flock(fd, LOCK_EX) < 0) {
    throw("Casarc::lock, failed to lock: " + filename);
  }
#else
  struct flock lock;
...
void Casarc::unlock(int fd) {
  ...
  if (have_lock.size() == 0) {
    ...
    if (flock(fd, LOCK_UN) < 0) {
      throw("Casarc::unlock, failed to unlock: " + filename);
    }
#else
    struct flock lock;
    ...
    if (fcntl(fd, F_SETLK, &lock) < 0) {
      throw("Casarc::unlock, failed to lock: " + filename);
    }
#endif
  }
  close(fd);
...
#define is_added_tag(PTR, OFF)                                                                 \
  (PTR[OFF + 0] == '#' && PTR[OFF + 1] == ' ' && PTR[OFF + 2] == 'a' && PTR[OFF + 3] == 'd' && \
   PTR[OFF + 4] == 'd' && PTR[OFF + 5] == 'e' && PTR[OFF + 6] == 'd' && PTR[OFF + 7] == ' ' && \
   isdigit(PTR[OFF + 8]) && isdigit(PTR[OFF + 9]) && isdigit(PTR[OFF + 10]) &&                 \
   isdigit(PTR[OFF + 11]) && PTR[OFF + 12] == '-' && isdigit(PTR[OFF + 13]) &&                 \
   isdigit(PTR[OFF + 14]) && PTR[OFF + 15] == '-' && isdigit(PTR[OFF + 16]) &&                 \
   isdigit(PTR[OFF + 17]) && PTR[OFF + 18] == ' ' && isdigit(PTR[OFF + 19]) &&                 \
   isdigit(PTR[OFF + 20]) && PTR[OFF + 21] == ':' && isdigit(PTR[OFF + 22]) &&                 \
   isdigit(PTR[OFF + 23]) && PTR[OFF + 24] == ':' && isdigit(PTR[OFF + 25]) &&                 \
   isdigit(PTR[OFF + 26]))

#define is_modified_tag(PTR, OFF)                                                                  \
  (PTR[OFF + 0] == '#' && PTR[OFF + 1] == ' ' && PTR[OFF + 2] == 'm' && PTR[OFF + 3] == 'o' &&     \
   PTR[OFF + 4] == 'd' && PTR[OFF + 5] == 'i' && PTR[OFF + 6] == 'f' && PTR[OFF + 7] == 'i' &&     \
   PTR[OFF + 8] == 'e' && PTR[OFF + 9] == 'd' && PTR[OFF + 10] == ' ' && isdigit(PTR[OFF + 11]) && \
   isdigit(PTR[OFF + 12]) && isdigit(PTR[OFF + 13]) && isdigit(PTR[OFF + 14]) &&                   \
   PTR[OFF + 15] == '-' && isdigit(PTR[OFF + 16]) && isdigit(PTR[OFF + 17]) &&                     \
   PTR[OFF + 18] == '-' && isdigit(PTR[OFF + 19]) && isdigit(PTR[OFF + 20]) &&                     \
   PTR[OFF + 21] == ' ' && isdigit(PTR[OFF + 22]) && isdigit(PTR[OFF + 23]) &&                     \
   PTR[OFF + 24] == ':' && isdigit(PTR[OFF + 25]) && isdigit(PTR[OFF + 26]) &&                     \
   PTR[OFF + 27] == ':' && isdigit(PTR[OFF + 28]) && isdigit(PTR[OFF + 29]))
...
void Casarc::read_file() {
  ...
  if (buf.st_size == 0) {
    AlwaysAssert(write(fd, "\n", 1) == 1, AipsError);
    lseek(fd, 0, SEEK_SET);
    if (fstat(fd, &buf) < 0) throw("Casarc::read_file, internal error");
  }
  ...
  for (off_t off = 0; off < mapped_file_size; ++off) {
    ...
    off_t keystart = off;
    int keylen = 0;
    for (; !isspace(mapped_file[keystart + keylen]) && mapped_file[keystart + keylen] != ':' &&
           (keystart + keylen) < mapped_file_size;
         ++keylen) {
    }
    ...
    rcmetamap.insert(
        make_pair(string((const char *)keyword),
                  meta_entry_(keystart, keylen, valstart, vallen, timestp, timestplen)));
