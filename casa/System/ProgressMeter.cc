...
static void stderr_show_function(Int id, Double value) {
  ...
  fstream file_op(ProgressMeter::PROGRESSFILE, ios::out | ios::app);
  file_op << stderr_time[id] << "," << stderr_title[id] << "," << stderr_min[id] << ","
          << stderr_max[id] << "," << stderr_last[id] << "\n";
  file_op.close();
  return;
}
...
static void stderr_busy_function(Int id) {
  ...
  id--;  // 0-relative
  fstream file_op(ProgressMeter::PROGRESSFILE, ios::out | ios::app);
  file_op << stderr_time[id] << "," << stderr_title[id] << ","
          << "0,0,1\n";
  file_op.close();
  return;
}
...
ProgressMeter::ProgressMeter(Double min, Double max, const String &title, const String &subtitle,
  ...
  if (creation_function_p) {
    id_p = creation_function_p(min, max, title, subtitle, minlabel, maxlabel, estimateTime);
  }
}
...
void ProgressMeter::busy() { busy_function_p(id_p); }
...
void ProgressMeter::update(Double value, Bool force) {
  ...
  if (showProgress) {
    if ((value >= min_p) && (value <= max_p)) {
      ...
      // cerr << "WARNING: progress meter trying to update beyond range" << endl;//The user does not
      // need to know that the programmer does not know how to add.
    }
  }
}
