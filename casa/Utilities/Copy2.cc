...
void objthrowmv1(const void *to, const void *from, const size_t n) {
  if (n > 0 && (!from || !to))
    throw(
        AipsError("objmove(T* to, const T* from, size_t n)"
                  " - illegal argument"));
}

void objthrowmv2(const void *to, const void *from, const size_t n, const size_t toStride,
                 const size_t fromStride) {
  if (n > 0 && (!from || !to || !toStride || !fromStride))
    throw(
        AipsError("objmove(T* to, const T* from, size_t n, size_t toStride, "
...
void objthrowcp1(const void *to, const void *from, const size_t n) {
  if (n > 0 && (!from || !to))
    throw(
        AipsError("objcopy(T* to, const T* from, size_t n)"
                  " - illegal argument"));
}

void objthrowcp2(const void *to, const void *from, const size_t n, const size_t toStride,
                 const size_t fromStride) {
  if (n > 0 && (!from || !to || !toStride || !fromStride))
    throw(
        AipsError("objcopy(T* to, const T* from, size_t n, size_t toStride, "
                  "size_t fromStride) - illegal argument"));
}
...
void objthrowfl1(const void *to, const size_t n) {
  if (n > 0 && !to)
    throw(
        AipsError("objset(T* to, const T fillValue, size_t n)"
                  " - illegal argument"));
}
...
void objtestmv(size_t &nLeft, size_t &startLeft, size_t &startRight, const void *to,
               const void *from, const size_t n, const size_t toStride, const size_t fromStride,
               const void *toPn, const void *fromPn, const size_t fromMto, const size_t toMfrom) {
