...
template <class T, class INX>
INX GenSortIndirect<T, INX>::sort(Vector<INX>& indexVector, const Block<T>& data, INX nr,
                                  Sort::Order ord, int opt) {
  return sort(indexVector, data.storage(), std::min(nr, static_cast<INX>(data.nelements())), ord,
              opt);
}

// Use quicksort if nothing given.
template <class T, class INX>
INX GenSortIndirect<T, INX>::sort(Vector<INX>& indexVector, const T* data, INX nr, Sort::Order ord,
                                  int opt) {
...
template <class T, class INX>
INX GenSortIndirect<T, INX>::insSort(INX* inx, const T* data, INX nr, Sort::Order ord, int opt) {
...
template <class T, class INX>
INX GenSortIndirect<T, INX>::heapSort(INX* inx, const T* data, INX nr, Sort::Order ord, int opt) {
  INX n = nr;
...
template <class T, class INX>
INX GenSortIndirect<T, INX>::parSort(INX* inx, const T* data, INX nr, Sort::Order ord, int opt,
                                     int nthread) {
  int nthr = nthread;  // to avoid compiler warning
#ifdef _OPENMP
  if (nthread > 0) {
...
#pragma omp parallel for num_threads(nthr)
  ...
  if (nparts < nr) {
    Block<INX> inxtmp(nr);
    INX* res = merge(data, inx, inxtmp.storage(), nr, index.storage(), nparts);
    // Skip duplicates if needed.
    if ((opt & Sort::NoDuplicates) != 0) {
...
template <class T, class INX>
INX* GenSortIndirect<T, INX>::merge(const T* data, INX* inx, INX* tmp, INX nr, INX* index,
                                    INX nparts) {
  INX* a = inx;
  INX* b = tmp;
  int np = nparts;
...
void GenSortIndirect<T, INX>::quickSortAsc(INX* inx, const T* data, INX nr, Bool multiThread,
  ...
  INX* sf = inx;
  INX* sl = inx + nr - 1;
  if (isAscending(data, *sf, *mid)) swapInx(*sf, *mid);
  if (isAscending(data, *sf, *sl)) swapInx(*sf, *sl);
  if (isAscending(data, *sl, *mid)) swapInx(*sl, *mid);
  T partVal = data[*sl];
  INX partInx = *sl;
  ...
  sl--;
  for (;;) {
    while (data[*sf] < partVal || (partVal == data[*sf] && *sf < partInx)) {
      sf++;
    }
    while (data[*sl] > partVal || (partVal == data[*sl] && *sl > partInx)) {
      sl--;
    }
    if (sf >= sl) break;
    swapInx(*sf, *sl);
...
INX GenSortIndirect<T, INX>::kthLargest(T* data, INX nr, INX k) {
  ...
  while (end > st + 1) {
    ...
    if (data[inx[i]] < data[*sf]) swapInx(inx[i], *sf);
    if (data[*sl] < data[*sf]) swapInx(*sl, *sf);
    if (data[inx[i]] < data[*sl]) swapInx(inx[i], *sl);
...
// Do an insertion sort in ascending order.
template <class T, class INX>
INX GenSortIndirect<T, INX>::insSortAsc(INX* inx, const T* data, INX nr, int opt) {
  if ((opt & Sort::NoDuplicates) == 0) {
...
template <class T, class INX>
void GenSortIndirect<T, INX>::heapAscSiftDown(INX* inx, INX low, INX up, const T* data) {
  INX sav = inx[low];
  INX c;
  INX i;
  for (i = low; (c = 2 * i) <= up; i = c) {
