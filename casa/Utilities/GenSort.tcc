...
template<class T>
void GenSort<T>::quickSortAsc (T* data, int32_t nr, bool multiThread, int32_t rec_lim)
{
    ...
    int32_t i = (nr-1)/2;                        // middle element
...
template<class T>
T GenSort<T>::kthLargest (T* data, uint32_t nr, uint32_t k)
{
    if (k >= nr) {
	throw (AipsError ("kthLargest(data, nr, k): k must be < nr"));
    }
    int32_t st = 0;
    int32_t end = int32_t(nr) - 1;
	...
	// Do not use Sedgewick\'s advise to store the partition element in
	// data[nr-2]. This has dramatic results for reversed ordered arrays.
	int32_t i = (st+end)/2;                      // middle element
...
template<class T>
uint32_t GenSort<T>::insSortAsc (T* data, int32_t nr, int opt)
{
...
template<class T>
uint32_t GenSort<T>::insSortAscDup (T* data, int32_t nr)
{
    int32_t  j;
    T cur;
...
template<class T>
uint32_t GenSort<T>::insSortAscNoDup (T* data, int32_t nr)
{
    if (nr < 2) {
...
template<class T>
void GenSort<T>::heapSortAsc (T* data, int32_t nr)
{
...
template<class T>
void GenSort<T>::heapAscSiftDown (int32_t low, int32_t up, T* data)
{
    T sav = data[low];
    int32_t c;
    int32_t i;
...
template<class T>
uint32_t GenSort<T>::parSort (T* data, uint32_t nr, Sort::Order ord, int opt,
                          int nthread)
{
  ...
  nthr = 1;
#endif
  Block<uint32_t> index(nr+1);
  Block<uint32_t> tinx(nthr+1);
  Block<uint32_t> np(nthr);
...
#pragma omp parallel for num_threads(nthr)
  ...
  for (int i=0; i<nthr; ++i) {
    int nparts = 1;
    index[tinx[i]] = tinx[i];
    for (uint32_t j=tinx[i]+1; j<tinx[i+1]; ++j) {
      if (data[j-1] > data[j]) {
...
template<class T>
void GenSort<T>::reverse (T* data, const T* res, uint32_t nr)
{
  ...
  if (res == data) {
    ...
    for (uint32_t i=0; i<nr; ++i) data[i] = res[nr-1-i];
  }
}

template<class T>
T* GenSort<T>::merge (T* data, T* tmp, uint32_t nr, uint32_t* index,
                      uint32_t nparts)
{
  T* a = data;
  T* b = tmp;
...
#pragma omp parallel for schedule(dynamic)
	...
	T* f2 = a+index[i+1];
	T* to = b+index[i];
	uint32_t na = index[i+1]-index[i];
	uint32_t nb = index[i+2]-index[i+1];
        if (i == np-2) {
          //cout<<"swap last np=" <<np<<endl;
          f2 = last;
          last = to;
        }
	uint32_t ia=0, ib=0, k=0;
	while (ia < na && ib < nb) {
	  if (f1[ia] < f2[ib]) {
	    to[k] = f1[ia++];
	  } else {
	...
	if (ia < na) {
	  for (uint32_t p=ia; p<na; p++,k++) to[k] = f1[p];
	} else {
	  for (uint32_t p=ib; p<nb; p++,k++) to[k] = f2[p];
	}
      }
    }
    // Collapse the index.
...
template<class T>
uint32_t GenSort<T>::insSort (T* data, uint32_t nr, Sort::Order ord, int opt)
{
  uint32_t n = insSortAsc (data, nr, opt);
  if (ord == Sort::Descending) {
    reverse (data, data, n);
  }
  return n;
...
template<class T>
uint32_t GenSort<T>::quickSort (T* data, uint32_t nr, Sort::Order ord, int opt)
{
...
template<class T>
uint32_t GenSort<T>::heapSort (T* data, uint32_t nr, Sort::Order ord, int opt)
{
  uint32_t n = nr;
  heapSortAsc (data, nr);
  if ((opt & Sort::NoDuplicates) != 0) {
...
template<class T>
uint32_t GenSort<T>::sort (T* data, uint32_t nr, Sort::Order ord, int opt)
{
...
template<class T>
uint32_t GenSort<T>::sort (Array<T>& data, Sort::Order ord, int opt)
{
  bool del;
  T* dptr = data.getStorage(del);
  uint32_t nr = sort (dptr, data.nelements(), ord, opt);
  data.putStorage (dptr, del);
  return nr;
}

template<class T>
uint32_t GenSort<T>::sort (Block<T>& data, uint32_t nr, Sort::Order ord, int opt)
{
  return sort (data.storage(), std::min<uint32_t>(nr, data.nelements()), ord, opt);
}
...
void GenSortIndirect<T,INX>::quickSortAsc (INX* inx, const T* data, INX nr,
                                           bool multiThread, int32_t rec_lim)
{
    if (nr <= 32) {
