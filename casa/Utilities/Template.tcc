...
const Regex Template::fileRE = String(
    "^[[:space:]]*[[:digit:]]*"
    "[[:space:]]*"
    "[^.[:space:]]*[.](cc|h)");
const Regex Template::typedefRE = String("^[[:space:]]*typedef[[:space:]]");
const Regex Template::auxtemplRE = String(
    "^[[:space:]]*"
...
const Regex Template::PATcanon[Ncanon] = {String("[[:space:]]"),  // 00
                                          String(" ="),
                                          String("   *"),
                                          String("^ "),
                                          String(" $"),
                                          ...
                                          String("  *"),  // 50
                                          String(">const")};
const String Template::REPcanon[Ncanon] = {" ",  // 00
                                           " = ",
                                           " ",
                                           "",
                                           "",
...
const Regex Template::PATcanon20[Ncanon2] = {String("[^[:alnum:]_]char[^[:alnum:]_]"),  // 00
...
const Regex Template::PATcanon21[Ncanon2] = {
    String("char"),  // 00
    String("short"),        String("unsigned"),       String("signed"),
    String("int"),          String("long"),           String("float"),
    String("double"),       String("complex<Float>"), String("complex<Double>"),
    String("<Complex >"),  // 10
    String("<DComplex >"),  String("bool"),           String("std::Complex"),
    String("std::DComplex")};
const String Template::REPcanon2[Ncanon2] = {"Char",  // 00
                                             "Short",
                                             "uInt",
                                             "Int",
                                             "Int",
...
const Regex Template::PATnmin[Nnmin] = {String("^[^[:digit:]]"),  // 00
                                        String("^[[:digit:]] "), String("^[[:digit:]][[:digit:]] "),
                                        String("^[[:digit:]][[:digit:]][[:digit:]] ")};
const String Template::REPnmin[Nnmin] = {"0000 ",  // 00
                                         "000", "00", "0"};

// Make canonical numbers of 4 digits maximum
const Regex Template::PATnmax[Nnmax] = {
    String("^[[:digit:]][[:digit:]][[:digit:]][[:digit:]][[:digit:]]+ ")};
const Regex Template::REPnmax[Nnmax] = {String("[[:digit:]][[:digit:]][[:digit:]][[:digit:]] ")};
...
const String Template::PATinif[Ninif] = {String("& &"),  // 00
                                         String("= ="), String("<"), String("< ="), String(">")};
const String Template::REPinif[Ninif] = {"&&",  // 00
                                         "==", " < ", "<=", " >"};
...
const Regex Template::PATtypedef1[Ntypedef] = {String("FitsLong"),  // 00
                                               String("lDouble"),
                                               String("Long"),
                                               String("DataStatus"),
...
String Template::REPtypedef[Ntypedef] = {"Long",  // 00
                                         "lDouble",
                                         "Long",
                                         "DataStatusStructure",
...
void Template::read(const String &filename) {
  ...
  while (ok && (((extracted = ""), (ok = getline(file, extracted))) || !combine.empty())) {
    ...
    if ((extracted.contains(ifRE) || extracted.contains(endifRE) || extracted.contains(elseRE) ||
         extracted.contains(templateRE) || extracted.contains(contRE) ||
         extracted.contains(typedefRE) || extracted.contains(auxtemplRE) ||
         extracted.contains(namespaceRE)) &&
        ok && !err) {
...
void Template::writeOut(ostream &os, const Bool warn) {
  ...
  if (cwarn) {
    cerr << "Warning: One or more possibly superfluous template arguments "
            "given.\n         Run reident with the -v (verbose) switch to learn more"
         << endl;
  }
}
