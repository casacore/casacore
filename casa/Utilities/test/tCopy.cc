...
int main() {
  ...
  for (i = 1; i < size - 1; i += 2) AlwaysAssertExit(ia2[i] == 99 && ia2[i + 1] == 66);
  AlwaysAssertExit(ia2[0] == 0 && ia2[size - 1] == 99);
  objcopy(ia2, ia, size, 1, 1);
  for (i = 0; i < size; i += 2) AlwaysAssertExit(ia2[i] == 99 && ia2[i + 1] == 66);
  // With toStride=fromStride=2.
  objcopy(ia2 + 1, ia, size / 2, 2, 2);
  for (i = 0; i < size; i++) AlwaysAssertExit(ia2[i] == 99);
  // With fromStride=2.
  objcopy(ia2, ia + 1, size / 2, 1, 2);
  for (i = 0; i < size / 2; i++) AlwaysAssertExit(ia2[i] == 66);
  for (i = size / 2; i < size; i++) AlwaysAssertExit(ia2[i] == 99);
  // With toStride=2.
  objcopy(ia, ia2, size / 2, 2, 1);
  for (i = 0; i < size; i++) AlwaysAssertExit(ia[i] == 66);
  ...
  for (i = 0; i < size; i++) ia[i] = i;
  // Move to left without strides.
  objmove(ia, ia + 1, size - 1);
  for (i = 0; i < size - 1; i++) AlwaysAssertExit(ia[i] == i + 1);
  AlwaysAssertExit(ia[size - 1] == size - 1);
  // Move to right without strides.
  objmove(ia + 1, ia, size - 1);
  for (i = 1; i < size; i++) AlwaysAssertExit(ia[i] == i);
  AlwaysAssertExit(ia[0] == 1);

  // Move to left with toStride=fromStride=2.
  objmove(ia, ia + 1, size / 2, 2, 2);
  for (i = 0; i < size; i += 2) AlwaysAssertExit(ia[i] == i + 1 && ia[i + 1] == i + 1);
  // Move to right with toStride=fromStride=2.
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia + 1, ia, size / 2, 2, 2);
  for (i = 0; i < size; i += 2) AlwaysAssertExit(ia[i] == i && ia[i + 1] == i);

  // Move in place with different strides.
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia, ia, size);
  for (i = 0; i < size; i++) AlwaysAssertExit(ia[i] == i);
  objmove(ia, ia, size / 2, 1, 2);
  for (i = 0; i < size / 2; i++) AlwaysAssertExit(ia[i] == 2 * i);
  for (i = size / 2; i < size; i++) AlwaysAssertExit(ia[i] == i);
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia, ia, size / 2, 2, 1);
  for (i = 0; i < size; i += 2) AlwaysAssertExit(ia[i] == i / 2 && ia[i + 1] == i + 1);

  // Move to the left with different strides.
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia, ia + 50, size / 2, 2, 1);
  for (i = 0; i < size; i += 2) AlwaysAssertExit(ia[i] == size / 2 + i / 2 && ia[i + 1] == i + 1);
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia, ia + 2, size / 2 - 1, 1, 2);
  for (i = 0; i < size / 2 - 1; i++) AlwaysAssertExit(ia[i] == 2 * (i + 1));
  for (i = size / 2 - 1; i < size; i++) AlwaysAssertExit(ia[i] == i);

  // Move to the right with different strides.
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia + size / 2, ia, size / 2, 1, 2);
  for (i = 0; i < size / 2; i++) AlwaysAssertExit(ia[i] == i);
  for (i = size / 2; i < size; i++) AlwaysAssertExit(ia[i] == 2 * (i - size / 2));
  ...
  for (i = 0; i < size / 4; i++) AlwaysAssertExit(ia[i] == i);
  for (i = size / 4; i < 3 * size / 4; i++) AlwaysAssertExit(ia[i] == 2 * (i - size / 4));
  for (i = 3 * size / 4; i < size; i++) AlwaysAssertExit(ia[i] == i);
  for (i = 0; i < size; i++) ia[i] = i;
  ...
  for (i = 0; i < size; i += 2) AlwaysAssertExit(ia[i] == i / 2 + size / 4 && ia[i + 1] == i + 1);
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia, ia + size / 4, size / 4, 4, 2);
  for (i = 0; i < size; i += 4)
    AlwaysAssertExit(ia[i] == i / 2 + size / 4 && ia[i + 1] == i + 1 && ia[i + 2] == i + 2 &&
                     ia[i + 3] == i + 3);
  for (i = 0; i < size; i++) ia[i] = i;
  objmove(ia + 1, ia + size / 4, size / 4, 4, 2);
  for (i = 1; i < size - 4; i += 4)
    AlwaysAssertExit(ia[i] == (i - 1) / 2 + size / 4 && ia[i + 1] == i + 1 && ia[i + 2] == i + 2 &&
                     ia[i + 3] == i + 3);
  ...
  for (i = 0; i < size; i++) AlwaysAssertExit(ta[i][0] == 1 && ta[i][1] == 2 && ta[i][2] == 3);
  ...
  for (i = 0; i < size / 2; i++)
    AlwaysAssertExit(ta2[i][0] == 2 * i && ta2[i][1] == 2 * i + size &&
                     ta2[i][2] == 2 * i + 2 * size);
  ...
  objcopy(ta, ta2, size);
  for (i = 0; i < size / 2; i++)
    AlwaysAssertExit(ta[i][0] == 2 * i && ta[i][1] == 2 * i + size && ta[i][2] == 2 * i + 2 * size);
  for (i = size / 2; i < size; i++)
  ...
  for (i = size / 2; i < size; i++)
    AlwaysAssertExit(ta[i][0] == 2 * (i - size / 2) + 1 &&
                     ta[i][1] == 2 * (i - size / 2) + 1 + size &&
                     ta[i][2] == 2 * (i - size / 2) + 1 + 2 * size);
  AlwaysAssertExit(ta[0][0] == 2 && ta[0][1] == 2 + size && ta[0][2] == 2 + 2 * size);
