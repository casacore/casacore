...
int main() {
  ...
  cout << "-----------------------------------------------" << endl;
  for (i = 0; i < ntests; i++) {
    cout << p[i] << " --> " << Regex::makeCaseInsensitive(Regex::fromPattern(p[i])) << endl;
  }
...
#define CHECKPATT(i, str) AlwaysAssertExit(RegexMatches(str, Regex(Regex::fromPattern(p[i]))))
#define CHECKNPATT(i, str) AlwaysAssertExit(!RegexMatches(str, Regex(Regex::fromPattern(p[i]))))
...
#define CHECKNPATTCI(i, str) \
  ...
  AlwaysAssertExit(Regex::makeCaseInsensitive(Regex::fromPattern("[[:alpha:]x[:bb:]]")) ==
                   "[[:alpha:]xX[:bb:]]");
