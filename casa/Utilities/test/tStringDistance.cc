  ...
  try {
    if (argc < 3) {
      cout << "Run as: tStringDistance source target [countSwap] "
           << "[ignoreBlanks] [caseInsensitive]" << endl;
      return 1;
    }
    ...
    cout << dist.source() << ' ' << dist.maxDistance() << ": match=" << dist.match(argv[2])
         << " dist=" << dist.distance(argv[2])
         << " distlr=" << StringDistance::distance(argv[1], argv[2], countSwap)
         << " distrl=" << StringDistance::distance(argv[2], argv[1], countSwap) << endl;
    cout << dist.matrix();
  } catch (exception& x) {
