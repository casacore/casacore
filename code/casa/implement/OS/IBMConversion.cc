...
void IBMConversion::toLocal (long* to, const void* from,
			     unsigned int nr)
{
#if !defined(AIPS_LITTLE_ENDIAN)
    if (sizeof(long) == SIZE_IBM_LONG) {
	memcpy (to, from, nr*sizeof(long));
	return;
    }
#endif
...
void IBMConversion::toLocal (unsigned long* to, const void* from,
			     unsigned int nr)
{ 
#if !defined(AIPS_LITTLE_ENDIAN)
    if (sizeof(unsigned long) == SIZE_IBM_ULONG) {
	memcpy (to, from, nr*sizeof(unsigned long));
	return;
    }
#endif
    const char* data = (const char*)from;
    unsigned long* last = to + nr;
    while (to < last) {
	toLocal (*to++, data);
	data += 4;
    }
...
void IBMConversion::toLocal (long long* to, const void* from,
			     unsigned int nr)
{
#if !defined(AIPS_LITTLE_ENDIAN)
    if (sizeof(long long) == SIZE_IBM_LLONG) {
	memcpy (to, from, nr*sizeof(long long));
	return;
    }
#endif
    const char* data = (const char*)from;
...
void IBMConversion::toLocal (unsigned long long* to, const void* from,
			     unsigned int nr)
{ 
#if !defined(AIPS_LITTLE_ENDIAN)
    if (sizeof(unsigned long long) == SIZE_IBM_ULLONG) {
	memcpy (to, from, nr*sizeof(unsigned long long));
	return;
    }
#endif
    const char* data = (const char*)from;
...
void IBMConversion::fromLocal (void* to, const long* from,
			       unsigned int nr)
{
    char* data = (char*)to;
    const long* last = from + nr;
    while (from < last) {
	fromLocal (data, *from++);
	data += 4;
    }
...
void IBMConversion::fromLocal (void* to, const unsigned long* from,
			       unsigned int nr)
{ 
    char* data = (char*)to;
...
void IBMConversion::fromLocal (void* to, const long long* from,
			       unsigned int nr)
{
    char* data = (char*)to;
...
void IBMConversion::fromLocal (void* to, const unsigned long long* from,
			       unsigned int nr)
{ 
    char* data = (char*)to;
