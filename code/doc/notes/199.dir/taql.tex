...
\begin{description}
  ...
  \item[ \label{TAQL:INTO}INTO table ]
       ...
       \htmlref{GIVING table}{TAQL::GIVING} clause. INTO is available
       for SQL compliance.
...
\begin{itemize}
  ...
  \item A Regex constant has to be given as \texttt{m/regex/} or
       \texttt{p/pattern/} after operator ~ or !~.
       Besides /, the characters \% and \# can also be used as
       delimiter, as long as the same delimiter is used on both sides.
       The delimiter can not be part of the regex itself.
       \begin{verbatim}
       ...
       \begin{verbatim}
         name LIKE '3c%'
       \end{verbatim}
       Note that the string does not need to be a constant.
...
transformed to a 1-dimensional array and can be used. E.g.
\begin{verbatim}
     [1:10]
\end{verbatim}
is equal to a 1D array with 10 elements with values 1..10.
...
\begin{description}
  ...
  \item[] \textbf{==, $!=$, $>$, $>=$, $<$}, and \textbf{$<=$}
       ...
       $==$ and $<>$ can be used for $!=$.
  \item[] \textbf{~, $!~$}, and \textbf{LIKE} and \textbf{NOT LIKE}.
       ...
       characters) and \_ (any character). 
...
Thus, for example, \texttt{maxs(ARRAY,[2:10])} works for virtually
any array and results in a 1-dim array.
...
\begin{description}
  ...
  \item[ \texttt{anytype ARRAY(anytype,shape)}]
