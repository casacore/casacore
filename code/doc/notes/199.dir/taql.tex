...
character (e.g. \texttt{\\in} or \texttt{m\\/s}).
...
`	TIME column in a MeasurementSet by 86400 to convert it to
 ...
 \item A (sub)expression can be followed by a simple or compound unit.
       ...
       \\Note that some units can be the same as a reserved word
...
\end{itemize}
Please note that some units can conflict with the reserved TaQL words.
An example is unit \texttt{in} (inch). In such a case the unit should
