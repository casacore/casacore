...
SELECT [[DISTINCT] column_list] [INTO table]
  ...
  [ORDERBY [DISTINCT] sort_list] [GIVING table|set]
\end{verbatim}
...
%  \item[ Binary \verb+^+, *, /, \%, +, and -] 
  ...
  \item[] \textbf{IN}
       ...
       values (see discussion of \htmlref{sets}{TAQL:SETS}).
...
These functions reduce an array to a smaller array by collapsing the
the given axes using the function. The axes are the last argument(s).
...
\begin{verbatim}
   ALL AND AS ASC DESC DISTINCT FROM GIVING
   GROUPBY HAVING IN INTO NODUPLICATES NOT
   OR ORDERBY SAVETO SELECT UNIQUE WHERE
\end{verbatim}
...
the set consists of only one set element.
