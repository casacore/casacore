...
\begin{itemize}
\item Set up a MeasurementSet: {\tt imagedemo(T);di.init_ms()}
\item Browse the MS: {\tt t:=table('MS');t.browse();}
...
t.flush()
\end{verbatim}
\item Make an image: {\tt di.inputs();di.deconvolve();}
\item Display it: {\tt di.display()}
\item Set levels on aipsview (-0.03, 0.05 works). Go through all planes. Show the axes.
\item Do a selfcal.
\begin{itemize}
\item Set a threshold: {\tt di.model.threshold:=0.030}
\item Derive the model: {\tt di.derive_model();di.derive_ft()}
\item Do the solution: {\tt di.solve_cal()}
\item Repeat the deconvolution after changing the name of the output
image: {\tt di.images.restored:='restored.1'}
