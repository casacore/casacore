...
\begin{verbatim}
  lat1 * (mean(lat1) * pi())
\end{verbatim}
because in the latter way the scalars form a scalar subexpression
which is calculated only once. Note that the subexpression parentheses
are strongly recommended. In general one cannot tell how the
expression tree is built, thus if LEL recognizes scalar
subexpressions. E.g. in
\begin{verbatim}
  mean(lat1) * pi() * lat1
\end{verbatim}
the tree may be such that the second multiplication is done before
the first one.
\\In the future LEL will be optimized to shuffle the operands where needed.
\item
...
\item
  ...
  \htmlref{\texttt{calc}}{image:calc} and
  \htmlref{\texttt{expr}}{image:expr} functions in module
  \htmlref{\texttt{image}}{image} (in script \texttt{image.g}).
  ...
  \htmlref{\texttt{expr}}{image:subim} function in the image module.
