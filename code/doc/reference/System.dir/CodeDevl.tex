...
\exe{trial} and \exe{aips} packages, creating a list with duplicates in the
\exe{<type>.dup} file. This output can be used to check if templates can
e.g. be safely be moved from some local test or application templates file to
one of the standard \exe{aips} or \exe{trial} packages. An \exe{-s} switch is
available to run in {\em system} mode, which will only indicate duplicates
that are a problem as it is. \\
The normal operation is to check \code{aips} package templates against its
repository; the check \code{trial} package templates against its and aips'
repository; to check all other package templates files agains aips' and
trial's repository templates.
...
\item[\code{\exe{-s}}]
    ...
    problem.
\item[\code{\exe{UPdup.packages}}]
    If the \exe{UPdup.packages} file is present in the directory from which
    ...
    first is the indicator for the package to be checked, the rest are
    packages, in addition to aips and trial, which repositories are used in
    the checking process. I.e. if \exe{UPdup.packages} contains a line:
\begin{verbatim}
  atnf trialdisplay
\end{verbatim}
    then the duplicates check for the \code{atnf} package will be aginat
    trial, aips and trialdisplay.
\end{description}
...
files in the \aipspp\ system. By default it runs on the \code{aips} and
\code{trial} packages. Switches are available to execute it for all packages,
...
\begin{description}
\item[\code{\exe{-a}}]
    Execute for all packages, rather than only for the \code{aips} and
    \code{trial} ones.
\item[\code{\exe{-z}}]
