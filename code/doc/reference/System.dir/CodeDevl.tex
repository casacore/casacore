...
\begin{synopsis}
   \code{\exe{duplicates} [templates file list]}
\end{synopsis}
...
\begin{synopsis}
   \code{\exe{reident} [\exe{-z}] [file [file \ldots]]}
\end{synopsis}
...
\exe{reident} also converts each entry in the input to a fromatted multi-line
output.
...
written to \file{templates} -- if the input came from \file{templates} it will
be overwritten.

Normally missing idents, idents specified as \code{0000} and the second and
...
collected at the top or bottom of the file in sort order.
