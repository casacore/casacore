...
Find files containing patterns. File list to be used in action \exe{UP}
scripts.

\subsubsection*{Synopsis}

\begin{synopsis}
   \code{\exe{UPfind} [\exe{-c}] [\exe{-r}] [\exe{-g}] [\exe{-f}]
...
\code{<type name>.found} file.

The information supplied in answer to questions asked will be stored as well,
for the use in subsequent scripts with the same \code{<type name>}.
...
\item[\code{\exe{-h}}]
    Give some useage information
\item[\code{\exe{-c}}]
    Clear all information already given for \exe{<type name>}, and re-ask it.
\item[\code{\exe{-r}}]
...
\item[\code{\exe{-f}}]
    Generate the file list from a set of \exe{file} patterns, rather than a set of
    \exe{egrep} patterns
\item[\code{\exe{-d}}]
    Debug mode
\item[\code{\exe{-n}}]
...
\item[\code{<type name>}]
    An identification name for related \exe{UP} scripts.
\end{description}
...
introduction), in \code{assert.found}.
