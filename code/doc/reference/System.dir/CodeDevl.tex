...
\noindent
The \exe{UPlock} script lists all the files listed in the \exe{<typ>.found}
file that are locked in the \aipspp\ master tree.
The command can be used to check the status of files before actually changing
or moving files, but is also valid in its own right.
...
Assuming <typ> as 'strcat'.
Getting info...
Locked files can only be found while running at the aoc master site
\end{verbatim}
\ldots
...
UPload spec
Getting info...
Remote directory [wbrouw@aips2.nrao.edu:aips++/test/found]: 
19 files in spec.scp
Uploading 19 files ....
spec.cfnm                 |          0 KB |   0.0 kB/s | ETA: 00:00:00 | 100%
\end{verbatim}
\ldots
