...
\section{\exe{exhale}}
\label{exhale}
\index{distribution -- export}
\index{code distribution -- export}
\index{exhale}
\index{ftp}
...
\item
   ...
   \file{master-\textit{mm}.\textit{nnn}.ALL.tar.gz}.  Two versions are kept,
   one corresponding to the latest version and one for the latest version
   which passed the testsuite.
...
\item
   ...
   \item
      ...
      using a specially written utility called \exeref{rcscat}).  This
      includes files rescinded files.  Files created in the master since the
      previous base release are simply copied to the archive.
   \end{itemize}
...
\item
   ...
   second line of the \file{VERSION} file.  Normally 'inhale' obtains the
   current version number from the first line, but if invoked with the
...
\section{\exe{inhale}}
\label{inhale}
\index{distribution -- import}
\index{code distribution -- import}
\index{inhale}
\index{ftp}
...
\begin{synopsis}
   \code{\exe{inhale} [\exe{-c}] [\exe{-D}] [\exe{-d}] [\exe{-l}] [\exe{-n}]
      [\exe{-r}\#] [\exe{-w}\#]}
\end{synopsis}
...
\item[\exe{-D}]
   Fetch the latest update which has passed the test suite.
...
\item
   ...
   \acct{aips2mgr} uses the SysV version of \unixexe{sum}, leading repeated
...
\label{inhold}
\index{inhold}
\index{code distribution -- synchronization}
\index{inhale -- synchronization}
\index{synchronization -- inhale}
...
\section{\exe{sneeze}}
\label{sneeze}
\index{distribution -- rebuild}
\index{code distribution -- rebuild}
\index{sneeze}
