...
\begin{itemize}
\item
   If an unwritable (i.e. read-only) copy of the file being checked out
   exists in the target directory then \exe{rao} will fail due to the
   underlying \unixexe{tar} command's inability to overwrite the file;
   the file must be removed locally before any checkout is attempted.
\end{itemize}
