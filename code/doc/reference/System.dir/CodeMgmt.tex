...
\section{\exe{ae}}
\label{ae}
\index{code management -- semaphore file deletion}
\index{checkin -- semaphore file deletion}
\index{checkout -- semaphore file deletion}
\index{semaphore file -- deletion}
\index{ae}
...
\section{\exe{ai}}
\label{ai}
\index{code management -- checkin}
\index{checkin -- source code}
\index{checkout -- source code}
\index{ai}
\index{ci}
...
\exeref{mktree}, create \aipspp\ directory hierarchy.\\
\exeref{rai}, invoke \exe{ai} remotely via \unixexe{rsh}(1).\\
...
\section{\exe{ao}}
\label{ao}
\index{code management -- checkout}
\index{checkin -- source code}
\index{checkout -- source code}
\index{ao}
\index{co}
...
\noindent
would checkout \file{Foo.cc} with an exclusive lock.  Then
...
\exeref{ax}, \aipspp\ code deletion utility.\\
\exeref{mktree}, create \aipspp\ directory hierarchy.\\
\exeref{rao}, invoke \exe{ao} remotely via \unixexe{rsh}(1).
...
\section{\exe{am}}
\label{am}
\index{code management -- descriptive text}
\index{descriptive text -- change}
\index{am}
...
\item
   ...
   is to allow \exeref{gmake} to automatically checkout the \file{makefile}
...
\section{\exe{amv}}
\label{amv}
\index{code management -- renaming}
\index{rename -- source code}
\index{amv}
\index{mv}
...
\begin{status}
   ...
   1:  initialization error\\
   2:  usage error\\
   3:  failed to get \aipsexee{ax\_master}{ax}
\end{status}
...
within a \code{setuid} script to allow a command can be executed with the real
uid.
...
\section{\exe{au}}
\label{au}
\index{code management -- update}
\index{update -- source code}
\index{au}
...
uses \unixexe{rsh} to transfer the \rcs\ version files in compressed form.
...
\exeref{ao}, \aipspp\ code checkout utility.\\
\exeref{ax}, \aipspp\ code deletion utility.\\
\exeref{rau}, update \aipspp\ sources from the master via \unixexe{rsh}(1).\\
\exeref{mktree}, create \aipspp\ directory hierarchy.
...
\section{\exe{av}}
\label{av}
\index{code management -- filename validation}
\index{filename -- validation}
\index{validation -- filename}
\index{av}
...
\section{\exe{ax}}
\label{ax}
\index{code management -- deletion}
\index{delete -- source code}
\index{ax}
\index{ax\_master}
\index{rm}
...
\exe{ax\_master} does not actually delete any anything from the \aipspp\ 
master \rcs\ repositories but instead ``rescinds'' them by renaming them so as
to hide them from the code distribution system, see \exeref{exhale}.  
Rescinded files and directories are given names of the form
\file{.MM.NNN.filename}, where \file{MM.NNN} is the \aipspp\ version number at
the time the file is rescinded (see \exeref{avers}).
...
\begin{status}
   0: success\\
   1: initialization error\\
   2: failed to get \exe{ax\_master}\\
   3: argument error
\end{status}
...
Check in \aipspp\ sources by invoking \exeref{ai} remotely via \unixexe{rsh}.
...
\exe{rai} is an \unixexe{rsh}-based front-end to \exeref{ai}.  It checks in
files by transmitting them to the \aipspp\ master host via \unixexe{rsh} as a
...
\host{aips2.nrao.edu} and it must be set up for \unixexe{rsh} access from the
local host via a suitable entry in the \file{.rhosts} file.  The programmer
...
\item
   ...
   \item
      Although it needs to invoke \unixexe{rsh} two or three times the
      overhead involved in this is less than that of one automount of a
      remote filesystem.
   \end{itemize}

\item
   When accessing \host{aips2.nrao.edu} via \unixexe{rsh} the programmer's
...
\end{verbatim}
   ...
   \noindent
   The \file{.rhosts} file for the \acct{remotename} account on
   \host{aips2.nrao.edu} should have an entry of the form
...
\end{verbatim}
   ...
   \noindent
   where \host{localhost} is the (case-sensitive) name by which the local
   host identifies itself when \unixexe{ping -s} is run on
   \host{aips2.nrao.edu} and \acct{localname} is the local account name.
...
\item
   ...
   of the fact that it runs \aipsexe{ai} remotely through an \unixexe{rsh}:
...
\end{verbatim}
      ...
      \unixexe{rsh} as well as \unixexe{ci}, so if you use \verb+^D+ when
...
\item
   \unixexe{rsh} is used as the transfer agent rather than \unixexe{ftp} since
...
Status return values:
\begin{status}
   0:  success (returned from \aipsexee{ai\_master}{ai})\\
...
Check out \aipspp\ sources by invoking \exeref{ao} remotely via \unixexe{rsh}.
...
\exe{rao} is an \unixexe{rsh}-based front-end to \exeref{ao}.  It checks out
files by running \aipsexe{ao} on the \aipspp\ master host and transmitting
them to the local host via \unixexe{rsh} as a \unixexe{gzip}'d tar file.  This
...
\item
   ...
   \item
      The overhead involved in the invokation of \unixexe{rsh} is much less
...
Status return values:
\begin{status}
   0:  success (returned from \aipsexee{ao\_master}{ao})\\
   1:  initialization error, no check out\\
\end{status}
...
Update \aipspp\ sources from the master via \unixexe{rsh}.
...
\unixexe{rsh} as a \unixexe{gzip}'d tar file.  It gives them a file ownership
...
\exe{rcscat} is a special-purpose utility which concatenates the archived \rcs
version file to the current \rcs\ version file for an \aipspp\ source.  It is
used by \exeref{exhale} to update the \aipspp\ \rcs\ archive when creating a
new base release.
...
\section{\exe{squiz}}
\label{squiz}
\index{sources - search}
\index{search - sources}
\index{code management - source search}
\index{squiz}
...
\index{file - age}
\index{directory - age}
\index{code management - file age}
\index{tract}
...
\section{\exe{xrcs}}
\label{xrcs}
\index{code management -- verification}
\index{xrcs}
