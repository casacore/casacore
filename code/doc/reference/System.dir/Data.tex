...
After you have had code check out for some time, you may want to bring it
...
conflicts occur, it does includes your changes and other changes in
...
included by other makefile to allow them to do their work. A \file{config}
...
update and error logs.

Another tool which is often useful is \code{\$(MKHIER)}. When a
fully qualified path is passed in, this will create the entire
path if it doesn't exist, e.g. a makefile rule like:
...
So for example, the makefile to just copy a table which is not updated to the
