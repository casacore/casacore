...
\index{installation -- end-user}
\index{installation -- production-line}
...
\index{installation -- consortium}
\index{installation -- code-development}
\index{code management -- configuration}
\index{checkin -- configuration}
\index{checkout -- configuration}
...
However, the \textsc{nfs} mount must be set up on every machine on which checkins
and checkouts are to be enabled, and \acct{root} privilege is required to do
this.  Therefore, your system administrator's assistance will again be
required.
