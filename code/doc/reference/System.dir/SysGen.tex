...
\label{configure}
\index{distribution -- installation}
\index{installation}
\index{system generation -- installation}
\index{configure}
...
\\    2: error fetching required utilities or installing \unixexe{gzip}
\\    3: error extracting compressed \unixexe{tar} files
\\    4: error installing the \exeref{aipsinit} scripts or databases
\\    5: error constructing the system directory tree
\\    6: error installing \unixexe{gzip}, \aipsexe{gmake}
\\    7: error installing local \file{makedefs} and \file{aipsinit} files
...
\index{installation -- dependency analysis}
\index{system generation -- dependency analysis}
\index{depend}
...
The \code{C++} header and implementation files may be specified in full or
...
\begin{itemize}
\item
   The complete list of header and implemention files for a package given in
...
\section{\exe{gmake}}
\label{gmake}
\index{distribution -- gmake}
\index{installation -- gmake}
\index{system generation -- gmake}
\index{gmake}
\index{make}
...
\label{makedefs}
\index{makedefs -- makefile database}
\index{system generation -- makedefs}
\index{database -- makefiles}
...
\subsubsection*{Library control variables}

Although \file{makedefs} is intended to be self-documenting, a set of
variables referred to as the {\em library control} variables deserves a
separate mention.
...
corresponding to the debug and optimized \code{C++}, \code{C}, and
...
\noindent
If the entry for a library has been set to ``\code{defeat}'' then the library
will not be built (and if an old version exists it will be deleted).
Otherwise, if set to ``\code{static}'' or ``\code{shared}'' the static library
will be built.  If ``\code{shared}'', the shared library will also be built
and is the one linked to.
...
\subsubsection*{Conventions for \file{makedefs} variable definitions}

Some features of the \aipspp\ programmer compilations are defined as
conventions for variable usage which are implemented within \file{makedefs}
itself.
...
\noindent
If the \code{OPT} variable is defined the default \file{makedefs} will set all
programmer compile options to be the same as used for optimized system
...
\noindent
This is a signal to the site-specific \file{makedefs} to substitute a
...
\file{makedefs} by use of \code{EXTRA\_*FLAGS}, for example
...
Strictly speaking this is not a \file{makedefs} convention, but nevertheless
...
the site-specific \file{makedefs}.  However, they need not be implemented if
...
command line then a redefinition within \file{makedefs} has no effect unless
...
programmer dependency analyses.  The default \file{makedefs} sets these, and
...
\file{makedefs} files can reset these variables as it sees fit, except that
...
\index{rules, makefile -- system generation}
\index{compilation}
\index{makefiles}
...
\index{targets}
\index{system generation -- makefiles}
\index{makefile.app}
\index{makefile.aps}
\index{makefile.chk}
\index{makefile.doc}
...
\subsection*{\file{makedefs} adjuncts}

The \file{app}, \file{imp}, \file{ftn} and \file{tst} makefiles look for and,
if found, include an adjunct \file{makedefs} file which may contain
definitions (or, in principle, rules) which apply to a particular source code
directory.  The adjunct files are architecture-specific and are primarily
intended to control compiler options on a per-sourcefile basis.
...
The file, \file{\$(AIPSCODE)/install/alpha\_gnu/aips-Tables.defs}, is included
...
\item
   ...
   \item
      \file{imp}: Rebuild a \code{C++} library from its sources updating the
   ...
   \item
      ...
      Document extraction from \code{C++} sources is then performed under
...
\item
   ...
   \item
      \file{imp}: The debug and optimized static \code{C++} object libraries
      ...
      object module, the debug and optimized \code{C++} template repositories
   ...
   \item
      ...
      generated by the \code{runtests} rule.  Also, the intermediate object
      library produced when compiling all test executables in the directory.
   \end{itemize}
...
\item
   ...
   \begin{itemize}
   \item
      \file{imp}: Build the system dependency lists for the \code{C++}
      libraries.
   \end{itemize}
...
\item
   ...
   \item
      ...
      implementation and insert it into the \code{C++} library.
   \end{itemize}
...
\item
   ...
   \item
      \file{imp}: Update the package library for all \code{C++} sources in
...
\item
   ...
   \item
      \file{imp}: Update the \code{C++} libraries.
   \end{itemize}
...
\item
   ...
   \item
      \file{imp}: \unixexe{ranlib} the \code{C++} libraries.
   \end{itemize}
...
\item
   ...
   \\ Scan \code{C++} sources building the database used by the document
...
\item
   ...
   \\ Extract documentation from \code{C++} sources.  The general rule does
   ...
   \item
      ...
      documentation from the \code{C++} sources and install it in the
...
be invoked on the \code{C++} sources (if enabled).  Note, however, that no
...
\\    0: success
\\    1: the \file{templates} file contains one or more bad entries
...
\index{compilation -- template instantiation}
\index{instantiation -- templates}
\index{system generation -- template instantiation}
\index{templates -- instantiation}

\cplusplus\ template instantiation in \aipspp
 
\subsection*{\cplusplus\ template classes}

\aipspp\ makes heavy use of \cplusplus\ template classes.  These are typically
used to implement ``container'' classes, for example \code{Set}, \code{List},
and \code{Array} classes, which contain serve to aggregate objects of another
type.

Rather than separately define, say, \code{Set} classes for \code{int},
\code{float}, or general \code{Object} data types, \cplusplus\ allows
\code{Set} to be defined in ``template'' form, say \code{Set<T>} where the
template type, \code{T}, represents some as yet unspecified data type.

The \code{Set<T>} template is said to be ``instantiated'' by the compiler for
objects of type \code{float} (say) when a piece of \cplusplus\ code declares
that it wants to use Set<float>.
...
\item[manual:]
   The \gnu\ \cplusplus\ compiler, \unixexe{g++}, must be told what templates
   to instantiate.  It accepts a small \cplusplus\ source file which includes
   the necessary class implementation and header files and declares the
   template of the desired type.
\end{description}
...
\code{C++} libraries.  The libraries themselves have names of this form, for
