...
void ImageProfileFit::fitPolynomial  (ImageInterface<Float>* pFit,
                                      ImageInterface<Float>* pResid,
                                      Int order)

{
   LogIO os(LogOrigin("ImageProfileFit", "fitPolynomial", WHERE));
   if (!itsFitRegion) {
      os << "You cannot call this function as you are averaging all profiles" << LogIO::EXCEPTION;
   }
//
...
//
/*
cerr << "Total time = " << timer2.all() << endl;
cerr << "Fitting time = " << t1 << endl;
*/
