...
void LCPolygon::defineBox()
{
	...
	throw (AipsError ("LCPolygon::LCPolygon - "
			  "lattice, x, or y is not 2-dimensional"));
    }
    ...
    // The maxima form the bounding box.
    ...
    IPosition blc(2, 0);
    IPosition trc(shape);
    if (minx > 0) blc(0) = Int(minx+0.99);
    if (miny > 0) blc(1) = Int(miny+0.99);
    if (maxx < shape(0)) trc(0) = Int(maxx);
    if (maxy < shape(1)) trc(1) = Int(maxy);
...
void LCPolygon::defineMask()
{
    ...
    // Fill the mask for all vertical lines on a pixel position.
    // Also determine the index of the last non-vertical line.
    Int prev = -1;
    for (i=0; i<nrline; i++) {
	if (!near (itsX(i), itsX(i+1))) {
	    ...
	    Int x = Int(itsX(i) + 0.99);
	    if (near (itsX(i), Float(x))) {
		x -= blc(0);
		Int ys, ye;
		if (itsY(i) < itsY(i+1)) {
		    ys = Int(itsY(i)+0.99) - blc(1);
		    ye = Int(itsY(i+1)) - blc(1);
		} else {
		    ys = Int(itsY(i+1)+0.99) - blc(1);
		    ye = Int(itsY(i)) - blc(1);
		}
		while (ys <= ye) {
		    mask(x,ys++) = True;
    ...
    // This is done by determining the crossing point of all the x-line
	...
	for (i=0; i<nrline; i++) {
		...
		// When not a pixel point, count if it crosses the segment.
		if (near (xf, itsX(i))) {
		    if (dir[i] == 1) {
			cross[nrcross++] = itsY(i);
		    }
		} else if (near(xf, itsX(i+1))) {
		    cross[nrcross++] = itsY(i+1);
		} else if ((xf > itsX(i)  &&  xf < itsX(i+1))
		       ||  (xf > itsX(i+1)  &&  xf < itsX(i))) {
		    cross[nrcross++] = a[i] * xf + b[i];
		}
	    }
	}
	...
	for (i=0; i<nrcross; i+=2) {
	    Int ys = Int (cross[i] + 0.99) - blc(1);
	    Int ye = Int(cross[i+1]) - blc(1);
	    while (ys <= ye) {
		mask(x,ys++) = True;
	    }
	}
