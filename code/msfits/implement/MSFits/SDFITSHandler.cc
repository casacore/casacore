...
void SDFITSHandler::initAll(MeasurementSet &ms, Vector<Bool> &handledCols, const Record &row)
{
	...
	TableDesc td = requiredTableDesc(handledCols, row);
...
void SDFITSHandler::initRow(Vector<Bool> &handledCols, const Record &row)
{
    ...
    Vector<String> tabCols(tab_p->tableDesc().columnNames());
    for (uInt i=0;i<tabCols.nelements();i++) {
	Int field = row.fieldNumber(tabCols(i));
	if (field >= 0) {
	    fieldMap(field) = i;
...
TableDesc SDFITSHandler::requiredTableDesc(Vector<Bool> &handledCols, const Record &row)
{
	...
	if (!handledCols(i)) {
	    ...
	    if (colName.matches("^NS_SDFITS_.*")) {
		colName = colName.after("^NS_SDFITS_");
	    }
	    switch (row.type(i)) {
	    case TpBool:
