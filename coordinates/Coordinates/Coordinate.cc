...
Coordinate::Coordinate() : worldMin_p(0), worldMax_p(0) {}

Coordinate::Coordinate(const Coordinate& other)
    : worldMin_p(0), worldMax_p(0), error_p(other.error_p) {
...
String Coordinate::formatQuantity(String& units, Coordinate::formatType format2,
  ...
  return format(units, format2, worldValue.getValue(Unit(worldAxisUnits()(worldAxis))), worldAxis,
...
//
{
  ...
  } else if (type == Coordinate::LINEAR || type == Coordinate::SPECTRAL ||
...
Bool Coordinate::toWorldWCS(Vector<Double>& world, const Vector<Double>& pixel,
  ...
  if (nAxes <= NAXES_THRESHOLD) {
    ...
    iret = wcsp2s(&wcs, 1, nAxes, pixelStore, imgCrd, &phi, &theta, worldStore, &stat);
  } else {
    Block<double> imgCrd(nAxes);
    iret = wcsp2s(&wcs, 1, nAxes, pixelStore, imgCrd.storage(), &phi, &theta, worldStore, &stat);
  }
...
Bool Coordinate::toPixelWCS(Vector<Double>& pixel, const Vector<Double>& world,
  ...
  if (nAxes <= NAXES_THRESHOLD) {
    thread_local static double imgCrd[NAXES_THRESHOLD];
    iret = wcss2p(&wcs, 1, nAxes, worldStore, &phi, &theta, imgCrd, pixelStore, &stat);
  } else {
    Block<double> imgCrd(nAxes);
    iret = wcss2p(&wcs, 1, nAxes, worldStore, &phi, &theta, imgCrd.storage(), pixelStore, &stat);
  }
