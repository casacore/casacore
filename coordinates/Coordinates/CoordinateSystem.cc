...
CoordinateSystem::CoordinateSystem()
    : coordinates_p(0),
      world_maps_p(0),
      world_tmps_p(0),
      world_replacement_values_p(0),
      pixel_maps_p(0),
      pixel_tmps_p(0),
      pixel_replacement_values_p(0),
      worldAxes_tmps_p(0),
      pixelAxes_tmps_p(0),
      worldOut_tmps_p(0),
      pixelOut_tmps_p(0),
...
void CoordinateSystem::subImageInSitu(const Vector<Float>& originShift,
  ...
  for (uInt i = 0; i < n; i++) {
    ...
    if (cType == Coordinate::SPECTRAL && spectralCoordinate(coordinate).worldValues().size() > 0) {
      ...
      SpectralCoordinate newCoord(baseType, newWorldValues, spCoord.restFrequency());
...
const DirectionCoordinate& CoordinateSystem::directionCoordinate() const {
  if (!hasDirectionCoordinate()) {
    throw AipsError(String(__FUNCTION__) + ": Coordinate system has no direction coordinate");
  }
...
const SpectralCoordinate& CoordinateSystem::spectralCoordinate() const {
  if (!this->hasSpectralAxis()) {
    throw AipsError(String(__FUNCTION__) + ": Coordinate system has no spectral coordinate");
  }
...
const StokesCoordinate& CoordinateSystem::stokesCoordinate() const {
  if (!this->hasPolarizationCoordinate()) {
    throw AipsError(String(__FUNCTION__) + ": Coordinate system has no polarization coordinate");
  }
...
Coordinate::Type CoordinateSystem::type() const { return Coordinate::COORDSYS; }
...
Bool CoordinateSystem::toMix(Vector<Double>& worldOut, Vector<Double>& pixelOut,
  ...
  for (uInt i = 0; i < nCoord; i++) {
    ...
    //
    if (!coordinates_p[i]->toMix(*(worldOut_tmps_p[i]), *(pixelOut_tmps_p[i]), *(world_tmps_p[i]),
  ...
  for (uInt i = 0; i < nc; i++) {
    ...
    for (j = 0; j < nwa; j++) {
      ...
      if (where >= 0) {
        ...
        world_tmps_p[i]->operator()(j) = world_replacement_values_p[i]->operator()(j);
        worldOut_tmps_p[i]->operator()(j) = coordinates_p[i]->referenceValue()(j);  // Use refval
      }
    }

    // Convert for this coordinate.

    coordinates_p[i]->makeWorldAbsoluteRef(*(world_tmps_p[i]), *(worldOut_tmps_p[i]));
...
Bool CoordinateSystem::convert(Matrix<Double>& coordsOut, const Matrix<Double>& coordsIn,
  ...
  for (uInt j = 0; j < nCoords; j++) {
    ...
    if (allPixIn && allAbsIn) {
      ...
      if (!cSysIn.toMix(absWorldOut, absPixelOut, absWorldIn, absPixelIn, worldAxesIn, pixelAxesIn,
                        worldMin, worldMax)) {
        set_error(cSysIn.errorMessage());
        cleanUpSpecCoord(specCoordsIn, specCoordsOut);
        return False;
...
String CoordinateSystem::format(String& units, Coordinate::formatType format, Double worldValue,
  ...
  return coordinates_p[coord]->format(units, format, worldValue, axis, isAbsolute, showAsAbsolute,
                                      precision, usePrecForMixed);
}

ObsInfo CoordinateSystem::obsInfo() const { return obsinfo_p; }
...
String CoordinateSystem::coordRecordName(uInt which) const {
  ...
  switch (coordinates_p[which]->type()) {
    ...
    case Coordinate::TABULAR:
      basename = "tabular";
      break;
    case Coordinate::QUALITY:
      basename = "quality";
      break;
    case Coordinate::COORDSYS:
      basename = "coordsys";
      break;
  }
  ostringstream onum;
...
CoordinateSystem* CoordinateSystem::restore(const RecordInterface& container,
  ...
  while (True) {
    ...
    if (subrec.isDefined(linear + num)) {
      ...
      coords.resize(nc);
      coords[nc - 1] = DirectionCoordinate::restore(subrec, direction + num);
    ...
    if (axes(i)) {
      ...
      if (coord == Int(which)) {
        ...
        if (worldAxis < 0) {
          ...
          //               os << LogIO::WARN << "This does not affect the Fourier Transform" <<
          //               LogIO::POST;
        }
      }
    }
...
Vector<String> CoordinateSystem::list(LogIO& os, MDoppler::Types doppler,
  ...
  getFieldWidths(os, widthAxis, widthCoordType, widthCoordNumber, widthName, widthProj, widthShape,
                 ...
                 nameCoordType, nameCoordNumber, nameName, nameProj, nameShape, nameTile,
                 nameRefValue, nameRefPixel, nameInc, nameUnits, doppler, latticeShape, tileShape);
  ...
  for (uInt i = 0; i < cSys.nCoordinates(); i++) {
    ...
    if (!noWorld || !noPixel) {
      ...
      for (uInt k = worldAxes.nelements(); k > 0;) {
        ...
        //
        if (worldAxes(k) == -1) {
          cSysOut.removeWorldAxis(worldAxesOut(k),
      ...
      for (uInt k = worldAxes.nelements(); k > 0;) {
        ...
        //
        if (pixelAxes(k) == -1) {
          cSysOut.removePixelAxis(pixelAxesOut(k),
                                  refPix(k));  // Assumes pixelAxes in ascending order
        }
      }
      //
      j++;
...
Vector<Int> CoordinateSystem::getWorldAxesOrder(Vector<String>& myNames, Bool requireAll,
  ...
  for (uInt i = 0; i < myNames.size(); i++) {
    ...
    if (matchedNames.size() == 0) {
      os << "No axis matches requested axis " << name << ". Image axis names are " << axisNames
         << LogIO::EXCEPTION;
    ...
    if (matchMap[axisIndex].empty()) {
      ...
      os << "Ambiguous axis specification. Both " << matchMap[axisIndex] << " and " << name
         << " match image axis name " << matchedNames[0] << LogIO::EXCEPTION;
    }
  }
  return myorder;
}
...
void CoordinateSystem::setSpectralConversion(const String frequencySystem) {
  String err;
