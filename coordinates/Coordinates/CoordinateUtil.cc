...
CoordinateSystem CoordinateUtil::defaultCoords(uInt dims) {
  switch (dims) {
    ...
    default:
      throw(
          AipsError("defaultCoords() - cannot create cordinates except "
...
uInt CoordinateUtil::addAxes(CoordinateSystem& csys, Bool direction, Bool spectral,
  ...
  ThrowIf(nExtra == 0 && !silent, "No degenerate axes specified");
  return nExtra;
}
...
Bool CoordinateUtil::makeDirectionMachine(LogIO& os, MDirection::Convert& machine,
  ...
  if (telFrom == String("UNKNOWN")) {
    os << "The output CoordinateSystem has no valid observatory name - cannot divine its position"
       << LogIO::EXCEPTION;
  }
  if (telTo == String("UNKNOWN")) {
    os << "The input CoordinateSystem has no valid observatory name - cannot divine its position"
       << LogIO::EXCEPTION;
  }
  //
...
Bool CoordinateUtil::makeFrequencyMachine(LogIO& os, MFrequency::Convert& machine, Int, Int,
  ...
  findObservatoryOrRaiseException(os, posTo, telTo);
  //
  return makeFrequencyMachine(os, machine, typeTo, typeFrom, dirTo, dirFrom, obsInfoTo.obsDate(),
                              obsInfoFrom.obsDate(), posTo, posFrom, unit);
}
...
String CoordinateUtil::axisLabel(const Coordinate& coord, uInt axis, Bool doWorld, Bool doAbs,
  ...
  if (ctype == Coordinate::DIRECTION) {
    ...
    if (doAbs) {
      ...
      base = String("Relative ") + stype + String(" ") + axisName + String(" (") + nativeUnit +
             String(")");
    }
    ...
    if (doWorld) {
      ...
      if (!doAbs) base = String("Relative ") + base + String(" (") + nativeUnit + String(")");
    }
  } else {
