...
// Create Projection (throw away projection_p ?)
   ...
   for (uint32_t i=0; i<n; i++) pars(i) = wcs.pv[i].value;
...
uint32_t DirectionCoordinate::nPixelAxes() const
{
    return 2;
}

uint32_t DirectionCoordinate::nWorldAxes() const
{
    return 2;
}
...
void DirectionCoordinate::getPrecision (int32_t& precision,
...
// Number of pixel and world axes is the same for a DirectionCoordinate
   ...
   if (nExcl > 0) {
      for (uint32_t i=0; i<nPixelAxes(); i++) {
         if (linearSearch(found, excludeAxes, int32_t(i), nExcl) >= 0) exclude[i] = true;
      }
   }
...
// Check names 
   ...
   for (uint32_t i=0; i<names_p.nelements(); i++) {
      if (!exclude[i]) {
...
// Check units
   ...
   for (uint32_t i=0; i<units_p.nelements(); i++) {
      if (!exclude[i]) {
...
// Check reference  value

   {
      ...
      for (uint32_t i=0; i<thisVal.nelements(); i++) {
         if (!exclude[i]) {
...
Coordinate* DirectionCoordinate::makeFourierCoordinate(
    const Vector<bool>& axes, const Vector<int32_t>& shape
) const {
...
bool DirectionCoordinate::cylindricalFix (int32_t shapeLong, int32_t shapeLat)
//
...
// Do something arbitrary if the shape is <= 1
      ...
      if (shape(i)<=1) {
         ...
         n2 = (shape(i) + int32_t(frac*shape(i))) / 2;
      }
//
...
//
    ...
    const uint32_t nProj = projParameters.nelements();
    uint32_t startAt = ((proj.type() == Projection::ZPN) ? 0 : 1);     // Only ZPN uses prjprm->p[0]
//
    wcs.npv = nProj;
...
void DirectionCoordinate::normalizePCMatrix()
{
    ...
    for (uint32_t i=0; i<2; i++) {
       ...
       // compute the factor
       uint32_t jStart = i*2;
       uint32_t jEnd   = jStart + 2;
       uint32_t idiag =  jStart + i;
