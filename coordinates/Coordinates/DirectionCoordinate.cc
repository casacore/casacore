...
DirectionCoordinate::DirectionCoordinate(
  ...
  //
  makeDirectionCoordinate(type_p, projection_p, lon, lat, dlon, dlat, xform, refX, refY, dLongPole,
                          dLatPole);
  setDefaultWorldMixRanges();
  setRotationMatrix();
}
...
Coordinate::Type DirectionCoordinate::type() const { return Coordinate::DIRECTION; }
...
Vector<String> DirectionCoordinate::worldAxisNames() const { return names_p; }
...
DirectionCoordinate DirectionCoordinate::convert(Quantity& angle,
                                                 MDirection::Types directionType) const {
  ThrowIf(!hasSquarePixels(),
  ...
  DirectionCoordinate converted(directionType, projection_p, refValNewFrameQ[0], refValNewFrameQ[1],
...
String DirectionCoordinate::formatLongitude(String& units, MVAngle& mVA,
  ...
  if (gtype == MDirection::GRADEC) {
    ...
    if (emptyUnits) {
      value = mVA2.get().getValue() * 24.0 / (2.0 * M_PI);
      units = "h";
    } else {
      value = mVA2.get(Unit(units)).getValue();
    }
    ...
    if (type_p == MDirection::ECLIPTIC || type_p == MDirection::MECLIPTIC ||
...
DirectionCoordinate* DirectionCoordinate::restore(const RecordInterface& container,
  ...
  if (!subrec.isDefined("projection") || !subrec.isDefined("projection_parameters")) {
    ostringstream oss;
    oss << "DirectionCoordinate::restore: "
...
//
{
  ...
  mix_vstep = 0.0;
  mix_viter = 5;
  int iret = wcsmix(&wcs_p, mixpix, mixcel, mix_vspan, mix_vstep, mix_viter, mix_world, &mix_phi,
                    &mix_theta, mix_imgcrd, mix_pixcrd);
  if (iret != 0) {
...
Coordinate* DirectionCoordinate::makeFourierCoordinate(const Vector<Bool>& axes,
  ...
  fourierUnits(namesOut[0], unitsOut[0], unitsCanon[0], Coordinate::DIRECTION, 0, units[0],
               names[0]);
  fourierUnits(namesOut[1], unitsOut[1], unitsCanon[1], Coordinate::DIRECTION, 1, units[1],
               names[1]);
...
void DirectionCoordinate::makeDirectionCoordinate(MDirection::Types directionType,
  ...
  //
  makeWCS(wcs_p, xform, proj, directionType, refX, refY, refLong * to_degrees_p[0],
          refLat * to_degrees_p[1], incLong * to_degrees_p[0], incLat * to_degrees_p[1], longPole2,
          latPole2);
}
...
//
{
  ...
  if (ierr == 0) {
    ...
    //
    os << LogIO::NORMAL << "A cylindrical coordinate update was required and applied"
       << LogIO::POST;
  } else {
