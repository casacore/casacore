...
Bool FITSCoordinateUtil::toFITSHeader(RecordInterface& header, IPosition& shape,
  ...
  if (writeWCS && Int(coordsys.nPixelAxes()) == n) {
    ...
    os << LogIO::SEVERE
       << "writeWCS && nPixelAxes() != n. Requires "
          "development!!!"
       << LogIO::POST;
  }
  ...
  if (skyCoord >= 0 && pvi_ma.nelements() > 0) {
    if (!writeWCS) {
      ...
      if (theLatAxisNum == -1) {
        ...
        os << LogIO::DEBUG1 << "Identified axis number " << theLatAxisNum + 1
           << " as latitude axis for WCS. " << s << " is the keyword name." << LogIO::POST;
      }
    }
  }
  if (specAxis >= 0) {
    const SpectralCoordinate& spec = coordsys.spectralCoordinate(specCoord);
    spec.toFITS(header, specAxis, os, oneRelative, preferVelocity, opticalVelocity,
...

{
  ...
  for (int i = 0; i < nkeys; i++) {
    ...
    if (hsize >= 19 &&  // kludge changes 'RA--SIN ' to 'RA---SIN', etc.
        header[i][0] == 'C' && header[i][1] == 'T' && header[i][2] == 'Y' && header[i][3] == 'P' &&
        header[i][4] == 'E' && (header[i][5] == '1' || header[i][5] == '2') &&
      ...
      os << LogIO::NORMAL << "Header\n"
        ...
        !(StringContains(header[i], "FREQ") || StringContains(header[i], "VRAD") ||
          StringContains(header[i], "VOPT")) &&
        hsize >= 19 && (header[i].starts_with("CTYPE1") || header[i].starts_with("CTYPE2")) &&
        header[i][15] == ' ' && header[i][16] == ' ' && header[i][17] == ' ' &&
        header[i][18] == ' ') {
      ...
      os << LogIO::NORMAL << "Header\n"
         ...
         << tmp << LogIO::POST;
               ...
               header[i][3] == 'F' && header[i][4] == 'R' && header[i][5] == 'E' &&
               header[i][6] == 'Q' && header[i][7] == ' ') {
      strncpy(tmp, header[i].c_str(), hsize + 1);
      tmp[0] = 'R';
      tmp[1] = 'E';
      tmp[2] = 'S';
      ...
      os << LogIO::NORMAL << "Header\n"
         ...
         << tmp << LogIO::POST;
               ...
               header[i][0] == 'C' && header[i][1] == 'T' && header[i][2] == 'Y' &&
               header[i][3] == 'P' && header[i][4] == 'E' &&
               (header[i][5] == '1' || header[i][5] == '2') && header[i][19] == '-' &&
               header[i][20] == 'S' && header[i][21] == 'I' && header[i][22] == 'P' &&
               header[i][23] == '\'') {
      strncpy(tmp, header[i].c_str(), hsize + 1);
      tmp[19] = '\'';
      ...
      if (StringContains(header[i], "-GLS")) {
        os << LogIO::WARN << "Note: The GLS projection is deprecated. Use SFL instead."
           << LogIO::POST;
      }
  ...
  // The wcs* functions need a writable char*, so data() is used instead of c_str() to get a
  // writeable char*.
  ...
  if (wcsfix(ctrl, &(tmpshp[0]), &wcsPtr[which], stat) > 0) {
    for (int i = 0; i < NWCSFIX; i++) {
      int err = stat[i];
      if (err > 0) {
        os << LogIO::NORMAL << wcsNames(i) << " incurred the error " << wcsfix_errmsg[err]
           << LogIO::POST;
        eCount++;
        if (i == CELFIX) {
          doAbort = True;
    ...
    if (eCount > 1 || doAbort) {
      os << LogIO::WARN << "The wcs function failures are too severe to continue ..."
         << LogIO::POST;
  ...
  if (nAxes > shape.size()) {
    ...
    os << LogIO::WARN << "WCS does only provide information for " << nAxes << " out of "
...
Bool FITSCoordinateUtil::addStokesCoordinate(CoordinateSystem& cSys, Int& stokesAxis,
  ...
  if (ok && nsub == 1) {
    ...
    String errMsg;
    if (stokesCoordinateFromWCS(os, c, stokesFITSValue, errMsg, wcsDest, stokesAxisShape,
                                warnStokes)) {
      cSys.addCoordinate(c);
    } else {
      os << LogIO::WARN << errMsg << LogIO::POST;
...
Bool FITSCoordinateUtil::addSpectralCoordinate(CoordinateSystem& cSys, Int& specAxis,
  ...
  if (ok && nsub == 1) {
    ...
    // os << LogIO::DEBUG1 << "passing empty or nonexistant spectral Coordinate axis" <<
    // LogIO::POST;
...
Bool FITSCoordinateUtil::stokesCoordinateFromWCS(LogIO& os, StokesCoordinate& coord,
  ...
  for (uInt k = 0; k < shape; k++) {
    ...
    if (stokes(k) == 0) {
      if (warnStokes) {
        os << LogIO::NORMAL << "Detected Stokes coordinate = 0; this is an unoffical" << endl;
      ...
      os << LogIO::SEVERE
         << "The FITS image Stokes axis has the unofficial percentage polarization value." << endl;
      ...
      if (warnStokes) {
        os << LogIO::SEVERE << "The FITS image Stokes axis has the unofficial spectral index value."
           << endl;
...
ObsInfo FITSCoordinateUtil::getObsInfo(LogIO& os, RecordInterface& header,
  ...
  if (mjdIsDefined) {
    ...
    if (FITSDateUtil::fromFITS(time, timeSystem, dateObsStr, timeSysStr)) {
      ...
      os << LogIO::NORMAL << "Failed to decode DATE-OBS & TIMESYS keywords - no date set"
         << LogIO::POST;
    }
  }
        ...
        if (i == 0) {
          ...
          if (printError) {
            os << LogIO::WARN << proj.name()
               << " is not known to standard FITS (it is known to WCS)." << LogIO::POST;
          }
        }
        name = name + "-" + proj.name();
        break;
  ...
  if (header.isDefined("pc")) {
    ...
    for (uInt i = 0; i < crota.nelements(); i++) {
      if (!casacore::near(crota(i), 0.0)) {
        if (rotationAxis >= 0) {
          os << LogIO::SEVERE
             << "Can only convert one non-zero"
                " angle from "
             << sprefix << "rota to pc matrix. Using the first." << LogIO::POST;
        } else {
          rotationAxis = i;
        }
      }
    ...
    if (rotationAxis >= 0 && pc.nrow() > 1) {  // can't rotate 1D!
      if (rotationAxis > 0) {
        pc(rotationAxis - 1, rotationAxis - 1) = pc(rotationAxis, rotationAxis) =
            cos(crota(rotationAxis) * M_PI / 180.0);
        ...
        os << LogIO::NORMAL
           << "Unusual to rotate about first"
              " axis."
           << LogIO::POST;
        pc(rotationAxis + 1, rotationAxis + 1) = pc(rotationAxis, rotationAxis) =
            cos(crota(rotationAxis) * M_PI / 180.0);

        // Assume sign of rotation is correct although its not on the expected axis (AIPS
        // convention)
...
//
{
  ...
  for (Int i = 0; i < nCards; i++) {
    ...
    switch (type) {
      ...
      {
        os << LogIO::WARN << "Cannot yet handle very long Ints; dropping card " << name
           << LogIO::POST;
        break;
      }
    ...
    if (subRec.isDefined("value")) {
      ...
      if (rec.isDefined(name)) {
        os << LogIO::NORMAL << "Duplicate card '" << name << "'in header - only first will be used"
           << LogIO::POST;
      } else {
  ...
  for (uInt i = 0; i < n; i++) {
    if (casacore::near(cdelt(i), 0.0)) {
      if (type == Coordinate::DIRECTION) {
        ...
        os << LogIO::WARN << "Zero increment in coordinate of type " << sType
           << " setting  to 1 deg" << LogIO::POST;
      } else {
        cdelt[i] = crval[i] * 0.1;
        os << LogIO::WARN << "Zero increment in coordinate of type " << sType
           << " setting  to refVal/10" << LogIO::POST;
      }
    }
  }
  //
