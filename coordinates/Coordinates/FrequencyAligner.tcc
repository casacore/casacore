...
FrequencyAligner<T>::FrequencyAligner(const SpectralCoordinate& specCoord, uInt nPixels,
  ...
  itsSpecCoord.setReferenceConversion(itsSpecCoord.frequencySystem(), refEpoch, pos, dir);
...
FrequencyAligner<T>::FrequencyAligner(const FrequencyAligner<T>& other)
    : itsRefFreqX(0), itsFreqX(0), itsDiffTol(0.0) {
  copyOther(other);
}
...
template <class T>
void FrequencyAligner<T>::makeMachine(const MEpoch& refEpoch, const MDirection& dir,
                                      const MPosition& pos, MFrequency::Types freqSystem,
                                      const Unit& unit) {
  ...
  if (!CoordinateUtil::makeFrequencyMachine(os, itsMachine, freqSystem,
                                            itsSpecCoord.frequencySystem(), dir, dir, refEpoch,
                                            refEpoch, pos, pos, unit)) {
    os << "A trial conversion failed - something wrong with frequency conversion machine"
       << LogIO::EXCEPTION;
  }
  //
  MeasFrame frameOut;
