...
void GaussianConvert::checkWorldAxes() {
  ...
  if (u0 != u1) {
    String msg(
        "GaussianConvert::checkWorldAxes - units of specified axes must be dimensionally "
        "consistent");
    throw(AipsError(msg));
  }
}
...
void GaussianConvert::checkCoordinateSystem() {
  if (itsCSys.nWorldAxes() < 2) {
    String msg(
        "GaussianConvert::checkCoordinateSystem - the coordinate system must have at least 2 world "
        "axes");
    throw(AipsError(msg));
  }
}
