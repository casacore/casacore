...
LinearCoordinate::LinearCoordinate(const LinearCoordinate& other) : Coordinate(other) {
...
Coordinate::Type LinearCoordinate::type() const { return Coordinate::LINEAR; }
...
LinearCoordinate* LinearCoordinate::restore(const RecordInterface& container,
  ...
  //
  LinearCoordinate* pLinear = new LinearCoordinate(axes, units, crval, cdelt, pc, crpix);
...
void LinearCoordinate::makeWCS(::wcsprm& wcs, uInt naxis, const Vector<Double>& refPix,
  ...
  AlwaysAssert(refPix.nelements() == naxis && refVal.nelements() == naxis &&
                   ...
                   units.nelements() == naxis && names.nelements() == naxis,
               AipsError);
