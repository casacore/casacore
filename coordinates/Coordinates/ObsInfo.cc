...
String ObsInfo::defaultTelescope() { return "UNKNOWN"; }

String ObsInfo::defaultObserver() { return defaultTelescope(); }
...
ObsInfo::ObsInfo(const ObsInfo &other) : RecordTransformable() { copy_other(other); }
...
String ObsInfo::telescope() const { return telescope_p; }
...
String ObsInfo::telescopePositionString() const {
  ...
  if (isTelPositionSet_p) {
    ...
    oss << "[" << pos1.getValue()[0] << "m, " << pos1.getValue()[1] << "m, " << pos1.getValue()[2]
        << "m] (ITRF)";
  }
  return oss.str();
}

String ObsInfo::observer() const { return observer_p; }
...
MEpoch ObsInfo::obsDate() const { return obsdate_p; }
...
MVDirection ObsInfo::pointingCenter() const { return pointingCenter_p; }
...
ostream &operator<<(ostream &os, const ObsInfo &info) {
  ...
  os << " Observer: " << info.observer() << " Date Observed: " << info.obsDate()
