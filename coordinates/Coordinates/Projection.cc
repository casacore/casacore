...
String Projection::name() const { return name(which_p); }
...
void Projection::validate(const Bool verbose) {
  ...
  if (actualSize < minSize) {
    ...
    if (!(name(which_p) == "SFL" and actualSize == 3)) {
      cerr << "Projection::validate() - " << actualSize
           << " projection parameters provided, at most " << requiredSize
           << " expected. Will try to continue ..." << endl;
    }
...
Projection::Type Projection::type(String &ctypeLong, String &ctypeLat) const {
