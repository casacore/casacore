...
SpectralCoordinate::SpectralCoordinate()
  ...
  restfreqs_p(0) = 0.0;
  makeVelocityMachine(velUnit_p, velType_p, unit_p, type_p, restfreqs_p(restfreqIdx_p));
  //
...
SpectralCoordinate::SpectralCoordinate(MFrequency::Types type, const Quantum<Double>& refVal,
  ...
  makeVelocityMachine(velUnit_p, velType_p, unit_p, type_p, restfreqs_p(restfreqIdx_p));
  //
  makeWCS(wcs_p, String("FREQ"), refPix, refVal.getValue(hz), inc.getValue(hz), 1.0,
          restfreqs_p(0));
  to_hz_p = 1.0;
  to_m_p = 0.001;
  nativeType_p = SpectralCoordinate::FREQ;
...
Coordinate::Type SpectralCoordinate::type() const { return Coordinate::SPECTRAL; }
...
void SpectralCoordinate::setFrequencySystem(MFrequency::Types type, Bool verbose) {
  ...
  type_p = type;
  deleteVelocityMachine();
  makeVelocityMachine(String("km/s"), velType_p, unit_p, type_p, restfreqs_p(restfreqIdx_p));
  ...
  if (verbose && oldType != conversionType_p) {
    ...
    os << LogIO::WARN << "Resetting the conversion frequency system "
       << MFrequency::showType(conversionType_p) << endl;
...
Bool SpectralCoordinate::near(const Coordinate& other, const Vector<Int>& excludeAxes,
  ...
  // Tabular spectral coordinates with 1 channel has increment 0. by definition in
  // TabularCoordinates ! so linear transform test is bound to fail so skip for that case
...
//
{
  ...
  if (subrec.isDefined("conversion")) {
    ...
    if (!pSpectral->setReferenceConversion(conversionFreqSys, mhE.asMEpoch(), mhP.asMPosition(),
                                           mhD.asMDirection())) {
      delete pSpectral;
...
void SpectralCoordinate::toFITS(RecordInterface& header, uInt whichAxis, LogIO& logger,
  ...
  if (preferVelocity && preferWavelength) {
    throw AipsError(
        "Cannot export spectral axis for velocity AND wavelength. You have to choose one.");
  }
  ...
  // Determine possible changes to RefFreq etc. and check if we are linear in the preferred
  ...
  for (uInt i = 0; i < pixel.nelements(); i++) {
    ...
    if (preferWavelength) {  // check if we are linear in wavelength
      if (actual > 0. && RefFreq > 0. && (RefFreq + FreqInc) > 0.) {
        actual = C::c / actual;
        linear = C::c / RefFreq + (C::c / (RefFreq + FreqInc) - C::c / RefFreq) *
                                      (linTrans * pixel(i) - (RefPix - offset));
  ...
  if (maxDeviation > 0. && gridSpacing > 0. && maxDeviation / gridSpacing > 1E-3) {
    ...
    logger << LogIO::WARN << "Spectral axis is non-linear in the requested output quantity" << endl
           ...
           << sUnit << endl
           << " or " << maxDeviation / gridSpacing * 100. << "% of the grid spacing."
           << LogIO::POST;
  }

  AlwaysAssert(FITSSpectralUtil::toFITSHeader(Ctype, Crval, Cdelt, Crpix, Cunit, HaveAlt, Altrval,
                                              Altrpix, Velref, Restfreq, Specsys, logger, RefFreq,
                                              RefPix, FreqInc, type_p, preferVelocity,
                                              VelPreference, preferWavelength, airWaveDef),
               AipsError);
  ...
  if (HaveAlt && !preferWavelength) {  // alternate representation not valid for ctype WAVE
    ...
    FITSKeywordUtil::addComment(header, "casacore non-standard usage: 4 LSD, 5 GEO, 6 SOU, 7 GAL");
  }
...
//
{
  ...
  //
  fourierUnits(namesOut[0], unitsOut[0], unitsCanon[0], Coordinate::SPECTRAL, 0, units[0],
               names[0]);
...
String SpectralCoordinate::format(String& units, Coordinate::formatType format, Double worldValue,
  ...
  if (units.empty() || unit == unitsHZ) {
    ...
    theString = Coordinate::format(units, form, worldValue, worldAxis, isAbsolute, showAsAbsolute,
    ...
    if (unit == unitsKMS_c) {  // unit consistent with velocty
      ...
      if (unit != unitsM_c) {
        throw(AipsError(
            "Requested units must be consistent with km/s, m, or Hz for a SpectralCoordinate"));
      }
