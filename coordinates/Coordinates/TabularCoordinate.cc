...
TabularCoordinate::~TabularCoordinate() { clear_self(); }

Coordinate::Type TabularCoordinate::type() const { return Coordinate::TABULAR; }
...
TabularCoordinate *TabularCoordinate::restore(const RecordInterface &container,
  ...
  if (pixels.nelements() > 0) {
    ...
    retval = new TabularCoordinate(crval(0), cdelt(0), crpix(0), units(0), axes(0));
  }
  //
  return retval;
}
...
void TabularCoordinate::makeNonLinearTabularCoordinate(const Vector<Double> &pixelValues,
  ...
  if (n < 1 || n != worldValues.nelements()) {
    throw(
        AipsError("TabularCoordinate::TabularCoordinate - illegal table "
  ...
  if (n == 1) {  // trivial case
    ...
    crval_p = worldValues(0);
    crpix_p = pixelValues(0);
    cdelt_p = (worldValues(n - 1) - worldValues(0)) / (pixelValues(n - 1) - pixelValues(0));
    matrix_p = 1.0;

    if (cdelt_p == 0.0) {
      throw(
          AipsError("TabularCoordinate - start and "
