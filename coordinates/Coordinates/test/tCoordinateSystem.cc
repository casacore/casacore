...
CoordinateSystem makeCoordinateSystem(uInt& nCoords, Vector<Int>& types, Vector<String>& sTypes,
                                      uInt& iDC, uInt& iSpC, uInt& iTC, uInt& iStC, uInt& iQuC,
                                      uInt& iLC, DirectionCoordinate& dC, SpectralCoordinate& spC,
                                      TabularCoordinate& tC, StokesCoordinate& stC,
                                      QualityCoordinate& quC, LinearCoordinate& lC);

void doit(CoordinateSystem& lc, uInt nCoords, const Vector<Int>& types,
          const Vector<String>& sTypes, const uInt iDC, const uInt iSpC, const uInt iTC,
          const uInt iStC, const uInt iQuC, const uInt iLC, const DirectionCoordinate&,
...
int main() {
  try {
    ...
    uInt iLC;
    {
      CoordinateSystem cSys = makeCoordinateSystem(nCoords, types, sTypes, iDC, iSpC, iTC, iStC,
                                                   iQuC, iLC, dC, spC, tC, stC, quC, lC);
    }
    {
      CoordinateSystem cSys1 = makeCoordinateSystem(nCoords, types, sTypes, iDC, iSpC, iTC, iStC,
                                                    iQuC, iLC, dC, spC, tC, stC, quC, lC);
      CoordinateSystem cSys2 = makeCoordinateSystem(nCoords, types, sTypes, iDC, iSpC, iTC, iStC,
                                                    iQuC, iLC, dC, spC, tC, stC, quC, lC);

      // near tests

      if (!cSys1.near(cSys1)) {
        String msg = String("Failed near test 0 because ") + cSys1.errorMessage();
        throw(AipsError(msg));
      }
      if (!cSys1.near(cSys2)) {
        String msg = String("Failed near test 1 because ") + cSys1.errorMessage();
        throw(AipsError(msg));
      }
      ...
      if (!cSys1.near(cSys2, excludeAxes)) {
        String msg = String("Failed near test 2 because ") + cSys1.errorMessage();
        throw(AipsError(msg));
      }
      ...
      if (!cSys1.nearPixel(cSys1)) {
        String msg = String("Failed nearPixel test 0 because ") + cSys1.errorMessage();
        throw(AipsError(msg));
      }
      if (!cSys1.nearPixel(cSys2)) {
        String msg = String("Failed nearPixel test 1 because ") + cSys1.errorMessage();
        throw(AipsError(msg));
      }
      //
      ...
      cSys2.removePixelAxis(pAxis, 0.0);
      if (!cSys1.nearPixel(cSys2)) {
        String msg = String("Failed nearPixel test 9 because ") + cSys1.errorMessage();
        throw(AipsError(msg));
      }
    }
    {
    ...
    }
    {
      CoordinateSystem cSys = makeCoordinateSystem(nCoords, types, sTypes, iDC, iSpC, iTC, iStC,
                                                   iQuC, iLC, dC, spC, tC, stC, quC, lC);
      doit(cSys, nCoords, types, sTypes, iDC, iSpC, iTC, iStC, iQuC, iLC, dC, spC, tC, stC, quC,
           lC);
    }
    {
      CoordinateSystem cSys = makeCoordinateSystem(nCoords, types, sTypes, iDC, iSpC, iTC, iStC,
    ...
    }
    {
      ...
      xform.diagonal() = 1;
      DirectionCoordinate dc(MDirection::J2000, Projection::SIN, Quantity(0, "deg"),
                             Quantity(0, "deg"), Quantity(-1, "arcsec"), Quantity(1, "arcsec"),
                             xform, 0, 0);
      dc.setReferenceConversion(MDirection::GALACTIC);
      SpectralCoordinate sc(MFrequency::LSRK, Quantity(1500, "MHz"), Quantity(1, "kHz"), 0,
                            Quantity(1500, "MHz"));
      MEpoch epoch(Quantity(60000, "d"), MEpoch::UTC);
      MPosition position(Quantity(10, "m"), Quantity(135, "deg"), Quantity(40, "deg"),
                         MPosition::ITRF);
...
void doit(CoordinateSystem& cSys, uInt nCoords, const Vector<Int>& types,
          const Vector<String>& sTypes, const uInt iDC, const uInt iSpC, const uInt iTC,
          const uInt iStC, const uInt iQuC, const uInt iLC, const DirectionCoordinate& dC,
  ...
  // Test copy constructor
  {
    ...
    if (!cSys.near(cSys2)) {
      String msg = String("Failed copy constructor test because ") + cSys.errorMessage();
      throw(AipsError(msg));
    }
  }
...

  {
    ...
    cSys2 = cSys;
    if (!cSys.near(cSys2)) {
      String msg = String("Failed assignment test because ") + cSys.errorMessage();
      throw(AipsError(msg));
    }
  }
  ...
  if (!dC.near(cSys.directionCoordinate(uInt(iC)))) {
    msg = String("Failed directionCoordinate test because ") + dC.errorMessage();
    throw(AipsError(msg));
  }
  //
  ...
  if (!spC.near(cSys.spectralCoordinate(uInt(iC)))) {
    msg = String("Failed spectralCoordinate test because ") + spC.errorMessage();
    throw(AipsError(msg));
  }
  //
  ...
  if (!tC.near(cSys.tabularCoordinate(uInt(iC)))) {
    msg = String("Failed tabularCoordinate test because ") + tC.errorMessage();
    throw(AipsError(msg));
  }
  //
  ...
  if (!stC.near(cSys.stokesCoordinate(uInt(iC)))) {
    msg = String("Failed stokesCoordinate test because ") + stC.errorMessage();
    throw(AipsError(msg));
  }
  ...
  if (!quC.near(cSys.qualityCoordinate(uInt(iC)))) {
    msg = String("Failed qualityCoordinate test because ") + quC.errorMessage();
    throw(AipsError(msg));
  }
  ...
  if (!lC.near(cSys.linearCoordinate(uInt(iC)))) {
    msg = String("Failed linearCoordinate test because ") + lC.errorMessage();
    throw(AipsError(msg));
  }
  //
  uInt nPixelAxes = 0;
  ...
  for (uInt i = 0; i < cSys.nWorldAxes(); i++) {
    ...
    if (coordinate == -1) {
      ...
      if (axisInCoordinate >= Int(worldAxes.nelements()) || axisInCoordinate < 0) {
  ...
  for (uInt i = 0; i < cSys.nPixelAxes(); i++) {
    ...
    if (coordinate == -1) {
      ...
      if (axisInCoordinate >= Int(pixelAxes.nelements()) || axisInCoordinate < 0) {
  ...
  if (!cSys.setReferenceValue(refValues)) {
    throw(AipsError(String("Failed to set reference value because") + cSys.errorMessage()));
  }
  ...
  if (!cSys.setIncrement(inc)) {
    throw(AipsError(String("Failed to set increment because") + cSys.errorMessage()));
  }
  ...
  //
  {
    ...
    if (!cSys.setWorldAxisUnits(worldAxisUnits)) {
      throw(AipsError(String("Failed to set axis units because") + cSys.errorMessage()));
    }
  ...
  if (!cSys.setReferencePixel(refPixels)) {
    throw(AipsError(String("Failed to set reference pixel because") + cSys.errorMessage()));
  }
  ...
  if (!cSys.setLinearTransform(xform)) {
    throw(AipsError(String("Failed to set linear transform because") + cSys.errorMessage()));
  }
  ...
  //
  {
    ...
    if (cSys.nWorldAxes() != (nWorldAxes - 1) || cSys.nPixelAxes() != (nPixelAxes - 1)) {
      throw(AipsError("removeWorldAxis test failed"));
    }
    //
    cSys.restoreOriginal();
    cSys.removePixelAxis(0, 0.0);
    if (cSys.nWorldAxes() != nWorldAxes || cSys.nPixelAxes() != (nPixelAxes - 1)) {
  ...
  //
  {
    ...
    if (oI2.telescope() != oI1.telescope() || oI2.observer() != oI1.observer()) {
  ...
  //
  {
    ...
    //
    {
      ...
      //
      {
        ...
        Vector<Int> newStokes2 =
            oldStokes(IPosition(1, 0), IPosition(1, oldStokes.nelements() - 1), IPosition(1, 2));
        Vector<Int> newQual2 =
            oldQual(IPosition(1, 0), IPosition(1, oldQual.nelements() - 1), IPosition(1, 2));
...

{
  ...
  if (!cSys.toWorld(world, pixel)) {
    throw(AipsError(String("toWorld #1 conversion failed because ") + cSys.errorMessage()));
  }
  ...
  if (!cSys.toWorld(world, pixel)) {
    throw(AipsError(String("toWorld #2 conversion failed because ") + cSys.errorMessage()));
  }
  if (!cSys.toPixel(pixel, world)) {
    throw(AipsError(String("toPixel #3 conversion failed because ") + cSys.errorMessage()));
  }
  ...
  if (!cSys.toWorld(world, pixel)) {
    throw(AipsError(String("toWorld #4 conversion failed because ") + cSys.errorMessage()));
  }
  if (!cSys.toWorld(world2, iPixel)) {
    throw(AipsError(String("toWorld #5 conversion failed because ") + cSys.errorMessage()));
  }
  ...
  if (!cSys.toWorldMany(world3, pixel3, failures)) {
    throw(AipsError(String("toWorldMany conversion failed because ") + cSys.errorMessage()));
  }
  for (uInt i = 0; i < nBatch; i++) {
  ...
  if (!cSys.toPixelMany(pixel3, world3, failures)) {
    throw(AipsError(String("toPixelMany conversion failed because ") + cSys.errorMessage()));
  }
  for (uInt i = 0; i < nBatch; i++) {
...
void doit3(CoordinateSystem& cSys) {
  ...
  //
  {
    ...
    if (!allEQ(newWorldAxes, cSys.worldAxes(iDC)) || !allEQ(newPixelAxes, cSys.pixelAxes(iDC))) {
  ...
  //
  {
    ...
    if (!allEQ(pMap, pixelAxisMap) || !allEQ(pTranspose, pixelAxisTranspose)) {
...
//
{
  ...
  if (!cSys.toMix(worldOut2, pixelOut, worldIn, pixelIn, worldAxes, pixelAxes, worldMin,
                  worldMax)) {
    throw(AipsError(String("toMix 1 conversion failed because ") + cSys.errorMessage()));
  }
  ...
  if (!cSys.toMix(worldOut, pixelOut2, worldIn, pixelIn, worldAxes, pixelAxes, worldMin,
                  worldMax)) {
    throw(AipsError(String("toMix 2 conversion failed because ") + cSys.errorMessage()));
  }
  ...
  //
  if (!cSys.toMix(worldOut, pixelOut, worldIn, pixelIn, worldAxes, pixelAxes, worldMin, worldMax)) {
    throw(AipsError(String("toMix 5 conversion failed because ") + cSys.errorMessage()));
  }
  //
...
{
  {
    ...
    pixelIn = cSys.referencePixel().copy();
    //
    if (!cSys.toMix(worldOut, pixelOut, worldIn, pixelIn, worldAxes, pixelAxes, worldMin,
                    worldMax)) {
      throw(AipsError(String("toMix conversion failed because ") + cSys.errorMessage()));
    }
    if (!allNear(worldOut, cSys.referenceValue(), 1e-8)) {
...
DirectionCoordinate makeDirectionCoordinate(Bool unitsAreDegrees, MDirection::Types type) {
  ...
  xform = 0.0;
  xform.diagonal() = 1.0;
  DirectionCoordinate dC(type, proj, crval(0), crval(1), cdelt(0), cdelt(1), xform, crpix(0),
                         crpix(1), 999.0, 999.0);
  //
  if (unitsAreDegrees) {
...
LinearCoordinate makeLinearCoordinate(uInt nAxes) {
  ...
  //
  return LinearCoordinate(names, units, crval, cdelt, xform, crpix);
}
...
void doit6() {
...

  {
    ...
    if (names2(0) != String("Time") || names2(1) != String("UU") || names2(2) != String("VV")) {
