...
DirectionCoordinate makeCoordinate(MDirection::Types type, Projection& proj, Vector<Double>& crval,
                                   Vector<Double>& crpix, Vector<Double>& cdelt,
                                   Matrix<Double>& xform);

void doit(DirectionCoordinate& lc, MDirection::Types type, const Vector<String>& axisName,
          const String& axisUnit);
void doit2(DirectionCoordinate& lc, Vector<Double>& crval, Vector<Double>& crpix,
           Vector<Double>& cdelt, Matrix<Double>& xform);
...
int main() {
  ...
  try {
    ...
    //
    {
      for (uInt i = 0; i < MDirection::N_Types; i++) {
        ...
        if (gt != MDirection::GAZEL) {
          DirectionCoordinate lc = makeCoordinate(tp, proj, crval, crpix, cdelt, xform);
        }
      }
    }
    ...
    {
      DirectionCoordinate lc = makeCoordinate(MDirection::J2000, proj, crval, crpix, cdelt, xform);
      DirectionCoordinate lc2 = makeCoordinate(MDirection::J2000, proj, crval, crpix, cdelt, xform);
      if (!lc.near(lc2)) {
...

    {
      ...
      cdelt(1) = 2e-6;
      //
      DirectionCoordinate dc1(type, proj, crval(0), crval(1), cdelt(0), cdelt(1), xform, crpix(0),
                              crpix(1));
      //
      ...
      //
      DirectionCoordinate dc2(type, proj, lon, lat, dlon, dlat, xform, crpix(0), crpix(1), longPole,
                              latPole);
      //
      if (!dc1.near(dc2)) {
    ...
    {
      DirectionCoordinate lc =
          makeCoordinate(MDirection::GALACTIC, proj, crval, crpix, cdelt, xform);
      Vector<String> axisNames(2);
    ...
    {
      // getPixelArea
      DirectionCoordinate dc = makeCoordinate(MDirection::J2000, proj, crval, crpix, cdelt, xform);
      ...
      AlwaysAssert(pixelArea.getUnit() == (Quantity(1, units[0]) * Quantity(1, units[1])).getUnit(),
                   AipsError);
    }
    {
    ...
    }
    {
      ...
      DirectionCoordinate origGalactic(
          MDirection::GALACTIC, p, Quantity(0.15064170309942734, "rad"),
          Quantity(-0.0027706408478612907, "rad"), Quantity(-6, "arcsec"), Quantity(6, "arcsec"),
          xform, 1573.5, -6.5);
      ...
      MDirection origRefDir(Quantity(origRefVal[0], units[0]), Quantity(origRefVal[1], units[1]),
                            MDirection::GALACTIC);
      Quantum<Vector<Double>> expRefDir =
          MDirection::Convert(origRefDir, MDirection::J2000)().getAngle();
      Vector<Double> gotRefDir = converted.referenceValue();
      AlwaysAssert(near(gotRefDir[0], expRefDir.getValue(units[0])[0], 1e-8), AipsError);
      AlwaysAssert(near(gotRefDir[1], expRefDir.getValue(units[1])[1], 1e-8), AipsError);
      Vector<Double> origWorld;
      Vector<Double> pixel(2, 20);
      origGalactic.toWorld(origWorld, pixel);
      MDirection origDir2(Quantity(origWorld[0], units[0]), Quantity(origWorld[1], units[1]),
                          MDirection::GALACTIC);
      Quantum<Vector<Double>> expDir =
          MDirection::Convert(origDir2, MDirection::J2000)().getAngle();
      ...
      for (Int ha = 0; ha < 24; ha += 4) {
        for (Int dec = -80; dec < 85; dec += 20) {
          DirectionCoordinate j2000(MDirection::J2000, p, Quantity(ha * 15, "deg"),
          ...
          MDirection j2000RefDir(Quantity(j2000RefVal[0], units[0]),
                                 Quantity(j2000RefVal[1], units[1]), MDirection::J2000);
          DirectionCoordinate toGalactic = j2000.convert(angle, MDirection::GALACTIC);
          AlwaysAssert(allEQ(toGalactic.worldAxisUnits(), units), AipsError);
          AlwaysAssert(allEQ(toGalactic.linearTransform(), xform), AipsError);
          AlwaysAssert(toGalactic.projection().type() == p.type(), AipsError);
          AlwaysAssert(near(angle.getValue("deg"), expAngle[std::pair<Int, Int>(ha, dec)], 1e-7),
                       AipsError);
          DirectionCoordinate backToJ2000 = toGalactic.convert(angle, MDirection::J2000);
          Vector<Double> newJ2000RefVal = backToJ2000.referenceValue();
          if (newJ2000RefVal[0] < -0.1) {
            newJ2000RefVal[0] += 2 * M_PI;
          }
          AlwaysAssert(allNearAbs(newJ2000RefVal, j2000RefVal, 1e-9), AipsError);
          AlwaysAssert(near(angle.getValue("deg"), -expAngle[std::pair<Int, Int>(ha, dec)], 1e-7),
                       AipsError);
        }
      }
    }
    {
      ...
      DirectionCoordinate dc(MDirection::J2000, Projection::SIN, Quantity(1, "rad"),
                             Quantity(1, "rad"), Quantity(-6, "arcsec"), Quantity(6, "arcsec"),
                             xform, 1573.5, -6.5);
      AlwaysAssert(dc.hasSquarePixels(), AipsError);
      dc = DirectionCoordinate(MDirection::J2000, Projection::SIN, Quantity(1, "rad"),
                               Quantity(1, "rad"), Quantity(-6, "arcsec"), Quantity(-6, "arcsec"),
                               xform, 1573.5, -6.5);
      dc = DirectionCoordinate(MDirection::J2000, Projection::SIN, Quantity(1, "rad"),
                               Quantity(1, "rad"), Quantity(6, "arcsec"), Quantity(-6, "arcsec"),
      ...
      AlwaysAssert(dc.hasSquarePixels(), AipsError);
      dc = DirectionCoordinate(MDirection::J2000, Projection::SIN, Quantity(1, "rad"),
                               Quantity(1, "rad"), Quantity(7, "arcsec"), Quantity(6, "arcsec"),
                               xform, 1573.5, -6.5);
      AlwaysAssert(!dc.hasSquarePixels(), AipsError);
    }
    {
      ...
      Double refX = 0;
      Double refY = 0;
      DirectionCoordinate dc(MDirection::J2000, projection, refLong, refLat, inc, -inc, xform, refX,
                             refY);
      AlwaysAssert(!dc.isNCP(), AipsError);
      parms[1] = 1 / tan(refLat);
      projection = Projection(Projection::SIN, parms);
      dc = DirectionCoordinate(MDirection::J2000, projection, refLong, refLat, inc, -inc, xform,
                               refX, refY);
      AlwaysAssert(dc.isNCP(), AipsError);
...
DirectionCoordinate makeCoordinate(MDirection::Types type, Projection& proj, Vector<Double>& crval,
  ...
  xform.diagonal() = 1.0;
  proj = Projection::SIN;
  return DirectionCoordinate(type, proj, crval(0), crval(1), cdelt(0), cdelt(1), xform, crpix(0),
                             crpix(1), 999.0, 999.0);
}

void doit(DirectionCoordinate& lc, MDirection::Types type, const Vector<String>& axisNames,
          const String& axisUnit) {
...
void doit3(DirectionCoordinate& lc) {
  ...
  if (!lc.setWorldAxisUnits(axisUnits)) {
    throw(AipsError(String("failed to set world axis units to radians because ") +
                    lc.errorMessage()));
  }
  //
  pixel(0) = 12.2;
  ...
  axisUnits.set("deg");
  if (!lc.setWorldAxisUnits(axisUnits)) {
    throw(AipsError(String("failed to set world axis units to degrees because ") +
                    lc.errorMessage()));
  }
  if (!allNear(dirV.get(), world, 1e-6)) {
    throw(AipsError("Coordinate conversion values (MVDirection) are wrong"));
  ...
  if (!lc.toPixel(pixel4, converted)) {
    throw(AipsError(String("toPixel conversion (3) after conversion failed because ") +
                    lc.errorMessage()));
  }
...
//
{
  ...
  //
  if (!dC.toMix(worldOut, pixelOut, worldIn, pixelIn, worldAxes, pixelAxes, minWorld, maxWorld)) {
    throw(AipsError(String("Failed pixel->world conversion failed because ") + dC.errorMessage()));
  }
...
void doit6() {
  ...
  Double latPole = -90;
  //
  DirectionCoordinate dc(type, proj, crval(0) * M_PI / 180.0, crval(1) * M_PI / 180.0,
                         cdelt(0) * M_PI / 180.0, cdelt(1) * M_PI / 180.0, xform, crpix(0),
  ...
  //
  DirectionCoordinate dc2(type, proj, refLong, refLat, incLong, incLat, xform, crpix(0), crpix(1),
                          longPole2, latPole2);
  //
  Vector<Double> poles2 = dc2.longLatPoles();
  ok =
      (near(poles2(0), refLong.getValue(Unit("deg"))) &&
       near(poles2(1), refLat.getValue(Unit("deg"))) && near(poles2(2), longPole2.getValue("deg")));
...
void doit7() {
  ...
  Vector<Double> world;
  if (!lc.toWorld(world, pixel)) {
    throw(AipsError(String("toWorld + type change conversion (1) failed because ") +
                    lc.errorMessage()));
  }
  //
  Vector<Double> pixel2;
  if (!lc.toPixel(pixel2, world)) {
    throw(AipsError(String("toPixel + type change conversion (1) failed because ") +
                    lc.errorMessage()));
  }
...
void doit9() {
  ...
  xform = 0.0;
  xform.diagonal() = 1.0;
  DirectionCoordinate dc(MDirection::SUN, Projection::SIN, 0.0, 0.0, -1.0e-6, 1.0e-6, xform, 0.0,
                         0.0, 999.0, 999.0);
...
void doit10() {
  ...
  xform = 0.0;
  xform.diagonal() = 1.0;
  DirectionCoordinate dc(MDirection::GALACTIC, Projection::SIN, 0.0, 0.0, -1.0e-6, 1.0e-6, xform,
                         0.0, 0.0, 999.0, 999.0);
  //
  Vector<String> units = dc.worldAxisUnits().copy();
  units(0) = String("deg");
