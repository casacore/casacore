...
int main() {
  try {
    ...
    SpectralCoordinate lc = makeLinearCoordinate(sysIn, f0, finc, refchan, restFreq, unit);
...

    {
      ...
      AlwaysAssert(!fa.align(yOut, maskOut, yIn, maskIn, epoch, useCachedX, method, extrapolate),
                   AipsError);
      /*
               cerr << "   yIn = " << yIn << endl;
               cerr << "   yOut = " << yOut << endl;
