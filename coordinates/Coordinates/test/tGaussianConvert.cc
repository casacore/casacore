...
void doit(Double majorPixels, Double minorPixels, const Quantum<Double>& pa1,
          const CoordinateSystem& cSys, const Vector<uInt>& worldAxes, Double exMinorWorld,
          Double exMajorWorld, Double exPAWorld);

void doit2(Vector<Double>& pixel, const CoordinateSystem& cSys, const Vector<uInt>& worldAxes);
...
int main() {
  try {
    ...
    //
    {
      ...
      doit(majorPixels, minorPixels, pa1, cSys, worldAxes, 5.0, 10.0, 30.0);
    }
    {
      Vector<Double> deltas(2);
      deltas(0) = 1.0;
      ...
      doit(majorPixels, minorPixels, pa1, cSys, worldAxes, 5.0, 10.0, 150.0);
    }
    {
      Vector<Double> deltas(2);
      deltas(0) = -1.0;
      ...
      doit(majorPixels, minorPixels, pa1, cSys, worldAxes, 10.0, 20.0, 0.0);
    }
    {
      Vector<Double> deltas(2);
      deltas(0) = -1.0;
      ...
      doit(majorPixels, minorPixels, pa1, cSys, worldAxes, 10.0, 10.0, 45.0);
    }
    //
...
void doit(Double majorPixels, Double minorPixels, const Quantum<Double>& pa1,
  ...
  Quantum<Double> pa2;
  AlwaysAssert(gc.toWorld(majorWorld, minorWorld, pa2, majorPixels, minorPixels, pa1), AipsError);
  //
  // Reflect back to pixels
  //
  Double majorPixels2, minorPixels2;
  Quantum<Double> pa3;
  AlwaysAssert(gc.toPixel(majorPixels2, minorPixels2, pa3, majorWorld, minorWorld, pa2), AipsError);
  //
  cout << "Major axis : pixel, world, pixel = " << majorPixels << " " << majorWorld << " "
       << majorPixels2 << endl;
  cout << "Minor axis : pixel, world, pixel = " << minorPixels << " " << minorWorld << " "
       << minorPixels2 << endl;
  cout << "Position Angle : pixel, world, pixel = " << pa1 << " " << pa2 << " " << pa3 << endl
       << endl;
  //
