  ...
  try {
    ...
    for (Int i = 0; i < Projection::N_PROJ; i++) {
...

      {
        ...
        String ctypeLat("DEC--");
        //
        Projection proj2(ctypeLong.append(p), ctypeLat.append(p), pars);
      }
    }
...

    {
      ...
      proj2 = proj;
      //
      if (proj2.name() != proj.name() || !allEQ(proj2.parameters(), proj.parameters()) ||
          proj2.type() != proj.type() || !proj2.near(proj, 1e-6)) {
    ...
    //
    {
      ...
      if (proj2.name() != proj.name() || !allEQ(proj2.parameters(), proj.parameters()) ||
          proj2.type() != proj2.type() || !proj2.near(proj, 1e-6)) {
