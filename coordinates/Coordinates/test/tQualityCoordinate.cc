...
void doit(QualityCoordinate& lc, const Vector<Int>& whichQuality, Bool verbose) {
  // Test copy constructor
  {
    ...
    if (verbose) cout << "Passed copy constructor test!" << endl;
  }
  ...
  // Test assignment
  {
    ...
    if (verbose) cout << "Passed assignment test!" << endl;
  }
  ...
  if (verbose) cout << "Passed type test!" << endl;

  //
  if (lc.showType() != "Quality") {
    throw(AipsError("Failed showType test"));
  }
  if (verbose) cout << "Passed showType test!" << endl;

  //
  if (lc.nPixelAxes() != 1) {
    throw(AipsError("Failed nPixelAxes test"));
  }
  if (verbose) cout << "Passed nPixelAxes test!" << endl;

  //
  if (lc.nWorldAxes() != 1) {
    throw(AipsError("Failed nWorldAxes test"));
  }
  if (verbose) cout << "Passed nWorldAxes test!" << endl;
  ...
  if (verbose) cout << "Passed world axis name recovery test!" << endl;
  ...
  if (verbose) cout << "Failed as expected as world axis units can not be set/recovered!" << endl;

  //
  if (!allEQ(whichQuality, lc.quality())) {
    throw(AipsError("Failed Quality recovery test"));
  }
  if (verbose) cout << "Passed quality recovery test!" << endl;
  ...
  if (verbose) cout << "Passed coordinate saving to Record !" << endl;
  ...
  if (verbose) cout << "Passed coordinate reflection through record interface!" << endl;
  delete plc;
  ...
  if (verbose) cout << "Passed clone function!" << endl;
  delete plc2;
}
...
void doit2(QualityCoordinate& lc, const Vector<Int>& whichQuality, Bool verbose) {
  ...
  if (verbose) cout << "Passed to set reference value!" << endl;

  if (!allEQ(oldRefVal, lc.referenceValue())) {
    throw(AipsError("Failed reference value set/recovery test"));
  }
  if (verbose) cout << "Passed reference value set/recovery test!" << endl;
  ...
  if (verbose) cout << "Passed to set increment!" << endl;

  //
  if (!allEQ(oldIncr, lc.increment())) {
    throw(AipsError("Failed increment set/recovery test"));
  }
  if (verbose) cout << "Passed increment set/recovery test!" << endl;
  ...
  if (verbose) cout << "Passed to set reference pixel!" << endl;

  //
  if (!allEQ(oldRefPix, lc.referencePixel())) {
    throw(AipsError("Failed reference pixel set/recovery test"));
  }
  if (verbose) cout << "Passed reference pixel set/recovery test!" << endl;
  ...
  if (verbose) cout << "Passed to set linear transform!" << endl;
...
void doit3(QualityCoordinate& lc, const Vector<Int>& whichQuality,
  ...
  if (verbose) cout << "Passed toPixel conversion!" << endl;
  ...
  if (lc.toPixel(pixel2, world)) {
    ...
    if (verbose) cout << "Failed as expected with" << lc.errorMessage() << endl;
  }
  ...
  for (uInt i = 0; i < whichQuality.nelements(); i++) {
    ...
    if (verbose) cout << "Passed formatting!" << endl;
  }
}
...
void doit4(QualityCoordinate& lc, Bool verbose) {
  ...
  if (!failed) {
    ...
    if (verbose) cout << "Succeeded to induce forced error (1) in makeFourierCoordinate!" << endl;
  }
  delete pC;
}
