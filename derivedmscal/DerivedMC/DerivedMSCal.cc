...
DataManager* DerivedMSCal::clone() const { return new DerivedMSCal(*this); }

String DerivedMSCal::dataManagerType() const { return "DerivedMSCal"; }

Record DerivedMSCal::dataManagerSpec() const { return Record(); }

DataManagerColumn* DerivedMSCal::makeScalarColumn(const String& name, int, const String&) {
  ...
  if (name == "HA") {
    ...
    throw DataManError(name + " is an unknown scalar column for DerivedMSCal");
  }
  itsColumns.push_back(col);
  return col;
}

DataManagerColumn* DerivedMSCal::makeIndArrColumn(const String& name, int, const String&) {
  DataManagerColumn* col;
  if (name == "HADEC") {
    ...
    throw DataManError(name + " is an unknown array column for DerivedMSCal");
  }
  itsColumns.push_back(col);
  return col;
}

void DerivedMSCal::prepare() { itsEngine.setTable(table()); }

DataManager* DerivedMSCal::makeObject(const String&, const Record& spec) {
