...
void HelpMsCalUDF::showFuncsDerived(ostream& os) {
  ...
  os << "  double MSCAL.HA()             "
        " hourangle of array center"
     << endl;
  os << "  double MSCAL.HA1()            "
        " hourangle of ANTENNA1"
     << endl;
  os << "  double MSCAL.HA2()            "
        " hourangle of ANTENNA2"
     << endl;
  os << "  doublearray MSCAL.HADEC()     "
        " hourangle/declination of array center"
     << endl;
  os << "  doublearray MSCAL.HADEC1()    "
        " hourangle/declination of ANTENNA1"
     << endl;
  os << "  doublearray MSCAL.HADEC2()    "
        " hourangle/declination of ANTENNA2"
     << endl;
  os << "  doublearray MSCAL.AZEL()      "
        " azimuth/elevation of array center"
     << endl;
  os << "  doublearray MSCAL.AZEL1()     "
        " azimuth/elevation of ANTENNA1"
     << endl;
  os << "  doublearray MSCAL.AZEL2()     "
        " azimuth/elevation of ANTENNA2"
     << endl;
  os << "  doublearray MSCAL.ITRF()      "
        " direction in ITRF coordinates"
     << endl;
  os << "  double MSCAL.LAST()           "
        " local sidereal time of array center"
     << endl;
  os << "  double MSCAL.LAST1()          "
        " local sidereal time of ANTENNA1"
     << endl;
  os << "  double MSCAL.LAST2()          "
        " local sidereal time of ANTENNA2"
     << endl;
  os << "  double MSCAL.PA1()            "
        " parallactic angle of ANTENNA1"
     << endl;
  os << "  double MSCAL.PA2()            "
        " parallactic angle of ANTENNA2"
     << endl;
  os << "  doublearray MSCAL.UVWWVL()    "
        " stored UVW coordinates in wvl for reffreq"
     << endl;
  os << "  doublearray MSCAL.UVWWVLS()   "
        " stored UVW coordinates in wvl per channel"
     << endl;
  os << "  doublearray MSCAL.UVWJ2000()    "
        " calc J2000 UVW coordinates in meters"
     << endl;
  os << "  doublearray MSCAL.UVWJ2000WVL() "
        " calc J2000 UVW coordinates in wvl for reffreq"
     << endl;
  os << "  doublearray MSCAL.UVWJ2000WVLS()"
        " calc J2000 UVW coordinates in wvl per channel"
     << endl;
  os << "  doublearray MSCAL.UVWAPP()    "
        " calc Apparent UVW coordinates in meters"
     << endl;
  os << "  doublearray MSCAL.UVWAPPWVL() "
        " calc Apparent UVW coordinates in wvl for reffreq"
     << endl;
  os << "  doublearray MSCAL.UVWAPPWVLS()"
        " calc Apparent UVW coordinates in wvl per channel"
     << endl;
  os << "  double MSCAL.DELAY1()  "
        " calc delay (seconds) of ANTENNA1 w.r.t. array center"
     << endl;
  os << "  double MSCAL.DELAY2()  "
        " calc delay (seconds) of ANTENNA2 w.r.t. array center"
     << endl;
  os << "  double MSCAL.DELAY1()  "
        " calc delay (seconds) of ANTENNA1 w.r.t. ANTENNA2"
     << endl;
}
...
void HelpMsCalUDF::showFuncsSelection(ostream& os) {
  ...
  os << "  bool MSCAL.BASELINE (string) "
        " select using a baseline string"
     << endl;
  os << "  bool MSCAL.TIME (string)     "
        " select using a time string"
     << endl;
  os << "  bool MSCAL.FIELD (string)    "
        " select using a field string"
     << endl;
  os << "  bool MSCAL.FEED (string)     "
        " select using a feed string"
     << endl;
  os << "  bool MSCAL.SCAN (string)     "
        " select using a scan string"
     << endl;
  os << "  bool MSCAL.SPW (string)      "
        " select using a spectral-window string"
     << endl;
  os << "  bool MSCAL.UVDIST (string)   "
        " select using a uv-distance string"
     << endl;
  os << "  bool MSCAL.STATE (string)    "
        " select using a state string"
     << endl;
  os << "  bool MSCAL.OBS (string)      "
        " select using an observation string"
     << endl;
  os << "  bool MSCAL.ARRAY (string)    "
        " select using an array string"
     << endl;
  os << " More information about the CASA selection syntax can be found at" << endl
     << " http://casacore.github.io/casacore-notes/263.html" << endl;
}

void HelpMsCalUDF::showFuncsSubtable(ostream& os) {
  os << "Subtable information functions:" << endl;
  os << "  MSCAL.ANT1NAME()         "
        " the name of ANTENNA1"
     << endl;
  os << "  MSCAL.ANT2NAME()         "
        " the name of ANTENNA2"
     << endl;
  os << "  MSCAL.ANT1COL('column')  "
        " for ANTENNA1 the value in given column in ANTENNA subtable"
     << endl;
  os << "  MSCAL.ANT2COL('column')  "
        " for ANTENNA2 the value in given column in ANTENNA subtable"
     << endl;
  os << "  MSCAL.STATECOL('column') "
        " for STATE_ID the value in given column in STATE subtable"
     << endl;
  os << "  MSCAL.OBSCOL('column')   "
        " for OBSERVATION_ID the value in given column in OBSERVATION subtable"
     << endl;
  os << "  MSCAL.SPWCOL('column')   "
        " for DATA_DESC_ID the value in given column in SPECTRAL_WINDOW subtable"
     << endl;
  os << "  MSCAL.POLCOL('column')   "
        " for DATA_DESC_ID the value in given column in POLARIZATION subtable"
     << endl;
  os << "  MSCAL.FIELDCOL('column') "
        " for FIELD_ID the value in given column in FIELD subtable"
     << endl;
  os << "  MSCAL.PROCCOL('column')  "
        " for PROCESSOR_ID the value in given column in PROCESSOR subtable"
     << endl;
  os << "  MSCAL.SUBCOL('subtable', 'column', 'idcolumn') " << endl
     << "    for the id-column the value in given column in given subtable" << endl;
}

UDFBase* HelpMsCalUDF::makeHELP(const String&) { return new HelpMsCalUDF(); }
...
String HelpMsCalUDF::getString(const TableExprId& id) {
  ...
  if (type.empty()) {
    ...
    os << endl << "Use 'show function mscal <type> for more information" << endl;
  ...
  if (os.str().empty()) {
    os << type << " is an unknown mscal subtype; use derived, stokes, selection or subtable"
       << endl;
  } else {
    os << endl
       << "See also section 'Special MeasurementSet functions' at" << endl
       << "http://casacore.github.io/casacore-notes/199.html" << endl;
  }
  return os.str();
}
