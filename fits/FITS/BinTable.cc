...
BinaryTable::BinaryTable(FitsInput &fitsin, FITSErrorHandler errhandler, Bool useIncrSM,
    ...
    for (uInt i = 0; i < ncol; ++i) {
      ...
      if (field(i).fieldtype() == FITS::VADESC) {
        ...
        if (vatypes_p[i] == FITS::NOVALUE) {
          cerr << "Error in VA desc format for column " << i << " : " << tform(i) << endl;
        } else {
          switch (vatypes_p[i]) {
            case FITS::LOGICAL:
  ...
  while ((kw = kwl.next())) {
    if (!kw->isreserved() || (sdfits && isSDFitsColumn(kw->kw().name()))) {
      ...
      if (kwSet.isDefined(kwname)) {
        ...
        cout << "Duplicate keyword name : " << kwname << " most recent occurrance takes precedence"
             << endl;
      ...
      switch (kw->type()) {
        ...
        case FITS::NOVALUE:
          ...
          break;
        case FITS::LOGICAL:
          kwSet.define(kwname, kw->asBool());
          break;
        case FITS::CHAR:
          kwSet.define(kwname, kw->asString());
          break;
        case FITS::STRING:
          kwSet.define(kwname, kw->asString());
          break;
        case FITS::LONG:
          kwSet.define(kwname, kw->asInt());
          break;
        case FITS::FLOAT:
          kwSet.define(kwname, kw->asFloat());
          break;
        case FITS::DOUBLE:
          kwSet.define(kwname, kw->asDouble());
          break;
        case FITS::COMPLEX:
          kwSet.define(kwname, kw->asComplex());
          break;
          //		the above types are all that should be present as keywords
        default:
          cerr << "Error: unrecognized table data type for keyword " << kwname
               << " type = " << kw->type() << endl;
  ...
  for (Int i = 0; i < nfield; i++) {
    ...
    if (td.isColumn(colname)) {
      ...
      colname = newname.str();
      cout << "Duplicate column name : " << ttype(i) << " this occurance will be named " << colname
           << endl;
    ...
    switch (field(i).fieldtype()) {
      ...
      case FITS::COMPLEX:
        if (isArray) {
          td.addColumn(
              ArrayColumnDesc<Complex>(colname, "", IPosition(1, nelem[i]), ColumnDesc::Direct));
        } else {
      ...
      case FITS::DCOMPLEX:
        if (isArray) {
          td.addColumn(
              ArrayColumnDesc<DComplex>(colname, "", IPosition(1, nelem[i]), ColumnDesc::Direct));
        } else {
      ...
      case FITS::VADESC: {
        ...
        switch (vatypes_p[i]) {
          ...
          case FITS::DCOMPLEX:
            ...
            break;
          default:
            cerr << "Error:: column " << i << " has impossible type for variable array descriptor "
                 << vatypes_p[i] << endl;
            break;
        }
      } break;
      ...
      default:
        cerr << "Error: column " << i << " has untranslatable type " << field(i).fieldtype()
    ...
    //		for integer types
    if (field(i).fieldtype() == FITS::SHORT || field(i).fieldtype() == FITS::LONG)
    ...
    switch (field(j).fieldtype()) {
      ...
      case FITS::VADESC: {
        ...
        switch (vatypes_p[j]) {
          ...
          default:
            cerr << "Error unrecognized variable array type for field " << j
                 << " type : " << vatypes_p[j] << endl;
            break;
        }
      } break;
      default:
        // NOVALUE (which shouldn't occur here
        cerr << "Error: unrecognized table data type for field " << j << endl;
        cerr << "That should not have happened" << endl;
        continue;
...
const TableDesc &BinaryTable::getDescriptor() { return currRowTab->tableDesc(); }

TableRecord &BinaryTable::getKeywords() { return currRowTab->rwKeywordSet(); }

const Table &BinaryTable::thisRow() { return (*currRowTab); }
