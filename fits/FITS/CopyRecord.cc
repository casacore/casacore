...
CopyRecordToTable::CopyRecordToTable(Table &outputTable, const RecordInterface &inputBuffer,
    ...
    if (inputMap(i) != -1) {
      ...
      switch (inputBuffer.description().type(i)) {
        ...
        case TpUChar:
          ...
          table_char[which] = new ScalarColumn<uChar>(outputTable, colnames(inputMap(i)));
        ...
        case TpArrayUChar:
          ...
          table_array_char[which] = new ArrayColumn<uChar>(outputTable, colnames(inputMap(i)));
...
CopyRecordToTable::CopyRecordToTable(const CopyRecordToTable &other) { *this = other; }

CopyRecordToTable::~CopyRecordToTable() { clearAll(); }
  ...
  for (uInt i = 0; i < n; i++) {
    ...
    if (recDesc.isScalar(i)) {
      ...
      switch (recDesc.type(i)) {
        ...
        case TpArrayUChar:
          if (options != 0) {
            tableDescription.addColumn(ArrayColumnDesc<uChar>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<uChar>(colname, options));
          }
          break;
        case TpArrayShort:
          if (options != 0) {
            tableDescription.addColumn(ArrayColumnDesc<Short>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<Short>(colname, options));
          }
          break;
        case TpArrayInt:
          if (options != 0) {
            tableDescription.addColumn(ArrayColumnDesc<Int>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<Int>(colname, options));
          }
          break;
        case TpArrayFloat:
          if (options != 0) {
            tableDescription.addColumn(ArrayColumnDesc<Float>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<Float>(colname, options));
          }
          break;
        case TpArrayDouble:
          if (options != 0) {
            tableDescription.addColumn(ArrayColumnDesc<Double>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<Double>(colname, options));
          }
          break;
        case TpArrayComplex:
          if (options != 0) {
            tableDescription.addColumn(
                ArrayColumnDesc<Complex>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<Complex>(colname, options));
          }
          break;
        case TpArrayDComplex:
          if (options != 0) {
            tableDescription.addColumn(
                ArrayColumnDesc<DComplex>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<DComplex>(colname, options));
          }
          break;
        case TpArrayString:
          if (options != 0) {
            tableDescription.addColumn(ArrayColumnDesc<String>(colname, recDesc.shape(i), options));
          } else {
            tableDescription.addColumn(ArrayColumnDesc<String>(colname, options));
          }
          break;
        default:
          AlwaysAssertExit(0);  // NOTREACHED
    ...
    if (inputMap(i) != -1) {
      ...
      switch (inputBuffer.description().type(i)) {
        case TpBool:
          ...
          out_record_bool[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpUChar:
          in_record_char[which].attachToRecord(inputBuffer, i);
          out_record_char[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpShort:
          in_record_short[which].attachToRecord(inputBuffer, i);
          out_record_short[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpInt:
          in_record_int[which].attachToRecord(inputBuffer, i);
          out_record_int[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpFloat:
          in_record_float[which].attachToRecord(inputBuffer, i);
          out_record_float[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpDouble:
          in_record_double[which].attachToRecord(inputBuffer, i);
          out_record_double[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpComplex:
          in_record_complex[which].attachToRecord(inputBuffer, i);
          out_record_complex[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpDComplex:
          in_record_dcomplex[which].attachToRecord(inputBuffer, i);
          out_record_dcomplex[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpString:
          in_record_string[which].attachToRecord(inputBuffer, i);
          out_record_string[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayBool:
          in_record_array_bool[which].attachToRecord(inputBuffer, i);
          out_record_array_bool[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayUChar:
          in_record_array_char[which].attachToRecord(inputBuffer, i);
          out_record_array_char[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayShort:
          in_record_array_short[which].attachToRecord(inputBuffer, i);
          out_record_array_short[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayInt:
          in_record_array_int[which].attachToRecord(inputBuffer, i);
          out_record_array_int[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayFloat:
          in_record_array_float[which].attachToRecord(inputBuffer, i);
          out_record_array_float[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayDouble:
          in_record_array_double[which].attachToRecord(inputBuffer, i);
          out_record_array_double[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayComplex:
          in_record_array_complex[which].attachToRecord(inputBuffer, i);
          out_record_array_complex[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayDComplex:
          in_record_array_dcomplex[which].attachToRecord(inputBuffer, i);
          out_record_array_dcomplex[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        case TpArrayString:
          in_record_array_string[which].attachToRecord(inputBuffer, i);
          out_record_array_string[which].attachToRecord(outputBuffer, inputMap(i));
          break;
        default:
          throw(AipsError("CopyRecordToRecord::CopyRecordToRecord - unknown type"));
      }
      where[inputBuffer.description().type(i)]++;
    }
  }
