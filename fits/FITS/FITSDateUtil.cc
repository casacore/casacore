  ...
  switch (style) {
    ...
    case NEW_DATEANDTIME: {
      ...
      if (abs(time.day() - timeCheck.day()) > 0.5) {
        ...
        logger << LogIO::SEVERE << "unexpected problem converting time to FITS string - "
               << "the resulting string is off by more than a day - " << LogIO::POST;
        logger << LogIO::SEVERE << "The output date string is : " << date << LogIO::POST;
        logger << LogIO::SEVERE << "please report this bug using bug()." << LogIO::POST;
      }
    } break;
    case AUTO_PICK: {
