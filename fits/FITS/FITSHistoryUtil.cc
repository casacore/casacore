  ...
  while (key) {
    if (key->type() == FITS::NOVALUE && key->kw().name() == FITS::HISTORY) {
      ...
      if (std::regex_search(tmp, groupstart)) {
        ...
        if (foundStart) {
          os << LogIO::SEVERE
             << "Cannot handle nested CASA START"
                " history keywords. Ignoring"
             << LogIO::POST;
        } else if (nFound > 0) {
        ...
        if (foundStart) {
          ...
          if (tmp != "") {
            if (tmp != groupType) {
              os << LogIO::SEVERE << "HISTORY START and END types do not match (" << groupType
                 << "," << tmp << ")" << LogIO::POST;
            }
          }
          break;
          ...
          os << LogIO::DEBUG1
             << "CASA END found in history without"
        ...
        if (nFound == 0 || (tmp.length() > 0 && tmp[0] != '>')) {
          ...
          if (!tmp.empty()) strings(nFound - 1) += tmp.substr(1, tmp.length() - 1);
        }
      }
    }
...
void FITSHistoryUtil::addHistoryGroup(FitsKeywordList &out, const vector<String> &strings,
  ...
  if (nstrings > strings.size()) {
    os << LogIO::SEVERE
       << "Asked to add more lines to history than there "
          "are strings (adjusting)."
       << LogIO::POST;
  ...
  if (aipsppFormat && (nstrings % 2 == 0)) {
    ...
    for (uInt i = 0; i < nstrings / 2; i++) {
      ...
      try {
        ...
        os << LogIO::WARN << "Problem while parsing image HISTORY: " << x.what() << endl
           << "Will try to continue ... " << LogIO::POST;
      }
    }
  } else {
    ...
    if (line >= firstLine) {
      ...
      if (aipsppFormat == thisLineFormat) {
        ...
        if (aipsppFormat) {
          ...
          FITSDateUtil::toFITS(tmp1, tmp2, time, MEpoch::UTC, FITSDateUtil::NEW_DATEANDTIME, 6);
          tmp1 += " ";
          tmp1 += priority;
          if (location != "") {
            tmp1 += " SRCCODE='";
