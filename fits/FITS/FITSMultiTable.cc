...
const RecordDesc &FITSMultiTable::description() const { return table_p->description(); }

const Record &FITSMultiTable::units() const { return table_p->units(); }

const Record &FITSMultiTable::displayFormats() const { return table_p->displayFormats(); }
...
void FITSMultiTable::next() {
  ...
  if (table_p->pastEnd()) {
    ...
    while (which_file_p < nfiles_p && !status) {
      ...
      if (!status) {
        cerr << "FITSMultiTable::next() - Problem opening : " << file_names_p(which_file_p)
...
const Record &FITSMultiTable::currentRow() const { return row_p; }
