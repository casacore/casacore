...
void showHDU(HeaderDataUnit *h) {
  ...
  for (int n = 0; n < h->dims(); n++) {
    os << LogOrigin("FITSReader", "showHDU", WHERE) << LogIO::NORMAL5 << "Axis " << (n + 1)
       << " size " << h->dim(n) << LogIO::POST;
  }
  ...
  os << LogOrigin("FITSReader", "showHDU", WHERE) << LogIO::NORMAL << oss.str() << LogIO::POST;
}

// Read the data in a Primary Group and display the first few groups
#define DOGROUP(Z)                                                                          \
  void showPrimaryGroup(PrimaryGroup<Z> &x) {                                               \
    LogIO os;                                                                               \
    int i, j;                                                                               \
    ...
    if (x.err() != HeaderDataUnit::OK) {                                                    \
      os << LogOrigin("FITSReader", "showPrimaryGroup", WHERE) << LogIO::SEVERE             \
         << "Error occured during construction process" << LogIO::POST;                     \
    }                                                                                       \
    os << LogOrigin("FITSReader", "showPrimaryGroup", WHERE) << LogIO::NORMAL << x.gcount() \
       << " groups total, display first " << nele_to_display << " elements of the first "   \
       << ngroup_to_display << " groups\n";                                                 \
    for (i = 0; i < x.gcount(); ++i) {                                                      \
      x.read();                                                                             \
      if (i < ngroup_to_display) {                                                          \
        os << "Group " << i << " parms: ";                                                  \
        for (j = 0; j < x.pcount(); ++j) os << " " << x.parm(j);                            \
        os << "\n";                                                                         \
        os << "Group " << i << " data: ";                                                   \
        for (j = 0; j < 3 * nele_to_display; j++) os << " " << x(j);                        \
        os << "... \n";                                                                     \
      }                                                                                     \
    }                                                                                       \
    os << LogIO::POST;                                                                      \
...
#define DOARRAY(Z)                                                                        \
  void showPrimaryArray(PrimaryArray<Z> &x) {                                             \
    LogIO os;                                                                             \
    int i, j, n0, n1;                                                                     \
    if (x.fitsdatasize()) {                                                               \
    ...
    showHDU(&x);                                                                          \
    if (x.err() != HeaderDataUnit::OK) {                                                  \
      os << LogOrigin("FITSReader", "showPrimaryArray", WHERE) << LogIO::SEVERE           \
         << "Error occured during construction process" << LogIO::POST;                   \
    }                                                                                     \
    os << LogOrigin("FITSReader", "showPrimaryArray", WHERE) << LogIO::NORMAL;            \
    if (x.dims() == 2) {                                                                  \
      n0 = x.dim(0) > 60 ? 60 : x.dim(0);                                                 \
      n1 = x.dim(1) > 60 ? 60 : x.dim(1);                                                 \
      for (i = 0; i < n0; ++i)                                                            \
        for (j = 0; j < n1; ++j) os << "(" << i << "," << j << ") = " << x(i, j) << "\n"; \
    }                                                                                     \
    os << LogIO::POST;                                                                    \
    delete &x;                                                                            \
  }
...
void showBinaryTable(BinaryTableExtension &x) {
  ...
  if (x.err() != 0) {
    os << LogOrigin("FITSReader", "showBinaryTable", WHERE) << LogIO::WARN << "BT ERROR! "
       << x.err() << LogIO::POST;
    return;
  }
  showHDU(&x);
  ...
  ostringstream oss;
  oss << x.nrows() << " rows, " << x.ncols() << " cols, " << x.fitsdatasize() << " bytes total\n";
  os << LogOrigin("FITSReader", "showBinaryTable", WHERE) << LogIO::NORMAL << oss.str()
     << LogIO::POST;
  ...
  int i;
  for (i = 0; i < x.ncols(); ++i) {
    oss << "Col " << i << ": " << x.field(i).nelements() << " " << x.field(i).fieldtype() << " "
        << x.ttype(i) << " " << x.tunit(i) << "\n";
  }
  os << LogOrigin("FITSReader", "showBinaryTable", WHERE) << LogIO::DEBUGGING << oss.str()
     << LogIO::POST;
  ...
  for (i = 0; i < x.ncols(); ++i) {
    ...
    if (x.field(i).fieldtype() == FITS::VADESC) {
      ...
      if (vatypes[i] == FITS::NOVALUE) {
        oss << "Error in VA desc format for column " << i << " : " << x.tform(i) << '\n';
      } else {
        switch (vatypes[i]) {
          case FITS::LOGICAL:
          ...
          case FITS::DCOMPLEX:
            ...
            break;
          default:
            oss << "Impossible VADesc type in column " << i << " : " << vatypes[i] << '\n';
            break;
        }
      }
    } else {
  ...
  for (int n = 0; n < displayLines && n < x.nrows(); ++n) {
    ...
    for (i = 0; i < displayCols && i < x.ncols(); ++i) {
      if (i != 0) oss << "| ";
      if (x.field(i).nelements() != 0) oss << x.field(i) << ' ';
      if (x.field(i).fieldtype() == FITS::VADESC) {
        ...
        if (thisva.num()) {
          switch (vatypes[i]) {
            case FITS::LOGICAL: {
              ...
              oss << vptr[0];
              for (int k = 1; k < thisva.num(); ++k) oss << ", " << vptr[k];
            } break;
            case FITS::BIT: {
            ...
            case FITS::BYTE: {
              ...
              FITS::f2l(vptr, (void *)(theheap + thisva.offset()), thisva.num());
              oss << (int)vptr[0];
              for (int k = 1; k < thisva.num(); ++k) oss << ", " << (int)vptr[k];
            } break;
            case FITS::SHORT: {
            ...
            case FITS::CHAR: {
              ...
              FITS::f2l(vptr, (void *)(theheap + thisva.offset()), thisva.num());
              for (int k = 0; k < thisva.num() && vptr[k] != '\0'; ++k) oss << vptr[k];
            } break;
            case FITS::FLOAT: {
  ...
  for (i = 0; i < x.ncols(); ++i) {
    if (vaptr[i]) {
      switch (vatypes[i]) {
        case FITS::LOGICAL:
          delete[] (FitsLogical *)vaptr[i];
          break;
        case FITS::BIT:
        case FITS::BYTE:
          delete[] (unsigned char *)vaptr[i];
          break;
        case FITS::SHORT:
          delete[] (short *)vaptr[i];
          break;
        case FITS::LONG:
          delete[] (FitsLong *)vaptr[i];
          break;
        case FITS::CHAR:
          delete[] (char *)vaptr[i];
          break;
        case FITS::FLOAT:
          delete[] (float *)vaptr[i];
          break;
        case FITS::DOUBLE:
          delete[] (double *)vaptr[i];
          break;
        case FITS::COMPLEX:
          delete[] (Complex *)vaptr[i];
          break;
        case FITS::DCOMPLEX:
          delete[] (DComplex *)vaptr[i];
          break;
        default:
          break;
      }
...
void FITSReader::listFits(const char *fitsfile) {
  ...
  LogIO os;
  os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::NORMAL << "read fitsfile=" << fitsfile
     << LogIO::POST;

  FitsInput fin(fitsfile, FITS::Disk);
  os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::NORMAL5
     << "FitsInput object created ok." << LogIO::POST;

  if (fin.err() == FitsIO::IOERR) {
    os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::WARN << "Error opening FITS input."
       << LogIO::POST;
  } else if (fin.err()) {
    os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::WARN
       << "Error reading initial record -- exiting." << LogIO::POST;
  }
  const int NMAXERRS = 5;

  int nerrs;
  for (nerrs = 0; nerrs < NMAXERRS && fin.rectype() != FITS::EndOfFile;) {
    if (fin.rectype() == FITS::HDURecord) {
      switch (fin.hdutype()) {
        case FITS::PrimaryArrayHDU:
          os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::NORMAL
             << "Primary Array HDU ------>>>" << LogIO::POST;
        ...
        case FITS::ImageExtensionHDU:
          os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::NORMAL
             << "Image Extension HDU ------>>>" << LogIO::POST;
          os << LogIO::DEBUGGING << "Data type is " << fin.datatype() << LogIO::POST;
          switch (fin.datatype()) {
            case FITS::BYTE:
        ...
        case FITS::UnknownExtensionHDU:
          os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::NORMAL
             << "Unknown Extension HDU ------>>>" << LogIO::POST;
          // os << LogIO::DEBUGGING << "Data type is " << fin.datatype() << LogIO::POST;
      ...
      os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::WARN << "Bad Record encountered"
         << LogIO::POST;
    } else if (fin.rectype() == FITS::SpecialRecord) {
      os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::WARN
         << "Special Record encountered" << LogIO::POST;
    }
    if (fin.err()) ++nerrs;
  }

  if (nerrs == NMAXERRS)
    os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::SEVERE
       << "Too many errors.  Processing terminated." << LogIO::POST;
  else
    os << LogOrigin("FITSReader", "listFits", WHERE) << LogIO::NORMAL
       << "finish processing of Header-Data Units." << LogIO::POST;
  return;
}
