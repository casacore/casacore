...
Bool FITSSpectralUtil::fromFITSHeader(Int &spectralAxis, Double &referenceChannel,
  ...
  if (!(header.isDefined(n_ctype) && header.dataType(n_ctype) == TpArrayString &&
        header.shape(n_ctype).nelements() == 1 && header.isDefined(n_crval) &&
        header.dataType(n_crval) == TpArrayDouble && header.shape(n_crval).nelements() == 1 &&
        header.isDefined(n_crpix) && header.dataType(n_crpix) == TpArrayDouble &&
        header.shape(n_crpix).nelements() == 1 && header.isDefined(n_cdelt) &&
        header.dataType(n_cdelt) == TpArrayDouble && header.shape(n_cdelt).nelements() == 1 &&
  ...
  if (header.isDefined(n_cunit)) header.get(n_cunit, cunit);
  ...
  if (header.isDefined("velref")) {
    if (header.dataType("velref") != TpInt) {
      logger << LogIO::SEVERE
             << "Illegal type for VELREF"
                ", assuming optical, topocentric"
             << LogIO::POST;
    } else {
    ...
    if (StringContains(ctype(spectralAxis), "VELO") ||
        StringContains(ctype(spectralAxis), "VRAD")) {
  ...
  if (header.isDefined("restfreq")) {
    if (header.dataType("restfreq") != TpDouble && header.dataType("restfreq") != TpFloat) {
      logger << LogIO::SEVERE << "Illegal type for RESTFREQ"
             << ", assuming 0.0 - velocity conversions will be impossible" << LogIO::POST;
    } else {
      header.get("restfreq", restFrequency);
    }
  } else if (header.isDefined("restfrq")) {
    if (header.dataType("restfrq") != TpDouble && header.dataType("restfrq") != TpFloat) {
    ...
    if (header.dataType("restwav") != TpDouble && header.dataType("restwav") != TpFloat) {
  ...
  if (header.isDefined("specsys")) {
    ...
    if (!FITSSpectralUtil::frameFromSpecsys(refFrame, specsys)) {
      logger << LogIO::WARN << "Illegal value for SPECSYS (" << specsys
             << "), will assume topocentric" << LogIO::POST;
    }
  } else if (!FITSSpectralUtil::frameFromTag(refFrame, spectralAxisQualifier, velref)) {
    if (spectralAxisQualifier == "") {
      if ((velref % 256) >= 0) {
        // no tag and velref is unrecognized
        logger << LogIO::SEVERE << "Illegal value for VELREF(" << velref << ") assuming topocentric"
               << LogIO::POST;
      }
    } else {
      // unrecognized tag
      logger << LogIO::SEVERE << "Unknown spectral reference frame " << spectralAxisQualifier
  ...
  if (ctype(spectralAxis).find("FREQ") != std::string::npos) {
    ...
    for (Int i = 0; i < nChan; i++) {
      frequencies(i) = referenceFrequency + (Double(i) - referenceChannel) * delt;
             ...
             StringContains(ctype(spectralAxis), "VOPT")) {
    if (restFrequency < 0) {
      logger << LogIO::SEVERE
    ...
    if (rval > 0. && rval + delt != 0.) {
      ...
      deltaFrequency = C::c / ((rval + delt) * to_m * refractiveIndex((rval + delt) * to_m * 1E6)) -
                       referenceFrequency;
    } else {
      logger << LogIO::SEVERE << "Zero or negative wavelength as CRVAL." << LogIO::POST;
      return False;
...
Bool FITSSpectralUtil::toFITSHeader(String &ctype, Double &crval, Double &cdelt, Double &crpix,
  ...
  if (preferWavelength) {
    ...
    if (airWavelength) {
      ...
      cdelt = roundDouble(C::c / (refFrequency + freqIncrement) /
                                  refractiveIndex(C::c / (refFrequency + freqIncrement) * 1E6) -
                              crval,
                          12);
      crpix = refChannel;
    } else {
