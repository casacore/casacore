  ...
  while (key) {
    ...
    if (!allKeywords && key->isreserved() &&
        (kwname == FITS::BITPIX || kwname == FITS::GCOUNT || kwname == FITS::NAXIS ||
         ...
         kwname == FITS::TDISP || kwname == FITS::TFIELDS || kwname == FITS::THEAP ||
         kwname == FITS::TFORM || kwname == FITS::TNULL || kwname == FITS::TSCAL ||
         kwname == FITS::TTYPE || kwname == FITS::TUNIT || kwname == FITS::TZERO ||
    ...
    switch (key->type()) {
      ...
      default:
        throw(
            AipsError("FITSTablular::keywordsFromHDU() - unknown keyword type"
                      " (cannot happen!)"));
    }
...
Record FITSTabular::nullsFromHDU(BinaryTableExtension &hdu) {
  ...
  for (uInt i = 0; i < ncol; i++) {
    ...
    if (hdu.kw(tnull, i) &&
        (hdu.field(i).fieldtype() == FITS::BYTE || hdu.field(i).fieldtype() == FITS::SHORT ||
  ...
  for (uInt i = 0; i < desc.nfields(); i++) {
    if (!desc.isArray(i)) {
      ...
      switch (desc.type(i)) {
        ...
        break;
        default:
          cerr << "Unrecognized scalar column data type in column " << desc.name(i) << " : "
               << desc.type(i) << endl;
          break;
      }
    } else {
      ...
      switch (desc.type(i)) {
        ...
        case TpArrayComplex:
          if (fixedShape) {
            td.addColumn(ArrayColumnDesc<Complex>(desc.name(i), desc.comment(i), shape, options));
          } else {
        ...
        case TpArrayDComplex:
          if (fixedShape) {
            td.addColumn(ArrayColumnDesc<DComplex>(desc.name(i), desc.comment(i), shape, options));
          } else {
        ...
        case TpArrayString:
          if (fixedShape) {
            td.addColumn(ArrayColumnDesc<String>(desc.name(i), desc.comment(i), shape, options));
          } else {
            td.addColumn(ArrayColumnDesc<String>(desc.name(i), desc.comment(i)));
          }
          break;
        default:
          cerr << "Unrecognized array column data type in column " << desc.name(i) << " : "
               << desc.type(i) << endl;
          break;
      }
    }
    ...
    if (units.isDefined(desc.name(i))) {
      td.rwColumnDesc(desc.name(i)).rwKeywordSet().define("UNIT", units.asString(desc.name(i)));
    }
    if (disps.isDefined(desc.name(i))) {
      td.rwColumnDesc(desc.name(i)).rwKeywordSet().define("DISP", disps.asString(desc.name(i)));
    }
    if (nulls.isDefined(desc.name(i))) {
      td.rwColumnDesc(desc.name(i)).rwKeywordSet().define("NULL", nulls.asInt(desc.name(i)));
    }
  }
  return td;
}
...
Bool FITSTable::reopen(const String &fileName) {
  ...
  if (raw_table_p->pcount()) {
    ...
    raw_table_p->ExtensionHeaderDataUnit::read(theheap_p, raw_table_p->pcount());
  } else {
  ...
  if (theheap_p) {
    ...
    for (i = 0; i < uInt(raw_table_p->ncols()); i++) {
      ...
      if (raw_table_p->field(i).fieldtype() == FITS::VADESC) {
        ...
        FITS::parse_vatform(raw_table_p->tform(i), vtype, maxsize);
        vatypes_p[i] = vtype;
        ...
        switch (vatypes_p[i]) {
          ...
          case FITS::DCOMPLEX:
            ...
            break;
          default:
            cerr << "Impossible VADesc type in column " << i << " : " << vatypes_p[i] << endl;
            break;
        }
      }
    }
...
void FITSTable::fill_row() {
  ...
  for (uInt i = 0; i < nfields_p; i++) {
    ...
    switch (raw_table_p->field(i).fieldtype()) {
      ...
      case FITS::COMPLEX: {
        FitsField<Complex> &fitsRef = (FitsField<Complex> &)(raw_table_p->field(i));
        if (field_types_p[i] == TpComplex) {
          RecordFieldPtr<Complex> &rowRef = *((RecordFieldPtr<Complex> *)row_fields_p[i]);
          const Complex &val = fitsRef();
          (*rowRef) = Complex(val.real() * scale + zero, val.imag() * scale + zero);
        } else {
          ...
          while (n) {
            ...
            data[n] = Complex(val.real() * scale + zero, val.imag() * scale + zero);
          }
          (*rowRef).putStorage(data, deleteIt);
        }
      } break;
      case FITS::DCOMPLEX: {
        FitsField<DComplex> &fitsRef = (FitsField<DComplex> &)(raw_table_p->field(i));
        if (field_types_p[i] == TpDComplex) {
          RecordFieldPtr<DComplex> &rowRef = *((RecordFieldPtr<DComplex> *)row_fields_p[i]);
          const DComplex &val = fitsRef();
          (*rowRef) = DComplex(val.real() * scale + zero, val.imag() * scale + zero);
        } else {
          ...
          while (n) {
            ...
            data[n] = DComplex(val.real() * scale + zero, val.imag() * scale + zero);
          }
          (*rowRef).putStorage(data, deleteIt);
        }
      } break;
      case FITS::ICOMPLEX: {
        FitsField<IComplex> &fitsRef = (FitsField<IComplex> &)(raw_table_p->field(i));
        if (field_types_p[i] == TpDComplex) {
          RecordFieldPtr<DComplex> &rowRef = *((RecordFieldPtr<DComplex> *)row_fields_p[i]);
...
Bool FITSTable::virtualColumns(const Vector<String> &keyNames) {
  ...
  for (uInt i = 0; i < keyNames.nelements(); i++) {
    ...
    if (fieldNumber >= 0) {
      ...
      row_p.setComment(keyNames(i), keywords_p.comment(keyNames(i)));
