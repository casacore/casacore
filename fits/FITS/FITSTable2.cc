...
FITSTableWriter::FITSTableWriter(FitsOutput *file, const RecordDesc &description,
  ...
  for (i = 0; i < nfields; i++) {
    ...
    if (units.isDefined(description.name(i)) && units.dataType(description.name(i)) == TpString) {
      String unitString(units.asString(description.name(i)));
      if (unitString != String("")) columns.mk(thisColumn, FITS::TUNIT, unitString.c_str());
    }
  ...
  for (i = 0; i < nfields; i++) {
    ...
    switch (description.type(i)) {
      ...
      case TpString: {
        ...
        FitsField<char> *fptr = new FitsField<char>(sizeofStringField(description, maxLengths, i));
        bintable_p->bind(whichField, *fptr);
      ...
      case TpArrayComplex: {
        ...
        FitsField<Complex> *fptr = new FitsField<Complex>(fieldSizes[i]);
        bintable_p->bind(whichField, *fptr);
        if (whichTdim >= 0) {
          FitsField<char> *tdirptr =
              new FitsField<char>(variableShapes.asString(description.name(i)).length());
      ...
      case TpArrayDComplex: {
        ...
        FitsField<DComplex> *fptr = new FitsField<DComplex>(fieldSizes[i]);
        bintable_p->bind(whichField, *fptr);
        if (whichTdim >= 0) {
          FitsField<char> *tdirptr =
              new FitsField<char>(variableShapes.asString(description.name(i)).length());
