  ...
  for (i = 0; i < nfields; i++) {
    if (description.type(i) == TpArrayFloat) {
      ...
      if (description.shape(i).product() < 0) {
        throw(
            AipsError("FitsGroupWriter::FitsGroupWriter - "
                      "Data array must be constant shape"));
      }
    } else if (description.type(i) != TpFloat) {
      throw(
          AipsError("FitsGroupWriter::FitsGroupWriter - all random"
...
void FITSGroupWriter::write() {
  ...
  if (nrows_written_p >= nrows_total_p) {
    ...
    log << LogIO::SEVERE << "You've already written all the rows!!" << LogIO::POST;
    return;
  }
...
void FITSGroupWriter::check_error(const char *extra_info) {
  ...
  if (group_p) {
    ...
    if (err != HeaderDataUnit::OK) {
      ...
      switch (err) {
        case HeaderDataUnit::NOMEM:
          os << "(NOMEM)";
          break;
        case HeaderDataUnit::MISSKEY:
          os << "(MISSKEY)";
          break;
        case HeaderDataUnit::BADBITPIX:
          os << "(BADBITPIX)";
          break;
        case HeaderDataUnit::NOAXISN:
          os << "(NOAXISN)";
          break;
        case HeaderDataUnit::NOPCOUNT:
          os << "(NOPCOUNT)";
          break;
        case HeaderDataUnit::NOGCOUNT:
          os << "(NOGCOUNT)";
          break;
        case HeaderDataUnit::BADPCOUNT:
          os << "(BADPCOUNT)";
          break;
        case HeaderDataUnit::BADGCOUNT:
          os << "(BADGCOUNT)";
          break;
        case HeaderDataUnit::NOGROUPS:
          os << "(NOGROUPS)";
          break;
        case HeaderDataUnit::BADNAXIS:
          os << "(BADNAXIS)";
          break;
        case HeaderDataUnit::BADREC:
          os << "(BADREC)";
          break;
        case HeaderDataUnit::BADTYPE:
          os << "(BADTYPE)";
          break;
        case HeaderDataUnit::BADRULES:
          os << "(BADRULES)";
          break;
        case HeaderDataUnit::BADSIZE:
          os << "(BADSIZE)";
          break;
        case HeaderDataUnit::BADOPER:
          os << "(BADOPER)";
          break;
        case HeaderDataUnit::BADCONV:
          os << "(BADCONV)";
          break;
        case HeaderDataUnit::BADIO:
          os << "(BADIO)";
          break;
        default:
          os << "(unknown error)";
    ...
    if (err != FitsIO::OK) {
      ...
      switch (err) {
        ...
        case FitsIO::MISSKEY:
          os << "(MISSKEY)";
          break;
        case FitsIO::BADBEGIN:
          os << "(BADBEGIN)";
          break;
        case FitsIO::EMPTYFILE:
          os << "(EMPTYFILE)";
          break;
        case FitsIO::NOPRIMARY:
          os << "(NOPRIMARY)";
          break;
        case FitsIO::BADOPER:
          os << "(BADOPER)";
        ...
        case FitsIO::MEMERR:
          os << "(MEMERR)";
          break;
        case FitsIO::BADBITPIX:
          os << "(BADBITPIX)";
          break;
        case FitsIO::NOAXISN:
          os << "(NOAXISN)";
          break;
        case FitsIO::NOPCOUNT:
          os << "(NOPCOUNT)";
          break;
        case FitsIO::NOGCOUNT:
          os << "(NOGCOUNT)";
          break;
        case FitsIO::BADPCOUNT:
          os << "(BADPCOUNT)";
          break;
        case FitsIO::BADGCOUNT:
          os << "(BADGCOUNT)";
          break;
        case FitsIO::NOGROUPS:
          os << "(NOGROUPS)";
          break;
        case FitsIO::BADNAXIS:
          os << "(BADNAXIS)";
          break;
        case FitsIO::BADPRIMARY:
          os << "(BADPRIMARY)";
          break;
        case FitsIO::BADSIZE:
          os << "(BADSIZE)";
