...
void BlockIO::errmsg(IOErrs e, const char *s) {
  ...
  msgline << " Physical record " << m_block_no << " logical record " << m_rec_no << " --\n\t" << s
          << endl;
  m_err_status = e;
    ...
    if (fits_open_file(&m_fptr, m_filename, iomode, &l_status)) {
      ...
      if (((m_fptr->Fptr)->bytepos) != 0) {
        if (ffmbyt(m_fptr, 0, REPORT_EOF, &l_status) != 0) {
          errmsg(OPENERR, "bytepos setting error!");
        }
      }
    }
  }
...
char *BlockInput::read() {
  ...
  if (m_current >= m_iosize) {
    ...
    // 		 << m_iosize << " " << m_blocksize << " " << m_recsize << " " << m_iosize %
    // m_recsize
    ...
    if (l_byte_left_in_file >= OFF_T(m_recsize)) {
      if (ffmbyt(m_fptr, bytepost, REPORT_EOF, &l_status) > 0) {
        errmsg(READERR, "bytepos setting error!");
      }
    } else {
...
void BlockOutput::flush_buffer() {
  ...
  // if(ffflsh(m_fptr, TRUE, &l_status)){ errmsg(WRITEERR,"[flush_buffer()] Error flushing
  // buffer!"); }
  ...
  if (m_current > 0) {
    ...
    if (ffflsh(m_fptr, TRUE, &l_status)) {
      errmsg(WRITEERR, "[flush_buffer()] Error flushing buffer!");
    }
...
//========================================================================================================
BlockInput::BlockInput(const char *f, int r, int n, FITSErrorHandler errhandler)
    : BlockIO(f, O_RDONLY, r, n, errhandler) {}

BlockInput::BlockInput(int f, int r, int n, FITSErrorHandler errhandler)
    : BlockIO(f, r, n, errhandler) {}

BlockOutput::BlockOutput(const char *f, int r, int n, FITSErrorHandler errhandler)
    : BlockIO(f, O_RDWR | O_CREAT | O_TRUNC, r, n, errhandler) {}
// replaced O_WRONLY by O_RDWR in above line.

BlockOutput::BlockOutput(int f, int r, int n, FITSErrorHandler errhandler)
    : BlockIO(f, r, n, errhandler) {}
