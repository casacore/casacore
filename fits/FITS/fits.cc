...
void FITS::f2l(long *local_addr, void *fits_addr, int number) {
  ...
  switch (sizeof(long)) {
    case 4:
      FITS::swap4(local_addr, fits_addr, number);
      break;
    case 8:
      FITS::swap8(local_addr, fits_addr, number);
      break;
  }
#else
...
const ReservedFitsKeyword &ReservedFitsKeywordCollection::match(int i, const char *s, int s_len,
  ...
  if (t == FITS::FSTRING) t = FITS::STRING;  // change t to STRING to match on types
  // match type and isindexed
  if (resword[i].type() != t) {
    while (resword[i + 1].name() == resword[i].name()) {
      ++i;
      if (resword[i].type() == t) break;
    }
    if (resword[i].type() != t) {
  ...
  if (resword[i].isindexed() != n) {
    while ((resword[i + 1].name() == resword[i].name()) &&
      ...
      ++i;
      if (resword[i].isindexed() == n) break;
    }
    if (resword[i].isindexed() != n) {
...
const ReservedFitsKeyword &ReservedFitsKeywordCollection::get(const char *s, int s_len, Bool n,
  ...
  for (; *(resword[i].aname()) == *s; ++i)  // search for a match
    if (resword[i].namesize() == s_len && strncmp(s, resword[i].aname(), s_len) == 0) break;
...
int ReservedFitsKeywordCollection::isreserved(const char *s, int s_len) const {
  ...
  if (!FITS::isa_letter(*s)) return 0;
  if ((i = resalpha[FITS::letter2bin(*s)]) == 0) return 0;
  for (; *(resword[i].aname()) == *s; ++i)  // search for a match
    if (resword[i].namesize() == s_len && strncmp(s, resword[i].aname(), s_len) == 0) break;
  if (*(resword[i].aname()) != *s || resword[i].namesize() != s_len) return 0;
  return i;
}
...
int ReservedFitsKeywordCollection::rules(const ReservedFitsKeyword &res, const char *s, int s_len,
  ...
  if (res.name() == FITS::USER_DEF) {
    for (int i = 0; i < s_len; ++i, ++s) {
      if (!(FITS::isa_letter(*s) || FITS::isa_digit(*s) || *s == '_' || *s == '-')) {
  ...
  switch (res.name()) {
    ...
    case FITS::DATE_OBS:
      ...
      if ((v_len >= 10) && FITS::isa_digit(p[0]) && FITS::isa_digit(p[1]) &&
          FITS::isa_digit(p[2]) && FITS::isa_digit(p[3]) && FITS::isa_digit(p[5]) &&
          FITS::isa_digit(p[6]) && FITS::isa_digit(p[8]) && FITS::isa_digit(p[9]) &&
          (p[4] == '-') && (p[7] == '-')) {
        // must be an attempt at the new format
        yy = FITS::digit2bin(p[0]) * 1000 + FITS::digit2bin(p[1]) * 100 +
        ...
        if (v_len > 10) {
          ...
          if (v_len >= 19 && FITS::isa_digit(p[11]) && FITS::isa_digit(p[12]) &&
              FITS::isa_digit(p[14]) && FITS::isa_digit(p[15]) && FITS::isa_digit(p[17]) &&
              FITS::isa_digit(p[18]) && (p[10] == 'T') && (p[13] == ':') && (p[16] == ':')) {
            // 24 is okay, more than that is not in hours field
            if ((FITS::digit2bin(p[11]) == 2 && FITS::digit2bin(p[12]) > 4) ||
                FITS::digit2bin(p[11]) > 2) {
              msg = "Illegal time.";
              return 1;
            }
            // 60 is okay, more than that is not in minutes field
            if ((FITS::digit2bin(p[14]) == 6 && FITS::digit2bin(p[15]) > 0) ||
                FITS::digit2bin(p[14]) > 6) {
              msg = "Illegal time.";
            }
            // 60 is okay, more than that is not in secomds field
            if ((FITS::digit2bin(p[17]) == 6 && FITS::digit2bin(p[18]) > 0) ||
      ...
      } else if (FITS::isa_digit(p[0]) && FITS::isa_digit(p[1]) && FITS::isa_digit(p[3]) &&
                 FITS::isa_digit(p[4]) && FITS::isa_digit(p[6]) && FITS::isa_digit(p[7]) &&
                 (p[2] == '/') && (p[5] == '/')) {
        // the old format
...
int ReservedFitsKeywordCollection::essential_name(const char *s, int s_len) const {
  ...
  for (; *(resword[i].aname()) == *s; ++i)  // search for a match
    if (resword[i].namesize() == s_len && strncmp(s, resword[i].aname(), s_len) == 0) break;
  if (*(resword[i].aname()) != *s || resword[i].namesize() != s_len) return 0;
  if (resword[i].isessential()) return i;
  while (resword[i + 1].name() == resword[i].name()) {
    ++i;
    if (resword[i].isessential()) return i;
  }
...
const ReservedFitsKeyword &ReservedFitsKeywordCollection::get_essential(int i, Bool n,
  ...
  if (t == FITS::FLOAT || t == FITS::DOUBLE) t = FITS::REAL;  // change t to REAL to match on types
  if (t == FITS::FSTRING) t = FITS::STRING;  // change to to STRING to match on types
  if (resword[i].type() != t) {
    while ((resword[i + 1].name() == resword[i].name()) && resword[i + 1].isessential()) {
      ++i;
      if (resword[i].type() == t) break;
    }
    if (resword[i].type() != t) {
...
void FITS::get_value(const char *s, int len, FitsValueResult &result) {
  ...
  switch (*s) {  // the first non-blank
    ...
    case '\'':  // string
      ...
      for (;;) {
        if (i == len) {
          ...
          if (!FITS::isa_text(*s)) {
            if (!result.errmsg) result.errmsg = "String value contains non-ASCII_text.";
          }
          ++j;
          ++s;
          ++i;
    ...
    default:
      ...
      result.endpos += i;
      if (result.isa_point) result.pointpos += i;
      if (result.errmsg) {
...
void FITS::get_numeric(const char *s, int len, FitsValueResult &result) {
    ...
    if (sigint == maxdigl && n < len) {
      ...
      if (isa_digit(*s) && n < len) {
        ...
        for (++n, ++s; isa_digit(*s) && (n < len); ++n, ++s) {
          ...
          if (sigint == maxsigdigits) break;
        }
      }
      if (sigint == maxsigdigits && n < len) {                          // Discard digits
        for (++n, ++s; isa_digit(*s) && (n < len); ++n, ++s) ++sigint;  // But count them
      }
    }
  }
  if (n == len || (!(*s == '.' || *s == 'E' || *s == 'D'))) {
    ...
    if (sigint > 10) {
      result.errmsg = negsign ? "Integer underflow" : "Integer overflow";
      return;
    }
    if (negsign) {
      intpart1 = -intpart1;
  ...
  if (*s == '.') {
    ...
    ++s;
    ++n;
    if (n == len) goto real_value;
    if (*s == 'E' || *s == 'D') goto get_exp;
    if (!(isa_digit(*s))) goto real_value;
    ...
    if (sigfrac > 0) {
      ...
      if (i == maxdigl && i < sigfrac) {
        if (i < sigfrac) {
          ...
          for (++i, ++p; i < sigfrac; ++i, ++p) fracpart2 = fracpart2 * 10 + digit2bin(*p);
        }
      }
    }
    // 8. Check next char after fraction part
    if (!(*s == 'E' || *s == 'D')) goto real_value;
  }
  ...
  if (result.no_sig <= 5 && (exp_type == ' ' || exp_type == 'E')) {
    if (intpart1 == 0) {
      if (fracpart1 == 0) {
        ...
        result.f = 0.0F;
        return;
      } else if ((exp - fracpos - 1) > minfltexp && (exp - fracpos) < maxfltexp) {
        result.type = FLOAT;
        result.f = tenF(fracpart1, (exp - sigfrac - fracpos));
        if (negsign) result.f = -result.f;
        return;
      }
    } else if ((sigint - 1 + exp) > minfltexp && (sigint + exp) < maxfltexp) {
      result.type = FLOAT;
      result.f = (fracpart1 == 0) ? 0.0F : tenF(fracpart1, (exp - sigfrac - fracpos));
  ...
  if (result.no_sig > 5 || exp_type == 'D') {
    if (intpart1 == 0) {
      if (fracpart1 == 0) {
        ...
        result.d = 0.0;
        return;
      } else if ((exp - fracpos - 1) > mindblexp && (exp - fracpos) < maxdblexp) {
        result.type = DOUBLE;
        result.d = (fracpart2 == 0) ? 0.0 : tenD(fracpart2, (exp - sigfrac - fracpos));
        i = (sigfrac < maxdigl) ? sigfrac : maxdigl;
        i = exp - fracpos - i;
        result.d += tenD(fracpart1, i);
        if (negsign) result.d = -result.d;
        return;
      }
    } else if ((sigint - 1 + exp) > mindblexp && (sigint + exp) < maxdblexp) {
      result.type = DOUBLE;
      result.d = (fracpart2 == 0) ? 0.0 : tenD(fracpart2, (exp - sigfrac - fracpos));
      if (fracpart1) {
        i = (sigfrac < maxdigl) ? sigfrac : maxdigl;
...
int FITS::str2fstr(char *target, const char *source, int len) {
  ...
  for (; (i < 8) && len; ++i, --len) *target++ = ' ';  // target must be at least 8 chars long
  return j;
}

void FITS::parse_vatform(const char *s, FITS::ValueType &valType, int &maxelem) {
  int ok = 1;
  if (s && *s != '\0') {
    ...
    if (ok && FITS::isa_letter(*s)) {
      switch (*s) {
        ...
        case 'D':
          ...
          break;
        case 'C':
          valType = FITS::COMPLEX;
          break;
        case 'M':
          valType = FITS::DCOMPLEX;
          break;
        default:
...
FitsKeyword &FitsParse::parse(const char *s, int len) {
  ...
  if (namelen > 8) seterr("Name cannot be greater than 8 chars.");
  ...
  if (!isa_value_id) {
    ...
    if (n != 0 && FITS::ResWord.requires_value(n)) {
      seterr(
          "No value indicator -- reserved keyword \
must have a value.");
      return mkerr(s, len);
    }
  ...
  if (s[pos] != ' ') seterr("Value indicator must be `= \' -- corrected");
  ++pos;
  if (pos == len) {
    n = FITS::ResWord.isreserved(&s[kword.begpos], kword.len);
    if (n != 0 && FITS::ResWord.requires_value(n)) {
  ...
  val.begpos += pos;
  val.endpos += pos;
  if (val.isa_point) val.pointpos += pos;
  if (val.errmsg) {
    seterr(val.errmsg);
    return mkerr(s, len);
  }
  ...
  if (res->name() == FITS::ERRWORD) {
    n = FITS::ResWord.isreserved(&s[kword.begpos], kword.len);
    if ((n != 0) && FITS::ResWord.isunique(n) && (FITS::ResWord[n].type() != val.type)) {
      // This is a attempt to correct a common error -- integers
      // where there should be reals. Convert these to doubles.
      if (val.type == FITS::LONG && FITS::ResWord[n].type() == FITS::REAL) {
  ...
  if (FITS::chk_comment(&s[comm_pos + pos], comm_len)) seterr("Comment contains non-ASCII_text.");
  if (res->name() == FITS::USER_DEF)
    return *new FitsKeyword(&s[kword.begpos], namelen, val.type, addrval, val.s[1],
                            &s[comm_pos + pos], comm_len);
  else {
    ...
    return *new FitsKeyword(res, kword.index, val.type, addrval, val.s[1], &s[comm_pos + pos],
                            comm_len);
  }
}
...
void FitsKeyword::err(const char *nm, const FITS::ValueType &ty, const void *val, const char *msg) {
  cout << "Keyword Error:  name = " << nm << " of type " << ty << " and value ";
...
void FitsKeyword::setval(const FITS::ValueType &ty, const void *v, int vlen) {
  if (ty == FITS::STRING || ty == FITS::FSTRING) {
    int i, plen = vlen;
    if (vlen < 8) plen = 8;  // FITS strings must be at least 8 chars
...
ostream &operator<<(ostream &o, const FitsKeyword &x) {
  ...
  if (x.commlen()) o << " \"" << x.comm() << "\"";
  o << "\n";
  return o;
}

FitsKeyword &FitsKeywordList::make(const char *nm, FITS::ValueType ty, const void *val,
                                   const char *cm) {
  FitsKeyword *kw;
  if (!nm) {
...
FitsKeyword &FitsKeywordList::make(FITS::ReservedName nm, FITS::ValueType ty, const void *val,
                                   const char *cm) {
  FitsKeyword *kw;
  int cmlen = 0;
  if (cm) cmlen = strlen(cm);
    ...
    if (val == 0)
      ...
      if (vallen > 68) {
        return makeErrKeyword(FITS::ResWord.aname(nm), ty, val,
                              "String values cannot be > 68 characters long.");
      }
    }
  }
  int valsize = (vallen < 8) ? 8 : vallen;
...
FitsKeyword &FitsKeywordList::make(int ind, FITS::ReservedName nm, FITS::ValueType ty,
                                   const void *val, const char *cm) {
  FitsKeyword *kw;
  int cmlen = 0;
  if (cm) cmlen = strlen(cm);
  int vallen = 0;
...
int FitsKeywordList::rules(FitsKeyword &x, FITSErrorHandler errhandler) {
  ...
  if (x.kw().name() == FITS::USER_DEF) return 0;
  if (x.kw().name() == FITS::ERRWORD) return -1;
  switch (x.kw().name()) {
    case FITS::NAXIS:
      if (x.isindexed()) {
        if ((*this)(FITS::NAXIS) == 0) {
          ...
          if (x.index() >= 1 && x.index() <= curr()->asInt()) {
            if (x.asInt() < 0) {
              ...
              msgline << "Illegal value for keyword NAXIS" << x.index();
    ...
    case FITS::END:  // there must be no comment
      if (x.commlen() != 0) {
        errhandler("Comments are not allowed on keyword END", FITSError::WARN);
        return 1;
      }
      break;
    case FITS::TBCOL:
      ...
      if ((*this)(FITS::TFIELDS) == 0) {
        ...
        if (x.index() >= 1 && x.index() <= curr()->asInt()) {
          if (x.asInt() < 0) {
            ostringstream msgline;
            msgline << "Illegal value for keyword TBCOL" << x.index();
            strncpy(msgstring, msgline.str().c_str(), sizeof(msgstring) - 1);
            errhandler(msgstring, FITSError::SEVERE);
            return -1;
          }
    ...
    case FITS::BLANK:  // BITPIX must exist and be positive
      if ((*this)(FITS::BITPIX) == 0) {
        ...
        if (curr()->asInt() < 0) {
          ...
          errhandler("Keyword BLANK not allowed when BITPIX < 0", FITSError::WARN);
          return 0;
        }
      }
      break;
...
FitsKeyCardTranslator::FitsKeyCardTranslator(int max) : cardno(0), max_errs(max), no_errs_(0) {
  ...
  for (int i = 0; i < FitsRecSize; ++i) blanks[i] = ' ';
  // check for storage allocation errors
}
...
FitsKeywordList &FitsKeyCardTranslator::parse(const char *buff, FitsKeywordList &kwlist, int count,
  ...
  for (i = 0; i < 36; ++i) {
    ...
    if (show_err && (kwlist.no_parse_errs() > 0)) {
      ...
      if (strcmp(kwlist.curr()->name(), "ERROR") == 0) errlev = FITSError::WARN;
      ostringstream msgline;
    ...
    if (end_found) {
      if (kwlist.curr()->isreserved() && kwlist.curr()->kw().name() == FITS::SPACES &&
        ...
        if (no_errs_ < max_errs) {
          ...
          errhandler("Invalid card after END keyword.", FITSError::WARN);
        }
      }
    }
    if (kwlist.curr()->isreserved() && kwlist.curr()->kw().name() == FITS::END) {
      end_found = 1;
...
void FitsKeyCardTranslator::fmtcard(char *card, const FitsKeyword &k) {
  ...
  if (k.isreserved() && k.isindexed()) {
    n = k.index();
    for (i = 0; n > 0; ++i, n /= 10) c[i] = n % 10 + '0';
    for (--i, n = k.namelen(); i >= 0; ++n, --i) card[n] = c[i];
  }
  if (k.type() == FITS::NOVALUE) {
    if ((n = (k.commlen() <= 72 ? k.commlen() : 72))) memcpy(&card[8], k.comm(), n);
    ...
    if (k.commlen()) {
      ...
      if (i <= 30) {
        card[31] = '/';
        if ((n = (k.commlen() <= 48 ? k.commlen() : 48))) memcpy(&card[32], k.comm(), n);
      } else {
        if (i < 80) {
          card[i - 1] = '/';
          if ((n = (k.commlen() <= (80 - i) ? k.commlen() : (80 - i))))
    ...
    if (k.commlen()) {
      if (!(k.type() == FITS::ICOMPLEX || k.type() == FITS::DCOMPLEX)) {
        card[31] = '/';
        if ((n = (k.commlen() <= 48 ? k.commlen() : 48))) memcpy(&card[32], k.comm(), n);
      } else {
        card[51] = '/';
        if ((n = (k.commlen() <= 28 ? k.commlen() : 28))) memcpy(&card[52], k.comm(), n);
      }
    }
  }
}
