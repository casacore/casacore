...
inline const ReservedFitsKeyword &ReservedFitsKeywordCollection::operator[](int i) const {
  return resword[i];
}
...
inline const ReservedFitsKeyword &ReservedFitsKeywordCollection::err_item() const {
  return error_item;
}
inline const ReservedFitsKeyword &ReservedFitsKeywordCollection::end_item() const {
  return end__item;
}
inline const ReservedFitsKeyword &ReservedFitsKeywordCollection::spaces() const {
  return spaces_item;
}
inline const ReservedFitsKeyword &ReservedFitsKeywordCollection::comment() const {
  return comment_item;
}
inline const ReservedFitsKeyword &ReservedFitsKeywordCollection::history() const {
  return history_item;
}
...
class FitsKeyword {
 ...
 private:
  ...
  FitsKeyword(const ReservedFitsKeyword *, int, FITS::ValueType, const void *, int, const char *,
              int);
  //</group>
};
...
inline IComplex FitsKeyword::asIComplex() const { return *((IComplex *)val); }
...
class FitsKeywordList {
 ...
 private:
  ...
  FitsKeyword &make(int ind, FITS::ReservedName nm, FITS::ValueType t, const void *v,
                    const char *c);
  ...
  FitsKeyword &makeErrKeyword(const char *name, FITS::ValueType type, const void *val,
                              const char *errmsg);
  FitsParse card;
};
...
inline void FitsKeywordList::last() {
  cursor = total;
  pos = end_;
}
...
inline FitsKeyword *FitsKeywordList::operator()(const FITS::ReservedName &n, int ndx) {
  first();
  return next(n, ndx);
}
...
inline void FitsKeywordList::parse(const char *s, int l) { insert(card.parse(s, l)); }
...
inline void FitsKeywordList::mk(int n, FITS::ReservedName k, double v, const char *c) {
...
inline void FitsKeywordList::mk(const char *n, float r, float i, const char *c) {
  Complex v(r, i);
  insert(make(n, FITS::COMPLEX, &v, c));
}
inline void FitsKeywordList::mk(const char *n, double r, double i, const char *c) {
  DComplex v(r, i);
...
class ConstFitsKeywordList {
 public:
  ...
  const FitsKeyword *operator()(const FITS::ReservedName &x) { return kw(x); }
  const FitsKeyword *next(const FITS::ReservedName &x) { return kw.next(x); }
  const FitsKeyword *operator()(const FITS::ReservedName &x, int n) { return kw(x, n); }
  const FitsKeyword *next(const FITS::ReservedName &x, int n) { return kw.next(x, n); }
...
inline int FitsKeyCardTranslator::err_cardno(int i) const { return err_cardno_[i]; }
