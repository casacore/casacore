...
FitsInput::~FitsInput() { delete &m_fin; }
...
void FitsInput::errmsg(FitsErrs e, const char *s) {
  ...
  msgline << " Physical record " << m_fin.blockno() << " logical record " << m_fin.recno()
          << " --\n\t" << s << endl;
  m_err_status = e;
...
void FitsOutput::errmsg(FitsErrs e, const char *s) {
  ...
  msgline << " Physical record " << m_fout.blockno() << " logical record " << m_fout.recno()
          << " --\n\t" << s << endl;
  m_err_status = e;
...
//==============================================================================================
BlockOutput &FitsOutput::make_output(const char *n, const FITS::FitsDevice &d, int b,
                                     FITSErrorHandler errhandler)
...
//========================================================================================
FitsOutput::FitsOutput(const char *n, const FITS::FitsDevice &d, int b, FITSErrorHandler errhandler)
...
FitsOutput::FitsOutput(FITSErrorHandler errhandler)
    : FitsIO(errhandler),
      m_fout(*(BlockOutput *)(new FitsStdOutput(m_recsize, errhandler))),
...
void FitsInput::init() {
  if (m_fin.err())
    ...
    if (!(m_hdu_type == FITS::PrimaryArrayHDU || m_hdu_type == FITS::PrimaryTableHDU ||
...
Vector<String> FitsInput::kwlist_str(Bool length80) {
  ...
  if (!m_header_done) {
    cout << "[FitsInput::kwlist_str()] If you need call this method, "
            "you should do so before reading any data from CHDU."
         << endl;
    return cards;
  } else {
    // remember the cfitsio bytepos before calling any cfitsio function
    ...
    if (ffghsp(m_fptr, &l_keysexist, &l_morekeys, &l_status)) {
      fits_report_error(stderr, l_status);  // print error report
      cout << "[FitsInput::kwlist_str()] Failed to get total number of keywords in CHDU." << endl;
      return cards;
    }
...
void FitsInput::read_header_rec() {
  // make the next hdu be the chdu of cfitsio and set the file position pointer at the begining of
  // the hdu.
...
int FitsInput::skip_hdu() {  // Skip an entire header-data unit
    ...
    if (ffgrec(m_fptr, nextkey, l_card, &l_status) > 0)  // get the 80-byte card
    {
      ...
      if (fftrec(l_keyname, &l_status) > 0) {  // test keyword name; catches no END
        snprintf(l_message, sizeof(l_message),
                 "Name of keyword no. %d contains illegal character(s): %s", nextkey, l_keyname);
  ...
  // pointer is; Since read_header_rec() will move the chdu forward for one, no need to call
  // ffmrhd() here. And read_header_rec() will also move the cfitsio bytepos.
  m_fin.reset_iosize();
...
int FitsInput::process_header(FITS::HDUType t, FitsKeywordList &uk) {
  ...
  for (;; m_kc.parse(m_curr, uk, cnt, m_errfn, True)) {
    ...
    for (i = 0; i < 8; ++i)
      if (!FITS::isa_text(m_curr[i])) break;
    if (i < 8) {
      errmsg(MISSKEY,
  ...
  if (m_data_size > 0) {
    ...
    if (t != FITS::PrimaryTableHDU) read_header_rec();
  }
  return 0;
}
//===============================================================================================
...
OFF_T FitsInput::read_all(FITS::HDUType t, char *addr) {
  if (m_curr_size < 0 || m_curr_size != m_data_size || m_rec_type != FITS::HDURecord ||
      t != m_hdu_type || (!m_header_done)) {
...
int FitsInput::read(FITS::HDUType t, char *addr, int nb) {
  ...
  if (OFF_T(nb) > m_curr_size) nb = m_curr_size;
  int n = nb;
...
int FitsOutput::write_hdr(FitsKeywordList &kwl, FITS::HDUType t, FITS::ValueType dt, OFF_T ds,
  ...
  // this boolean variable indicates wheather the user is using only the write_***_hdr() methods,
  // which write only the required key words for the corresponding hdu and they cannot work with
  // write_hdu(). So once write_hdu() is called, we set it to be FALSE here.
...
FitsDiskInput::FitsDiskInput(const char *f, int l, int n, FITSErrorHandler errhandler)
    : BlockInput(f, l, n, errhandler) {}

FitsDiskOutput::FitsDiskOutput(const char *f, int l, int n, FITSErrorHandler errhandler)
    : BlockOutput(f, l, n, errhandler) {}

FitsStdInput::FitsStdInput(int l, FITSErrorHandler errhandler) : BlockInput(0, l, 1, errhandler) {}
...
FitsTape9Input::FitsTape9Input(const char *f, int l, int n, FITSErrorHandler errhandler)
    : BlockInput(f, l, n, errhandler) {}

FitsTape9Output::FitsTape9Output(const char *f, int l, int n, FITSErrorHandler errhandler)
    : BlockOutput(f, l, n, errhandler) {}
...
FitsInput::FitsInput(FITSErrorHandler errhandler)
    : FitsIO(errhandler), m_fin(*(BlockInput *)(new FitsStdInput(m_recsize, errhandler))) {
  init();
}
}  // namespace casacore
