...
class HeaderDataUnit {
 ...
 public:
  ...
  const FitsKeyword *kw(const FITS::ReservedName &n) { return kwlist_(n); }
  const FitsKeyword *nextkw(FITS::ReservedName &n) { return kwlist_.next(n); }
  const FitsKeyword *kw(FITS::ReservedName &n, int i) { return kwlist_(n, i); }
...
inline void HeaderDataUnit::mk(FITS::ReservedName k, const char *v, const char *c) {
  posEnd();
  kwlist_.mk(k, v, c);
}
...
inline void HeaderDataUnit::mk(int n, FITS::ReservedName k, const char *v, const char *c) {
  posEnd();
  kwlist_.mk(n, k, v, c);
}
...
inline void HeaderDataUnit::mk(int n, FITS::ReservedName k, double v, const char *c) {
  posEnd();
  kwlist_.mk(n, k, v, c);
}
...
inline void HeaderDataUnit::mk(const char *n, double r, double i, const char *c) {
  posEnd();
  kwlist_.mk(n, r, i, c);
}
...
class PrimaryArray : public HeaderDataUnit {
 public:
  ...
  // </group>
  //<group>
  int write_priArr_hdr(FitsOutput &fout, int simple, int bitpix, int naxis, long naxes[],
                       int extend);
  //</group>
...
class PrimaryGroup : public PrimaryArray<TYPE> {
 public:
  ...
  //<group>
  int write_priGrp_hdr(FitsOutput &fout, int simple, int bitpix, int naxis, long naxes[],
                       long pcount, long gcount);
  //</group>
...
class FitsField : public FitsBase {
 public:
  FitsField(int n = 1) : FitsBase(FITS::getfitstype(NoConvert<TYPE>()), n), field(0) {}
  ~FitsField();
...
class FitsField<FitsBit> : public FitsBase {
 public:
  ...
  FitsField<FitsBit> &operator=(unsigned i) {
    (*field)[byte_offset] =
        (i == 0 ? ((*field)[byte_offset] & ~mask) : ((*field)[byte_offset] | mask));
    return *this;
  }
...
class BinaryTableExtension : public ExtensionHeaderDataUnit {
 public:
  ...
  int write_binTbl_hdr(FitsOutput &, long, int, const char **, const char **, const char **,
                       const char *, long);
...
class AsciiTableExtension : public BinaryTableExtension {
 public:
  ...
  int write_ascTbl_hdr(FitsOutput &, long, long, int, const char **, long *, const char **,
