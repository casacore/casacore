...
template <class TYPE>
PrimaryArray<TYPE>::PrimaryArray(FitsInput &f, FITSErrorHandler errhandler)
    : HeaderDataUnit(f, FITS::PrimaryArrayHDU, errhandler) {
  pa_assign();  // assign values from keyword list
}
//============================================================================
template <class TYPE>
PrimaryArray<TYPE>::PrimaryArray(FitsInput &f, FITS::HDUType t, FITSErrorHandler errhandler)
...
PrimaryArray<TYPE>::~PrimaryArray() {
  if (bunit_x != &char_null) delete[] bunit_x;
  if (no_dims > 0) {
    delete[] crpix_x;
    delete[] crota_x;
    delete[] crval_x;
    ...
    for (int i = 0; i < no_dims; i++)
      if (ctype_x[i] != &char_null) delete[] ctype_x[i];
    delete[] ctype_x;
    delete[] factor;
  }
  if (alloc_elems > 0) {
...
int PrimaryArray<TYPE>::offset(int d0, int d1, int d2, int d3, int d4) const {
  OFF_T offset;
  offset = d0 + d1 * factor[1] + d2 * factor[2] + d3 * factor[3] + d4 * factor[4];
  return offset;
}
//===================================================================================
template <class TYPE>
...
//====================================================================================
template <class TYPE>
double PrimaryArray<TYPE>::operator()(int d0, int d1, int d2, int d3, int d4) const {
...
int PrimaryArray<TYPE>::read() {  // read entire array
  ...
  if (set_next(nelements()) == -1) {  // allocate nelements array elements
    cerr << "Buffer array is too big to fit into memory. You are using PrimaryArray::read()."
         << endl;
...
int PrimaryArray<TYPE>::read(int ne) {  // read the next ne data elements
  ...
  if (nr != ne) end_elem = beg_elem + nr - 1;
...
int PrimaryArray<TYPE>::store(const TYPE *source, FITS::FitsArrayOption opt) {
  ...
  if (opt == FITS::CtoF) {
    ...
    count = 0;
    for (i = 0; i < dims(); ++i) sub[i] = 0;
    for (;;) {
      for (i = 1, offset = sub[0]; i < dims(); ++i) offset += sub[i] * factor[i];
      array[offset] = source[count++];
      if (count == nelements()) break;
      for (i = dims() - 1; i >= 0; --i) {
        ++sub[i];
        if (sub[i] < dim(i)) break;
        sub[i] = 0;
      }
    }
  } else {
...
void PrimaryArray<TYPE>::copy(double *target, FITS::FitsArrayOption opt) const {
  ...
  if (opt == FITS::FtoC) {
    ...
    for (i = 0; i < dims(); ++i) sub[i] = 0;
    for (;;) {
      for (i = 0, offset = 0; i < dims(); ++i) offset += sub[i] * C_factor[i];
...

{
  ...
  if (fout.rectype() != FITS::InitialState) {
    errmsg(BADOPER,
           "[PrimaryArray<TYPE>::write_priArr_hdr()] Primary Header must be the beginning of the "
           "file.");
    return -1;
  }
  ...
  if (ffflsh(fout.getfptr(), TRUE, &l_status)) {
...
template <class TYPE>
ImageExtension<TYPE>::ImageExtension(FitsInput &f, FITSErrorHandler errhandler)
    : PrimaryArray<TYPE>(f, FITS::ImageExtensionHDU, errhandler) {
  ie_assign();
}
//====================================================================================
template <class TYPE>
ImageExtension<TYPE>::ImageExtension(FitsKeywordList &k, FITSErrorHandler errhandler)
...

{
  ...
  if (!fout.hdu_complete()) {
    errmsg(BADOPER,
           "Previous HDU incomplete -- cannot write header[ImageExtension::write_imgext_hdr].");
    return -1;
  }
  if (!fout.isextend()) {
    errmsg(BADOPER,
           "Extensions are not allowed for this FITS file![ImageExtension::write_imgext_hdr].");
    return -1;
  }
  if (!fout.required_keys_only()) {
    cerr << "\n[ImageExtension::write_imgTbl_hdr()] write_imgTbl_hdr() works with other "
            "write_***_hdr()"
         << endl;
  ...
  // if(ffflsh(l_newfptr, TRUE, &l_status)){ errmsg(BADOPER,"[PrimaryArray::write_priArr_hdr()]
  ...
  // update the file pointer in FitsOutput. Always do this before updating the file pointer in
  // BlockOutpu.
...
//===================================================================================================
template <class TYPE>
PrimaryGroup<TYPE>::PrimaryGroup(FitsKeywordList &k, FITSErrorHandler errhandler)
...
PrimaryGroup<TYPE>::~PrimaryGroup() {
  if (pcount_x > 0) {
    for (int i = 0; i < pcount_x; i++)
      if (ptype_x[i] != &char_null) delete[] ptype_x[i];
    delete[] ptype_x;
    delete[] pzero_x;
    delete[] pscal_x;
  }
...
*/
template <class TYPE>
int PrimaryGroup<TYPE>::write_priGrp_hdr(
    ...
    long gcount)       // I - number of random groups ( greater than 1 for PrimaryGroup)
{
  if (naxes[0] != 0) {
    errmsg(
        BADOPER,
        "[PrimaryGroup<TYPE>::write_priGrp_hdr())] For Primary Group, keyword NAXIS1 must be 0!");
    return -1;
  }
  if (pcount < 1 && gcount < 2) {
    errmsg(BADOPER,
           "[PrimaryGroup<TYPE>::write_priGrp_hdr())] In order to create keyword GROUPS, PCOUNT "
           "and GCOUNT, either PCOUNT's value must be >0 or GCOUNT's value must be > 1 !");
    return -1;
  }
  ...
  if (fout.rectype() != FITS::InitialState) {
    errmsg(BADOPER,
           "Primary Header must be the begnning of the file[PrimaryGroup::write_priGrp_hdr].");
    return -1;
  }
  // write the required keywords for PrimaryGroup to fitsfile.
  int extend =
      0;  // EXTEND is not required by random group. Set it to zero so that it will not be written.
  if (ffphpr(fout.getfptr(), simple, bitpix, naxis, naxes, pcount, gcount, extend, &l_status)) {
...
void PrimaryGroup<TYPE>::pg_assign() {
  ...
  totsize = dimn[1];
  for (i = 2; i < no_dims; ++i) totsize *= dimn[i];
  factor[0] = 1;
  for (i = 1; i < (no_dims - 1); ++i) factor[i] = factor[i - 1] * dimn[i];
  for (i = 0; i < (no_dims - 1); ++i) dimn[i] = dimn[i + 1];
  --no_dims;

  // adjust all axis-relative keyword values
  if (ctype_x[0] != &char_null) delete[] ctype_x[0];
  for (i = 0; i < no_dims; ++i) {
...
template <class TYPE>
PrimaryTable<TYPE>::PrimaryTable(FitsInput &f, FITSErrorHandler errhandler)
    : PrimaryArray<TYPE>(f, FITS::PrimaryTableHDU, errhandler) {
  pt_assign();
}
//====================================================================================
template <class TYPE>
PrimaryTable<TYPE>::PrimaryTable(FitsKeywordList &k, FITSErrorHandler errhandler)
...
PrimaryTable<TYPE>::~PrimaryTable() {
  if (object_x != &char_null) delete[] object_x;
  if (telescop_x != &char_null) delete telescop_x;
}
//====================================================================================
template <class TYPE>
...

{
  ...
  if (!fout.hdu_complete()) {
    errmsg(BADOPER,
           "Previous HDU incomplete -- cannot write header[PrimaryTable::write_priTable_hdr].");
    return -1;
  }
  if (!fout.isextend()) {
    errmsg(BADOPER,
           "Extensions are not allowed for this FITS file![PrimaryTable::write_priTable_hdr].");
    return -1;
  }
  if (!fout.required_keys_only()) {
...
void FitsField<TYPE>::show(std::ostream &o) {
  ...
  if (fieldtype() == FITS::BYTE) {
    ...
    o << (int)s[0];
    for (i = 1; i < no_elements; ++i) o << ", " << (int)s[i];
  } else if (fieldtype() == FITS::CHAR) {
    p = (char *)(*field);
    for (i = 0; i < no_elements && *p != '\0'; ++i, ++p) o << *p;
  } else {
    o << (*field)[0];
    for (i = 1; i < no_elements; ++i) o << ", " << (*field)[i];
  }
}
//============================================================================
template <class TYPE>
FitsArray<TYPE>::FitsArray(int n, const int *d) : FitsField<TYPE>(1) {
  int i;
  if (n > 0) {
    no_dims = n;
...
inline TYPE &FitsArray<TYPE>::operator()(int d0, int d1, int d2, int d3) {
  return (*field)[d0 + (factor[1] * d1) + (factor[2] * d2) + (factor[3] * d3)];
}
//=============================================================================
template <class TYPE>
inline TYPE &FitsArray<TYPE>::operator()(int d0, int d1, int d2, int d3, int d4) {
  return (*field)[d0 + (factor[1] * d1) + (factor[2] * d2) + (factor[3] * d3) + (factor[4] * d4)];
}
//=============================================================================
