...
//=============================================================================
void *FitsField<FitsBit>::data() { return *field; }
//=============================================================================
void FitsField<FitsBit>::setaddr(void **addr) { field = (FitsBit **)addr; }
//=============================================================================
void FitsField<FitsBit>::show(ostream &o) {
  for (int i = 0; i < no_elements; ++i) o << (int)((*this)(i));
}
//=================================================================================
...
//=================================================================================
FitsField<FitsBit> &FitsArray<FitsBit>::operator()(int d0, int d1, int d2, int d3) {
  FitsField<FitsBit> *tmp = this;
  return (*tmp)(d0 + d1 * factor[1] + d2 * factor[2] + d3 * factor[3]);
}
//=================================================================================
FitsField<FitsBit> &FitsArray<FitsBit>::operator()(int d0, int d1, int d2, int d3, int d4) {
  FitsField<FitsBit> *tmp = this;
  return (*tmp)(d0 + d1 * factor[1] + d2 * factor[2] + d3 * factor[3] + d4 * factor[4]);
}
//=================================================================================
#if 0
FitsField<FitsBit> & FitsArray<FitsBit>::operator () (int d0, int d1, int d2,
...
Bool HeaderDataUnit::determine_type(FitsKeywordList &kw, FITS::HDUType &htype,
  ...
  if ((!naxis || !(naxis->kw().name() == FITS::NAXIS && naxis->index() == 0))) {
  ...
  if ((errstat == OK) && (naxis->asInt() != 0)) {
    if (!naxis1 || !(naxis1->kw().name() == FITS::NAXIS && naxis1->index() == 1)) {
  ...
  if (word1->kw().name() != FITS::SIMPLE && word1->kw().name() != FITS::XTENSION) {
    ...
    if (!word1) {
      ...
      if (word1)
        ...
        errhandler("Missing keywords SIMPLE and XTENSION.", FITSError::WARN);
    } else
  ...
  if (word1->kw().name() == FITS::SIMPLE) {
    ...
    if (naxis->asInt() > 0) {
      ...
      else if (naxis->asInt() == 2 && (naxis2->asInt() == 0 && naxis1->asInt() == 777777701))
...
Bool HeaderDataUnit::compute_size(FitsKeywordList &kw, OFF_T &datasize, Int &dims,
  ...
  if (htype == FITS::PrimaryArrayHDU) {
    ...
    for (n = 1; n <= dims; n++) {
      naxisn = kw.next();
      if (!naxisn || !(naxisn->kw().name() == FITS::NAXIS && naxisn->index() == n)) {
  ...
  // Image Extension HDU
    ...
    if (!kw(FITS::PCOUNT)) {
      st = NOPCOUNT;
      errhandler("Missing required PCOUNT keyword", FITSError::WARN);
    } else if (kw.curr()->asInt() != 0) {
      st = BADPCOUNT;
      errhandler("Invalid value of PCOUNT keyword", FITSError::WARN);
    }
    if (!kw(FITS::GCOUNT)) {
      st = NOGCOUNT;
      errhandler("Missing required GCOUNT keyword", FITSError::WARN);
  ...
  // ASCII Table HDU
    ...
    if (!pcount || !(pcount->kw().name() == FITS::PCOUNT && pcount->index() == 0)) {
      st = NOPCOUNT;
      errhandler("Missing required PCOUNT keyword", FITSError::WARN);
    ...
    if (!gcount || !(gcount->kw().name() == FITS::GCOUNT && gcount->index() == 0)) {
      st = NOGCOUNT;
      errhandler("Missing required GCOUNT keyword", FITSError::WARN);
...
//================================================================================
HeaderDataUnit::HeaderDataUnit(FitsInput &f, FITS::HDUType t, FITSErrorHandler errhandler)
    : kwlist_(*(new FitsKeywordList)),
      constkwlist_(kwlist_),
      fin(&f),
      errfn(errhandler),
      err_status(OK),
      no_dims(0),
  ...
  if (no_dims > 0) {
    if ((dimn = new Int[no_dims]) == 0) {
      ...
      for (int i = 0; i < no_dims; i++) dimn[i] = kwlist_(FITS::NAXIS, (i + 1))->asInt();
    }
  }
  // cout << "<<HeaderDataUnit::HeaderDataUnit ~ hdu_type=" << hdu_type
  //      << " f.hdutype()=" << f.hdutype()
...
//=================================================================================================
HeaderDataUnit::HeaderDataUnit(FitsKeywordList &k, FITS::HDUType t, FITSErrorHandler errhandler,
                               FitsInput *f)
    : kwlist_(*new FitsKeywordList(k)),
      constkwlist_(kwlist_),
      fin(f),
      errfn(errhandler),
      err_status(OK),
...
HeaderDataUnit::HeaderDataUnit(FITS::HDUType, FITSErrorHandler errhandler, FitsInput *f)
    : kwlist_(*new FitsKeywordList()),
      constkwlist_(kwlist_),
      fin(f),
      errfn(errhandler),
...
bool HeaderDataUnit::init_data_unit(FITS::HDUType t) {
  ...
  if (!determine_type(kwlist_, hdu_type, data_type, errfn, err_status)) {
    errmsg(BADTYPE,
           "Could not determine HDU type from keyword list [HeaderDataUnit::init_data_unit]");
    hdu_type = FITS::NotAHDU;
    return false;
  }
  if (!compute_size(kwlist_, fits_data_size, no_dims, hdu_type, data_type, errfn, err_status)) {
    errmsg(BADSIZE,
           "Could not compute data size from keyword list[HeaderDataUnit::init_data_unit]");
    hdu_type = FITS::NotAHDU;
    return false;
  }
  fits_item_size = FITS::fitssize(data_type);
...
ExtensionHeaderDataUnit::ExtensionHeaderDataUnit(FitsInput &f, FITSErrorHandler errhandler)
    : HeaderDataUnit(f, FITS::UnknownExtensionHDU, errhandler) {
  ex_assign();
}
//============================================================================
ExtensionHeaderDataUnit::ExtensionHeaderDataUnit(FitsInput &f, FITS::HDUType t,
...
ExtensionHeaderDataUnit::~ExtensionHeaderDataUnit() {
  if (xtension_x != &char_null) delete[] xtension_x;
  if (extname_x != &char_null) delete[] extname_x;
}
//====================================================================================
...
FitsBase::~FitsBase() {}
//====================================================================================
int FitsBase::dims() const { return 1; }
//====================================================================================
int FitsBase::dim(int n) const { return (n == 0 ? no_elements : 0); }
//====================================================================================
int *FitsBase::vdim() { return &no_elements; }
//====================================================================================
FitsBase *FitsBase::make(const FITS::ValueType &type, int n) {
  switch (type) {
    case FITS::LOGICAL:
      return (new FitsField<FitsLogical>(n));
    case FITS::BIT:
      return (new FitsField<FitsBit>(n));
    case FITS::CHAR:
      return (new FitsField<char>(n));
    case FITS::BYTE:
      return (new FitsField<unsigned char>(n));
    case FITS::SHORT:
      return (new FitsField<short>(n));
    case FITS::LONG:
      return (new FitsField<FitsLong>(n));
    case FITS::FLOAT:
      return (new FitsField<float>(n));
    case FITS::DOUBLE:
      return (new FitsField<double>(n));
    case FITS::COMPLEX:
      return (new FitsField<Complex>(n));
    case FITS::ICOMPLEX:
      return (new FitsField<IComplex>(n));
    case FITS::DCOMPLEX:
      return (new FitsField<DComplex>(n));
    case FITS::VADESC:
      return (new FitsField<FitsVADesc>(n));
...
FitsBase *FitsBase::make(const FITS::ValueType &type, int n, int *d) {
  switch (type) {
    case FITS::LOGICAL:
      return (new FitsArray<FitsLogical>(n, d));
    case FITS::BIT:
      return (new FitsArray<FitsBit>(n, d));
    case FITS::CHAR:
      return (new FitsArray<char>(n, d));
    case FITS::BYTE:
      return (new FitsArray<unsigned char>(n, d));
    case FITS::SHORT:
      return (new FitsArray<short>(n, d));
    case FITS::LONG:
      return (new FitsArray<FitsLong>(n, d));
    case FITS::FLOAT:
      return (new FitsArray<float>(n, d));
    case FITS::DOUBLE:
      return (new FitsArray<double>(n, d));
    case FITS::COMPLEX:
      return (new FitsArray<Complex>(n, d));
    case FITS::ICOMPLEX:
      return (new FitsArray<IComplex>(n, d));
    case FITS::DCOMPLEX:
      return (new FitsArray<DComplex>(n, d));
    case FITS::VADESC:
      return (new FitsArray<FitsVADesc>(n, d));
    // The following "default" has been added to prevent compilers
    // such as GNU g++ from complaining about the rest of FITS
    // enumerations not being handled in the switch statement.
    //           -OO
...
FitsBase *FitsBase::make(FitsBase &x) {
  if (x.dims() == 1) return make(x.fieldtype(), x.nelements());
  return make(x.fieldtype(), x.dims(), x.vdim());
}

//== AsciiTableExtension =====================================================

AsciiTableExtension::AsciiTableExtension(FitsInput &f, FITSErrorHandler errhandler)
    : BinaryTableExtension(f, FITS::AsciiTableHDU, errhandler) {
  pad_char = ' ';
  at_assign();
}
//============================================================================
AsciiTableExtension::AsciiTableExtension(FitsKeywordList &k, FITSErrorHandler errhandler)
...
AsciiTableExtension::~AsciiTableExtension() {
  ...
  if (tfields_x > 0) {
    for (i = 0; i < tfields_x; i++) {
      if (tnulla_x[i] != &char_null) delete tnulla_x[i];
      delete[] format[i];
    }
    delete[] tnulla_x;
    delete[] format;
...
void AsciiTableExtension::at_assign() {
    ...
    if (FITS::isa_digit(*s)) {
      ...
      if (typecode == 'F' || typecode == 'E' || typecode == 'D') {
        if (*s == '.') {
          format[i][n++] = '.';
          ++s;
          while (FITS::isa_digit(*s)) format[i][n++] = *s++;
        }
      }
    }
    switch (typecode) {
      case 'A':
        fld[i] = new FitsField<char>(fits_width[i]);
        format[i][n++] = 's';
        break;
      case 'I':
        fld[i] = new FitsField<FitsLong>(1);
        format[i][n++] = 'd';
        break;
      case 'F':
        fld[i] = new FitsField<float>(1);
        format[i][n++] = 'f';
        break;
      case 'E':
        fld[i] = new FitsField<float>(1);
  ...
  for (i = 0; i < tfields(); ++i)
    if (fld[i]->fieldtype() == FITS::DOUBLE) {
      ...
      ++n;
      if (sizeof(double) > row_align) row_align = sizeof(double);
    }
  if (n < tfields())
    for (i = 0; i < tfields(); ++i)
      if (fld[i]->fieldtype() == FITS::FLOAT) {
        ...
        ne += fld[i]->localfieldsize();
        ++n;
        if (sizeof(float) > row_align) row_align = sizeof(float);
      }
  if (n < tfields())
    for (i = 0; i < tfields(); ++i)
      if (fld[i]->fieldtype() == FITS::LONG) {
        ...
        ne += fld[i]->localfieldsize();
        ++n;
        if (sizeof(FitsLong) > row_align) row_align = sizeof(FitsLong);
      }
  if (n < tfields())
    for (i = 0; i < tfields(); ++i)
      if (fld[i]->fieldtype() == FITS::CHAR) {
        ...
        ne += fld[i]->localfieldsize();
        ++n;
        if (sizeof(unsigned char) > row_align) row_align = sizeof(unsigned char);
      }

  // check row alignment
  if ((tablerowsize % row_align) != 0) tablerowsize += row_align - (tablerowsize % row_align);
...
int AsciiTableExtension::readrow() {
  FitsValueResult res;
  if (read_data((char *)fitsrow, fitsrowsize) != (int)fitsrowsize) return -1;
  ...
  for (int i = 0; i < tfields(); ++i) {
    if (fld[i]->fieldtype() == FITS::CHAR)
      ...
      FITS::get_numeric((char *)(&fitsrow[fits_offset[i]]), fits_width[i], res);
      if (res.errmsg) {
...
int AsciiTableExtension::writerow(FitsOutput &fout) {
  ...
  for (i = 0; i < tfields(); ++i) {
    if (fld[i]->fieldtype() == FITS::CHAR) {
      ...
      s = (char *)(fld[i]->data());
      for (n = 0; n < fits_width[i] && *s != '\0'; ++n) *t++ = *s++;
      if (*s == '\0')
        for (; n < fits_width[i]; ++n) *t++ = ' ';
    } else {
      switch (fld[i]->fieldtype()) {
        case FITS::LONG:
          snprintf(tmp, sizeof(tmp), format[i], *((FitsLong *)(fld[i]->data())));
          if (strlen(tmp) > fits_width[i]) {
            errmsg(BADCONV, "Ascii Table conversion error: numeric value exceeds field size");
            for (t = (char *)&fitsrow[fits_offset[i]], n = 0; n < fits_width[i]; ++n, ++t)
              *t = ' ';  // fill with blanks
          } else
...
*/
int AsciiTableExtension::write_ascTbl_hdr(
    FitsOutput &fout,     // I - FITS output object
...

{
  ...
  if (fout.rectype() == FITS::InitialState) {
    errmsg(BADOPER,
           "[AsciiTableExtension::write_ascTbl_hdr()] Primary Header must be written first.");
    return -1;
  }
  if (!fout.hdu_complete()) {
    errmsg(BADOPER,
           "[AsciiTableExtension::write_ascTbl_hdr()] Previous HDU incomplete -- cannot write "
           "header.");
    return -1;
  }
  if (!fout.isextend()) {
    errmsg(
        BADOPER,
        "[AsciiTableExtension::write_ascTbl_hdr()] Cannot write extension HDU - EXTEND not True.");
    return -1;
  }
  if (!fout.required_keys_only()) {
    cerr << "\n[AsciiTableExtension::write_ascTbl_hdr()] write_ascTbl_hdr() works with other "
            "write_***_hdr()"
         << endl;
  ...
  if (ffphtb(l_newfptr, naxis1, naxis2, tfields, const_cast<char **>(ttype), tbcol,
             const_cast<char **>(tform), const_cast<char **>(tunit), const_cast<char *>(extname),
             &l_status)) {
...
//=================================================================================================
BinaryTableExtension::BinaryTableExtension(FitsInput &f, FITSErrorHandler errhandler)
    : ExtensionHeaderDataUnit(f, FITS::BinaryTableHDU, errhandler) {
  bt_assign();
}
//=================================================================================================
BinaryTableExtension::BinaryTableExtension(FitsKeywordList &k, FITSErrorHandler errhandler)
...
BinaryTableExtension::~BinaryTableExtension() {
  int i;
  if (author_x != &char_null) delete[] author_x;
  if (referenc_x != &char_null) delete[] referenc_x;
  if (tfields_x > 0) {
    delete[] tscal_x;
    delete[] tzero_x;
    for (i = 0; i < tfields_x; i++) {
      if (tform_x[i] != &char_null) delete[] tform_x[i];
      if (ttype_x[i] != &char_null) delete[] ttype_x[i];
      if (tunit_x[i] != &char_null) delete[] tunit_x[i];
      if (tdisp_x[i] != &char_null) delete[] tdisp_x[i];
      if (tdim_x[i] != &char_null) delete[] tdim_x[i];
      delete fld[i];
    }
    delete[] tform_x;
    delete[] isatnull_x;
...
void BinaryTableExtension::bt_assign() {
  ...
  for (i = 0; i < tfields(); ++i) {
    ...
    if (FITS::isa_digit(*s)) {
      ne = FITS::digit2bin(*s++);
      while (FITS::isa_digit(*s)) ne = ne * 10 + FITS::digit2bin(*s++);
    }
    p = tdim(i);
    if (*p != '\0') {  // the multidimensional array case
      ...
      for (j = 0; j < nd; ++j) {
        ...
        while (FITS::isa_digit(*p)) dd[j] = dd[j] * 10 + FITS::digit2bin(*p++);
        while (*p == ' ') ++p;  // skip spaces
        if (*p == ',')
          ++p;
        else if (*p == ')')
      ...
      switch (*s) {
        case 'L':
          fld[i] = new FitsArray<FitsLogical>(nd, dd);
          break;
        case 'X':
          fld[i] = new FitsArray<FitsBit>(nd, dd);
          break;
        case 'B':
          fld[i] = new FitsArray<unsigned char>(nd, dd);
          break;
        case 'I':
          fld[i] = new FitsArray<short>(nd, dd);
          break;
        case 'J':
          fld[i] = new FitsArray<FitsLong>(nd, dd);
          break;
        case 'A':
          fld[i] = new FitsArray<char>(nd, dd);
          break;
        case 'E':
          fld[i] = new FitsArray<float>(nd, dd);
          break;
        case 'D':
          fld[i] = new FitsArray<double>(nd, dd);
          break;
        case 'C':
          fld[i] = new FitsArray<Complex>(nd, dd);
          break;
        case 'M':
          fld[i] = new FitsArray<DComplex>(nd, dd);
          break;
        case 'P':
          fld[i] = new FitsArray<FitsVADesc>(nd, dd);
          break;
        default:
          errmsg(BADRULES, "Invalid type code for TFORM");
          fld[i] = 0;
      ...
      switch (*s) {
        case 'L':
          fld[i] = new FitsField<FitsLogical>(ne);
          break;
        case 'X':
          fld[i] = new FitsField<FitsBit>(ne);
          break;
        case 'B':
          fld[i] = new FitsField<unsigned char>(ne);
          break;
        case 'I':
          fld[i] = new FitsField<short>(ne);
          break;
        case 'J':
          fld[i] = new FitsField<FitsLong>(ne);
          break;
        case 'A':
          fld[i] = new FitsField<char>(ne);
          break;
        case 'E':
          fld[i] = new FitsField<float>(ne);
          break;
        case 'D':
          fld[i] = new FitsField<double>(ne);
          break;
        case 'C':
          fld[i] = new FitsField<Complex>(ne);
          break;
        case 'M':
          fld[i] = new FitsField<DComplex>(ne);
          break;
        case 'P':
          fld[i] = new FitsField<FitsVADesc>(ne);
          break;
        default:
          errmsg(BADRULES, "Invalid type code for TFORM");
          fld[i] = 0;
  ...
  if (isoptimum) {
    for (i = 0; i < tfields(); ++i) table_offset[i] = fits_offset[i];
    // Must find the row alignment requirement
    n = 0;          // the number of fields scanned
    row_align = 0;  // the row alignment requirement
    for (i = 0; i < tfields(); ++i)
      if (fld[i]->fieldtype() == FITS::DCOMPLEX || fld[i]->fieldtype() == FITS::DOUBLE) {
        ++n;
        if (sizeof(double) > row_align) row_align = sizeof(double);
      }
    if (n < tfields())
      for (i = 0; i < tfields(); ++i)
        if (fld[i]->fieldtype() == FITS::COMPLEX || fld[i]->fieldtype() == FITS::FLOAT) {
          ++n;
          if (sizeof(float) > row_align) row_align = sizeof(float);
        }
    if (n < tfields())
      for (i = 0; i < tfields(); ++i)
        if (fld[i]->fieldtype() == FITS::VADESC || fld[i]->fieldtype() == FITS::LONG) {
          ++n;
          if (sizeof(FitsLong) > row_align) row_align = sizeof(FitsLong);
        }
    if (n < tfields())
      for (i = 0; i < tfields(); ++i)
        if (fld[i]->fieldtype() == FITS::SHORT) {
          ++n;
          if (sizeof(short) > row_align) row_align = sizeof(short);
        }
    if (n < tfields())
      for (i = 0; i < tfields(); ++i)
        if (fld[i]->fieldtype() == FITS::BYTE || fld[i]->fieldtype() == FITS::CHAR ||
            fld[i]->fieldtype() == FITS::LOGICAL || fld[i]->fieldtype() == FITS::BIT) {
          ++n;
          if (sizeof(unsigned char) > row_align) row_align = sizeof(unsigned char);
        }
  } else {
...
int BinaryTableExtension::readrow() {
  ...
  if (!isoptimum) {
    for (i = 0; i < tfields(); ++i) {
      ...
      switch (fld[i]->fieldtype()) {
        case FITS::LOGICAL:
          FITS::f2l((FitsLogical *)(fld[i]->data()), src, ne);
          break;
        case FITS::BIT:
          FITS::f2l((FitsBit *)(fld[i]->data()), src, ne);
          break;
        case FITS::CHAR:
          FITS::f2l((char *)(fld[i]->data()), src, ne);
          break;
        case FITS::BYTE:
          FITS::f2l((unsigned char *)(fld[i]->data()), src, ne);
          break;
        case FITS::SHORT:
          FITS::f2l((short *)(fld[i]->data()), src, ne);
          break;
        case FITS::LONG:
          FITS::f2l((FitsLong *)(fld[i]->data()), src, ne);
          break;
        case FITS::FLOAT:
          FITS::f2l((float *)(fld[i]->data()), src, ne);
          break;
        case FITS::DOUBLE:
          FITS::f2l((double *)(fld[i]->data()), src, ne);
          break;
        case FITS::COMPLEX:
          FITS::f2l((Complex *)(fld[i]->data()), src, ne);
          break;
        case FITS::DCOMPLEX:
          FITS::f2l((DComplex *)(fld[i]->data()), src, ne);
          break;
        case FITS::VADESC:
          FITS::f2l((FitsVADesc *)(fld[i]->data()), src, ne);
          break;
          // The following "default" has been added to prevent compilers
          // such as GNU g++ from complaining about the rest of FITS
          // enumerations not being handled in the switch statement.
...
int BinaryTableExtension::writerow(FitsOutput &fout) {
  ...
  if (!isoptimum) {
    for (i = 0; i < tfields(); ++i) {
      ...
      switch (fld[i]->fieldtype()) {
        case FITS::LOGICAL:
          FITS::l2f(tg, (FitsLogical *)(fld[i]->data()), ne);
          break;
        case FITS::BIT:
          FITS::l2f(tg, (FitsBit *)(fld[i]->data()), ne);
          break;
        case FITS::CHAR:
          FITS::l2f(tg, (char *)(fld[i]->data()), ne);
          break;
        case FITS::BYTE:
          FITS::l2f(tg, (unsigned char *)(fld[i]->data()), ne);
          break;
        case FITS::SHORT:
          FITS::l2f(tg, (short *)(fld[i]->data()), ne);
          break;
        case FITS::LONG:
          FITS::l2f(tg, (FitsLong *)(fld[i]->data()), ne);
          break;
        case FITS::FLOAT:
          FITS::l2f(tg, (float *)(fld[i]->data()), ne);
          break;
        case FITS::DOUBLE:
          FITS::l2f(tg, (double *)(fld[i]->data()), ne);
          break;
        case FITS::COMPLEX:
          FITS::l2f(tg, (Complex *)(fld[i]->data()), ne);
          break;
        case FITS::DCOMPLEX:
          FITS::l2f(tg, (DComplex *)(fld[i]->data()), ne);
          break;
        case FITS::VADESC:
          FITS::l2f(tg, (FitsVADesc *)(fld[i]->data()), ne);
          break;
          // The following "default" has been added to prevent compilers
          // such as GNU g++ from complaining about the rest of FITS
          // enumerations not being handled in the switch statement.
...
void BinaryTableExtension::set_fitsrow(Int n) {
  ...
  for (int i = 0; i < tfields(); ++i) data_addr[i] = (void *)(addr + table_offset[i]);
}
...
*/
int BinaryTableExtension::write_binTbl_hdr(
    FitsOutput &fout,     // I - FITS output object
    long naxis2,          // I - number of rows in the table
    int tfields,          // I - number of columns in the table
    const char **ttype,   // I - name of each column
...

{
  ...
  if (fout.rectype() == FITS::InitialState) {
    errmsg(BADOPER,
           "[BinaryTableExtension::write_bintbl_hdr()] Primary Header must be written first.");
    return -1;
  }
  if (!fout.hdu_complete()) {
    errmsg(BADOPER,
           "[BinaryTableExtension::write_bintbl_hdr()] Previous HDU incomplete -- cannot write "
           "header.");
    return -1;
  }
  if (!fout.isextend()) {
    errmsg(
        BADOPER,
        "[BinaryTableExtension::write_bintbl_hdr()] Cannot write extension HDU - EXTEND not True.");
    return -1;
  }
  if (!fout.required_keys_only()) {
    cerr << "\n[BinaryTableExtension::write_binTbl_hdr()] write_binTbl_hdr() works with other "
            "write_***_hdr()"
         << endl;
    cerr << "methods only. It will not work with write_hdr()." << endl;
    errmsg(BADOPER, "Used wrong header-writting function.");
  ...
  if (ffphbn(l_newfptr, naxis2, tfields, const_cast<char **>(ttype), const_cast<char **>(tform),
             const_cast<char **>(tunit), const_cast<char *>(extname), pcount, &l_status)) {
  ...
  if (ffflsh(l_newfptr, TRUE, &l_status)) {
    errmsg(BADOPER, "[PrimaryArray::write_priArr_hdr()] Error flushing buffer!");
  }
  OFF_T l_headstart, l_datastart, l_dataend;
  l_status = 0;
  // get size info of the current HDU
...
FitsArray<FitsBit>::FitsArray(int n, const int *d) : FitsField<FitsBit>(1) {
  int i;
  if (n > 0) {
    no_dims = n;
...
//================================================
int FitsArray<FitsBit>::dims() const { return no_dims; }
//================================================
int FitsArray<FitsBit>::dim(int n) const { return dimn[n]; }
//================================================
int *FitsArray<FitsBit>::vdim() { return dimn; }
//================================================
