...
int main() {
  try {
    ...
    FitsKeywordList kwl;
    if (aipsppFormat) {
      FITSHistoryUtil::addHistoryGroup(kwl, history, nstrings, "LOGTABLE");
    } else {
    ...
    FITSHistoryUtil::addHistoryGroup(kwl, otherHistory, otherHistory.size(), "OTHER");
    ...
    FITSHistoryUtil::addHistoryGroup(kwl, moreHistory, moreHistory.size(), "");
