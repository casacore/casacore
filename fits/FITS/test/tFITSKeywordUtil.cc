...
int main() {
  try {
    ...
    FITSKeywordUtil::addComment(
        myKeywords, "This comment will extend across\nmultiple lines.\nThree in this case.");
    ...
    for (uInt i = 0; i < myKeywords.nfields(); i++) {
      ...
      if (outField >= 0) {
        ...
        AlwaysAssertExit(inType == outType || (inType == TpShort && outType == TpInt) ||
        ...
        // check comments
        AlwaysAssertExit(myKeywords.comment(i) == myNewKeywords.comment(outField));
        ...
        switch (myKeywords.dataType(i)) {
          ...
          case TpFloat:
            ...
            break;
          case TpDouble:
            AlwaysAssertExit(myKeywords.asDouble(i) == myNewKeywords.asDouble(outField));
            break;
          case TpString:
            AlwaysAssertExit(myKeywords.asString(i) == myNewKeywords.asString(outField));
            break;
          case TpArrayBool:
            AlwaysAssertExit(allEQ(myKeywords.asArrayBool(i), myNewKeywords.asArrayBool(outField)));
            break;
          case TpArrayInt:
          ...
          case TpArrayDouble:
            AlwaysAssertExit(
                allEQ(myKeywords.asArrayDouble(i), myNewKeywords.asArrayDouble(outField)));
            break;
          case TpArrayString:
            AlwaysAssertExit(
                allEQ(myKeywords.asArrayString(i), myNewKeywords.asArrayString(outField)));
            break;
          default:
    ...
    AlwaysAssertExit(myNewKeywords.nfields() == 1 && myNewKeywords.fieldNumber("sarrayn") >= 0);

    // illegal array type
    Vector<Complex> cvec(1);
    cvec = 1.0;
    myKeywords = myNewKeywords = empty;
    nativeList = FITSKeywordUtil::makeKeywordList();
