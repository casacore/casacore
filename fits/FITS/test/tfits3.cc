...
int main() {
  ...
  cout << "Data type   " << pg.datatype() << "\n"
       ...
       << "Dimensions  " << pg.dims() << "\n";
  for (int n = 0; n < pg.dims(); n++) cout << "Axis " << (n + 1) << " size " << pg.dim(n) << endl;
  cout << pg << endl;  // Display the keyword list
  pg.write_hdr(fout);
  int i, j;
  for (i = 0; i < no_groups; ++i) {
    for (j = 0; j < no_parms; ++j) pg.rawparm(j) = i * 10 + j;  // assign the parms
    for (j = 0; j < no_data; ++j) pg.data(j) = i * 10 + j + 5;  // assign the data
