...
void cvt_table(AsciiTableExtension &x, FitsOutput &fout) {
  ...
  for (n = 0; n < 50 && n < x.nrows(); ++n) {  // display first 50 rows
    cout << x.currrow() << ": | ";
    if (x.field(0).nelements() != 0) cout << x.field(0);
    for (i = 1; i < x.ncols(); ++i) {
      cout << " | ";
  ...
  for (i = 0; i < x.ncols(); ++i) rs += x.field(i).fitsfieldsize();
  while (bk.next()) {
    // change the EXTENSION keyword
    if (bk.curr()->isreserved() && bk.curr()->kw().name() == FITS::XTENSION) {
      bk.del();
      bk.mk(FITS::XTENSION, "BINTABLE");
    }
    if (bk.curr()->isreserved() && bk.curr()->isindexed()) {
      // delete TBCOL and TNULL keywords
      if (bk.curr()->kw().name() == FITS::TBCOL || bk.curr()->kw().name() == FITS::TNULL) bk.del();
      ...
      // and convert NAXIS1
      if (bk.curr()->kw().name() == FITS::NAXIS && bk.curr()->index() == 1) {
        bk.del();
    ...
    for (i = 0; i < x.ncols(); ++i) {
      switch (fb[i]->fieldtype()) {
        case FITS::CHAR:
          ...
          strncpy(&((*target_c)(0)), &((*source_c)(0)), fb[i]->fitsfieldsize());
          break;
        case FITS::LONG:
