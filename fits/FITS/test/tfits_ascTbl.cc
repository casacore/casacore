...
int main() {
  ...
  long tbcol =
      0;  // byte offset in row to each column( default value will be calculated when passing in 0)

  // define the name, datatype, and physical units for the columns
  // I - name of each column
  const char *ttype[] = {"GSC_ID", "RA_DEG",  "DEC_DEG",  "POS_ERR",
                         "MAG",    "MAG_ERR", "MAG_BAND", "PLATE_ID"};
