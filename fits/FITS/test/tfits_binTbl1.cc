...
int main() {
  ...
  int i, j;
  for (i = 0; i < col; ++i)
    for (j = 0; j < row; ++j) data[i][j] = j * 10 + i;
  ...
  */
  ...
  const char *ttype[] = {"GSC_ID",   "RA_DEG", "DEC_DEG",  "POS_ERR", "MAG", "MAG_ERR",
                         "MAG_BAND", "CLASS",  "PLATE_ID", "MULTIPLE"};  // I - name of each column
  const char *tform[] = {"J", "E", "E",  "E", "E", "E",
                         "I", "I", "4A", "1A"};  // I - value of TFORMn keyword for each column.
  const char *tunit[] = {"\0", "deg", "deg", "deg", "cm", "cm",
                         "cm", "cm",  "\0",  "\0"};  // I - value of TUNITn keyword for each column
