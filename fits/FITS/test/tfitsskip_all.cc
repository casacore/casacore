// # tfitsskip_all.cc: FITS test program to skip the data unit of primary array,
// # ( calling the FitsInput::skip_all())and read and display other values
// # from a FITS file
...
#define DOGROUP(Z)                                                      \
  void do_primary_group(PrimaryGroup<Z> &x) {                           \
    int i, j;                                                           \
    ...
    for (i = 0; i < x.gcount(); ++i) {                                  \
      ...
      if (i < number_to_display) {                                      \
        cout << "Group " << i << " parms: " << "\n";                    \
        for (j = 0; j < x.pcount(); ++j) cout << " " << x.parm(j);      \
        cout << "\n";                                                   \
        cout << "Group " << i << " data: " << "\n";                     \
        for (j = 0; j < 4; ++j) cout << " " << x(j);                    \
        cout << "\n";                                                   \
      }                                                                 \
    }                                                                   \
    delete &x;                                                          \
...
#define DOARRAY(Z)                                                                          \
  void do_primary_array(PrimaryArray<Z> &x) {                                               \
    cout << "[ tftis1.cc::do_primary_array() ] called. " << endl;                           \
    int i, j, n0, n1;                                                                       \
    if (x.fitsdatasize()) {                                                                 \
      if (x.hdutype() == FITS::PrimaryArrayHDU) {                                           \
        cout << "[ tftis1.cc::do_primary_array() ] skipping the data unit of primaryarray." \
             << endl;                                                                       \
        x.skip();                                                                           \
      } else {                                                                              \
        x.read();                                                                           \
...
void do_binary_table(BinaryTableExtension &x) {
  ...
  cout << x.nrows() << " rows   " << x.ncols() << " cols   " << x.fitsdatasize() << " bytes total\n"
       << endl;
  cout << "----- Keyword List -----\n" << x << endl;
  ...
  int i;
  for (i = 0; i < x.ncols(); ++i) {
    cout << "Col " << i << ": " << x.field(i).nelements() << " " << x.field(i).fieldtype() << " "
         << x.ttype(i) << " " << x.tunit(i) << "\n";
  }
  cout << endl;
  ...
  for (i = 0; i < x.ncols(); ++i) {
    ...
    if (x.field(i).fieldtype() == FITS::VADESC) {
      ...
      if (vatypes[i] == FITS::NOVALUE) {
        cout << "Error in VA desc format for column " << i << " : " << x.tform(i) << endl;
      } else {
        switch (vatypes[i]) {
          case FITS::LOGICAL:
          ...
          case FITS::DCOMPLEX:
            ...
            break;
          default:
            cout << "Impossible VADesc type in column " << i << " : " << vatypes[i] << endl;
            break;
        }
      }
    } else {
  ...
  for (int n = 0; n < 50 && n < x.nrows(); ++n) {  // display first 50 rows
    ...
    for (i = 0; i < x.ncols(); ++i) {
      if (i != 0) cout << " | ";
      if (x.field(i).nelements() != 0) cout << x.field(i);
      if (x.field(i).fieldtype() == FITS::VADESC) {
        ...
        if (thisva.num()) {
          switch (vatypes[i]) {
            case FITS::LOGICAL: {
              ...
              cout << vptr[0];
              for (int k = 1; k < thisva.num(); ++k) cout << ", " << vptr[k];
            } break;
            case FITS::BIT: {
            ...
            case FITS::BYTE: {
              ...
              FITS::f2l(vptr, (void *)(theheap + thisva.offset()), thisva.num());
              cout << (int)vptr[0];
              for (int k = 1; k < thisva.num(); ++k) cout << ", " << (int)vptr[k];
            } break;
            case FITS::SHORT: {
            ...
            case FITS::CHAR: {
              ...
              FITS::f2l(vptr, (void *)(theheap + thisva.offset()), thisva.num());
              for (int k = 0; k < thisva.num() && vptr[k] != '\0'; ++k) cout << vptr[k];
            } break;
            case FITS::FLOAT: {
