...
int main(int argc, const char* argv[]) {
  try {
    ...
    inputs.create("input", "", "The input FITS file", "String");
    inputs.create("output", "", "The output Casacore Table name", "String");
    inputs.create("which_hdu", "1",
                  "0-relative to primary HDU, i.e. 1 is the smallest valid value.",

                  "Integer");
    inputs.create("storage", "standard",
                  "The storage manager to use - standard or incremental "
                  "(memory)",
                  "String");
    inputs.create("sdfits", "True",
    ...
    if (!infits->isValid()) {
      cerr << "The indicated FITS file does not have a valid binary table at HDU=" << whichHDU
           << endl;
      return 1;
    }
