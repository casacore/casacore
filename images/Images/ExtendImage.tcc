...
template <class T>
ExtendImage<T>::ExtendImage(const ImageInterface<T>& image, const IPosition& newShape,
                            const CoordinateSystem& newCsys)
    : itsImagePtr(image.cloneII()) {
  IPosition newAxes, stretchAxes;
  if (!CoordinateUtil::findExtendAxes(newAxes, stretchAxes, newShape, image.shape(), newCsys,
                                      image.coordinates())) {
    throw AipsError(
        "ExtendImage - "
...
template <class T>
Bool ExtendImage<T>::doGetSlice(Array<T>& buffer, const Slicer& section) {
  return itsExtLatPtr->doGetSlice(buffer, section);
}

template <class T>
void ExtendImage<T>::doPutSlice(const Array<T>& sourceBuffer, const IPosition& where,
                                const IPosition& stride) {
  itsExtLatPtr->doPutSlice(sourceBuffer, where, stride);
}

template <class T>
Bool ExtendImage<T>::doGetMaskSlice(Array<Bool>& buffer, const Slicer& section) {
...
template <class T>
LatticeIterInterface<T>* ExtendImage<T>::makeIter(const LatticeNavigator& navigator,
                                                  Bool useRef) const {
