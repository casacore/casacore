...
const char *FITSImgParser::storeKwords_p[] = {"HDUCLASS", "HDUDOC",   "HDUVERS", "HDUCLAS1",
                                              "HDUCLAS2", "HDUCLAS3", "SCIDATA", "ERRDATA",
                                              "QUALDATA", "QUALMASK"};
...
FITSImgParser::~FITSImgParser() { delete[] extensions_p; }
    ...
    if (error_ext.size() > 0) {
      ...
      if (error_HDU > -1) {
        ...
        if (actkeyw) {
          ...
          if (kw_errtype.size() > 0) error_type = kw_errtype;
        }
      }
    } else {
    ...
    if (mask_ext.size() > 0) {
      ...
      if (mask_HDU > -1) {
        ...
        if (actkeyw) {
          ...
          // set the value if possible
          if (kw_masktype.size() > 0) mask_type = kw_masktype;
        }
...
void FITSImgParser::setup(void) {
  ...
  if (fin.err() == FitsIO::IOERR)
    ...
    throw(AipsError("FITSImgParser::setup - " + name_p +
...
void FITSImgParser::process_extension(HeaderDataUnit *h, const uInt &extindex) {
  ...
  actkeyw = h->kw("EXTVER");
  if (actkeyw) extversion = actkeyw->asInt();
...
Bool FITSImgParser::confirm_fix_keywords(const Int &ext_index) {
  ...
  key_words(0) = String("HDUCLASS");
  key_words(1) = String("HDUDOC");
  key_words(2) = String("HDUCLAS1");
  key_values(0) = String("ESO");
  key_values(1) = String("DICD");
    ...
    if (index_is_HDUtype((Int)index, "DATA")) {
      ...
      */
      ...
      if (errext.size() > 0 || maskext.size() > 0) {
        ...
        if (errext.size() > 0) qualimgstr += String(",") + errext;
