...
Bool FITSQualityImage::qualFITSInfo(String& error, TableRecord& dataExtMiscInfo,
  ...
  if (miscInfo.fieldNumber("sciextname") > -1 &&
      miscInfo.type(miscInfo.fieldNumber("sciextname") == TpString)) {
  ...
  if (miscInfo.fieldNumber("errextname") > -1 &&
      miscInfo.type(miscInfo.fieldNumber("errextname") == TpString)) {
    // set the given extension name
    miscInfo.get(String("errextname"), tmpString);
    errorExtMiscInfo.define("extname", tmpString);

    // check for a dedicated extension name in the record "errextname"
    if (miscInfo.fieldNumber("hduclas3") > -1 &&
        miscInfo.type(miscInfo.fieldNumber("hduclas3") == TpString)) {
      // read the string
...
void FITSQualityImage::setCacheSizeFromPath(const IPosition& sliceShape,
  ...
  reopenIfNeeded();
  fitsdata_p->setCacheSizeFromPath(sliceShape, windowStart, windowLength, axisPath);
...
void FITSQualityImage::getExtInfo() {
  ...
  if (!fip.is_qualityimg(extexpr))
    throw(
        AipsError("FITSQualityImage::getExtInfo - "
                  "The extensions " +
                  extexpr + " in image: " + name_p + " can not be loaded as quality image!"));

  // determine all information to be able loading a quality image
  fip.get_quality_data(extexpr, whichDataHDU, whichErrorHDU, errTypeStr, whichMaskHDU, maskTypeStr,
                       maskValue);
  ...
  if (errTypeStr.size() > 0) {
    ...
    if (errType_p == FITSErrorImage::UNKNOWN)
      throw(
          AipsError("FITSQualityImage::getExtInfo - "
                    "Unknown ERRTYPE value: " +
                    errTypeStr));
  } else {
    os << LogIO::WARN
       << "No proper error type defined in the error extension. Assuming MSE (mean squared error)."
       << LogIO::POST;
  }
