...
template <class T>
void HDF5Image<T>::doPutSlice(const Array<T>& sourceBuffer, const IPosition& where,
...
void HDF5Image<T>::restoreUnits(const RecordInterface& rec) {
  ...
  if (rec.isDefined("units")) {
    if (rec.dataType("units") != TpString) {
      ...
      os << LogOrigin("HDF5Image<T>", "units()", WHERE)
         << "'units' keyword in hdf5image is not a string! Units not restored." << LogIO::SEVERE
         << LogIO::POST;
    } else {
  ...
  if (!unitName.empty()) {
    ...
    if (!UnitVal::check(unitName)) {
      LogIO os;
      os << LogOrigin("HDF5Image<T>", "units()", WHERE) << LogIO::SEVERE << "Unit '" << unitName
