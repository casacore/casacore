...
ImageAttrGroupCasa::ImageAttrGroupCasa(const Table& image, const String& attrName)
    : itsTable(image.keywordSet().subRecord("ATTRGROUPS").asTable(attrName)) {}

ImageAttrGroupCasa::~ImageAttrGroupCasa() { flush(); }
...
void ImageAttrGroupCasa::checkRows(const std::string& attrName, uInt rownr) {
  ...
  if (rownr > nrow) {
    throw AipsError("ImageAttrGroupCasa: row " + ValueToString(rownr) + " of attribute " +
                    attrName + " cannot be added; beyond current #rows " + ValueToString(nrow));
  }
  if (rownr == nrow) {
    itsTable.addRow(1);
  }
