...
ImageAttrHandler::~ImageAttrHandler() { flush(); }
...
ImageAttrGroup& ImageAttrHandler::openGroup(const String& groupName) {
  throw AipsError("ImageAttrHandler: openGroup " + groupName + " does not exist");
}

ImageAttrGroup& ImageAttrHandler::createGroup(const String& groupName) {
  throw AipsError("ImageAttrHandler: creation of group " + groupName + " cannot be done");
}
