...
void ImageAttrHandlerCasa::flush() {
  for (map<String, ImageAttrGroupCasa>::iterator it = itsGroupMap.begin(); it != itsGroupMap.end();
       ++it) {
    it->second.flush();
  }
}
...
ImageAttrGroup& ImageAttrHandlerCasa::openGroup(const String& groupName) {
  ...
  if (pos == itsGroupMap.end()) {
    throw AipsError("ImageAttrHandlerCasa: group " + groupName + " does not exist");
  }
...
ImageAttrGroup& ImageAttrHandlerCasa::createGroup(const String& groupName) {
  ...
  SetupNewTable newtab(itsImageTable.tableName() + '/' + groupName, TableDesc(), Table::New);
  Table tab(newtab);
  tab.flush();
