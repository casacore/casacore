...
ImageAttrHandlerHDF5& ImageAttrHandlerHDF5::attachHid(const HDF5Object& hid, Bool createHandler,
  ...
  if (HDF5Group::exists(hid, "ATTRGROUPS")) {
    ...
    if (!isWritable) {
      throw AipsError(
          "ImageAttrHandlerHDF5: cannot add ATTRGROUPS because "
...
void ImageAttrHandlerHDF5::flush() {
  for (map<String, ImageAttrGroupHDF5>::iterator it = itsGroupMap.begin(); it != itsGroupMap.end();
       ++it) {
...
ImageAttrGroup& ImageAttrHandlerHDF5::openGroup(const String& groupName) {
  ...
  if (pos == itsGroupMap.end()) {
    throw AipsError("ImageAttrHandlerHDF5: group " + groupName + " does not exist");
  }
