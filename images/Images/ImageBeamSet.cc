...
ImageBeamSet::ImageBeamSet(const Matrix<GaussianBeam>& beams) : _beams(beams) {
  ThrowIf(beams.empty(),
...
ImageBeamSet::ImageBeamSet(uInt nchan, uInt nstokes, const GaussianBeam& beam)
...
const GaussianBeam& ImageBeamSet::getBeam(Int chan, Int stokes) const {
  ...
  AlwaysAssert(chan >= 0 && chan < Int(nchan()) && stokes >= 0 && stokes < Int(nstokes()),
               AipsError);
...
const GaussianBeam& ImageBeamSet::getBeam() const {
  if (size() > 1) {
    ...
    throw AipsError(String(className()) + "::" + __FUNCTION__ + ": This object is empty.");
  }
  return _beams(0, 0);
}
...
void ImageBeamSet::setBeams(const Matrix<GaussianBeam>& beams) {
  ...
  AlwaysAssert((beamNchan == nch || beamNchan == 1) && (beamNstokes == nst || beamNstokes == 1),
               AipsError);
...
void ImageBeamSet::setBeam(Int chan, Int stokes, const GaussianBeam& beam) {
  AlwaysAssert(Int(chan) < _beams.shape()[0] && Int(stokes) < _beams.shape()[1], AipsError);
  if (chan < 0 && stokes < 0) {
    *this = ImageBeamSet(beam);
  } else if (chan >= 0 && stokes >= 0) {
    IPosition location(2, chan, stokes);
    _replaceBeam(beam, location, location, _maxBeamPos == location || _minBeamPos[1] == location);
  } else if (chan < 0) {
    // && stokes >= 0
    _replaceBeam(beam, IPosition(2, 0, stokes), IPosition(2, nchan() - 1, stokes),
                 _maxBeamPos[1] == stokes || _minBeamPos[1] == stokes);
  } else {
    // chan >=0 && stokes < 0
    _replaceBeam(beam, IPosition(2, chan, 0), IPosition(2, chan, nstokes() - 1),
...
const GaussianBeam& ImageBeamSet::getMaxAreaBeamForPol(IPosition& pos, uInt stokes) const {
  pos.resize(2);
...
const GaussianBeam& ImageBeamSet::getMinAreaBeamForPol(IPosition& pos, uInt stokes) const {
  pos.resize(2);
...
const GaussianBeam& ImageBeamSet::getMedianAreaBeamForPol(IPosition& pos, uInt stokes) const {
  pos.resize(2);
  pos = _beams.shape() - 1;
  if (nstokes() > 1) {
    pos[1] = stokes;
  ...
  Vector<uInt> indices;
  GenSortIndirect<Double, uInt>::sort(
      indices, _areas(IPosition(2, 0, pos[1]), IPosition(2, nchan() - 1, pos[1])));
...
GaussianBeam ImageBeamSet::getMedianAreaBeam() const {
  ...
  if (shape[0] > 1 && shape[1] > 1) {
    ...
    GaussianBeam medbeam = shape[0] > 1 ? _beams(indices[indices.size() / 2], 0)
...
const GaussianBeam ImageBeamSet::getSmallestMinorAxisBeam() const {
  ...
  for (BeamIter ibeam = _beams.begin(); ibeam != ibend; ++ibeam) {
    if (found) {
      ...
      if (test < minAxis || (test == minAxis && ibeam->getArea(_DEFAULT_AREA_UNIT) <
                                                    res.getArea(_DEFAULT_AREA_UNIT))) {
        minAxis = test;
        res = *ibeam;
      }
...
void ImageBeamSet::_calculateAreas() {
  ...
  if (!_beams.empty()) {
    ...
    BeamIter ibend = _beams.end();
    for (BeamIter ibeam = _beams.begin(); ibeam != ibend; ++ibeam, ++iareas) {
...
ImageBeamSet ImageBeamSet::subset(const Slicer& slicer, const CoordinateSystem& csys) const {
...
ImageBeamSet ImageBeamSet::fromRecord(const Record& rec) {
  ThrowIf(!rec.isDefined("nChannels"), "no nChannels field found");
  ThrowIf(!rec.isDefined("nStokes"), "no nStokes field found");
  ...
  for (auto iter = beams.begin(); iter != iterend; ++iter, ++count) {
    String field = "*" + ValueToString(count);
    ThrowIf(!rec.isDefined(field), "Field " + field + " is not defined");
...
Record ImageBeamSet::toRecord() const {
  ...
  for (const auto& beam : beams) {
    ThrowIf(beam.isNull(), "Invalid per plane beam found");
...
void ImageBeamSet::summarize(LogIO& log, Bool verbose, const CoordinateSystem& csys) const {
  ...
  for (Matrix<GaussianBeam>::const_iterator iter = _beams.begin(); iter != _beams.end(); iter++) {
    if (iter->getMajor("deg") < 1 / 3600 || iter->getMinor("deg") < 1 / 3600) {
      u = Unit("mas");
      break;
    }
    if (iter->getMajor("deg") < 1.0 || iter->getMinor("deg") < 1.0) {
      u = Unit("arcsec");
    }
  }
  ...
  if (hasSpectral) {
    ...
    if (myverbose) {
      ...
      os << std::setw(freqWidth) << "Freq" << " ";
      os << std::setw(velWidth) << "Vel";
    } else {
      if (hasStokes) {
  ...
  if (myverbose) {
    ...
    while (!iter.pastEnd()) {
      ...
      if (hasSpectral) {
        _chanInfoToStream(os, spCoord, pos[0], chanWidth, freqPrec, velWidth, velPrec);
      }
      if (hasStokes) {
    ...
    for (uInt i = 0; i < mymax; i++) {
      ...
      for (uInt j = 0; j < 3; j++) {
        ...
        switch (j) {
          ...
          case 2: {
            aggType = "Median";
            beam = getMedianAreaBeamForPol(pos, hasStokes ? i : -1);
            break;
          }
          default: {
...
const std::map<String, Quantum<Matrix<double>>> ImageBeamSet::paramMatrices(
    const Unit& majminUnit, const Unit& paUnit) const {
...
void ImageBeamSet::_chanInfoToStream(ostream& os, const SpectralCoordinate* spCoord,
  ...
  os << scientific << std::setprecision(freqPrec) << freq << " ";
  Double vel;
  spCoord->pixelToVelocity(vel, chan);
  os << std::setw(velWidth) << fixed << std::setprecision(velPrec) << vel << " ";
}

void ImageBeamSet::_beamToStream(ostream& os, const GaussianBeam& beam, const Unit& unit) {
