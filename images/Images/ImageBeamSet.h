...
class ImageBeamSet {
 public:
  ...
  const IPosition& shape() const { return _beams.shape(); }
  ...
  // Get the beam array.
  const Matrix<GaussianBeam>& getBeams() const { return _beams; }
  ...
  GaussianBeam getMinAreaBeam() const { return _minBeam; }

  // Get the beam in the set which has the largest area.
  // Get the beam in the set which has the largest area.
  GaussianBeam getMaxAreaBeam() const { return _maxBeam; }
  ...
  IPosition getMinAreaBeamPosition() const { return _minBeamPos; }

  // Get the position of the beam with the maximum area.
  IPosition getMaxAreaBeamPosition() const { return _maxBeamPos; }
  ...
  const std::map<String, Quantum<Matrix<double>>> paramMatrices(
      const Unit& majminUnit = Unit("arcsec"), const Unit& paUnit = "deg") const;
