...
void ImageConcat<T>::setImage(ImageInterface<T>& image, Bool relax) {
  ...
  if (nIm == 0) {
    ...
    this->rwImageInfo().combineBeams(image.imageInfo(), oldShape, image.shape(),
    ...
    ThrowIf(cSys.pixelAxisToWorldAxis(latticeConcat_p.axis()) < 0 ||
    ...
    if (!allEQ(cSys.worldAxisNames(), cSys0.worldAxisNames())) {
      ImageInfo::logMessage(warnAxisNames_p, os, relax, "Image axis names differ");
    }
    ...
    if (isContig_p) {
      _checkContiguous(pImLast->shape(), cSysLast, cSys, os, latticeConcat_p.axis(), relax);
    } else {
      ThrowIf(!relax,
  ...
  if (isContig_p) {
    ...
    try {
      ...
      if (!ImageInterface<T>::setCoordinateInfo(cSys)) {
        ...
        os << "Failed to save new CoordinateSystem with " << ctype << "Coordinate"
           << LogIO::EXCEPTION;
      }
  ...
  if (isImage_p(iIm)) {
    ...
    for (uInt j = 0; j < shapeNew; j++) {
      ...
      if (cSys2.toWorld(w, p)) {
        ...
        ThrowCc("Coordinate conversion failed because" + cSys2.errorMessage());
      }
    }
  } else {
