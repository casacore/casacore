...
template <class T>
ImageExpr<T>::ImageExpr(const LatticeExpr<T>& latticeExpr, const String& expr,
...
template <class T>
void ImageExpr<T>::init(const LatticeExpr<T>& latticeExpr, const String& expr,
                        const String& fileName, const JsonKVMap& jmap) {
  ...
  if (!pLattCoord->hasCoordinates() || pLattCoord->classname() != "LELImageCoord") {
    throw(
        AipsError("ImageExpr::constructor - the "
...
template <class T>
ImageExpr<T>::ImageExpr(const LatticeExpr<T>& latticeExpr, const String& expr,
                        const String& fileName, const LELImageCoord& imCoord)

    : latticeExpr_p(latticeExpr), fileName_p(fileName) {
  exprString_p = expr;
...
void ImageExpr<T>::save(const String& fileName) const {
  ...
  if (exprString_p.empty()) {
    throw AipsError(
        "ImageExpr cannot be made persistent, because "
...
template <class T>
void ImageExpr<T>::doPutSlice(const Array<T>&, const IPosition&, const IPosition&) {
  throw(
      AipsError("ImageExpr::putSlice - "
