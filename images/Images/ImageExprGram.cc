...
int ImageExprGramwrap() { return 1; }
...
class ImageExprGramState {
 public:
  ImageExprGramState(YY_BUFFER_STATE state) : itsState(state) {}
  ~ImageExprGramState() { clear(); }
  void clear() {
    if (itsState) {
...
void ImageExprGramerror(const char*) {
  throw(AipsError("Image Expression: Parse error at or near '" + String(ImageExprGramtext) + "'"));
}
...
String imageExprGramRemoveQuotes(const String& in) {
  ...
  while (pos < leng) {
    ...
    if (inx == std::string::npos) {
      throw(AipsError("ImageExprParse - Ill-formed quoted string: " + str));
    }
