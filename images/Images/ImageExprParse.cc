...
ImageExprParse::ImageExprParse(const String& value) : itsType(TpString), itsSval(value) {
  ThrowIf(itsSval.empty(), "Illegal empty expression");
}
...
LatticeExprNode ImageExprParse::command(const String& str, const String& dirName) {
...
LatticeExprNode ImageExprParse::command(const String& str,
  ...
  if (error) {
    ...
    throw(AipsError(message + '\n' +
                    "Scanned so far: " + command.substr(0, imageExprGramPosition())));
  }
...
LatticeExprNode ImageExprParse::makeValueList(const Block<LatticeExprNode>& values) {
...
Slice* ImageExprParse::makeSlice(const ImageExprParse& start, const ImageExprParse& end) {
...
LatticeExprNode ImageExprParse::makeLRNode() const {
  ...
  if (names.size() != 1) {
    if ((names.size() == 2 && names[1].empty()) ||
        (names.size() == 3 && !names[1].empty() && names[2].empty()) || names.size() > 3) {
  ...
  if (names.size() == 1) {
    ...
    if (Table::isReadable(fileName)) {
      ...
      throw(AipsError("ImageExprParse: the table used in region name'" + itsSval + "' is unknown"));
    }
  }
  ...
  if (regPtr == 0) {
    if (index == 0) {
      ...
      throw(AipsError("ImageExprParse: region '" + itsSval + " is an unknown region"));
    }
  }
...
String ImageExprParse::setAddDir(const String& dirName, const String& fileName) {
  theDirName = dirName;
...
Bool ImageExprParse::tryLatticeNode(LatticeExprNode& node, const String& name) const {
  ...
  if (table.nrow() != 1) {
    throw(
        AipsError("ImageExprParse can only handle Lattices/"
                  "Images with 1 row"));
  }
