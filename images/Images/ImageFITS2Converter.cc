...
Bool ImageFITSConverter::FITSToImage(ImageInterface<Float>*& newImage, String& error,
  ...
  while (!success && theHDU < numHDU) {
    ...
    if (whichHDU >= 0 || success) {
      ...
      while (theHDU < numHDU) {
        os << LogIO::WARN << "This HDU (" << theHDU << ") did not contain a legible image."
           << LogIO::POST;
        theHDU++;
        while (theHDU < numHDU && (infile.err() || infile.rectype() != FITS::HDURecord ||
                                   (infile.hdutype() != FITS::PrimaryArrayHDU &&
                                    infile.hdutype() != FITS::ImageExtensionHDU))) {
          infile.skip_hdu();
          theHDU++;
...
Bool ImageFITSConverter::ImageToFITS(String& error, ImageInterface<Float>& image,
  ...
  if (cSys.hasQualityAxis()) {
    ...
    if (!ImageFITSConverter::QualImgToFITSOut(error, os, image, outfile, memoryInMB, preferVelocity,
...
IPosition ImageFITSConverter::copyCursorShape(String& report, const IPosition& shape,
  ...
  if (axis == Int(ndim) - 1) {
    ...
    switch (axis) {
      case 0:
        buffer << "Copying row by row";
        break;
      case 1:
        buffer << "Copying plane by plane";
        break;
      case 2:
        buffer << "Copying cube by cube";
        break;
      default:
        buffer << "Copying hypercube by hypercube";
    }
  }
...
CoordinateSystem ImageFITSConverter::getCoordinateSystem(Int& stokesFITSValue,
  ...
  if (!CoordinateSystem::fromFITSHeader(stokesFITSValue, cSys, headerRec, header, shape,
                                        whichRep)) {
    os << LogIO::WARN
       << "No proper coordinate system defined in FITS file. Using dummy linear system instead."
       << LogIO::POST;
  ...
  if (shape.nelements() != cSys.nPixelAxes()) {
    ...
    if (cSys.nPixelAxes() > shape.nelements()) {
      ...
      //
      os << LogIO::NORMAL
         << "Image dimension appears to be less than number of pixel axes in CoordinateSystem"
         << endl;
...
void ImageFITSConverter::readBeamsTable(ImageInfo& info, const String& filename,
                                        const DataType type) {
      ...
      if (UnitVal::check(unitString)) {
        ...
        if ((bracketPos = unitString.find("(")) != String::npos ||
          ...
          os << LogIO::WARN << "FITS unit \"" << unitString
             << "\" unknown to CASA, was truncated to \"" << truncUnitString << "\"."
             << LogIO::POST;
        ...
        if (!uFixed) {  // try adding the most common problematic units occuring in BUNIT
          ...
          if (UnitVal::check(truncUnitString)) {
            ...
            os << LogIO::NORMAL << "FITS unit \"" << truncUnitString
               ...
               << "Correct units are always lower case except when derived from a name." << endl
               << "Please use \"beam\" instead of \"Beam\", \"pixel\" instead of \"Pixel\"."
               << LogIO::POST;
          }
        }

        if (!uFixed) {  // recovery attempt failed as well
          UnitMap::putUser("\"" + unitString + "\"", UnitVal(1.0, UnitDim::Dnon),
                           "\"" + unitString + "\"");
          os << LogIO::WARN << "FITS unit \"" << unitString
...
void ImageFITSConverter::restoreHistory(LoggerHolder& logger, ConstFitsKeywordList& kw) {
  Vector<String> lines;
  String groupType;
  kw.first();
  uInt n;
...
Bool ImageFITSConverter::removeFile(String& error, const File& outFile, const String& outName,
  ...
  if (basename.empty() || basename == "." || basename == "..") {
    throw AipsError("Invalid file path " + outFile.path().absoluteName() +
                    ". You really don't want me to delete the directory you're in.");
  }
  if (outFile.exists()) {
    if (allowOverwrite) {
      String msg;
...
Bool ImageFITSConverter::ImageHeaderToFITS(String& error, ImageFITSHeaderInfo& fhi,
    ...
    if (stokesLast) {
      ...
      for (uInt i = 0; i < ndim; i++) {  // loop over axes
        if (cNames(i) == "Stokes") {     // swap to back
          ...
          fhi.newShape(i) = shape(ndim - nStokes);
        }
      }
    }
  ...
  header.define("bscale", fhi.bscale);
  header.setComment("bscale", "PHYSICAL = PIXEL*BSCALE + BZERO");
  header.define("bzero", fhi.bzero);
  if (BITPIX > 0 && fhi.hasBlanks) {
    header.define("blank", fhi.minshort);
  ...
  if (!ok) {
    os << LogIO::SEVERE
       << "Could not make a standard FITS header. Setting"
    ...
    LinearCoordinate linear(cSys.worldAxisNames(), cSys.worldAxisUnits(), cSys.referenceValue(),
                            cSys.increment(), cSys.linearTransform(), cSys.referencePixel());
  ...
  for (i = 0; i < nmisc; i++) {
    ...
    if (tmp0.length() > 8) {
      os << LogIO::NORMAL << "Truncating miscinfo field " << tmp0 << " to " << miscname
         << LogIO::POST;
    }
    //
        ...
        switch (misctype) {
          ...
          case TpString:
            if (StringContains(miscname, "date") && miscname != "date") {
              ...
              // copy through
              if (FITSDateUtil::convertDateString(outdate, miscInfo.asString(i))) {
          ...
          default: {
            ...
            mytype << misctype;
            os << LogIO::NORMAL << "Not writing miscInfo field '" << miscname
               << "' - cannot handle type " << mytype.str() << LogIO::POST;
          }
        }
      }
      if (header.isDefined(miscname)) {
  ...
  if (history) {
    ...
    while (1) {
      ...
      FITSHistoryUtil::addHistoryGroup(fhi.kw, historyChunk, nstrings, groupType);
    }
  }
  //
  // END
...
Bool ImageFITSConverter::ImageToFITSOut(String& error, LogIO& os,
  ...
  if (fhi.needNonOptimalCursor && fhi.newShape.nelements() > 0) {
    ...
    newCursorShape = fhi.newShape(0);
  }

  if (verbose) {
    os << "Copying '" << image.name() << "' to file " << report << LogIO::POST;
  }
  ...
  try {
    ...
    for (iter.reset(); !iter.atEnd(); iter++) {
      ...
      if (fhi.applyMask) fhi.pMask->freeStorage(maskPtr, deleteMaskPtr);
      //
