...
void ImageFITSConverterImpl<HDUType>::FITSToImage(ImageInterface<Float>*& pNewImage, String& error,
  ...
  if (bitpix < 0 && isBlanked) {
    if (blankVal != -1) {
      ...
      os << LogIO::WARN << WHERE << "For floating point images, BLANK may only be set to -1<n"
         << blankVal
         << " is invalid. Ignoring (but will pass through "
            "NaN's."
         << LogIO::POST;
    }
  }
  ...
  if (!imageInfo.hasBeam() && !imageInfo.getRestoringBeam(logger)) {
    if (headerRec.isDefined(ImageFITSConverter::CASAMBM) &&
