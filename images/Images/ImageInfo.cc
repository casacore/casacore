...
ImageInfo::ImageInfo(const ImageInfo& other) : RecordTransformable() { copy_other(other); }
...
ImageInfo::ImageTypes ImageInfo::defaultImageType() { return ImageInfo::Intensity; }

String ImageInfo::defaultObjectName() { return String(); }
...
void ImageInfo::setRestoringBeam(const GaussianBeam& beam) {
  ThrowIf(_beams.hasMultiBeam(),
          "This object has multiple beams. They must be removed before you can define a single "
          "global restoring beam");
  ThrowIf(beam.isNull(), "Beam is null and therefore invalid.");
...
String ImageInfo::objectName() const { return itsObjectName; }
...
const ImageBeamSet& ImageInfo::getBeamSet() const { return _beams; }
...
void ImageInfo::checkBeamSet(const CoordinateSystem& coords, const IPosition& shape,
...
logSink << "Image " << imageName << " has no direction coordinate so "
  ...
  // Same for Stokes.
  ThrowIf(beamChannels != 1 && beamChannels != crdChannels, "Number of channels is not consistent");
  ThrowIf(beamStokes != 1 && beamStokes != crdStokes, "Number of polarizations is not consistent");
  // Check if no null beams.
  Array<GaussianBeam>::const_iterator iterEnd = _beams.getBeams().end();
  for (Array<GaussianBeam>::const_iterator iter = _beams.getBeams().begin(); iter != iterEnd;
       ++iter) {
    ThrowIf(iter->isNull(), "At least one of the beams in the beam set of " + imageName +
                                " is null and thus invalid");
  }
}
...
void ImageInfo::_checkBeamShape(uInt& nchan, uInt& npol, const IPosition& shape,
  ...
  AlwaysAssert(getBeamSet().nchan() == nchan || getBeamSet().nchan() == 1, AipsError);
  npol = 0;
  if (csys.hasPolarizationCoordinate()) {
    npol = shape[csys.polarizationAxisNumber()];
  }
  AlwaysAssert(getBeamSet().nstokes() == npol || getBeamSet().nstokes() == 1, AipsError);
}
...
void ImageInfo::combineBeams(const ImageInfo& infoThat, const IPosition& shapeThis,
  ...
  if (hasBeam() != infoThat.hasBeam()) {
    logMessage(_warnBeam, os, relax, "One image does not have a beam while another does",
...
uInt ImageInfo::setInfoSplitBeamSet(uInt ndone, const ImageInfo& concatInfo, const IPosition& shape,
  ...
  IPosition ss(shape);
  st[concatAxis] = ndone;
  if (csys.hasSpectralAxis() && concatAxis == csys.spectralAxisNumber() &&
      concatInfo.getBeamSet().nchan() > 1) {
    setBeams(concatInfo.getBeamSet().subset(Slicer(st, ss), csys));
    return shape[concatAxis];
  } else if (csys.hasPolarizationAxis() && concatAxis == csys.polarizationAxisNumber() &&
...
void ImageInfo::mergeBeams(ImageBeamSet& beamsOut, const ImageInfo& infoThat, Bool relax,
  ...
  if (nc1 == nc && np1 == np) {
    ...
    logMessage(_warnBeam, os, relax, "One beam set varies in frequency, the other in polarization",
