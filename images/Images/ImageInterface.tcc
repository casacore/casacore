...
Bool ImageInterface<T>::setCoordinateInfo(const CoordinateSystem& coords) {
  ...
  if (ok) {
    coords_p = coords;
    LogIO os;
    os << LogIO::DEBUGGING << "Changing coordinate system:\n"
       ...
       << "       delta = " << coords_p.increment() << " units = " << coords_p.worldAxisUnits()
       << endl
       << "linear xfrom = " << coords_p.linearTransform() << LogIO::POST;
  } else {
    LogIO os;
...
LELCoordinates ImageInterface<T>::lelCoordinates() const {
  return LELCoordinates(new LELImageCoord(coords_p, imageInfo_p, units(), miscInfo_p));
}
...
template <class T>
Vector<String> ImageInterface<T>::regionNames(RegionHandler::GroupType type) const {
