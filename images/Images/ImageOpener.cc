...
// Initialize registry.
std::map<ImageOpener::ImageTypes, ImageOpener::OpenImageFunction*> ImageOpener::theirOpenFuncMap;

void ImageOpener::registerOpenImageFunction(ImageTypes type, OpenImageFunction* func) {
...
ImageOpener::ImageTypes ImageOpener::imageType(const String& name) {
  ...
  if (file.isDirectory()) {
    ...
    if (Table::isReadable(name)) {
      ...
      if (File(name + "/header").isRegular() && File(name + "/image").isRegular()) {
        return MIRIAD;
      }
    }
    ...
    if (i > 0 && base.substr(i + 1) == "image") {
      String descName = std::string(file.path().dirName()) + '/' + base.substr(0, i) + ".descr";
...
LatticeBase* ImageOpener::openPagedImage(const String& fileName, const MaskSpecifier& spec) {
...
LatticeBase* ImageOpener::openExpr(const String& expr, const Block<LatticeExprNode>& nodes,
  ...
  switch (node.dataType()) {
    ...
    case TpComplex:
      ...
      break;
    case TpDComplex:
      lattice = new ImageExpr<DComplex>(LatticeExpr<DComplex>(node), expr, fileName, jmap);
      break;
    default:
