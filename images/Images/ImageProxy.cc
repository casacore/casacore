...
ImageProxy::ImageProxy(const ValueHolder& values, const ValueHolder& mask,
  ...
  if (!overwrite) {
    File file(fileName);
    if (file.exists()) {
      throw AipsError("file " + fileName + " already exists and should not be overwritten");
    }
  }
  switch (values.dataType()) {
    ...
    case TpArrayInt64:
    case TpArrayFloat:
      makeImage(values.asArrayFloat(), mask.asArrayBool(), IPosition(), coordinates, fileName,
                asHDF5, maskName, tileShape);
      break;
    case TpArrayDouble:
      makeImage(values.asArrayDouble(), mask.asArrayBool(), IPosition(), coordinates, fileName,
                asHDF5, maskName, tileShape);
      break;
    case TpArrayComplex:
      makeImage(values.asArrayComplex(), mask.asArrayBool(), IPosition(), coordinates, fileName,
                asHDF5, maskName, tileShape);
      break;
    case TpArrayDComplex:
      makeImage(values.asArrayDComplex(), mask.asArrayBool(), IPosition(), coordinates, fileName,
                asHDF5, maskName, tileShape);
      break;
    default:
...
Vector<String> ImageProxy::getAttrUnit(const String& groupName, const String& attrName) const {
  checkNull();
  return itsAttrHandler->openGroup(groupName).getUnit(attrName);
}

Vector<String> ImageProxy::getAttrMeas(const String& groupName, const String& attrName) const {
  checkNull();
...
void ImageProxy::putAttr(const String& groupName, const String& attrName, uInt rownr,
  ...
  checkNull();
  itsAttrHandler->openGroup(groupName).putData(attrName, rownr, value, units, measInfo);
}

ValueHolder ImageProxy::getData(const IPosition& blc, const IPosition& trc, const IPosition& inc) {
  IPosition shp = shape();
...
ValueHolder ImageProxy::getMask(const IPosition& blc, const IPosition& trc, const IPosition& inc) {
  IPosition shp = shape();
  Slicer slicer(adjustBlc(blc, shp), adjustTrc(trc, shp), adjustInc(inc, shp), Slicer::endIsLast);
  if (itsImageFloat) {
...
void ImageProxy::putData(const ValueHolder& value, const IPosition& blc, const IPosition& inc) {
  IPosition shp = shape();
...
void ImageProxy::putMask(const ValueHolder& value, const IPosition& blc, const IPosition& inc) {
  IPosition shp = shape();
  IPosition ablc = adjustBlc(blc, shp);
  IPosition ainc = adjustInc(inc, shp);
  if (itsImageFloat) {
...
void ImageProxy::saveAs(const String& fileName, Bool overwrite, Bool hdf5, Bool copyMask,
  ...
  if (itsImageFloat) {
    saveImage(fileName, hdf5, copyMask, newMaskName, newTileShape, *itsImageFloat);
  } else if (itsImageDouble) {
    saveImage(fileName, hdf5, copyMask, newMaskName, newTileShape, *itsImageDouble);
  } else if (itsImageComplex) {
    saveImage(fileName, hdf5, copyMask, newMaskName, newTileShape, *itsImageComplex);
  } else if (itsImageDComplex) {
    saveImage(fileName, hdf5, copyMask, newMaskName, newTileShape, *itsImageDComplex);
  } else {
    throw AipsError("ImageProxy does not contain an image object");
  }
}

TiledShape ImageProxy::makeTiledShape(const IPosition& newTileShape, const IPosition& shape,
...
void ImageProxy::saveImage(const String& fileName, Bool hdf5, Bool copyMask,
  ...
  TiledShape tiledShape(makeTiledShape(newTileShape, image.shape(), image.niceCursorShape()));
  if (hdf5) {
...
Record ImageProxy::statistics(const Vector<Int>& axes, const String& mask,
  ...
  if (itsImageFloat) {
    return makeStatistics(*itsImageFloat, axesc, mask, minMaxValues, exclude, robust);
  } else if (itsImageDouble) {
...
ImageProxy ImageProxy::regrid(const Vector<Int>& axes, const String& outFile, Bool overwrite,
  ...
  if (!overwrite) {
    File file(outFile);
    if (file.exists()) {
      throw AipsError("file " + outFile + " already exists and should not be overwritten");
    }
  }
  if (itsImageFloat) {
    return doRegrid(*itsImageFloat, axes, outFile, shape, coordSys, method, decimate, replicate,
                    doRefChange, forceRegrid);
  } else if (itsImageDouble) {
...
ImageProxy ImageProxy::doRegrid(const ImageInterface<T>& image, const Vector<Int>& axes,
  ...
  if (cSys.nPixelAxes() != outShape.nelements()) {
    throw AipsError(
        "The number of pixel axes in the output shape and "
...
CoordinateSystem ImageProxy::makeCoordinateSystem(const Record& coordinates,
