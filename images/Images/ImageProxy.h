...
class ImageProxy {
 public:
  ...
  static LatticeBase* openImage(const String& name, const String& mask = String(),
  ...
  static LatticeBase* openImageOrExpr(const String& str, const MaskSpecifier&,
  ...
  */
  ...
  LatticeBase* getLattice() const { return itsLattice.operator->(); }
 ...
 private:
  ...
  template <typename T>
  void doPutMask(ImageInterface<T>& image, const ValueHolder& value, const IPosition& blc,
                 const IPosition& inc);
  ...
  TiledShape makeTiledShape(const IPosition& newTileShape, const IPosition& shape,
