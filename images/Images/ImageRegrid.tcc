...
void ImageRegrid<T>::regrid(ImageInterface<T>& outImage, typename Interpolate2D::Method method,
  ...
  if (inImage.imageInfo().hasMultipleBeams()) {
    ThrowIf(inImage.coordinates().hasSpectralAxis() &&
                anyTrue(outPixelAxesU.asVector() == inImage.coordinates().spectralAxisNumber()),
            "This image has multiple beams. The spectral axis cannot be regridded");
    ThrowIf(inImage.coordinates().hasPolarizationCoordinate() &&
...

  {
    ...
    if (regridDirAxis && info.hasBeam()) {
      ...
      if ((method == Interpolate2D::NEAREST && inbeam / inpix < Quantity(5, "") &&
  ...
  for (uInt i = 0; i < nOutRegridPixelAxes; i++) {
    if (!doneOutPixelAxes(outPixelAxes[i])) {
      ...
      _regridOneCoordinate(os, outShape2, doneOutPixelAxes, finalOutPtr, inPtr, outPtr, outCoords,
                           inCoords2, outPixelAxes[i], inImage, outShape, replicate, decimate,
                           outIsMasked, showProgress, forceRegrid, method, verbose);
...
void ImageRegrid<T>::_regridOneCoordinate(
  ...
  if (inCoordinate == -1 || inAxisInCoordinate == -1) {
    ...
    oss2 << outPixelAxis + 1;
    String msg = "Output axis (" + oss2.str() + ") of coordinate type " + oss1.str() +
  ...
  if ((type == Coordinate::DIRECTION) ||
      (type == Coordinate::LINEAR && outPixelAxes.nelements() == 2 &&
    ...
    ThrowIf(outShape2(outPixelAxes[0]) == 1 && outShape2(outPixelAxes[1]) == 1,
    ...
    if (!regridIt) {
      if (verbose) {
        os << "Input and output shape/coordinate information for "
           << Coordinate::typeToString(cIn.type()) << " axes equal - no regridding needed"
           << LogIO::POST;
      }
      if (lastPass) {
    ...
    //
    regridTwoAxisCoordinate(os, *outPtr, *inPtr, inImage.units(), inCoords, outCoords, inCoordinate,
                            outCoordinate, inPixelAxes, outPixelAxes, pixelAxisMap1, pixelAxisMap2,
                            method, replicate, decimate, showProgress);
  } else {
    ...
    ThrowIf(!outCoords.setWorldAxisUnits(outUnits), "Failed to set output CoordinateSystem units");
    ...
    if (!regridIt) {
      if (verbose) {
        os << "Input and output shape/coordinate information for "
           << Coordinate::typeToString(inCoord.type()) << " axis equal - no regridding needed"
           << LogIO::POST;
      }
      if (lastPass) {
        // Can't avoid this copy
    ...
    regrid1D(*outPtr, *inPtr, inCoord, outCoord, inPixelAxes, outPixelAxes, inAxisInCoordinate,
             outAxisInCoordinate, pixelAxisMap2, method, machine, replicate, madeIt, showProgress);
  }
  //
  if (itsShowLevel > 0) {
    s0 += t0.all();
...
Bool ImageRegrid<T>::insert(ImageInterface<T>& outImage, const Vector<Double>& outPixel,
  ...
  //
  ThrowIf(outImage.ndim() != inImage.ndim(),
  ...
  for (uInt i = 0; i < nPixelAxes; i++) {
    ...
    ThrowIf(type == Coordinate::STOKES && outShape(i) != inShape(i),
...
CoordinateSystem ImageRegrid<T>::makeCoordinateSystem(
  ...
  IPosition outPixelAxes2 =
      outPixelAxes.nelements() == 0 ? IPosition::makeAxisPath(nPixelAxesFrom) : outPixelAxes;
  ...
  for (uInt i = 0; i < nCoordsFrom; i++) {
    ...
    if (regridIt) {
      ...
      if (iCoordTo < 0) {
        ...
        ThrowIf(cSysTo.pixelAxes(iCoordTo).nelements() != cSysFrom.pixelAxes(i).nelements(),
                "Wrong number of pixel axes in 'To' CoordinateSystem for "
                "coordinate of type " +
                    cSysTo.showType(iCoordTo));
...
void ImageRegrid<T>::regridTwoAxisCoordinate(
  ...
  t1.mark();
  if (itsUser2DCoordinateGrid.nelements() > 0 && itsUser2DCoordinateGridMask.nelements() > 0) {
    if (itsNotify) {
    ...
    //
    {
      ...
      if (shp1.isEqual(shp2)) {
        ...
        os << "User set 2D coordinate grid has the wrong shape" << LogIO::POST;
      }
    }
    {
      ...
      if (shp1.isEqual(shp2)) {
        ...
        os << "User set 2D coordinate grid mask has the wrong shape" << LogIO::POST;
      }
    }
    //
    allFailed = False;
    ...
    if (replicate) {
      ...
      make2DCoordinateGrid(os, allFailed, missedIt, minInX, minInY, maxInX, maxInY,
                           ...
                           inCoordinate, outCoordinate, xInAxis, yInAxis, xOutAxis, yOutAxis,
                           inPixelAxes, outPixelAxes, inShape, outPosFull, outShape, decimate);
    }
  }
  s1 += t1.all();
  ...
  for (outIter.reset(); !outIter.atEnd(); outIter++) {
    ...
    s3 += t3.all();
    if (itsShowLevel > 0) {
      cerr << "missedIt, allFailed, minInX, maxInX, minInY, maxInY = " << missedIt << ", "
           << allFailed << ", " << minInX << ", " << maxInX << ", " << minInY << ", " << maxInY
           << endl;
    }
    if (missedIt || allFailed) {
      ...
      t4.mark();
      ThrowIf(inChunkShape(xInAxis) == 1 && inChunkShape(yInAxis) == 1,
              "Cannot regrid degenerate DirectionCoordinate plane");
      ThrowIf(inChunkShape(xInAxis) == 1 || inChunkShape(yInAxis) == 1,
              "Cannot yet handle DirectionCoordinate plane with one "
              "degenerate axis");
      regrid2DMatrix(outCursor, outMaskIterPtr, interp, pProgressMeter, iPix, nDim, xInAxis,
                     yInAxis, xOutAxis, yOutAxis, scale, inIsMasked, outIsMasked, outPos,
                     outCursorShape, inChunkShape, inChunkBlc, pixelAxisMap2, inDataChunk,
                     inMaskChunkPtr, its2DCoordinateGrid, its2DCoordinateGridMask);
  ...
  if (itsShowLevel > 0) {
    ...
    if (replicate) {
      cerr << "      Function make2DCoordinateGrid (replication) took " << s1 << endl;
    } else {
      cerr << "      Function make2DCoordinateGrid (regridding)  took " << s1 << endl;
    }
...
void ImageRegrid<T>::make2DCoordinateGrid(
  ...
  if (isDir) {
    ...
    units.set("deg");
    ThrowIf(!inDir.setWorldAxisUnits(units),
            "Failed to set input DirectionCoordinate units to degrees");
    ThrowIf(!outDir.setWorldAxisUnits(units),
...
//
{
  ...
  if (blc(0) == 0 && blc(1) == 0 && trc(0) == (s(0) - 1) && trc(1) == (s(1) - 1)) {
    ...
    allFailed =
        minmax(minInX, maxInX, minInY, maxInY, in2DPos.xyPlane(0), in2DPos.xyPlane(1), succeed);

  } else {
    // Pull out the relevant piece

    allFailed = minmax(minInX, maxInX, minInY, maxInY, in2DPos.xyPlane(0)(blc, trc),
...
void ImageRegrid<T>::regrid2DMatrix(Lattice<T>& outCursor, LatticeIterator<Bool>*& outMaskIterPtr,
  ...
  LatticeStepper outCursorIterStepper(outCursor.shape(), outCursorIterShape, outCursorAxes,
                                      axisPath);
...
//
{
  ...
  auto method1D = InterpolateArray1D<typename NumericTraits<T>::BaseType, T>::linear;
  if (method == Interpolate2D::NEAREST) {
  ...
  for (outIter.reset(); !outIter.atEnd(); outIter++) {
    ...
    if (allGood) {
      if (outIsMasked) {
        InterpolateArray1D<typename NumericTraits<T>::BaseType, T>::interpolate(
            outIter.rwVectorCursor(), outMaskIterPtr->rwVectorCursor(), outX, inX, inY, inMask,
            method1D, goodIsTrue, extrapolate);
      } else {
        InterpolateArray1D<typename NumericTraits<T>::BaseType, T>::interpolate(
            outIter.rwVectorCursor(), dummyOutMask, outX, inX, inY, inMask, method1D, goodIsTrue,
            extrapolate);
      }
    } else {
      ...
      if (outIsMasked) {
        InterpolateArray1D<typename NumericTraits<T>::BaseType, T>::interpolate(
            outIter.rwVectorCursor(), outMaskIterPtr->rwVectorCursor(), outX, inX, inY,
            (failed && inMask), method1D, goodIsTrue, extrapolate);
      } else {
        InterpolateArray1D<typename NumericTraits<T>::BaseType, T>::interpolate(
            outIter.rwVectorCursor(), dummyOutMask, outX, inX, inY, (failed && inMask), method1D,
            goodIsTrue, extrapolate);
      }
    }
    //
    if (itsShowLevel > 1) {
...
void ImageRegrid<T>::_checkAxes(IPosition& outPixelAxes, const IPosition& inShape,
  ...
  for (Int i = 0; i < n1; i++) {
    ...
    if (type == Coordinate::STOKES) {
      os << LogIO::POST
         << "The Stokes axis cannot be regridded "
            "- removing from list"
         << endl;
    } else {
      Bool ok = True;
  ...
  for (Int i = 0; i < nOut; i++) {
    ...
    if (!foundIt && outShape(i) != inShape(pixelAxisMap1[i])) {
      ...
      oss << "Any axis not being regridded must have the same "
          << "input and output shapes. Output axis " << i << ", which corresponds to input axis "
          << pixelAxisMap1[i] << ", has a length of " << outShape(i)
          << ", whereas the corresponding input axis has length " << inShape(pixelAxisMap1[i]);
      ThrowCc(oss.str());
    }
  }
}
...
void ImageRegrid<T>::findMaps(uInt nDim, Vector<Int>& pixelAxisMap1, Vector<Int>& pixelAxisMap2,
  ...
  //
  if (itsShowLevel > 0) {
    cerr << "worldmap, worldtranspose, refChange = " << worldAxisMap << worldAxisTranspose
         << worldRefChange << endl;
