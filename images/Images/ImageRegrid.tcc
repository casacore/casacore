...
namespace casa { //# NAMESPACE CASA - BEGIN
                               ...
                               Bool forceRegrid)
  {
    ...
    if (inImage.imageInfo().hasMultipleBeams()) {
      if (inImage.coordinates().hasSpectralAxis()  &&
      ...
      if (inImage.coordinates().hasPolarizationCoordinate()  &&
          anyTrue(outPixelAxesU.asVector()
    ...
    _checkAxes(outPixelAxes, inShape, outShape, pixelAxisMap1, outCoords);
    {
      ...
      if (regridDirAxis) {
        ...
        Quantity inpix = min(Quantity(inc[0], units[0]),
                             Quantity(inc[1], units[1]));
        ...
        if ((method == Interpolate2D::NEAREST  &&
             inbeam/inpix < Quantity(5, "")  &&
             outpix/inpix > Quantity(0.5, ""))  ||
            (method == Interpolate2D::LINEAR  &&
             inbeam/inpix < Quantity(3, "")  &&
             outpix/inpix > Quantity(0.75, ""))  ||
            (method == Interpolate2D::CUBIC  &&
             inbeam/inpix < Quantity(3, "")  &&
    ...
    for (uInt i=0; i<nCoordsFrom; i++) {
      ...
      if (typeFrom == Coordinate::STOKES) {
        if (outPixelAxes.size() > 0) {
          os << LogIO::WARN
             << "A stokes coordinate cannot be regridded, ignoring"
             << LogIO::POST;
        }
        continue;
      }
      ...
      if (regridIt) {
        ...
        if (iCoordTo < 0) {
          os << LogIO::WARN << Coordinate::typeToString(typeFrom)
             << " coordinate is not present "
             << " in the output coordinate system, so it cannot be regridded"
             << LogIO::POST;
        } else {
          ThrowIf(cSysTo.pixelAxes(iCoordTo).nelements()
