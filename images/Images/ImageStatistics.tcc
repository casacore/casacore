...
void ImageStatistics<T>::displayStats(AccumType nPts, AccumType sum, AccumType median,
  ...
  if (LattStatsSpecialize::hasSomePoints(nPts)) {
    oss << "        -- Mean of the pixel values [mean]:         " << mean << " " << sbunit;
    messages.push_back(oss.str());
    oss.str("");
    if (!doBiweight) {
      oss << "        -- Variance of the pixel values :           " << var << " " << sbunit;
      messages.push_back(oss.str());
      oss.str("");
    }
    oss << "        -- Standard deviation of the Mean [sigma]:  " << sigma << " " << sbunit;
    messages.push_back(oss.str());
    oss.str("");
    if (!doBiweight) {
      oss << "        -- Root mean square [rms]:                  " << rms << " " << sbunit;
      messages.push_back(oss.str());
      oss.str("");
    }
    if (_showRobust) {
      oss << "        -- Median of the pixel values [median]:     " << median << " " << sbunit;
      messages.push_back(oss.str());
      oss.str("");
      ...
      messages.push_back(oss.str());
      oss.str("");
      oss << "        -- IQR [quartile]:                          " << quartile << " " << sbunit;
      messages.push_back(oss.str());
      oss.str("");
  ...
  for (vector<String>::const_iterator iter = messages.begin(); iter != messages.end();
       iter++, jiter++) {
