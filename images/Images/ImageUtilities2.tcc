...
void ImageUtilities::addDegenerateAxes(LogIO& os, std::unique_ptr<ImageInterface<T>>& outImage,
  ...
  if (outFile.empty()) {
    os << LogIO::NORMAL << "Creating (temp)image of shape " << shape << LogIO::POST;
    outImage.reset(new TempImage<T>(shape, cSys));
  } else {
    os << LogIO::NORMAL << "Creating image '" << outFile << "' of shape " << shape << LogIO::POST;
...
void ImageUtilities::bin(MaskedArray<T>& out, Coordinate& coordOut, const MaskedArray<T>& in,
  ...
  AlwaysAssert(
      type == Coordinate::LINEAR || type == Coordinate::SPECTRAL || type == Coordinate::TABULAR,
      AipsError);
  //
...
template <typename T, typename U>
void ImageUtilities::copyMask(ImageInterface<T>& out, const ImageInterface<U>& in,
...
template <typename T>
void ImageUtilities::openImage(ImageInterface<T>*& pImage, const String& fileName) {
  ThrowIf(fileName.empty(), "The image filename is empty");
  File file(fileName);
  ThrowIf(!file.exists(), "File '" + fileName + "' does not exist");
  FITSImage::registerOpenFunction();
  MIRIADImage::registerOpenFunction();
  LatticeBase* lattPtr = ImageOpener::openImage(fileName);
  ThrowIf(!lattPtr, "Image " + fileName + " cannot be opened; its type is unknown");
  if (lattPtr->dataType() != whatType<T>()) {
    delete lattPtr;
    ThrowCc("Logic Error: " + fileName +
