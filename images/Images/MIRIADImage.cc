...
MIRIADImage::~MIRIADImage() { delete pPixelMask_p; }

LatticeBase* MIRIADImage::openMIRIADImage(const String& name, const MaskSpecifier& spec) {
...
void MIRIADImage::setCacheSizeFromPath(const IPosition& sliceShape, const IPosition& windowStart,
...
void MIRIADImage::setup() {
  ...
  getImageAttributes(cSys, shape, imageInfo, brightnessUnit, rec_p, hasBlanks_p, fullName);
...
void MIRIADImage::open() {
  ...
  pTiledFile_p =
      std::make_shared<TiledFileAccess>(iname, fileOffset_p, shape_p.shape(), shape_p.tileShape(),
...
void MIRIADImage::getImageAttributes(CoordinateSystem& cSys, IPosition& shape, ImageInfo& imageInfo,
  ...
  for (i = 0; i < ndim; i++) {
    ...
    if (subRA == String("RA") || StringContains(ctype(i), "LON") || subRA == String("LL")) {
      ...
      stokesAxis = i;
    } else if (StringContains(ctype(i), "FREQ") || StringContains(ctype(i), "FELO") ||
    ...
    if (proj1 == "" && proj2 == "") {
      ...
      os << WHERE << LogIO::WARN
         << "No projection has been defined so cannot make a Celestial Coordinate\n"
            "from this miriad header.  Will make a LinearCoordinate instead"
         << LogIO::POST;
      longAxis = -1;
      latAxis = -1;
    }
  ...
  if (longAxis >= 0) {
    ...
    if (proj1 == "NCP") {
      os << LogIO::NORMAL
         << "NCP projection is now SIN projection in"
    ...
    if (::casacore::near(cdelt(latAxis), 0.0) && ::casacore::near(crpix(latAxis) + offset, 1.0) &&
        rotationAxis < 0) {
      cdelt(latAxis) = 1.0;  // degrees
    }
    //
    if (::casacore::near(cdelt(longAxis), 0.0) && ::casacore::near(crpix(longAxis) + offset, 1.0) &&
        rotationAxis < 0) {
      cdelt(longAxis) = 1.0;  // degrees
    }

    DirectionCoordinate dir(radecsys, projn, crval(longAxis), crval(latAxis), cdelt(longAxis),
                            cdelt(latAxis), dirpc, crpix(longAxis), crpix(latAxis), longPole,
                            latPole);
  ...
  if (spectralAxis >= 0) {
    ...
    if (!ok) {
      if (spectralAxisQualifier == "") {
        if ((velref % 256) >= 0) {
          ...
          os << LogIO::SEVERE << "Illegal value for VELREF(" << velref << ") assuming topocentric"
             << LogIO::POST;
        }
      } else {
        // unrecognized tag
        os << LogIO::SEVERE << "Unknown spectral reference frame " << spectralAxisQualifier
    ...
    if (StringContains(ctype(spectralAxis), "FREQ")) {
      ...
      if (restFrequency < 0) {
        ...
        // Have RESTFREQ
        os << LogIO::NORMAL
           << "ALTRVAL and ALTRPIX have not been "
        ...
        switch (velocityPreference) {
          case MDoppler::RADIO: {
            referenceFrequency = -rval / C::c * restFrequency + restFrequency;
            deltaFrequency = -delt * referenceFrequency / (C::c - rval);
          } break;
          case MDoppler::OPTICAL: {
    ...
    SpectralCoordinate tmp(refFrame, referenceFrequency, deltaFrequency, referenceChannel,
                           restFrequency);
    ...
    for (Int k = 0; k < shape(stokesAxis); k++) {
      ...
      switch (stokes(k)) {
        ...
        case -1:
          ...
          break;
        case 0: {
          os << LogIO::WARN << "Detected Stokes coordinate = 0; this is an unoffical" << endl;
        ...
        case 5:
          ...
          {
            os << LogIO::SEVERE
               << "The Stokes axis has the unofficial percentage polarization value." << endl;
        ...
        case 8:
          ...
          {
            os << LogIO::SEVERE
               << "The FITS image Stokes axis has the unofficial spectral index value." << endl;
