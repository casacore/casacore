...
void PagedImage<T>::applyMask(const String& maskName) {
  ...
  LatticeRegion* latReg = new LatticeRegion(regPtr->toLatticeRegion(coordinates(), shape()));
  delete regPtr;
...
void PagedImage<T>::resize(const TiledShape& newShape) {
  if (newShape.shape().nelements() != coordinates().nPixelAxes()) {
    throw(
        AipsError("PagedImage<T>::resize: coordinate info is "
                  "the incorrect shape."));
  }
...
Bool PagedImage<T>::setCoordinateInfo(const CoordinateSystem& coords) {
  ...
  if (ok) {
    ...
    if (tab.isWritable()) {
      ...
      LogIO os;
      os << LogIO::SEVERE << "Image " << name() << " is not writable; not saving coordinates"
         << LogIO::POST;
    }
  }
  return ok;
...
template <class T>
void PagedImage<T>::doPutSlice(const Array<T>& sourceBuffer, const IPosition& where,
  ...
  if (rec.isDefined("units")) {
    if (rec.dataType("units") != TpString) {
      ...
      os << LogOrigin("PagedImage<T>", "units()", WHERE)
         << "'units' keyword in image table is not a string! Units not restored." << LogIO::SEVERE
         << LogIO::POST;
    } else {
  ...
  if (!unitName.empty()) {
    ...
    if (!UnitVal::check(unitName)) {
      ...
      os << LogIO::WARN << "FITS unit \"" << unitName
         << "\" unknown to CASA - will treat it as non-dimensional." << LogIO::POST;
...
Bool PagedImage<T>::setImageInfo(const ImageInfo& info) {
  ...
  if (ok) {
    ...
    if (tab.isWritable()) {
      ...
      // Table not writable.
      LogIO os;
      os << LogIO::SEVERE << "Image " << name() << " is not writable; not saving ImageInfo"
         << LogIO::POST;
    }
  }
  return ok;
...
void PagedImage<T>::restoreImageInfo(const TableRecord& rec) {
  if (rec.isDefined("imageinfo")) {
    ...
    if (ok) {
      ...
      LogIO os;
      os << LogIO::WARN << "Failed to restore the ImageInfo in image " << name() << "; " << error
         << LogIO::POST;
    }
  }
}
