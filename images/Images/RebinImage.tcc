...
RebinImage<T>::RebinImage(const ImageInterface<T>& image, const IPosition& factors)
  ...
  ThrowIf(image.imageInfo().hasMultipleBeams() && image.coordinates().hasSpectralAxis() &&
...
RebinImage<T>::RebinImage(const RebinImage<T>& other)
    : ImageInterface<T>(other), itsImagePtr(other.itsImagePtr->cloneII()) {
...
template <class T>
void RebinImage<T>::doPutSlice(const Array<T>& sourceBuffer, const IPosition& where,
