...
template <class T>
void TempImage<T>::doPutSlice(const Array<T>& buffer, const IPosition& where,
...
template <class T>
void TempImage<T>::setCacheSizeFromPath(const IPosition& sliceShape, const IPosition& windowStart,
                                        const IPosition& windowLength, const IPosition& axisPath) {
  mapPtr_p->setCacheSizeFromPath(sliceShape, windowStart, windowLength, axisPath);
}
