...
void testVectorROIter(const Lattice<Float>& extendlat, const Lattice<Float>& lattice, Int nnew) {
  Int nstep;
  ...
  try {
    {
      ...
      for (Int i = 0; i < 4; i++) {
        for (Int j = 0; j < 3; j++) {
          Array<Float> parr = arr1(IPosition(4, 0, i, j, 0), IPosition(4, 16 - 1, i, j, 6 - 1));
          AlwaysAssertExit(allEQ(parr.reform(latticeShape), arr));
        }
      }
