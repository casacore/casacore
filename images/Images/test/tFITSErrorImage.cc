...
int main(int argc, const char* argv[]) {
  try {
    ...
    // Give some feedback
    if (verbose) cerr << "Input image: " << in << " extension: " << hdunum << endl;
    ...
    AlwaysAssert(FITSErrorImage::errorTypeToString(FITSErrorImage::INVRMSE) == "INVRMSE",
                 AipsError);
    AlwaysAssert(FITSErrorImage::MSE == FITSErrorImage::DEFAULT, AipsError);
    AlwaysAssert(FITSErrorImage::stringToErrorType("WHATSUP") == FITSErrorImage::UNKNOWN,
                 AipsError);
    ...
    if (verbose) cerr << "Checked the image type." << endl;
    ...
    fitsErrImage.tempClose();
    if (print) printArray(fitsErrImage, size);
    ...
    if (verbose) cerr << "Compared data, mask as well as coordinate systems." << endl;
    ...
    if (verbose) cerr << "Compared cloned data, mask as well as coordinate systems." << endl;

    // Test copy
    FITSErrorImage fitsErrImage2 = FITSErrorImage(fitsErrImage);
    if (verbose) cerr << "Checked the copy constructor." << endl;
    ...
    AlwaysAssert(fitsCS2.near(dataCS), AipsError);
    if (verbose) cerr << "Compared copied data, mask and coordinate systems." << endl;

    // Test assignment
    fitsErrImage2 = fitsErrImage;
    if (verbose) cerr << "Checked the assignment operator." << endl;
    AlwaysAssert(fitsErrImage2.errorType() == defErrType, AipsError);
    if (verbose) cerr << "Checked for ErrorType in copied instance." << endl;
    ...
    AlwaysAssert(allNear(dataArray, dataMask, fitsArray3, fitsMask3), AipsError);
    AlwaysAssert(fitsCS2.near(dataCS), AipsError);
    if (verbose) cerr << "Compared assigned data, mask and coordinate systems." << endl;
    ...
    if (verbose) cerr << "Opened a RMSE error image." << endl;
    ...
    if (verbose) cerr << "Checked type and error type" << endl;
    ...
    // Print some data and mask values if desired
    fitsErrImage2.tempClose();
    if (print) printArray(fitsErrImage2, size);
    ...
    fitsErrImage2.tempClose();
    if (verbose) cerr << "Opened a INVMSE error image." << endl;
    ...
    AlwaysAssert(allNear(tmpData, dataMask, fitsArray3, fitsMask3), AipsError);
    AlwaysAssert(fitsCS2.near(dataCS), AipsError);
    if (verbose) cerr << "Compared data and coordinate systems in INVMSE image." << endl;
    ...
    fitsErrImage2.tempClose();
    if (verbose) cerr << "Opened an INVRMSE error image." << endl;
