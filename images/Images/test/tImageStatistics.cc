...
void writeTestString(const String& test) { cout << "\n" << "*** " << test << " ***" << endl; }
...
int main() {
  ...
  try {
    {
      writeTestString("test normal and copy constructors");
    ...
    {
      writeTestString("test multi-beam images");
      ...
      GaussianBeam beam(Quantity(3, "arcmin"), Quantity(2.5, "arcmin"), Quantity(30, "deg"));
      ...
      info.setBeam(1, 1, beam.getMajor() + Quantity(0.2, "arcmin"), beam.getMinor(), beam.getPA());
    ...
    {
      writeTestString("test getConvertedStatistic(FLUX) and getStats() for FLUX");
      ...
      ImageStatistics<Float> stats(tim);
      Array<Float> flux;
      AlwaysAssert(stats.getConvertedStatistic(flux, LatticeStatsBase::FLUX), AipsError);
      ...
      uInt count = 0;
      while (bIter != bEnd) {
        *bIter = GaussianBeam(Quantity(3 + count, "arcmin"), Quantity(2.5, "arcmin"),
                              Quantity(30, "deg"));
        ++count;
        ++bIter;
      }
      ...
      AlwaysAssert(stats.getConvertedStatistic(fluxDensities, LatticeStatsBase::FLUX), AipsError);
