...
void doTypes() {
  ...
  AlwaysAssertExit(ImageOpener::imageType("tImageUtilities_tmp/mir.img") == ImageOpener::UNKNOWN);
  {
    RegularFile rfile("tImageUtilities_tmp/mir.img/header");
    rfile.create();
  }
  AlwaysAssertExit(ImageOpener::imageType("tImageUtilities_tmp/mir.img") == ImageOpener::MIRIAD);
  {
    RegularFile rfile("tImageUtilities_tmp/a.image");
    rfile.create();
  }
  AlwaysAssertExit(ImageOpener::imageType("tImageUtilities_tmp/a.image") == ImageOpener::UNKNOWN);
  char buf[2880];
  memset(buf, ' ', 2880);
  memcpy(buf, "SIMPLE  =   T  ", 15);
  {
    RegularFileIO file(RegularFile("tImageUtilities_tmp/a.image"), ByteIO::Update);
    file.write(2879, buf);
  }
  AlwaysAssertExit(ImageOpener::imageType("tImageUtilities_tmp/a.image") == ImageOpener::UNKNOWN);
  {
  ...
  AlwaysAssertExit(ImageOpener::imageType("tImageUtilities_tmp/a.image") == ImageOpener::FITS);
  {
    RegularFile rfile("tImageUtilities_tmp/a.descr");
    rfile.create();
  }
  AlwaysAssertExit(ImageOpener::imageType("tImageUtilities_tmp/a.image") == ImageOpener::GIPSY);
  //
  dir.removeRecursive();
}
...
void listWorld(const Vector<Quantum<Double>>& wPars) {
  ...
  if (wPars.nelements() == 5) {
    ...
    cerr << "  Major, minor, pa = " << wPars(2) << ", " << wPars(3) << ", " << wPars(4) << endl;
  }
}
...
void listWorld(const GaussianBeam& wPars) {
  cerr << "World" << endl;
  if (!wPars.isNull()) {
    cerr << "  Major, minor, pa = " << wPars.getMajor() << ", " << wPars.getMinor() << ", "
         << wPars.getPA() << endl;
  }
}
...
void listPixel(const Vector<Double>& pPars) {
  ...
  if (pPars.nelements() == 3) {
    ...
    cerr << "  Major, minor, pa = " << pPars(2) << ", " << pPars(3) << ", " << pPars(4) / C::degree
         << endl;
  }
}
