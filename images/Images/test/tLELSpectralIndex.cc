...
Bool doIt() {
  ...
  }
  {
    ...
    PagedImage<Float> pa2(TiledShape(shp2, IPosition(3, 4, 3, 6)), cSys2,
                          "tLELSpectralIndex_tmp.pa2");
  ...
  }
  {
    ...
    Array<Float> expect = log(arr1a / arr2) / arrf;
    Array<Float> subarr = expect(IPosition(3, 0, 0, 0), IPosition(3, shp2(0) - 1, shp2(1) - 1, 0));
    subarr = 0;
    if (!allNear(result, expect, 1e-5)) {
      cout << expect << endl;
      cout << result << endl;
