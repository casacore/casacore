...
RegionHandlerHDF5::RegionHandlerHDF5(GetCallback* callback, void* objectPtr)
    : itsChanged(False), itsCallback(callback), itsObjectPtr(objectPtr) {}
...
RegionHandlerHDF5& RegionHandlerHDF5::operator=(const RegionHandlerHDF5& that) {
  if (this != &that) {
    itsChanged = that.itsChanged;
    itsCallback = that.itsCallback;
    itsObjectPtr = that.itsObjectPtr;
...
RegionHandlerHDF5* RegionHandlerHDF5::clone() const { return new RegionHandlerHDF5(*this); }

void RegionHandlerHDF5::setObjectPtr(void* objectPtr) { itsObjectPtr = objectPtr; }

Bool RegionHandlerHDF5::canDefineRegion() const { return True; }
...
Bool RegionHandlerHDF5::defineRegion(const String& name, const ImageRegion& region,
  ...
  itsRecord.rwSubRecord(groupName).defineRecord(name, region.toRecord(file()->getName()));
  itsChanged = True;
  return True;
}

Bool RegionHandlerHDF5::hasRegion(const String& name, RegionHandler::GroupType type) const {
  ...
  if (groupField >= 0) {
    if (groupField != oldGroupField) {
      throw(AipsError("RegionHandlerHDF5::renameRegion - file " + file()->getName() +
                      " already has a region or mask with name " + newName + " in another group"));
    }
    if (!overwrite) {
      throw(AipsError("RegionHandlerHDF5::renameRegion - file " + file()->getName() +
...
Int RegionHandlerHDF5::findRegionGroup(const String& regionName, RegionHandler::GroupType type,
  ...
  if (throwIfUnknown) {
    ...
    throw(AipsError("RegionHandlerHDF5: " + typeName + regionName + " does not exist in file " +
                    file()->getName()));
  }
  return -1;
}

ImageRegion RegionHandlerHDF5::makeMask(const LatticeBase& lattice, const String& name) {
  if (!lattice.isPaged()) {
    throw(
        AipsError("RegionHandlerHDF5::makeMask - "
                  "cannot create mask, because image is transient"));
  }
  LCHDF5Mask* mask =
      new LCHDF5Mask(TiledShape(lattice.shape(), lattice.niceCursorShape()), file(), name);
...
void RegionHandlerHDF5::restore() { itsRecord = HDF5Record::readRecord(*file(), "maskinfo"); }
