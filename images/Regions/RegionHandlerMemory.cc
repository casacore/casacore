...
RegionHandlerMemory::RegionHandlerMemory(const RegionHandlerMemory& that) : RegionHandler(that) {
  operator=(that);
}

RegionHandlerMemory::~RegionHandlerMemory() { clear(); }

RegionHandlerMemory& RegionHandlerMemory::operator=(const RegionHandlerMemory& that) {
  if (this != &that) {
    clear();
...
Bool RegionHandlerMemory::defineRegion(const String& name, const ImageRegion& region,
  ...
  if (groupField >= 0) {
    if (!overwrite) {
      throw(
          AipsError("RegionHandlerMemory::defineRegion -"
                    " a region or mask with name " +
                    name + " already exists"));
    }
...
Bool RegionHandlerMemory::renameRegion(const String& newName, const String& oldName,
  ...
  if (groupField >= 0) {
    if (groupField != oldGroupField) {
      throw(
          AipsError("RegionHandlerMemory::renameRegion -"
                    " a region or mask with name " +
                    newName + " already exists in another group"));
    }
    if (!overwrite) {
      throw(
          AipsError("RegionHandlerMemory::renameRegion -"
...
Bool RegionHandlerMemory::removeRegion(const String& name, RegionHandler::GroupType type,
  ...
  if (groupField >= 0) {
    ...
    if (regPtr->isLCRegion()) {
      ...
      delete lcPtr;
      if (error) {
        throw(AipsError("RegionHandlerMemory - region " + name + " could not be removed\n" + msg));
      }
    }
...
Int RegionHandlerMemory::findRegionGroup(const String& regionName, RegionHandler::GroupType type,
  ...
  if (throwIfUnknown) {
    ...
    throw(AipsError("RegionHandlerMemory - " + typeName + regionName + " does not exist"));
  }
  return -1;
}

ImageRegion RegionHandlerMemory::makeMask(const LatticeBase& lattice, const String&) {
