...
RegionHandlerTable::RegionHandlerTable(GetCallback* callback, void* objectPtr)
    : itsCallback(callback), itsObjectPtr(objectPtr) {}

RegionHandlerTable::RegionHandlerTable(const RegionHandlerTable& that)
    : RegionHandler(that), itsCallback(that.itsCallback), itsObjectPtr(that.itsObjectPtr) {}

RegionHandlerTable::~RegionHandlerTable() {}

RegionHandlerTable& RegionHandlerTable::operator=(const RegionHandlerTable& that) {
  if (this != &that) {
...
RegionHandlerTable* RegionHandlerTable::clone() const { return new RegionHandlerTable(*this); }
...
Int RegionHandlerTable::findRegionGroup(const String& regionName, RegionHandler::GroupType type,
  ...
  if (throwIfUnknown) {
    ...
    throw(AipsError("RegionHandlerTable: " + typeName + regionName + " does not exist in table " +
                    tab.tableName()));
  }
  return -1;
}

ImageRegion RegionHandlerTable::makeMask(const LatticeBase& lattice, const String& name) {
  if (!lattice.isPaged()) {
    throw(
        AipsError("RegionHandlerTable::makeMask - "
                  "cannot create mask, because image is transient"));
  }
  LCPagedMask* mask = new LCPagedMask(TiledShape(lattice.shape(), lattice.niceCursorShape()),
                                      lattice.name() + '/' + name);
