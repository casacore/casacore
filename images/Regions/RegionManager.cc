...
RegionManager::RegionManager() : itsCSys(nullptr) { itsLog = new LogIO(); }
...
String RegionManager::absreltype(const Int absrelval) {
  ...
  *itsLog << LogIO::WARN << "absrelvalue " << absrelval << " is not valid" << LogIO::POST;
...
ImageRegion* RegionManager::wpolygon(const Vector<Quantity>& x, const Vector<Quantity>& y,
  ...
  *itsLog << LogOrigin("RegionManager", "wpolygon");
  ...
  for (uInt k = 0; k < nvertices; ++k) {
    ...
    } else if ((StringContains(x[k].getUnit(), "pix") && !StringContains(y[k].getUnit(), "pix")) ||
               (!StringContains(x[k].getUnit(), "pix") && StringContains(y[k].getUnit(), "pix"))) {
  ...
  for (uInt i = 0; i < innerRadii.size(); i++) {
    if (innerRadii[i].getValue() > outerRadii[i].getValue(innerRadii[i].getUnit())) {
      throw AipsError("RegionManager::" + std::string(__FUNCTION__) + ": For radius " +
                      ...
                      " is greater than outer radius " + ValueToString(outerRadii[i]));
    }
  }
  RegionType::AbsRelType leType = RegionType::absRelTypeFromString(absrel);
...
ImageRegion* RegionManager::doUnion(const WCRegion& reg1, const WCRegion& reg2) {
...
ImageRegion* RegionManager::doUnion(const ImageRegion& reg1, const ImageRegion& reg2) const {
  *itsLog << LogOrigin("RegionManager", String(__FUNCTION__) + "_3");
  *itsLog << LogIO::DEBUGGING << "reg1 type " << reg1.isWCRegion() << " " << reg1.isLCRegion()
          << " " << reg1.isLCSlicer() << "\nreg2 type " << reg2.isWCRegion() << " "
          << reg2.isLCRegion() << " " << reg2.isLCSlicer() << LogIO::POST;
...
ImageRegion* RegionManager::doIntersection(const WCRegion& reg1, const WCRegion& reg2) {
  ImageRegion imageReg1(reg1);
  ImageRegion imageReg2(reg2);
...
ImageRegion* RegionManager::doIntersection(const ImageRegion& reg1, const ImageRegion& reg2) {
...
ImageRegion* RegionManager::doDifference(const WCRegion& reg1, const WCRegion& reg2) {
  ImageRegion imageReg1(reg1);
  ImageRegion imageReg2(reg2);
...
ImageRegion* RegionManager::doDifference(const ImageRegion& reg1, const ImageRegion& reg2) {
...
ImageRegion* RegionManager::doConcatenation(const WCRegion& region, const WCBox& box) {
...
ImageRegion* RegionManager::doConcatenation(const Block<const ImageRegion*>& regions,
...
ImageRegion* RegionManager::doConcatenation(const Record& regions, const TableRecord& box) {
  *itsLog << LogOrigin("RegionManager", "doConcatenation");
...
Bool RegionManager::writeImageFile(const String& file, const String& regionname,
  ...
  try {
    ...
    throw(AipsError(
        String("Could not create the region file.\n Please check pathname and directory \n") +
