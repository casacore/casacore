...
WCBox::WCBox(const Vector<Quantum<Double>>& blc, const Vector<Quantum<Double>>& trc,
  ...
  if (itsAbsRel.nelements() != 0 && itsAbsRel.nelements() != itsBlc.nelements()) {
...
WCBox::WCBox(const LCRegion& region, const CoordinateSystem& cSys)
    //
  ...
  if (start.nelements() != itsCSys.nPixelAxes() || end.nelements() != itsCSys.nPixelAxes()) {
...
WCBox* WCBox::fromRecord(const TableRecord& rec, const String&) {
...
String WCBox::className() { return "WCBox"; }

String WCBox::type() const { return className(); }
