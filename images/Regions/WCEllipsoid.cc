...
WCEllipsoid::WCEllipsoid(const Vector<Quantity>& center, const Vector<Quantity>& radii,
                         const IPosition& pixelAxes, const CoordinateSystem& csys,
                         const RegionType::AbsRelType absRel)
    : _center(center),
      _radii(radii),
      _pixelAxes(pixelAxes),
      _csys(csys),
      _absRel(absRel),
      _theta(Quantity(0, "rad")),
      _specType(NOT_SPECIAL) {
  _init();
}

WCEllipsoid::WCEllipsoid(const Vector<Quantity>& center, const Quantity& radius,
                         const IPosition& pixelAxes, const CoordinateSystem& csys,
                         const RegionType::AbsRelType absRel)
    : _center(center),
      _radii(_center.size(), radius),
      _pixelAxes(pixelAxes),
      _csys(csys),
      _absRel(absRel),
      _theta(Quantity(0, "rad")),
...
WCEllipsoid::WCEllipsoid(const Quantity& xcenter, const Quantity& ycenter,
  ...
  if (!theta.isConform("rad")) {
    throw AipsError(String(__FUNCTION__) + ": theta is not an angular quantity");
  }
  if (!xcenter.isConform(ycenter)) {
    throw AipsError(String(__FUNCTION__) + ": xcenter and ycenter do not have the same base unit");
  }
  ...
  if (majorAxis.getValue() < minorAxis.getValue(majorAxis.getUnit())) {
    throw AipsError(String(__FUNCTION__) + ": major axis is smaller than minor axis.");
  }
...
String WCEllipsoid::className() { return "WCEllipsoid"; }
...
TableRecord WCEllipsoid::toRecord(const String&) const {
  ...
  }
  {
    ...
    if (!qh.toRecord(error, rec2)) {
      throw(AipsError("WCEllipsoid::" + String(__FUNCTION__) + ": could not save theta because " +
                      error));
    }
...
WCEllipsoid* WCEllipsoid::fromRecord(const TableRecord& rec, const String&) {
    ...
    switch (specType) {
      case SPHERE: {
        ...
        if (!qh.fromRecord(error, radiusRecord)) {
          throw(AipsError("WCEllipsoid::fromRecord - could not recover sphere radius because of " +
                          error));
        }
      }
      ...
      case ELLIPSE_2D:
        AlwaysAssert(pixelAxes.size() == 2, AipsError);
        {
          ...
          if (!qh.fromRecord(error, thetaRecord)) {
            throw(AipsError(
                "WCEllipsoid::fromRecord - could not recover 2-d ellipse theta because of " +
                error));
          }
        }
        theta = qh.asQuantity();
  ...
  switch (specType) {
    case SPHERE:
      ellipsoid = new WCEllipsoid(center, radius, pixelAxes, *csys, absRel);
      break;
    case ELLIPSE_2D:
      ellipsoid = new WCEllipsoid(center[0], center[1], radii[0], radii[1], theta, pixelAxes[0],
                                  pixelAxes[1], *csys, absRel);
      break;
    default:
      ellipsoid = new WCEllipsoid(center, radii, pixelAxes, *csys, absRel);
  }
  delete csys;
  return ellipsoid;
}
...
LCRegion* WCEllipsoid::doToLCRegion(const CoordinateSystem& csys, const IPosition& latticeShape,
  ...
  switch (_specType) {
    ...
    case ELLIPSE_2D:
      ...
      return new LCEllipsoid(outCenter[0], outCenter[1], outRadius[0], outRadius[1],
...
void WCEllipsoid::_checkUnits() const {
  ...
  try {
    ...
    throw AipsError(std::string(x.what()) + " Checking center units");
  }
}
