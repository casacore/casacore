...
LCRegion* WCExtension::doToLCRegion(const CoordinateSystem& cSys, const IPosition& shape,
  ...
  LCRegion* regptr = regions()[0]->toLCRegionAxes(cSys, shape, regPixMap, regOutOrd);
  if (ndstr > 0) {
...
String WCExtension::className() { return "WCExtension"; }

String WCExtension::type() const { return className(); }
