...
WCPolygon::WCPolygon(const LCPolygon& polyLC, const IPosition& pixelAxes,
...
WCPolygon* WCPolygon::fromRecord(const TableRecord& rec, const String&) {
  ...
  //
  {
    ...
    if (!h.fromRecord(error, subRecord)) {
      throw(
          AipsError("WCPolygon::fromRecord - could not recover X Quantum vector because " + error));
    }
  ...
  }
  {
    ...
    if (!h.fromRecord(error, subRecord)) {
      throw(
          AipsError("WCPolygon::fromRecord - could not recover Y Quantum vector because " + error));
    }
...
LCRegion* WCPolygon::doToLCRegion(const CoordinateSystem& cSys, const IPosition& latticeShape,
...
String WCPolygon::className() { return "WCPolygon"; }

String WCPolygon::type() const { return className(); }
