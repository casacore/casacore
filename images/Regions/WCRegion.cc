...
WCRegion::WCRegion(const WCRegion& other)
    : itsComment(other.itsComment), itsAxesDesc(other.itsAxesDesc) {}
...
Bool WCRegion::isAxisDescEqual(const Record& desc1, const Record& desc2) const {
  ...
  for (uInt j = 0; j < nf; j++) {
    ...
    switch (desc2.dataType(j)) {
      ...
      default:
        throw(
            AipsError("WCRegion::isAxisEqual - "
...
LCRegion* WCRegion::toLCRegion(const CoordinateSystem& cSys, const IPosition& shape) const {
  ...
  if (shape.nelements() != cSys.nPixelAxes()) {
    ...
    os << "WCRegion::" << __FUNCTION__ << ": shape has " << shape.nelements()
       << " elements, the coordinate system has " << cSys.nPixelAxes()
       << " axes. The actual shape is " << shape;
...
LCRegion* WCRegion::toLCRegionAxes(const CoordinateSystem& cSys, const IPosition& shape,
  ...
  LCRegion* extPtr = new LCExtension(*regPtr, extendAxes, LCBox(extendShape));
  delete regPtr;
  return extPtr;
}
...
void WCRegion::checkAxes(const IPosition& pixelAxes, const CoordinateSystem& cSys,
  ...
  for (uInt i = 0; i < pixelAxes.size(); i++) {
    ...
    if (worldAxes[i] == -1) {
      throw(AipsError("WCRegion::" + std::string(__FUNCTION__) + "from " + std::string(type()) +
                      ": pixelAxes[" + ValueToString(i) + "]=" + ValueToString(pixelAxes[i]) +
                      " has no corresponding world axis"));
    }
    std::string unit = quantityUnits[i];
    if (unit == "default") {
      throw(AipsError("WCRegion::" + String(__FUNCTION__) + "from " + type() +
                      ": default units are not allowed"));
    }
    if (unit != "pix" && unit != "frac") {
      if (Unit(unit) != Unit(units(worldAxes[i]))) {
        throw(AipsError("WCRegion::" + std::string(__FUNCTION__) + "from " + std::string(type()) +
                        ": units of quantity[" + ValueToString(i) + "]=" + unit +
                        " are inconsistent with units of coordinate system" + "units (" +
                        std::string(units(worldAxes[i])) + ")"));
      }
    }
  }
}
