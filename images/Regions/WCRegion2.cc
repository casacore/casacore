...
WCRegion* WCRegion::fromRecord(const TableRecord& rec, const String& tableName) {
  if (!rec.isDefined("isRegion") || rec.asInt("isRegion") != RegionType::WC) {
    throw(
        AipsError("WCRegion::fromRecord - "
  ...
  if (name == WCBox::className()) {
    ...
    throw(AipsError("WCRegion::fromRecord - " + name + " is unknown derived WCRegion class"));
  }
