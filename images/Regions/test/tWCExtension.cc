...
void testRegionBox(const CoordinateSystem& cSys, const IPosition& regAxes,
                   const IPosition& boxAxes) {
  ...
  if (!resBlc.isEqual(bbox.start())) {
    cout << regAxes << boxAxes << " Expected blc " << resBlc << ", found " << bbox.start() << endl;
  }
  if (!resTrc.isEqual(bbox.end())) {
    cout << regAxes << boxAxes << " Expected trc " << resTrc << ", found " << bbox.end() << endl;
  }
  delete regptr;
}
