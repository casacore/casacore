...
int main(int argc, const char* argv[]) {
  try {
    ...
    inputs.create("refval", "", "New center of the image in degrees (reference value)",
                  "Block<Double>");
    inputs.create("interpolation", "linear",
                  "Interpolation method (linear, nearest, cubic,lanczos)");
    ...
    DirectionCoordinate dirCoordFrom(itsImage->coordinates().directionCoordinate(dircoordNo));
    ...
    DirectionCoordinate dirCoordTo(mdirt, project, refValFrom(0), refValFrom(1), incrFrom(0),
                                   incrFrom(1), dirCoordFrom.linearTransform(), refPixFrom(0),
                                   refPixFrom(1));
