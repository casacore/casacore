...
public:
    ...
    ImageProxy(const casa::ImageInterface<casa::Float>* inImage);
    
    virtual ~ImageProxy();

    Bool addnoise(const String& type, const Vector<Double>& pars,
		  Record& region, Bool zero = False);
    ...
    Bool imagefromarray(const String& outfile, Array<Float>& pixels, 
			...
			Bool overwrite = False, Bool log = True);
    ...
    Bool imagefromfits(const String& outfile, const String& infile, 
		       Int whichrep = 0, Int whichhdu = 0, 
		       Bool zeroblanks = False, 
		       Bool overwrite = False, Bool old = False);

    Bool imagefromforeign(const String& outfile, const String& infile, 
			  const String& format, Bool overwrite = False);
   ...
   Matrix<Float> decompose(Record& region, const String& mask, 
    ...
    Record findsources(Int nmax, Double cutoff, Record& region, 
			const String& mask, Bool point = True, 
			Int width = 5, Bool negfind = False);

    Bool fitallprofiles(Record& region, Int axis, const String& mask,
			Int ngauss, Int poly, 
			const String& sigma = "", const String& fit = "", 
			const String& resid = "");

    Record fitprofile(Vector<Float>& values, Vector<Float>& resid, 
		       Record& region, Int axis, const String& mask, 
		       Record& estimate, Int ngauss = -1, 
		       Int poly = -1, Bool fit = True, 
		       const String sigma = "");
    ...
    Record fitsky(Array<Float>& pixels, Array<Bool>& pixelmask, 
		   ...
		   Bool fit = True, 
		   Bool deconvolve = False, Bool list = True);
    ...
    Bool getchunk(Array<Float>& pixel, Array<Bool>& pixmask, 
		  ...
		  Bool list = False, Bool dropdeg = False, 
		  Bool getmask = False);
    ...
    Bool histograms(Record& histout, const Vector<Int>& axes, Record& region, 
		    const String& mask, Int nbins, 
		    const Vector<Double>& includepix, Bool gauss, 
		    Bool cumu, Bool log, Bool list, 
		    const String& plotter, Int nx, Int ny, 
		    const Vector<Int>& size, Bool force = False, 
		    Bool disk = False);
    ...
    Bool lock(Bool writelock = False, Int nattempts = 0);
    ...
    Bool modify(Record& model, Record& region , const String& mask, 
		Bool subtract = True, Bool list = True);

    Record maxfit(Record& region, Bool point, Int width = 5, 
		   Bool negfind = False, Bool list = True);

    ImageInterface<Float> * moments(const Vector<Int>& moments, Int axis,
				    Record& region, const String& mask, 
    ...
    String name(Bool strippath = False);
    ...
    Bool putchunk(const Array<Float>& pixels, const Vector<Int>& blc, 
		  const Vector<Int>& inc, Bool list = False, 
		  Bool locking = True, Bool replicate = False);

    Bool putregion(const Array<Float>& pixels, const Array<Bool>& pixelmask, 
		   Record& region, Bool list = False, 
		   Bool usemask = True, 
		   Bool locking = True, Bool replicate = False);
    ...
    Bool replacemaskedpixels(const String& pixels, Record& region,
			     const String& mask, Bool update = False, 
			     Bool list = False);
    ...
    Bool set(const String& pixels, Int pixelmask, 
	     Record& region, Bool list = false);
    ...
    Bool setrestoringbeam(const Quantity& major, const Quantity& minor, 
			  ...
			  Bool remove = False, Bool log = True);

    Bool statistics(Record& statsout, const Vector<Int>& axes, Record& region, 
		    const String& mask, const Vector<String>& plotstats, 
		    const Vector<Float>& includepix, 
		    const Vector<Float>& excludepix, 
		    const String& plotter = "/NULL", Int nx = 1, 
		    Int ny = 1, Bool list = True, 
		    Bool force = False, Bool disk = False, 
    ...
    Record setboxregion(const Vector<Double>& blc, const Vector<Double>& trc,
			Bool frac = False, const String& infile = "");

    //make test image...cube or 2d (default)
    bool maketestimage(const String& outfile="", Bool overwrite=False, 
    ...
    Record* echo(Record& v, Bool godeep = False);
