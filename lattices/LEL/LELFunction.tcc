...
template <class T>
void LELFunction1D<T>::eval(LELArray<T>& result, const Slicer& section) const {
#if defined(AIPS_TRACE)
...
// LELFunctionReal1D
template <class T>
LELFunctionReal1D<T>::LELFunctionReal1D(const LELFunctionEnums::Function function,
                                        const std::shared_ptr<LELInterface<T>>& expr)
    : function_p(function) {
  switch (function_p) {
...
template <class T>
void LELFunctionReal1D<T>::eval(LELArray<T>& result, const Slicer& section) const {
#if defined(AIPS_TRACE)
...
LELScalar<T> LELFunctionReal1D<T>::getScalar() const {
  ...
  switch (function_p) {
    ...
    case LELFunctionEnums::MEDIAN1D: {
      ...
      Vector<T> median = LatticeFractile<T>::maskedFractile(LatticeExpr<T>(pExpr_p), 0.5);
...
template <class T>
void LELFunctionND<T>::eval(LELArray<T>& result, const Slicer& section) const {
#if defined(AIPS_TRACE)
  ...
  switch (function_p) {
    case LELFunctionEnums::IIF: {
      ...
      if (arg_p[0].isScalar()) {
        ...
        // of them has a mask.
        Array<Bool> newMask;
        Bool makeMask = (arg_p[1].isInvalidScalar() || arg_p[1].isMasked() ||
                         arg_p[2].isInvalidScalar() || arg_p[2].isMasked());
        ...
        if (arg_p[1].isScalar()) {
          ...
          if (arg_p[2].isScalar()) {
            ...
            if (makeMask) {
              maskData = newMask.getStorage(deleteMask);
              if (tmp.isMasked()) {
                const Bool* tmpMaskData = tmp.mask().getStorage(deleteTmpMask);
                for (size_t i = 0; i < n; i++) {
                  if (!tmpbData[i]) {
                    resData[i] = tmpData[i];
...
Bool LELFunctionND<T>::prepareScalarExpr() {
  ...
  for (i = 0; i < arg_p.nelements(); i++) {
    ...
    if (invalid && function_p != LELFunctionEnums::IIF && function_p != LELFunctionEnums::REPLACE) {
      return True;
    }
  }
