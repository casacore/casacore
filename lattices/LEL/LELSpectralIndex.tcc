...
LELSpectralIndex<T>::LELSpectralIndex(const Block<LatticeExprNode>& expr) {
  ...
  itsFreqAxis = attr0.coordinates().coordinates().getSpectralInfo(freq0, attr0.shape());
  Int freqAxis1 = attr1.coordinates().coordinates().getSpectralInfo(freq1, attr1.shape());
...
template <class T>
void LELSpectralIndex<T>::eval(LELArray<T>& result, const Slicer& section) const {
