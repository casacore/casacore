...
template <class T>
LELUnaryConst<T>::LELUnaryConst(const T val) : val_p(val) {
  setAttr(LELAttribute());
...
template <class T>
void LELUnaryConst<T>::eval(LELArray<T>&, const Slicer&) const {
...
template <class T>
void LELUnary<T>::eval(LELArray<T>& result, const Slicer& section) const {
#if defined(AIPS_TRACE)
