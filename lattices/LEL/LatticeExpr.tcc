...
LatticeExpr<T>::LatticeExpr(const LatticeExprNode& expr)
  ...
  if (!expr.isScalar() && shape_p.nelements() == 0) {
    throw AipsError(
        "LatticeExpr cannot be constructed from a lattice "
...
//
{
  ...
  if (!expr.isScalar() && expr.shape().nelements() > 0 && !(shape_p.isEqual(expr.shape()))) {
    throw AipsError(
        "LatticeExpr::constructor - "
...
void LatticeExpr<T>::init(const LatticeExprNode& expr) {
  ...
  if (expr.dataType() == thisDT) {
    ...
    if (expr.dataType() == TpBool) {
      throw(
          AipsError("LatticeExpr::constructor - "
...
template <class T>
void LatticeExpr<T>::doPutSlice(const Array<T>&, const IPosition&, const IPosition&) {
