...
#if defined(AIPS_TRACE)
  cout << "LatticeExprNode::destructor; pExpr_p.nrefs() = " << pExprDouble_p.nrefs() << endl;
#endif
}
...
#if defined(AIPS_TRACE)
  cout << "LatticeExprNode:: Unary constructor (T); pExpr_p.nrefs() = " << pExprDouble_p.nrefs()
       << endl;
#endif
}
...
#if defined(AIPS_TRACE)
  cout << "LatticeExprNode:: Unary constructor (T); pExpr_p.nrefs() = " << pExprComplex_p.nrefs()
       << endl;
#endif
}
...
#if defined(AIPS_TRACE)
  cout << "LatticeExprNode:: Unary constructor (T); pExpr_p.nrefs() = " << pExprDComplex_p.nrefs()
       << endl;
#endif
}
...
#if defined(AIPS_TRACE)
  cout << "LatticeExprNode:: Unary constructor (T); pExpr_p.nrefs() = " << pExprBool_p.nrefs()
       << endl;
#endif
}
...
#if defined(AIPS_TRACE)
  cout << "LatticeExprNode:: constructor (Lattice<T>); pExpr_p.nrefs() = " << pExprBool_p.nrefs()
       << endl;
#endif
}
...
void LatticeExprNode::unlock() {
  switch (dataType()) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::unlock - "
                    "unknown data type"));
  }
}
...
void LatticeExprNode::resync() {
  switch (dataType()) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::resync - "
                    "unknown data type"));
  }
}
...
//
{
  switch (dataType()) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::replaceScalarExpr - "
                    "unknown data type"));
  }
  return isInvalid_p;
}
...
void LatticeExprNode::eval(LELArray<Complex>& result, const Slicer& section) const {
  // If first time, try to do optimization.
...
void LatticeExprNode::eval(LELArray<DComplex>& result, const Slicer& section) const {
  // If first time, try to do optimization.
...
#endif
  AlwaysAssert(expr.dataType() == TpFloat || expr.dataType() == TpDouble, AipsError);
  Block<LatticeExprNode> arg(1);
  arg[0] = expr.makeFloat();
  return LatticeExprNode(std::make_shared<LELFunctionFloat>(LELFunctionEnums::SIGN, arg));
}
...
LatticeExprNode formComplex(const LatticeExprNode& left, const LatticeExprNode& right) {
  ...
  if (left.dataType() == TpFloat && right.dataType() == TpFloat) {
    ...
    return LatticeExprNode(std::make_shared<LELFunctionComplex>(LELFunctionEnums::COMPLEX, arg));
  }
  arg[0] = left.makeDouble();
  arg[1] = right.makeDouble();
  return LatticeExprNode(std::make_shared<LELFunctionDComplex>(LELFunctionEnums::COMPLEX, arg));
}
...
LatticeExprNode variance(const LatticeExprNode& expr) {
  ...
  if (expr.dataType() == TpFloat) {
    return toFloat(sum(pow(expr - toDouble(mean(expr)), 2)) / max(1, nelements(expr) - 1));
  } else if (expr.dataType() == TpComplex) {
    return toComplex(sum(pow(expr - toDComplex(mean(expr)), 2)) / max(1, nelements(expr) - 1));
  }
...
LatticeExprNode avdev(const LatticeExprNode& expr) {
  ...
  // Use high enough precision for Float and Complex.
  if (expr.dataType() == TpFloat) {
    return toFloat(sum(abs(expr - toDouble(mean(expr)))) / max(1, nelements(expr)));
  } else if (expr.dataType() == TpComplex) {
    return toComplex(sum(abs(expr - toDComplex(mean(expr)))) / max(1, nelements(expr)));
  }
...
LatticeExprNode fractile(const LatticeExprNode& expr, const LatticeExprNode& fraction) {
  ...
  switch (dtype) {
    case TpFloat:
      return LatticeExprNode(std::make_shared<LELFunctionFloat>(LELFunctionEnums::FRACTILE1D, arg));
    case TpDouble:
      return LatticeExprNode(
          std::make_shared<LELFunctionDouble>(LELFunctionEnums::FRACTILE1D, arg));
    default:
      throw(
          AipsError("LatticeExprNode::fractile - "
...
LatticeExprNode fractileRange(const LatticeExprNode& expr, const LatticeExprNode& fraction) {
  ...
  switch (dtype) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::fractileRange - "
                    "Bool or complex argument used in real "
                    "numerical function"));
  }
  return LatticeExprNode();
...
LatticeExprNode fractileRange(const LatticeExprNode& expr, const LatticeExprNode& fraction1,
...
LatticeExprNode rebin(const LatticeExprNode& lat, const LatticeExprNode& bin) {
#if defined(AIPS_TRACE)
...
#endif
  AlwaysAssert(left.dataType() != TpBool && right.dataType() != TpBool, AipsError);
...
LatticeExprNode pa(const LatticeExprNode& left, const LatticeExprNode& right) {
  ...
  AlwaysAssert(
      left.dataType() != TpComplex && left.dataType() != TpDComplex && left.dataType() != TpBool,
      AipsError);
  AlwaysAssert(
      right.dataType() != TpComplex && right.dataType() != TpDComplex && right.dataType() != TpBool,
      AipsError);
...
#endif
  DataType dtype = LatticeExprNode::resultDataType(left.dataType(), right.dataType());
  Block<LatticeExprNode> arg(2);
  switch (dtype) {
    case TpFloat:
      arg[0] = left.makeFloat();
...
LatticeExprNode operator&&(const LatticeExprNode& left, const LatticeExprNode& right) {
  ...
  if (LatticeExprNode::areRegions(left, right)) {
    return LELRegion::makeIntersection(*left.pExprBool_p, *right.pExprBool_p);
  }
...
LatticeExprNode operator!(const LatticeExprNode& expr) {
  ...
  return LatticeExprNode(std::make_shared<LELUnaryBool>(LELUnaryEnums::NOT, expr.pExprBool_p));
}
...
LatticeExprNode indexin(const LatticeExprNode& axis, const LatticeExprNode& indexFlags) {
  ...
  arg[0] = axis;
  arg[1] = indexFlags;
  return LatticeExprNode(std::make_shared<LELFunctionBool>(LELFunctionEnums::INDEXIN, arg));
}
...
LatticeExprNode all(const LatticeExprNode& expr) {
  ...
  return LatticeExprNode(std::make_shared<LELFunctionBool>(LELFunctionEnums::ALL, arg));
}
...
#endif
  Block<LatticeExprNode> arg(1, toBool(expr));
  return LatticeExprNode(std::make_shared<LELFunctionBool>(LELFunctionEnums::ANY, arg));
}
...
#endif
  Block<LatticeExprNode> arg(1, toBool(expr));
  return LatticeExprNode(std::make_shared<LELFunctionDouble>(LELFunctionEnums::NTRUE, arg));
}
...
#endif
  Block<LatticeExprNode> arg(1, toBool(expr));
  return LatticeExprNode(std::make_shared<LELFunctionDouble>(LELFunctionEnums::NFALSE, arg));
}
...
LatticeExprNode nelements(const LatticeExprNode& expr) {
  ...
  return LatticeExprNode(std::make_shared<LELFunctionDouble>(LELFunctionEnums::NELEM, arg));
}
...
#endif
  Block<LatticeExprNode> arg(1, expr);
  return LatticeExprNode(std::make_shared<LELFunctionFloat>(LELFunctionEnums::NDIM, arg));
}
...
LatticeExprNode length(const LatticeExprNode& expr, const LatticeExprNode& axis) {
  ...
  arg[0] = expr;
  arg[1] = axis;
  return LatticeExprNode(std::make_shared<LELFunctionFloat>(LELFunctionEnums::LENGTH, arg));
}
...
#endif
  Block<LatticeExprNode> arg(1, expr);
  return LatticeExprNode(std::make_shared<LELFunctionBool>(LELFunctionEnums::ISNAN, arg));
}
...
LatticeExprNode mask(const LatticeExprNode& expr) {
  ...
  return LatticeExprNode(std::make_shared<LELFunctionBool>(LELFunctionEnums::MASK, arg));
}
...
LatticeExprNode value(const LatticeExprNode& expr) {
  ...
  if (expr.dataType() == TpBool) {
    Block<LatticeExprNode> arg(1, toBool(expr));
    return LatticeExprNode(std::make_shared<LELFunctionBool>(LELFunctionEnums::VALUE, arg));
  }
...
LatticeExprNode iif(const LatticeExprNode& condition, const LatticeExprNode& arg1,
  ...
  DataType dtype = LatticeExprNode::resultDataType(arg1.dataType(), arg2.dataType());
  Block<LatticeExprNode> arg(3);
  ...
  switch (dtype) {
    ...
    case TpComplex:
      ...
      return LatticeExprNode(std::make_shared<LELFunctionND<Complex>>(LELFunctionEnums::IIF, arg));
    case TpDComplex:
      arg[1] = arg1.makeDComplex();
      arg[2] = arg2.makeDComplex();
      return LatticeExprNode(std::make_shared<LELFunctionND<DComplex>>(LELFunctionEnums::IIF, arg));
    case TpBool:
      arg[1] = arg1.makeBool();
      arg[2] = arg2.makeBool();
...
//
{
  switch (expr.dataType()) {
    ...
    case TpDComplex:
      return LatticeExprNode(std::make_shared<LELUnary<DComplex>>(oper, expr.pExprDComplex_p));
    default:
      throw(
          AipsError("LatticeExprNode::newNumUnary - "
...
//
{
  switch (expr.dataType()) {
    ...
    case TpDComplex:
      return LatticeExprNode(std::make_shared<LELFunction1D<DComplex>>(func, expr.pExprDComplex_p));
    default:
      throw(
          AipsError("LatticeExprNode::newNumFunc1D - "
...
LatticeExprNode LatticeExprNode::newComplexFunc1D(LELFunctionEnums::Function func,
  ...
  switch (expr.dataType()) {
    case TpComplex:
      return LatticeExprNode(std::make_shared<LELFunctionComplex>(func, arg));
    case TpDComplex:
      return LatticeExprNode(std::make_shared<LELFunctionDComplex>(func, arg));
    default:
      throw(
          AipsError("LatticeExprNode::newComplexFunc1D - "
...
//
{
  ...
  switch (dtype) {
    case TpFloat:
    case TpComplex:
      return LatticeExprNode(std::make_shared<LELFunctionFloat>(func, arg));
    case TpDouble:
    case TpDComplex:
      return LatticeExprNode(std::make_shared<LELFunctionDouble>(func, arg));
    default:
      throw(
          AipsError("LatticeExprNode::newNumReal1D - "
...
//
{
  ...
  switch (dtype) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::newNumFunc2D - "
                    "Bool argument used in numerical function"));
  }
  return LatticeExprNode();
}
...
//
{
  ...
  switch (dtype) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::newNumBinary - "
  ...
  switch (dtype) {
    ...
    default:
      return LatticeExprNode(std::make_shared<LELBinary<DComplex>>(oper, expr0.pExprDComplex_p,
...
//
{
  ...
  switch (dtype) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::newLogBinary - "
                    "Non-Bool argument used in logical binary operation"));
  }
  // Make the operands the same dimensionality (if needed and possible).
  makeEqualDim(expr0, expr1);
  return LatticeExprNode(
      std::make_shared<LELBinaryBool>(oper, expr0.pExprBool_p, expr1.pExprBool_p));
}
...
//
{
  ...
  switch (dtype) {
    ...
    case TpBool:
      if (oper != LELBinaryEnums::EQ && oper != LELBinaryEnums::NE) {
        throw(
            AipsError("LatticeExprNode::newBinaryCmp - "
  ...
  switch (dtype) {
    ...
    case TpDComplex:
      return LatticeExprNode(std::make_shared<LELBinaryCmp<DComplex>>(oper, expr0.pExprDComplex_p,
                                                                      expr1.pExprDComplex_p));
    default:
      return LatticeExprNode(
          std::make_shared<LELBinaryBool>(oper, expr0.pExprBool_p, expr1.pExprBool_p));
...
//
{
  ...
  if (left == TpBool || right == TpBool) {
    throw(
        AipsError("LatticeExprNode::resultDataType - "
...
const IPosition& LatticeExprNode::getIPosition() const {
  if (dataType() != TpOther) {
    throw(
        AipsError("LatticeExprNode::getIPosition - "
...
std::shared_ptr<LELInterface<Double>> LatticeExprNode::makeDouble() const {
  switch (dataType()) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::makeDouble - "
...
std::shared_ptr<LELInterface<Complex>> LatticeExprNode::makeComplex() const {
  switch (dataType()) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::makeComplex - "
...
std::shared_ptr<LELInterface<DComplex>> LatticeExprNode::makeDComplex() const {
  switch (dataType()) {
    ...
    default:
      throw(
          AipsError("LatticeExprNode::makeDComplex - "
