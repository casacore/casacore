...
Bool checkAttribute(const LELAttribute& attr, const Bool isMasked, const Bool isScalar,
...
Bool checkDComplex(LELInterface<DComplex>& expr, const DComplex& Result, const String& name,
                   const IPosition& shape, const Bool shouldBeScalar, const Bool suppress);

Bool checkBool(LELInterface<Bool>& expr, const Bool Result, const String& name,
               const IPosition& shape, const Bool shouldBeScalar, const Bool suppress,
...
int main(int argc, const char* argv[]) {
  try {
    ...
    //
    {
      ...
      if (!checkAttribute(attr1, False, True, nullIPos, nullIPos, lattCoord2)) ok = False;
      ...
      if (!checkAttribute(attr2, True, isScalar2, shape2, tileShape2, lattCoord2)) ok = False;

      LELAttribute attr3 = attr2;
      if (!checkAttribute(attr3, attr2.isMasked(), attr2.isScalar(), attr2.shape(),
                          attr2.tileShape(), attr2.coordinates())) {
      ...
      if (!checkAttribute(attr4, attr2.isMasked(), attr2.isScalar(), attr2.shape(),
                          attr2.tileShape(), attr2.coordinates())) {
    ...
    //
    {
...

      {
        ...
        CResult = sin(bCVal);
        if (!checkComplex(expr, CResult, String("LELFunction1D"), shape, False, suppress))
          ok = False;
      }
...

      {
        ...
        bC.getSlice(CArr, IPosition(CArr.ndim(), 0), CArr.shape(), IPosition(CArr.ndim(), 1));
        CResult = min(CArr);
        if (!checkComplex(expr, CResult, String("LELFunction1D"), shape, True, suppress))
          ok = False;
      }
    ...
    //
    {
...

      {
        ...
        DCResult = sin(bDCVal);
        if (!checkDComplex(expr, DCResult, String("LELFunction1D"), shape, False, suppress))
          ok = False;
      }
...

      {
        ...
        bDC.getSlice(DCArr, IPosition(DCArr.ndim(), 0), DCArr.shape(), IPosition(DCArr.ndim(), 1));
        DCResult = min(DCArr);
        if (!checkDComplex(expr, DCResult, String("LELFunction1D"), shape, True, suppress))
          ok = False;
      }
    ...
    //
    {
      ...
      cout << "   Function iif" << endl;
      {
        ...
        CResult = bCVal;
        if (!checkComplex(expr1, CResult, String("LELFunctionND"), shape, True, suppress))
          ok = False;

        arga[0] = LatticeExprNode(False);
        LELFunctionND<Complex> expr2(LELFunctionEnums::IIF, arga);
        CResult = cCVal;
        if (!checkComplex(expr2, CResult, String("LELFunctionND"), shape, True, suppress))
          ok = False;
      }

      {
        ...
        if (!checkComplex(expr1, CResult, String("LELFunctionND"), shape, False, suppress))
          ok = False;

        arga[0] = LatticeExprNode(False);
        LELFunctionND<Complex> expr2(LELFunctionEnums::IIF, arga);
        CResult = cCVal;
        if (!checkComplex(expr2, CResult, String("LELFunctionND"), shape, False, suppress))
          ok = False;
      }
      {
        cout << "     Scalar, array, scalar" << endl;
    ...
    //
    {
      ...
      cout << "   Function iif" << endl;
      {
        ...
        DCResult = bDCVal;
        if (!checkDComplex(expr1, DCResult, String("LELFunctionND"), shape, True, suppress))
          ok = False;

        arga[0] = LatticeExprNode(False);
        LELFunctionND<DComplex> expr2(LELFunctionEnums::IIF, arga);
        DCResult = cDCVal;
        if (!checkDComplex(expr2, DCResult, String("LELFunctionND"), shape, True, suppress))
          ok = False;
      }

      {
        ...
        if (!checkDComplex(expr1, DCResult, String("LELFunctionND"), shape, False, suppress))
          ok = False;

        arga[0] = LatticeExprNode(False);
        LELFunctionND<DComplex> expr2(LELFunctionEnums::IIF, arga);
        DCResult = cDCVal;
        if (!checkDComplex(expr2, DCResult, String("LELFunctionND"), shape, False, suppress))
          ok = False;
      }
      {
        cout << "     Scalar, array, scalar" << endl;
    ...
    //
    {
...

      {
        ...
        if (!checkFloat(expr, FResult, String("LELFunctionFloat"), shape, True, suppress))
          ok = False;
      }
    ...
    //
    {
      ...
      argc[0] = LatticeExprNode(bB);
      {
        ...
        if (!checkDouble(expr, DResult, String("LELFunctionDouble"), shape, True, suppress))
          ok = False;
      }
    ...
    //
    {
...

      {
        ...
        if (!checkComplex(expr, CResult, String("LELFunctionComplex"), shape, False, suppress))
          ok = False;
      }
    ...
    //
    {
...

      {
        ...
        if (!checkDComplex(expr, DCResult, String("LELFunctionDComplex"), shape, False, suppress))
          ok = False;
      }
    ...
    //
    {
...

      {
        ...
        cout << "   Function isNaN" << endl;
        {
          ...
          BResult = False;
          if (!checkBool(expr, BResult, String("LELFunctionBool"), shape, False, suppress))
            ok = False;
        }
        {
...

      {
        ...
        cout << "   Function indexin" << endl;
        {
          ...
          BResult = True;
          if (!checkBool(expr, BResult, "LELFunctionBool", IPosition(2, 2, 4), False, suppress,
                         True))
            ok = False;
        }
        {
          ...
          LELFunctionBool expr(LELFunctionEnums::INDEXIN, argb);
          BResult = True;
          if (!checkBool(expr, BResult, "LELFunctionBool", IPosition(2, 10, 2), False, suppress,
                         True))
            ok = False;
        }
        {
          ...
          LELFunctionBool expr(LELFunctionEnums::INDEXIN, argb);
          BResult = False;
          if (!checkBool(expr, BResult, "LELFunctionBool", IPosition(2, 6, 2), False, suppress,
                         True))
            ok = False;
          if (!checkBool(expr, BResult, "LELFunctionBool", IPosition(2, 2, 6), False, suppress,
                         True))
            ok = False;
        }
      }
