...
Bool FITSMask::doGetSlice(Array<Bool>& mask, const Slicer& section) {
  ...
  if (itsTiledFilePtr->dataType() == TpFloat) {
    ...
    itsTiledFilePtr->get(itsBuffer, section, itsScale, itsOffset, itsLongMagic, itsHasIntBlanks);
  } else if (itsTiledFilePtr->dataType() == TpShort) {
    itsTiledFilePtr->get(itsBuffer, section, itsScale, itsOffset, itsShortMagic, itsHasIntBlanks);
