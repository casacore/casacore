...
LCBox::LCBox(const IPosition& latticeShape) : LCRegionFixed(latticeShape) {
  // Set the box to the full lattice.
  setBoundingBox(Slicer(IPosition(latticeShape.nelements(), 0), latticeShape));
  // Fill the blc and trc vectors.
  fillBlcTrc();
}

LCBox::LCBox(const Slicer& box, const IPosition& latticeShape) : LCRegionFixed(latticeShape) {
...
LCBox::LCBox(const IPosition& blc, const IPosition& trc, const IPosition& latticeShape)
...
LCBox::LCBox(const LCBox& other)
    : LCRegionFixed(other), itsBlc(other.itsBlc), itsTrc(other.itsTrc) {}
...
String LCBox::className() { return "LCBox"; }

String LCBox::type() const { return className(); }
