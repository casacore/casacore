...
LCDifference::LCDifference(Bool takeOver, const Block<const LCRegion*>& regions)
    : LCRegionMulti(takeOver, regions) {
  defineBox();
}

LCDifference::LCDifference(const LCDifference& other) : LCRegionMulti(other) {}
...
String LCDifference::className() { return "LCDifference"; }

String LCDifference::type() const { return className(); }
...
LCDifference* LCDifference::fromRecord(const TableRecord& rec, const String& tableName) {
  Block<const LCRegion*> regions;
...
void LCDifference::multiGetSlice(Array<Bool>& buffer, const Slicer& section) {
