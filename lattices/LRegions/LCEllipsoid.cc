...
LCRegion* LCEllipsoid::doTranslate(const Vector<Float>& translateVector,
  ...
  if (itsCenter.size() != 2 || _theta == 0) {
    ...
    return new LCEllipsoid(center[0], center[1], itsRadii[0], itsRadii[1], _theta, newLatticeShape);
  }
}

String LCEllipsoid::className() { return "LCEllipsoid"; }

String LCEllipsoid::type() const { return className(); }
...
Slicer LCEllipsoid::makeBox(const Vector<Float>& radii, const IPosition& latticeShape) {
  ...
  for (uInt i = 0; i < nrdim; ++i) {
    ...
    if (blc[i] > trc[i]) {
      ostringstream rstr;
      rstr << radii;
      ThrowCc("ellipsoid is empty (radii " + rstr.str() + " too small)");
    }
  }
...
void LCEllipsoid::_defineMask2D() {
  ...
  mask.putStorage(maskData, deleteIt);
  ThrowIf(!_centerIsInside && !casacore::anyTrue(mask),
