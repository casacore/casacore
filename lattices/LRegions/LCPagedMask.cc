...
LCPagedMask::LCPagedMask(const TiledShape& latticShape, const String& tableName)
...
LCPagedMask::LCPagedMask(const TiledShape& maskShape, const LCBox& box, const String& tableName)
    : LCRegionSingle(box.latticeShape()), itsBox(box) {
  // Check if box shape and mask shape are equal.
  if (itsBox.shape() != maskShape.shape()) {
    throw(
        AipsError("LCPagedMask::LCPagedMask- "
...
LCPagedMask::LCPagedMask(const LCPagedMask& other)
    : LCRegionSingle(other), itsBox(other.itsBox), itsMask(other.itsMask) {
  setMaskPtr(itsMask);
}
...
void LCPagedMask::setCacheSizeFromPath(const IPosition& sliceShape, const IPosition& windowStart,
...
void LCPagedMask::clearCache() { itsMask.clearCache(); }
...
void LCPagedMask::tempClose() { itsMask.tempClose(); }
...
String LCPagedMask::type() const { return className(); }
...
LCPagedMask* LCPagedMask::fromRecord(const TableRecord& rec, const String& tableName) {
