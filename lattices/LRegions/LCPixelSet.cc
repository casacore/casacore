...
LCPixelSet::LCPixelSet(const Array<Bool>& mask, const LCBox& box)
  ...
  if (!mask.shape().isEqual(itsBox.shape())) {
    throw(
        AipsError("LCPixelSet::LCPixelSet - "
...
LCPixelSet::LCPixelSet(const LCPixelSet& that) : LCRegionFixed(that), itsBox(that.itsBox) {}
...
String LCPixelSet::className() { return "LCPixelSet"; }

String LCPixelSet::type() const { return className(); }
...
LCPixelSet* LCPixelSet::fromRecord(const TableRecord& rec, const String& tableName) {
