...
LCRegion* LCRegion::fromRecord(const TableRecord& rec, const String& tableName) {
  if (!rec.isDefined("isRegion") || rec.asInt("isRegion") != RegionType::LC) {
    throw(
        AipsError("LCRegion::fromRecord - "
  ...
  if (name == LCBox::className()) {
    ...
    throw(AipsError("LCRegion::fromRecord - " + name + " is unknown derived LCRegion class"));
  }
