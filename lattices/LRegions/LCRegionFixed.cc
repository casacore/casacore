...
LCRegionFixed::LCRegionFixed(const IPosition& latticeShape) : LCRegionSingle(latticeShape) {}

LCRegionFixed::LCRegionFixed(const LCRegionFixed& other)
    : LCRegionSingle(other), itsMask(other.itsMask) {
  setMaskPtr(itsMask);
}
...
Bool LCRegionFixed::operator==(const LCRegion& other) const { return LCRegion::operator==(other); }
