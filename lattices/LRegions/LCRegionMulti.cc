...
LCRegionMulti::LCRegionMulti(const LCRegion& region1, const LCRegion& region2)
...
LCRegionMulti::LCRegionMulti(const LCRegion* regionPtr, const IPosition& latticeShape)
    : LCRegion(latticeShape), itsRegions(1) {
...
Bool LCRegionMulti::findAreas(IPosition& bufStart, IPosition& bufEnd, IPosition& regStart,
  ...
  for (uInt j = 0; j < nrdim; j++) {
    ...
    DebugAssert(bufEnd(j) - bufStart(j) == (regEnd(j) - regStart(j)) / secinc, AipsError);
  }
  return overlap;
}
