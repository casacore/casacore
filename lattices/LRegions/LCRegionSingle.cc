...
Bool LCRegionSingle::masksEqual(const LCRegion& other) const {
  ...
  RO_LatticeIterator<Bool> iter1(*itsMaskPtr, itsMaskPtr->niceCursorShape());
  RO_LatticeIterator<Bool> iter2(*(that.itsMaskPtr), itsMaskPtr->niceCursorShape());
  while (!iter1.atEnd()) {
...
Bool LCRegionSingle::hasMask() const { return itsHasMask; }

Bool LCRegionSingle::doGetSlice(Array<Bool>& buffer, const Slicer& section) {
  if (itsHasMask != 0) {
...
LatticeIterInterface<Bool>* LCRegionSingle::makeIter(const LatticeNavigator& navigator,
                                                     Bool useRef) const {
  if (itsHasMask != 0) {
...
void LCRegionSingle::doPutSlice(const Array<Bool>& sourceBuffer, const IPosition& where,
