...
LCSlicer::LCSlicer(const Vector<Float>& blc, const Vector<Float>& trc, Bool fractional,
  ...
  itsInc = 1;
  fillFlags(fractional, absRel, blc.size(), trc.size(), itsInc.size());
  fill();
}
...
void LCSlicer::fill() {
  ...
  if (itsFracTrc.size() != nrtrc || itsAbsRelTrc.size() != nrtrc) {
    throw(
        AipsError("LCSlicer::LCSLicer - "
...
String LCSlicer::className() { return "LCSlicer"; }

String LCSlicer::type() const { return className(); }
...
LCSlicer* LCSlicer::fromRecord(const TableRecord& rec, const String&) {
  ...
  LCSlicer* regPtr = new LCSlicer(blc, trc, rec.toArrayFloat("inc"), fracblc, fractrc,
                                  rec.toArrayBool("fracinc"), arblc, artrc);
