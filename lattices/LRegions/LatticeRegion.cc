...
LatticeRegion::LatticeRegion(const Slicer& slicer, const IPosition& latticeShape)
  ...
  itsRegion = new LCBox(Slicer(slicer.start(), slicer.length()), latticeShape);
}
...
LatticeRegion::~LatticeRegion() { delete itsRegion; }
...
void LatticeRegion::setCacheSizeFromPath(const IPosition& sliceShape, const IPosition& windowStart,
...
void LatticeRegion::clearCache() { itsRegion->clearCache(); }
...
void LatticeRegion::tempClose() { itsRegion->tempClose(); }

void LatticeRegion::reopen() { itsRegion->reopen(); }
