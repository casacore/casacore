...
void doIt(const LCRegion& region, const IPosition& axes, const IPosition& blc, const IPosition& trc,
  ...
  try {
    ...
    cout << prism.extendAxes() << prism.extendBox().blc() << prism.extendBox().trc() << endl;
    {
      // Test cloning.
...
int main() {
  try {
    ...
    // Error; no extendaxes
    doIt(polygon, IPosition(), IPosition(1, 2), IPosition(1, 3), IPosition(1, 20));
    // Error; #extendAxes mismatches blc/trc
    doIt(polygon, IPosition(2, 1, 2), IPosition(1, 2), IPosition(1, 3), IPosition(1, 20));
    // Error; incorrect order of extendAxes
    doIt(polygon, IPosition(2, 1), IPosition(2, 2), IPosition(2, 3), IPosition(2, 20));
  } catch (std::exception& x) {
