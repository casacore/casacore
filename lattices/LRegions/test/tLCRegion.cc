...
void doIt(const IPosition& latticeShape, const IPosition& start, const IPosition& end,
  ...
  cout << box.boundingBox().start() << box.boundingBox().end() << box.boundingBox().length()
       << box.latticeShape() << box.comment() << endl;
  ...
  cout << cir.center() << cir.radii() << endl;
  {
    ...
    AlwaysAssertExit(box.boundingBox().start() == boxcop->boundingBox().start());
    AlwaysAssertExit(box.boundingBox().end() == boxcop->boundingBox().end());
    AlwaysAssertExit(box.boundingBox().stride() == boxcop->boundingBox().stride());
    AlwaysAssertExit(box.boundingBox().length() == boxcop->boundingBox().length());
    ...
    AlwaysAssertExit(cir.boundingBox().start() == circop->boundingBox().start());
    AlwaysAssertExit(cir.boundingBox().end() == circop->boundingBox().end());
    AlwaysAssertExit(cir.boundingBox().stride() == circop->boundingBox().stride());
    AlwaysAssertExit(cir.boundingBox().length() == circop->boundingBox().length());
    AlwaysAssertExit(cir.comment() == circop->comment());
    AlwaysAssertExit(allEQ(cir.center(), ((LCEllipsoid*)circop)->center()));
    AlwaysAssertExit(allEQ(cir.radii(), ((LCEllipsoid*)circop)->radii()));
    delete circop;
  }
  {
    // Test persistency.
    LCRegionFixed* boxcop = (LCRegionFixed*)(LCRegion::fromRecord(box.toRecord(""), ""));
    AlwaysAssertExit(box.hasMask() == boxcop->hasMask());
    AlwaysAssertExit(allEQ(box.maskArray(), boxcop->maskArray()));
    AlwaysAssertExit(box.boundingBox().start() == boxcop->boundingBox().start());
    AlwaysAssertExit(box.boundingBox().end() == boxcop->boundingBox().end());
    ...
    AlwaysAssertExit(box.comment() == boxcop->comment());
    delete boxcop;
    LCRegionFixed* circop = (LCRegionFixed*)(LCRegion::fromRecord(cir.toRecord(""), ""));
    AlwaysAssertExit(cir.hasMask() == circop->hasMask());
    AlwaysAssertExit(allEQ(cir.maskArray(), circop->maskArray()));
    AlwaysAssertExit(cir.boundingBox().start() == circop->boundingBox().start());
    AlwaysAssertExit(cir.boundingBox().end() == circop->boundingBox().end());
