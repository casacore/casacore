...
CLInterpolator2D<T>::CLInterpolator2D(const CLInterpolator2D<T>& that) {
  set(that.itsLatticePtr, that.itsAxesMap, that.itsAxis1, that.itsAxis2, that.itsCurveAxis);
}

template <class T>
CLInterpolator2D<T>& CLInterpolator2D<T>::operator=(const CLInterpolator2D<T>& that) {
  if (this != &that) {
    set(that.itsLatticePtr, that.itsAxesMap, that.itsAxis1, that.itsAxis2, that.itsCurveAxis);
  }
  return *this;
