...
//
{
  ...
  if (nPts == 0) {
    itsLogger << LogIO::WARN << "There are not enough good points in the array for a good estimate"
              << LogIO::POST;
    return parameters;
  }
  //
  ...
  if (type == Fit2D::GAUSSIAN || type == Fit2D::DISK) {
    ...
    parameters(2) = YP;
    //
    parameters(3) = sqrt(fac * (XXP + YYP + sqrt(square(XXP - YYP) + 4 * square(XYP))));
    parameters(4) = sqrt(fac * (XXP + YYP - sqrt(square(XXP - YYP) + 4 * square(XYP))));
...
template <class T>
Fit2D::ErrorTypes Fit2D::fit(const MaskedLattice<T>& data, const Lattice<T>& sigma) {
  if (!itsValid) {
...
template <class T>
Fit2D::ErrorTypes Fit2D::fit(const Lattice<T>& data, const Lattice<T>& sigma) {
  if (!itsValid) {
    itsErrorMessage = "No models have been set - use function addModel";
    return Fit2D::NOMODELS;
  }
...
template <class T>
Fit2D::ErrorTypes Fit2D::fit(const Array<T>& data, const Array<T>& sigma) {
  if (!itsValid) {
    itsErrorMessage = "No models have been set - use function addModel";
    return Fit2D::NOMODELS;
  }
...
Fit2D::ErrorTypes Fit2D::residual(Array<T>& resid, Array<T>& model, const Array<T>& data,
  ...
  if (resid.nelements() == 0) {
    ...
    ThrowIf(!shape.isEqual(resid.shape()), "Residual and pixel arrays must be the same shape");
  }
  if (model.nelements() == 0) {
    model.resize(shape);
  } else {
    ThrowIf(!shape.isEqual(model.shape()), "Residual and pixel arrays must ");
  }
