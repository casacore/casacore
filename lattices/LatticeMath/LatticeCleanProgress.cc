...
Bool LatticeCleanProgress::info(const Bool lastcall, const Int iteration,
  ...
  if (!lastcall) {
    if (maxima.nelements() == 1) {
      os << "Maximum abs = " << maxima(0) << " at " << posMaximum[0] + 1 << endl;
...
void LatticeCleanProgress::basicSetUp(Bool doPlot) {
  ...
  for (scale = 0; scale < nScales; scale++) {
    ...
    ostringstream oos;
    oos << "Scale " << scale + 1;
    itsPgplotter->text(0.85 * xMax, (logMaxRes - 0.1 * (1 + scale) * deltaY), oos.str());
  }
...

  {
    ...
    ostringstream oos;
    oos << "Total Flux ";
    itsPgplotter->text(0.85 * xMax, (0.5 * (currentFluxScale - currentMinFluxScale)), oos.str());
  }
