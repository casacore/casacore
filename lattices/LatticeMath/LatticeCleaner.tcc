...
template <class T>
LatticeCleaner<T>::LatticeCleaner(const Lattice<T>& psf, const Lattice<T>& dirty)
    : itsMask(0),
      itsScaleSizes(0),
      itsMaximumResidual(0.0),
  ...
  AlwaysAssert(psf.shape().nelements() == dirty.shape().nelements(), AipsError);
...
template <class T>
LatticeCleaner<T>::LatticeCleaner(const LatticeCleaner<T>& other)
...
template <class T>
LatticeCleaner<T>& LatticeCleaner<T>::operator=(const LatticeCleaner<T>& other) {
  if (this != &other) {
    itsCleanType = other.itsCleanType;
    itsXfr = other.itsXfr;
    itsMask = other.itsMask;
...
Int LatticeCleaner<T>::clean(Lattice<T>& model, LatticeCleanProgress* progress) {
  ...
  if (nScalesToClean > 1) {
    ...
    for (scale = 0; scale < nScalesToClean; scale++) {
      scaleBias(scale) =
          1 - itsSmallScaleBias * itsScaleSizes(scale) / itsScaleSizes(nScalesToClean - 1);
      if (scale) os << ",";
      os << " " << scaleBias(scale);
    }
    os << LogIO::POST;
  ...
  for (scale = 0; scale < nScalesToClean; scale++) {
    ...
    findMaxAbsLattice(*itsPsfConvScales[scale], maxPsfConvScales(scale), positionPeakPsfConvScales);
    if (nScalesToClean == 1) {
      os << LogIO::NORMAL << "Peak of PSF: " << maxPsfConvScales(scale) << " at "
         << positionPeakPsfConvScales + 1 << LogIO::POST;
    } else {
      os << LogIO::NORMAL << "  " << scale + 1 << "    " << positionPeakPsfConvScales + 1 << "  "
         << maxPsfConvScales(scale) << LogIO::POST;
    }
  ...
  if (itsMask) {
    ...
    if (itsMaskThreshold < 0) {
      os << LogIO::NORMAL << "Mask thresholding is not used, values are interpreted as weights"
         << LogIO::POST;
    } else {
      os << LogIO::NORMAL << "Cleaning pixels with mask values above " << itsMaskThreshold
         << LogIO::POST;
    }
    ...
    if (!itsIgnoreCenterBox) {
      if ((xend - xbeg) > nx / 2) {
        ...
        os << LogIO::WARN
           << "Mask span over more than half the x-axis: Considering inner half of the x-axis"
           << LogIO::POST;
      }
      if ((yend - ybeg) > ny / 2) {
        ybeg = ny / 4 - 1;
        os << LogIO::WARN
           << "Mask span over more than half the y-axis: Considering inner half of the y-axis"
           << LogIO::POST;
      }
  ...
  for (Int ii = itsStartingIter; ii < itsMaxNiter; ii++) {
    ...
    if (ii == itsStartingIter) {
      ...
      os << "Initial maximum residual is " << itsMaximumResidual << LogIO::POST;
    }

    // Various ways of stopping:
    //    1. stop if below threshold
    if (abs(itsStrengthOptimum) < threshold()) {
      os << "Reached stopping threshold " << threshold() << " at iteration " << ii << LogIO::POST;
    ...
    if ((nScalesToClean > 1) && itsStopAtLargeScaleNegative &&
        optimumScale == (nScalesToClean - 1) && itsStrengthOptimum < 0.0) {
    ...
    // If actual value is 50% above the maximum residual. ..good chance it will not recover at this
    // stage
    ...
    if (progress) {
      ...
      if ((itsIteration % (itsMaxNiter / 10 > 0 ? itsMaxNiter / 10 : 1)) == 0) {
        ...
        os << itsIteration << "      " << itsStrengthOptimum << "      " << totalFlux
           << LogIO::POST;
      }
    }
  ...
  for (scale = 0; scale < nScalesToClean; scale++) {
    os << LogIO::NORMAL << "  " << scale + 1 << "    " << totalFluxScale(scale) << LogIO::POST;
  }
...
Bool LatticeCleaner<T>::setscales(const Vector<Float>& scaleSizes) {
  ...
  for (scale = 0; scale < itsNscales; scale++) {
    ...
    itsScales[scale] = new TempLattice<T>(itsDirty->shape(), itsMemoryMB);
    AlwaysAssert(itsScales[scale], AipsError);
    // First make the scale
    makeScale(*itsScales[scale], scaleSizes(scale));
    itsScaleXfrs[scale] = new TempLattice<Complex>(itsScales[scale]->shape(), itsMemoryMB);
    // Now store the XFR
...
Bool LatticeCleaner<T>::makeScaleMasks() {
  ...
  for (scale = 0; scale < itsNscales; scale++) {
    ...
    itsScaleMasks[scale] = new TempLattice<T>(itsMask->shape(), itsMemoryMB);
...
template <class T>
void LatticeCleaner<T>::makeBoxesSameSize(IPosition& blc1, IPosition& trc1, IPosition& blc2,
                                          IPosition& trc2) {
