        ...
        if (dim == firstAxis) {
          if (inShape(dim) != 1) {  // Do real->complex Transforms
            LatticeIterator<typename NumericTraits<ComplexType>::ConjugateType> inIter(
                inlocal, TiledLineStepper(inShape, tileShape, dim));
            LatticeIterator<ComplexType> outIter(out, TiledLineStepper(outShape, tileShape, dim));
            for (inIter.reset(), outIter.reset(); !inIter.atEnd() && !outIter.atEnd();
                 inIter++, outIter++) {
              if (doShift) {
                if (doFast) {
        ...
        if (dim == firstAxis) {
          if (inShape(dim) != 1) {  // Do real->complex Transforms
            RO_LatticeIterator<typename NumericTraits<ComplexType>::ConjugateType> inIter(
                in, TiledLineStepper(inShape, tileShape, dim));
            LatticeIterator<ComplexType> outIter(out, TiledLineStepper(outShape, tileShape, dim));
            for (inIter.reset(), outIter.reset(); !inIter.atEnd() && !outIter.atEnd();
                 inIter++, outIter++) {
              if (doShift) {
    ...
    if (whichAxes(dim) == True) {
      if (dim != firstAxis) {     // Do complex->complex Transforms
        ...
        if (inShape(dim) != 1) {  // Do complex->real transforms
          RO_LatticeIterator<ComplexType> inIter(in, TiledLineStepper(inShape, tileShape, dim));
          LatticeIterator<typename NumericTraits<ComplexType>::ConjugateType> outIter(
              out, TiledLineStepper(outShape, tileShape, dim));
          for (inIter.reset(), outIter.reset(); !inIter.atEnd() && !outIter.atEnd();
               inIter++, outIter++) {
            if (doShift) {
              if (doFast) {
