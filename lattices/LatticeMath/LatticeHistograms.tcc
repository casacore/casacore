...
Bool LatticeHistograms<T>::getHistograms(Array<T>& values, Array<T>& counts,
  ...
  LatticeStepper histStepper(pStoreLattice_p->shape(), cursorShape, vectorAxis,
  ...
  stats.resize(shape.size() == 1 ? IPosition(1, 1) : shape.removeAxes(removeAxis));
...
//
{
  ...
  for (histIterator.reset(); !histIterator.atEnd(); histIterator++) {
    ...
    extractOneHistogram(linearSum, linearYMax, values, counts, stats, histIterator.vectorCursor());
...
//
{
  ...
  const T binWidth = LatticeHistSpecialize::setBinWidth(stats(LatticeStatsBase::MIN),
                                                        stats(LatticeStatsBase::MAX), nBins_p);
  // Do plots
...
template <class T>
void LatticeHistograms<T>::extractOneHistogram(T& linearSum, T& linearYMax, Vector<T>& values,
                                               Vector<T>& counts, const Vector<T>& stats,
...
//
{
  ...
  if (doCumu_p) {
    LatticeHistSpecialize::makeCumulative(counts, linearYMax, nBins,
                                          typename NumericTraits<T>::BaseType(1.0));
  }
...
template <class T>
void LatticeHistograms<T>::getStatistics(Vector<T>& stats, const IPosition& histPos) const
//
...
template <class T>
void LatticeHistograms<T>::listStatistics(LogIO& os, const Vector<T>& stats, T binWidth) {
