...
LatticeStatistics<T>::LatticeStatistics(const LatticeStatistics<T>& other)
    : pInLattice_p(0),
      ...
      _aNew(other._aNew),
      _bNew(other._bNew),
      _latticeStatsAlgortihm(other._latticeStatsAlgortihm
...
//
{
  if (this != &other) {
    ...
    _aOld = other._aOld;
    _bOld = other._bOld;
    _latticeStatsAlgortihm.reset(other._latticeStatsAlgortihm
                                     ? new LatticeStatsAlgorithm(*other._latticeStatsAlgortihm)
                                     : nullptr);
  }
  return *this;
...
Bool LatticeStatistics<T>::getStatistic(Array<AccumType>& stats,
  ...
  if (_getAlgorithm() == StatisticsData::BIWEIGHT) {
    ...
    ThrowIf(type == LatticeStatsBase::RMS,
            "The biweight algorithm does not support"
            "computation of the rms");
    ThrowIf(type == LatticeStatsBase::SUM,
            "The biweight algorithm does not support"
            "computation of the sum");
    ThrowIf(type == LatticeStatsBase::SUMSQ,
            "The biweight algorithm does not support"
            "computation of the sum of squres");
    ThrowIf(type == LatticeStatsBase::VARIANCE,
            "The biweight algorithm does not support"
            "computation of the variance");
    ThrowIf(type == LatticeStatsBase::MEDIAN || type == LatticeStatsBase::MEDABSDEVMED ||
  ...
  } else if (type == LatticeStatsBase::MEDIAN || type == LatticeStatsBase::MEDABSDEVMED ||
             type == LatticeStatsBase::QUARTILE || type == LatticeStatsBase::Q1 ||
             type == LatticeStatsBase::Q3) {
    if (!doRobust_p) {
      doRobust_p = True;
...
Bool LatticeStatistics<T>::configureFitToHalf(FitToHalfStatisticsData::CENTER centerType,
  ...
  if (!reconfig) {
    ...
    reconfig =
        centerType != data.center || useData != data.side ||
        (centerType == FitToHalfStatisticsData::CVALUE && !near(centerValue, data.centerValue));
  }
  if (reconfig) {
...
void LatticeStatistics<T>::forceUseStatsFrameworkUsingArrays() {
  _latticeStatsAlgortihm.reset(new LatticeStatsAlgorithm(STATS_FRAMEWORK_ARRAYS));
}

template <class T>
void LatticeStatistics<T>::forceUseOldTiledApplyMethod() {
  _latticeStatsAlgortihm.reset(new LatticeStatsAlgorithm(TILED_APPLY));
}
...
Bool LatticeStatistics<T>::generateStorageLattice() {
  ...
  displayAxes_p.resize(0);
  displayAxes_p = IPosition::otherAxes(pInLattice_p->ndim(), cursorAxes_p).asVector();
  ...
  if (haveLogger_p) {
    os_p << LogIO::NORMAL1 << "Creating new statistics storage lattice of shape "
         << storeLatticeShape << endl
         << LogIO::POST;
  }
  pStoreLattice_p =
      std::make_shared<TempLattice<AccumType>>(TiledShape(storeLatticeShape, tileShape), useMemory);
...
void LatticeStatistics<T>::_doStatsLoop(uInt nsets,
  ...
  if (nArrMaxThreads >= nDPMaxThreads || forceUsingArrays) {
    if (forceUsingArrays && haveLogger_p) {
      os_p << LogIO::NORMAL << "Forcing use of Stats Framework using Arrays method" << LogIO::POST;
    }
  ...
  const auto forceUsingDP =
      _latticeStatsAlgortihm && *_latticeStatsAlgortihm == STATS_FRAMEWORK_DATA_PROVIDERS;
  if (!computed || forceUsingDP) {
    if (forceUsingDP && haveLogger_p) {
      os_p << LogIO::NORMAL << "Forcing use of Stats Framework using Data Providers method"
           << LogIO::POST;
    }
    _computeStatsUsingLattDataProviders(stepper, subLat, slicer, progressMeter, nsets);
  }
  if (!doRobust_p) {
    ...
    const auto arrShape = pStoreLattice_p->shape().removeAxes(IPosition(1, ndim - 1));
...
#pragma omp parallel for num_threads(nthreads)
  ...
  for (uInt i = 0; i < nArrays; ++i) {
    ...
    if (isMasked && maskArray[i].size() > 0) {
      if (!range) {
        sa[tid]->setData(dataArray[i].begin(), maskArray[i].begin(), dataArray[i].size());
      } else {
        sa[tid]->setData(dataArray[i].begin(), maskArray[i].begin(), dataArray[i].size(), *range,
                         !noInclude_p);
      }
    } else {
      if (!range) {
        ...
        sa[tid]->setData(dataArray[i].begin(), dataArray[i].size(), *range, !noInclude_p);
      }
    }
    statsArray[i] = sa[tid]->getStatistics();
    if (doRobust_p) {
      _computeQuantilesForStatsFramework(statsArray[i], q1[i], q3[i], sa[tid]);
    }
    if (isChauv) {
  ...
  for (uInt i = 0; i < nArrays; ++i) {
    ...
    _fillStorageLattice(currentMin, currentMax, mypos, stats, doRobust_p, qq1, qq3);
    if (isChauv) {
      chos.str("");
      chos << mypos;
...
void LatticeStatistics<T>::_computeStatsUsingLattDataProviders(
  ...
  for (stepper.reset(); !stepper.atEnd(); stepper++) {
    ...
    if (doRobust_p) {
      _computeQuantilesForStatsFramework(stats, q1, q3, sa);
    }
...
void LatticeStatistics<T>::_updateMinMaxPos(T& overallMin, T& overallMax, T currentMin,
  ...
  } else if (currentMax > overallMax || currentMin < overallMin) {
...
void LatticeStatistics<T>::generateRobust() {
  ...
  for (stepper.reset(); !stepper.atEnd(); stepper++) {
    ...
    _computeQuantiles(median, medAbsDevMed, q1, q3, sa, knownNpts, knownMin, knownMax);
...
void LatticeStatistics<T>::_computeQuantilesForStatsFramework(
    ...
    std::shared_ptr<StatisticsAlgorithm<AccumType, U, V>> statsAlg) const {
  if (stats.npts > 0) {
    AccumType median, medAbsDevMed;
    _computeQuantiles(median, medAbsDevMed, q1, q3, statsAlg, stats.npts, *stats.min, *stats.max);
...
void LatticeStatistics<T>::_configureDataProviders(
    LatticeStatsDataProvider<T>& lattDP, MaskedLatticeStatsDataProvider<T>& maskedLattDP) const {
  if (!noInclude_p || !noExclude_p) {
    DataRanges range;
    if (!noInclude_p || !noExclude_p) {
      range.push_back(std::pair<T, T>(range_p[0], range_p[1]));
...
Bool LatticeStatistics<T>::getLayerStats(String& stats, Double area, Int zAxis, Int zLayer,
  ...
  if (displayAxes_p.nelements() == 0) {
    ...
    os << setw(10) << "Npts";
    os << setw(oDWidth) << "Sum";
    if (_canDoFlux()) os << setw(oDWidth) << "Flux (Jy)";
    os << setw(oDWidth) << "Mean";
    if (doRobust_p) os << setw(oDWidth) << "Median";
    os << setw(oDWidth) << "Rms";
    ...
    os << setw(oDWidth) << rms;
    setStream(os, oPrec);
    os << setw(oDWidth) << sigma;
    setStream(os, oPrec);
    os << setw(oDWidth) << dMin;
    setStream(os, oPrec);
...
Bool LatticeStatistics<T>::listLayerStats(const Matrix<AccumType>& stats, ostringstream& os,
  ...
  for (uInt j = 0; j < n1; j++) {
    if (zLayer == (Int)j || n1 == 1) {
      ...
      os.setf(ios::fixed, ios::floatfield);
      os.setf(ios::left, ios::adjustfield);
      os << setw(10) << stats.column(NPTS)(j);

      if (stats.column(NPTS)(j) > 0) {
        setStream(os, oPrec);
        os << setw(oDWidth) << stats.column(SUM)(j);
        if (_canDoFlux()) {
          setStream(os, oPrec);
          os << setw(oDWidth) << stats.column(FLUX)(j);
        }
        setStream(os, oPrec);
        os << setw(oDWidth) << stats.column(MEAN)(j);
        if (doRobust_p) {
          setStream(os, oPrec);
          os << setw(oDWidth) << stats.column(MEDIAN)(j);
        }
        setStream(os, oPrec);
        os << setw(oDWidth) << stats.column(RMS)(j);
        setStream(os, oPrec);
        os << setw(oDWidth) << stats.column(SIGMA)(j);
        setStream(os, oPrec);
        os << setw(oDWidth) << stats.column(MIN)(j);
        setStream(os, oPrec);
        os << setw(oDWidth) << stats.column(MAX)(j);
      }
      os << endl;
    }
  }
...
template <class T>
void LatticeStatistics<T>::_latticePosToStoragePos(IPosition& storagePos,
                                                   const IPosition& latticePos) {
  ThrowIf(latticePos.nelements() != pInLattice_p->ndim(), "Incorrectly sized position given");
  ThrowIf(storagePos.size() < displayAxes_p.size(),
          "storage position does not have enough elements");
  ;
  ThrowIf(latticePos.size() < displayAxes_p.size(),
...
// is the statistics axis
{
  ...
  shape(pStoreLattice_p->ndim() - 1) = pStoreLattice_p->shape()(pStoreLattice_p->ndim() - 1);
  return shape;
}
...
//
{
  ...
  if (nPts > 0) {
    displayStats(nPts, sum, median, medAbsDevMed, quartile, sumSq, mean, var, rms, sigma, dMin,
                 dMax, q1, q3);
  }
}
...
void LatticeStatistics<T>::displayStats(AccumType nPts, AccumType sum, AccumType median,
  ...
  setStream(os8, oPrec);
  setStream(os9, oPrec);
  setStream(os10, oPrec), setStream(os11, oPrec);
