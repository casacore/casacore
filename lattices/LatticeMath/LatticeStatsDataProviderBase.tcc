...
LatticeStatsDataProviderBase<T>::getRanges() {
  return _ranges;
}
...
template <class T>
void LatticeStatsDataProviderBase<T>::minMaxPos(IPosition& minPos, IPosition& maxPos) const {
  minPos = _minPos;
  maxPos = _maxPos;
}
...
template <class T>
void LatticeStatsDataProviderBase<T>::setProgressMeter(std::shared_ptr<LattStatsProgress> pm) {
  _progressMeter = pm;
}
