  ...
  try {
    {
      ...
      {  // test the complex->complex fft function
        ...
        centre(2) = nz / 2;
        AlwaysAssert(near(cArr.getAt(centre), Complex(nx * ny * nz, 0), 1E-5), AipsError);
        cArr.putAt(Complex(0, 0), centre);
        ...
        planeShape(2) = 1;
        {
          ...
          for (planeIter.reset(); !planeIter.atEnd(); planeIter++) {
            if (planeIter.position()(2) == centre(2))
              ...
              cValue = Complex(0, 0);
            AlwaysAssert(allNearAbs(planeIter.cursor(), cValue, 1E-5), AipsError);
          }
        }
      }
      {  // test the real->complex fft function
        ...
        AlwaysAssert(near(cArr.getAt(centre), Complex(ny * ny * nz, 0), 1E-5), AipsError);
        cArr.putAt(Complex(0, 0), centre);
        {
          RO_LatticeIterator<Complex> iter(cArr);
          for (iter.reset(); !iter.atEnd(); iter++) {
