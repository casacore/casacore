...
int main() {
  try {
    ...
    }
    {
      ...
      lh.getHistograms(values, counts, stats);
      AlwaysAssert(stats(IPosition(1, 0))[LatticeStatsBase::SUM] == sum, AipsError);
      AlwaysAssert(stats(IPosition(1, 0))[LatticeStatsBase::NPTS] == 48, AipsError);
      AlwaysAssert(stats(IPosition(1, 0))[LatticeStatsBase::MEAN] == sum / 48, AipsError);
      Vector<Int> axes(2, 0);
      axes[1] = 1;
      lh.setAxes(axes);
      lh.getHistograms(values, counts, stats);
      ...
      for (uInt i = 0; i < 6; ++i) {
        Float sum = 16 + 8 * i;
        AlwaysAssert(stats(IPosition(1, i))[LatticeStatsBase::SUM] == sum, AipsError);
        AlwaysAssert(stats(IPosition(1, i))[LatticeStatsBase::NPTS] == 8, AipsError);
        AlwaysAssert(stats(IPosition(1, i))[LatticeStatsBase::MEAN] == sum / 8.0, AipsError);
      }
      axes.resize(1);
      axes[0] = 1;
      lh.setAxes(axes);
      ...
      for (uInt i = 0; i < 2; ++i) {
        for (uInt j = 0; j < 6; ++j) {
          Float sum = 4 * (i + j) + 6;
          AlwaysAssert(stats(IPosition(2, i, j))[LatticeStatsBase::SUM] == sum, AipsError);
          AlwaysAssert(stats(IPosition(2, i, j))[LatticeStatsBase::NPTS] == 4, AipsError);
          AlwaysAssert(stats(IPosition(2, i, j))[LatticeStatsBase::MEAN] == sum / 4.0, AipsError);
        }
      }
    }
