...
ArrayLattice<T>::ArrayLattice(const ArrayLattice<T>& other)
    : Lattice<T>(), itsData(other.itsData), itsWritable(other.itsWritable) {}
...
template <class T>
void ArrayLattice<T>::doPutSlice(const Array<T>& sourceBuffer, const IPosition& where,
  ...
  if (sdim == ldim) {
    itsData(where, where + (sourceBuffer.shape() - 1) * stride, stride) = sourceBuffer;
  } else {
    AlwaysAssert(ldim > sdim, AipsError);
    Array<T> allAxes(sourceBuffer.addDegenerate(ldim - sdim));
    itsData(where, where + (allAxes.shape() - 1) * stride, stride) = allAxes;
  }
}

template <class T>
void ArrayLattice<T>::getIterSlice(Array<T>& buffer, const IPosition& start, const IPosition& end,
