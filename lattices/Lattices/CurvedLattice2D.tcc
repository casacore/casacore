...
CurvedLattice2D<T>::CurvedLattice2D()
    : itsLatticePtr(0), itsInterpolator(0), itsAxis1(0), itsAxis2(0), itsCurveAxis(0) {}
...
CurvedLattice2D<T>::CurvedLattice2D(const MaskedLattice<T>& lattice,
  ...
  itsInterpolator->set(itsLatticePtr, itsAxesMap, itsAxis1, itsAxis2, itsCurveAxis);
}
...
template <class T>
CurvedLattice2D<T>& CurvedLattice2D<T>::operator=(const CurvedLattice2D<T>& other) {
  if (this != &other) {
    delete itsLatticePtr;
...
template <class T>
void CurvedLattice2D<T>::doPutSlice(const Array<T>&, const IPosition&, const IPosition&) {
