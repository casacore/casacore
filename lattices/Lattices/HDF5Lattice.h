...
template <typename T>
class HDF5Lattice : public Lattice<T> {
 ...
 public:
  ...
  HDF5Lattice(const TiledShape& shape, const String& filename, const String& arrayName = "array",
  ...
  explicit HDF5Lattice(const String& fileName, const String& arrayName = "array",
                       const String& groupName = String());

  // Reconstruct from a pre-existing HDF5Lattice in the HDF5 file and group
  // with the given name.
  explicit HDF5Lattice(const std::shared_ptr<HDF5File>& file, const String& arrayName,
                       const String& groupName = String());
  ...
  const std::shared_ptr<HDF5Group>& group() const { return itsGroup; }

  // Returns the current HDF5DataSet object
  const std::shared_ptr<HDF5DataSet>& array() const { return itsDataSet; }

  // Returns the name of this HDF5Lattice.
  const String& arrayName() const { return itsDataSet->getName(); }
