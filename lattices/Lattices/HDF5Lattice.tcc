...
template <typename T>
HDF5Lattice<T>::HDF5Lattice(const TiledShape& shape, const std::shared_ptr<HDF5File>& file,
                            const String& arrayName, const String& groupName)
    : itsFile(file) {
  makeArray(shape, arrayName, groupName);
  DebugAssert(ok(), AipsError);
...
template <typename T>
HDF5Lattice<T>::HDF5Lattice(const String& fileName, const String& arrayName,
...
template <typename T>
HDF5Lattice<T>::HDF5Lattice(const std::shared_ptr<HDF5File>& file, const String& arrayName,
                            const String& groupName)
    : itsFile(file) {
  openArray(arrayName, groupName);
  DebugAssert(ok(), AipsError);
...
template <typename T>
void HDF5Lattice<T>::openArray(const String& arrayName, const String& groupName) {
  if (groupName.empty()) {
    // Use root group.
...
template <typename T>
void HDF5Lattice<T>::makeArray(const TiledShape& shape, const String& arrayName,
                               const String& groupName) {
