...
template <class T>
void LatticeConcat<T>::setup1(IPosition& blc, IPosition& trc, IPosition& stride, IPosition& blc2,
                              IPosition& trc2, IPosition& blc3, IPosition& trc3, IPosition& stride3,
...
Bool LatticeConcat<T>::getSlice2(Array<T>& buffer, const Slicer& section, uInt nLattices) {
  ...
  for (uInt i = 0; i < nLattices; i++) {
    ...
    if (!(blc(axis_p) > end || trc(axis_p) < start)) {
      ...
      section2 = setup2(first, blc2, trc2, shape2, axis_p, blc, trc, stride, start);

      // Put it in output buffer

      // cout << "blc2, trc2, stride2, shape = "
      //      << section2.start() << section2.end() << section2.stride() << section2.length() <<
      //      endl;
...
Bool LatticeConcat<T>::putSlice1(const Array<T>& buffer, const IPosition& where,
  ...
  for (Int i = section.start()(axis_p); i <= section.end()(axis_p); i += section.stride()(axis_p)) {
    ...
    trc3(axis_p) = k;
    Array<T> buf0(buffer);
    lattices_p[i]->putSlice(buf0(blc3, trc3, stride3).nonDegenerate(axis_p - 1), section2.start(),
                            section2.stride());
    if (tempClose_p) lattices_p[i]->tempClose();
    k++;
  }
