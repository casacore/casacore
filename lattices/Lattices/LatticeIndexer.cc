...
LatticeIndexer::LatticeIndexer(const IPosition& shape, const IPosition& blc, const IPosition& trc,
                               const IPosition& inc)
    : itsFullShape(shape),
      itsNdim(shape.nelements()),
      itsShape(shape),
...
Bool LatticeIndexer::tiledCursorMove(Bool incr, IPosition& cursorPos, const IPosition& cursorShape,
  ...
  while (indexToActiveAxis < itsNdim) {
    ...
    if (incr) {
      ...
      candidateCursorPos(activeAxis) +=
          ((itsShape(activeAxis) - candidateCursorPos(activeAxis) - 1) / cursorShape(activeAxis)) *
          cursorShape(activeAxis);
    }
    indexToActiveAxis++;
  }  // while
