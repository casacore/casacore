...
LatticeIterInterface<T>::LatticeIterInterface(const Lattice<T>& lattice,
                                              const LatticeNavigator& nav, Bool useRef)
    : itsNavPtr(nav.clone()),
...
template <class T>
LatticeIterInterface<T>::LatticeIterInterface(const LatticeIterInterface<T>& other) : itsCurPtr(0) {
  copyBase(other);
  DebugAssert(ok() == True, AipsError);
}
...
template <class T>
LatticeIterInterface<T>& LatticeIterInterface<T>::operator=(const LatticeIterInterface<T>& other) {
  if (this != &other) {
    rewriteData();
    copyBase(other);
  }
...
Vector<T>& LatticeIterInterface<T>::vectorCursor(Bool doRead, Bool autoRewrite) {
  ...
  if (itsCurPtr->ndim() != 1) {
    throw(
        AipsError("LatticeIterInterface<T>::vectorCursor"
...
Matrix<T>& LatticeIterInterface<T>::matrixCursor(Bool doRead, Bool autoRewrite) {
  ...
  if (itsCurPtr->ndim() != 2) {
    throw(
        AipsError("LatticeIterInterface<T>::matrixCursor"
...
Cube<T>& LatticeIterInterface<T>::cubeCursor(Bool doRead, Bool autoRewrite) {
  ...
  if (itsCurPtr->ndim() != 3) {
    throw(
        AipsError("LatticeIterInterface<T>::cubeCursor"
...
void LatticeIterInterface<T>::rewriteData() {
  if (itsRewrite) {
    ...
    if (itsCursor.data() != itsCurPtr->data()) {
      throw(
          AipsError("LatticeIterInterface::rewriteData - "
