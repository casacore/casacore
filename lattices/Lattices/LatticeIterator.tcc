...
template <class T>
RO_LatticeIterator<T>& RO_LatticeIterator<T>::operator=(const RO_LatticeIterator<T>& other) {
  DebugAssert(ok(), AipsError);
  if (this != &other) {
    itsIterPtr = other.itsIterPtr;
  }
...
LatticeIterator<T>::LatticeIterator(Lattice<T>& lattice, Bool useRef)
  ...
  if (!lattice.isWritable()) {
    throw(
        AipsError("LatticeIterator cannot be constructed; "
...
template <class T>
LatticeIterator<T>& LatticeIterator<T>::operator=(const LatticeIterator<T>& other) {
