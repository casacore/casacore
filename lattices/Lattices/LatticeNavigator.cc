...
IPosition LatticeNavigator::relativePosition() const { return (position() - blc()) / increment(); }

IPosition LatticeNavigator::relativeEndPosition() const {
  return (endPosition() - blc()) / increment();
}

IPosition LatticeNavigator::subLatticeShape() const { return latticeShape(); }
...
void LatticeNavigator::subSection(const IPosition& blc, const IPosition& trc) {
  subSection(blc, trc, IPosition(latticeShape().nelements(), 1));
}

void LatticeNavigator::subSection(const IPosition&, const IPosition&, const IPosition&) {
  throw(
      AipsError("LatticeNavigator::subSection(blc, trc, inc)"
                " - sub-Lattice's are not supported"));
}

IPosition LatticeNavigator::blc() const { return IPosition(latticeShape().nelements(), 0); }

IPosition LatticeNavigator::trc() const { return latticeShape() - 1; }
