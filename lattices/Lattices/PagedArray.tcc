...
template <class T>
PagedArray<T>::PagedArray(Table& file)
    : itsTable(file),
      itsColumnName(defaultColumn()),
      itsRowNumber(defaultRow()),
      itsIsClosed(False),
...
Bool PagedArray<T>::isWritable() const {
  ...
  if (itsIsClosed) {
    return (itsWritable || Table::isWritable(itsTableName));
  }
  return (itsTable.isWritable() || Table::isWritable(itsTable.tableName()));
}
...
template <class T>
void PagedArray<T>::doPutSlice(const Array<T>& sourceArray, const IPosition& where,
...
void PagedArray<T>::makeArray(const TiledShape& shape) {
  ...
  if (!itsTable.tableDesc().isColumn(itsColumnName)) {
    ...
    description.defineHypercolumn(itsColumnName, ndim, stringToVector(itsColumnName));
