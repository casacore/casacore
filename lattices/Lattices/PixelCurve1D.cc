...
PixelCurve1D::PixelCurve1D (const Function1D<float,float>& func,
			    float x1, float x2, uint32_t npoints)
{
  ...
  uint32_t np = uint32_t(1000 * max(abs(x2-x1), abs(func(x2) - func(x1))));
...
PixelCurve1D::PixelCurve1D (const Vector<int32_t>& x, const Vector<int32_t>& y,
			    uint32_t npoints)
{
			 ...
			 uint32_t npoints)
{
  ...
  if (npoints == 0) {
    npoints = 1 + uint32_t(totleng + 0.1);
  }
  itsNpoints = npoints;
...
void PixelCurve1D::getPixelCoord (Vector<float>& x, Vector<float>& y,
				  uint32_t start, uint32_t end, uint32_t incr) const
{
  AlwaysAssert (start<=end && end<itsNpoints, AipsError);
  uint32_t nr = 1 + (end-start) / incr;
  x.resize (nr);
  y.resize (nr);
  for (uint32_t i=0; i<nr; i++) {
    x[i] = itsX[start];
