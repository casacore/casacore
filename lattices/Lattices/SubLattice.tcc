...
void SubLattice<T>::setRegion(const LatticeRegion& region) {
  ThrowIf(!(itsLatticePtr->shape().isEqual(region.region().latticeShape())),
...
void SubLattice<T>::setRegion() {
  ...
  setRegion(LatticeRegion(Slicer(IPosition(shape.nelements(), 0), shape), shape));
}
...
template <class T>
void SubLattice<T>::doPutSlice(const Array<T>& sourceBuffer, const IPosition& where,
  ...
  if (!itsAxesMap.isRemoved()) {
    ...
    Array<T> tmp = sourceBuffer.reform(itsAxesMap.shapeToOld(sourceBuffer.shape()));
    itsLatticePtr->putSlice(tmp, itsRegion.convert(itsAxesMap.posToOld(where)),
                            itsAxesMap.shapeToOld(stride) * itsRegion.slicer().stride());
  }
}
