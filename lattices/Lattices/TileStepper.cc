  ...
  while (empty) {
    ...
    for (int i = 0; i < nrdim; i++) {
      ...
      if (itsCurBlc(i) <= itsBlc(i)) {
        ...
        itsCurBlc(i) =
            (itsCurBlc(i) - itsBlc(i) + itsInc(i) - 1) / itsInc(i) * itsInc(i) + itsBlc(i);
      }
      itsCurTrc(i) = (itsCurTrc(i) - itsBlc(i)) / itsInc(i) * itsInc(i) + itsBlc(i);
...
void TileStepper::reset() {
  ...
  for (int i = 0; i < nrdim; i++) {
    ...
    if (itsCurBlc(i) <= itsBlc(i)) {
      ...
      itsCurBlc(i) = (itsCurBlc(i) - itsBlc(i) + itsInc(i) - 1) / itsInc(i) * itsInc(i) + itsBlc(i);
    }
    itsCurTrc(i) = (itsCurTrc(i) - itsBlc(i)) / itsInc(i) * itsInc(i) + itsBlc(i);
    //      cout << itsCurBlc << itsCurTrc << endl;
  }
...
// degenerate axes.
void TileStepper::subSection(const IPosition& blc, const IPosition& trc, const IPosition& inc) {
...
Bool TileStepper::ok() const {
  ...
  if (!(itsTiler.isInside(itsTilerCursorPos) ||
    ...
    str << "cursor beginning " << itsTilerCursorPos << " or end "
