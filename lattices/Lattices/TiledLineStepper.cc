    ...
    for (uInt i = 0; i < nrdim; i++) {
      if (i != itsAxis) {
        ...
        if (tileblc(i) <= itsBlc(i)) {
          ...
          tileblc(i) = (tileblc(i) - itsBlc(i) + itsInc(i) - 1) / itsInc(i) * itsInc(i) + itsBlc(i);
        }
        tiletrc(i) = (tiletrc(i) - itsBlc(i)) / itsInc(i) * itsInc(i) + itsBlc(i);
...
void TiledLineStepper::reset() {
  ...
  tilerTrc(itsAxis) =
      std::min(latticeShape()(itsAxis) - 1, tilerBlc(itsAxis) + itsTileShape(itsAxis) - 1);
  itsTiler.fullSize();
  itsTiler.subSection(tilerBlc, tilerTrc);
  itsTilerCursorPos = 0;
  itsCursorShape(itsAxis) = 1 + (itsTrc(itsAxis) - itsBlc(itsAxis)) / itsInc(itsAxis);
  // # Calculate the boundaries of the tile.
  IPosition tileblc = itsTiler.absolutePosition(itsTilerCursorPos);
  IPosition tiletrc = tileblc + itsTileShape - 1;
  tileblc(itsAxis) = itsBlc(itsAxis);
...
// degenerate axes.
void TiledLineStepper::subSection(const IPosition& blc, const IPosition& trc,
                                  const IPosition& inc) {
...
Bool TiledLineStepper::ok() const {
  ...
  if (!(itsTiler.isInside(itsTilerCursorPos) ||
    ...
    str << "tiler cursor beginning " << itsTilerCursorPos << " or end "
  ...
  if (!(itsIndexer.isInside(itsIndexerCursorPos) ||
    ...
    str << "cursor beginning " << itsIndexerCursorPos << " or end "
