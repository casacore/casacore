  ...
  while (True) {
    ...
    if (opt == 1) {
      ...
      if (!tab.hasLock((opt == 4))) {
        cout << "Cannot get/put; lattice is not (correctly) locked" << endl;
      } else {
        if (opt == 4) {
          cout << "value: ";
          cin >> val;
...
int main(int argc, const char* argv[]) {
  if (argc < 2) {
    cout << "Execute as: tLatticeLocker 1  to create new table" << endl;
    cout << "            tLatticeLocker 0  to update existing table" << endl;
  } else {
    try {
      if (*(argv[1]) == '1') {
