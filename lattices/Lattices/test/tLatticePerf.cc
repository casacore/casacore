...
void makeCube(bool useHDF, const IPosition& cubeShape, const IPosition& tileShape) {
  ...
  if (useHDF) {
    ...
    cout << "Creating tLatticePerf_tmp.tab with shape " << cubeShape << " and tile shape "
         << tileShape << endl;
    cout << "CCTS    ";
...
int main(int argc, char* argv[]) {
  if (argc <= 1) {
    cerr << "Run as:  tLatticePerf nx ny nz ntx nty ntz [hdf5]   to create" << endl;
    cerr << "or       tLatticePerf type [hdf5]                   to read back" << endl;
