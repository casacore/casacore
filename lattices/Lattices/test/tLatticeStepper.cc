...
int main() {
  try {
    ...
    logger << "Expect to see a number of SEVERE errors logged shortly"
           << " (if in Debug mode)" << LogIO::POST;
    {
      IPosition badCursor(4, 1);
      IPosition smallLatticeShape(3, latticeShape(0), latticeShape(1), latticeShape(2));
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(), IPosition(2, 1, 2),
                             IPosition());
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(5, 1), IPosition(2, 1, 2),
                             IPosition());
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(2, 1), IPosition(5, 1),
                             IPosition());
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(2, 1), IPosition(3, 1),
                             IPosition());
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(2, 1), IPosition(2, 4),
                             IPosition());
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(4, 2, 3, 4, 1), IPosition(2, 1, 2),
                             IPosition());
      ...
      try {
        LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(2, 1), IPosition(2, 2, 1),
                             IPosition());
    ...
    logger << "End of section which checks error detection" << LogIO::POST;

    // Try  LatticeStepper with cursorAxes given.
    {
      LatticeStepper step1(IPosition(4, 2, 3, 4, 5), IPosition(2, 3, 4), IPosition(2, 1, 2),
                           IPosition());
      AlwaysAssertExit(step1.cursorShape() == IPosition(4, 1, 3, 4, 1));
      AlwaysAssertExit(step1.cursorAxes() == IPosition(2, 1, 2));
      LatticeStepper step2(IPosition(4, 2, 3, 4, 5), IPosition(4, 1, 3, 4, 1), IPosition(2, 1, 2),
                           IPosition());
      AlwaysAssertExit(step2.cursorShape() == IPosition(4, 1, 3, 4, 1));
      AlwaysAssertExit(step2.cursorAxes() == IPosition(2, 1, 2));
      LatticeStepper step3(IPosition(4, 2, 3, 4, 5), IPosition(4, 1, 3, 4, 1), IPosition(),
                           IPosition());
    ...
    AlwaysAssert(s0.endPosition().isEqual(IPosition(4, 3, 0, 0, 0)), AipsError);
    s0++;
    AlwaysAssert(s0.position().isEqual(IPosition(4, 4, 0, 0, 0)), AipsError);
    AlwaysAssert(s0.endPosition().isEqual(IPosition(4, 7, 0, 0, 0)), AipsError);
    s0++;
    AlwaysAssert(s0.position().isEqual(IPosition(4, 8, 0, 0, 0)), AipsError);
    AlwaysAssert(s0.endPosition().isEqual(IPosition(4, 9, 0, 0, 0)), AipsError);
    s0++;
    AlwaysAssert(s0.position().isEqual(IPosition(4, 0, 1, 0, 0)), AipsError);
    AlwaysAssert(s0.endPosition().isEqual(IPosition(4, 3, 1, 0, 0)), AipsError);
    s0++;
    AlwaysAssert(s0.position().isEqual(IPosition(4, 4, 1, 0, 0)), AipsError);
    AlwaysAssert(s0.endPosition().isEqual(IPosition(4, 7, 1, 0, 0)), AipsError);

    count = 0;
    for (s0.reset(); !s0.atEnd(); s0++) count++;
    // Check the cursor is at the end (note the overhang)
    AlwaysAssert(s0.endPosition().isEqual(IPosition(4, 9, 11, 3, 6)), AipsError);
    AlwaysAssert(count == 12 * 4 * 7 * 3, AipsError);

    for (; !s0.atStart(); s0--) count--;
    // Should end up where we started
    ...
    LatticeStepper s4(latticeShape, stepperShape, stepperOrientation, LatticeStepper::RESIZE);
    s4.subSection(IPosition(4, 1, 0, 0, 0), IPosition(4, 9, 0, 0, 0), IPosition(4, 2, 1, 1, 1));
    ...
    AlwaysAssert(clonePtr->cursorShape() == method.cursorShape(), AipsError);
    ...
    method.subSection(IPosition(3, 0, 1, 0), IPosition(3, 3, 4, 3), IPosition(3, 1, 2, 3));
    ...
    AlwaysAssert(stepPtr->relativeEndPosition() == IPosition(3, 3, 0, 0), AipsError);
