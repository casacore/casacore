...
void DirectionEngine::handleScalars(const TENShPtr& e1, const TENShPtr& e2, const TENShPtr& e3) {
  if (!(e1->isConstant() && e2->isConstant()) || (e3 && !e3->isConstant())) {
    throw AipsError(
        "Scalar values given as direction in a MEAS function "
  ...
  if (e3) {
    ...
    if (!(u1.empty() && u2.empty() && e3->unit().empty())) {
      throw AipsError(
          "Directions given as x,y,z in a MEAS function "
          "cannot have units");
    }
  } else {
    if (u1.empty()) u1 = "rad";
  ...
  if (e3) {
    itsConstants.data()[0] = MDirection(MVDirection(v1, v2, v3), itsRefType);
  } else {
    itsConstants.data()[0] = MDirection(Quantity(v1, u1), Quantity(v2, u2), itsRefType);
  }
}
...
void DirectionEngine::handleNames(const TENShPtr& operand) {
  if (!operand->isConstant()) {
    throw AipsError(
        "Object names given as directions in a MEAS function "
        "must be constant values");
  }
    ...
    if (name.substr(0, 3) == "SUN") {
      ...
      if (!ext.empty()) {
        if (ext == "-C") {
          ...
          throw AipsError(
              "invalid SUN type; use -C, -U, -L, -CR, -UR, -LR,"
              " -CT, -NT, -AT, -ST");
        }
      }
...
void DirectionEngine::handleValues(TableExprNode& operand, const TableExprId& id,
  ...
  if (shape[0] % 2 == 0) {
    ...
    throw AipsError(
        "Number of values in a direction in a MEAS function "
  ...
  for (uInt i = 0; i < directions.size(); ++i) {
    if (nrv == 2) {
      ...
      dirVec[i] =
          MDirection(MVDirection(valVec[i * 3], valVec[i * 3 + 1], valVec[i * 3 + 2]), itsRefType);
    }
  }
...
Array<Double> DirectionEngine::getArrayDouble(const TableExprId& id, Bool riseSet, Bool asDirCos) {
  ...
  if (res.size() > 0 && eps.size() > 0 && pos.size() > 0) {
    ...
    for (Array<MPosition>::const_contiter posIter = pos.cbegin(); posIter != pos.cend();
         ++posIter) {
      // Convert to desired position.
      if (itsPositionEngine) {
        itsFrame.resetPosition(*posIter);
      }
      for (Array<MEpoch>::const_contiter epsIter = eps.cbegin(); epsIter != eps.cend(); ++epsIter) {
        ...
        for (Array<MDirection>::const_contiter resIter = res.cbegin(); resIter != res.cend();
             ++resIter, ++hIndex) {
          if (riseSet) {
            calcRiseSet(*resIter, *posIter, *epsIter, (hIndex < itsH.size() ? itsH[hIndex] : 0),
                        outPtr[0], outPtr[1]);
            outPtr += 2;
          } else {
...
void DirectionEngine::calcRiseSet(const MDirection& dir, const MPosition& pos, const MEpoch& epoch,
...
int DirectionEngine::fillRiseSet(double epoch, const MDirection& dir, double lat, double h,
                                 const MEpoch& off, double* rise, double* set) {
