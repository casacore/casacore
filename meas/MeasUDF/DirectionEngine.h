...
class DirectionEngine : public MeasEngine<MDirection> {
 ...
 private:
  ...
  void calcRiseSet(const MDirection& dir, const MPosition& pos, const MEpoch& epoch, double h,
                   double& rise, double& set);
  int fillRiseSet(double epoch, const MDirection& dir, double lat, double h, const MEpoch& off,
                  double* rise, double* set);
