...
void DopplerEngine::handleRestFreq(vector<TENShPtr>& args, uInt& argnr) {
  ...
  if (itsRestFreqNode->dataType() == TableExprNodeRep::NTString) {
    ...
    if (!(itsRestFreqNode->unit().empty() || itsRestFreqNode->unit() == Unit("Hz"))) {
...
void DopplerEngine::handleLine(const TENShPtr& operand) {
  ...
  if (!operand->isConstant()) {
    throw AipsError(
        "A line name used as frequency in a MEAS function"
  ...
  for (uInt i = 0; i < names.size(); ++i) {
    ...
    if (!MeasTable::Line(mfreq, names.data()[i])) {
      throw AipsError("Line '" + names.data()[i] +
                      "' used as a"
...
void DopplerEngine::handleValues(TableExprNode& operand, const TableExprId& id,
                                 Array<MDoppler>& dopplers) {
  if (itsType == DOPPLER) {
...
Array<Double> DopplerEngine::getArrayDouble(const TableExprId& id) {
  ...
  if (res.size() > 0) {
    ...
    for (Array<MDoppler>::const_contiter resIter = res.cbegin(); resIter != res.cend(); ++resIter) {
