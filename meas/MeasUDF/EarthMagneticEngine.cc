...
void EarthMagneticEngine::handleEarthMagnetic(vector<TENShPtr>& args, uInt& argnr) {
  ...
  // can be 3 scalars.
  if (args.size() > nargnr + 1 && args[argnr]->isReal() &&
...
void EarthMagneticEngine::deriveAttr(const Unit& unit, Int) {
  ...
  if (unit.empty()) {
    ...
    if (q.isConform("T")) {
      ...
      throw AipsError(
          "Invalid unit given for an EarthMagnetic value"
...
void EarthMagneticEngine::handleScalars(const TENShPtr& e1, const TENShPtr& e2,
  ...
  if (!(e1->isConstant() && e2->isConstant() && e3->isConstant())) {
    throw AipsError(
        "Scalar values given as EarthMagnetic in a MEAS "
  ...
  itsConstants.data()[0] = makeEarthMagnetic(Quantity(vh, uh), Quantity(v1, u1), Quantity(v2, u2));
}

MEarthMagnetic EarthMagneticEngine::makeEarthMagnetic(const Quantity& qh, const Quantity& q1,
                                                      const Quantity& q2) const {
  if (itsValueType == 3) {
    Unit m("nT");
    return MEarthMagnetic(MVEarthMagnetic(q1.getValue(m), q2.getValue(m), qh.getValue(m)),
                          itsRefType);
  }
...
void EarthMagneticEngine::handleValues(TableExprNode& operand, const TableExprId& id,
  ...
  if (shape[0] % 3 != 0) {
    throw AipsError(
        "Number of values in an EarthMagnetic in a MEAS "
...
Array<Double> EarthMagneticEngine::getArrayDouble(const TableExprId& id) {
  ...
  for (Array<MPosition>::const_contiter posIter = pos.cbegin(); posIter != pos.cend(); ++posIter) {
    ...
    for (Array<MEpoch>::const_contiter epsIter = eps.cbegin(); epsIter != eps.cend(); ++epsIter) {
      ...
      for (size_t i2 = 0; i2 < nval2; ++i2) {
        if (itsUseModel) {
          EarthMagneticMachine emm(dPtr[i2].getRef(), dPtr[i2].getValue(), itsFrame);
          // Calculate the values for all heights.
          for (size_t i1 = 0; i1 < nval1; ++i1) {
            emm.calculate(hPtr[i1]);
            if (itsConvertModel) {
              itsConverter.setModel(MEarthMagnetic(emm.getField(), MEarthMagnetic::ITRF));
...
void EarthMagneticEngine::copyEM(const MVEarthMagnetic& em, double*& outPtr) {
  if (itsToValueType == 1) {
    // Get as length.
...
void EarthMagneticEngine::copyLLEM(EarthMagneticMachine& emm, double*& outPtr) {
  if (itsAsLOS == 1) {
