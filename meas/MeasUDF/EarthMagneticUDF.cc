...
EarthMagneticUDF::EarthMagneticUDF(FuncType type)
    : itsType(type), itsValueType(0), itsRefType(MEarthMagnetic::ITRF) {}

UDFBase* EarthMagneticUDF::makeEMXYZ(const String&) { return new EarthMagneticUDF(EMXYZ); }
UDFBase* EarthMagneticUDF::makeEMANG(const String&) { return new EarthMagneticUDF(EMANG); }
UDFBase* EarthMagneticUDF::makeEMLEN(const String&) { return new EarthMagneticUDF(EMLEN); }
UDFBase* EarthMagneticUDF::makeIGRFXYZ(const String&) { return new EarthMagneticUDF(IGRFXYZ); }
UDFBase* EarthMagneticUDF::makeIGRFANG(const String&) { return new EarthMagneticUDF(IGRFANG); }
UDFBase* EarthMagneticUDF::makeIGRFLEN(const String&) { return new EarthMagneticUDF(IGRFLEN); }
UDFBase* EarthMagneticUDF::makeIGRFLOS(const String&) { return new EarthMagneticUDF(IGRFLOS); }
UDFBase* EarthMagneticUDF::makeIGRFLONG(const String&) { return new EarthMagneticUDF(IGRFLONG); }
...
void EarthMagneticUDF::setup(const Table&, const TaQLStyle&) {
  ...
  if (itsEngine.handleMeasType(operands()[0], False)) {
    ...
    if (itsRefType == MEarthMagnetic::IGRF) {
      throw AipsError(
          "IGRF cannot be used as to type; "
    ...
    if (itsEngine.valueType() != 0) {
      if (itsValueType != 0) {
        throw AipsError(
            "Valuetype should be given once; "
  ...
  if (useModel) {
    ...
    if (operands().size() <= argnr + 1) {
      throw AipsError(
          "No heights and directions given in a MEAS IGRF "
          "function");
    }
  ...
  if (operands().size() > argnr) {
    throw AipsError(
        "Too many arguments given in a MEAS EarthMagnetic "
        "function");
  }
