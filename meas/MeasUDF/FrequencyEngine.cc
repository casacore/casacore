...
Array<Double> FrequencyEngine::getArrayDouble(const TableExprId& id, int type) {
  ...
  if (!(res.empty() || dir.empty() || eps.empty() || pos.empty() || rv.empty() || dop.empty())) {
    ...
    if (res.size() > 1 || dir.size() > 1 || eps.size() > 1 || pos.size() > 1 || rv.size() > 1 ||
        dop.size() > 1) {
    ...
    for (Array<MPosition>::const_contiter posIter = pos.cbegin(); posIter != pos.cend();
      ...
      for (Array<MEpoch>::const_contiter epsIter = eps.cbegin(); epsIter != eps.cend(); ++epsIter) {
        ...
        for (Array<MDirection>::const_contiter dirIter = dir.cbegin(); dirIter != dir.cend();
             ++dirIter) {
          if (itsDirectionEngine) {
            itsFrame.resetDirection(*dirIter);
            itsRVFrame.resetDirection(*dirIter);
          }
          for (Array<MRadialVelocity>::const_contiter rvIter = rv.cbegin(); rvIter != rv.cend();
               ++rvIter) {
            if (itsRadVelEngine) {
            ...
            for (Array<MDoppler>::const_contiter dopIter = dop.cbegin(); dopIter != dop.cend();
                 ++dopIter) {
              for (Array<MFrequency>::const_contiter resIter = res.cbegin(); resIter != res.cend();
                   ++resIter) {
