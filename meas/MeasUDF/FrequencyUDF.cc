...
FrequencyUDF::FrequencyUDF(FuncType type) : itsType(type), itsRefType(MFrequency::DEFAULT) {}

UDFBase* FrequencyUDF::makeFREQ(const String&) { return new FrequencyUDF(FREQUENCY); }
UDFBase* FrequencyUDF::makeREST(const String&) { return new FrequencyUDF(REST); }
UDFBase* FrequencyUDF::makeSHIFT(const String&) { return new FrequencyUDF(SHIFT); }
