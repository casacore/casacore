...
void MeasEngine<M>::handleMeasArray(const TENShPtr& operand) {
  ...
  if ((!operand->isReal()) || (operand->valueType() != TableExprNodeRep::VTScalar &&
                               operand->valueType() != TableExprNodeRep::VTArray)) {
    throw AipsError(M::showMe() +
                    " value given in a MEAS function "
  ...
  if (tabCol) {
    ...
    if (TableMeasDescBase::hasMeasures(*tabCol)) {
      TableMeasColumn measTmp(tabCol->table(), tabCol->columnDesc().name());
      // Check the measure is the correct type.
      AlwaysAssert(measTmp.measDesc().type() == M::showMe(), AipsError);
      ...
      if (directCol) {
        if (scaNode) {
          itsMeasScaCol.attach(tabCol->table(), tabCol->columnDesc().name());
        } else {
          itsMeasArrCol.attach(tabCol->table(), tabCol->columnDesc().name());
        }
        return;
      }
  ...
  if (itsMeasScaCol.isNull() && itsMeasArrCol.isNull()) {
    if (itsRefType == M::N_Types) {
      throw AipsError(
          "No reference type given for a non-constant MEAS "
          "function " +
          M::showMe() + " argument");
    }
