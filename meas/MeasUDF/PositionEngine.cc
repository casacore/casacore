...
PositionEngine::PositionEngine() : itsValueType(0) {}
...
void PositionEngine::deriveAttr(const Unit& unit, Int nval) {
  ...
  if (itsValueType == 0) {
    ...
    throw AipsError(
        "The nr of position values in a MEAS function does not "
  ...
  if (!unit.empty()) {
    if (!itsInUnit.empty()) {
      ...
      if (itsInUnit != unit) {
        throw AipsError(
            "Unit of a position in a MEAS function does not "
            "match the reference type suffix");
      }
    }
    itsInUnit = unit;
  ...
  if (itsInUnit.empty()) {
    ...
    if (itsValueType < 0) {
      throw AipsError(
          "A position in a MEAS function given as llh "
          "should not have a unit");
    }
    if (itsInUnit == Unit("m")) {
      if (itsValueType == 2) {
        throw AipsError(
            "For unit m 1 or 3 position values should be given "
            "in a MEAS function");
      }
    } else if (itsInUnit == Unit("rad")) {
      if (itsValueType != 2) {
        throw AipsError(
            "For unit deg 2 position values should be given "
            "in a MEAS function");
      }
    } else {
      throw AipsError(
          "Invalid unit given for a position value"
  ...
  if (itsValueType == 0) {
    throw AipsError(
        "The value type of a position in a MEAS function is "
...
void PositionEngine::handleScalars(const TENShPtr& e1, const TENShPtr& e2, const TENShPtr& e3,
  ...
  if (!e1->isConstant() || (e2 && !e2->isConstant()) ||
    ...
    throw AipsError(
        "Scalar values given as position in a MEAS function "
  ...
  itsConstants.data()[0] = makePosition(Quantity(v1, u1), Quantity(v2, u2), Quantity(v3, u3));
}

MPosition PositionEngine::makePosition(const Quantity& q1, const Quantity& q2,
  ...
  if (itsValueType == 1) {
    ...
    // xyz.
    Unit m("m");
    return MPosition(MVPosition(q1.getValue(m), q2.getValue(m), q3.getValue(m)), itsRefType);
  }
  // height,lon,lat
...
void PositionEngine::handleObservatory(const TENShPtr& operand) {
  ...
  if (!operand->isConstant()) {
    throw AipsError(
        "An observatory name used as position in a MEAS function"
...
void PositionEngine::handlePosArray(const TENShPtr& anglesNode, const TENShPtr& heightNode) {
  if (!anglesNode->isReal() || anglesNode->valueType() != TableExprNodeRep::VTArray ||
      ...
      heightNode->valueType() != TableExprNodeRep::VTArray || !heightNode->isConstant()) {
    throw AipsError(
        "Positions given as angles,heights in a MEAS "
