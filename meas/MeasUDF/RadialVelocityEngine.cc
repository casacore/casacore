...
void RadialVelocityEngine::handleValues(TableExprNode& operand, const TableExprId& id,
...
Array<Double> RadialVelocityEngine::getArrayDouble(const TableExprId& id) {
  ...
  if (res.size() > 0 && dir.size() > 0 && eps.size() > 0 && pos.size() > 0) {
    ...
    for (Array<MRadialVelocity>::const_contiter resIter = res.cbegin(); resIter != res.cend();
         ++resIter) {
      ...
      for (Array<MDirection>::const_contiter dirIter = dir.cbegin(); dirIter != dir.cend();
        ...
        for (Array<MEpoch>::const_contiter epsIter = eps.cbegin(); epsIter != eps.cend();
             ++epsIter) {
