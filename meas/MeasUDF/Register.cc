...
void HelpMeasUDF::showFuncsDirection(ostream& os, Bool showTypes) {
  ...
  if (showTypes) {
    ...
    os << " In function RISET type SUN can have a suffix -XX where XX can be (default -UR):"
       << endl;
    ...
    os << "   AT   amateur astronomy twilight (-15)   ST  scientific astronomy twilight (-18)"
       << endl;
...
void HelpMeasUDF::showFuncsFrequency(ostream& os, Bool showTypes) {
  ...
  os << "  MEAS.REST (freq, radvel, direction, epoch, position)         convert to rest freq"
     << endl;
  os << "  MEAS.REST (freq, doppler)                                    convert to rest freq"
     << endl;
...
UDFBase* HelpMeasUDF::makeHELP(const String&) { return new HelpMeasUDF(); }
...
String HelpMeasUDF::getString(const TableExprId& id) {
  ...
  if (os.str().empty()) {
    os << type << " is an unknown meas subtype; use pos(ition), epoch, dir(ection),"
       << " earthmagnetic (em), freq(uency) or radialvelocity (radvel)" << endl;
  } else {
    os << endl
       << "See also section 'Special Measures functions'"
