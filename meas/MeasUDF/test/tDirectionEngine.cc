...
void testScalar(Bool asDirCos) {
  ...
  MeasFrame frame(coord, epo, pos);
  {
    ...
    TableExprNode node(tableCommand("calc meas." + funcStr +
  ...
  }
  {
    ...
    if (asDirCos) {
      ...
      dir = MDirection::Convert(coord, MDirection::Ref(MDirection::J2000))()
                .getValue()
                .getAngle("deg")
                .getValue();
    }
  ...
  }
  {
    ...
    TableExprNode node(tableCommand("calc meas." + funcStr + "meas." + funcStr2 +
                                    "[185.425833deg, 31.799167deg],"
                                    "'B1950'))deg")
                           .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
...
void testArray(Bool asDirCos) {
  ...
  String funcStr = (asDirCos ? "dircos('APP'," : "app(");
  TableExprNode node1(tableCommand("calc meas." + funcStr +
                                   "[185.425833deg, 31.799167deg,"
  ...
  TableExprNode node2(tableCommand("using style python calc meas." + funcStr +
                                   "[[[185.425833deg, 31.799167deg],"
                                   "[175.425833deg, 41.799167deg]],"
                                   "[[165.425833deg, 51.799167deg],"
                                   "[155.425833deg, 61.799167deg]]],'J2000',"
...
void testColumn(Bool asDirCos) {
  ...
  pos[2] = MPosition(Quantity(2000, "m"), Quantity(-6.60417, "deg"), Quantity(32.8, "deg"),
                     MPosition::WGS84);
  /// for (int i=0; i<3; ++i) {
  /// cout << MPosition::Convert(pos[i], MPosition::Ref(MPosition::ITRF))().getValue().getValue() <<
  /// endl;
  /// }
  // Convert a few directions from J2000.
  String funcStr = (asDirCos ? "dircos('APP'," : "app(");
  TableExprNode node1(tableCommand("using style python calc meas." + funcStr +
                                   "DIR[0,],TIME,POS1)deg "
...
void testName() {
  ...
  TableExprNode node1(tableCommand("using style python calc meas.app ("
...
void testRiset() {
  ...
  TableExprNode node(tableCommand("calc str(meas.riseset(['SUN','MOON'], "
                                  "10Aug2010/13:12:11, 'WSRT'))")
                         .node());
  AlwaysAssertExit(node.getNodeRep()->isConstant());
...
void testErr() {
  ...
  int nsucc = 0;
  nsucc += checkErr(
      "using style python calc meas.j2000("
      "[DIR[0,0], DIR[0,1]]deg, TIME,POS2)deg "
      "from tDirectionEngine_tmp.tab");
  nsucc += checkErr(
      "using style python calc meas.app(TIME) "
      "from tDirectionEngine_tmp.tab");
  nsucc += checkErr(
      "using style python calc meas.app(POS1) "
      "from tDirectionEngine_tmp.tab");
  nsucc += checkErr(
      "using style python calc meas.app(DIR) "
      "from tDirectionEngine_tmp.tab");
