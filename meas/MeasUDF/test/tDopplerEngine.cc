...
void testDopplerScalar() {
  ...
  MVDoppler ndop = MDoppler::Convert(dop, MDoppler::Ref(MDoppler::Z))().getValue();
  /// cout << "meas=" << ndop << endl;
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
                                     "0.5, 'BETA')")
                            .node());
...
void testRadVelScalar() {
  ...
  MVDoppler ndop = MDoppler::Convert(vel.toDoppler(), MDoppler::Ref(MDoppler::Z))().getValue();
  /// cout << "meas=" << ndop << endl;
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
                                     "200 'km/s', 'BARY')")
...
void testFreqScalar() {
  ...
  MVDoppler ndop = MDoppler::Convert(freq.toDoppler(rest), MDoppler::Ref(MDoppler::Z))().getValue();
  /// cout << "meas=" << ndop << endl;
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
...
void testDopplerArray() {
  ...
  MVDoppler ndop1 = MDoppler::Convert(dop1, MDoppler::Ref(MDoppler::OPTICAL))().getValue();
  MDoppler dop2(Quantity(0.6, ""), MDoppler::RADIO);
  MVDoppler ndop2 = MDoppler::Convert(dop2, MDoppler::Ref(MDoppler::OPTICAL))().getValue();
  /// cout << "meas=" << ndop << endl;
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('OPTICAL',"
                                     "[0.5, 0.6], 'RADIO')")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
...
void testRadVelArray() {
  ...
  MVDoppler ndop1 =
      MDoppler::Convert(vel1.toDoppler(), MDoppler::Ref(MDoppler::RADIO))().getValue();
  MRadialVelocity vel2(Quantity(300, "km/s"), MRadialVelocity::BARY);
  MVDoppler ndop2 =
      MDoppler::Convert(vel2.toDoppler(), MDoppler::Ref(MDoppler::RADIO))().getValue();
  /// cout << "meas=" << ndop << endl;
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('RADIO',"
                                     "[200,300] 'km/s', 'BARY')")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
    AlwaysAssertExit(node1.getNodeRep()->valueType() == TableExprNodeRep::VTArray);
...
void testFreqArray() {
  ...
  MVDoppler ndopa1 =
      MDoppler::Convert(freq1.toDoppler(rest1), MDoppler::Ref(MDoppler::Z))().getValue();
  MVDoppler ndopa2 =
      MDoppler::Convert(freq2.toDoppler(rest2), MDoppler::Ref(MDoppler::Z))().getValue();
  MVDoppler ndopb1 =
      MDoppler::Convert(freq1.toDoppler(rest1), MDoppler::Ref(MDoppler::Z))().getValue();
  MVDoppler ndopb2 =
      MDoppler::Convert(freq1.toDoppler(rest2), MDoppler::Ref(MDoppler::Z))().getValue();
  MVDoppler ndopc1 =
      MDoppler::Convert(freq1.toDoppler(rest1), MDoppler::Ref(MDoppler::Z))().getValue();
  MVDoppler ndopc2 =
      MDoppler::Convert(freq2.toDoppler(rest1), MDoppler::Ref(MDoppler::Z))().getValue();
  /// cout << "meas=" << ndop << endl;
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
                                     "[[200,300]] MHz, 'LSRK', [[1.425445GHz,4.874157GHz]])")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
    AlwaysAssertExit(node1.getNodeRep()->valueType() == TableExprNodeRep::VTArray);
    Array<Double> arr1 = node1.getArrayDouble(0);
  ...
  {
    TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
                                     "[200,300] MHz, 'LSRK', ['CII166A','H110A'])")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
    AlwaysAssertExit(node1.getNodeRep()->valueType() == TableExprNodeRep::VTArray);
    Array<Double> arr1 = node1.getArrayDouble(0);
...
void testDopplerColumn() {
  ...
  /// cout << "meas=" << ndop << endl;
  TableExprNode node1(tableCommand("calc meas.doppler ('OPTICAL',"
                                   "DOPPCOL) from tDopplerEngine_tmp.tab")
                          .node());
  AlwaysAssertExit(!node1.getNodeRep()->isConstant());
...
void testRadVelColumn() {
  ...
  MRadialVelocity vel1(Quantity(200, "km/s"), MRadialVelocity::BARY);
  MRadialVelocity vel2(Quantity(300, "km/s"), MRadialVelocity::BARY);
  MVDoppler ndop1 = MDoppler::Convert(vel1.toDoppler(), MDoppler::Ref(MDoppler::Z))().getValue();
  MVDoppler ndop2 = MDoppler::Convert(vel2.toDoppler(), MDoppler::Ref(MDoppler::Z))().getValue();
  /// cout << "meas=" << ndop << endl;
  TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
                                   "RVCOL) from tDopplerEngine_tmp.tab")
                          .node());
  AlwaysAssertExit(!node1.getNodeRep()->isConstant());
  AlwaysAssertExit(node1.getNodeRep()->valueType() == TableExprNodeRep::VTArray);
  AlwaysAssertExit(node1.unit().getName() == "");
...
void testFreqColumn() {
  ...
  /// cout << "meas=" << ndop << endl;
  TableExprNode node1(tableCommand("calc meas.doppler ('Z',"
                                   "FREQ,RESTFREQ) from tDopplerEngine_tmp.tab")
                          .node());
  AlwaysAssertExit(!node1.getNodeRep()->isConstant());
  AlwaysAssertExit(node1.getNodeRep()->valueType() == TableExprNodeRep::VTScalar);
  AlwaysAssertExit(node1.unit().getName() == "");
