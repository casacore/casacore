...
void testScalar() {
  ...
  MEarthMagnetic coord(
      MVEarthMagnetic(Quantity(10, "nT"), Quantity(185.425833, "deg"), Quantity(31.799167, "deg")),
  ...
  MeasFrame frame(epo, pos);
  {
    Vector<Double> em =
        MEarthMagnetic::Convert(coord, MEarthMagnetic::Ref(MEarthMagnetic::APP, frame))()
            ...
            .getValue();
    cout << "meas=" << em << endl;
    TableExprNode node(tableCommand("calc meas.em('app',"
  ...
  }
  {
    Vector<Double> em = MEarthMagnetic::Convert(coord, MEarthMagnetic::Ref(MEarthMagnetic::J2000))()
                            ...
                            .getValue();
    cout << "meas=" << em << endl;
    TableExprNode node(tableCommand("calc meas.em('j2000',"
                                    "185.425833deg, 31.799167deg,10nT,'B1950')")
                           .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
    AlwaysAssertExit(node.unit().getName() == "nT");
  ...
  }
  {
    ...
    cout << "meas=" << em << endl;
    TableExprNode node(
        tableCommand("calc meas.emang('b1950',"
                     "-8.46092318369e-9nT, -8.03641753778e-10,5.26943439197e-9,'B1950')")
            .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
  ...
  }
  {
    ...
    cout << "meas=" << em << endl;
    TableExprNode node(
        tableCommand("calc meas.emlen('b1950',"
                     "-8.46092318369e-9nT, -8.03641753778e-10,5.26943439197e-9,'B1950')")
            .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
...
void testArray() {
  ...
  TableExprNode node1(tableCommand("calc meas.em('APP',"
  ...
  TableExprNode node2(tableCommand("using style python calc meas.em('APP',"
                                   "[[[185.425833/180*pi(), 31.799167/180*pi(), 5],"
                                   "[175.425833/180*pi(), 41.799167/180*pi(), 10]],"
                                   "[[165.425833/180*pi(), 51.799167/180*pi(), 20],"
                                   "[155.425833/180*pi(), 61.799167/180*pi(), 30]]],"
  ...
  coord[0] = MEarthMagnetic(
      MVEarthMagnetic(Quantity(5, "nT"), Quantity(185.425833, "deg"), Quantity(31.799167, "deg")),
      MEarthMagnetic::J2000);
  coord[1] = MEarthMagnetic(
      MVEarthMagnetic(Quantity(10, "nT"), Quantity(175.425833, "deg"), Quantity(41.799167, "deg")),
      MEarthMagnetic::J2000);
  coord[2] = MEarthMagnetic(
      MVEarthMagnetic(Quantity(20, "nT"), Quantity(165.425833, "deg"), Quantity(51.799167, "deg")),
      MEarthMagnetic::J2000);
  coord[3] = MEarthMagnetic(
      MVEarthMagnetic(Quantity(30, "nT"), Quantity(155.425833, "deg"), Quantity(61.799167, "deg")),
      MEarthMagnetic::J2000);
...
void testColumn() {
  ...
  coord[0] = MEarthMagnetic(MVEarthMagnetic(-4.22979e-09, -3.98632e-10, 2.63628e-09),
                            MEarthMagnetic::ITRF);
  coord[1] =
      MEarthMagnetic(MVEarthMagnetic(-7.42905e-09, 6.00219e-10, 6.66701e-09), MEarthMagnetic::ITRF);
  coord[2] =
      MEarthMagnetic(MVEarthMagnetic(-1.28774e-08, 5.90596e-09, 2.64441e-08), MEarthMagnetic::ITRF);
  epo[0] = MEpoch(Quantity(50217.625, "d"));  // 14-may-1996/15:00
  epo[1] = MEpoch(Quantity(50417.625, "d"));  // 30-nov-1996/15:00
  epo[2] = MEpoch(Quantity(50617.625, "d"));  // 18-jun-1997/15:00
  pos[0] = MPosition(Quantity(10, "m"), Quantity(6.60417, "deg"), Quantity(52.8, "deg"),
  ...
  pos[2] = MPosition(Quantity(2000, "m"), Quantity(-6.60417, "deg"), Quantity(32.8, "deg"),
                     MPosition::WGS84);
  // Convert a few earthMagnetics from J2000.
  TableExprNode node1(tableCommand("using style python calc meas.em('app',"
                                   "EM,TIME,POS1) "
                                   "from tEarthMagneticEngine_tmp.tab")
                          .node());
  TableExprNode node2(tableCommand("using style python calc meas.em('app',"
...
void testModel() {
  ...
  {
    TableExprNode node(tableCommand("calc "
                                    "meas.igrf(0,[3828488.86,443253.42,5064977.78],'ITRF',"
...
void testModelConv() {
  ...
  MEarthMagnetic coord(fm.getField(), MEarthMagnetic::ITRF);
  {
    ...
    cout << "meas=" << em << endl;
    TableExprNode node(tableCommand("calc "
                                    "meas.igrf('APP',0,[3828488.86,443253.42,5064977.78],'ITRF',"
                                    "18may1998,[3828488.86m,443253.42m,5064977.78m])")
                           .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
    Array<Double> arr1 = node.getArrayDouble(0);
...
void testErr() {
  ...
  nsucc += checkErr(
      "using style python calc meas.emxyz('app',POS1[0]'',"
