...
void testScalar() {
  ...
  MeasFrame frame(pos);
  {
    Double value = MEpoch::Convert(epo, MEpoch::Ref(MEpoch::LAST, frame))().getValue().get();
    TableExprNode node(tableCommand("calc meas.epoch('f-last',"
                                    "mjdtodate(50217.625d),'UTC',"
                                    "6.60417deg, 52.8deg, 10m, 'WGS84')d")
                           .node());
  ...
  }
  {
    TableExprNode node(tableCommand("calc "
                                    "meas.epoch('ut1', 50217.625d,'UTC')d")
                           .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
    Double sca1 = node.getDouble(0);
    AlwaysAssertExit(node.unit().getName() == "d");
  ...
  {
    TableExprNode node(tableCommand("calc meas.epoch('utc',"
                                    "meas.epoch('ut1', 50217.625d,'UTC'))d")
                           .node());
    AlwaysAssertExit(node.getNodeRep()->isConstant());
    Double sca1 = node.getDouble(0);
...
void testArray() {
  ...
  TableExprNode node1(tableCommand("calc meas.epoch('f-last',"
                                   "mjdtodate([50217.625d,50417.625d,50617.625]),'UTC',"
                                   "[6.60417deg, 52.8deg, -60.60417deg, -32.8deg],"
                                   "[10m,1000m], 'WGS84')d")
                          .node());
  // Do it as multi-dim TaQL arrays.
  TableExprNode node2(tableCommand("using style python calc meas.epoch('f-last',"
  ...
  for (uInt ip = 0; ip < pos.size(); ++ip) {
    for (uInt ie = 0; ie < epo.size(); ++ie) {
      double epn = MEpoch::Convert(epo[ie], MEpoch::Ref(MEpoch::LAST, MeasFrame(pos[ip])))()
                       .getValue()
                       .get();
...
void testColumn() {
  ...
  TableExprNode node1(tableCommand("using style python calc meas.epoch('F_LAST',"
                                   "TIME,POS1)d "
                                   "from tEpochEngine_tmp.tab")
                          .node());
  // Use a column with POS in xyz.
  TableExprNode node2(tableCommand("using style python calc meas.epoch('F_LAST',"
                                   "TIME,POS2)d "
                                   "from tEpochEngine_tmp.tab")
                          .node());
  // Use a nested MEAS function.
  TableExprNode node3(tableCommand("calc meas.epoch ('UTC', meas.epoch('F_LAST',"
                                   "TIME,POS2), POS2)d "
                                   "from tEpochEngine_tmp.tab")
                          .node());
  // Now use a column expression, so reftype must be given.
  TableExprNode node4(tableCommand("using style python calc meas.epoch('F_LAST',"
                                   "TIME-0,'UTC',POS1)d "
                                   "from tEpochEngine_tmp.tab")
                          .node());
  ...
  for (uInt i = 0; i < 3; ++i) {
    ...
    double epn =
        MEpoch::Convert(epo[i], MEpoch::Ref(MEpoch::LAST, MeasFrame(pos[i])))().getValue().get();
...
void testErr() {
  ...
  nsucc += checkErr(
      "using style python calc meas.epoch(POS1) "
      "from tEpochEngine_tmp.tab");
  nsucc += checkErr(
      "using style python calc meas.epoch('ut1',TIME+0) "
      "from tEpochEngine_tmp.tab");
