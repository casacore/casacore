...
void testScalar() {
  ...
  // cout << "meas=" << res1 << endl;
  {
    TableExprNode node(tableCommand("calc meas.freq('BARY',1GHz, 'LSRK',"
  ...
  }
  {
    ...
    TableExprNode node(tableCommand("calc meas.freq('BARY',1e-9 s, 'LSRK',"
                                    "185.425833deg, 31.799167deg,'B1950',"
                                    "mjdtodate(50217.625d),'UTC',"
                                    "6.60417deg, 52.8deg, 10m, 'WGS84')")
                           .node());
  ...
  }
  {
    ...
    TableExprNode node(tableCommand("calc meas.freq('BARY',c()/1GHz, 'LSRK',"
                                    "185.425833deg, 31.799167deg,'B1950',"
                                    "mjdtodate(50217.625d),'UTC',"
                                    "6.60417deg, 52.8deg, 10m, 'WGS84')")
                           .node());
  ...
  }
  {
    ...
    TableExprNode node(tableCommand("calc meas.rest(1GHz, 'LSRK',"
                                    "1000 'km/s', 'BARY',"
                                    "185.425833deg, 31.799167deg,'B1950',"
                                    "mjdtodate(50217.625d),'UTC',"
                                    "6.60417deg, 52.8deg, 10m, 'WGS84')")
  ...
  {
    // Convert to rest which requires radvel.
    TableExprNode node(tableCommand("calc meas.freq('rest',1GHz, 'LSRK',"
                                    "1000 'km/s', 'BARY',"
                                    "185.425833deg, 31.799167deg,'B1950',"
                                    "mjdtodate(50217.625d),'UTC',"
                                    "6.60417deg, 52.8deg, 10m, 'WGS84')")
  ...
  }
  {
    ...
    TableExprNode node(tableCommand("calc meas.freq('LSRK',"
...
void testArray() {
  ...
  TableExprNode node1(tableCommand("calc meas.freq ('LSRK', [[200],[220]]MHz, 'BARY',"
                                   ...
                                   "[10m,1000m], 'WGS84')")
                          .node());
  TableExprNode node2(tableCommand("calc meas.rest ([[200],[220]]MHz, 'BARY',"
  ...
  coord[1] = MDirection(Quantity(175.425833, "deg"), Quantity(41.799167, "deg"), MDirection::J2000);
  coord[2] = MDirection(Quantity(165.425833, "deg"), Quantity(51.799167, "deg"), MDirection::J2000);
  coord[3] = MDirection(Quantity(155.425833, "deg"), Quantity(61.799167, "deg"), MDirection::J2000);
...
void testDopplerArray() {
  ...
  TableExprNode node1(tableCommand("calc meas.rest([1,1.1,1.2]GHz, 'LSRK', [3.5,2.5])").node());
  TableExprNode node2(tableCommand("calc meas.shift([400,440,480]MHz, 'LSRK', [3.5])").node());
  TableExprNode node3(tableCommand("calc meas.shift([400,440,480]MHz/0.6, 'LSRK', 2.5)").node());
  AlwaysAssertExit(node1.getNodeRep()->isConstant());
  AlwaysAssertExit(node2.getNodeRep()->isConstant());
...
void testColumn() {
  ...
  TableExprNode node1(tableCommand("using style python calc meas.freq("
                                   "'BARY',FREQ,"
                                   "DIR[0,],TIME,POS1) "
                                   "from tFrequencyEngine_tmp.tab")
                          .node());
  TableExprNode node2(tableCommand("using style python calc meas.rest("
                                   "FREQ,RVCOL,"
                                   "DIR[0,],TIME,POS1) "
                                   "from tFrequencyEngine_tmp.tab")
                          .node());
...
void testDopplerColumn() {
  ...
  dop[1] = MDoppler(MVDoppler(0.6), MDoppler::RADIO);
  // Convert a few frequencies.
  TableExprNode node1(tableCommand("calc meas.rest(FREQ, DOPPCOL) "
                                   "from tFrequencyEngine_tmp.tab")
                          .node());
  TableExprNode node2(tableCommand("calc meas.shift(FREQ, DOPPCOL) "
                                   "from tFrequencyEngine_tmp.tab")
                          .node());
  AlwaysAssertExit(!node1.getNodeRep()->isConstant());
  AlwaysAssertExit(!node2.getNodeRep()->isConstant());
