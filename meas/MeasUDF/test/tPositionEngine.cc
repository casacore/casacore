...
void testScalar() {
  ...
  MVPosition npos = MPosition::Convert(pos, MPosition::Ref(MPosition::ITRF))().getValue();
  /// cout << "meas=" << npos << endl;
  {
    TableExprNode node1(tableCommand("calc meas.pos ('ITRF',"
                                     "6.60417deg, 52.8deg, 10m, 'WGS84')")
                            .node());
    TableExprNode node2(tableCommand("calc meas.itrfxyz ("
                                     "6.60417deg, 52.8deg, 10m, 'WGS84')")
                            .node());
  ...
  }
  {
    MPosition hpos1(MVPosition(Quantity(10, "m")),
                    MPosition::WGS84);  // near WSRT
    MVPosition nhpos1 = MPosition::Convert(hpos1, MPosition::Ref(MPosition::ITRF))().getValue();
    MPosition hpos2(MVPosition(Quantity(1000, "m")),
                    MPosition::WGS84);  // near WSRT
    MVPosition nhpos2 = MPosition::Convert(hpos2, MPosition::Ref(MPosition::ITRF))().getValue();
    TableExprNode node1(tableCommand("calc meas.pos ('ITRF',"
                                     "10m)")
                            .node());
    TableExprNode node2(tableCommand("calc meas.itrfxyz ("
  ...
  {
    TableExprNode node1(tableCommand("calc meas.itrfll ("
                                     "6.60417deg, 52.8deg, 10m, 'WGS84')")
                            .node());
    TableExprNode node2(tableCommand("calc meas.itrfh ("
                                     "6.60417*pi()/180., 52.8*pi()/180, 10,"
                                     "'WGS84LL')")
                            .node());
    TableExprNode node3(tableCommand("calc meas.itrfllh ("
                                     "[6.60417*pi()/180., 52.8*pi()/180, 10],"
                                     "'WGS84LLH')")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
    ...
    AlwaysAssertExit(allNear(npos.getAngle().getValue(), veciter1.vector(), 1e-8));
    AlwaysAssertExit(near(npos.getLength().getValue(), arr2, 1e-8));
    AlwaysAssertExit(near(npos.getAngle().getValue()[0], arr3.data()[0], 1e-8));
    AlwaysAssertExit(near(npos.getAngle().getValue()[1], arr3.data()[1], 1e-8));
    AlwaysAssertExit(near(npos.getLength().getValue(), arr3.data()[2], 1e-8));
  }
  {
    TableExprNode node1(tableCommand("calc meas.itrfll ("
                                     "6.60417deg, 52.8deg, 10m, 'WGS84')")
  ...
  }
  {
    ...
    TableExprNode node1(tableCommand("calc meas.itrfllh (meas.itrfxyz ("
                                     "6.60417deg, 52.8deg, 10m, 'WGS84'))")
                            .node());
    TableExprNode node2(tableCommand("calc meas.itrfxyz (meas.itrfllh ("
                                     "6.60417deg, 52.8deg, 10m, 'WGS84'))")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
    AlwaysAssertExit(node2.getNodeRep()->isConstant());
    ...
    AlwaysAssertExit(node2.unit().getName() == "m");
    AlwaysAssertExit(near(npos.getAngle().getValue()[0], arr1.data()[0], 1e-8));
    AlwaysAssertExit(near(npos.getAngle().getValue()[1], arr1.data()[1], 1e-8));
    AlwaysAssertExit(near(npos.getLength().getValue(), arr1.data()[2], 1e-8));
    AlwaysAssertExit(allNear(npos.getValue(), veciter2.vector(), 1e-8));
  }
}
...
void testArray() {
  ...
  TableExprNode node1(tableCommand("calc meas.wgs ("
  ...
  TableExprNode node2(tableCommand("using style python calc meas.wgsll ("
                                   "[[[3.82849e+06, 443253, 5.06498e+06,"
  ...
  pos[1] = MPosition(MVPosition(3.83924e+06, 430428, 5.05801e+06), MPosition::ITRF);
  pos[2] = MPosition(MVPosition(3.83849e+06, 443453, 5.06598e+06), MPosition::ITRF);
...
void testColumn() {
  ...
  pos[0] = MPosition(Quantity(10, "m"), Quantity(6.60417, "deg"), Quantity(52.8, "deg"),
                     MPosition::WGS84);  // near WSRT
  pos[1] = MPosition(Quantity(1000, "m"), Quantity(-60.60417, "deg"), Quantity(-32.8, "deg"),
                     MPosition::WGS84);
  pos[2] = MPosition(Quantity(2000, "m"), Quantity(-6.60417, "deg"), Quantity(32.8, "deg"),
                     MPosition::WGS84);
  // Convert a few positions.
  TableExprNode node1(tableCommand("calc meas.itrfxyz (POS1) "
                                   "from tPositionEngine_tmp.tab")
                          .node());
...
void testName() {
  ...
  TableExprNode node1(tableCommand("using style python calc meas.itrfxyz ("
                                   "['WSRT','VLA'])")
                          .node());
  TableExprNode node2(tableCommand("using style python calc meas.wgsh ("
                                   "['WSRT','VLA'])")
                          .node());
  AlwaysAssertExit(node1.getNodeRep()->isConstant());
  AlwaysAssertExit(node2.getNodeRep()->isConstant());
  ...
  for (uInt ip = 0; ip < pos.size(); ++ip) {
    ...
    AlwaysAssertExit(near(npos2.getLength().getValue(), arr2.data()[ip], 1e-8));
    arr1iter.next();
  }
}
...
void testErr() {
  ...
  int nsucc = 0;
  nsucc += checkErr(
      "calc meas.itrfxyz ([POS1[1], POS1[2]], [POS1[3]],"
      "'WGS84') from tPositionEngine_tmp.tab");
  nsucc += checkErr(
      "calc meas.itrf ([POS1[1], POS1[2]], [POS1[3]],"
      "'WGS84') from tPositionEngine_tmp.tab");
  nsucc += checkErr(
      "calc meas.itrfxyz (1,2,"
      "'WGS84XYZ') from tPositionEngine_tmp.tab");
  nsucc += checkErr(
      "calc meas.itrfxyz ([1,2],"
      "'WGS84XYZ') from tPositionEngine_tmp.tab");
  nsucc += checkErr(
      "calc meas.itrfxyz ([POS1[1], POS1[2]],"
      "'WGS84XYZ') from tPositionEngine_tmp.tab");
  nsucc += checkErr(
      "calc meas.itrfxyz (POS1[1], POS1[2], POS1[3],"
      "'WGS84') from tPositionEngine_tmp.tab");
