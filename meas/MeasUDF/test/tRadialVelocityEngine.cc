...
void testScalar() {
  ...
  MRadialVelocity rv(Quantity(100, "km/s"), MRadialVelocity::Ref(MRadialVelocity::BARY, frame));
  MVRadialVelocity nrv =
      MRadialVelocity::Convert(rv, MRadialVelocity::Ref(MRadialVelocity::LSRK))().getValue();
  // cout << "meas=" << nrv << " m/s" << endl;
  {
    TableExprNode node1(tableCommand("calc meas.rv ('LSRK',"
                                     "0100'km/s', 'BARY',"
                                     "185.425833deg, 31.799167deg, 'B1950',"
                                     "50217.625d,"
                                     "6.60417deg, 52.8deg, 'WGS84')'m/s'")
                            .node());
...
void testDopplerScalar() {
  ...
  // cout << "meas=" << vel << endl;
  {
    TableExprNode node1(tableCommand("calc meas.radvel('LSRK',"
                                     "2.5, 'Z')")
                            .node());
    AlwaysAssertExit(node1.getNodeRep()->isConstant());
    Double val1 = node1.getDouble(0);
...
void testArray() {
  ...
  MPosition pos(Quantity(10, "m"), Quantity(6.60417, "deg"), Quantity(52.8, "deg"),
                MPosition::WGS84);  // near WSRT
  MeasFrame frame(coord, epo, pos);
  MRadialVelocity rv1(Quantity(100, "km/s"), MRadialVelocity::Ref(MRadialVelocity::LSRK, frame));
  MRadialVelocity rv2(Quantity(200, "km/s"), MRadialVelocity::Ref(MRadialVelocity::LSRK, frame));
  MVRadialVelocity nrv1 =
      MRadialVelocity::Convert(rv1, MRadialVelocity::Ref(MRadialVelocity::TOPO))().getValue();
  MVRadialVelocity nrv2 =
      MRadialVelocity::Convert(rv2, MRadialVelocity::Ref(MRadialVelocity::TOPO))().getValue();
  // cout << "meas=" << nrv1 <<' '<<nrv2<< << " m/s" << endl;
  {
    TableExprNode node1(tableCommand("calc meas.rv ('TOPO',"
                                     "[100,200]'km/s', 'LSRK',"
                                     "185.425833deg, 31.799167deg, 'B1950',"
                                     "50217.625d,"
                                     "6.60417deg, 52.8deg, 'WGS84')'m/s'")
...
void testColumn() {
  ...
  // Check with Measures.
  MDirection coord1(Quantity(185.425833, "deg"), Quantity(31.799167, "deg"), MDirection::J2000);
  MEpoch epo1(Quantity(50217.625, "d"));  // 14-may-1996/15:00
  MPosition pos1(Quantity(10, "m"), Quantity(6.60417, "deg"), Quantity(52.8, "deg"),
                 MPosition::WGS84);  // near WSRT
  MeasFrame frame1(coord1, epo1, pos1);
  MRadialVelocity rv1a(Quantity(200, "km/s"), MRadialVelocity::Ref(MRadialVelocity::BARY, frame1));
  MRadialVelocity rv2a(Quantity(300000, "m/s"),
                       MRadialVelocity::Ref(MRadialVelocity::BARY, frame1));
  MRadialVelocity nrv1a =
      MRadialVelocity::Convert(rv1a, MRadialVelocity::Ref(MRadialVelocity::GEO))();
  MRadialVelocity nrv2a =
      MRadialVelocity::Convert(rv2a, MRadialVelocity::Ref(MRadialVelocity::GEO))();
  Double vel1a = nrv1a.getValue().getValue() / 1000.;  // km/s
  Double vel2a = nrv2a.getValue().getValue() / 1000.;  // km/s

  MDirection coord2(Quantity(175.425833, "deg"), Quantity(41.799167, "deg"), MDirection::J2000);
  MEpoch epo2(Quantity(50417.625, "d"));  // 14-may-1996/15:00
  MPosition pos2(Quantity(1000, "m"), Quantity(-60.60417, "deg"), Quantity(-32.8, "deg"),
                 MPosition::WGS84);  // near WSRT
  MeasFrame frame2(coord2, epo2, pos2);
  MRadialVelocity rv1b(Quantity(300, "km/s"), MRadialVelocity::Ref(MRadialVelocity::BARY, frame2));
  MRadialVelocity rv2b(Quantity(200000, "m/s"),
                       MRadialVelocity::Ref(MRadialVelocity::BARY, frame2));
  MRadialVelocity nrv1b =
      MRadialVelocity::Convert(rv1b, MRadialVelocity::Ref(MRadialVelocity::GEO))().getValue();
  MRadialVelocity nrv2b =
      MRadialVelocity::Convert(rv2b, MRadialVelocity::Ref(MRadialVelocity::GEO))().getValue();
  ...
  TableExprNode node1(tableCommand("using style python calc meas.rv ('GEO',"
                                   "RVCOL, DIR[0,], TIME, POS1)"
                                   " from tRadialVelocityEngine_tmp.tab")
                          .node());
  AlwaysAssertExit(!node1.getNodeRep()->isConstant());
...
void testDopplerColumn() {
  ...
  Double vel2 = mvel2.getValue().getValue() / 1000.;  // km/s
  /// cout << "meas=" << ndop << endl;
  TableExprNode node1(tableCommand("calc meas.radvel('LSRK',DOPPCOL)"
                                   " from tRadialVelocityEngine_tmp.tab")
                          .node());
  AlwaysAssertExit(!node1.getNodeRep()->isConstant());
