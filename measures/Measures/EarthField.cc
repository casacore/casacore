      ...
      default: {
        ...
        for (Int lp = 0; lp < 4; lp++) {
          ...
          for (Int k = 0; k < PQ_LEN; k++) {
            ...
            if (k - 4 >= 0) {
              if (m + 1 - n == 0) {
                ...
                i = k - n;
                j = k - 2 * n + 1;
                p_p(k) = (fn + 1.0) * (three * slat / fn * p_p(i) - two / (fn - 1.0) * p_p(j));
                q_p(k) = three * (slat * q_p(i) - clat / fn * p_p(i)) - two * q_p(j);
              }
            }
            //
            ...
            if (m == -1) {
              ...
              z = z - three * p_p(k);
              if (clat > 0) {
                y = y + (one * sl_p(m) - two * cl_p(m)) * fm * p_p(k) / ((fn + 1.0) * clat);
              } else {
                y = y + (one * sl_p(m) - two * cl_p(m)) * q_p(k) * slat;
              }
              l += 2;
            }
            m++;
          ...
          if (lp == 0) {
            ...
            dval_p[lp - 1][0] =
                (+x * slat * clong + z * clat * clong + y * slong - pval_p[0]) / DER_INTV;
            dval_p[lp - 1][1] =
                (-x * slat * slong + z * clat * slong - y * clong - pval_p[1]) / DER_INTV;
            dval_p[lp - 1][2] = (-x * clat + z * slat - pval_p[2]) / DER_INTV;
          }
          if (lp < 3) {
