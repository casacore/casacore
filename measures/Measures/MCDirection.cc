...
// # Destructor
MCDirection::~MCDirection() { clearConvert(); }
...
void MCDirection::doConvert(MeasValue &in, MRBase &inref, MRBase &outref, const MConvertBase &mc) {
  doConvert(*(MVDirection *)&in, inref, outref, mc);
}
  ...
  for (Int i = 0; i < mc.nMethod(); i++) {
    switch (mc.getMethod(i)) {
      ...
      case R_PLANET: {
        ...
        if (planID != MeasTable::SUN &&
            (g3 = in * *MVPOS3,
             !nearAbs(g3, 1.0, 1.0 - cos(MeasTable::Planetary(MeasTable::RADS) / g2)))) {
      ...
      case R_COMET0: {
        MDirection::Ref::frameComet(inref, outref).getCometType(comID);
        if (!MDirection::Ref::frameComet(inref, outref).getComet(*MVPOS1)) {
...
String MCDirection::showState() {
  std::call_once(theirInitOnceFlag, doFillState);
  return MCBase::showState(MCDirection::FromTo_p[0], MDirection::N_Types, MCDirection::N_Routes,
                           MCDirection::ToRef_p);
}
