...
// # Constructors
MCDoppler::MCDoppler() { std::call_once(theirInitOnceFlag, doFillState); }

// # Destructor
MCDoppler::~MCDoppler() { clearConvert(); }
...
void MCDoppler::doConvert(MeasValue &in, MRBase &inref, MRBase &outref, const MConvertBase &mc) {
  doConvert(*(MVDoppler *)&in, inref, outref, mc);
}
...
String MCDoppler::showState() {
  ...
  return MCBase::showState(MCDoppler::FromTo_p[0], MDoppler::N_Types, MCDoppler::N_Routes,
                           MCDoppler::ToRef_p);
}
