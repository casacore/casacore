...
uInt MCEarthMagnetic::FromTo_p[MEarthMagnetic::N_Types][MEarthMagnetic::N_Types];
...
// # Destructor
MCEarthMagnetic::~MCEarthMagnetic() { clearConvert(); }

// # Operators

// # Member functions

void MCEarthMagnetic::getConvert(MConvertBase &mc, const MRBase &inref, const MRBase &outref) {
...
void MCEarthMagnetic::doConvert(MeasValue &in, MRBase &inref, MRBase &outref,
                                const MConvertBase &mc) {
  doConvert((MVEarthMagnetic &)in, inref, outref, mc);
}

void MCEarthMagnetic::doConvert(MVEarthMagnetic &in, MRBase &inref, MRBase &outref,
                                const MConvertBase &mc) {
  Double g2, tdbTime;
  ...
  for (Int i = 0; i < mc.nMethod(); i++) {
    switch (mc.getMethod(i)) {
      ...
      case R_MODEL:
        if (!EFIELD) {
          MEarthMagnetic::Ref::frameEpoch(inref, outref).getTDB(tdbTime);
          EFIELD = new EarthField(modID, tdbTime);
        }
        MEarthMagnetic::Ref::framePosition(outref, inref).getITRF(*MVPOS1);
...
String MCEarthMagnetic::showState() {
  std::call_once(theirInitOnceFlag, doFillState);
  return MCBase::showState(MCEarthMagnetic::FromTo_p[0], MEarthMagnetic::N_Types,
                           MCEarthMagnetic::N_Routes, MCEarthMagnetic::ToRef_p);
}
