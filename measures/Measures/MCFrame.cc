...
void MCFrame::resetRadialVelocity() { impl_->radLSRp.reset(); }
...
Bool MCFrame::getTDB(Double &tdb, const MeasFrame &frame) {
  if (frame.epoch()) {
    if (!impl_->epTDBp) {
      impl_->epTDBp =
          impl_->epConvTDB.value()(*dynamic_cast<const MVEpoch *>(frame.epoch()->getData()))
              .getValue()
              .get();
    }
    tdb = *impl_->epTDBp;
...
Bool MCFrame::getUT1(Double &tdb, const MeasFrame &frame) {
  if (frame.epoch()) {
    if (!impl_->epUT1p) {
      impl_->epUT1p =
          impl_->epConvUT1.value()(*dynamic_cast<const MVEpoch *>(frame.epoch()->getData()))
              .getValue()
              .get();
    }
    tdb = *impl_->epUT1p;
...
Bool MCFrame::getTT(Double &tdb, const MeasFrame &frame) {
  if (frame.epoch()) {
    if (!impl_->epTTp) {
      impl_->epTTp =
          impl_->epConvTT.value()(*dynamic_cast<const MVEpoch *>(frame.epoch()->getData()))
              .getValue()
              .get();
    }
    tdb = *impl_->epTTp;
...
Bool MCFrame::getLong(Double &tdb, const MeasFrame &frame) {
  if (frame.position()) {
    if (!(impl_->posLongp.has_value())) {
      impl_->posITRFp =
          impl_->posConvLong.value()(*dynamic_cast<const MVPosition *>(frame.position()->getData()))
              .getValue();
  ...
  if (frame.position()) {
    if (!(impl_->posLongGeop.has_value())) {
      impl_->posGeop = impl_->posConvLongGeo
                           .value()(*dynamic_cast<const MVPosition *>(frame.position()->getData()))
                           .getValue();
...
Bool MCFrame::getLAST(Double &tdb, const MeasFrame &frame) {
  if (frame.epoch()) {
    if (!impl_->epLASTp) {
      impl_->epLASTp =
          impl_->epConvLAST.value()(*dynamic_cast<const MVEpoch *>(frame.epoch()->getData()))
              .getValue()
              .get();
    }
...
Bool MCFrame::getJ2000Long(Double &tdb, const MeasFrame &frame) {
  if (frame.direction()) {
    if (!(impl_->j2000Longp.has_value())) {
      impl_->dirJ2000p =
          impl_->dirConvJ2000
              .value()(*dynamic_cast<const MVDirection *>(frame.direction()->getData()))
              .getValue();
...
Bool MCFrame::getLSR(Double &tdb, const MeasFrame &frame) {
  if (frame.radialVelocity()) {
    if (!impl_->radLSRp) {
      impl_->radLSRp =
          impl_->radConvLSR
              .value()(*dynamic_cast<const MVRadialVelocity *>(frame.radialVelocity()->getData()))
              .getValue();
    }
    tdb = *impl_->radLSRp;
    return True;
...
void MCFrame::makeEpoch(MeasFrame &frame) {
  ...
  impl_->epConvLAST = MEpoch::Convert(*(frame.epoch()), MEpoch::Ref(MEpoch::LAST, frame));
...
void MCFrame::makePosition(const MeasFrame &frame) {
  static const MPosition::Ref REFLONG = MPosition::Ref(MPosition::ITRF);
  impl_->posConvLong = MPosition::Convert(*frame.position(), REFLONG);
  impl_->posLongp.reset();
  impl_->posITRFp.reset();
  impl_->epLASTp.reset();
  impl_->radLSRp.reset();
  static const MPosition::Ref REFGEO = MPosition::Ref(MPosition::WGS84);
  impl_->posConvLongGeo = MPosition::Convert(*frame.position(), REFGEO);
  impl_->posLongGeop.reset();
  impl_->posGeop.reset();
}
...
void MCFrame::makeDirection(MeasFrame &frame) {
  ...
  impl_->dirConvJ2000 =
      MDirection::Convert(*frame.direction(), MDirection::Ref(MDirection::J2000, frame));
  impl_->dirConvB1950 =
      MDirection::Convert(*frame.direction(), MDirection::Ref(MDirection::B1950, frame));
  impl_->dirConvApp =
      MDirection::Convert(*frame.direction(), MDirection::Ref(MDirection::APP, frame));
  frame.rep.Unfreeze(state);
  impl_->j2000Longp.reset();
  impl_->dirJ2000p.reset();
  impl_->b1950Longp.reset();
...
void MCFrame::makeRadialVelocity(const MeasFrame &frame) {
  static const MRadialVelocity::Ref REFLSR = MRadialVelocity::Ref(MRadialVelocity::LSRK);
  impl_->radConvLSR = MRadialVelocity::Convert(*frame.radialVelocity(), REFLSR);
  impl_->radLSRp.reset();
}
