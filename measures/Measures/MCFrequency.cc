...
uInt MCFrequency::ToRef_p[N_Routes][3] = {
    ...
    {MFrequency::LSRD, MFrequency::GALACTO, 0}, {MFrequency::GALACTO, MFrequency::LSRD, 0},
    {MFrequency::LSRK, MFrequency::BARY, 0},    {MFrequency::BARY, MFrequency::LSRK, 0},
...
// # Destructor
MCFrequency::~MCFrequency() { clearConvert(); }
...
void MCFrequency::doConvert(MeasValue &in, MRBase &inref, MRBase &outref, const MConvertBase &mc) {
  doConvert(*(MVFrequency *)&in, inref, outref, mc);
}
  ...
  for (Int i = 0; i < mc.nMethod(); i++) {
    switch (mc.getMethod(i)) {
      case LSRD_BARY: {
        ...
        MFrequency::Ref::frameDirection(outref, inref).getJ2000(*MVDIR1);
      ...
      case BARY_LSRD: {
        ...
        MFrequency::Ref::frameDirection(inref, outref).getJ2000(*MVDIR1);
        g1 = (*MVPOS1 * *MVDIR1) / C::c;
        g2 = in.getValue();
        in = g2 * sqrt((1 - g1) / (1 + g1));
      } break;
      ...
      case BARY_GEO: {
        MFrequency::Ref::frameEpoch(outref, inref).getTDB(tdbTime);
      ...
      case GEO_TOPO: {
        MFrequency::Ref::frameEpoch(outref, inref).getLASTr(g1);
        MFrequency::Ref::framePosition(outref, inref).getRadius(lengthE);
        MFrequency::Ref::frameEpoch(outref, inref).getTDB(tdbTime);
        MFrequency::Ref::framePosition(outref, inref).getLat(g3);
        ...
        MFrequency::Ref::frameDirection(outref, inref).getApp(*MVDIR1);
        g1 = *MVPOS1 * *MVDIR1;
        g2 = in.getValue();
        in = g2 * sqrt((1 + g1) / (1 - g1));
      } break;
      ...
      case GEO_BARY: {
        MFrequency::Ref::frameEpoch(inref, outref).getTDB(tdbTime);
      ...
      case TOPO_GEO: {
        MFrequency::Ref::frameEpoch(inref, outref).getLASTr(g1);
        MFrequency::Ref::framePosition(inref, outref).getRadius(lengthE);
        MFrequency::Ref::frameEpoch(inref, outref).getTDB(tdbTime);
        MFrequency::Ref::framePosition(inref, outref).getLat(g3);
        g2 = MeasTable::diurnalAber(lengthE, tdbTime);
        // *MVPOS1 = MVDirection(C::pi_2 + g1, 0.0);
        updatePosition(M_PI_2 + g1, 0.0, *MVPOS1);
        MVPOS1->readjust(g2 * cos(g3));
      ...
      case REST_LSRK:
        MFrequency::Ref::frameRadialVelocity(inref, outref).getLSR(g1);
        MFrequency::Ref::frameDirection(inref, outref).getJ2000(*MVDIR1);
        g1 /= C::c;
        g2 = in.getValue();
        in = g2 * sqrt((1 - g1) / (1 + g1));
...
String MCFrequency::showState() {
  std::call_once(theirInitOnceFlag, doFillState);
  return MCBase::showState(MCFrequency::FromTo_p[0], MFrequency::N_Types, MCFrequency::N_Routes,
                           MCFrequency::ToRef_p);
}

void MCFrequency::doFillState() {
