...
// # Destructor
MCRadialVelocity::~MCRadialVelocity() { clearConvert(); }
...
void MCRadialVelocity::doConvert(MeasValue &in, MRBase &inref, MRBase &outref,
                                 const MConvertBase &mc) {
  doConvert(*(MVRadialVelocity *)&in, inref, outref, mc);
}
    ...
    switch (mc.getMethod(i)) {
      case LSRD_BARY: {
        *MVPOS1 = MVPosition(MeasTable::velocityLSR(0));
        MRadialVelocity::Ref::frameDirection(outref, inref).getJ2000(*MVDIR1);
      ...
      case BARY_LSRD: {
        *MVPOS1 = MVPosition(MeasTable::velocityLSR(0));
        MRadialVelocity::Ref::frameDirection(inref, outref).getJ2000(*MVDIR1);
        g1 = (*MVPOS1 * *MVDIR1) / C::c;
        g2 = in.getValue() / C::c;
        in = (g2 + g1) / (1 + g2 * g1) * C::c;
      } break;
      ...
      case BARY_GEO: {
        MRadialVelocity::Ref::frameEpoch(outref, inref).getTDB(tdbTime);
      ...
      case GEO_TOPO: {
        MRadialVelocity::Ref::frameEpoch(outref, inref).getLASTr(g1);
        MRadialVelocity::Ref::framePosition(outref, inref).getRadius(lengthE);
        MRadialVelocity::Ref::frameEpoch(outref, inref).getTDB(tdbTime);
        MRadialVelocity::Ref::framePosition(outref, inref).getLat(g3);
        g2 = MeasTable::diurnalAber(lengthE, tdbTime);
        *MVPOS1 = MVDirection(M_PI_2 + g1, 0.0);
        MVPOS1->readjust(g2 * cos(g3));
        MRadialVelocity::Ref::frameDirection(outref, inref).getApp(*MVDIR1);
        g1 = *MVPOS1 * *MVDIR1;
        g2 = in.getValue() / C::c;
        in = (g2 - g1) / (1 - g2 * g1) * C::c;
      } break;
      ...
      case GEO_BARY: {
        MRadialVelocity::Ref::frameEpoch(inref, outref).getTDB(tdbTime);
      ...
      case TOPO_GEO: {
        MRadialVelocity::Ref::frameEpoch(inref, outref).getLASTr(g1);
        MRadialVelocity::Ref::framePosition(inref, outref).getRadius(lengthE);
        MRadialVelocity::Ref::frameEpoch(inref, outref).getTDB(tdbTime);
        MRadialVelocity::Ref::framePosition(inref, outref).getLat(g3);
        g2 = MeasTable::diurnalAber(lengthE, tdbTime);
        *MVPOS1 = MVDirection(M_PI_2 + g1, 0.0);
        MVPOS1->readjust(g2 * cos(g3));
        MRadialVelocity::Ref::frameDirection(outref, inref).getApp(*MVDIR1);
...
String MCRadialVelocity::showState() {
  std::call_once(theirInitOnceFlag, doFillState);
  return MCBase::showState(MCRadialVelocity::FromTo_p[0], MRadialVelocity::N_Types,
                           MCRadialVelocity::N_Routes, MCRadialVelocity::ToRef_p);
}
