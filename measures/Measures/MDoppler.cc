...
// # Constructors
MDoppler::MDoppler() : MeasBase<MVDoppler, MDoppler::Ref>() {}
...
MDoppler::MDoppler(const Measure *dt) : MeasBase<MVDoppler, MDoppler::Ref>(dt) {}
...
// # Member functions
const String &MDoppler::tellMe() const { return MDoppler::showMe(); }
...
void MDoppler::assure(const Measure &in) {
  if (!dynamic_cast<const MDoppler *>(&in)) {
    throw(AipsError("Illegal Measure type argument: " + MDoppler::showMe()));
  }
}
...
const String &MDoppler::showType(MDoppler::Types tp) {
  static const String tname[MDoppler::N_Types] = {"RADIO", "OPTICAL", "RATIO", "TRUE", "GAMMA"};
...
void MDoppler::checkTypes() const { MDoppler::checkMyTypes(); }
...
String MDoppler::getRefString() const { return MDoppler::showType(ref.getType()); }

Quantity MDoppler::get(const Unit &un) const { return data.get(un); }
