...
// # Constructors
MFrequency::MFrequency() : MeasBase<MVFrequency, MFrequency::Ref>() {}
...
MFrequency::MFrequency(const Measure *dt) : MeasBase<MVFrequency, MFrequency::Ref>(dt) {}

MFrequency::MFrequency(const MeasValue *dt)
    : MeasBase<MVFrequency, MFrequency::Ref>(*(MVFrequency *)dt, MFrequency::DEFAULT) {}
...
const String &MFrequency::tellMe() const { return MFrequency::showMe(); }
...
void MFrequency::assure(const Measure &in) {
  if (!dynamic_cast<const MFrequency *>(&in)) {
    throw(AipsError("Illegal Measure type argument: " + MFrequency::showMe()));
  }
}
...
MFrequency::Types MFrequency::castType(uInt tp) {
  ...
  if ((tp & MFrequency::EXTRA) == 0) {
    ...
    AlwaysAssert((tp & ~MFrequency::EXTRA) < (MFrequency::N_Other - MFrequency::EXTRA), AipsError);
  }
...
const String &MFrequency::showType(MFrequency::Types tp) {
  static const String tname[MFrequency::N_Types] = {"REST", "LSRK",    "LSRD",   "BARY", "GEO",
                                                    "TOPO", "GALACTO", "LGROUP", "CMB"};
  static const String ename[MFrequency::N_Other - MFrequency::EXTRA] = {"Undefined"};
...
MFrequency::Types MFrequency::typeFromString(const String &in) {
  MFrequency::Types tp;
  ThrowIf(!getType(tp, in), in + " is not a recognized type identifier");
  return tp;
}
...
String MFrequency::getRefString() const { return MFrequency::showType(ref.getType()); }
...
Measure *MFrequency::clone() const { return (new MFrequency(*this)); }
