...
// # Constructors
MPosition::MPosition() : MeasBase<MVPosition, MPosition::Ref>() {}
...
MPosition::MPosition(const Quantity &dt, const Quantity &dt1, const Quantity &dt2)
    : MeasBase<MVPosition, MPosition::Ref>(MVPosition(dt, dt1, dt2), MPosition::DEFAULT) {}
...
MPosition::MPosition(const MPosition &other) : MeasBase<MVPosition, MPosition::Ref>(other) {}
...
const String &MPosition::tellMe() const { return MPosition::showMe(); }
...
void MPosition::assure(const Measure &in) {
  if (!dynamic_cast<const MPosition *>(&in)) {
    throw(AipsError("Illegal Measure type argument: " + MPosition::showMe()));
  }
}
...
void MPosition::checkTypes() const { MPosition::checkMyTypes(); }
...
const String &MPosition::showType(MPosition::Types tp) {
  static const String tname[MPosition::N_Types] = {"ITRF", "WGS84"};
...
Measure *MPosition::clone() const { return (new MPosition(*this)); }
