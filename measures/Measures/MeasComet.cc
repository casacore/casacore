...
// # Member functions
const String &MeasComet::getName() const { return name_p; }

const MVPosition &MeasComet::getTopo() const { return topo_p; }
...
Bool MeasComet::initMeas(const String &which, const Table *tabin) {
  ...
  if (!measured_p && measFlag_p) {
    ...
    if (!kws.isDefined("MJD0") || kws.asDouble("MJD0") < 10000 || !kws.isDefined("dMJD") ||
      ...
      if (!kws.isDefined("NAME")) os << "NAME is not defined.";
      os << LogIO::POST;
    }
    if (ok) {
      ...
      if (kws.isDefined("posrefsys")) {
        ...
        if (StringContains(prs, "J2000")) {
          ...
          os << LogIO::SEVERE
             << "Unrecognized position reference frame (posrefsys): " << kws.asString("posrefsys")
