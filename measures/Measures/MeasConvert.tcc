...
template <class M>
MeasConvert<M>::MeasConvert(const MeasConvert<M> &other) : MConvertBase(other) {
  init();
  copy(other);
}
...
template <class M>
MeasConvert<M>::MeasConvert(const M &ep) : unit(ep.unit) {
  init();
...
template <class M>
MeasConvert<M>::MeasConvert(const M &ep, const typename M::Ref &mr) : unit(ep.unit) {
  init();
  model = std::make_unique<M>(ep);
  outref = mr;
  create();
...
template <class M>
MeasConvert<M>::MeasConvert(const Measure &ep, const typename M::Ref &mr) : unit(ep.getUnit()) {
  init();
...
template <class M>
MeasConvert<M>::MeasConvert(const M &ep, typename M::Types mr) : unit(ep.unit) {
  init();
  model = std::make_unique<M>(ep);
...
template <class M>
MeasConvert<M>::MeasConvert(const Measure &ep, typename M::Types mr) : unit(ep.getUnit()) {
  init();
  model.reset(ep.clone());
  outref = typename M::Ref(mr);
  create();
...
template <class M>
MeasConvert<M>::MeasConvert(const typename M::Ref &mrin, const typename M::Ref &mr) {
  init();
...
template <class M>
MeasConvert<M>::MeasConvert(const typename M::Ref &mrin, typename M::Types mr) {
  init();
  model = std::make_unique<M>(typename M::MVType(), mrin);
  outref = typename M::Ref(mr);
  create();
...
template <class M>
MeasConvert<M>::MeasConvert(typename M::Types mrin, const typename M::Ref &mr) {
  init();
...
template <class M>
MeasConvert<M>::MeasConvert(typename M::Types mrin, typename M::Types mr) {
  init();
  model = std::make_unique<M>(typename M::MVType(), typename M::Ref(mrin));
  outref = typename M::Ref(mr);
  create();
...
template <class M>
MeasConvert<M>::MeasConvert(const Unit &inunit, const typename M::Ref &mrin, typename M::Types mr)
    : unit(inunit) {
  init();
  model = std::make_unique<M>(typename M::MVType(), mrin);
  outref = typename M::Ref(mr);
...
template <class M>
MeasConvert<M>::MeasConvert(const Unit &inunit, typename M::Types mrin, const typename M::Ref &mr)
    : unit(inunit) {
  init();
  model = std::make_unique<M>(typename M::MVType(), typename M::Ref(mrin));
  outref = mr;
...
template <class M>
MeasConvert<M>::MeasConvert(const Unit &inunit, typename M::Types mrin, typename M::Types mr)
    : unit(inunit) {
  init();
  model = std::make_unique<M>(typename M::MVType(), typename M::Ref(mrin));
  outref = typename M::Ref(mr);
...
void MeasConvert<M>::create() {
  ...
  if (model && model->getRefPtr()->offset()) {
    typename M::MVType *ptmp = (typename M::MVType *)(model->getRefPtr()->offset()->getData());
    ...
    typename M::Ref mrtmp(*(typename M::Ref *)(model->getRefPtr()->offset()->getRefPtr()));
    if (!mrtmp.empty()) {
      M mtmp(*ptmp, mrtmp);
  ...
  offout.reset();
  if (outref.offset()) {
    typename M::MVType *ptmp = (typename M::MVType *)(outref.offset()->getData());
...
template <class M>
const typename M::MVType &MeasConvert<M>::convert(const typename M::MVType &val) {
  *locres = val;
