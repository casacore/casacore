...
MeasFrame::MeasFrame(const Measure &meas1, const Measure &meas2, const Measure &meas3) {
  create();
  fill(&meas1);
  fill(&meas2);
  fill(&meas3);
...
MeasFrame::MeasFrame(details::CyclicPtr<FrameRep> new_rep) : rep(std::move(new_rep)) {}
...
void MeasFrame::set(const Measure &meas1) { fill(&meas1); }
...
void MeasFrame::set(const Measure &meas1, const Measure &meas2, const Measure &meas3) {
  fill(&meas1);
  fill(&meas2);
  fill(&meas3);
}
...
void MeasFrame::fill(const Measure *in) {
  ...
  if (in) {
    if (dynamic_cast<const MEpoch *>(in)) {
      ...
      throw(AipsError("Unknown MeasFrame Measure type " + in->tellMe()));
    }
  }
}
...
void MeasFrame::makeEpoch() { rep->mymcf->makeEpoch(*this); }

void MeasFrame::makePosition() { rep->mymcf->makePosition(*this); }

void MeasFrame::makeDirection() { rep->mymcf->makeDirection(*this); }

void MeasFrame::makeRadialVelocity() { rep->mymcf->makeRadialVelocity(*this); }

void MeasFrame::makeComet() { rep->mymcf->makeComet(); }
...
ostream &operator<<(ostream &os, MeasFrame &mf) {
  ...
  if (mf.rep && mf.rep->dirval) {
    if (mf.rep && (mf.rep->epval || mf.rep->posval)) os << endl << "       ";
    os << *(mf.rep->dirval);
    MVDirection tmp;
    if (mf.getJ2000(tmp)) {
      os << endl << "        (J2000 = " << tmp.getAngle("deg") << ")";
    }
  }
  if (mf.rep && mf.rep->radval) {
    if (mf.rep && (mf.rep->epval || mf.rep->posval || mf.rep->dirval)) {
      os << endl << "       ";
    }
    os << *(mf.rep->radval);
    if (mf.getLSR(tmp)) {
      tmp /= 1000.;
      os << endl << "        (LSR velocity = " << Quantity(tmp, "km/s") << ")";
    }
  }
  if (mf.rep && mf.rep->comval) {
    if (mf.rep && (mf.rep->epval || mf.rep->posval || mf.rep->dirval || mf.rep->radval)) {
      os << endl << "       ";
    }
    os << mf.rep->comval->getName() << " comet between MJD " << mf.rep->comval->getStart()
       << " and " << mf.rep->comval->getEnd();
  }
  return os;
}
...
MeasFrame MeasFrame::independentCopy() const {
  ...
  if (new_frame.rep->epval) new_frame.makeEpoch();
  if (new_frame.rep->posval) new_frame.makePosition();
  if (new_frame.rep->dirval) new_frame.makeDirection();
  if (new_frame.rep->radval) new_frame.makeRadialVelocity();
  return new_frame;
}
