    ...
    if (mjds.empty() || ut < mjds[0] || ut >= mjds[mjds.size() - 1]) {
      ...
      if (!msgDone) {
        ...
        if (date > now.modifiedJulianDay()) {
          ...
          os << LogIO::NORMAL3
             << "High precision Earth axis data is not yet available for requested JD " << date
             << LogIO::POST;
        } else {
          os << LogIO::NORMAL << "Requested JD " << date
...
void MeasIERS::initMeas() {
  static const String names[MeasIERS::N_Types] = {"MJD",  "x",     "y",    "dUT1", "LOD",
                                                  "dPsi", "dEps",  "Dx",   "Dy",   "DdUT1",
                                                  "DLOD", "DdPsi", "DdEps"};
  ...
  for (Int which = 0; which < N_Files; ++which) {
    if (!MeasIERS::getTable(tab, kws, row, rfp, vs, dt, N_Types, names, tp[which], tplc[which],
      ...
      os << LogIO::NORMAL1 << "Cannot read IERS (Earth axis data) table " << tp[which]
         << "\nCalculations will proceed with lower precision" << LogIO::POST;
    } else {
...
Bool MeasIERS::getTable(Table &table, TableRecord &kws, ROTableRow &row,
  ...
  LogIO os(LogOrigin("MeasIERS", "getTable(Vector<String>& optcols)", WHERE));
...
Bool MeasIERS::findTab(Table &tab, const Table *tabin, const String &rc, const String &dir,
  ...
  if (!tabin) {  // No table object given: search name
    ...
    if (!Table::isReadable(ldir + name)) {
      os << LogIO::WARN << "Requested data table " << name
