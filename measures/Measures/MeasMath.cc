...
void MeasMath::getFrame(FrameType i) {
  ...
  static FRFCT frameInfo[N_FrameType] = {&MeasFrame::epoch, &MeasFrame::position,
                                         &MeasFrame::direction, &MeasFrame::radialVelocity};
...
void MeasMath::applyPrecNutat(MVPosition &in) {
  if (MeasTable::useIAU2000()) {
    getInfo(TT);
    in *= MeasTable::frameBias00();
    in *= (RotMatrix((*PRECESIAU)(info_p[TT])) * RotMatrix((*NUTATIAU)(info_p[TT])));
  } else {
    getInfo(TDB);
    in *= (RotMatrix((*PRECESIAU)(info_p[TDB])) * RotMatrix((*NUTATIAU)(info_p[TDB])));
  }
}
...
void MeasMath::deapplyPrecNutat(MVPosition &in) {
  if (MeasTable::useIAU2000()) {
    getInfo(TT);
    in = MeasTable::frameBias00() * in;
    in = (RotMatrix((*PRECESIAU)(info_p[TT])) * RotMatrix((*NUTATIAU)(info_p[TT]))) * in;
  } else {
    getInfo(TDB);
    in = (RotMatrix((*PRECESIAU)(info_p[TDB])) * RotMatrix((*NUTATIAU)(info_p[TDB]))) * in;
  }
}
...
void MeasMath::applySolarPos(MVPosition &in, Bool doin) {
  ...
  g2 = MVPOS2 * MVPOS1;
  // Check if near sun
  if (!nearAbs(g2, 1.0, 1.0 - cos(MeasData::SunSemiDiameter() / lengthE))) {
    MVPOS1 -= g2 * MVPOS2;
    MVPOS1 *= (g1 / (1.0 - g2));
...
void MeasMath::applyGALtoJ2000(MVPosition &in) { in = MeasData::GALtoJ2000() * in; }

void MeasMath::deapplyGALtoJ2000(MVPosition &in) { in = MeasData::J2000toGAL() * in; }

void MeasMath::applyGALtoB1950(MVPosition &in) { in = MeasData::GALtoB1950() * in; }

void MeasMath::deapplyGALtoB1950(MVPosition &in) { in = MeasData::B1950toGAL() * in; }

void MeasMath::applyGALtoSUPERGAL(MVPosition &in) { in = MeasTable::galToSupergal() * in; }

void MeasMath::deapplyGALtoSUPERGAL(MVPosition &in) { in *= MeasTable::galToSupergal(); }

void MeasMath::applyICRStoJ2000(MVPosition &in) { in = MeasTable::ICRSToJ2000() * in; }
...
void MeasMath::applyMECLIPtoJMEAN(MVPosition &in) {
  getInfo(TDB);
  in = RotMatrix(Euler(MeasTable::fundArg(0)((info_p[TDB] - MeasData::MJD2000) / MeasData::JDCEN),
                       1, 0, 0)) *
       in;
}
...
void MeasMath::deapplyMECLIPtoJMEAN(MVPosition &in) {
  getInfo(TDB);
  in *= RotMatrix(
      Euler(MeasTable::fundArg(0)((info_p[TDB] - MeasData::MJD2000) / MeasData::JDCEN), 1, 0, 0));
}

void MeasMath::applyTECLIPtoJTRUE(MVPosition &in) {
  getInfo(TDB);
  in = RotMatrix(Euler(-Nutation(Nutation::STANDARD)(info_p[TDB])(2), 1, 0, 0)) * in;
}
...
void MeasMath::applyAPPtoTOPO(MVPosition &in, const Double len, Bool doin) {
  if (len != 0) {
    ...
    // Correction
    MVPOS1 = (ROTMAT1 * MVPosition(Quantity(info_p[RADIUS], "m"), info_p[LONG], info_p[LAT])) *
             (1.0 / len);
    rotateShift(in, -MVPOS1, APPLONG, APPLAT, doin);
  }
}
...
Bool MeasMath::getInfo(FrameInfo i, Bool ret) {
  ...
  static FRDINFO InfoDFrame[N_FrameDInfo] = {
      &MeasFrame::getTDB,      &MeasFrame::getLASTr,     &MeasFrame::getTT,
      &MeasFrame::getUT1,      &MeasFrame::getLong,      &MeasFrame::getLat,
      &MeasFrame::getRadius,   &MeasFrame::getLatGeo,    &MeasFrame::getJ2000Long,
      &MeasFrame::getJ2000Lat, &MeasFrame::getB1950Long, &MeasFrame::getB1950Lat,
      &MeasFrame::getAppLong,  &MeasFrame::getAppLat};

  static FRMVDINFO InfoMVDFrame[N_FrameMVDInfo] = {&MeasFrame::getJ2000, &MeasFrame::getB1950,
                                                   &MeasFrame::getApp};
...
void MeasMath::rotateShift(MVPosition &in, const MVPosition &shft, const FrameInfo lng,
  ...
  if (doin) {
    ...
    ROTMAT1 = RotMatrix(Euler(-M_PI_2 + info_p[lat], 2u, -info_p[lng], 3u));
    // Rotation towards correction
    ROTMAT1 = RotMatrix(Euler(-(ROTMAT1 * shft).getLong(), 3u)) * ROTMAT1;
    // Rotate over correction
    in = ((RotMatrix(Euler((ROTMAT1 * shft).getValue()(0), 2u)) * ROTMAT1) * in) * ROTMAT1;
  }
}
