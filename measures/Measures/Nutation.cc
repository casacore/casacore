...
Nutation::Nutation(const Nutation &other) { copy(other); }

Nutation::Nutation(NutationTypes type) : method_p(type), lres_p(0) { fill(); }
...
void Nutation::calcNut(Double time, Bool calcDer) {
  ...
  if (!nearAbs(time, checkEpoch_p, epsilon)) {
    ...
    switch (method_p) {
      ...
      default:
        ...
        ///      cout <<"nval_p="<< nval_p[0]<<' '<<nval_p[1] << ' '<<nval_p[2]<<'
        ///      '<<nval_p[3]<<endl;
        nval_p[2] += dEps;
        nval_p[1] += dPsi;
        break;
  ...
  if ((renew && calcDer) || (!renew && calcDer && checkEpoch_p != checkDerEpoch_p) ||
      (!renew && !calcDer && t != checkEpoch_p && checkEpoch_p != checkDerEpoch_p)) {
    t = checkEpoch_p;
    checkDerEpoch_p = t;
    switch (method_p) {
      case B1950:
    ...
    switch (method_p) {
      ...
      default:
        ...
        ///      cout <<"dval_p="<< dval_p[0]<<' '<<dval_p[1] << ' '<<dval_p[2]<<'
        ///      '<<dval_p[3]<<endl;
        break;
    }
    dval_p[1] = -dval_p[1] / MeasData::JDCEN;
    dval_p[2] = (-dval_p[0] - dval_p[2]) / MeasData::JDCEN;
    deqeq_p = -dval_p[1] * cos(nval_p[2]) + nval_p[1] * sin(nval_p[2]) * dval_p[2];
    // Complimentary terms equation of equinoxes
    switch (method_p) {
      case IAU2000B:
        for (uInt i = 0; i < 14; i++) {
