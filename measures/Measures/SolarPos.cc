...
SolarPos::SolarPos(const SolarPos &other) { copy(other); }

SolarPos::SolarPos(SolarPosTypes type) : method(type), lres(0) {
  std::call_once(initialize_once_flag, initialize_statics);
}
      ...
      default:
        if (AipsrcValue<Bool>::get(SolarPos::usejpl_reg)) {
          ...
          for (i = 0; i < 189; i++) {
            ...
            deval[1] += mulPosEarthXY[7] * sinpos2 + mulPosEarthXY[3] * cospos2 * ddtmp;
            mulPosEarthXY += 8;
          }
