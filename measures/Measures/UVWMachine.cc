...
Vector<MVPosition> UVWMachine::operator()(const Vector<MVPosition> &uv) const {
  if (nop_p) return uv;
...
Vector<Vector<Double>> UVWMachine::operator()(const Vector<Vector<Double>> &uv) const {
  if (nop_p) return uv;
...
// # Member functions
void UVWMachine::reCalculate() { init(); }

const MDirection &UVWMachine::phaseCenter() const { return out_p; }

const RotMatrix &UVWMachine::rotationUVW() const { return uvproj_p; }

const MVPosition &UVWMachine::rotationPhase() const { return phrot_p; }
...
void UVWMachine::convertUVW(Vector<Double> &phase, Vector<Vector<Double>> &uv) const {
  phase = 0;
  if (!nop_p) {
    MVPosition tmp;
...
void UVWMachine::convertUVW(Vector<Double> &phase, Vector<MVPosition> &uv) const {
  phase.resize(uv.nelements());
  phase = 0;
  if (!nop_p) {
    for (uInt i = 0; i < uv.nelements(); i++) {
...
void UVWMachine::init() {
  ...
  if (!nop_p) {
    ...
    // x-axis (lat-90).
    rot1_p = RotMatrix(
        Euler(-(M_PI_2 - in_p.getValue().get()(0)), 3, in_p.getValue().get()(1) - M_PI_2, 1));
    // Convert the input axes directions to the output reference frame, and
    // deduce a rotation matrix from these
    rot2_p.set(conv_p(mVx).getValue().getValue(), conv_p(mVy).getValue().getValue(),
    ...
    rot3_p = RotMatrix(
        Euler(M_PI_2 - out_p.getValue().get()(1), 1, -(out_p.getValue().get()(0) - M_PI_2), 3));
    ...
    if (proj_p) {
      RotMatrix x(Euler(-(M_PI_2 - out_p.getValue().get()(1)), 1,
                        out_p.getValue().get()(0) - in_p.getValue().get()(0), 3,
    ...
    phrot_p = rot3_p * (MVPosition(out_p.getValue()) - MVPosition(outin_p.getValue()));
