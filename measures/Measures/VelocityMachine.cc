...
VelocityMachine::VelocityMachine(const MFrequency::Ref &freqRef, const Unit &freqUnits,
                                 const MVFrequency &restFreq, const MDoppler::Ref &velRef,
                                 const Unit &velUnits)
    : fref_p(freqRef), fun_p(freqUnits), rest_p(restFreq), vref_p(velRef), vun_p(velUnits) {
...
VelocityMachine::VelocityMachine(const MFrequency::Ref &freqRef, const Unit &freqUnits,
                                 const MVFrequency &restFreq, const MDoppler::Ref &velRef,
                                 const Unit &velUnits, const MeasFrame &frame)
    : fref_p(freqRef), fun_p(freqUnits), rest_p(restFreq), vref_p(velRef), vun_p(velUnits) {
  fref_p.set(frame);
  vfm_p = (MFrequency::Types)fref_p.getType();
  init();
...
const Quantum<Double> &VelocityMachine::operator()(const MVFrequency &in) {
  resv_p.setValue(cvvo_p(cvfv_p(in).toDoppler(rest_p).getValue()).getValue().get().getValue() /
                  vfac_p);
  return resv_p;
}

const Quantum<Double> &VelocityMachine::operator()(const MVDoppler &in) {
  resf_p.setValue(MVFrequency(cvvf_p(MFrequency::fromDoppler(cvov_p(in), rest_p, vfm_p).getValue())
                                  .getValue()
                                  .getValue())
                      .get(fun_p)
                      .getValue());
...
const Quantum<Double> &VelocityMachine::operator()(const Quantum<Double> &in) {
  ...
  if (in.getFullUnit().getValue() == Velocity || in.getFullUnit().getValue() == UnitVal::NODIM) {
...
const Quantum<Double> &VelocityMachine::makeVelocity(Double in) {
  ...
  ThrowIf(rfreqValue == 0, "Rest frequency is 0 so cannot convert to velocity");
  ThrowIf(rfreqValue < 0, "Rest frequency is " + std::to_string(rest_p) +
...
const Quantum<Vector<Double>> &VelocityMachine::makeVelocity(const Vector<Double> &in) {
  uInt n = in.nelements();
  ...
  for (uInt i = 0; i < n; ++i) {
    ...
    t *= t;
    vresv_p.getValue()[i] =
        cvvo_p(MVDoppler((1 - t) / (1 + t))).getValue().getValue() * C::c / vfac_p;
  }
  return vresv_p;
}

const Quantum<Vector<Double>> &VelocityMachine::makeFrequency(const Vector<Double> &in) {
  uInt n = in.nelements();
  vresf_p.getValue().resize(n);
  for (uInt i = 0; i < n; i++) {
    vresf_p.getValue()(i) =
        MVFrequency(cvvf_p(MFrequency::fromDoppler(cvov_p(in(i)), rest_p, vfm_p).getValue())
                        .getValue()
                        .getValue())
            .get(fun_p)
            .getValue();
...
void VelocityMachine::reCalculate() { init(); }
...
const MFrequency::Ref &VelocityMachine::getFrequencyReference() const { return fref_p; }

const Unit &VelocityMachine::getFrequencyUnits() const { return fun_p; }

const MDoppler::Ref &VelocityMachine::getDopplerReference() const { return vref_p; }

const Unit &VelocityMachine::getDopplerUnits() const { return vun_p; }

const MVFrequency &VelocityMachine::getRestFrequency() const { return rest_p; }

const MFrequency::Types &VelocityMachine::getConversionReference() const { return vfm_p; }
