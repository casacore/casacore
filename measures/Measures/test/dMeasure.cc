  ...
  try {
    ...
    while (True) {
      ...
      while (True) {
        ...
        cout << epoch << " coordinates: " << ncoord.getAngle("deg") << endl;
        MDirection npole(toward(pole));
        cout << "Rotation angle: " << -ncoord.getValue().positionAngle(npole.getValue(), "deg")
             << endl;
        cout << "Apparent" << " coordinates: " << from(ncoord).getAngle("deg") << endl;
        MDirection apole(MVDirection(0., 0., 1.), appref);
        MDirection mpole(from(pole));
        cout << "Rotation angle: " << from(ncoord).getValue().positionAngle(mpole.getValue(), "deg")
             << endl;
      };
    };
