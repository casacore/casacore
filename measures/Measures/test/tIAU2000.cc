...
int main() {
  try {
    ...
    IAUR(pn00a)(MJD0, tMJD, padpsi, padeps, paepsa, &parb[0][0], &parp[0][0], &parpb[0][0],
                &parn[0][0], &parpn[0][0]);
    IAUR(pn00b)(MJD0, tMJD, pbdpsi, pbdeps, pbepsa, &pbrb[0][0], &pbrp[0][0], &pbrpb[0][0],
                &pbrn[0][0], &pbrpn[0][0]);
    ...
    cout << "Casacore precession corrections: "
         ...
         << MeasTable::precRate00(1) * (tMJD - MeasData::MJD2000) / MeasData::JDCEN << endl;
    ...
    cout.precision(9);
    cout.setf(ios::fixed);
    cout << "Difference (arcsec) SOFA 2000A - SOFA 2000B:         " << checkRot(narm, nbrm) << endl;
    cout << "Difference (arcsec) Casacore 2000A - Casacore 2000B: " << checkRot(cnarm, cnbrm)
         << endl;
    cout << "(The above two differences are 0.512mas (note next note))" << endl;
    cout << "Difference (arcsec) SOFA 2000A - Casacore 2000A:   " << checkRot(narm, cnarm) << endl;
    cout << "Difference (arcsec) SOFA 2000B - Casacore 2000B:   " << checkRot(nbrm, cnbrm) << endl;
    cout << "(This should be 0.422 uas, due to different definition of\n"
            "\tthe fundamental arguments)"
         << endl;
    cout << "Equation of equinoxes (IAU2000A)(SOFA, Casacore, diff): " << endl;
    cout << "    " << IAUR(ee00a)(MJD0, tMJD) << ", " << n00a.eqox(tMJD) << ", "
    ...
    Double y, tta, ttb, uta, utb;
    {
      ...
      cout << "Difference (arcsec) SOFA CEO and Equinox based in 1935:   " << checkRot(rm1, rm2)
           << endl;
      cout << "Difference (arcsec) SOFA CEO based and old J2000 in 1935: " << checkRot(rm1, rm3)
           << endl;
      cout << "(The above should be 0.119 uas and 65.034 mas respectively)" << endl;
      SEPAR();
    }

    {
      ...
      cout << mcv2() << endl;
      cout << "Difference: "
           << (mcv2().getValue().getValue() - mdcv.getValue().getValue()) * 200000. << endl;
      SEPAR();
    }

    {
      ...
      cout << "s'   (Sofa, Casacore, diff): " << sp << ", " << MeasTable::sprime00(ttb) << ", "
           << sp - MeasTable::sprime00(ttb) << endl;
      cout << "ERA  (Sofa, Casacore, diff): " << era << ", " << MeasTable::ERA00(utb) << ", "
           << era - MeasTable::ERA00(utb) << endl;
      cout << "GMST (Sofa, Casacore, diff): " << gmst << ", "
           << fmod((ttb + 6713.) * C::_2pi + MeasTable::GMST00(utb, ttb), C::_2pi) << ", "
           << gmst - fmod((ttb + 6713.) * C::_2pi + MeasTable::GMST00(utb, ttb), C::_2pi) << endl;
      cout << "GMST82 (GMST82, GMST00, diff): "
           << utb + MeasTable::GMST0(utb) / MeasData::SECinDAY + 6713. << ", "
           << ttb + MeasTable::GMST00(utb, ttb) / C::_2pi + 6713. << ", "
           << utb + MeasTable::GMST0(utb) / MeasData::SECinDAY - ttb -
                  MeasTable::GMST00(utb, ttb) / C::_2pi
           << endl;
      cout << "EqEqCT00 (Sofa, Casacore, diff): " << eect << ", " << nuta.eqoxCT(ttb) << ", "
