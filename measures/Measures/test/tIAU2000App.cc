...
Double angularSepMas(const Double a[3], const Double b[3]) {
  const Double cross[3] = {a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2],
                           a[0] * b[1] - a[1] * b[0]};
  const Double sinang = std::sqrt(cross[0] * cross[0] + cross[1] * cross[1] + cross[2] * cross[2]);
...
void checkEpoch(const char* label, const Double mjdUtc) {
  ...
  const MPosition pos(
      MVPosition(Quantity(obsHeight, "m"), Quantity(obsLon, "rad"), Quantity(obsLat, "rad")),
      MPosition::WGS84);
  ...
  const MVDirection casaApp =
      MDirection::Convert(dirJ2000, MDirection::Ref(MDirection::APP, frame))().getValue();
  disableIAU2000A();
  ...
  const Double p[3] = {std::cos(dirLat) * std::cos(dirLon), std::cos(dirLat) * std::sin(dirLon),
                       std::sin(dirLat)};
  Double pTrue[3];
  ...
  const Double v[3] = {pvb[1][0] / cAuPerDay, pvb[1][1] / cAuPerDay, pvb[1][2] / cAuPerDay};
  Double vTrue[3];
  matVec(bpn00a, v, vTrue);

  const Double s = std::sqrt(pvh[0][0] * pvh[0][0] + pvh[0][1] * pvh[0][1] + pvh[0][2] * pvh[0][2]);
  ...
  const Double casaVec[3] = {std::cos(casaApp.getLat()) * std::cos(casaApp.getLong()),
                             std::cos(casaApp.getLat()) * std::sin(casaApp.getLong()),
                             std::sin(casaApp.getLat())};
  const Double sepMas = angularSepMas(casaVec, pApp);

  cout << label << " separation to SOFA/ERFA reference: " << sepMas << " mas" << endl;
