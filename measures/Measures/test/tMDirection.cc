...
Bool testShiftAngle() {
  ...
  AlwaysAssert(abs((angle.getValue("deg")[0] - rav) / rav) < 1e-6, AipsError);
  ...
  angle = x.getAngle();
  AlwaysAssert(abs((angle.getValue("deg")[0] - exp) / exp) < 1e-6, AipsError);
  exp = (decv);
...
int main() {
  try {
    ...
    cout << "tMDirection failed: " << x.what() << endl;
    return 1;
  }
  return 0;
}
