...
int main() {
  try {
    ...
    cout << "10 deg, 20 deg:      " << MVDirection(Quantity(10., "deg"), Quantity(20., "deg"))
         << endl;
    ...
    cout << "6 mm, 10 deg, 20 deg: "
         << MVPosition(Quantity(6., "mm"), Quantity(10., "deg"), Quantity(20., "deg")) << endl;
  ...
  try {
    ...
    cout << "Rotate (10,20 deg) over 0,0,30 deg: " << (rot10 * dc10).getAngle("deg") << endl;
  } catch (std::exception& x) {
    cout << x.what() << endl;
  }
  ...
  try {
    ...
    cout << "B1950 precession MJD 45700 using derivative from 45701: "
         << pc1(45701.).getAngle("''") - pc1.derivative(45701).getAngle("''") << endl;
    cout << "with rotation matrix: " << RotMatrix(pc1(45700)) << endl;
    cout << "J2000 precession J1984.5: " << pc2(45883.125).getAngle("''") << endl;
    ...
    cout << "J2000 nutation J1984.5 derivative from +0.04 day: "
         << nt1(45883.165).getAngle("\"") - hun4 * nt1.derivative(45883.165).getAngle("\"") << endl;
    cout << "with rotation matrix: " << RotMatrix(nt1(45883.125)) << endl;
    cout << "with rotation matrix combined 45882.5: "
         << RotMatrix(nt1(45882)) * RotMatrix(pc2(45882))
         << "or:" << RotMatrix(pc2(45882)) * RotMatrix(nt1(45882)) << endl;
    cout << " equation of equinoxes 45882.5: " << nt1.getEqoxAngle(45882, "''") << endl;
    cout << " equation at 45882.5 from derivative at 45882.7: "
         << nt1.getEqoxAngle(45882.2, "''") -
                0.2 * Quantity(nt1.derivativeEqox(45882.2) / C::arcsec, "''")
         << endl;
    cout << " equation at 45882.5 from derivative at 45882.54: "
         << nt1.getEqoxAngle(45882.04, "''") -
    ...
    for (eq = 40632.; eq < 40678.; eq++) {
      cout << eq + 0.5 << ": " << nt2(eq).getAngle("\"") << (nt2(eq)(0) + nt2(eq)(2)) / C::arcsec
           << " " << nt2.getEqoxAngle(eq, "''") / Quantity(15., "''/s") << endl;
    }
    cout << "J2000 nutation 50449.5: " << nt1(50449.).getAngle("''")
    ...
    cout << (ab1(45837.1) - 0.1 * ab1.derivative(45837.1)) * (C::c / factor) << endl;

    cout << "J2000 aberration for 45837 from derivative at +0.04: " << endl;
    cout << (ab1(45837.04) - 0.04 * ab1.derivative(45837.04)) * (C::c / factor) << endl;
    ...
    cout << (ab2(44238.1) - 0.1 * ab2.derivative(44238.1)) * (C::c / factor) << endl;

    cout << "B1950 aberration for 44238 from derivative at +0.04: " << endl;
    cout << (ab2(44238.04) - 0.04 * ab2.derivative(44238.04)) * (C::c / factor) << endl;

    cout << "J2000 aberration: " << endl;
    for (eq = 45837.; eq < 45884.; eq++) {
      cout << eq + 0.5 << ": " << ab1(eq) * (C::c / factor) << endl;
    }
    ...
    cout << "J2000 geocentric Sun at 45837: " << sp1(45837.) * facAU << endl;
  ...
  try {
    ...
    cout << "Separation between (0.1, 0.2) and (0.1, 0.20001): " << dc1.separation(dc2) << endl;
    cout << "Separation between (0.1, 0.2) and (0.1000001, 0.2): " << dc1.separation(dc3) << endl;
    cout << "Separation between (0.1, 0.2) and (0.1000001, 0.2): " << dc1.separation(dc3, "arcsec")
         << endl;
