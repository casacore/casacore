...
int main() {
  try {
    ...
    mf.set(MDirection(Quantity(10, "deg"), Quantity(80, "deg"), MDirection::HADEC));
    MVuvw mvb0(100, 10, 0);
    cout << "uvw: " << mvb0 << endl;
    ...
    cout << "Converted " << mb0 << endl << " to " << mbref1 << endl << " as " << bconv() << endl;

    Muvw::Convert bconvb(mbref1, mbref0);
    if (allNearAbs(mb0.getValue().getValue(), bconvb(bconv()).getValue().getValue(), 1e-8)) {
      cout << "Back      " << mb0 << " : ok" << endl;
    } else {
      cout << "Back      " << mb0 << " : not ok" << endl << " as " << bconvb(bconv()) << endl;
    };
