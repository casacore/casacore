...
int main() {
  try {
    ...
    MPosition mLocation = MPosition(MVPosition(-1601162, -5042003, 3554915), MPosition::ITRF);
...

    {
      ...
      cout << "                 : " << mImage.getAngle("deg") << endl;
      MDirection::Ref oref(MDirection::J2000);
      MVDirection mvo(MVPosition(mIm1.getValue()) + MVPosition(0.01, 0.01, 0.01));
      mvo.adjust();
  ...
  try {
    ...
    // Output direction
    MDirection::Ref oref(MDirection::J2000);
    MDirection odir((MVDirection(Quantity(30, "deg"), Quantity(37, "deg"))), oref);
    // Main direction
