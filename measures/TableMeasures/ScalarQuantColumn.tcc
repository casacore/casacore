  ...
  if (tqDesc->isUnitVariable()) {
    ...
    if (units.nelements() > 0) {
      if (units.nelements() > 1) {
        throw(AipsError("ScalarQuantColumn is used for column " + columnName +
                        " but its description has >1 units"));
      }
      itsUnit = units(0);
    }
  }
...
template <class T>
void ScalarQuantColumn<T>::attach(const Table& tab, const String& columnName) {
  reference(ScalarQuantColumn<T>(tab, columnName));
}

template <class T>
void ScalarQuantColumn<T>::attach(const Table& tab, const String& columnName, const Unit& u) {
...
template <class T>
void ScalarQuantColumn<T>::get(rownr_t rownr, Quantum<T>& q, const Unit& u) const {
  getData(rownr, q);
  q.convert(u);
}

template <class T>
void ScalarQuantColumn<T>::get(rownr_t rownr, Quantum<T>& q, const Quantum<T>& other) const {
  getData(rownr, q);
  q.convert(other);
}
...
template <class T>
Quantum<T> ScalarQuantColumn<T>::operator()(rownr_t rownr, const Unit& u) const {
  Quantum<T> q;
  get(rownr, q, u);
  return q;
}
...
template <class T>
Quantum<T> ScalarQuantColumn<T>::operator()(rownr_t rownr, const Quantum<T>& other) const {
  Quantum<T> q;
  get(rownr, q, other);
  return q;
}
