...
TableMeasDescBase::TableMeasDescBase(const TableMeasValueDesc& value, const TableMeasRefDesc& ref)
    : itsValue(value), itsRef(ref) {}
...
TableMeasDescBase* TableMeasDescBase::reconstruct(const Table& tab, const String& columnName) {
  ...
  if (fnr >= 0) {
    ...
    throw(
        AipsError("TableMeasDescBase::reconstruct; MEASINFO record not "
                  "found for column " +
                  columnName));
  }
...
void TableMeasDescBase::setMeasUnits(const Measure& meas, const Vector<Quantum<Double>>& val,
  ...
  if (units.nelements() > val.nelements()) {
    throw(
        AipsError("TableMeasDescBase::setMeasUnits; Unit vector"
                  " for column " +
  ...
  for (uInt i = 0; i < val.nelements(); i++) {
    if (i >= units.nelements() || units(i).empty()) {
      ...
      if (!(units(i) == val(i).getUnit())) {
        throw(AipsError("TableMeasDescBase::setMeasUnits; invalid unit " + units(i).getName() +
...
void TableMeasDescBase::resetUnits(const Vector<Unit>& units) {
  if (units.nelements() > itsUnits.nelements()) {
    throw(
        AipsError("TableMeasDescBase::resetUnits: Unit vector"
                  " for column " +
                  columnName() + " is too long"));
  }
