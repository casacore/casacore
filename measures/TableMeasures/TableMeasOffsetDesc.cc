...
TableMeasOffsetDesc::~TableMeasOffsetDesc() { delete itsTMDesc; }
...
TableMeasOffsetDesc& TableMeasOffsetDesc::operator=(const TableMeasOffsetDesc& that) {
  if (this != &that) {
    delete itsTMDesc;
...
const Measure& TableMeasOffsetDesc::getOffset() const {
  if (isVariable()) {
    throw(
        AipsError("TableMeasOffsetDesc::getOffset() "
...
void TableMeasOffsetDesc::writeKeys(TableRecord& measInfo, const String& prefix) {
...
void TableMeasOffsetDesc::resetOffset(const Measure& offset) {
  if (isVariable()) {
    throw(
        AipsError("tableMeasOffsetDesc::resetOffset cannot be done;"
