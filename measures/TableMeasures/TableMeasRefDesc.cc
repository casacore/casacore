...
TableMeasRefDesc::~TableMeasRefDesc() { delete itsOffset; }
...
uInt TableMeasRefDesc::fillMap(Block<Int>& f2t, const Vector<uInt>& codesf,
  ...
  for (uInt i = 0; i < typesf.size(); i++) {
    ...
    if (inx >= 0) {
      ...
      if (maxnr < 0) {
        LogIO os;
        os << LogIO::WARN << "TableMeasRefDesc warning: refcode " << typesf[i]
...
void TableMeasRefDesc::writeKeys(TableRecord& measInfo, const TableMeasDescBase& measDesc) {
...
void TableMeasRefDesc::checkColumn(const TableDesc& td) {
  if (!td.isColumn(itsColumn)) {
    throw(AipsError("TableMeasRefDesc::checkColumn; No such column: " + itsColumn));
  } else {
    if (td.columnDesc(itsColumn).dataType() != TpString) {
      if (td.columnDesc(itsColumn).dataType() != TpInt) {
        throw AipsError(
            "TableMeasRefDesc::checkColumn; Reference column's "
...
void TableMeasRefDesc::resetOffset(const Measure& offset) {
  ...
  if (isOffsetVariable()) {
    throw(
        AipsError("tableMeasRefDesc::resetOffset cannot be done;"
