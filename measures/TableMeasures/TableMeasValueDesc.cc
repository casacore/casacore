...
TableMeasValueDesc::TableMeasValueDesc(const TableDesc& td, const String& columnName)
...
TableMeasValueDesc& TableMeasValueDesc::operator=(const TableMeasValueDesc& that) {
  if (this != &that) {
    itsColumn = that.itsColumn;
  }
  return *this;
...
void TableMeasValueDesc::writeKeys(TableRecord& columnKeyset, const TableRecord& measInfo) {
...
void TableMeasValueDesc::checkColumn(const TableDesc& td) const {
  if (!td.isColumn(itsColumn)) {
    throw(AipsError("TableMeasValueDesc::checkColumn; No such column: " + itsColumn));
  } else if (td.columnDesc(itsColumn).dataType() != TpDouble) {
    throw(
        AipsError("TableMeasValueDesc::checkColumn; Column's type "
                  "must be Double: " +
                  itsColumn));
  }
}
