...
int main(int argc, const char* argv[]) {
  ...
  try {
    ...
    ScalarColumnDesc<String> scdStr("varUnitsColumn", "Units columns for column ScaQuantComplex");

    // Also create ArrayQuantum columns for the test
    ArrayColumnDesc<Double> acdQD("ArrQuantDouble", "A Quantum<double> array column");
    ArrayColumnDesc<Double> acdQD3("ArrQuantDoubleNonVar", "A Quantum<double> array column");
    ArrayColumnDesc<Double> acdQD4("ArrQuantDoubleNonVar2",
                                   "A Quantum<double> array column with 2 units");
    ArrayColumnDesc<Double> acdQD2("ArrQuantScaUnits", "A Quantum<double> array column");
    ArrayColumnDesc<String> acdStr("varArrUnitsColumn", "String column for array of units");
    ...
    if (doExcep) {
      ...
      try {
        ...
        ScalarColumnDesc<Int> eucol("testvarcolumn", "variable units column with incorrect type");
        td.addColumn(eucol);
    ...
    if (tqdSQD.isUnitVariable()) {
      cout << "Quantum column " + tqdSQD.columnName() << " has variable units.\n";
    ...
    AlwaysAssertExit(TableQuantumDesc::hasQuanta(TableColumn(qtab, "ScaQuantDouble")));
    AlwaysAssertExit(TableQuantumDesc::hasQuanta(TableColumn(qtab, "ScaQuantComplex")));
    AlwaysAssertExit(!TableQuantumDesc::hasQuanta(TableColumn(qtab, "varUnitsColumn")));
    AlwaysAssertExit(TableQuantumDesc::hasQuanta(TableColumn(qtab, "ArrQuantDouble")));
    AlwaysAssertExit(TableQuantumDesc::hasQuanta(TableColumn(qtab, "ArrQuantDoubleNonVar")));
    AlwaysAssertExit(TableQuantumDesc::hasQuanta(TableColumn(qtab, "ArrQuantDoubleNonVar2")));
    AlwaysAssertExit(TableQuantumDesc::hasQuanta(TableColumn(qtab, "ArrQuantScaUnits")));
    AlwaysAssertExit(!TableQuantumDesc::hasQuanta(TableColumn(qtab, "varArrUnitsColumn")));
    AlwaysAssertExit(!TableQuantumDesc::hasQuanta(TableColumn(qtab, "varArrScaUnitsColumn")));
    AlwaysAssertExit(!TableQuantumDesc::hasQuanta(TableColumn(qtab, "BogusQuantCol")));
    {
    ...
    }
    {
      ...
      Quantum<Double> q;
      for (i = 0; i < qtab.nrow(); i++) {
        cout << "Quantum arcmin " << i << ": " << rosqCol(i, Unit("arcmin")) << endl;
      }
      ...
      for (i = 0; i < qtab.nrow(); i++) {
        cout << "Quantum arcmin " << i << ": " << rosqCol(i, "arcmin") << endl;
      }
    ...
    }
    {
      ...
      if (sqCol.isUnitVariable()) {
        ...
        cout << "getUnits() should produce an empty string: " << sqCol.getUnits() << endl;
      } else {
    ...
    }
    {
      ...
      uInt i;
      for (i = 0; i < qtab.nrow(); i++) {
        cout << "Complex quantum (var unit) " << i << ": " << rosqCol(i) << endl;
      }
      Quantum<Complex> q(1., "m/s");
      for (i = 0; i < qtab.nrow(); i++) {
        // this get the units converted to the units in q
        cout << "Complex quantum (var unit) " << i << ": " << rosqCol(i, q) << endl;
      }
    }
    ...
    }
    {
      ...
      for (uInt i = 0; i < 5; ++i) {
        AlwaysAssert(near(w->getValue()[i], frac * v->getValue()[i]), AipsError);
      }
      ...
      for (uInt i = 0; i < 5; ++i) {
        AlwaysAssert(near(y->getValue()[i], frac * x->getValue()[i]), AipsError);
      }
    }
