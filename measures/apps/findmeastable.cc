...
int main(int argc, char* argv[]) {
  ...
  try {
    if (tableName == "help" || tableName == "-h" || tableName == "--help") {
      cerr << endl;
      cerr << "This program shows the location where a Measures table is found." << endl;
    ...
    Int N = 0;
    String rfn[1];
    if (!MeasIERS::getTable(table, kws, row, rfp, vs, dt, N, rfn, tableName,
                            "measures." + tableNameLC + ".directory", "")) {
      cerr << tableName << " measures table could not be found" << endl;
      return 1;
    }
    cout << "Measures table " << tableName << " found as " << table.tableName() << endl;
