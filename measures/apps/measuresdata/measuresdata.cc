...
// IERSpredict
const String IERSpredictFormat = String("") + "i2 i2 i2 x f8 x a1 x f9 f9 x f9 f9 x2 " +
...
// IGRF
const String IGRFFormat = String("") + "a1 i3 i3 f7 f7 f7 f7 f7 f7 f7 f7 f7 f7 f7" +
...
// TAI_UTC
const columnDescr TAI_UTCCol[] = {{"MJD", "d", columnDescr::CTD, 4},
...
const columnDescr IERSeop97Col[] = {
    {"MJD", "d", columnDescr::CTD, 4},         {"x", "arcsec", columnDescr::CTD, 5},
    {"Dx", "arcsec", columnDescr::CTD, 13},    {"y", "arcsec", columnDescr::CTD, 6},
    {"Dy", "arcsec", columnDescr::CTD, 14},    {"dUT1", "s", columnDescr::CTD, 7},
    {"DdUT1", "s", columnDescr::CTD, 15},      {"LOD", "s", columnDescr::CTD, 12},
    {"DLOD", "s", columnDescr::CTD, 20},       {"dPsi", "arcsec", columnDescr::CTD, 8},
    {"DdPsi", "arcsec", columnDescr::CTD, 16}, {"dEps", "arcsec", columnDescr::CTD, 9},
    {"DdEps", "arcsec", columnDescr::CTD, 17}, {"", "", columnDescr::N_ColTypes, 0}};

// IERSeop2000
const columnDescr IERSeop2000Col[] = {
    {"MJD", "d", columnDescr::CTD, 3},       {"x", "arcsec", columnDescr::CTD, 4},
    {"Dx", "arcsec", columnDescr::CTD, 10},  {"y", "arcsec", columnDescr::CTD, 5},
    {"Dy", "arcsec", columnDescr::CTD, 11},  {"dUT1", "s", columnDescr::CTD, 6},
    {"DdUT1", "s", columnDescr::CTD, 12},    {"LOD", "s", columnDescr::CTD, 7},
    {"DLOD", "s", columnDescr::CTD, 13},     {"dX", "arcsec", columnDescr::CTD, 8},
    {"DdX", "arcsec", columnDescr::CTD, 14}, {"dY", "arcsec", columnDescr::CTD, 9},
    {"DdY", "arcsec", columnDescr::CTD, 15}, {"", "", columnDescr::N_ColTypes, 0}};

// IERSpredict
const columnDescr IERSpredictCol[] = {
    {"MJD", "d", columnDescr::CTD, 3},         {"x", "arcsec", columnDescr::CTD, 5},
    {"Dx", "arcsec", columnDescr::CTD, 6},     {"y", "arcsec", columnDescr::CTD, 7},
    {"Dy", "arcsec", columnDescr::CTD, 8},     {"dUT1", "s", columnDescr::CTD, 10},
    {"DdUT1", "s", columnDescr::CTD, 11},      {"LOD", "s", columnDescr::CTD, 12},
    {"DLOD", "s", columnDescr::CTD, 13},       {"dPsi", "arcsec", columnDescr::CTD, 15},
    {"DdPsi", "arcsec", columnDescr::CTD, 16}, {"dEps", "arcsec", columnDescr::CTD, 17},
    {"DdEps", "arcsec", columnDescr::CTD, 18}, {"", "", columnDescr::N_ColTypes, 0}};
...
const columnDescr IERSpredict2000Col[] = {
    ...
    {"Dy", "arcsec", columnDescr::CTD, 8},   {"dUT1", "s", columnDescr::CTD, 10},
    {"DdUT1", "s", columnDescr::CTD, 11},    {"LOD", "s", columnDescr::CTD, 12},
    {"DLOD", "s", columnDescr::CTD, 13},     {"dX", "arcsec", columnDescr::CTD, 15},
    {"DdX", "arcsec", columnDescr::CTD, 16}, {"dY", "arcsec", columnDescr::CTD, 17},
    {"DdY", "arcsec", columnDescr::CTD, 18}, {"", "", columnDescr::N_ColTypes, 0}};

// IGRF
const columnDescr IGRFCol[] = {{"MJD", "d", columnDescr::CTD, 0},
                               {"COEF", "nT/km", columnDescr::CTAD, 0},
                               {"dCOEF", "nT/km/a", columnDescr::CTAD, 0},
                               {"", "", columnDescr::N_ColTypes, 0}};

// DE200
const columnDescr DE200Col[] = {{"MJD", "d", columnDescr::CTD, 0},
                                {"x", "", columnDescr::CTAD, 0},
                                {"", "", columnDescr::N_ColTypes, 0}};

// DE405
const columnDescr DE405Col[] = {{"MJD", "d", columnDescr::CTD, 0},
                                {"x", "", columnDescr::CTAD, 0},
                                {"", "", columnDescr::N_ColTypes, 0}};
...
const String intypes[][2] = {{"ALL", "IERS IGRF JPL"},
...
void makeProperties() {
    ...
    for (uInt j = 0;; ++j) {
      ...
      switch (allProperties[i].cdesc[j].colType) {
        ...
        case columnDescr::CTS:
          tcd = new ScalarColumnDesc<String>(allProperties[i].cdesc[j].colName, "",
                                             "IncrementalStMan", "IncrementalStMan");
          if (!allProperties[i].cdesc[j].unit.empty()) {
            tcd->rwKeywordSet().define("UNIT", allProperties[i].cdesc[j].unit);
          };
  ...
  for (uInt i = 0;; ++i) {
    ...
    for (uInt j = 0; j < tmp.size(); ++j) {
      ...
      if (upcase(tmp[j]) == "X")
        fd.n = 1;
      else if (upcase(tmp[j]) == "A")
        fd.n = 1;
      else if (tmp[j].size() < 2)
        throw(AipsError("Illegal Format specifier"));
      else {
        fd.n = int_data(tmp[j].substr(1));
        if (upcase(tmp[j])[0] == 'X')
          ;
        else if (upcase(tmp[j])[0] == 'A')
          fd.form = formatDescr::A;
        else if (upcase(tmp[j])[0] == 'I')
          fd.form = formatDescr::I;
        else if (upcase(tmp[j])[0] == 'F')
          fd.form = formatDescr::F;
        else
          throw(AipsError("Unknown  Format specifier"));
      };
      off += fd.n;
      if (fd.form != formatDescr::X) properties[allProperties[i].type].fdesc.push_back(fd);
    };
  };
  ...
  for (uInt i = 0;; ++i) {
    ...
    if (!split_data(properties[allProperties[i].type].vinfo, allProperties[i].info)) {
...
Table *create_table(const inputValues &inVal, tableProperties &tprop) {
  ...
  if (Table::isWritable(tprop.tnam))
    tab = new Table(tprop.tnam, Table::Update);
  else {
...
Bool read_data(vector<String> &out, const String &, const Path &in, Bool del = True) {
  ...
  if (!infile) {
    throw(AipsError(String("Cannot open the input data file ") + in.absoluteName()));
  };
  String line;
...
Bool TAI_UTC(tableProperties &tprop, inputValues &inVal) {
  ...
  // Check format.
  if (fields.size() < 10 || fields[0].size() < 15 || fields[0].size() != fields.back().size()) {
...
Bool IGRF(tableProperties &tprop, inputValues &inVal) {
  ...
  if (n == 0 || n != m) {
    throw(
        AipsError("IGRF data has incorrect nm pair: " + uIntToString(n) + ", " + uIntToString(m)));
  };
  if (allcol.size() != n * (n + 2)) {
    throw(AipsError("IGRF data has incorrect number of entries: " + uIntToString(allcol.size()) +
                    " for nm"));
  };
...
Bool JPLDE(tableProperties &tprop, inputValues &inVal) {
  ...
  for (++bc; bc < bl; ++bc) {  // Group 1030: epochs
    if (hfields[bc].size() < 2 || hfields[bc][0] != "GROUP" || hfields[bc][1] != "1030") continue;
    for (++bc; bc < bl; ++bc) {
  ...
  if (!ksize || !incepo || kwnames.size() != kwval.size() || ptt.size() != 3 * 13) {
  ...
  infile.clear();
  infile.close();
  ///  cout << "sz: " << allmjd[0] << ":" << allmjd.size() << ":" << allcol.size() << ":" <<
  ///  allcol[0].size() << endl;;;
...
int main(int argc, const char **argv) {
  try {
    ...
    inputs.create("type", inVal.type, "Type of table to create", "string");
    inputs.create("dir", inVal.dir, "Path to data table base directory", "string");
    inputs.create("refresh", boolToString(inVal.refresh), "Force refresh, even when not needed yet",
                  "bool");
    ...
    inputs.create("in", inVal.in, "Hidden: name of input (ASCII) file", "string");
    inputs.create("x__n", uIntToString(inVal.x__n), "Hidden: pointer into execution list", "int");
    inputs.create("x__fn", boolToString(inVal.x__fn), "Hidden: filename should have been given",
                  "bool");
    inputs.create("x__rep", boolToString(inVal.x__rep), "Hidden: in repeating input cycle", "bool");
    ...
    fout.remove();
    if (!fout.canCreate()) {
      throw(AipsError("Cannot create in ./ the link file " + pout.absoluteName()));
    };
    ...
    if (String(inVal.types[inVal.x__n], 0, 2) == String("DE")) {
      cout << "The requested DE table range is " << inVal.derange[0] << "-" << inVal.derange[1]
           << endl;
    };
