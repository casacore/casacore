...
#define uvputvra_c(tno, name, value) uvputvr_c(tno, H_BYTE, name, value, strlen(value))
#define uvputvrj_c(tno, name, value, n) uvputvr_c(tno, H_INT2, name, (char *)(value), n)
#define uvputvri_c(tno, name, value, n) uvputvr_c(tno, H_INT, name, (char *)(value), n)
#define uvputvrr_c(tno, name, value, n) uvputvr_c(tno, H_REAL, name, (char *)(value), n)
#define uvputvrd_c(tno, name, value, n) uvputvr_c(tno, H_DBLE, name, (char *)(value), n)
#define uvputvrc_c(tno, name, value, n) uvputvr_c(tno, H_CMPLX, name, (char *)(value), n)

#define uvgetvra_c(tno, name, value, n) uvgetvr_c(tno, H_BYTE, name, value, n)
#define uvgetvrj_c(tno, name, value, n) uvgetvr_c(tno, H_INT2, name, (char *)(value), n)
#define uvgetvri_c(tno, name, value, n) uvgetvr_c(tno, H_INT, name, (char *)(value), n)
#define uvgetvrr_c(tno, name, value, n) uvgetvr_c(tno, H_REAL, name, (char *)(value), n)
#define uvgetvrd_c(tno, name, value, n) uvgetvr_c(tno, H_DBLE, name, (char *)(value), n)
#define uvgetvrc_c(tno, name, value, n) uvgetvr_c(tno, H_CMPLX, name, (char *)(value), n)

#define uvrdvra_c(tno, name, data, def, len) uvrdvr_c(tno, H_BYTE, name, data, def, len)
#define uvrdvri_c(tno, name, data, def) uvrdvr_c(tno, H_INT, name, (char *)(data), (char *)(def), 1)
...
void xyzsetup_c(int tno, Const char *subcube, Const int blc[], Const int trc[], int viraxlen[],
                long vircubesize[]);
...
void inWrite_c(const int conid, const int icocd, const int traid, const int inhid, const int ints,
               const int itq, const float az, const float el, const float ha, const int iut,
               const int iref_time, const double dhrs, const float vc, const int ivctype,
               const double sx, const double sy, const double sz, const float rinteg,
               const int proid, const int souid, const int isource, const int ipos,
               const float offx, const float offy, const int iofftype, const int ira,
               const int idec, const double rar, const double decr, const float epoch,
               const float sflux, const float size);
void blWrite_c(const int blhid, const int inhid, const int isb, const int ipol, const float pa,
               const int iaq, const int ibq, const int icq, const int ioq, const int irec,
               const int iffc, const float u, const float v, const float w, const float prbl,
               const float angres, const float vis, const float coh, const float sigcoh,
               const float csnr, const float vflux, const float cnoise, const double avedhrs,
               const float ampav, const float phaave, const float tpvar, const int blsid,
               const int itel1, const int itel2, const int iblcd, const float ble, const float bln,
               const float blu, const int soid);
void spWrite_c(const int sphid, const int blhid, const int inhid, const int igq, const int ipq,
               const int iband, const int ipstate, const float tau0, const double vel,
               const float vres, const int ivtype, const double fsky, const float fres,
               const float tssb, const float integ, const float wt, const int itaper,
               const float snoise, const int nch, const int nrec, const int dataoff,
               const int linid, const int itrans, const double rfreq, const int pasid,
               const int gaiidamp, const int gaiidpha, const int flcid, const int atmid);
void codeWrite_c(const char *v_name, const int icode, const char *code, const int ncode);
void visWrite_c(const float *re, const float *im, const int numvis, const int startvis,
                int *nbytes);
