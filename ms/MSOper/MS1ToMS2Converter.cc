...
Bool MS1ToMS2Converter::convert() {
  ...
  {
    Table t(ms1_p);
    if (t.keywordSet().isDefined("MS_VERSION") && t.keywordSet().asFloat("MS_VERSION") >= 2.0) {
  ...
  if (t.keywordSet().isDefined("SORTED_TABLE")) t.rwKeywordSet().removeField("SORTED_TABLE");

  if (t.keywordSet().isDefined("SORT_COLUMNS")) t.rwKeywordSet().removeField("SORT_COLUMNS");
  ...
  // FIELD
  {
    ...
    for (Int i = 0; i < nRow; i++) {
      ...
      if (numPol == 1) {
        ...
        pdir(0, 1) = pdr(0, i);
        pdir(1, 1) = pdr(1, i);
        pntdir(0, 1) = pntdr(0, i);
        pntdir(1, 1) = pntdr(1, i);
        rdir(0, 1) = rdr(0, i);
        rdir(1, 1) = rdr(1, i);
      }
      delDir.put(i, ddir);
  ...
  // FLAG_CMD
  {
    ...
    t.rwKeywordSet().defineTable(MS::keywordName(MS::FLAG_CMD), flagCmdt);
  }
  ...
  // SPECTRAL_WINDOW
  {
    ...
    t.rwKeywordSet().defineTable(MS::keywordName(MS::POLARIZATION), polTab);
  ...
  // PROCESSOR
  {
    ...
    t.rwKeywordSet().defineTable(MS::keywordName(MS::PROCESSOR), processorSetupt);
  }
  ...
  // STATE
  {
    ...
    t.rwKeywordSet().defineTable(MS::keywordName(MS::STATE), stateSetupt);
  }
