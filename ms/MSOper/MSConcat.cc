  ...
  while (isFixed == False && hc < nHyperCols) {
    ...
    while (isFixed == False && dc < nDataCol) {
      ...
      if (dataColName == MS::columnName(MS::FLAG_CATEGORY) ||
          dataColName == MS::columnName(MS::DATA) || dataColName == MS::columnName(MS::FLAG) ||
          ...
          dataColName == MS::columnName(MS::LAG_DATA) || dataColName == MS::columnName(MS::SIGMA) ||
...
void MSConcat::virtualconcat(MeasurementSet& otherMS, const Bool checkShapeAndCateg,
  ...
  if (itsMS.tableDesc().isColumn("FLOAT_DATA") && otherMS.tableDesc().isColumn("FLOAT_DATA"))
    ...
    log << itsMS.tableName() << " has FLOAT_DATA column but not " << otherMS.tableName()
        << LogIO::EXCEPTION;
    log << "Cannot concatenate these MSs yet...you may split the corrected column of the SD as a "
           "work around."
        << LogIO::EXCEPTION;
  }
  if (itsMS.tableDesc().isColumn("MODEL_DATA") && otherMS.tableDesc().isColumn("MODEL_DATA"))
    ...
    log << itsMS.tableName() << " has MODEL_DATA column but not " << otherMS.tableName()
        << LogIO::EXCEPTION;
    log << "You may wish to create this column by loading " << otherMS.tableName()
  ...
  // See if there is a SOURCE table and concatenate and reindex it
  {
    ...
    if (Table::isReadable(itsMS.sourceTableName())) {
      ...
      if (addedRows > 0) {
        log << "Added " << addedRows << " rows to the source subtable" << LogIO::POST;
      }
    }
  }
  ...
  if (Table::isReadable(itsMS.sourceTableName())) {
    ...
    if (removedRows > 0) {
      log << "Removed " << removedRows << " redundant rows from the source subtable" << LogIO::POST;
    }
  }
  ...
  }
  {
    ...
    log << "Added " << addedRows << " rows to the feed subtable" << std::endl;
  }
  ...
  if (reindexObsidAndScan) {
    ...
    if (ifs.good()) {
      ...
      for (uInt i = 0; i < n; i++) {
        if (ifs.good()) {
          ...
          ifs >> tminscan;
          minScan.push_back(tminscan);
          ifs >> tmaxscan;
          maxScan.push_back(tmaxscan);
        } else {
      ...
      if (ifs.good()) {
        ...
        log << LogIO::WARN << "Error reading file " << obsidAndScanTableName << LogIO::POST;
        log << LogIO::WARN << "Will continue with uninitialized obsid and scans information"
            << LogIO::POST;
    ...
    for (uInt i = 0; i < distinctObsIdSet.size(); i++) {
      ...
      if (scanOffset < 0) {
        log << LogIO::WARN
            << "Zero or negative scan numbers in MS. May lead to duplicate scan numbers in "
               "concatenated MS."
            << LogIO::POST;
        scanOffset = 0;
      }
  ...
  if (reindexObsidAndScan) {
      ...
      if (oid != otherObsIds[r]) {                                   // obsid actually changed
        ...
        if (encountered.find(oid) == encountered.end() && scanOffsetForOid.at(oid) != 0) {
          log << LogIO::NORMAL << "Will offset scan numbers by " << scanOffsetForOid.at(oid)
              << " for observations with Obs ID " << oid << " in order to make scan numbers unique."
              << LogIO::POST;
          encountered[oid] = 0;
        }
    ...
    if (!ofs) {
      ...
      ofs << maxScanOther
          << std::endl;  // note: this max is determined after the modification of scanOther
    }
    ofs.close();
  }
    ...
    if (itsChanReversed[otherDDId[r]]) {
      ...
      for (Int k1 = 0; k1 < datShape[0]; ++k1) {
        for (Int k2 = 0; k2 < datShape[1]; ++k2) {
          ...
          if (doModelData) {
            reversedModData(k1, k2) =
                (Matrix<Complex>(otherModelData(r)))(k1, datShape[1] - 1 - k2);
          }
          if (doCorrectedData) {
            reversedCorrData(k1, k2) =
                (Matrix<Complex>(otherCorrectedData(r)))(k1, datShape[1] - 1 - k2);
          }
        }
      }
      if (doFloatData) {
...
void MSConcat::concatenate(const MeasurementSet& otherMS, const uInt handling,
  ...
  if (destMSName.empty()) {
    log << "Appending " << otherMS.tableName() << " to " << itsMS.tableName() << std::endl
        << LogIO::POST;
  } else {
    log << "Virtually appending " << otherMS.tableName() << " to " << itsMS.tableName() << std::endl
        << LogIO::POST;
  }
  ...
  switch (handling) {
    ...
    case 2:
      ...
      break;
    case 3:
      log << "*** At user\'s request, MAIN and POINTING tables will not be concatenated!"
          << LogIO::POST;
      break;
    default:
  ...
  if (!destMS->canAddRow()) {
    log << LogIO::WARN << "Can't add rows to this ms!  Something is seriously wrong with "
        << destMS->tableName() << std::endl
        << LogIO::POST;
  }

  log << LogIO::DEBUG1 << "trying to add " << newRows
      << " data rows to the ms, now at: " << destMS->nrow() << std::endl
      << LogIO::POST;
  destMS->addRow(newRows);
  log << LogIO::DEBUG1 << "added " << newRows << " data rows to the ms, now at: " << destMS->nrow()
      << std::endl
      << LogIO::POST;
...
Bool MSConcat::copyPointing(const MSPointing& otherPoint, const Block<uInt>& newAntIndices) {
  ...
  if (rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid antenna ids in the POINTING table; the POINTING table will be emptied "
...
Bool MSConcat::copySysCal(const MSSysCal& otherSysCal, const Block<uInt>& newAntIndices) {
  ...
  if (rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid antenna ids in the SYSCAL table; the SYSCAL table will be emptied as it "
            "is inconsistent"
         << LogIO::POST;
  ...
  if (doSPW_p && rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid spectral window ids in the SYSCAL table; the SYSCAL table will be "
            "emptied as it is inconsistent"
         << LogIO::POST;
      RowNumbers rowtodel(sysCal.nrow());
      indgen(rowtodel);
      sysCal.removeRow(RowNumbers(rowtodel));
...
Bool MSConcat::copyGainCurve(const MeasurementSet& otherMS, const Block<uInt>& newAntIndices) {
  ...
  if (rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid antenna ids in the GAIN_CURVE table; the GAIN_CURVE table will be "
            "emptied as it is inconsistent"
         << LogIO::POST;
  ...
  if (doSPW_p && rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid spectral window ids in the GAIN_CURVE table; the GAIN_CURVE table will "
            "be emptied as it is inconsistent"
         << LogIO::POST;
      RowNumbers rowtodel(gainCurve.nrow());
      indgen(rowtodel);
      gainCurve.removeRow(RowNumbers(rowtodel));
...
Bool MSConcat::copyPhaseCal(const MeasurementSet& otherMS, const Block<uInt>& newAntIndices) {
  ...
  if (rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid antenna ids in the PHASE_CAL table; the PHASE_CAL table will be emptied "
            "as it is inconsistent"
         << LogIO::POST;
  ...
  if (doSPW_p && rowToBeAdded > 0) {
    ...
    if (!idsOK) {
      os << LogIO::WARN
         << "Found invalid spectral window ids in the PHASE_CAL table; the PHASE_CAL table will be "
            "emptied as it is inconsistent"
         << LogIO::POST;
      RowNumbers rowtodel(phaseCal.nrow());
      indgen(rowtodel);
  ...
  if (remRedunObsId) {  // remove redundant rows
    ...
    for (rownr_t i = 0; i < otherObs.nrow(); i++) {      // loop over rows of second table
      if (tempObsIndex.find(i) != tempObsIndex.end()) {  // ID changed because of addition to table
        if (tempObsIndex2.find(tempObsIndex.at(i)) !=
            tempObsIndex2.end()) {  // ID also changed because of removal
    ...
    os << "Added " << obs.nrow() - originalNrow << " rows and matched " << rowsToBeRemoved.size()
       << " rows in the observation subtable." << LogIO::POST;
    ...
    os << "Added " << obs.nrow() - originalNrow << " rows in the observation subtable."
       << LogIO::POST;
...
Int MSConcat::copyProcessor(const MSProcessor& otherProc, const Bool remRedunProcId) {
  ...
  if (remRedunProcId) {  // remove redundant rows
    ...
    for (uInt i = 0; i < otherProc.nrow(); i++) {  // loop over rows of second table
      if (tempProcIndex.find(i) !=
        ...
        if (tempProcIndex2.find(tempProcIndex.at(i)) !=
            tempProcIndex2.end()) {  // ID also changed because of removal
    ...
    os << "Added " << proc.nrow() - originalNrow << " rows and matched " << rowsToBeRemoved.size()
       << " rows in the processor subtable." << LogIO::POST;
    ...
    os << "Added " << proc.nrow() - originalNrow << " rows in the processor subtable."
       << LogIO::POST;
...
Block<uInt> MSConcat::copyAntennaAndFeed(const MSAntenna& otherAnt, const MSFeed& otherFeed) {
  ...
  if (!feedCols.focusLengthQuant().isNull() && otherFeedCols.focusLengthQuant().isNull()) {
    os << LogIO::WARN
       << "MS appended to has optional column FOCUS_LENGTH in FEED table, but MS to be appended "
          "does not.\n"
    ...
    os << LogIO::WARN
       << "MS appended to does not have optional column FOCUS_LENGTH in FEED table, but MS to be "
          "appended does.\n"
  ...
  for (uInt a = 0; a < nAntIds; a++) {
    ...
    if (newAntId >= 0) {
      ...
      *itsAntInd = newAntId;
      ...
      if (itsFeedsToCompare.nelements() == nFeedsToCompare) {
        ...
        for (uInt f = 0; f < nFeedsToCompare; f++) {
          ...
          const Int matchingFeedRow = feedCols.matchFeed(
              ...
              otherFeedCols.polarizationType()(k), otherFeedCols.polResponse()(k),
              otherFeedCols.positionQuant()(k), otherFeedCols.receptorAngleQuant()(k), ignoreRows,
              fLengthQ);
      ...
      //       else{
      // 	cout << "Antenna " << a << " found all matching feeds: " << matchingFeeds <<
      // std::endl;
      //       }
    }
    ...
    if (addNewEntry) {  // need to add a new entry in the ANTENNA subtable
      ...
      if ((movedAntId = antCols.matchAntenna(otherAntCols.name()(a), otherAntCols.positionMeas()(a),
        ...
        // 						     otherAntCols.positionMeas()(a),
        // Quantum<Double>(100, "AU")))
...
Block<uInt> MSConcat::copyField(const MeasurementSet& otherms) {
  ...
  const MDirection::Types dirType =
      MDirection::castType(fieldCols.referenceDirMeasCol().getMeasRef().getType());
  const MDirection::Types otherDirType =
      MDirection::castType(otherFieldCols.referenceDirMeasCol().getMeasRef().getType());
  ...
  if (maxThisEphId >
      -1) {  // this MS has at least one field using an ephemeris.
  ...
  for (uInt f = 0; f < nFlds; f++) {
    ...
    try {
      ...
      if (!ephPath.empty()) {
        ...
        os << LogIO::SEVERE << "Field " << f << " (" << otherFieldCols.name()(f)
           << ", to be appended)"
           << " is using an ephemeris with incorrect time origin setup: the time origin ("
           << otherOrigTime
           << " s) in the FIELD table is outside the validity range of the ephemeris."
           << LogIO::POST;
      }
      throw(x);
    }
        ...
        if (ephPath.empty()) {     // the other field does not
          ...
          if (!canUseThisEntry) {
            ...
            os << LogIO::NORMAL << "Ephemeris " << thisEphPath << std::endl
               ...
               << " cannot be used for data from field " << f << " (" << otherFieldCols.name()(f)
               << ", to be appended)"
    ...
    if (canUseThisEntry &&
      ...
      if (!ephPath.empty() &&
          otherFieldCols.ephemerisId()(f) > -1) {  // f has a non-trivial ephemeris id
        maxThisEphId++;
        String ephName = Path(ephPath).baseName();
        if (!fld.addEphemeris(
                maxThisEphId, ephPath,
                ephName.substr(ephName.find("_") + 1, ephName.size() - 4 - ephName.find("_") -
                                                          1))  // extract comment from name
        ) {
          LogIO os(LogOrigin("MSConcat", "copyField"));
          os << LogIO::SEVERE << "Error transferring ephemeris " << ephPath << " to concatvis."
             << LogIO::POST;
        }
      ...
      for (Int j = 0; j < numrows_this; ++j) {
        if (thisSPWId(j) < -1) {  // came from the second input table
          ...
          if (doSPW_p || newSPWIndex_p.find(thisSPWId(j) + 10000) !=
                             newSPWIndex_p.end()) {  // the SPW table was rearranged
          ...
          } else {  // the SPW table did not have to be rearranged, just revert changes to SPW from
                    // copySource
          ...
          if (!rowToBeRemoved(k)) {
            if (thisSPWIdB(j) == thisSPWIdB(k)) {  // the SPW id is the same
              ...
              if (sourceRowsEquivalent(
                      sourceCol, j, k,
                      sameSolSystObjects)) {  // and all columns are the same (not testing source,
                                              // spw id, time, and interval)
                // cout << "Found SOURCE rows " << j << " and " << k << " to be identical." <<
                // std::endl;
        ...
        for (Int k = j + 1; k < newNumrows_this; ++k) {
          if (thisSourceId(j) != thisSourceId(k)) {
            ...
            if (sourceRowsEquivalent(sourceCol, j, k, sameSolSystObjects)) {
              ...
              // cout << "Found SOURCE rows " << j << " and " << k << " to be identical except for
              // the SPW ID and source id. "
      ...
      if (rowsRenamed) {                                  // reduce ID values to minimal range
        ...
        for (Int j = 0; j < newNumrows_this; ++j) {
          if (newThisId(j) >= nDistinctSources) {
            ...
            //               << ". "
            // 		 << newThisId(j) << " mapped to " << nDistinctSources-counter-1 <<
            // std::endl;
          }
        }
      }
        ...
        for (Int j = 0; j < numrows_this; ++j) {  // loop over old indices
          if (tempSourceIndex.find(j) !=
            ...
            if (tempSourceIndex2.find(tempSourceIndex.at(j)) !=
                tempSourceIndex2.end()) {  // ID changed also because of renumbering
              if (tempSourceIndex3.find(tempSourceIndex2.at(tempSourceIndex.at(j))) !=
                  tempSourceIndex3.end()) {  // ID also changed because of renaming
                newSourceIndex2_p[j] =
                    tempSourceIndex3.at(tempSourceIndex2.at(tempSourceIndex.at(j)));  // abc
              } else {  // ID changed because of redundancy and renumberning
                newSourceIndex2_p[j] = tempSourceIndex2.at(tempSourceIndex.at(j));  // ab
              }
            } else {
              if (tempSourceIndex3.find(tempSourceIndex.at(j)) !=
                  tempSourceIndex3.end()) {  // ID  changed because of redundancy and renaming
            ...
            if (tempSourceIndex3.find(tempSourceIndex2.at(j)) !=
                tempSourceIndex3.end()) {  // ID  changed because of renumbering and renaming
          ...
          } else if (tempSourceIndex3.find(j) !=
                     tempSourceIndex3.end()) {              // ID only changed because of renaming
  ...
  if (Table::isReadable(itsMS.sourceTableName())) {
    ...
    if (numrows_this > 0) {  // the source table is not empty
      ...
      if (rowsToBeRemoved.size() > 0) {  // actually remove the rows
        Vector<rownr_t> rowsTBR(rowsToBeRemoved);
        newSource.removeRow(rowsTBR);
        // cout << "Removed " << rowsToBeRemoved.size() << " stray rows from SOURCE table." <<
        // std::endl;
      }
    }
  }
...
Bool MSConcat::sourceRowsEquivalent(const MSSourceColumns& sourceCol, const rownr_t& rowi,
  ...
  if (areEQ(sourceCol.calibrationGroup(), rowi, rowj) && areEQ(sourceCol.code(), rowi, rowj) &&
      areEQ(sourceCol.name(), rowi, rowj) && areEQ(sourceCol.numLines(), rowi, rowj) &&
      // do NOT test SPW ID!
...
Block<uInt> MSConcat::copySpwAndPol(const MSSpectralWindow& otherSpw,
  ...
  for (uInt otherSpwId = 0; otherSpwId < otherSpw.nrow(); otherSpwId++) {
    if (!foundInDD(otherSpwId)) {  // not already processed above
      ...
      if (newSpwId < 0) {
        // cout << "Second iteration: no counterpart found for other spw " << otherSpwId <<
        // std::endl; need to add a new entry in the SPECTRAL_WINDOW subtable
        newSpwId = spw.nrow();
        spw.addRow();
