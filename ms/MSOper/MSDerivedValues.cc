...
MSDerivedValues::MSDerivedValues() { init(); }

MSDerivedValues::MSDerivedValues(const MSDerivedValues& other) { operator=(other); }
...
MSDerivedValues& MSDerivedValues::operator=(const MSDerivedValues& other) {
...
const Vector<MPosition>& MSDerivedValues::getAntennaPositions() const { return mAntPos_p; }
...
MSDerivedValues& MSDerivedValues::setAntennaPositions(const Vector<MPosition>& antPosition) {
...
MSDerivedValues& MSDerivedValues::setObservatoryPosition(const MPosition& obsPosition) {
...
Double MSDerivedValues::parAngle() {
  ...
  if (mount_p(antenna_p) == 0 || mount_p(antenna_p) == 4 || mount_p(antenna_p) == 5 ||
      mount_p(antenna_p) == 6 || mount_p(antenna_p) == 7) {
    ...
    pa = mRADecInAzEl_p.getValue().positionAngle(mHADecPoleInAzEl_p.getValue());
    ...
    if (mount_p(antenna_p) == 4) {
      ...
      pa += mRADecInAzEl_p.getAngle().getValue()[1] - mRADecInAzEl_p.getAngle().getValue()[0];
...
MRadialVelocity::Types MSDerivedValues::getRadialVelocityType() const {
...
Double MSDerivedValues::hourAngle() { return cRADecToHADec_p().getValue().get()(0); }

const MDirection& MSDerivedValues::azel() { return cRADecToAzEl_p(); }

const MEpoch& MSDerivedValues::last() { return cUTCToLAST_p(); }

const MRadialVelocity& MSDerivedValues::obsVel() { return cTOPOToLSR_p(); }

Quantity MSDerivedValues::toFrequency(const Quantity& vel, const Quantity& restFreq) {
  VelocityMachine vm(frqref_p, Unit("GHz"), MVFrequency(restFreq), velref_p, vel.getUnit(),
                     fAntFrame_p);

  return vm.makeFrequency(vel.getValue());
}

Quantity MSDerivedValues::toFrequency(const Quantity& vel) { return toFrequency(vel, restFreq_p); }

Quantity MSDerivedValues::toVelocity(const Quantity& freq, const Quantity& restFreq) {
  VelocityMachine vm(frqref_p, freq.getUnit(), MVFrequency(restFreq), velref_p, Unit("km/s"),
                     fAntFrame_p);

  return vm.makeVelocity(freq.getValue());
}

Quantity MSDerivedValues::toVelocity(const Quantity& freq) { return toVelocity(freq, restFreq_p); }
...
void MSDerivedValues::init() {
  ...
  cRADecToAzEl_p.set(MDirection(), MDirection::Ref(MDirection::AZEL, fAntFrame_p));
  ...
  cTOPOToLSR_p.set(MRadialVelocity(MVRadialVelocity(0.0),
                                   MRadialVelocity::Ref(MRadialVelocity::TOPO, fAntFrame_p)),
