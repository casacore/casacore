...
Record MSFlagger::diffDataBuffer(const String& direction, Int window, Bool doMedian) {
  ...
  if (dir != "time" && dir != "channel") {
    os << LogIO::WARN << "Unrecognized direction " << direction << ", specify TIME or CHANNEL"
       << LogIO::POST;
    return retVal;
  }
  ...
  if (!buffer_p.isDefined("datafield")) {
    os << LogIO::WARN << "Buffer is empty, use fillbuffer first" << LogIO::POST;
    return retVal;
  }
  ...
  switch (fld) {
    ...
    case MSS::OBS_RESIDUAL_DATA: {
      ...
      if (dir == "time") {
        diff = MSSelUtil<Complex>::diffData(data, flag, flagRow, timeAxis, win, doMedian);
      } else {
        diff = MSSelUtil<Complex>::diffData(data, flag, flagRow, chanAxis, win, doMedian);
      }
  ...
  */
  ...
  while (iter) {
    ...
    if (iter) {
      ...
      applyRowFlags(flag, flagRow);  // need to apply row flags to flags for stats
      getStats(medTF, adTF, medT, medFmedT, adT, medF, medTmedF, adF, diff, flag, flagRow);
      if (deleteFlag || deleteFlagRow) {
    ...
    for (Int pol = 0; pol < nCorr; pol++) {
      ...
      if (sumChan(pol, ifr) > 0) {
        os << LogIO::NORMAL << "Flagged " << sumChan(pol, ifr) << " channel"
           << multiple(sumChan(pol, ifr)) << " with abs(median - " << medTmedF(pol, ifr) << ") > "
           << channelLevel << "*" << adF(pol, ifr) << LogIO::POST;
      }
      if (sumTime(pol, ifr) > 0) {
        os << LogIO::NORMAL << "Flagged " << sumTime(pol, ifr) << " time"
           << multiple(sumTime(pol, ifr)) << " with abs(median - " << medFmedT(pol, ifr) << ") > "
           << timeLevel << "*" << adT(pol, ifr) << LogIO::POST;
      }
      if (sum(pol, ifr) > 0) {
        os << LogIO::NORMAL << "Flagged " << sum(pol, ifr) << " pixel" << multiple(sum(pol, ifr))
