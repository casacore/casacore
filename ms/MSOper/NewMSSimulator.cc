...
NewMSSimulator::NewMSSimulator(const String& MSName)
    : dataAcc_p(), scratchDataAcc_p(), sigmaAcc_p(), flagAcc_p(), maxData_p(2e9) {
  ...
  */
  ...
  // These columns contain the bulk of the data so save them in a tiled way
  {
    ...
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::DATA), dataMan);
  }
  {
    TiledShapeStMan dataMan("FlagColumn", tileShape);
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::FLAG), dataMan);
  }
  {
    TiledShapeStMan dataMan("ModelDataColumn", tileShape);
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::MODEL_DATA), dataMan);
  }
  {
    TiledShapeStMan dataMan("CorrectedDataColumn", tileShape);
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::CORRECTED_DATA), dataMan);
  }
  {
    TiledShapeStMan dataMan("SigmaColumn", IPosition(2, tileShape(0), tileShape(2)));
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::SIGMA), dataMan);
  }
  {
    TiledShapeStMan dataMan("WeightColumn", IPosition(2, tileShape(0), tileShape(2)));
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::WEIGHT), dataMan);
  }

  {
    TiledShapeStMan dataMan(flagCol, IPosition(4, tileShape(0), tileShape(1), 1, tileShape(2)));
    newMS.bindColumn(MeasurementSet::columnName(MeasurementSet::FLAG_CATEGORY), dataMan);
  ...
  ms_p->rwKeywordSet().defineTable(MS::keywordName(MS::SOURCE), Table(sourceSetup));
...
NewMSSimulator::NewMSSimulator(MeasurementSet& theMS)
  ...
  os << "Opening MeasurementSet " << ms_p->tableName() << " with " << ms_p->nrow() << " rows"
     << LogIO::POST;
...
void NewMSSimulator::initAnt(const String& telescope, const Vector<Double>& x,
  ...
  if (coordsystem == "global") {
    ...
    os << "Using local coordinates for the antennas" << std::endl << "Reference position = ";
    os.output().width(13);
  ...
  Slicer antSlice(IPosition(1, numOfAnt), IPosition(1, numOfAnt + nAnt - 1), IPosition(1, 1),
                  Slicer::endIsLast);
...
void NewMSSimulator::initFields(const String& sourceName, const MDirection& sourceDirection,
  ...
  if (!sourcec.sourceModel().isNull()) {
    // The following commented out code is an example of how to insert component list tables into
    // the sourceModel column
...
void NewMSSimulator::initSpWindows(const String& spWindowName, const Int& nChan,
  ...
  for (Int j = 0; j < 4; j++) {
    ...
    if (myStokesString.length() == 0) break;
...
void NewMSSimulator::initFeeds(const String& mode, const Vector<Double>& x, const Vector<Double>& y,
  ...
  if (nFeed > 1) {
    ...
    if (pol.nelements() != x.nelements()) {
      os << "Feed polarization list must be same length as the number of positions"
         << LogIO::EXCEPTION;
    }
  ...
  if (isList) {
    ...
    for (Int i = 0; i < nAnt; i++) {
      for (Int j = 0; j < nFeed; j++) {
        ...
        os << "Row " << iRow + 1 << " : Feed " << j + 1 << " on antenna " << i + 1 << " " << x(j)
           << " " << y(j) << " " << pol(j) << LogIO::POST;
        iRow++;
      }
    }
  } else {
...
void NewMSSimulator::observe(const String& sourceName, const String& spWindowName,
  ...
  NewMSSimulator::observe(sourceNames, spWindowName, qStartTimes, qStopTimes, directions,
                          add_observation, state_sig, state_ref, state_cal, state_load,
                          state_sub_scan, state_obs_mode, observername, projectname);
}
...
void NewMSSimulator::observe(const Vector<String>& sourceNames, const String& spWindowName,
  ...
  {
    os << "First source: " << sourceNames[iSrc] << " @ " << formatDirection(fieldCenter)
       << std::endl;
  }
  ...
  Tstart = qStartTimes(0).getValue("s") + taiRefTime.get("s").getValue("s") + t_offset_p;
  Tend = qStopTimes(nPts - 1).getValue("s") + taiRefTime.get("s").getValue("s") + t_offset_p;
  os << "Full time range: " << formatTime(Tstart) << " -- " << formatTime(Tend)
     << " TAI with int = " << Tint << LogIO::POST;
  ...
  for (Int feed = 0; feed < nFeed; feed++) {
    ...
    for (Int pointing = 0; pointing < nPts; pointing++) {
      ...
      // One call to observe corresponds to at least one scan, depending on whether sourceName has
      ...
      if (pointing == 0) {
        ...
        if (not(sourceNames(pointing) == sourceNames(pointing - 1))) {
          ...
          if (existingFieldID < 0) {
            os << "   Field " << pointing << ": " << sourceNames(pointing) << " not yet defined"
               << LogIO::EXCEPTION;
          }
          fcs = northPole;
          fieldc.phaseDirMeasCol().get(existingFieldID, fcs);
      ...
      if (pointing < 20) {
        os << LogIO::DEBUG1 << "   Field " << pointing << ": " << sourceNames(pointing) << " @ "
           << formatDirection(fieldCenter) << " for " << nIntegrations << " integrations "
           << std::endl;
      } else {
        if (pointing == 20) {
          os << LogIO::DEBUG1 << "   (continuing without printing to log -- see MS for details) "
             << std::endl
             << LogIO::POST;
        }
      }
