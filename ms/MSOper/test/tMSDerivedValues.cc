...
int main() {
  try {
    Quantity longitude;
    Quantity::read(longitude, "149.33.00.5");
    cout << "longitude: " << MVAngle(longitude) << std::endl;
    Quantity latitude;
    Quantity::read(latitude, "-30.18.46.385");
    cout << "latitude : " << MVAngle(latitude) << std::endl;
    Vector<MPosition> pos(1);
    pos(0) = MPosition(Quantity(236.9, "m"), longitude, latitude, MPosition::Ref(MPosition::WGS84));

    MSDerivedValues msd;
    msd.setAntennaPositions(pos);

    Quantity time;
    MVTime::read(time, "today");
    MEpoch today(time);
    ...
    Quantity ra;
    Quantity::read(ra, "12:00:00.0");
    Quantity dec;
    Quantity::read(dec, "-30.00.00.0");
    ...
    cout << " observatory velocity in LSR  frame: " << msd.obsVel().get("km/s").getValue() << "km/s"
         << std::endl;

    msd.setVelocityFrame(MRadialVelocity::BARY);
    cout << " observatory velocity in BARY frame: " << msd.obsVel().get("km/s").getValue() << "km/s"
         << std::endl;

    msd.setVelocityFrame(MRadialVelocity::GEO);
    cout << " observatory velocity in GEO frame: " << msd.obsVel().get("km/s").getValue() << "km/s"
         << std::endl;
    ...
    cout << "radio velocity " << radioVel << " is frequency " << msd.toFrequency(radioVel, restFreq)
         << std::endl;
    //====
    /*
