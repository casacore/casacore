...
class NewMSSimulatorTester {
 public:
  NewMSSimulatorTester(const std::string& ms_name) : msName_p(ms_name) {
    simulator_p.reset(new casacore::NewMSSimulator(ms_name));
  }

  NewMSSimulatorTester(casacore::MeasurementSet& otherMs) : msName_p(otherMs.tableName()) {
...
void test_NewMSSimulator_RandomAntenna() {
  ...
  for (int nAnt = 2; nAnt <= nAntMax; ++nAnt) {
    ...
    simulatorTester.simulator_p->initAnt("Simulated", x, y, z, diam, offset, mount, name, pad,
                                         "global", vlaPosition);
