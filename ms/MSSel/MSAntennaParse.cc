...
MSAntennaParse::MSAntennaParse(const MSAntenna& antSubTable, const TableExprNode& ant1AsTEN,
...
const TableExprNode* MSAntennaParse::selectAntennaIds(const Vector<Int>& antennaIds1,
  ...
  if (negate)
    makeBaselineList(-antennaIds1, -antennaIds2, baselineList, baselineType, negate);
  else
    makeBaselineList(antennaIds1, antennaIds2, baselineList, baselineType, negate);
...
const TableExprNode* MSAntennaParse::selectNameOrStation(const Vector<String>& antenna1,
  ...
  TableExprNode condition = (column1AsTEN_p.in(a1) && column2AsTEN_p.in(a2)) ||
...
const TableExprNode* MSAntennaParse::selectNameOrStation(const String& antenna1,
  ...
  TableExprNode condition = (column1AsTEN_p >= antenna1 && column2AsTEN_p <= antenna2) ||
