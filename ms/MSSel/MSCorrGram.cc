...
const TableExprNode* msCorrGramParseNode() { return MSCorrParse::node(); }
...
void MSCorrGramerror(const char*) {
  throw(AipsError("Corr Expression: Parse error at or near '" + String(MSCorrGramtext) + "'"));
}
...
String msCorrGramRemoveQuotes(const String& in) {
  ...
  while (pos < leng) {
    ...
    if (inx == std::string::npos) {
      throw(AipsError("MSCorrParse - Ill-formed quoted string: " + str));
    }
