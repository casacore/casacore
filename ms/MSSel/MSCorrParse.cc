...
MSCorrParse::MSCorrParse(const MeasurementSet* ms) : MSParse(ms, "Corr") {
...
const TableExprNode* MSCorrParse::selectCorrType(const String& corrType) {
  ...
  selData.putColumn(Slicer(IPosition(2, 0, 0), IPosition(2, 0, rowShape(1) - 1), IPosition(2, 1, 1),
                           Slicer::endIsLast),
                    datacol);
  ...
  TableExprNode condition = selms.col(colName).in(msDDI.matchPolId(msPI.matchCorrType(corrtype)));
...
const TableExprNode* MSCorrParse::node() { return node_p; }
