...
MSFeedParse::MSFeedParse(const MSFeed& feedSubTable, const TableExprNode& feed1AsTEN,
...
const TableExprNode* MSFeedParse::selectFeedIds(const Vector<Int>& feedIds,
  ...
  if ((baselineType == AutoCorrAlso) || (baselineType == AutoCorrOnly)) {
    ...
    condition = (column1AsTEN_p.in(feedIds) || column2AsTEN_p.in(feedIds));
  }
  {
    ...
    */
    ...
    if (negate)
      makeFeedPairList(-feedIds, f2, feedPairList, baselineType, negate);
    else
      makeFeedPairList(feedIds, f2, feedPairList, baselineType, negate);
  }
...
const TableExprNode* MSFeedParse::selectFeedIds(const Vector<Int>& feedIds1,
  ...
  if (negate)
    makeFeedPairList(-feedIds1, -feedIds2, feedPairList, baselineType, negate);
  else
    makeFeedPairList(feedIds1, feedIds2, feedPairList, baselineType, negate);
