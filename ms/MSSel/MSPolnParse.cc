...
Int MSPolnParse::theParser(const String& command) {
  ...
  for (Int i = 0; i < nSpecList; i++) {
    ...
    if (nTokens > 2)
      throw(
          MSSelectionPolnParseError(String("Too many ':'s.  [Tip: Channel "
    ...
    try {
      ...
      msSpwGramParseCommand(ms()->spectralWindow(), ms()->dataDescription(), colAsTEN, spwExpr,
