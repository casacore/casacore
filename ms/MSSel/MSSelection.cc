...
MSSelection::MSSelection(const MeasurementSet& ms, const MSSMode& mode, const String& timeExpr,
                         const String& antennaExpr, const String& fieldExpr, const String& spwExpr,
                         const String& uvDistExpr, const String& taqlExpr, const String& polnExpr,
                         const String& scanExpr, const String& arrayExpr, const String& stateExpr,
  ...
  initErrorHandler(SPW_EXPR);
  initErrorHandler(FEED_EXPR);
  reset2(ms, mode, timeExpr, antennaExpr, fieldExpr, spwExpr, uvDistExpr, taqlExpr, polnExpr,
         scanExpr, arrayExpr, stateExpr, observationExpr, feedExpr);
}

//----------------------------------------------------------------------------

void MSSelection::reset(MSSelectableTable& msLike, const MSSMode& mode, const String& timeExpr,
                        const String& antennaExpr, const String& fieldExpr, const String& spwExpr,
                        const String& uvDistExpr, const String& taqlExpr, const String& polnExpr,
                        const String& scanExpr, const String& arrayExpr, const String& stateExpr,
                        const String& observationExpr) {
  reset2(msLike, mode, timeExpr, antennaExpr, fieldExpr, spwExpr, uvDistExpr, taqlExpr, polnExpr,
         scanExpr, arrayExpr, stateExpr, observationExpr, "");
}

void MSSelection::reset2(MSSelectableTable& msLike, const MSSMode& mode, const String& timeExpr,
                         const String& antennaExpr, const String& fieldExpr, const String& spwExpr,
                         const String& uvDistExpr, const String& taqlExpr, const String& polnExpr,
                         const String& scanExpr, const String& arrayExpr, const String& stateExpr,
                         const String& observationExpr, const String& feedExpr) {
  ...
  if (mode == PARSE_NOW) fullTEN_p = toTableExprNode(&msLike);
}

void MSSelection::reset(const MeasurementSet& ms, const MSSMode& mode, const String& timeExpr,
                        const String& antennaExpr, const String& fieldExpr, const String& spwExpr,
                        const String& uvDistExpr, const String& taqlExpr, const String& polnExpr,
                        const String& scanExpr, const String& arrayExpr, const String& stateExpr,
                        const String& observationExpr) {
...
void MSSelection::reset2(const MeasurementSet& ms, const MSSMode& mode, const String& timeExpr,
                         const String& antennaExpr, const String& fieldExpr, const String& spwExpr,
                         const String& uvDistExpr, const String& taqlExpr, const String& polnExpr,
                         const String& scanExpr, const String& arrayExpr, const String& stateExpr,
                         const String& observationExpr, const String& feedExpr) {
  ...
  if (mode == PARSE_NOW) fullTEN_p = toTableExprNode(ms_p);
}
  ...
  if (isMS_p == False) {
    if (toTENCalled_p == True)
      ...
      throw(MSSelectionError(
          "MSSelection::getTEN() called before calling MSSelection::toTableExprNode()"));
  }
...
const MeasurementSet* MSSelection::getMS(MSSelectableTable* msLike) {
  ...
  if (msLike->isMS() && (ms == NULL))
    throw(
        MSSelectionError(String("MSSelection::toTableExprNode(MSSelectableTable*): "
  ...
  if (!msLike->isMS() &&
      (
          ...
          //(taqlExpr_p != "") ||
          (polnExpr_p != "") || (stateExpr_p != "") || (feedExpr_p != "")))
    throw(MSSelectionError(String(
        "MSSelection::toTableExprNode(MSSelectableTable*): "
        "Only field-, spw-, scan-, time- and antenna-selection is supported for CalTables")));
  return ms;
}
//
//----------------------------------------------------------------------------
// Method to install the defaul error handler or reset existing
// error handlers. If the pointer to error handler is NULL, this
...
TableExprNode MSSelection::toTableExprNode(MSSelectableTable* msLike) {
  ...
  try {
      ...
      switch (exprOrder_p[i]) {
        case ANTENNA_EXPR: {
          if (antennaExpr_p != "") {
            ...
            node = msAntennaGramParseCommand(*msLike, antennaExpr_p, antenna1IDs_p, antenna2IDs_p,
                                             baselineIDs_p);
          }
          // if(antennaExpr_p != "")
          //   {
        ...
        case SPW_EXPR: {
          if (spwExpr_p != "") {
            ...
            spwIDs_p.resize(0);
            if (msSpwGramParseCommand(msLike->spectralWindow(), msLike->dataDescription(), colAsTEN,
                                      spwExpr_p, spwIDs_p, chanIDs_p, spwDDIDs_p) == 0)
        ...
        case OBSERVATION_EXPR: {
          ...
          if (observationExpr_p != "")
            node = msObservationGramParseCommand(ms, msLike->observation(), colAsTEN,
                                                 observationExpr_p, observationIDs_p);
          break;
        }
        case ARRAY_EXPR: {
          arrayIDs_p.resize(0);
        ...
        case FEED_EXPR: {
          if (feedExpr_p != "") {
            ...
            node = msFeedGramParseCommand(ms, feedExpr_p, feed1IDs_p, feed2IDs_p, feedPairIDs_p);
          }
          break;
        }
        case UVDIST_EXPR: {
          ...
          if (uvDistExpr_p != "" &&
              msUvDistGramParseCommand(ms, uvDistExpr_p, selectedUVRange_p, selectedUVUnits_p) == 0)
        ...
        case POLN_EXPR: {
          ...
          // variable).
          if (polnExpr_p != "") {
            msPolnGramParseCommand(ms, polnExpr_p, node, ddIDs_p, selectedPolMap_p,
                                   selectedSetupMap_p);
          }
          break;
        }
        case STATE_EXPR: {
          ...
          if (stateExpr_p != "" &&
            ...
            if (stateObsModeIDs_p.nelements() == 0)
              throw(MSSelectionStateError(String("No match found for state expression: ") +
                                          stateExpr_p));
          }
          break;
        }
    ...
    if (timeExpr_p != "" &&
        ...
        msTimeGramParseCommand(ms, timeExpr_p, colAsTEN, *mainColInterface, condition,
                               selectedTimesList_p) == 0)
...
void MSSelection::setErrorHandler(const MSExprType type, MSSelectionErrorHandler* mssEH,
  ...
  switch (type) {
    ...
    default:
      throw(MSSelectionError(String("Wrong MSExprType in MSSelection::setErrorHandler()")));
  };
}
...
Bool MSSelection::getSelectedMS(MeasurementSet& selectedMS, const String& outMSName) {
  ...
  if ((ms_p == NULL) || ms_p->isNull())
    throw(
        MSSelectionError("MSSelection::getSelectedMS() called without setting the parent MS.\n"
  ...
  if (type == NO_EXPR)
    for (uInt i = 0; i < exprOrder_p.nelements(); i++) {
      exprIsNull = (antennaExpr_p == "") && (feedExpr_p == "") && (fieldExpr_p == "") &&
                   ...
                   (polnExpr_p == "") && (stateExpr_p == "") && (observationExpr_p == "");
    }
  else {
    switch (type) {
      case ANTENNA_EXPR:
        exprIsNull = (antennaExpr_p == "");
        break;
      case FEED_EXPR:
        exprIsNull = (feedExpr_p == "");
        break;
      case FIELD_EXPR:
        exprIsNull = (fieldExpr_p == "");
        break;
      case SPW_EXPR:
        exprIsNull = (spwExpr_p == "");
        break;
      case SCAN_EXPR:
        exprIsNull = (scanExpr_p == "");
        break;
      case ARRAY_EXPR:
        exprIsNull = (arrayExpr_p == "");
        break;
      case TIME_EXPR:
        exprIsNull = (timeExpr_p == "");
        break;
      case UVDIST_EXPR:
        exprIsNull = (uvDistExpr_p == "");
        break;
      case TAQL_EXPR:
        exprIsNull = (taqlExpr_p == "");
        break;
      case POLN_EXPR:
        exprIsNull = (polnExpr_p == "");
        break;
      case STATE_EXPR:
        exprIsNull = (stateExpr_p == "");
        break;
      case OBSERVATION_EXPR:
        exprIsNull = (observationExpr_p == "");
        break;
      default:;
    };
  }
