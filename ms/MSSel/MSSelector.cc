...
Bool MSSelector::initSelection(const Vector<Int>& dataDescId, Bool reset) {
  ...
  if (ms_p.dataDescription().nrow() <= 1) {
    if (dataDescId.nelements() > 1) {
      os << LogIO::NORMAL
         << "data desc id selection ignored, "
            "there is only one"
         << LogIO::POST;
    }
  } else {
    if (dataDescId.nelements() > 0 && dataDescId(0) != -1) {
      selms_p = selms_p(selms_p.col(MS::columnName(MS::DATA_DESC_ID)).in(dataDescId));
    }
  }
  ...
  if (!constantShape) {
    selms_p = selms_p(selms_p.col(MS::columnName(MS::DATA_DESC_ID)) == ddId(0));
    os << LogIO::WARN
       << "Data shape varies, selected first data desc id"
          " only"
       << LogIO::POST;
  ...
  for (Int i = 0; i < nSpW; i++) {
    ...
    for (Int j = 0; j < nChan; j++) {
      ...
      if (n > 1) {
        ...
        bandwidth_p(j, i) =
            (bandwidth(start, i) + bandwidth(start + incr * j + width - 1, i)) / 2 +
    ...
    switch (fld) {
      ...
      case MSS::SCAN_NUMBER: {
        ...
        if (id.nelements() > 0) {
          ...
          os << LogIO::WARN << "Illegal value for item " << downcase(column) << LogIO::POST;
        }
      } break;
      case MSS::IFR_NUMBER: {
        ...
        if (ifrNum.nelements() > 0) {
          ...
          if (ifrNum.nelements() == 1) {
            selms_p = selms_p((selms_p.col(MS::columnName(MS::ANTENNA1)) * 1000 +
                               selms_p.col(MS::columnName(MS::ANTENNA2))) == ifrNum(0));
          } else {
            selms_p = selms_p((selms_p.col(MS::columnName(MS::ANTENNA1)) * 1000 +
      ...
      case MSS::TIME: {
        ...
        if (range.nelements() == 2) {
          ...
          os << LogIO::WARN
             << "Illegal value for time range: " << "two element numeric vector required"
             << LogIO::POST;
        }
      } break;
      case MSS::TIMES: {
      ...
      case MSS::W: {
        ...
        if (range.nelements() == 2) {
          ...
          os << LogIO::WARN
             << "Illegal value for u, v, w range: " << "two element numeric vector required"
             << LogIO::POST;
        }
      } break;
      case MSS::UVDIST: {
        ...
        if (range.nelements() == 2) {
          ...
          os << LogIO::WARN
             << "Illegal value for uvdist range: " << "two element numeric vector required"
             << LogIO::POST;
        }
      } break;
      case MSS::UNDEFINED:
      default:
        os << LogIO::WARN << "Unrecognized field in input ignored: " << downcase(column)
           << LogIO::POST;
        break;
    }
  }
...
Record MSSelector::getData(const Vector<String>& items, Bool ifrAxis, Int ifrAxisGap, Int inc,
  ...
  for (Int it = 0; it < nItems; it++) {
    ...
    switch (fld) {
        ...
        if (doIfrAxis) {
          ...
          for (Int k = 0; k < nIfr; k++) {
            if (ifrAxis_p(k) >= 0) {
              ...
              if (prefix.length() > 1) {
                sName(k) = std::string(GetStringUpToExcluding(GetStringAfter(antName(ant1), prefix),
                                                              kTerminatingZero));
                sName(k) += "-";
                sName(k) +=
                    GetStringUpToExcluding(GetStringAfter(antName(ant2), prefix), kTerminatingZero);
              } else {
                sName(k) = ifrName(k);
              }
          ...
          for (Int k = 0; k < nT; k++) {
            ...
            if (doLAST || doHA) {
              ...
              if (msc.field().numPoly()(curFieldId) > 0)
                msd_p.setFieldCenter(msc.field().phaseDirMeas(curFieldId, times(k)));
      ...
      case MSS::UNDEFINED:
      default:
        os << LogIO::WARN << "Unrecognized field in input ignored: " << item << LogIO::POST;
        break;
    }
  }
  ...
  for (Int dataType = Observed; dataType <= ObsResidual; dataType++) {
    if (anyEQ(want.column(dataType), True) || (dataType == Observed && keepObs) ||
      ...
      Array<Complex> data;
      if (dataType <= Model) {
        colData.reference(dataType == Observed
                              ? msc.data()
                              : (dataType == Corrected ? msc.correctedData() : msc.modelData()));
        if (colData.isNull()) {
          os << LogIO::WARN << "Requested column doesn't exist" << LogIO::POST;
        } else {
          getAveragedData(data, flag, colData);
          if (doIfrAxis)
            MSSelUtil2<Complex>::reorderData(data, ifrIndex, nIfr, slot, nSlot, Complex());
        }
      }
      String name;
      switch (dataType) {
      ...
      if (average) MSSelUtil2<Complex>::timeAverage(dataflags, data, flags, weights);
  ...
  if (average && wantFlag) {
    ...
    if (dataflags.nelements() == 0) {
      os << LogIO::WARN
         << "Flags not calculated because no DATA derived "
            "item was specified"
         << LogIO::POST;
    }
  }
    ...
    switch (fld) {
      ...
      case MSS::MODEL_DATA: {
        ArrayColumn<Complex>& col =
            (fld == MSS::DATA
                 ? msc.data()
                 : (fld == MSS::CORRECTED_DATA ? msc.correctedData() : msc.modelData()));
        // averaging not supported
        if (chanSel_p(2) > 1) {
          os << LogIO::SEVERE << "Averaging not supported when writing data" << LogIO::POST;
          break;
        }
        if (convert_p) {
      ...
      case MSS::FLOAT_DATA: {
        ...
        if (data.ndim() == 3) {
          if (useSlicer_p)
            msc.floatData().putColumn(slicer_p, data);
          else
  ...
  if (msIter_p) {
    ...
    if (startRow_p > 0 || (more && maxRow_p > 0 && nIterRow > maxRow_p)) {
      ...
      if (more)
        ...
        selms_p = msIter_p->ms();  // put back the original selection at the end
    }
  }
  return more;
}
...
void MSSelector::getAveragedData(Array<Float>& avData, const Array<Bool>& flag,
  ...
  if (convert_p) {
    ...
    LogIO os;
    os << LogIO::WARN
       << "Polarization conversion for FLOAT_DATA "
          "not implemented"
       << LogIO::POST;
  } else if (polIndex_p.nelements() > 0) {
    Int n = polIndex_p.nelements();
