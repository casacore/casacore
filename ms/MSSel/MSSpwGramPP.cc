...
#define register
#include "MSSpwGramPP.ycc"                  // bison output
#include "MSSpwGramPP.lcc"                  // flex output
...
namespace casacore { //# NAMESPACE CASACORE - BEGIN
  ...
  static const char*           strpMSSpwGramPP = 0;
  static Int                   posMSSpwGramPP = 0;
  static string tt="";
  ...
  int msSpwGramPPParseCommand (const MeasurementSet* ms, const String& command) 
  {
    try 
      {
	Int ret;
	MSSpwGramPPrestart (MSSpwGramPPin);
	yy_start = 1;
	strpMSSpwGramPP = command.chars();     // get pointer to command string
	posMSSpwGramPP  = 0;                   // initialize string position
	MSSpwParse parser(ms);               // setup measurement set
	MSSpwParse::thisMSSParser = &parser; // The global pointer to the parser
	parser.reset();
	ret=MSSpwGramPPparse();                // parse command string
	return ret;
      }
    catch (MSSelectionSpwError &x)
      {
	String newMesgs;
	newMesgs = constructMessage(msSpwGramPPPosition(), command);
	x.addMessage(newMesgs);
	throw;
      }
  }
  
  int baseMSSpwGramPPParseCommand (MSSpwParse* parser, const String& command,
  ...
  int msSpwGramPPParseCommand (const MSSpectralWindow& spwSubTable, 
			     ...
			     Vector<Int>& selectedDDIDs) 
  {
    ...
    try
      {
	ret=baseMSSpwGramPPParseCommand(thisParser, command, selectedIDs, selectedChans, selectedDDIDs);
      }
    catch(MSSelectionSpwError &x)
      {
	delete thisParser;
  ...
  int msSpwGramPPParseCommand (const MeasurementSet *ms, 
  ...
  const TableExprNode* msSpwGramPPParseNode()
  {
    return MSSpwParse::node();
  }
  
  void msSpwGramPPParseDeleteNode()
  {
    MSSpwParse::cleanupNode();
  }
  
  //# Give the string position.
  Int& msSpwGramPPPosition()
  {
    return posMSSpwGramPP;
  }
  
  //# Get the next input characters for flex.
  int msSpwGramPPInput (char* buf, int max_size)
  {
    int nr=0;
    while (*strpMSSpwGramPP != 0) {
      if (nr >= max_size) {
	break;                         // get max. max_size char.
      }
      buf[nr++] = *strpMSSpwGramPP++;
    }
    return nr;
  }
  
  void MSSpwGramPPerror (const char*)
  {
    throw (MSSelectionSpwParseError("Spw Expression: Parse error at or near '" +
				    String(MSSpwGramPPtext) + "'"));
  }
