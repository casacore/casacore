...
Vector<Int> MSSpwIndex::matchFrequencyRange(const Float f0, const Float f1, Bool approx,
  ...
  for (Int n = 0; n < nSpwRows; n++) {
    ...
    // cout << chanFreqList[0] << " " << chanFreqList[nChan-1] << " " << f0 << " " << f1 << " " <<
    // f3 << " " << maxChanWidth << std::endl;
    ...
    switch (mode) {
      ...
      default: {
        throw(MSSelectionSpwError(
            "Internal error: Unknown mode in MSSpwIndex::matchFrequencyRange()"));
      }
    }
    if (Found) {
      //
        ...
        if ((freqList(j + 3) == static_cast<int>(MSSpwIndex::MSSPW_INDEX)) ||
          ...
          if (stop == start) {
            ...
            if (stop >= numChans(spw(i))) {
              Mesg << "Spot-channel " << stop << " out of range for SPW " << spw(i)
                   << " (valid range 0~" << numChans(spw(i)) - 1 << ")."
            ...
            if (stop >= numChans(spw(i))) {
              //			  std::ostringstream Mesg;
              Mesg << "Channel " << stop << " out of range for SPW " << spw(i) << " (valid range 0~"
                   << numChans(spw(i)) - 1 << ")."
                   << " Limiting it to be within the available range.";
              //			  throw(MSSelectionSpwError(Mesg.str()));
              log_l << Mesg.str() << LogIO::WARN << LogIO::POST;
...
Vector<Int> MSSpwIndex::convertToSpwIndex(const Vector<Float>& freqList, Int& nFSpec) {
  ...
  if (nFList > 0) {
    ...
    throw(
        MSSelectionSpwError("Internal error. Contact CASA manager.\n"
