...
// Constructor
//
MSTimeParse::MSTimeParse() : MSParse(), colName(MS::columnName(MS::TIME)) {
  defaultYear = defaultMonth = defaultDay = defaultHour = defaultMinute = defaultSeconds =
      defaultFractionalSec = -1;
  defaultExposure = 1.0;
...
const TableExprNode* MSTimeParse::selectTimeRange(const MEpoch& lowboundTime,
  ...
  if (!edgeInclusive) {
    ...
    condition =
        (((columnAsTEN_p > lowerBound) || (abs(columnAsTEN_p - lowerBound) < edgeWidth_l)) &&
...
//-------------------------------------------------------------------
//
const TableExprNode* MSTimeParse::node() { return node_p; }
//
//-------------------------------------------------------------------
//
