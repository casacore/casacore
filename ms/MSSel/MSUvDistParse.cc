...
MSUvDistParse::MSUvDistParse(const MeasurementSet* ms) : MSParse(ms, "UvDist") {
  ...
  if (!doSlow) {
    ...
    if (wavelengthUnit) {
      ...
      for (Int i = 0; i < nDDIDRows; i++) {
        ...
        TableExprNode pickUVWDist =
            ((ms()->col(DATA_DESC_ID) == i) &&
             ((uvwDist >= scaledStartPoint) && (uvwDist <= scaledEndPoint)));
        if (condition.isNull())
          condition = pickUVWDist;
        else
          condition = condition || pickUVWDist;
      }
    } else {
