...
MSAntenna::MSAntenna(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSAntenna(String &, TableOption) - "
...
MSAntenna::MSAntenna(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSAntenna(String &, String &, TableOption) - "
                  "table is not a valid MSAntenna"));
}
...
MSAntenna::MSAntenna(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSAntenna(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSAntenna"));
}
...
MSAntenna::MSAntenna(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSAntenna(const Table &) - "
                  "table is not a valid MSAntenna"));
}
...
MSAntenna::MSAntenna(const MSAntenna &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSAntenna(const MSAntenna &) - "
                    "table is not a valid MSAntenna"));
}
...
MSTableMaps MSAntenna::initMaps() {
  ...
  // DISH_DIAMETER
  colMapDef(maps, DISH_DIAMETER, "DISH_DIAMETER", TpDouble, "Physical diameter of dish", "m", "");
  // FLAG_ROW
  colMapDef(maps, FLAG_ROW, "FLAG_ROW", TpBool, "Flag for this row", "", "");
  // MOUNT
  colMapDef(maps, MOUNT, "MOUNT", TpString, "Mount type e.g. alt-az, equatorial, etc.", "", "");
  // NAME
  colMapDef(maps, NAME, "NAME", TpString, "Antenna name, e.g. VLA22, CA03", "", "");
  // OFFSET
  colMapDef(maps, OFFSET, "OFFSET", TpArrayDouble, "Axes offset of mount to FEED REFERENCE point",
            "m", "Position");
  // POSITION
  colMapDef(maps, POSITION, "POSITION", TpArrayDouble, "Antenna X,Y,Z phase reference position",
            "m", "Position");
  // STATION
  colMapDef(maps, STATION, "STATION", TpString, "Station (antenna pad) name", "", "");
  // TYPE
  colMapDef(maps, TYPE, "TYPE", TpString, "Antenna type (e.g. SPACE-BASED)", "", "");

  // Optional columns follow
  // MEAN_ORBIT
  colMapDef(maps, MEAN_ORBIT, "MEAN_ORBIT", TpArrayDouble, "Mean Keplerian elements", "", "");
  // ORBIT_ID
  colMapDef(maps, ORBIT_ID, "ORBIT_ID", TpInt, "index into ORBIT table (ignore if<0)", "", "");
  // PHASED_ARRAY_ID
  colMapDef(maps, PHASED_ARRAY_ID, "PHASED_ARRAY_ID", TpInt, "index into PHASED_ARRAY table", "",
            "");
...
MSAntenna MSAntenna::referenceCopy(const String &newTableName,
  ...
  return MSAntenna(MSTable<MSAntennaEnums>::referenceCopy(newTableName, writableColumns));
}
