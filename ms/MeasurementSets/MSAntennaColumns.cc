...
MSAntennaColumns::MSAntennaColumns(const MSAntenna& msAntenna) { attach(msAntenna); }

MSAntennaColumns::~MSAntennaColumns() {}

void MSAntennaColumns::attach(const MSAntenna& msAntenna) {
  dishDiameter_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::DISH_DIAMETER));
  flagRow_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::FLAG_ROW));
  ...
  position_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::POSITION));
  ...
  offsetMeas_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::OFFSET));
  positionMeas_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::POSITION));
  dishDiameterQuant_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::DISH_DIAMETER));
  offsetQuant_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::OFFSET));
  positionQuant_p.attach(msAntenna, MSAntenna::columnName(MSAntenna::POSITION));
...
void MSAntennaColumns::setPositionRef(MPosition::Types ref) { positionMeas_p.setDescRefCode(ref); }
...
Int64 MSAntennaColumns::matchAntenna(const MPosition& antennaPos, const Quantum<Double>& tolerance,
  ...
  const MPosition::Types refType = MPosition::castType(antennaPos.getRef().getType());
  ...
  if (MPosition::castType(positionMeas().getMeasRef().getType()) != refType) {
    throw(
        AipsError("MSAntennaColumns::matchAntenna(...) - "
  ...
  if (tryRow >= 0) {
    ...
    if (!flagRow()(tr) && matchPosition(tr, antPosInM, tolInM)) {
      return tr;
    }
    if (tr == r - 1) r--;
  }
  while (r > 0) {
    r--;
    if (!flagRow()(r) && matchPosition(r, antPosInM, tolInM)) {
      return r;
    }
  }
  return -1;
