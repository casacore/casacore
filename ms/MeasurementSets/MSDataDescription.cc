...
MSDataDescription::MSDataDescription(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDataDescription(String &, TableOption) - "
...
MSDataDescription::MSDataDescription(const String &tableName, const String &tableDescName,
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDataDescription(String &, String &, TableOption) - "
                  "table is not a valid MSDataDescription"));
}
...
MSDataDescription::MSDataDescription(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDataDescription(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSDataDescription"));
}
...
MSDataDescription::MSDataDescription(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDataDescription(const Table &) - "
                  "table is not a valid MSDataDescription"));
}
...
MSDataDescription::MSDataDescription(const MSDataDescription &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSDataDescription(const MSDataDescription &) - "
                    "table is not a valid MSDataDescription"));
}
...
MSTableMaps MSDataDescription::initMaps() {
  ...
  // FLAG_ROW
  colMapDef(maps, FLAG_ROW, "FLAG_ROW", TpBool, "Flag this row", "", "");
  // LAG_ID
  colMapDef(maps, LAG_ID, "LAG_ID", TpInt, "The lag index", "", "");
  // POLARIZATION_ID
  colMapDef(maps, POLARIZATION_ID, "POLARIZATION_ID", TpInt, "Pointer to polarization table", "",
            "");
  // SPECTRAL_WINDOW_ID
...
MSDataDescription MSDataDescription::referenceCopy(const String &newTableName,
  ...
  return MSDataDescription(
      MSTable<MSDataDescriptionEnums>::referenceCopy(newTableName, writableColumns));
}
