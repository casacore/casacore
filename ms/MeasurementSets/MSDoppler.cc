...
MSDoppler::MSDoppler(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDoppler(String &, TableOption) - "
...
void MSDoppler::addVelDef() {
  ...
  if (tableDesc().isColumn(columnName(TRANSITION_ID))) {
    ...
    if (!tableDesc().isColumn(columnName(VELDEF))) {
      if (!isWritable()) {
        throw(
            AipsError("Missing VELDEF column in MSDoppler table -"
...
MSDoppler::MSDoppler(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDoppler(String &, String &, TableOption) - "
                  "table is not a valid MSDoppler"));
}
...
MSDoppler::MSDoppler(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDoppler(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSDoppler"));
}
...
MSDoppler::MSDoppler(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDoppler(const Table &) - "
                  "table is not a valid MSDoppler"));
}
...
MSDoppler::MSDoppler(const MSDoppler &other)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSDoppler(const MSDoppler &) - "
                  "table is not a valid MSDoppler"));
}
...
MSTableMaps MSDoppler::initMaps() {
  ...
  //
  colMapDef(maps, DOPPLER_ID, "DOPPLER_ID", TpInt, "Doppler tracking id", "", "");
  // SOURCE_ID
  colMapDef(maps, SOURCE_ID, "SOURCE_ID", TpInt, "Pointer to SOURCE table", "", "");
  // TRANSITION_ID
  colMapDef(maps, TRANSITION_ID, "TRANSITION_ID", TpInt,
            "Pointer to list of transitions in SOURCE table", "", "");
  // VELDEF
  colMapDef(maps, VELDEF, "VELDEF", TpDouble, "Velocity Definition for Doppler shift", "m/s",
            "Doppler");
  // PredefinedKeywords
...
MSDoppler MSDoppler::referenceCopy(const String &newTableName,
  ...
  return MSDoppler(MSTable<MSDopplerEnums>::referenceCopy(newTableName, writableColumns));
}
