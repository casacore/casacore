...
MSFeed::MSFeed(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFeed(String &, TableOption) - "
...
MSFeed::MSFeed(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFeed(String &, String &, TableOption) - "
                  "table is not a valid MSFeed"));
}
...
MSFeed::MSFeed(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFeed(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSFeed"));
}
...
MSTableMaps MSFeed::initMaps() {
  ...
  // BEAM_ID
  colMapDef(maps, BEAM_ID, "BEAM_ID", TpInt, "Id for BEAM model", "", "");
  // BEAM_OFFSET
  ...
  // FOCUS_LENGTH
  colMapDef(maps, FOCUS_LENGTH, "FOCUS_LENGTH", TpDouble, "Focus length", "m", "");
  // INTERVAL
  ...
  colMapDef(maps, POSITION, "POSITION", TpArrayDouble,
            "Position of feed relative to feed reference position", "m", "Position");
  // RECEPTOR_ANGLE
...
MSFeed MSFeed::referenceCopy(const String &newTableName,
  ...
  return MSFeed(MSTable<MSFeedEnums>::referenceCopy(newTableName, writableColumns));
}
