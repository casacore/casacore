...
MSFeedColumns::MSFeedColumns(const MSFeed& msFeed) { attach(msFeed); }

MSFeedColumns::~MSFeedColumns() {}

void MSFeedColumns::attach(const MSFeed& msFeed) {
  ...
  numReceptors_p.attach(msFeed, MSFeed::columnName(MSFeed::NUM_RECEPTORS));
  polResponse_p.attach(msFeed, MSFeed::columnName(MSFeed::POL_RESPONSE));
  polarizationType_p.attach(msFeed, MSFeed::columnName(MSFeed::POLARIZATION_TYPE));
  position_p.attach(msFeed, MSFeed::columnName(MSFeed::POSITION));
  receptorAngle_p.attach(msFeed, MSFeed::columnName(MSFeed::RECEPTOR_ANGLE));
  spectralWindowId_p.attach(msFeed, MSFeed::columnName(MSFeed::SPECTRAL_WINDOW_ID));
  time_p.attach(msFeed, MSFeed::columnName(MSFeed::TIME));
  beamOffsetMeas_p.attach(msFeed, MSFeed::columnName(MSFeed::BEAM_OFFSET));
  positionMeas_p.attach(msFeed, MSFeed::columnName(MSFeed::POSITION));
  timeMeas_p.attach(msFeed, MSFeed::columnName(MSFeed::TIME));
  beamOffsetQuant_p.attach(msFeed, MSFeed::columnName(MSFeed::BEAM_OFFSET));
  ...
  receptorAngleQuant_p.attach(msFeed, MSFeed::columnName(MSFeed::RECEPTOR_ANGLE));
  timeQuant_p.attach(msFeed, MSFeed::columnName(MSFeed::TIME));
  attachOptionalCols(msFeed);
}

void MSFeedColumns::attachOptionalCols(const MSFeed& msFeed) {
...
void MSFeedColumns::setDirectionRef(MDirection::Types ref) { beamOffsetMeas_p.setDescRefCode(ref); }
...
Int64 MSFeedColumns::matchFeed(Quantum<Double>& newTimeQ, Quantum<Double>& newIntervalQ, Int antId,
  ...
  while (r > 0) {
    ...
    if (!ignore) {
      ...
      if (antennaId()(r) == antId && feedId()(r) == fId && spectralWindowId()(r) == spwId &&
        ...
        for (Int i = 0; i < numRec; ++i) {  // compare all receptors
          if (!(beamOffsetQuant()(r)(IPosition(2, 0, i)).getValue(d) ==
                ...
                beamOffsetQuant()(r)(IPosition(2, 1, i)).getValue(d) ==
                    beamOffsetQ(IPosition(2, 1, i)).getValue(d) &&
