...
MSField::MSField(const String& tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSField(String &, TableOption) - "
...
MSField::MSField(const String& tableName, const String& tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSField(String &, String &, TableOption) - "
                  "table is not a valid MSField"));
}
...
MSField::MSField(SetupNewTable& newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSField(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSField"));
}
...
MSField::MSField(const MSField& other) : MSTable<MSFieldEnums>(other), hasBeenDestroyed_p(False) {
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSField(const MSField &) - "
                    "table is not a valid MSField"));
}
...
MSTableMaps MSField::initMaps() {
  ...
  colMapDef(maps, DELAY_DIR, "DELAY_DIR", TpArrayDouble,
            ...
            "rad", "Direction");
  // EPHEMERIS_ID
  colMapDef(maps, EPHEMERIS_ID, "EPHEMERIS_ID", TpInt, "Ephemeris id, pointer to EPHEMERIS table",
            "", "");
  // FLAG_ROW
  colMapDef(maps, FLAG_ROW, "FLAG_ROW", TpBool, "Row Flag", "", "");
  // NAME
  colMapDef(maps, NAME, "NAME", TpString, "Name of this field", "", "");
  // NUM_POLY
  colMapDef(maps, NUM_POLY, "NUM_POLY", TpInt, "Polynomial order of _DIR columns", "", "");
  // PHASE_DIR
  colMapDef(maps, PHASE_DIR, "PHASE_DIR", TpArrayDouble,
            "Direction of phase center (e.g. RA, DEC).", "rad", "Direction");
  // REFERENCE_DIR
  ...
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble, "Time origin for direction and rate", "s", "Epoch");
