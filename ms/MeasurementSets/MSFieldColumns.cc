...
MSFieldColumns::MSFieldColumns(const MSField& msField) { attach(msField); }
...
void MSFieldColumns::attach(const MSField& msField) {
  ...
  referenceDir_p.attach(msField, MSField::columnName(MSField::REFERENCE_DIR));
  ...
  referenceDirMeas_p.attach(msField, MSField::columnName(MSField::REFERENCE_DIR));
...
MDirection MSFieldColumns::delayDirMeas(rownr_t row, Double interTime) const {
  ...
  if (npoly > 0) {
    ...
    return extractDirMeas(vecDir(0), measCometIndex(row), interTime, timeMeas()(row));
  }
}
  ...
  if (measCometsV_p.size() > 0) {
    ...
    if (index >= 0) {
      ...
      if (!measCometsV_p(index)->get(mvpos, interMJD)) {
        std::stringstream ss;
        ss << "MSFieldColumns::rho(...) - No valid ephemeris entry for MJD " << setprecision(11)
           << interMJD << " for field " << row;
        throw(AipsError(ss.str()));
      }
...
Int64 MSFieldColumns::matchDirection(const MDirection& referenceDirection,
  ...
  if (tryRow >= 0) {
    ...
    if (tr >= r) {
      throw(
          AipsError("MSFieldColumns::matchDirection(...) - "
                    "row " +
                    std::to_string(tr) + " you suggest is too big"));
    }
    if (!flagRow()(tr) && numPoly()(tr) == 0) {
  ...
  if (index < 0) {  // no ephemeris available
    ...
    if (!measCometsV_p(index)->get(xmvpos, interMJD)) {
      ...
      ss << "MSFieldColumns::extractDirMeas(...) - No valid ephemeris entry for MJD "
         << setprecision(11) << interMJD << " in ephemeris "
         << measCometsV_p(index)->getTablePath();
      throw(AipsError(ss.str()));
    }
