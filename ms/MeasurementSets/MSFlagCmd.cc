...
MSFlagCmd::MSFlagCmd(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFlagCmd(String &, TableOption) - "
...
MSFlagCmd::MSFlagCmd(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFlagCmd(String &, String &, TableOption) - "
                  "table is not a valid MSFlagCmd"));
}
...
MSFlagCmd::MSFlagCmd(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFlagCmd(SetupNewTable &, uInt, Bool) - "
                  "table is not a valid MSFlagCmd"));
}
...
MSFlagCmd::MSFlagCmd(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFlagCmd(const Table &) - "
                  "table is not a valid MSFlagCmd"));
}
...
MSFlagCmd::MSFlagCmd(const MSFlagCmd &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSFlagCmd(const MSFlagCmd &) - "
                    "table is not a valid MSFlagCmd"));
}
...
MSTableMaps MSFlagCmd::initMaps() {
  ...
  // APPLIED
  colMapDef(maps, APPLIED, "APPLIED", TpBool, "True if flag has been applied to main table", "",
            "");
  // COMMAND
  colMapDef(maps, COMMAND, "COMMAND", TpString, "Flagging command", "", "");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble, "Time interval for which this flag is valid", "s",
            "");
  // LEVEL
  colMapDef(maps, LEVEL, "LEVEL", TpInt, "Flag level - revision level ", "", "");
  // REASON
  colMapDef(maps, REASON, "REASON", TpString, "Flag reason", "", "");
  // SEVERITY
  colMapDef(maps, SEVERITY, "SEVERITY", TpInt, "Severity code (0-10) ", "", "");
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble, "Midpoint of interval for which this flag is valid", "s",
            "Epoch");
  // TYPE
  colMapDef(maps, TYPE, "TYPE", TpString, "Type of flag (FLAG or UNFLAG)", "", "");
...
MSFlagCmd MSFlagCmd::referenceCopy(const String &newTableName,
  ...
  return MSFlagCmd(MSTable<MSFlagCmdEnums>::referenceCopy(newTableName, writableColumns));
}
