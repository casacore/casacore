...
MSFreqOffset::MSFreqOffset(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFreqOffset(String &, TableOption) - "
...
MSFreqOffset::MSFreqOffset(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFreqOffset(String &, String &, TableOption) - "
                  "table is not a valid MSFreqOffset"));
}
...
MSFreqOffset::MSFreqOffset(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFreqOffset(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSFreqOffset"));
}
...
MSFreqOffset::MSFreqOffset(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSFreqOffset(const Table &) - "
                  "table is not a valid MSFreqOffset"));
}
...
MSFreqOffset::MSFreqOffset(const MSFreqOffset &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSFreqOffset(const MSFreqOffset &) - "
                    "table is not a valid MSFreqOffset"));
}
...
MSTableMaps MSFreqOffset::initMaps() {
  ...
  // ANTENNA1
  colMapDef(maps, ANTENNA1, "ANTENNA1", TpInt, "Antenna1 id", "", "");
  // ANTENNA2
  colMapDef(maps, ANTENNA2, "ANTENNA2", TpInt, "Antenna2 id", "", "");
  // FEED_ID
  colMapDef(maps, FEED_ID, "FEED_ID", TpInt, "Feed id", "", "");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble, "Time interval", "s", "");
  // OFFSET
  colMapDef(maps, OFFSET, "OFFSET", TpDouble, "Frequency offset - antenna based", "Hz", "");
  // SPECTRAL_WINDOW_ID
  colMapDef(maps, SPECTRAL_WINDOW_ID, "SPECTRAL_WINDOW_ID", TpInt, "Spectral window id", "", "");
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble, "Midpoint of interval", "s", "Epoch");
...
MSFreqOffset MSFreqOffset::referenceCopy(const String &newTableName,
  ...
  return MSFreqOffset(MSTable<MSFreqOffsetEnums>::referenceCopy(newTableName, writableColumns));
}
