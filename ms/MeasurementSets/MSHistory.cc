...
MSHistory::MSHistory(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSHistory(String &, TableOption) - "
...
MSHistory::MSHistory(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSHistory(String &, String &, TableOption) - "
                  "table is not a valid MSHistory"));
}
...
MSHistory::MSHistory(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSHistory(SetupNewTable &, uInt, Bool) - "
                  "table is not a valid MSHistory"));
}
...
MSHistory::MSHistory(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSHistory(const Table &) - "
                  "table is not a valid MSHistory"));
}
...
MSHistory::MSHistory(const MSHistory &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSHistory(const MSHistory &) - "
                    "table is not a valid MSHistory"));
}
...
MSTableMaps MSHistory::initMaps() {
  ...
  // APPLICATION
  colMapDef(maps, APPLICATION, "APPLICATION", TpString, "Application name", "", "");
  // APP_PARAMS
  colMapDef(maps, APP_PARAMS, "APP_PARAMS", TpArrayString, "Application parameters", "", "");
  // CLI_COMMAND
  colMapDef(maps, CLI_COMMAND, "CLI_COMMAND", TpArrayString, "CLI command sequence", "", "");
  // MESSAGE
  colMapDef(maps, MESSAGE, "MESSAGE", TpString, "Log message", "", "");
  // OBJECT_ID
  colMapDef(maps, OBJECT_ID, "OBJECT_ID", TpInt, "Originating ObjectID", "", "");
  // OBSERVATION_ID
  colMapDef(maps, OBSERVATION_ID, "OBSERVATION_ID", TpInt,
            "Observation id (index in OBSERVATION table)", "", "");
  // ORIGIN
  colMapDef(maps, ORIGIN, "ORIGIN", TpString, "(Source code) origin from which message originated",
            "", "");
  // PRIORITY
  colMapDef(maps, PRIORITY, "PRIORITY", TpString, "Message priority", "", "");
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble, "Timestamp of message", "s", "Epoch");
  // PredefinedKeywords
...
MSHistory MSHistory::referenceCopy(const String &newTableName,
  ...
  return MSHistory(MSTable<MSHistoryEnums>::referenceCopy(newTableName, writableColumns));
}
