...
MSMainColumns::MSMainColumns(const MeasurementSet& ms) { attach(ms); }
...
void MSMainColumns::attach(const MeasurementSet& ms) {
  ...
  timeMeas_p.attach(ms, MS::columnName(MS::TIME)),
      ...
      uvwQuant_p.attach(ms, MS::columnName(MS::UVW)), attachOptionalCols(ms);
}
...
Vector<String> MSMainColumns::flagCategories() const {
  ...
  DebugAssert(nrow() == 0 || keywords.shape(key)(0) == flagCategory().shape(0)(2), AipsError);
