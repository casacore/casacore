...
MSObservation::MSObservation(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSObservation(String &, TableOption) - "
...
MSObservation::MSObservation(const String &tableName, const String &tableDescName,
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSObservation(String &, String &, TableOption) - "
                  "table is not a valid MSObservation"));
}
...
MSObservation::MSObservation(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSObservation(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSObservation"));
}
...
MSObservation::MSObservation(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSObservation(const Table &) - "
                  "table is not a valid MSObservation"));
}
...
MSObservation::MSObservation(const MSObservation &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSObservation(const MSObservation &) - "
                    "table is not a valid MSObservation"));
}
...
MSTableMaps MSObservation::initMaps() {
  ...
  // LOG
  colMapDef(maps, LOG, "LOG", TpArrayString, "Observing log", "", "");
  // OBSERVER
  colMapDef(maps, OBSERVER, "OBSERVER", TpString, "Name of observer(s)", "", "");
  // PROJECT
  colMapDef(maps, PROJECT, "PROJECT", TpString, "Project identification string", "", "");
  // RELEASE_DATE
  colMapDef(maps, RELEASE_DATE, "RELEASE_DATE", TpDouble, "Release date when data becomes public",
            "s", "Epoch");
  // SCHEDULE
  colMapDef(maps, SCHEDULE, "SCHEDULE", TpArrayString, "Observing schedule", "", "");
  // SCHEDULE_TYPE
  colMapDef(maps, SCHEDULE_TYPE, "SCHEDULE_TYPE", TpString, "Observing schedule type", "", "");
  // TELESCOPE_NAME
  colMapDef(maps, TELESCOPE_NAME, "TELESCOPE_NAME", TpString, "Telescope Name (e.g. WSRT, VLBA)");
  // TIME_RANGE
  colMapDef(maps, TIME_RANGE, "TIME_RANGE", TpArrayDouble, "Start and end of observation", "s",
            "Epoch");
  // PredefinedKeywords
...
MSObservation MSObservation::referenceCopy(const String &newTableName,
  ...
  return MSObservation(MSTable<MSObservationEnums>::referenceCopy(newTableName, writableColumns));
}
