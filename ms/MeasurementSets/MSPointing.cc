...
MSPointing::MSPointing(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPointing(String &, TableOption) - "
...
MSPointing::MSPointing(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPointing(String &, String &, TableOption) - "
                  "table is not a valid MSPointing"));
}
...
MSPointing::MSPointing(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPointing(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSPointing"));
}
...
MSPointing::MSPointing(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPointing(const Table &) - "
                  "table is not a valid MSPointing"));
}
...
MSPointing::MSPointing(const MSPointing &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSPointing(const MSPointing &) - "
                    "table is not a valid MSPointing"));
}
...
MSTableMaps MSPointing::initMaps() {
  ...
  // ANTENNA_ID
  colMapDef(maps, ANTENNA_ID, "ANTENNA_ID", TpInt, "Antenna Id", "", "");
  // DIRECTION
  colMapDef(maps, DIRECTION, "DIRECTION", TpArrayDouble,
            "Antenna pointing direction as polynomial in time", "rad", "Direction");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble, "Time interval", "s", "");
  // NAME
  colMapDef(maps, NAME, "NAME", TpString, "Pointing position name", "", "");
  // NUM_POLY
  colMapDef(maps, NUM_POLY, "NUM_POLY", TpInt, "Series order", "", "");
  // TARGET
  colMapDef(maps, TARGET, "TARGET", TpArrayDouble, "target direction as polynomial in time", "rad",
            "Direction");
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble, "Time interval midpoint", "s", "Epoch");
  // TIME_ORIGIN
  colMapDef(maps, TIME_ORIGIN, "TIME_ORIGIN", TpDouble, "Time origin for direction", "s", "Epoch");
  // TRACKING
  colMapDef(maps, TRACKING, "TRACKING", TpBool, "Tracking flag - True if on position", "", "");
  // ENCODER
  colMapDef(maps, ENCODER, "ENCODER", TpArrayDouble, "Encoder values", "rad", "Direction");
  // ON_SOURCE
  colMapDef(maps, ON_SOURCE, "ON_SOURCE", TpBool, "On source flag", "", "");
  // OVER_THE_TOP
  colMapDef(maps, OVER_THE_TOP, "OVER_THE_TOP", TpBool, "Antenna over the top", "", "");
  // POINTING_MODEL_ID
  colMapDef(maps, POINTING_MODEL_ID, "POINTING_MODEL_ID", TpInt, "Pointing model id", "", "");
  // POINTING_OFFSET
  colMapDef(maps, POINTING_OFFSET, "POINTING_OFFSET", TpArrayDouble,
            "A priori pointing correction as polynomial in time", "rad", "Direction");
  // SOURCE_OFFSET
...
MSPointing MSPointing::referenceCopy(const String &newTableName,
  ...
  return MSPointing(MSTable<MSPointingEnums>::referenceCopy(newTableName, writableColumns));
}
