...
MSPolarizationColumns::MSPolarizationColumns(const MSPolarization& msPolarization) {
  attach(msPolarization);
}

MSPolarizationColumns::~MSPolarizationColumns() {}

void MSPolarizationColumns::attach(const MSPolarization& msPolarization) {
  corrProduct_p.attach(msPolarization, MSPolarization::columnName(MSPolarization::CORR_PRODUCT));
  corrType_p.attach(msPolarization, MSPolarization::columnName(MSPolarization::CORR_TYPE));
  flagRow_p.attach(msPolarization, MSPolarization::columnName(MSPolarization::FLAG_ROW));
...
Int64 MSPolarizationColumns::match(const Vector<Stokes::StokesTypes>& polType, Int64 tryRow) {
  ...
  if (tryRow >= 0) {
    ...
    if (tr >= r) {
      throw(
          AipsError("MSPolarszationColumns::match(...) - "
                    "row " +
                    std::to_string(tr) + " you suggest is too big"));
    }
    if (!flagRow()(tr) && numCorr()(tr) == nCorr && matchCorrType(tr, polInt)) {
      return tr;
    }
    if (tr == r - 1) r--;
  }
  while (r > 0) {
    r--;
    if (!flagRow()(r) && numCorr()(r) == nCorr && matchCorrType(r, polInt)) {
      return r;
    }
  }
  return -1;
