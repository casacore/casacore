...
MSPolarization::MSPolarization(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPolarization(String &, TableOption) - "
...
MSPolarization::MSPolarization(const String &tableName, const String &tableDescName,
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPolarization(String &, String &, TableOption) - "
                  "table is not a valid MSPolarization"));
}
...
MSPolarization::MSPolarization(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPolarization(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSPolarization"));
}
...
MSPolarization::MSPolarization(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSPolarization(const Table &) - "
                  "table is not a valid MSPolarization"));
}
...
MSPolarization::MSPolarization(const MSPolarization &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSPolarization(const MSPolarization &) - "
                    "table is not a valid MSPolarization"));
}
...
MSTableMaps MSPolarization::initMaps() {
  ...
  // NUM_CORR
  colMapDef(maps, NUM_CORR, "NUM_CORR", TpInt, "Number of correlation products", "", "");
...
MSPolarization MSPolarization::referenceCopy(const String &newTableName,
  ...
  return MSPolarization(MSTable<MSPolarizationEnums>::referenceCopy(newTableName, writableColumns));
}
