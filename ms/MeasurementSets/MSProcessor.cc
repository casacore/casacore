...
MSProcessor::MSProcessor(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSProcessor(String &, TableOption) - "
...
MSProcessor::MSProcessor(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSProcessor(String &, String &, TableOption) - "
                  "table is not a valid MSProcessor"));
}
...
MSProcessor::MSProcessor(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSProcessor(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSProcessor"));
}
...
MSProcessor::MSProcessor(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSProcessor(const Table &) - "
                  "table is not a valid MSProcessor"));
}
...
MSProcessor::MSProcessor(const MSProcessor &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSProcessor(const MSProcessor &) - "
                    "table is not a valid MSProcessor"));
}
...
MSTableMaps MSProcessor::initMaps() {
  ...
  //
  colMapDef(maps, MODE_ID, "MODE_ID", TpInt, "Processor mode id", "", "");
  // PASS_ID
  colMapDef(maps, PASS_ID, "PASS_ID", TpInt, "Processor pass number", "", "");
  // TYPE
  colMapDef(maps, TYPE, "TYPE", TpString, "Processor type", "", "");
  // TYPE_ID
  colMapDef(maps, TYPE_ID, "TYPE_ID", TpInt, "Processor type id", "", "");
  // SUB_TYPE
  colMapDef(maps, SUB_TYPE, "SUB_TYPE", TpString, "Processor sub type", "", "");
...
MSProcessor MSProcessor::referenceCopy(const String &newTableName,
  ...
  return MSProcessor(MSTable<MSProcessorEnums>::referenceCopy(newTableName, writableColumns));
}
