...
MSRange::MSRange(const MSRange& other) { operator=(other); }
...
MSRange& MSRange::operator=(const MSRange& other) {
  ...
  ddId_p = other.ddId_p;
  spwId_p.resize(0);
  spwId_p = other.spwId_p;
  polId_p.resize(0);
  polId_p = other.polId_p;
  ...
  for (Int i = 0; i < n; i++) {
    ...
    switch (fld) {
      ...
      case MSS::PHASE_DIR: {
        ...
        phasedir.define("epoch",
                        MDirection::showType(msc.field().phaseDirMeasCol().getMeasRef().getType()));
      ...
      case MSS::REF_FREQUENCY: {
        checkShapes();
        out.define(keyword,
                   msc.spectralWindow().refFrequency().getColumnCells(RowNumbers(spwId_p)));
      } break;
      case MSS::ROWS: {
