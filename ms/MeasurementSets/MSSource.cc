...
MSSource::MSSource(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSource(String &, TableOption) - "
...
MSSource::MSSource(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSource(String &, String &, TableOption) - "
                  "table is not a valid MSSource"));
}
...
MSSource::MSSource(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSource(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSSource"));
}
...
MSSource::MSSource(const Table &table) : MSTable<MSSourceEnums>(table), hasBeenDestroyed_p(False) {
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSource(const Table &) - "
                  "table is not a valid MSSource"));
}
...
MSSource::MSSource(const MSSource &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSSource(const MSSource &) - "
                    "table is not a valid MSSource"));
}
...
MSTableMaps MSSource::initMaps() {
  ...
  colMapDef(maps, CODE, "CODE", TpString,
            ...
            "", "");
  // DIRECTION
  colMapDef(maps, DIRECTION, "DIRECTION", TpArrayDouble, "Direction (e.g. RA, DEC).", "rad",
            "Direction");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble,
            ...
            "s", "");
  // NAME
  colMapDef(maps, NAME, "NAME", TpString, "Name of source as given during observations", "", "");
  // NUM_LINES
  colMapDef(maps, NUM_LINES, "NUM_LINES", TpInt, "Number of spectral lines", "", "");
  // POSITION
  colMapDef(maps, POSITION, "POSITION", TpArrayDouble, "Position (e.g. for solar system objects",
            "m", "Position");
  // PROPER_MOTION
  colMapDef(maps, PROPER_MOTION, "PROPER_MOTION", TpArrayDouble, "Proper motion", "rad/s", "");
  // PULSAR_ID
  colMapDef(maps, PULSAR_ID, "PULSAR_ID", TpInt, "Pulsar Id, pointer to pulsar table", "", "");
  // REST_FREQUENCY
  colMapDef(maps, REST_FREQUENCY, "REST_FREQUENCY", TpArrayDouble, "Line rest frequency", "Hz",
            "Frequency");
  // SOURCE_ID
  colMapDef(maps, SOURCE_ID, "SOURCE_ID", TpInt, "Source id", "", "");
  // SOURCE_MODEL
  colMapDef(maps, SOURCE_MODEL, "SOURCE_MODEL", TpRecord, "Component Source Model", "", "");
  // SPECTRAL_WINDOW_ID
  colMapDef(maps, SPECTRAL_WINDOW_ID, "SPECTRAL_WINDOW_ID", TpInt,
            "ID for this spectral window setup", "", "");
  // SYSVEL
  colMapDef(maps, SYSVEL, "SYSVEL", TpArrayDouble, "Systemic velocity at reference", "m/s",
            "Radialvelocity");
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble,
            ...
            "s", "Epoch");
  // TRANSITION
  colMapDef(maps, TRANSITION, "TRANSITION", TpArrayString, "Line Transition name", "", "");
  // PredefinedKeywords
...
MSSource MSSource::referenceCopy(const String &newTableName,
  ...
  return MSSource(MSTable<MSSourceEnums>::referenceCopy(newTableName, writableColumns));
}
