...
class MSSpWindowColumns {
 public:
  ...
  ScalarQuantColumn<Double>& totalBandwidthQuant() { return totalBandwidthQuant_p; }
  // </group>
  ...
  const ArrayQuantColumn<Double>& chanFreqQuant() const { return chanFreqQuant_p; }
  const ArrayMeasColumn<MFrequency>& chanFreqMeas() const { return chanFreqMeas_p; }
  const ArrayColumn<Double>& chanWidth() const { return chanWidth_p; }
  const ArrayQuantColumn<Double>& chanWidthQuant() const { return chanWidthQuant_p; }
  const ArrayColumn<Double>& effectiveBW() const { return effectiveBW_p; }
  const ArrayQuantColumn<Double>& effectiveBWQuant() const { return effectiveBWQuant_p; }
  ...
  const ScalarQuantColumn<Double>& refFrequencyQuant() const { return refFrequencyQuant_p; }
  const ScalarMeasColumn<MFrequency>& refFrequencyMeas() const { return refFrequencyMeas_p; }
  const ArrayColumn<Double>& resolution() const { return resolution_p; }
  const ArrayQuantColumn<Double>& resolutionQuant() const { return resolutionQuant_p; }
  const ScalarColumn<Double>& totalBandwidth() const { return totalBandwidth_p; }
  const ScalarQuantColumn<Double>& totalBandwidthQuant() const { return totalBandwidthQuant_p; }
  // </group>
  ...
  RowNumbers allMatchedSpw(const MFrequency& refFreq, uInt nChan, const Quantum<Double>& bandwidth,
