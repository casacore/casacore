...
MSSpectralWindow::MSSpectralWindow(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSpectralWindow(String &, TableOption) - "
...
MSSpectralWindow::MSSpectralWindow(const String &tableName, const String &tableDescName,
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSpectralWindow(String &, String &, TableOption) - "
                  "table is not a valid MSSpectralWindow"));
}
...
MSSpectralWindow::MSSpectralWindow(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSpectralWindow(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSSpectralWindow"));
}
...
MSSpectralWindow::MSSpectralWindow(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSpectralWindow(const Table &) - "
                  "table is not a valid MSSpectralWindow"));
}
...
MSSpectralWindow::MSSpectralWindow(const MSSpectralWindow &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSSpectralWindow(const MSSpectralWindow &) - "
                    "table is not a valid MSSpectralWindow"));
}
...
MSTableMaps MSSpectralWindow::initMaps() {
  ...
  // ASSOC_SPW_ID
  colMapDef(maps, ASSOC_SPW_ID, "ASSOC_SPW_ID", TpArrayInt, "Associated spectral window id", "",
            "");
  // BBC_NO
  colMapDef(maps, BBC_NO, "BBC_NO", TpInt, "Baseband converter number", "", "");
  // BBC_SIDEBAND
  colMapDef(maps, BBC_SIDEBAND, "BBC_SIDEBAND", TpInt, "BBC sideband", "", "");
  // CHAN_FREQ
  colMapDef(maps, CHAN_FREQ, "CHAN_FREQ", TpArrayDouble,
            "Center frequencies for each channel in the data matrix", "Hz", "Frequency");
  // CHAN_WIDTH
  colMapDef(maps, CHAN_WIDTH, "CHAN_WIDTH", TpArrayDouble, "Channel width for each channel", "Hz",
            "");
  // DOPPLER_ID
  colMapDef(maps, DOPPLER_ID, "DOPPLER_ID", TpInt, "Doppler Id, points to DOPPLER table", "", "");
  // EFFECTIVE_BW
  colMapDef(maps, EFFECTIVE_BW, "EFFECTIVE_BW", TpArrayDouble,
            "Effective noise bandwidth of each channel", "Hz", "");
  // FLAG_ROW
  colMapDef(maps, FLAG_ROW, "FLAG_ROW", TpBool, "Row flag", "", "");
  // FREQ_GROUP
  colMapDef(maps, FREQ_GROUP, "FREQ_GROUP", TpInt, "Frequency group", "", "");
  // FREQ_GROUP_NAME
  colMapDef(maps, FREQ_GROUP_NAME, "FREQ_GROUP_NAME", TpString, "Frequency group name", "", "");
  // IF_CONV_CHAIN
  colMapDef(maps, IF_CONV_CHAIN, "IF_CONV_CHAIN", TpInt, "The IF conversion chain number", "", "");
  // MEAS_FREQ_REF
  colMapDef(maps, MEAS_FREQ_REF, "MEAS_FREQ_REF", TpInt, "Frequency Measure reference", "", "");
  // NAME
  colMapDef(maps, NAME, "NAME", TpString, "Spectral window name", "", "");
  // NET_SIDEBAND
  colMapDef(maps, NET_SIDEBAND, "NET_SIDEBAND", TpInt, "Net sideband", "", "");
  // NUM_CHAN
  colMapDef(maps, NUM_CHAN, "NUM_CHAN", TpInt, "Number of spectral channels", "", "");
  // RECEIVER_ID
  colMapDef(maps, RECEIVER_ID, "RECEIVER_ID", TpInt, "Receiver Id for this spectral window", "",
            "");
  // REF_FREQUENCY
  colMapDef(maps, REF_FREQUENCY, "REF_FREQUENCY", TpDouble, "The reference frequency", "Hz",
            "Frequency");
  // RESOLUTION
  colMapDef(maps, RESOLUTION, "RESOLUTION", TpArrayDouble,
            "The effective noise bandwidth for each channel", "Hz", "");
  // TOTAL_BANDWIDTH
...
MSSpectralWindow MSSpectralWindow::referenceCopy(const String &newTableName,
  ...
  return MSSpectralWindow(
      MSTable<MSSpectralWindowEnums>::referenceCopy(newTableName, writableColumns));
}
