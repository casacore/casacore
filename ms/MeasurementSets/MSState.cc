...
MSState::MSState(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSState(String &, TableOption) - "
...
MSState::MSState(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSState(String &, String &, TableOption) - "
                  "table is not a valid MSState"));
}
...
MSState::MSState(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSState(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSState"));
}
...
MSState::MSState(const MSState &other) : MSTable<MSStateEnums>(other), hasBeenDestroyed_p(False) {
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSState(const MSState &) - "
                    "table is not a valid MSState"));
}
...
MSTableMaps MSState::initMaps() {
  ...
  // CAL
  colMapDef(maps, CAL, "CAL", TpDouble, "Noise calibration temperature", "K", "");
  // FLAG_ROW
  colMapDef(maps, FLAG_ROW, "FLAG_ROW", TpBool, "Row flag", "", "");
  // LOAD
  colMapDef(maps, LOAD, "LOAD", TpDouble, "Load temperature", "K", "");
  // OBS_MODE
  colMapDef(maps, OBS_MODE, "OBS_MODE", TpString, "Observing mode, e.g., OFF_SPECTRUM", "", "");
  // REF
  colMapDef(maps, REF, "REF", TpBool, "True for a reference observation", "", "");
  // SIG
  colMapDef(maps, SIG, "SIG", TpBool, "True for a source observation", "", "");
  // SUB_SCAN
  colMapDef(maps, SUB_SCAN, "SUB_SCAN", TpInt, "Sub scan number, relative to scan number", "", "");
...
MSState MSState::referenceCopy(const String &newTableName,
  ...
  return MSState(MSTable<MSStateEnums>::referenceCopy(newTableName, writableColumns));
}
