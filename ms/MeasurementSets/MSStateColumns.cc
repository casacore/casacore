...
MSStateColumns::MSStateColumns(const MSState& msState) { attach(msState); }
...
Int64 MSStateColumns::matchState(const Quantum<Double>& stateCalQ,
  ...
  if (tryRow >= 0) {
    ...
    if (tr >= r) {
      throw(
          AipsError("MSStateColumns::matchState(...) - "
                    "row " +
                    std::to_string(tr) + " you suggest is too big"));
    }
    if (!flagRow()(tr) && std::fabs(calQuant()(tr).getValue(k) - calInK) < tolInK &&
        std::fabs(loadQuant()(tr).getValue(k) - loadInK) < tolInK &&
        obsMode()(tr) == stateObsMode && ref()(tr) == stateRef && sig()(tr) == stateSig &&
  ...
  while (r > 0) {
    r--;
    if (!flagRow()(r) && std::fabs(calQuant()(r).getValue(k) - calInK) < tolInK &&
        std::fabs(loadQuant()(r).getValue(k) - loadInK) < tolInK && obsMode()(r) == stateObsMode &&
        ref()(r) == stateRef && sig()(r) == stateSig && subScan()(r) == stateSubScan) {
      return r;
    }
  }
  return -1;
