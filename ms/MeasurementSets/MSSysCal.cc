...
MSSysCal::MSSysCal(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSysCal(String &, TableOption) - "
...
MSSysCal::MSSysCal(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSysCal(String &, String &, TableOption) - "
                  "table is not a valid MSSysCal"));
}
...
MSSysCal::MSSysCal(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSysCal(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSSysCal"));
}
...
MSSysCal::MSSysCal(const Table &table) : MSTable<MSSysCalEnums>(table), hasBeenDestroyed_p(False) {
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSSysCal(const Table &) - "
                  "table is not a valid MSSysCal"));
}
...
MSSysCal::MSSysCal(const MSSysCal &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSSysCal(const MSSysCal &) - "
                    "table is not a valid MSSysCal"));
}
...
MSTableMaps MSSysCal::initMaps() {
  ...
  // ANTENNA_ID
  colMapDef(maps, ANTENNA_ID, "ANTENNA_ID", TpInt, "ID of antenna in this array", "", "");
  // FEED_ID
  colMapDef(maps, FEED_ID, "FEED_ID", TpInt, "Feed id", "", "");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble,
            "Interval for which this set of parameters is accurate", "s", "");
  // SPECTRAL_WINDOW_ID
  ...
  colMapDef(maps, PHASE_DIFF, "PHASE_DIFF", TpFloat,
            "Phase difference between receptor 2 and receptor 1", "rad", "");
  // PHASE_DIFF_FLAG
  colMapDef(maps, PHASE_DIFF_FLAG, "PHASE_DIFF_FLAG", TpBool, "Flag for PHASE_DIFF", "", "");
  // TANT
  colMapDef(maps, TANT, "TANT", TpArrayFloat, "Antenna temperature for each receptor", "K", "");
  // TANT_FLAG
  colMapDef(maps, TANT_FLAG, "TANT_FLAG", TpBool, "Flag for TANT", "", "");
  // TANT_SPECTRUM
  ...
  // TANT_TSYS_FLAG
  colMapDef(maps, TANT_TSYS_FLAG, "TANT_TSYS_FLAG", TpBool, "Flag for TANT_TSYS", "", "");
  // TANT_TSYS_SPECTRUM
  colMapDef(maps, TANT_TSYS_SPECTRUM, "TANT_TSYS_SPECTRUM", TpArrayFloat,
            ...
            "", "");
  // TCAL
  colMapDef(maps, TCAL, "TCAL", TpArrayFloat, "Calibration temperature for each receptor", "K", "");
  // TCAL_FLAG
  colMapDef(maps, TCAL_FLAG, "TCAL_FLAG", TpBool, "Flag for TCAL", "", "");
  // TCAL_SPECTRUM
  colMapDef(maps, TCAL_SPECTRUM, "TCAL_SPECTRUM", TpArrayFloat,
            "Calibration temperature for each channel and receptor", "K", "");
  // TRX
  colMapDef(maps, TRX, "TRX", TpArrayFloat, "Receiver temperature for each of the two receptors",
            "K", "");
  // TRX_FLAG
  colMapDef(maps, TRX_FLAG, "TRX_FLAG", TpBool, "Flag for TRX", "", "");
  // TRX_SPECTRUM
  colMapDef(maps, TRX_SPECTRUM, "TRX_SPECTRUM", TpArrayFloat,
            "Receiver temperature for each channel and receptor", "K", "");
  // TSKY
  colMapDef(maps, TSKY, "TSKY", TpArrayFloat, "Sky temperature for each of the two receptors", "K",
            "");
  // TSKY_FLAG
  colMapDef(maps, TSKY_FLAG, "TSKY_FLAG", TpBool, "Flag for TSKY", "", "");
  // TSKY_SPECTRUM
  colMapDef(maps, TSKY_SPECTRUM, "TSKY_SPECTRUM", TpArrayFloat,
            "Sky temperature for each channel and receptor", "K", "");
  // TSYS
  colMapDef(maps, TSYS, "TSYS", TpArrayFloat, "System temp. for each of the two receptors", "K",
            "");
  // TSYS_FLAG
  colMapDef(maps, TSYS_FLAG, "TSYS_FLAG", TpBool, "Flag for TSYS", "", "");
  // TSYS_SPECTRUM
...
MSSysCal MSSysCal::referenceCopy(const String &newTableName,
  ...
  return MSSysCal(MSTable<MSSysCalEnums>::referenceCopy(newTableName, writableColumns));
}
