...
class MSTable : public Table {
 public:
  ...
  const String& unit(ColEnum which) const { return unit(columnName(which)); }
  // </group>
  ...
  // the ColumnDesc option (Fixed, Undefined, Direct)
  // For Measure columns you can define a variable reference column.
  static void addColumnToDesc(MSTableMaps&, ColEnum which, const IPosition& shape,
                              ColumnDesc::Option option, const String& refCol = "");
 ...
 protected:
  ...
  static void colMapDef(MSTableMaps& maps, ColEnum col, const String& colName, DataType colType,
                        const String& colComment, const String& colUnit = "",
                        const String& colMeasureType = "");

  // Define an entry in the keyword maps
  static void keyMapDef(MSTableMaps& maps, KeyEnum key, const String& keyName, DataType keyType,
  ...
  // Convert a ColEnum to the actual column name.
  inline static const String& columnName(const MSTableMaps& maps, ColEnum which) {
    return maps.columnMap_p.at(which);
  }

  inline static DataType columnDataType(const MSTableMaps& maps, ColEnum which) {
    return DataType(maps.colDTypeMap_p.at(which));
  }

  inline static const String& columnStandardComment(const MSTableMaps& maps, ColEnum which) {
    return maps.colCommentMap_p.at(which);
  }

  inline static const String& columnUnit(const MSTableMaps& maps, ColEnum which) {
    return maps.colUnitMap_p.at(which);
  }

  inline static const String& columnMeasureType(const MSTableMaps& maps, ColEnum which) {
    return maps.colMeasureTypeMap_p.at(which);
  }

  inline static const String& keywordName(const MSTableMaps& maps, KeyEnum which) {
    return maps.keywordMap_p.at(which);
  }

  inline static DataType keywordDataType(const MSTableMaps& maps, KeyEnum which) {
    return DataType(maps.keyDTypeMap_p.at(which));
  }

  inline static const String& keywordStandardComment(const MSTableMaps& maps, KeyEnum which) {
