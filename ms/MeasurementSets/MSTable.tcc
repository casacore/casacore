...
template <class MSEnum>
MSTable<MSEnum>::MSTable(const String& tableName, Table::TableOption option)
    : Table(tableName, option) {}

template <class MSEnum>
MSTable<MSEnum>::MSTable(const String& tableName, const TableLock& lockOptions,
                         Table::TableOption option)
    : Table(tableName, lockOptions, option) {}

template <class MSEnum>
MSTable<MSEnum>::MSTable(const String& tableName, const String& tableDescName,
                         Table::TableOption option)
    : Table(tableName, tableDescName, option) {}

template <class MSEnum>
MSTable<MSEnum>::MSTable(const String& tableName, const String& tableDescName,
                         const TableLock& lockOptions, Table::TableOption option)
...
template <class MSEnum>
MSTable<MSEnum>::MSTable(const Table& table) : Table(table) {}

template <class MSEnum>
MSTable<MSEnum>::MSTable(const MSTable<MSEnum>& other) : Table(other) {}

template <class MSEnum>
MSTable<MSEnum>::~MSTable() {}

template <class MSEnum>
MSTable<MSEnum>& MSTable<MSEnum>::operator=(const MSTable<MSEnum>& other) {
...
const String& MSTable<MSEnum>::unit(const String& which) const {
  return tableDesc().columnDesc(which).keywordSet().asArrayString("QuantumUnits")(IPosition(1, 0));
}
...
void MSTable<MSEnum>::addColumnToDesc(TableDesc& td, MSTable<MSEnum>::ColEnum which, Int ndim,
  ...
  MSTableImpl::addColumnToDesc(td, columnName(which), columnDataType(which),
                               ...
                               columnMeasureType(which), ndim, IPosition(), 0, refCol);
}
...
void MSTable<MSEnum>::addColumnToDesc(TableDesc& td, MSTable<MSEnum>::ColEnum which,
  ...
  MSTableImpl::addColumnToDesc(td, columnName(which), columnDataType(which),
                               columnStandardComment(which), columnUnit(which),
                               columnMeasureType(which), -1, shape, option, refCol);
}
...
void MSTable<MSEnum>::addColumnToDesc(MSTableMaps& maps, MSTable<MSEnum>::ColEnum which, Int ndim,
  ...
  MSTableImpl::addColumnToDesc(maps.requiredTD_p, columnName(maps, which),
                               ...
                               columnUnit(maps, which), columnMeasureType(maps, which), ndim,
                               IPosition(), 0, refCol);
}
...
void MSTable<MSEnum>::addColumnToDesc(MSTableMaps& maps, MSTable<MSEnum>::ColEnum which,
  ...
  MSTableImpl::addColumnToDesc(maps.requiredTD_p, columnName(maps, which),
                               columnDataType(maps, which), columnStandardComment(maps, which),
                               columnUnit(maps, which), columnMeasureType(maps, which), -1, shape,
                               option, refCol);
}
...
void MSTable<MSEnum>::addKeyToDesc(MSTableMaps& maps, MSTable<MSEnum>::KeyEnum key) {
  MSTableImpl::addKeyToDesc(maps.requiredTD_p, keywordName(maps, key), keywordDataType(maps, key),
...
template <class MSEnum>
void MSTable<MSEnum>::colMapDef(MSTableMaps& maps, MSTable<MSEnum>::ColEnum col,
                                const String& colName, DataType colType, const String& colComment,
  ...
  MSTableImpl::colMapDef(maps.columnMap_p, maps.colDTypeMap_p, maps.colCommentMap_p,
                         maps.colUnitMap_p, maps.colMeasureTypeMap_p, col, colName, colType,
                         colComment, colUnit, colMeasureType);
}

template <class MSEnum>
void MSTable<MSEnum>::keyMapDef(MSTableMaps& maps, MSTable<MSEnum>::KeyEnum key,
                                const String& keyName, DataType keyType, const String& keyComment) {
  MSTableImpl::keyMapDef(maps.keywordMap_p, maps.keyDTypeMap_p, maps.keyCommentMap_p, key, keyName,
                         keyType, keyComment);
}
