...
void MSTableImpl::addMeasColumn(TableDesc& td, const String& column, const String& measure,
                                const String& refCol) {
  String meas = measure;
...
void MSTableImpl::addColumnToDesc(TableDesc& td, const String& colName, Int colDType,
  ...
  if ((colDType >= TpBool && colDType <= TpString) || colDType == TpRecord) {
    ...
    if (option == 0) {
      ...
      switch (colDType) {
        ...
        case TpArrayDouble:
          ...
          break;
        case TpArrayComplex:
          td.addColumn(ArrayColumnDesc<Complex>(colName, colComment, shape, option));
          break;
        case TpArrayString:
          td.addColumn(ArrayColumnDesc<String>(colName, colComment, shape, option));
          break;
          /*
                      case TpArrayChar:
    ...
    cerr << "MSTableImpl::addColumnToDesc - Invalid data type: " << colDType << ", " << colName
         << std::endl;
...
SetupNewTable& MSTableImpl::setupCompression(SetupNewTable& newtab) {
  ...
  for (uInt i = 0; i < td.ncolumn(); i++) {
    ...
    if (keyset.isDefined("CompressFloat_AutoScale")) {
      ...
      CompressFloat engine(cdesc.name(), cname, cdesc.name() + "_SCALE", cdesc.name() + "_OFFSET",
      ...
      if (type == "SD") {
        CompressComplexSD engine(cdesc.name(), cname, cdesc.name() + "_SCALE",
                                 cdesc.name() + "_OFFSET",
                                 keyset.asBool("CompressComplex_AutoScale"));
        newtab.bindColumn(cname, cdesc.name());
        newtab.bindColumn(cdesc.name(), engine);
      } else {
        CompressComplex engine(cdesc.name(), cname, cdesc.name() + "_SCALE",
                               cdesc.name() + "_OFFSET",
                               keyset.asBool("CompressComplex_AutoScale"));
        newtab.bindColumn(cname, cdesc.name());
        newtab.bindColumn(cdesc.name(), engine);
    ...
    if (reqKeySet.isDefined("QuantumUnits")) {
...
#if defined(AIPS_DEBUG)
      if (!detail) {
        cerr << "MSTableImpl::validate - column " << colNames(colnr) << " doesn't have QuantumUnits"
             << std::endl;
      }
#endif
      if (detail)
        detail =
            allEQ(keySet.asArrayString("QuantumUnits"), reqKeySet.asArrayString("QuantumUnits"));
#if defined(AIPS_DEBUG)
      //*** testing
      ...
      if (!detail) {
        cerr << "MSTableImpl::validate - column " << colNames(colnr)
             << " has invalid QuantumUnits: " << keySet.asArrayString("QuantumUnits") << std::endl;
      }
#endif
      ...
      if (reqKeySet.isDefined("MEASINFO")) {
...
#if defined(AIPS_DEBUG)
        if (!detail) {
          cerr << "MSTableImpl::validate - column " << colNames(colnr) << " doesn't have MEASINFO"
               << std::endl;
        }
#endif
        if (detail) {
        ...
        if (!detail) {
          cerr << "MSTableImpl::validate - column " << colNames(colnr)
               << " has invalid MEASURE TYPE: " << keySet.asRecord("MEASINFO").asString("type")
               << std::endl;
        }
#endif
      }
...
MSTableMaps MSTableImpl::initMaps(MSMainEnums*) { return MeasurementSet::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSAntennaEnums*) { return MSAntenna::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSDataDescriptionEnums*) { return MSDataDescription::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSDopplerEnums*) { return MSDoppler::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSFeedEnums*) { return MSFeed::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSFieldEnums*) { return MSField::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSFlagCmdEnums*) { return MSFlagCmd::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSFreqOffsetEnums*) { return MSFreqOffset::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSHistoryEnums*) { return MSHistory::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSObservationEnums*) { return MSObservation::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSPointingEnums*) { return MSPointing::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSPolarizationEnums*) { return MSPolarization::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSProcessorEnums*) { return MSProcessor::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSSourceEnums*) { return MSSource::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSSpectralWindowEnums*) { return MSSpectralWindow::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSStateEnums*) { return MSState::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSSysCalEnums*) { return MSSysCal::initMaps(); }
MSTableMaps MSTableImpl::initMaps(MSWeatherEnums*) { return MSWeather::initMaps(); }
