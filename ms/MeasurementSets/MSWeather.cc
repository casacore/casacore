...
MSWeather::MSWeather(const String &tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSWeather(String &, TableOption) - "
...
MSWeather::MSWeather(const String &tableName, const String &tableDescName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSWeather(String &, String &, TableOption) - "
                  "table is not a valid MSWeather"));
}
...
MSWeather::MSWeather(SetupNewTable &newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSWeather(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MSWeather"));
}
...
MSWeather::MSWeather(const Table &table)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MSWeather(const Table &) - "
                  "table is not a valid MSWeather"));
}
...
MSWeather::MSWeather(const MSWeather &other)
  ...
  if (&other != this)
    if (!validate(this->tableDesc()))
      throw(
          AipsError("MSWeather(const MSWeather &) - "
                    "table is not a valid MSWeather"));
}
...
MSTableMaps MSWeather::initMaps() {
  ...
  // ANTENNA_ID
  colMapDef(maps, ANTENNA_ID, "ANTENNA_ID", TpInt, "Antenna number", "", "");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble, "Interval over which data is relevant", "s", "");
  // TIME
  colMapDef(maps, TIME, "TIME", TpDouble,
            ...
            "s", "Epoch");
  // DEW_POINT
  colMapDef(maps, DEW_POINT, "DEW_POINT", TpFloat, "Dew point", "K", "");
  // DEW_POINT_FLAG
  colMapDef(maps, DEW_POINT_FLAG, "DEW_POINT_FLAG", TpBool, "Flag for dew point", "", "");
  // H2O
  colMapDef(maps, H2O, "H2O", TpFloat, "Average column density of water-vapor", "m-2", "");
  // H2O_FLAG
  colMapDef(maps, H2O_FLAG, "H2O_FLAG", TpBool, "Flag for average column density of water-vapor",
            "", "");
  // IONOS_ELECTRON
  colMapDef(maps, IONOS_ELECTRON, "IONOS_ELECTRON", TpFloat, "Average column density of electrons",
            "m-2", "");
  // IONOS_ELECTRON_FLAG
  colMapDef(maps, IONOS_ELECTRON_FLAG, "IONOS_ELECTRON_FLAG", TpBool,
            "Flag for average column density of electrons", "", "");
  // PRESSURE
  colMapDef(maps, PRESSURE, "PRESSURE", TpFloat, "Ambient atmospheric pressure", "hPa", "");
  // PRESSURE_FLAG
  colMapDef(maps, PRESSURE_FLAG, "PRESSURE_FLAG", TpBool, "Flag for ambient atmospheric pressure",
            "", "");
  // REL_HUMIDITY
  colMapDef(maps, REL_HUMIDITY, "REL_HUMIDITY", TpFloat, "Ambient relative humidity", "%", "");
  // REL_HUMIDITY_FLAG
  colMapDef(maps, REL_HUMIDITY_FLAG, "REL_HUMIDITY_FLAG", TpBool,
            "Flag for ambient relative humidity", "", "");
  // TEMPERATURE
  colMapDef(maps, TEMPERATURE, "TEMPERATURE", TpFloat, "Ambient Air Temperature for an antenna",
            "K", "");
  // TEMPERATURE_FLAG
  colMapDef(maps, TEMPERATURE_FLAG, "TEMPERATURE_FLAG", TpBool,
            "Flag for ambient Air Temperature for an antenna", "", "");
  // WIND_DIRECTION
  colMapDef(maps, WIND_DIRECTION, "WIND_DIRECTION", TpFloat, "Average wind direction", "rad", "");
  // WIND_DIRECTION_FLAG
  colMapDef(maps, WIND_DIRECTION_FLAG, "WIND_DIRECTION_FLAG", TpBool, "Flag for wind direction", "",
            "");
  // WIND_SPEED
  colMapDef(maps, WIND_SPEED, "WIND_SPEED", TpFloat, "Average wind speed", "m/s", "");
  // WIND_SPEED_FLAG
  colMapDef(maps, WIND_SPEED_FLAG, "WIND_SPEED_FLAG", TpBool, "Flag for wind speed", "", "");
  // PredefinedKeywords
...
MSWeather MSWeather::referenceCopy(const String &newTableName,
  ...
  return MSWeather(MSTable<MSWeatherEnums>::referenceCopy(newTableName, writableColumns));
}
