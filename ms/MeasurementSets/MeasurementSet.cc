...
#define MrsDebugLog(level, message)    \
  {                                    \
    if ((level) <= mrsDebugLevel_p) {  \
      LogIO logIo(LogOrigin("MS"));    \
      logIo << (message) << std::endl; \
      logIo.post();                    \
    }                                  \
  }
...
MeasurementSet::MeasurementSet(const String& tableName, TableOption option)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MS(String &, TableOption) - "
...
void MeasurementSet::addCat() {
  ...
  if (!tableDesc().columnDesc(columnName(FLAG_CATEGORY)).keywordSet().isDefined("CATEGORY")) {
    if (!isWritable()) {
      throw(
...
MeasurementSet::MeasurementSet(const String& tableName, const TableLock& lockOptions,
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MS(String &, lockOptions, TableOption) - "
                  "table is not a valid MS"));
  initRefs();
}
...
MeasurementSet::MeasurementSet(const String& tableName, const String& tableDescName,
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MS(String &, String &, TableOption) - "
                  "table is not a valid MS"));
  initRefs();
}
...
MeasurementSet::MeasurementSet(SetupNewTable& newTab, rownr_t nrrow, Bool initialize)
  ...
  if (!validate(this->tableDesc()))
    throw(
        AipsError("MS(SetupNewTable &, rownr_t, Bool) - "
                  "table is not a valid MS"));
}
...
MeasurementSet::MeasurementSet(const MeasurementSet& other)
  ...
  if (!isNull()) {
    ...
    if (!validate(this->tableDesc())) {
      throw(
          AipsError("MS(const MeasurementSet &) - "
...
void MeasurementSet::copySubtables(const MeasurementSet& other) {
...
void MeasurementSet::copySubtable(const Table& otherSubtable, Table& subTable) {
...
MrsEligibility MeasurementSet::getMrsEligibility() const { return mrsEligibility_p; }
...
MSTableMaps MeasurementSet::initMaps() {
  ...
  // ANTENNA1
  colMapDef(maps, ANTENNA1, "ANTENNA1", TpInt, "ID of first antenna in interferometer", "", "");
  // ANTENNA2
  colMapDef(maps, ANTENNA2, "ANTENNA2", TpInt, "ID of second antenna in interferometer", "", "");
  // ANTENNA3
  colMapDef(maps, ANTENNA3, "ANTENNA3", TpInt, "ID of third antenna in interferometer", "", "");
  // ARRAY_ID
  colMapDef(maps, ARRAY_ID, "ARRAY_ID", TpInt, "ID of array or subarray", "", "");
  // BASELINE_REF
  ...
  colMapDef(maps, CORRECTED_DATA, "CORRECTED_DATA", TpArrayComplex, "The corrected data column", "",
            "");
  // the DATA columns,
  colMapDef(maps, DATA, "DATA", TpArrayComplex, "The data column", "", "");
  // the DATA_DESC_ID
  colMapDef(maps, DATA_DESC_ID, "DATA_DESC_ID", TpInt, "The data description table index", "", "");
  // EXPOSURE
  colMapDef(maps, EXPOSURE, "EXPOSURE", TpDouble, "The effective integration time", "s", "");
  // FEED1
  ...
  // FLAG_ROW
  colMapDef(maps, FLAG_ROW, "FLAG_ROW", TpBool, "Row flag - flag all data in this row if True", "",
            "");
  // FLOAT_DATA
  colMapDef(maps, FLOAT_DATA, "FLOAT_DATA", TpArrayFloat, "Floating point data - for single dish",
            "", "");
  // IMAGING_WEIGHT
  colMapDef(maps, IMAGING_WEIGHT, "IMAGING_WEIGHT", TpArrayFloat,
            "Weight set by imaging task (e.g. uniform weighting)", "", "");
  // INTERVAL
  colMapDef(maps, INTERVAL, "INTERVAL", TpDouble, "The sampling interval", "s", "");
  // the LAG_DATA column,
  colMapDef(maps, LAG_DATA, "LAG_DATA", TpArrayComplex, "The lag data column", "", "");
  // the MODEL_DATA column,
  colMapDef(maps, MODEL_DATA, "MODEL_DATA", TpArrayComplex, "The model data column", "", "");
  // OBSERVATION_ID
  colMapDef(maps, OBSERVATION_ID, "OBSERVATION_ID", TpInt,
            "ID for this observation, index in OBSERVATION table", "", "");
  // PHASE_ID
  colMapDef(maps, PHASE_ID, "PHASE_ID", TpInt, "Id for phase switching", "", "");
  // PROCESSOR_ID
  colMapDef(maps, PROCESSOR_ID, "PROCESSOR_ID", TpInt,
            "Id for backend processor, index in PROCESSOR table", "", "");
  // PULSAR_BIN
  colMapDef(maps, PULSAR_BIN, "PULSAR_BIN", TpInt, "Pulsar pulse-phase bin for this DATA", "", "");
  // PULSAR_GATE_ID
  colMapDef(maps, PULSAR_GATE_ID, "PULSAR_GATE_ID", TpInt,
            "ID for this gate, index into PULSAR_GATE table", "", "");
  // SCAN_NUMBER
  colMapDef(maps, SCAN_NUMBER, "SCAN_NUMBER", TpInt, "Sequential scan number from on-line system",
            "", "");
  // STATE_ID
  colMapDef(maps, STATE_ID, "STATE_ID", TpInt, "ID for this observing state", "", "");
  // SIGMA
  ...
  // TIME_CENTROID
  colMapDef(maps, TIME_CENTROID, "TIME_CENTROID", TpDouble, "Modified Julian Day", "s", "Epoch");
  // TIME_EXTRA_PREC
  colMapDef(maps, TIME_EXTRA_PREC, "TIME_EXTRA_PREC", TpDouble, "Additional precision for TIME",
            "s", "");
  // UVW
  colMapDef(maps, UVW, "UVW", TpArrayDouble, "Vector with uvw coordinates (in meters)", "m", "uvw");
  // UVW2
  colMapDef(maps, UVW2, "UVW2", TpArrayDouble,
            "uvw coordinates for second pair of triple corr product", "m", "uvw");
  // VIDEO_POINT
  colMapDef(maps, VIDEO_POINT, "VIDEO_POINT", TpArrayComplex,
            "zero frequency point, needed for transform to lag", "", "");
  // WEIGHT
  colMapDef(maps, WEIGHT, "WEIGHT", TpArrayFloat, "Weight for each polarization spectrum", "", "");
  // WEIGHT_SPECTRUM
  colMapDef(maps, WEIGHT_SPECTRUM, "WEIGHT_SPECTRUM", TpArrayFloat, "Weight for each data point",
            "", "");
  ...
  keyMapDef(maps, DATA_DESCRIPTION, "DATA_DESCRIPTION", TpTable,
            ...
            "in subtables");
  // FEED
  keyMapDef(maps, FEED, "FEED", TpTable, "Feed subtable. Responses, offsets, beams etc.");
  // FIELD
  keyMapDef(maps, FIELD, "FIELD", TpTable, "Field subtable. Position etc. for each pointing.");
  // FLAG_CMD
  keyMapDef(maps, FLAG_CMD, "FLAG_CMD", TpTable,
            "Flag command subtable. Stores global flagging commands");
  // HISTORY
  keyMapDef(maps, HISTORY, "HISTORY", TpTable, "Observation and processing history");
  // MS_VERSION
  keyMapDef(maps, MS_VERSION, "MS_VERSION", TpFloat, "MS version number, i.e., 2.0");
  // OBSERVATION
  keyMapDef(maps, OBSERVATION, "OBSERVATION", TpTable,
            "Observation subtable. Project, observer, schedule.");
  // POINTING
  keyMapDef(maps, POINTING, "POINTING", TpTable, "Pointing subtable. Antenna pointing info.");
  // POLARIZATION
  keyMapDef(maps, POLARIZATION, "POLARIZATION", TpTable, "Polarization set up subtable");
  // PROCESSOR
  keyMapDef(maps, PROCESSOR, "PROCESSOR", TpTable, "Backend Processor information subtable");
  // SPECTRAL_WINDOW
  keyMapDef(maps, SPECTRAL_WINDOW, "SPECTRAL_WINDOW", TpTable,
            "Spectral window subtable. Frequencies, bandwidths,"
            " polarizations");
  // STATE
  keyMapDef(maps, STATE, "STATE", TpTable, "State subtable. State information (cal, ref etc.)");
  // CAL_TABLES
  keyMapDef(maps, CAL_TABLES, "CAL_TABLES", TpTable, "Associated calibration tables, one per row");
  // DOPPLER
  keyMapDef(maps, DOPPLER, "DOPPLER", TpTable, "Doppler tracking info");
  // FREQ_OFFSET
  keyMapDef(maps, FREQ_OFFSET, "FREQ_OFFSET", TpTable, "Frequency offset information");
  // SORT_COLUMNS
  ...
  keyMapDef(maps, SORTED_TABLES, "SORTED_TABLES", TpTable,
            ...
            " main table");
  // SOURCE
  keyMapDef(maps, SOURCE, "SOURCE", TpTable, "Source subtable. Positions etc. for each source.");
  // SYSCAL
  keyMapDef(maps, SYSCAL, "SYSCAL", TpTable,
            "SysCal subtable. System calibration data (Tsys etc.).");
  // WEATHER
  keyMapDef(maps, WEATHER, "WEATHER", TpTable, "Weather subtable. Weather info for each antenna.");
  ...
  for (i = UNDEFINED_COLUMN + 1; i <= NUMBER_REQUIRED_COLUMNS; i++) {
    addColumnToDesc(maps, PredefinedColumns(i));
  }
  // Add the column keyword for the FLAG_CATEGORY column
  requiredTD.rwColumnDesc("FLAG_CATEGORY").rwKeywordSet().define("CATEGORY", Vector<String>(0));
...
void MeasurementSet::openMrSubtable(Subtable& subtable, const String& subtableName) {
...
void MeasurementSet::setMemoryResidentSubtables(const MrsEligibility& mrsEligibility) {
...
void MeasurementSet::clearSubtables() {
  antenna_p = MSAntenna();
...
void MeasurementSet::initRefs(Bool clear) {
  ...
  if (!isNull()) {
    ...
    if (this->tableInfo().type() == "") {
      ...
      this->tableInfo().readmeAddLine(
          "This is a MeasurementSet Table"
...
void MeasurementSet::createDefaultSubtables_impl(Table::TableOption option, T comm) {
  SetupNewTable antennaSetup(antennaTableName(), MSAntenna::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::ANTENNA), create_table(antennaSetup, comm));
  SetupNewTable dataDescSetup(dataDescriptionTableName(), MSDataDescription::requiredTableDesc(),
                              option);
  rwKeywordSet().defineTable(MS::keywordName(MS::DATA_DESCRIPTION),
                             create_table(dataDescSetup, comm));
  SetupNewTable feedSetup(feedTableName(), MSFeed::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::FEED), create_table(feedSetup, comm));
  SetupNewTable flagCmdSetup(flagCmdTableName(), MSFlagCmd::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::FLAG_CMD), create_table(flagCmdSetup, comm));
  SetupNewTable fieldSetup(fieldTableName(), MSField::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::FIELD), create_table(fieldSetup, comm));
  SetupNewTable historySetup(historyTableName(), MSHistory::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::HISTORY), create_table(historySetup, comm));
  SetupNewTable observationSetup(observationTableName(), MSObservation::requiredTableDesc(),
                                 option);
  rwKeywordSet().defineTable(MS::keywordName(MS::OBSERVATION),
                             create_table(observationSetup, comm));
  SetupNewTable pointingSetup(pointingTableName(), MSPointing::requiredTableDesc(), option);
  ...
  SetupNewTable processorSetup(processorTableName(), MSProcessor::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::PROCESSOR), create_table(processorSetup, comm));
  ...
  SetupNewTable stateSetup(stateTableName(), MSState::requiredTableDesc(), option);
  rwKeywordSet().defineTable(MS::keywordName(MS::STATE), create_table(stateSetup, comm));
  initRefs();
}
...
void MeasurementSet::checkVersion() {
  ...
  if (!keywordSet().isDefined("MS_VERSION") ||
      (keywordSet().isDefined("MS_VERSION") && keywordSet().asFloat("MS_VERSION") != 2.0)) {
    throw(AipsError("These data are not in MSv2 format - use ms1toms2 to convert"));
  }
}

Record MeasurementSet::msseltoindex(const String& spw, const String& field, const String& baseline,
...
MrsEligibility MrsEligibility::allEligible() {
  MrsEligibility all;

  // Start out by putting in all of the keywords defined in
  // the enum

  for (int i = MSMainEnums::UNDEFINED_KEYWORD + 1; i <= MSMainEnums::NUMBER_PREDEFINED_KEYWORDS;
       ++i) {
...
MrsEligibility MrsEligibility::defaultEligible() {
  // The following two subtables can become quite large
  // and should normally not be made memory resident

  MrsEligibility defaultSubtables =
      allButTheseSubtables(MSMainEnums::HISTORY, MSMainEnums::POINTING, MSMainEnums::SYSCAL,
...
MrsEligibility MrsEligibility::noneEligible() { return MrsEligibility(); }
...
MrsEligibility operator-(const MrsEligibility& a, const MrsEligibility& b) {
  ...
  std::set_difference(a.eligible_p.begin(), a.eligible_p.end(), b.eligible_p.begin(),
                      b.eligible_p.end(), inserter(result.eligible_p, result.eligible_p.begin()));
...
MrsEligibility operator+(const MrsEligibility& a, const MrsEligibility& b) {
  ...
  std::set_union(a.eligible_p.begin(), a.eligible_p.end(), b.eligible_p.begin(), b.eligible_p.end(),
