...
StokesConverter::StokesConverter(const StokesConverter& other) { operator=(other); }
...
void StokesConverter::setConversion(const Vector<Int>& out, const Vector<Int>& in, Bool rescale) {
  ...
  if (count == 0) {
    throw(
        AipsError("StokesConverter::setConversion - input polarization"
                  " frame not supported"));
  }
  if (count > 1) {
    throw(
        AipsError("StokesConverter::setConversion - input polarizations"
      ...
      if (out(i) >= Stokes::Ptotal && out(i) <= Stokes::Pangle) {
        ...
        for (Int j = 0; j < nIn; j++) {
          switch (out(i)) {
            case Stokes::Ptotal:
              flagConv_p(i, j) =
                  (iquvConv_p(1, j) != Complex(0.) || iquvConv_p(2, j) != Complex(0.) ||
            ...
            case Stokes::PFlinear:
              flagConv_p(i, j) =
                  (iquvConv_p(0, j) != Complex(0.) || iquvConv_p(1, j) != Complex(0.) ||
...
void StokesConverter::initConvMatrix() {
  Complex Slin[4][4] = {{0.5, 0.5, 0.0, 0.0},
                        {0.0, 0.0, 0.5, Complex(0.0, 0.5)},
                        {0.0, 0.0, 0.5, Complex(0.0, -0.5)},
                        {0.5, -0.5, 0.0, 0.0}};
  Complex H[2][2] = {{1.0, Complex(0.0, 1.0)}, {1.0, Complex(0.0, -1.0)}};
  ...
  for (Int i = 0; i < 20; i++) {
    for (Int j = 0; j < 20; j++) {
      ...
      if (nearAbs(polConv_p(i, j), Complex(-1., 0.), 1.e-4)) polConv_p(i, j) = Complex(-1., 0.);
      if (nearAbs(polConv_p(i, j), Complex(0., 1.), 1.e-4)) polConv_p(i, j) = Complex(0., 1.);
      if (nearAbs(polConv_p(i, j), Complex(0., -1.), 1.e-4)) polConv_p(i, j) = Complex(0., -1.);
      if (nearAbs(polConv_p(i, j), Complex(0.5, 0.), 1.e-4)) polConv_p(i, j) = Complex(0.5, 0.);
      if (nearAbs(polConv_p(i, j), Complex(-0.5, 0.), 1.e-4)) polConv_p(i, j) = Complex(-0.5, 0.);
      if (nearAbs(polConv_p(i, j), Complex(0., 0.5), 1.e-4)) polConv_p(i, j) = Complex(0., 0.5);
      if (nearAbs(polConv_p(i, j), Complex(0., -0.5), 1.e-4)) polConv_p(i, j) = Complex(0., -0.5);
    }
  }
}
...
void StokesConverter::convert(Array<Complex>& out, const Array<Complex>& in) const {
  ...
  IPosition iquvShape(outMat.shape());
  iquvShape(0) = 4;
  Matrix<Complex> iquv;
    ...
    for (uInt j = 0; j < inMat.ncolumn(); j++) {
      ...
      for (Int k = 0; k < nCorrIn; k++) {
        if (inMat(k, j) != 0)
          ...
          outMat(i, j) = 0;
          break;
        }  // flag output if one of inputs is zero
      }
      if (outMat(i, j) != 0) outMat(i, j) = (sigma ? sqrt(outMat(i, j)) : 1 / outMat(i, j));
    }
  }
}
