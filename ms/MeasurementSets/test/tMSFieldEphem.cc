  ...
  try {
    {
      ...
      SetupNewTable dataDescriptionSetup(ms.dataDescriptionTableName(), tddataDescription,
                                         Table::New);
    ...
    }
    {
      ...
      msfc.updateMeasComets();
      {
        ...
        AlwaysAssertExit(expDir.separation(MVDirection(eDir.getAngle())) <
                         Quantity(0.001 / 3600., "deg").getValue("rad"));
      }
      ...
      msfc.updateMeasComets();
      {
        ...
        // cout << "phasedir for row " << row << ", MJD-50802. " << mjds/86400.-50802. << ": " <<
        // pDir.getAngle(Unit("deg")) << std::endl;
        MDirection rDir = msfc.referenceDirMeas(row, mjds);
        // cout << "referencedir for row " << row << ", MJD-50802. " << mjds/86400.-50802. << ": "
        // << rDir.getAngle(Unit("deg")) << std::endl;
        MDirection eDir = msfc.ephemerisDirMeas(row, mjds);
        // cout << "ephemerisdir for row " << row << ", MJD-50802. " << mjds/86400.-50802. << ": "
        // << eDir.getAngle(Unit("deg")) << std::endl;

        MDirection expected(Quantity(-54.3855, "deg"), Quantity(-19.8873, "deg"), MDirection::APP);
        MVDirection expDir(expected.getAngle());

        AlwaysAssertExit(expDir.separation(MVDirection(dDir.getAngle())) <
                         Quantity(1 / 3600., "deg").getValue("rad"));
        // AlwaysAssertExit(dDir.getRef().getType()==expected.getRef().getType());
        AlwaysAssertExit(expDir.separation(MVDirection(pDir.getAngle())) <
                         Quantity(1 / 3600., "deg").getValue("rad"));
        // AlwaysAssertExit(pDir.getRef().getType()==expected.getRef().getType());
        AlwaysAssertExit(expDir.separation(MVDirection(rDir.getAngle())) <
                         Quantity(1 / 3600., "deg").getValue("rad"));
      ...
      msfc.updateMeasComets();
      {
        ...
        // dirb(0)*180./3.14159265/cos(-19.88733167*3.1415926/180.), "deg"),
        //		    Quantity(-19.88733167+dirb(1)*180./3.14159265, "deg"),
        //		    MDirection::TOPO);
        MVDirection expDir(expected.getAngle());
        MVDirection unalteredExpDir(unalteredExpected.getAngle());

        // cout << "separation " << expDir.separation(MVDirection(dDir.getAngle()), "deg") <<
        // std::endl;
        ...
        AlwaysAssertExit(rDir.getRef().getType() == expected.getRef().getType());
        AlwaysAssertExit(unalteredExpDir.separation(MVDirection(eDir.getAngle())) <
                         Quantity(1 / 3600., "deg").getValue("rad"));
        AlwaysAssertExit(eDir.getRef().getType() == expected.getRef().getType());
      }
      ...
      msfc.updateMeasComets();
      {
        ...
        MVDirection original(Quantity(305.6095079, "deg"), Quantity(-19.88256944, "deg"));
        ...
        AlwaysAssertExit((mradvel.get("AU/d") - Quantity(-0.0057244046, "AU/d")).getValue("km/s") <
                         0.01);
