...
void createMSSeveralDDFeedField(int nAnt, int nTime, int nDD, int nField, double msinterval,
                                std::string name) {
  // Create the MS (with DATA column) and its subtables.
  TableDesc td(MS::requiredTableDesc());
  MS::addColumnToDesc(td, MS::DATA, 2);
...
void iterMSCachedDDFeedInfo() {
  ...
  for (int useGenericSortCons = 0; useGenericSortCons < 2; useGenericSortCons++) {
    ...
    for (msIter->origin(); msIter->more(); (*msIter)++) {
      cout << "nrow=" << msIter->table().nrow() << std::endl;
      cout << "ddid = " << msIter->dataDescriptionId() << " spwid = " << msIter->spectralWindowId()
      ...
      for (int iRec = 0; iRec < nReceptors; ++iRec) {
        for (int iAnt = 0; iAnt < nAnt; iAnt++) {
          for (int iFeed = 0; iFeed < nFeed; iFeed++) {
            AlwaysAssertExit(std::abs(msIter->getBeamOffsets()(iRec, iAnt, iFeed)(0) -
                                      expectedBeamOffsets(iRec, iAnt, iFeed)(0)) < 1e-6);
            AlwaysAssertExit(std::abs(msIter->getBeamOffsets()(iRec, iAnt, iFeed)(1) -
                                      expectedBeamOffsets(iRec, iAnt, iFeed)(1)) < 1e-6);
          }
        }
      }
      ...
      for (int iAnt = 0; iAnt < nAnt; iAnt++) {
        for (int iFeed = 0; iFeed < nFeed; iFeed++) {
          AlwaysAssertExit(allNear(msIter->CJonesAll()(iAnt, iFeed).matrix(),
                                   expectedPartialCJones + Complex(iAnt * 0.01, iAnt * 0.01),
                                   1e-6));
        }
      }
  ...
  for (int useGenericSortCons = 0; useGenericSortCons < 2; useGenericSortCons++) {
    ...
    for (msIter->origin(); msIter->more(); (*msIter)++) {
      ...
      // Update the expected values for next iteration
      antena1Idx++;
      if (antena1Idx > nAnt - 1) {  // All ANTENNA1 iterations done, now iterate on a new DD
        expectedDDId++;
        expectedSPWId++;
        expectedPolId = expectedDDId % 2;
        expectedPolFrame = expectedDDId % 2;
        ...
        newPol = true;
        newDD = true;
      } else {  // Still iterating on ANTENNA1
        newSpw = false;
        newPol = false;
        newDD = false;
      }
      ...
      for (msIter->origin(); msIter->more(); (*msIter)++) {
        if (rowsToSkip == 0) {
          ...
          cout << "ddid = " << msIter->dataDescriptionId()
               ...
               << " polid = " << msIter->polarizationId() << std::endl;
          cout << "freqs = " << msIter->frequency() << " freq0 " << msIter->frequency0()
               << std::endl;
          // Check that the DD related metadata is what we expect
...
void iterMSCachedFieldInfo() {
  ...
  for (int useGenericSortCons = 0; useGenericSortCons < 2; useGenericSortCons++) {
    ...
    for (msIter->origin(); msIter->more(); (*msIter)++) {
      ...
      AlwaysAssertExit(
          !msIter->fieldName().compare(expectedFieldName + std::to_string(expectedFieldId)));
