...
uInt tNonStatic(const String& sdmsName) {
  ...
  if (!ms.isColumnWritable(MS::TIME)) {
    cerr << "TIME column should be writable but isColumnWritable() returned False" << std::endl;
    errCount++;
  }
...
uInt tConstructors(const String& msName) {
  ...
  // make non MS table
  {
    SetupNewTable newtab("tMeasurementSet_tmp.badmsTable", "badTD", Table::New);
    Table tab(newtab);
  }
...

  {
    ...
    SetupNewTable newtab2("tMeasurementSet_tmp.msant3", MSAntenna::requiredTableDesc(), Table::New);
    ...
    SetupNewTable newtab3("tMeasurementSet_tmp.msantTable", "antTD", Table::New);
    Table tab(newtab3);
    tab.markForDelete();
