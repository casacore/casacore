...
bool readParms(int argc, char* argv[]) {
  ...
  params.create("nms", "0", "Number of MeasurementSets to read (0 means 1 MS without suffix)",
                "int");
  params.create("msname", "", "Name of the MeasurementSet (suffix _p<i> is added if nms>0)",
                "string");
  params.create("data", "true", "Read DATA or FLOAT_DATA column?", "bool");
  params.create("flag", "true", "Read FLAG column?", "bool");
  params.create("weightspectrum", "false", "Read WEIGHT_SPECTRUM column?", "bool");
  params.create("npol", "0", "Number of polarizations to read (0=all, 1=XX, 2=XX/YY)", "int");
  params.create("startchan", "0", "First channel to read", "int");
  params.create("nchan", "0", "Number of channels to read (0=all)", "int");
  params.create("chansize", "0",
                "Number of channels to read jointly; allows channel iteration (0=all)", "int");
  params.create("baselines", "", "Baselines to read (CASA baseline selection)", "string");
  params.create("selection", "", "TaQL selection string", "string");
  params.create("iteration1", "DATA_DESC_ID", "Columns to iterate on before channel iteration",
                "string vector");
  params.create("iteration2", "TIME", "Columns to iterate on after channel iteration",
                "string vector");
  params.create("sort", "", "Columns to sort on (default none); . is iteration columns",
                "string vector");
  params.create("rowwise", "false", "Read the data row wise (thus a get per row)", "bool");
  params.create("datacachesize", "0", "TiledStMan cache size for DATA column (in tiles)", "int");
  params.create("flagcachesize", "0", "TiledStMan cache size for FLAG column (in tiles)", "int");
  params.create("weightcachesize", "0", "TiledStMan cache size for WEIGHT column (in tiles)",
                "int");
  ...
  if (mySortCols.size() == 1 && mySortCols[0] == ".") {
    ...
    Vector<String> scols(
        mySortCols(Slice(myIterCols1.size(), mySortCols.size() - myIterCols1.size())));
...
void showParms() {
  ...
  cout << " nant      = " << nant << "   (" << nbl << " baselines)" << std::endl;
  cout << " nspw      = " << ms.spectralWindow().nrow() << "   (" << nspw << " per ms)"
       << std::endl;
  ...
  if (opt.option() == StorageOption::MultiFile || opt.option() == StorageOption::MultiHDF5) {
    cout << " multi" << (opt.option() == StorageOption::MultiFile ? "file" : "hdf5");
...
Int64 readNoIter(MeasurementSet& tab, Int64& niter) {
  ...
  for (int fchan = myStartChan; fchan < lastchan; fchan += chansize) {
    ...
    for (rownr_t row = 0; row < tab.nrow(); row += ntoread) {
      Slicer rowRange(IPosition(1, row), IPosition(1, std::min(ntoread, tab.nrow() - row)));
...
void readRowsHDF5(const std::shared_ptr<HDF5DataSet>& hflag,
                  const std::shared_ptr<HDF5DataSet>& hdata,
                  const std::shared_ptr<HDF5DataSet>& hfloatdata,
                  const std::shared_ptr<HDF5DataSet>& hweight, const Slicer& slicer) {
  ...
  for (int fchan = myStartChan; fchan < lastchan; fchan += chansize) {
    ...
    for (Int64 row = 0; row < shape[2]; row += nrow) {
      if (fchan > 0 || lchan < shape[1] || myNPol < shape[0]) {
        ...
        Slicer slicer(IPosition(3, 0, fchan, row), IPosition(3, npol, nchan, nrow), stride);
        // Read the data per time step.
...
#pragma omp parallel for schedule(dynamic)
  ...
  if (myNPart == 1) {
    ...
    cout << "Read " << myNPart << " MS parts in " << nthread << " threads" << std::endl;
  }
  cout << "For each part " << selNrow << " rows out of " << nrow << " have been read" << std::endl;
}
