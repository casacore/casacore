...
class MSCreate {
 public:
  ...
  void init(const vector<double>& ra, const vector<double>& dec, const Matrix<double>& antPos,
            bool calcUVW, bool writeAutoCorr, bool writeFloatData, bool writeWeightSpectrum,
            bool createImagerColumns, const Vector<int>& npol, const Vector<int>& nfreq,
            const Vector<double>& startFreq, const Vector<double>& stepFreq, int spw, int nspw,
            int ntimeField, double startTime, double stepTime, const String& msName,
            const String& antennaTableName, int nflagBits, const String& flagColumn,
  ...
  // <group>
  virtual void fillAntenna(const Block<MPosition>& antMPos, const String& antennaTableName) = 0;
 ...
 private:
  ...
  virtual void createMS(const String& msName, int ntimeField, int useMultiFile, int multiBlockSize,
                        bool createImagerColumns, const String& flagColumn, int flagBits) = 0;
...
class MSCreateCasa : public MSCreate {
 ...
 private:
  ...
  // Create the MS and fill its subtables as much as possible.
  virtual void createMS(const String& msName, int ntimeField, int useMultiFile, int multiBlockSize,
                        bool createImagerColumns, const String& flagColumn, int flagBits);
  ...
  void addBand(int band, int npolarizations, int nchannels, double startFreq, double chanWidth);
  ...
  // Fill the various subtables.
  // <group>
  virtual void fillAntenna(const Block<MPosition>& antMPos, const String& antennaTableName);
...
MSCreate::MSCreate() : itsNrTimes(0) {}

MSCreate::~MSCreate() {}

void MSCreate::init(const vector<double>& ra, const vector<double>& dec,
                    const Matrix<double>& antPos, bool calcUVW, bool writeAutoCorr,
                    bool writeFloatData, bool writeWeightSpectrum, bool createImagerColumns,
                    const Vector<int>& npol, const Vector<int>& nfreq,
                    const Vector<double>& startFreq, const Vector<double>& stepFreq, int spw,
                    int nspw, int ntimeField, double startTime, double stepTime,
                    const String& msName, const String& antennaTableName, int nflagBits,
                    const String& flagColumn, const IPosition& dataTileShape, int useMultiFile,
                    int multiBlockSize) {
  itsRa = ra;
  itsDec = dec;
  itsCalcUVW = calcUVW;
  ...
  // Create the MS.
  createMS(msName, ntimeField, useMultiFile, multiBlockSize, createImagerColumns, flagColumn,
           nflagBits);
...
void MSCreateCasa::createMS(const String& msName, int ntimeField, int useMultiFile,
                            int multiBlockSize, bool createImagerColumns, const String& flagColumn,
                            int nflagBits) {
...
void MSCreateCasa::fillAntenna(const Block<MPosition>& antMPos, const String& antennaTableName) {
...
void MSCreateCasa::addBand(int band, int npolarizations, int nchannels, double startFreq,
                           double chanWidth) {
  AlwaysAssert(nchannels > 0, AipsError);
  AlwaysAssert(npolarizations == 1 || npolarizations == 2 || npolarizations == 4, AipsError);
...
void MSCreateCasa::addField(int field) {
  ...
  // Put the direction into the FIELD subtable.
  {
    ...
    msfieldCol.time().put(
        rownr, itsStartTime);  // really startTime; everywhere else it is midpoint with an interval
    ...
    for (Int i = 0; i < itsNrAnt; i++) {
      ...
      mspointingCol.time().put(rownr,
                               itsStartTime);  // actually midpoint (as in updateTimes()), but
                                               // interval is still 0.0 (unknown) at creation
...
void MSCreateCasa::addImagerColumns() {
  ...
  if (!itsMS.tableDesc().isColumn(colName)) {
    ...
    if (shape.empty()) {
      ...
      td.addColumn(
          ArrayColumnDesc<Complex>(colName, "corrected data", shape, ColumnDesc::FixedShape));
    }
...
void MSCreateHDF5::createMS(const String& msName, int ntimeField, int /*useMultiFile*/,
                            int /*multiBlockSize*/, bool createImagerColumns,
...
Array<double> parmArrayDouble(Input& params, const String& name, const String& unit = String()) {
...
bool readParms(int argc, char* argv[]) {
  ...
  params.create(
      ...
      "int");
  params.create("msname", "", "Name of the output MeasurementSet (suffix _p<i> is added if nms>0)",
                "string");
  params.create("ra", "",
                "One or more J2000 Right Ascensions (as hh:mm:ss.sss); defines the fields",
                "string");
  params.create("dec", "", "One or more J2000 Declinations (as dd.mm.ss.sss); defines the fields",
                "string");
  params.create("anttab", "",
                "Name of the ANTENNA table giving the antenna parameters to use (zero/empty values "
                "if not given)",
                "string");
  params.create("startfreq", "1 GHz",
                "Start frequency (Hz) per spw (1 value counts on for other spws)", "double");
  params.create("chanwidth", "1 MHz",
                "Channel frequency width (Hz) per spw (1 value applies to all spws)", "double");
  params.create("starttime", "", "Start time (e.g., 23Mar2016/12:00:00)", "string");
  params.create("timestep", "1", "Time interval (sec)", "double");
  params.create("ntime", "1", "Number of time steps", "int");
  params.create("ntimefield", "0", "Number of time steps per field (0=all fields for all times)",
                "int");
  params.create("totalspw", "nspw",
                "Total number of spectral windows (to write in SPECTRAL_WINDOW)", "int");
  params.create("firstspw", "0", "First spectral window to write in this set of MSs", "int");
  params.create("nspw", "1", "Number of spectral windows to write in this set of MSs", "int");
  params.create("npol", "4",
                "Number of polarizations per spectral window; 1 value applies to all spws", "int");
  params.create("nchan", "256",
                "Number of channels per spectral window; 1 value applies to all spws", "int");
  params.create("nant", "0",
                "Number of antennae to use; it anttab is given maximum to use is its size", "int");
  params.create("calcuvw", "false", "Calculate UVW coordinates?", "bool");
  params.create("autocorr", "true", "Write autocorrelations?", "bool");
  params.create("floatdata", "false", "Write only autocorrelations and FLOAT_DATA instead of DATA?",
                "bool");
  params.create("weightspectrum", "false", "Write WEIGHT_SPECTRUM column?", "bool");
  params.create("imagercolumns", "false", "Write imager columns (MODEL_DATA, CORRECTED_DATA)?",
                "bool");
  params.create("rowwise", "false", "Write the data row wise (thus a put per row)", "bool");
  params.create("nflagbits", "0", "Write multiple flag bits (0, 8, 16 or 32) mapped to FLAG",
                "int");
  params.create("flagcolumn", "FLAG_BITS", "Name of the FlagBits column if nflagbits>0", "string");
  params.create("tilesize", "-1", "Size of data tiles (bytes); default is 1024*1024", "int");
  params.create("tilesizepol", "-1", "Number of polarizations in data tiles; default is all",
                "int");
  params.create("tilesizefreq", "-1", "Number of channels in data tiles; default is all", "int");
  params.create("multifile", "false", "Use the MultiFile feature?", "bool");
  params.create("multihdf5", "false", "Use the MultiHDF5 feature?", "bool");
  params.create("mfsize", "-1", "MultiFile/HDF5 block size (bytes); default is tilesize", "int");
  params.create("ashdf5", "false", "Write the data in HDF5 format", "bool");
  params.create("useadios2", "false", "Use Adios2StMan for all columns", "bool");
...
void showParms() {
  ...
  cout << " totalspw = " << myTotalNBand << "   (firstspw = " << myFirstBand << ')' << std::endl;
  cout << " nspw     = " << myNBand << "   (" << myNBand / myNPart << " per ms)" << std::endl;
  ...
  IPosition tileShape =
      formTileShape(myTileSize, myTileSizePol, myTileSizeFreq, myWriteFloatData, myNPol, myNChan);
  int tileSize = tileShape.product() * 8;
  if (myWriteFloatData) {
    tileSize /= 2;
  }
  cout << " data tileshape      = " << tileShape << "  (tilesize = " << tileSize << " bytes)"
       << std::endl;
...
String doOne(int seqnr, const String& msName) {
  ...
  msmaker->init(myRa, myDec, myAntPos, myCalcUVW, myWriteAutoCorr, myWriteFloatData,
                myWriteWeightSpectrum, myCreateImagerColumns, myNPol, myNChan, myStartFreq,
                myStepFreq, myFirstBand + seqnr * nbpp, nbpp, myNTimeField, myStartTime, myStepTime,
                name, myAntennaTableName, myNFlagBits, myFlagColumn, dataTileShape, myUseMultiFile,
                myMultiBlockSize);
  ...
  msmaker->flush();
  timer.show("Wrote " + std::to_string(msmaker->nrow()) + " rows into MS " + std::string(msName));
...
#pragma omp parallel for schedule(dynamic)
  ...
  if (myNPart == 1) {
    ...
    cout << "Created " << myNPart << " MS parts in " << nthread << " threads" << std::endl;
  }
