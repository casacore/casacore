...
FITSIDItoMS1::FITSIDItoMS1(FitsInput& fitsin, const String& correlat, const Int& obsType,
  ...
  if (itsCorrelat.length() == 0 && array_p == "VLBA") itsCorrelat = "VLBA";
  ...
  for (Int icol = 0; icol < tfields(); icol++) {
    ...
    switch (field(icol).fieldtype()) {
      ...
      default:
        ...
        cerr << "Error: unrecognized table data type for field " << icol << endl;
...
const TableDesc& FITSIDItoMS1::getDescriptor() { return itsCurRowTab.tableDesc(); }

TableRecord& FITSIDItoMS1::getKeywords() { return itsCurRowTab.rwKeywordSet(); }

const Table& FITSIDItoMS1::thisRow() { return (itsCurRowTab); }
      ...
      if (kwname.substr(0, 3) == "MSK") {
        ...
        if (iMSK > 0) {
          ...
          if (kwname.substr(3, 1) == "C") {
            ...
            *itsLog << LogIO::WARN << "MSBinaryTable found unknown MSK keyword: " << kwname
                    << ". It will be ignored" << LogIO::POST;
          }
        } else {
          *itsLog << LogIO::WARN << "MSBinaryTable found unknown MSK keyword: " << kwname
                  << ". It will be ignored" << LogIO::POST;
        ...
        switch (kw->type()) {
          case FITS::NOVALUE:
            itsKwSet.define(kwname, "");
            // NOVALUE fields become string keywords with an emtpy string.
            *itsLog << LogIO::NORMAL << "FITS::NOVALUE found" << LogIO::POST;
            break;
          case FITS::LOGICAL:
            itsKwSet.define(kwname, kw->asBool());
            break;
          case FITS::CHAR:
            itsKwSet.define(kwname, kw->asString());
            break;
          case FITS::STRING:
            itsKwSet.define(kwname, kw->asString());
            break;
          case FITS::LONG:
            itsKwSet.define(kwname, kw->asInt());
            break;
          case FITS::FLOAT:
            itsKwSet.define(kwname, kw->asFloat());
            break;
          case FITS::DOUBLE:
            itsKwSet.define(kwname, kw->asDouble());
            break;
          case FITS::COMPLEX:
            itsKwSet.define(kwname, kw->asComplex());
            break;
          default:
      ...
      if (kwname.substr(0, 5) == "MAXIS") {
        ...
        if (weightyp_p != "NORMAL" && weightyp_p != "CORRELAT") {
          *itsLog << LogIO::WARN << "Found WEIGHTYP keyword with value \"" << weightyp_p
                  << "\" in UV_DATA table. Presently this keyword is ignored." << LogIO::POST;
        }
  ...
  for (Int icol = 0; icol < nfield; icol++) {
        ...
        if (itsgotMSK(ikey) && (itsMSKC(ikey) == colname)) {
          if (itsMSKN(ikey) == "OPTIONS") {
            if (itsMSKV(ikey) == "DIRECT") {
              ...
              *itsLog << LogIO::DEBUG1 << "found MSK OPTIONS = DIRECT for " << colname
                      << LogIO::POST;
            } else {
    ...
    switch (field(icol).fieldtype()) {
      ...
      case FITS::STRING:
        ...
        if (itsIsArray(icol)) {
          itsTableDesc.addColumn(ArrayColumnDesc<String>(colname, "", shape, option));
        } else {
      ...
      case FITS::COMPLEX:
        if (itsIsArray(icol)) {
          itsTableDesc.addColumn(ArrayColumnDesc<Complex>(colname, "", shape, option));
        } else {
      ...
      case FITS::DCOMPLEX:
        if (itsIsArray(icol)) {
          itsTableDesc.addColumn(ArrayColumnDesc<DComplex>(colname, "", shape, option));
        } else {
      ...
      default:
        ...
        *itsLog << LogIO::WARN << "Internal error: column " << icol << " has untranslatable type "
                << field(icol).fieldtype() << LogIO::POST;
        continue;
    ...
    itsTableDesc.rwColumnDesc(colname).rwKeywordSet().define("UNIT", unitstr);
    ...
    if (itsgotMSK(ikey)) {
      if (itsMSKC(ikey) == " ") {
        ...
        itsTableDesc.rwColumnDesc(itsMSKC(ikey))
            .rwKeywordSet()
            .define(itsMSKN(ikey), itsMSKV(ikey));
      }
    }
  }
}
...
void FITSIDItoMS1::getAxisInfo() {
  *itsLog << LogOrigin("FitsIDItoMS", "getAxisInfo");
  ...
  if ((getIndex(coordType_p, "RA") < 0) && (getIndex(coordType_p, "RA---SIN") < 0) &&
      (getIndex(coordType_p, "RA---NCP") < 0) && (getIndex(coordType_p, "RA---SCP") < 0)) {
    *itsLog << "Data does not have a RA axis" << LogIO::EXCEPTION;
  }
  if ((getIndex(coordType_p, "DEC") < 0) && (getIndex(coordType_p, "DEC--SIN") < 0) &&
      (getIndex(coordType_p, "DEC--NCP") < 0) && (getIndex(coordType_p, "DEC--SCP") < 0)) {
  ...
  for (uInt i = 0; i < numCorr; i++) {
    ...
    switch (corrType_p(i)) {
      ...
      case 1:
        corrType_p(i) = Stokes::XX;
        break;  // not correct but for a quick fix for sma data
      ...
      default:
        if (corrType_p(i) < -8 || corrType_p(i) > 4) {
          *itsLog << "Unknown Correlation type: " << corrType_p(i) << LogIO::EXCEPTION;
        }
    }
  }
  ...
  for (uInt i = 0; i < numCorr; i++) {
    ...
    if (receptor.has_value()) {
      ...
      *itsLog << "Cannot deduce receptor 1 for correlations of type: " << Stokes::name(cType)
              << LogIO::EXCEPTION;
    }
    ...
    if (receptor.has_value()) {
      ...
      *itsLog << "Cannot deduce receptor 2 for correlations of type: " << Stokes::name(cType)
              << LogIO::EXCEPTION;
    }
  }
  ...
  if (ok) {
    ...
    // in other ways.
    ok = FITSSpectralUtil::fromFITSHeader(spectralAxis, referenceChannel, referenceFrequency,
                                          deltaFrequency, frequencies, freqsys_p, velPref,
                                          restfreq_p, *itsLog, header);
  }
}
...
void FITSIDItoMS1::setupMeasurementSet(const String& MSFileName, Bool useTSM, Bool mainTbl,
  ...
  if (mainTbl) {
    ...
    TiledShapeStMan tiledStMan4("TiledWgt", IPosition(2, tileShape(0), tileShape(2)));
    TiledShapeStMan tiledStMan5("TiledSigma", IPosition(2, tileShape(0), tileShape(2)));
  ...
  if (addSyscal) {
    ...
    ms.rwKeywordSet().defineTable(MS::keywordName(MS::SYSCAL), Table(tabSetup));
  }

  if (addWeather) {
    TableDesc td = MSWeather::requiredTableDesc();
    MSWeather::addColumnToDesc(td, MSWeather::DEW_POINT);
    if (weather_hasWater_p) MSWeather::addColumnToDesc(td, MSWeather::H2O);
    if (weather_hasElectron_p) MSWeather::addColumnToDesc(td, MSWeather::IONOS_ELECTRON);
    ...
    ms.rwKeywordSet().defineTable(MS::keywordName(MS::WEATHER), Table(tabSetup));
  }
  ...
  *itsLog << LogIO::NORMAL << "Reading and writing visibility data" << LogIO::POST;
  ...
  for (Int trow = 0; trow < nRows; trow++) {
    ...
    if (iInttim > -1) {
      ...
      if (row < 0) {
        *itsLog << LogIO::WARN
                << "UV_DATA table contains no integration time information. Will try to derive it "
                   "from TIME."
                << LogIO::POST;
      }
      if (time > startTime) {
    ...
    if (antIdFromNo.find(ant1) != antIdFromNo.end()) {
      ...
      *itsLog << LogIO::SEVERE << "Inconsistent input dataset: unknown ANTENNA_NO " << ant1
              << " in baseline used in UV_DATA table." << LogIO::EXCEPTION;
    }
    if (antIdFromNo.find(ant2) != antIdFromNo.end()) {
      ant2 = antIdFromNo[ant2];
    } else {
      *itsLog << LogIO::SEVERE << "Inconsistent input dataset: unknown ANTENNA_NO " << ant2
              << " in baseline used in UV_DATA table." << LogIO::EXCEPTION;
    }
      ...
      for (Int chan = 0; chan < nChan; chan++) {
        for (Int pol = 0; pol < nCorr; pol++) {
          ...
          if (doConjugateVis) {  // need a conjugation to follow the ant1<=ant2 rule
            vis(p, chan) = Complex(visReal, visImag);  // NOTE: this means no conjugation of
                                                       // visibility because of FITS-IDI convention!
          } else {
            vis(p, chan) =
                Complex(visReal,
                        -visImag);  // NOTE: conjugation of visibility!
  ...
  *itsLog << LogIO::NORMAL << "array ref pos = " << arrayXYZ << LogIO::POST;
  ...
  if (btKeywords.isDefined("ARRNAM")) {
    ...
    if (array_p == "" || array_p == "unknown") {
      ...
      if (array_p != arrnam) {
        *itsLog << LogIO::SEVERE << "Conflicting observatory names: found " << arrnam << " and "
                << array_p << LogIO::POST;
      }
    }
  }
  ...
  if (anTab.tableDesc().isColumn("DIAMETER")) {
    ...
    *itsLog << LogIO::WARN
            << "ARRAY_GEOMETRY input table does not contain dish DIAMETER column.\n Will assume "
               "default diameter for TELESCOPE "
...
void FITSIDItoMS1::fillFeedTable() {
  ...
  try {
    ...
    *itsLog
        << LogIO::NORMAL
        << "Note: this ANTENNA table uses double precision for TIME_INTERVAL. Convention is single."
        << LogIO::POST;
  }
  ScalarColumn<String> name(anTab, "ANNAME");
  ...
  if (nPcal > 0) {
    if (anTab.tableDesc().isColumn("POLCALA") && anTab.tableDesc().isColumn("POLCALB")) {
      ...
      *itsLog << LogIO::WARN << "POLCALA and/or POLCALB column is missing in ANTENNA table."
              << LogIO::POST;
    }
  }

  //  ArrayColumn<Float> beamfwhm(anTab, "BEAMFWHM"); // this column is optional and there is
  //  presently
  ...
  for (Int inRow = 0; inRow < nAnt; inRow++) {
    ...
    if (itsCorrelat == "DIFX" && digLev(inRow) != 2 && digLev(inRow) != 4) {
      *itsLog << LogIO::SEVERE << "unsupported number of digitizer levels for ANTENNA_NO "
              << anNo(inRow) << "." << endl
              << " Digital corrections and amplitude scaling will not be applied to" << endl
              << " baselines involving this antenna." << LogIO::POST;
    }
  }
  ...
  for (Int inRow = 0; inRow < nAnt; inRow++) {
    for (Int inIF = 0; inIF < nIF; inIF++) {
      ...
      if (antIdFromNo.find(anNo(inRow)) != antIdFromNo.end()) {
        ...
        *itsLog << LogIO::SEVERE << "Internal error: no mapping for ANTENNA_NO " << anNo(inRow)
                << LogIO::EXCEPTION;
      }
...
void FITSIDItoMS1::fillFieldTable() {
  *itsLog << LogOrigin("FitsIDItoMS()", "fillFieldTable");
  ...
  if (suTab.tableDesc().isColumn("SOURCE_ID")) {
    ...
    *itsLog << LogIO::NORMAL
            << "No SOURCE_ID column in input SOURCE table. Using deprecated ID_NO column."
            << LogIO::POST;
  ...
  try {                            // try array column first
    ...
    *itsLog << LogIO::NORMAL
            << "Treating ?FLUX, ALPHA, FREQOFF, SYSVEL, and RESTFREQ columns in input SOURCE table "
               "as scalar,"
            << endl
            << " i.e. using same value for all bands." << LogIO::POST;
  }
  ...
  for (Int inRow = 0; inRow < (Int)suTab.nrow(); inRow++) {
    ...
    if (numPoly == 0) {
      if (equinox.isNull()) {
        if (near(epoch(inRow), 2000.0, 0.01)) {
          ...
          *itsLog << " Cannot handle epoch in SU table: " << epoch(inRow) << LogIO::EXCEPTION;
        }
      } else {  // have equinox
        if (StringContains(equinox(inRow), "J2000")) {
          ...
          *itsLog << " Cannot handle equinox in SU table: " << equinox(inRow) << LogIO::EXCEPTION;
        }
      }
    ...
    if (numPoly == 1) {
      radecMeas(1).set(
          MVDirection(pmra(inRow) * C::degree / C::day, pmdec(inRow) * C::degree / C::day),
  ...
  for (Int inRow = 0; inRow < nVal; inRow++) {
    for (Int inIF = 0; inIF < nIF; inIF++) {
      ...
      if (TSYSisScalar) {
        ...
        if (dualPol) tsys(1) = tsys_2S(inRow);
      } else {
        tsys(0) = tsys_1(inRow)(IPosition(1, inIF));
        if (dualPol) tsys(1) = tsys_2(inRow)(IPosition(1, inIF));
      }
...
Bool FITSIDItoMS1::fillFlagCmdTable() {
  *itsLog << LogOrigin("FitsIDItoMS()", "fillFlagCmdTable");
  ...
  if (firstSTK >= 0 || noSTKD < 1 || noSTKD > 4 || firstSTK - noSTKD < -9) {
    *itsLog << LogIO::SEVERE << "Unsupported stokes STK_1 " << firstSTK << "NO_STKD" << noSTKD
            << LogIO::EXCEPTION;
  }
  ...
  for (Int inRow = 0; inRow < nVal; inRow++) {
    ...
    if (ant2 != 0 && antIdFromNo.find(ant2) == antIdFromNo.end()) {
      *itsLog << LogIO::SEVERE << "No mapping for antenna " << ant2 << LogIO::EXCEPTION;
    }
...
Bool FITSIDItoMS1::fillWeatherTable() {
  *itsLog << LogOrigin("FitsIDItoMS()", "fillWeatherTable");
  ...
  for (Int inRow = 0; inRow < nVal; inRow++) {
    ...
    if (weather_hasWater_p) msWeather.H2O().put(outRow, wvr_h2o(inRow));
...
Bool FITSIDItoMS1::handleGainCurve() {
  *itsLog << LogOrigin("FitsIDItoMS()", "handleGainCurve");
  ...
  if (GCisScalar && nTabs == 1) {
    ...
    if (gcTab.tableDesc().isColumn("GAIN_2")) gain_2S.attach(gcTab, "GAIN_2");
  } else {
    gain_1.attach(gcTab, "GAIN_1");
    if (gcTab.tableDesc().isColumn("GAIN_2")) gain_2.attach(gcTab, "GAIN_2");
  }
  ...
  for (Int inRow = 0; inRow < nVal; inRow++) {
    for (Int inIF = 0; inIF < nIF; inIF++) {
      ...
      if (GCisScalar && inIF == 0) {
        if (dualPol && (type_1S(inRow) != type_2S(inRow) || xtyp_1S(inRow) != xtyp_2S(inRow) ||
                        ytyp_1S(inRow) != ytyp_2S(inRow) || nterm_1S(inRow) != nterm_2S(inRow))) {
          *itsLog << "Distinct gain curve types per polarisation" << LogIO::POST;
          continue;
        }
        if (type_1S(inRow) != 2) {
          *itsLog << "Unsupported gain curve type " << type_1S(inRow) << LogIO::POST;
          continue;
        }
        ytyp = ytyp_1S(inRow);
        nterm = nterm_1S(inRow);
      } else if (!GCisScalar) {
        if (dualPol && (type_1(inRow)(thisIF) != type_2(inRow)(thisIF) ||
        ...
        if (type_1(inRow)(thisIF) != 2) {
          *itsLog << "Unsupported gain curve type " << type_1(inRow)(thisIF) << LogIO::POST;
          continue;
        }
      ...
      switch (ytyp) {
        ...
        case 2:
          break;
        default:
          *itsLog << "Unsupported gain curve coordinate type " << ytyp << LogIO::POST;
          continue;
      }
      ...
      if (GCisScalar && nTabs == 1) {
        ...
        if (dualPol) gain.row(1) = gain_2(inRow)(Slice(inIF * nTabs, nterm));
      }
      gainCol.put(outRow, gain);
      if (GCisScalar) {
        sens(0) = sens_1S(inRow);
        if (dualPol) sens(1) = sens_2S(inRow);
      } else {
        sens(0) = sens_1(inRow)(thisIF);
        if (dualPol) sens(1) = sens_2(inRow)(thisIF);
      }
  ...
  for (Int inRow = 0; inRow < nVal; inRow++) {
    for (Int inIF = 0; inIF < nIF; inIF++) {
      ...
      if (PCisScalar) {
        ...
        if (dualPol) freq.row(1) = pc_freq_2(inRow)(Slice(inIF * nTones, nTones));
      }
      ...
      if (PCisScalar) {
        ...
        for (Int i = 0; i < nTones; i++) {
          ...
          if (dualPol)
            phase_cal(1, i) = Complex(pc_real_2(inRow)(thisTone), pc_imag_2(inRow)(thisTone));
        }
      }
...
Bool FITSIDItoMS1::handleModelComps() {
  *itsLog << LogOrigin("FitsIDItoMS()", "handleModelComps");
  ...
  if (array_p == "VLBA" && fftTwid != 1) {
    *itsLog << LogIO::SEVERE << "Data needs FFT artifact corrections; this is not yet implemented."
            << LogIO::POST;
  }
  if (array_p == "VLBA" && taperFn != "UNIFORM") {
    *itsLog << LogIO::SEVERE << "Data was correlated with " << taperFn
            << " taper; support for this taper is not yet implemented." << LogIO::POST;
  }
  ...
  *itsLog << LogIO::NORMAL << "Found binary table " << extname << LogIO::POST;
  ...
  if (extname == "UV_DATA") {
    ...
    if (extname == "ARRAY_GEOMETRY")
      ...
      fillFieldTable();
    else if (extname == "FREQUENCY")
      fillSpectralWindowTable();
    else if (extname == "ANTENNA")
      fillFeedTable();
    else if (extname == "INTERFEROMETER_MODEL")
      success = fillCorrelatorModelTable();
    else if (extname == "SYSTEM_TEMPERATURE")
      success = fillSysCalTable();
    else if (extname == "FLAG")
      success = fillFlagCmdTable();
    else if (extname == "GAIN_CURVE")
      success = handleGainCurve();
    else if (extname == "PHASE-CAL")
      success = handlePhaseCal();
    else if (extname == "WEATHER")
      success = fillWeatherTable();
    else if (extname == "MODEL_COMPS")
      success = handleModelComps();
    else if (extname == "CALC")
      success = handleCalc();
