...
void MSFitsIDI::init(const String& dataSource, const FITS::FitsDevice& deviceType,
  ...
  if (!sourcePath.isValid() || !File(sourcePath).exists() || !File(sourcePath).isReadable()) {
    os << LogIO::SEVERE << "FITS-IDI data source is not readable" << LogIO::EXCEPTION;
  }
    ...
    if (infits.hdutype() != FITS::BinaryTableHDU) {
      ...
      if (hduName != "") {
        ...
        if (success) {
          ...
          if (infits.currsize() > 0) infits.skip_all(FITS::BinaryTableHDU);
        }
      }
    }
  }  // end while
