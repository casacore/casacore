...
MSPrimaryGroupHolder::~MSPrimaryGroupHolder() { detach(); }
...
MSPrimaryTableHolder::~MSPrimaryTableHolder() { detach(); }
...
MSFitsInput::MSFitsInput(const String& msFile, const String& fitsFile, const Bool useNewStyle)
  ...
  if (!f.exists() || !f.isReadable()) {
    _log << LogOrigin("MSFitsInput", "MSFitsInput") << "File " << fitsFile
         << " does not exist or is not readable" << LogIO::EXCEPTION;
  }
  ...
  _log << LogOrigin("MSFitsInput", "MSFitsInput") << LogIO::NORMAL << "Converting FITS file '"
       << fitsFile << "' to MeasurementSet '" << msFile << "'" << LogIO::POST;
  ...
  if (_infile) {
    if (_infile->err() == FitsIO::IOERR) {
      ...
      _log << LogOrigin("MSFitsInput", "MSFitsInput") << "Failed to read initial record -- exiting."
           << LogIO::EXCEPTION;
    } else {
    ...
    _log << LogOrigin("MSFitsInput", "MSFitsInput") << "Failed to open fits file " << fitsFile
         << LogIO::EXCEPTION;
  }
}
...
void MSFitsInput::readRandomGroupUVFits(Int obsType) {
  ...
  if ((estMem < totMem) && (estMem < 1000000)) {
    ...
    try {
      ...
      _log << LogOrigin("MSFitsInput", __func__) << ex.what() << LogIO::EXCEPTION;
    }
  } else {
    // else fill row wise
    try {
  ...
  while (_infile->rectype() != FITS::EndOfFile && !_infile->err()) {
    if (_infile->hdutype() != FITS::BinaryTableHDU) {
      ...
      _log << LogOrigin("MSFitsInput", __func__) << LogIO::NORMAL << "extname=" << type
           << " nrows=" << binTab.nrows() << " ncols=" << binTab.ncols()
      ...
      if (StringContains(type, "AN") && !haveAn) {
        ...
        _log << LogOrigin("MSFitsInput", __func__) << LogIO::NORMAL
             << "Skipping table, duplicate or unrecognized type: " << type << LogIO::POST;
        binTab.fullTable();
      }
    }
  }
  ...
  if (!haveAn) {
    _log << LogOrigin("MSFitsInput", __func__) << "Cannot find an AN Table. This is required."
         << LogIO::EXCEPTION;
  }
  fillFeedTable();
}
  ...
  while (moreToDo && _infile->rectype() != FITS::EndOfFile && !_infile->err()) {
    if (  //_infile->rectype() != FITS::HDURecord ||
      ...
      while (moreToDo && _infile->hdutype() == FITS::BinaryTableHDU) {
        ...
        _log << LogOrigin("MSFitsInput", __func__) << LogIO::NORMAL << "extname=" << bt->extname()
             << " nrows=" << bt->nrows() << " ncols=" << bt->ncols() << " rowsize=" << bt->rowsize()
             << LogIO::POST;
        ...
        if (StringContains(type, "AN")) {
          ...
          _infile->skip_hdu();
          _log << LogOrigin("MSFitsInput", __func__) << LogIO::NORMAL << "skip " << type
               << LogIO::POST;
        }
        _log << LogOrigin("MSFitsInput", __func__) << LogIO::DEBUG1 << "<<<------ Binary Table HDU"
             << LogIO::POST;
...
Bool MSFitsInput::_checkInput(FitsInput& infile) {
  ...
  _log << LogOrigin("MSFitsInput", __func__) << LogIO::DEBUG1
       << "infile.hdutype(): " << infile.hdutype() << LogIO::POST;

  // visibilty must be one of these type
  if (infile.hdutype() != FITS::PrimaryGroupHDU && infile.hdutype() != FITS::PrimaryArrayHDU &&
      infile.hdutype() != FITS::PrimaryTableHDU) {
    _log << LogOrigin("MSFitsInput", __func__) << "Error, neither primary group nor primary table"
         << LogIO::EXCEPTION;
  }
...
void MSFitsInput::getPrimaryGroupAxisInfo() {
  ...
  if ((getIndex(_coordType, "RA") < 0) && (getIndex(_coordType, "RA---SIN") < 0) &&
      (getIndex(_coordType, "RA---NCP") < 0) && (getIndex(_coordType, "RA---SCP") < 0)) {
    _log << "Data does not have a RA axis" << LogIO::EXCEPTION;
  }
  if ((getIndex(_coordType, "DEC") < 0) && (getIndex(_coordType, "DEC--SIN") < 0) &&
      (getIndex(_coordType, "DEC--NCP") < 0) && (getIndex(_coordType, "DEC--SCP") < 0)) {
  ...
  for (uInt i = 0; i < numCorr; i++) {
    ...
    switch (_corrType(i)) {
      ...
      default:
        if (_corrType(i) < 0) {
          _log << "Unknown Correlation type: " << _corrType(i) << LogIO::EXCEPTION;
        }
    }
  }
  ...
  _object = (kwp = _priGroup.kw(FITS::OBJECT)) ? kwp->asString() : "unknown";
  RTrimInPlace(_object, ' ');
  // Save the array name
  _array = (kwp = _priGroup.kw(FITS::TELESCOP)) ? kwp->asString() : "unknown";
  RTrimInPlace(_array, ' ');
  ...
  if (_priGroup.kw(FITS::EPOCH))
    ...
    _epoch = 2000.0;
    _log << LogIO::WARN << "Cannot find epoch of data, defaulting to J2000" << LogIO::POST;
  }
  ...
  if (ok) {
    ...
    ok = FITSSpectralUtil::fromFITSHeader(spectralAxis, referenceChannel, referenceFrequency,
                                          deltaFrequency, frequencies, _freqsys, velPref, _restfreq,
                                          _log, header);
    ...
    _useAltrval = (!header.isDefined(String("specsys")) && header.isDefined(String("altrval")) &&
                   header.isDefined(String("restfreq")) && _freqsys != MFrequency::TOPO);
...
void MSFitsInput::setupMeasurementSet(const String& MSFileName, Bool useTSM, Int obsType) {
  ...
  if (useTSM) {
    ...
    IPosition tileShape = MSTileLayout::tileShape(dataShape, obsType, _array);
    _log << LogOrigin("MSFitsInput", __func__);
    _log << LogIO::NORMAL << "Using tile shape " << tileShape << " for " << _array
         << " with obstype=" << obsType << LogIO::POST;
...
void MSFitsInput::fillObsTables() {
  ...
  String telescope = (kwp = _priGroup.kw(FITS::TELESCOP)) ? kwp->asString() : "unknown";
...
void MSFitsInput::fillAntennaTable(BinaryTable& bt) {
    ...
    if (_array == "OVRO" || _array == "CARMA") {
      ...
      for (Int i = 0; i < nAnt; ++i) {
        ...
        if (myName.starts_with("CM")) {
          antDiams[i] = 7.0;
        } else if (myName.starts_with("DA") || myName.starts_with("DV") ||
  ...
  if (arrnam == "VLA" && (!btKeywords.isDefined("FRAME") || norm(arrayXYZ) > 1e6)) {
    _log << LogOrigin("MSFitsInput", __FUNCTION__) << LogIO::NORMAL
         << "This looks like an old VLA archive UVFITS file" << LogIO::POST;
    ...
    if (diff2 <= 10000.0) {
      ...
      _log << LogIO::WARN << "Array position from UVFITS file is not "
           ...
           << "No rotation of antenna positions will be performed." << LogIO::POST;
    }
  }
...
void MSFitsInput::fillSpectralWindowTable(BinaryTable& bt, Int nSpW) {
  ...
  if (_nIF == 1) {
    ...
    try {
      ...
      _log << LogOrigin("MSFitsInput", "fillSpectralWindowTable") << LogIO::DEBUG1 << x.what()
           << LogIO::POST;
    } catch (...) {
      _log << LogOrigin("MSFitsInput", "fillSpectralWindowTable") << LogIO::DEBUG1
           << "unknown Error" << LogIO::POST;
    }
  }
...
MDirection::Types MSFitsInput::getDirectionFrame(Double epoch) {
  ...
  _log << LogOrigin("MSFitsInput", "getDirectionFrame") << LogIO::DEBUG1 << "epochRef ok "
       << LogIO::POST;
  ...
  try {
    ...
    ThrowIf(throwImmediately,
            "Inconsistent SU table, number of elements in rest frequency column is " +
                std::to_string(nrestfreqs) + " but number of IFs is " + std::to_string(noif));
    sysvel.getColumn(_sysVel);
  } catch (const std::exception& x) {
    ThrowIf(throwImmediately, x.what());
    if (noif > 1) {
      _log << LogOrigin("MSFitsInput", __func__) << LogIO::WARN << x.what() << ": "
  ...
  for (Int inRow = 0; inRow < (Int)suTab.nrow(); inRow++) {
    ...
    if (numPoly == 0) {
      if (near(epoch(inRow), 2000.0, 0.01)) {
        ...
        _log << LogOrigin("MSFitsInput", __func__) << " Assuming standard epoch " << " for "
             << name(inRow) << ".  Be aware that this may not be correct." << endl;
      } else {
        _log << LogOrigin("MSFitsInput", __func__)
             << " Cannot handle epoch in SU table: " << epoch(inRow) << LogIO::EXCEPTION;
      }
...
void MSFitsInput::fillFieldTable(Int nField) {
  ...
  radecMeas(0).set(MVDirection(_refVal(getIndex(_coordType, "RA")) * C::degree,
                               _refVal(getIndex(_coordType, "DEC")) * C::degree),
        ...
        if (sourceFieldIndex.find(myfldspw) == sourceFieldIndex.end()) {
          ...
          if (0 <= lastFieldId && (uInt)lastFieldId < _sysVel.ncolumn()) {
            ...
            _log << LogOrigin("MSFitsInput", "fillExtraTable") << LogIO::WARN
                 << "No systemic velocity for field " << lastFieldId << LogIO::POST;
            sysVel(0) = 0.;
          }
          ...
          if (0 <= lastFieldId && (uInt)lastFieldId < _restFreq.ncolumn()) {
            ...
            _log << LogOrigin("MSFitsInput", "fillExtraTable") << LogIO::WARN
                 << "No rest frequency for field " << lastFieldId << LogIO::POST;
            restFreqs(0) = 0.;
          }
  ...
  if (_timsys == "UTC" || _timsys == "TAI") {
    ...
    if (_timsys != "")
      _log << LogOrigin("MSFitsInput", "fixEpochReferences") << LogIO::SEVERE
           << "Unhandled time reference frame: " << _timsys << LogIO::POST;
  }
}
...
void MSFitsInput::getAxisInfo(ConstFitsKeywordList& kwl) {
  ...
  _log << LogOrigin("MSFitsInput", "fillMSMainTable") << LogIO::DEBUG1 << "coordType=" << _coordType
       << "\nrefVal=" << _refVal << "\nrefPix=" << _refPix << "\ndelta=" << _delta
       << "\n_nPixel=" << _nPixel << LogIO::POST;
}
...
void MSFitsInput::fillMSMainTable(BinaryTable& bt) {
  ...
  _log << LogOrigin("MSFitsInput", "MSFitsInput") << ((needS > freeS) ? LogIO::WARN : LogIO::DEBUG1)
       << "Estimate of Needed Storage Space in MB: " << 0.9 * needS << "~" << 1.6 * needS
       << "\n                       Free Space in MB: " << freeS << LogIO::POST;
  ...
  for (Int group = 0; group < nrows; group++) {
    ...
    try {
      ...
      _log << LogOrigin("MSFitsInput", "fillMSMainTable")
           << "Exception while filling MS main table. " << x.what() << LogIO::EXCEPTION;
    }
  }
  // If determining interval on-the-fly, fill interval/exposure columns
  //  now:
