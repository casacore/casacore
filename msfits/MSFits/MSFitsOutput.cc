...
MSFitsOutput::MSFitsOutput(const String& fitsfile, const MeasurementSet& ms, const String& column)
    : _fitsfile(fitsfile),
      _column(column),
      _ms(ms),
      _startChan(0),
...
void MSFitsOutput::write() const {
  ...
  if (obsCols.nrow() > 0 && obsCols.telescopeName()(0) == "WSRT") {
    ThrowIf(!MSFitsOutputAstron::writeFitsFile(_fitsfile, _ms, _column, _startChan, _nchan,
                                               _stepChan, _writeSysCal, _asMultiSource, _combineSpw,
                                               _writeStation, _sensitivity),
  ...
  os << LogIO::NORMAL << "Converting MeasurementSet " << _ms.tableName() << " to FITS file '"
     << outfile << "'" << LogIO::POST;
  ...
  ThrowIf(!_writeFQ(fitsOutput, _ms, spwidMap, nrspw, refFreq, refPixelFreq, chanbw, _combineSpw,
                    _startChan, _nchan, _stepChan, _avgChan),
          "Could not write FQ table");
  os << LogIO::NORMAL << "Writing AIPS AN table" << LogIO::POST;
  ThrowIf(!_writeAN(fitsOutput, _ms, refFreq, _writeStation), "Could not write AN table");
  if (doMultiSource) {
    os << LogIO::NORMAL << "Writing AIPS SU table" << LogIO::POST;
    if (!_writeSU(fitsOutput, _ms, fieldidMap, nrfield, spwidMap, nrspw)) {
  ...
  if (_writeSysCal) {
    if (_ms.sysCal().tableDesc().ncolumn() == 0) {
      ...
      if (!bk) {
        ...
        bk = _writeGC(fitsOutput, _ms, syscal, spwidMap, nrspw, _combineSpw, _sensitivity,
...
std::shared_ptr<FitsOutput> MSFitsOutput::_writeMain(Int& refPixelFreq, Double& refFreq,
  ...
  for (i = 0; i < ndds; i++) {
    if (i < spwidMap.nelements() && spwidMap[i] >= 0) {
      ...
      if (numcorr0 == 0) {
        ...
        if ((_nchan > 0) && (_stepChan > 0) && (_startChan >= 0) &&
  ...
  if (col == "OBSERVED" || col == MS::columnName(MS::DATA)) {
    ...
    if (_ms.tableDesc().isColumn("MODEL_DATA")) {
      ...
      columnName = MS::columnName(MS::DATA);
      os << LogIO::SEVERE << "MODEL_DATA does not exist, writing DATA" << LogIO::POST;
    }
    ...
    if (_ms.tableDesc().isColumn("CORRECTED_DATA")) {
      ...
      columnName = MS::columnName(MS::DATA);
      os << LogIO::NORMAL << "CORRECTED_DATA does not exist, writing DATA" << LogIO::POST;
    }
  } else {
    columnName = MS::columnName(MS::DATA);
    os << LogIO::SEVERE << "Unrecognized column " << _column << ", writing DATA" << LogIO::POST;
  }
  ...
  if (stokes.nelements() > 1) {
    ek.define("cdelt3", (stokes(stokesIndex(1)) - stokes(stokesIndex(0))) * 1.0);
  } else {
  ...
  if (restFreq > 0.0) ek.define("restfreq", restFreq);
  ...
  if (_combineSpw) {
    ...
    for (uInt i = 0; i < ndds; ++i) {
      if (i < spwidMap.nelements() && spwidMap[i] >= 0) {
        if (ifnum < nif) {
          ...
          os << LogIO::WARN << "spwidMap selects more spws than there are IFs.  Expect problems."
             << LogIO::POST;
        }
      }
    }
    ...
    if (nif > 1) {          // Don't bother counting all the inspwinids
      ...
      if (nOutRow % nif) {
        os << LogIO::SEVERE << "The expected # of output rows, " << nOutRow
  ...
  while (tbfrownr < nrow) {
    ...
    for (uInt m = 0; m < nif; ++m) {
      ...
      if (_combineSpw && (rownr >= nrow  // flag remaining IFs in tbfrownr
        ...
        if (!padWithFlags || rawrownr <= tbfend) {
          ...
          rownr = _combineSpw && nif > 1 ? sortIndex[rawrownr] : rawrownr;
        }
      }
    ...
    *odate2 = dayFraction;
    ...
    // BASELINE
    if (maxant < 256) {
      *obaseline = antnumbers(inant1(tbfrownr)) * 256 + antnumbers(inant2(tbfrownr)) +
    ...
    *ofreqsel = _combineSpw ? 1 : 1 + spwidMap[inspwinid(tbfrownr)];
    ...
    if (rawrownr == tbfrownr) {
      os << LogIO::WARN << "No spectral windows were present for row # " << tbfrownr << "\n"
      ...
      if (nspws_found != old_nspws_found) {
        ...
        os << LogIO::DEBUG1 << " input (time_centroid, ant1, ant2, field) =" << LogIO::POST;
        ...
        os << LogIO::DEBUG1 << nspws_found << " spws present out of " << nif << " IFs."
           << LogIO::POST;
      }
    ...
    if (i < spwidMap.nelements() && spwidMap[i] >= 0) {
      *freqsel = 1 + spwidMap[i];
      ...
      if (telescopeName == "IRAM PDB" || telescopeName == "IRAM_PDB") {
        ...
        os << LogIO::DEBUG2 << "refPixelFreq=" << refPixelFreq << " refFreq="
           << refFreq
           //<< "\nfreqs=" << freqs
           << LogIO::POST;
        Int chancounter = 0;
...
Bool MSFitsOutput::_writeAN(std::shared_ptr<FitsOutput> output, const MeasurementSet& ms,
  ...
  for (uInt arraynum = 0; arraynum < narray; ++arraynum) {
    ...
    os << LogIO::NORMAL              // Requested by CAS-437.
       << "Using " << posref << " frame for antenna positions." << LogIO::POST;
    ...
    *polcalb = 0.0;
    ...
    for (uInt antnum = 0; antnum < nant; ++antnum) {
      ...
      *poltyb = " ";
        ...
        if (Int(antnum) == inantid(i) &&
          ...
          if (antToRA.find(antnum) == antToRA.end()) {
            ...
            try {
              ...
              os << LogOrigin("MSFitsOutput", __func__) << LogIO::WARN
                 << "Receptor Angles in FEED table are not constant for antenna " << antnum
                 << ". Will try to ignore this. If this data is not yet calibrated, a future "
                    "calibration of the uvfits data may fail: "
                 << x.what() << LogIO::POST;
            }
          }
        }
      ...
      if (!found) {
        os << LogIO::SEVERE << "Could not find polarization types for antenna " << antnum
           << LogIO::POST;
      }
            ...
            try {
              ...
              *pmdec = pm(1);
              os << LogIO::WARN << "Proper motion for source in SOURCE table row #" << rownr
                 << " has unfamiliar units: " << unit
                 << " .\n Value in uv fits table needs to be checked." << LogIO::POST;
            }
          }
...
Bool MSFitsOutput::_writeGC(std::shared_ptr<FitsOutput> output, const MeasurementSet& ms,
  ...
  Table sorcal = syscal.sort(sortNames, Sort::Ascending, Sort::InsSort + Sort::NoDuplicates);
...
Bool MSFitsOutput::_writeWX(std::shared_ptr<FitsOutput> output, const MeasurementSet& ms) {
  ...
  for (uInt i = 0; i < nrow; i++) {
    ...
    *antenna = (weatherColumns.antennaId()(i) == -1 ? 0 : antnums(weatherColumns.antennaId()(i)));
...
Bool MSFitsOutput::_writeSY(std::shared_ptr<FitsOutput> output, const MeasurementSet& ms,
  ...
  static const std::vector<String> expColNames{
      ...
      "INTERVAL", "SWITCHED_DIFF", "SWITCHED_SUM",     "REQUANTIZER_GAIN"};
  ...
  // IFs to the uvfits table currently is because the only way to test that is to read the uvfits
...
void MSFitsOutput::getStartHA(Double& startTime, Double& startHA, const MeasurementSet& ms,
  ...
  MeasFrame frame(pos);
  frame.set(stTime);
  MDirection out = MDirection::Convert(delay, MDirection::Ref(MDirection::HADEC, frame))();
...
Table MSFitsOutput::handleSysCal(const MeasurementSet& ms, const Vector<Int>& spwids,
  ...
  }
  {
    ...
    if (flagged) {
      ...
      os << LogIO::NORMAL << "Skipped unused antennas in SYSCAL table (" << nrrow - syscal.nrow()
         << " entries)" << LogIO::POST;
    }
  }
  ...
  if (isSubset) {
    syscal = syscal(
        syscal.col(MSSysCal::columnName(MSSysCal::SPECTRAL_WINDOW_ID)).in(TableExprNode(spwids)));
  }
...
void MSFitsOutput::_handleAntNumbers(const MeasurementSet& ms, Vector<Int>& antnumbers) {
  ...
  String arrayName;
  if (obscol.nrow() > 0) arrayName = obscol.telescopeName()(0);
  ...
  for (Int iant = 0; iant < nAnt; ++iant) {
    ...
    if (StringContains(arrayName, "VLA")) {
      ...
      if (int_pos != std::string::npos) name = name.substr(int_pos);
    } else {
      name = antname(iant);
    }
