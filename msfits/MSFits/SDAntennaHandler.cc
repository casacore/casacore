...
SDAntennaHandler &SDAntennaHandler::operator=(const SDAntennaHandler &other) {
  if (this != &other) {
    ...
    nameKey_p.attachToRecord(index_p->accessKey(), MSAntenna::columnName(MSAntenna::NAME));
...
void SDAntennaHandler::initAll(MeasurementSet &ms, Vector<Bool> &handledCols, const Record &row) {
  ...
  nameKey_p.attachToRecord(index_p->accessKey(), MSAntenna::columnName(MSAntenna::NAME));
  if (stationField_p.isAttached()) {
    stationKey_p.attachToRecord(index_p->accessKey(), MSAntenna::columnName(MSAntenna::STATION));
  }
  if (mountField_p.isAttached()) {
    mountKey_p.attachToRecord(index_p->accessKey(), MSAntenna::columnName(MSAntenna::MOUNT));
  }
  if (dishDiameterField_p.isAttached()) {
  ...
  if (flagRowField_p.isAttached()) {
    flagRowKey_p.attachToRecord(index_p->accessKey(), MSAntenna::columnName(MSAntenna::FLAG_ROW));
  }
...
void SDAntennaHandler::initRow(Vector<Bool> &handledCols, const Record &row) {
  ...
  if (row.fieldNumber("ANTENNA_OFFSET") >= 0 && row.dataType("ANTENNA_OFFSET") == TpArrayDouble) {
    offsetField_p.attachToRecord(row, "ANTENNA_OFFSET");
    handledCols(row.fieldNumber("ANTENNA_OFFSET")) = True;
  }
  if (row.fieldNumber("ANTENNA_ORBIT_ID") >= 0 && row.dataType("ANTENNA_ORBIT_ID") == TpInt) {
  ...
  if (row.fieldNumber("ANTENNA_FLAG_ROW") >= 0 && row.dataType("ANTENNA_FLAG_ROW") == TpBool) {
...
void SDAntennaHandler::addPhasedArrayIdColumn() {
  ...
  if (!phasedIdKey_p.isAttached() && index_p) {
    ...
    indexNames(indexNames.nelements() - 1) = MSAntenna::columnName(MSAntenna::PHASED_ARRAY_ID);
    ...
    if (anyEQ(indexNames, MSAntenna::columnName(MSAntenna::ORBIT_ID))) {
      orbitIdKey_p.attachToRecord(index_p->accessKey(), MSAntenna::columnName(MSAntenna::ORBIT_ID));
    }
  }
}
