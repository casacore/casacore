...
void SDFITSHandler::initAll(MeasurementSet &ms, Vector<Bool> &handledCols, const Record &row) {
  ...
  if (anyEQ(static_cast<Vector<Bool>>(handledCols), False)) {
    ...
    if (ms.keywordSet().fieldNumber("NS_SDFITS") >= 0 &&
      ...
      SetupNewTable newtab(ms.tableName() + "/NS_SDFITS", td, Table::New);
      StandardStMan stman;
...
TableDesc SDFITSHandler::requiredTableDesc(Vector<Bool> &handledCols, Vector<String> &colNames,
    ...
    if (!handledCols(i)) {
      ...
      if (colName == "TIME" || colName == "INTERVAL") {
        ...
        switch (row.type(i)) {
          ...
          default:
            LogIO os;
            os << LogIO::SEVERE << WHERE << "Field " << colName << " has an invalid type, "
               << Int(row.type(i)) << " -- this should never happen." << LogIO::EXCEPTION;
            break;
        }
      }
    }
  ...
  TableMeasDesc<MEpoch> measCol(TableMeasValueDesc(td, "TIME"), TableMeasRefDesc(MEpoch::DEFAULT));
  measCol.write(td);
