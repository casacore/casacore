  ...
  if (msField_p) {
    ...
    if (fieldIdField_p.isAttached() && *fieldIdField_p >= 0) {
      ...
      if (found && nameField_p.isAttached()) {
        found = name == *nameField_p && *nameField_p == msFieldCols_p->name()(thisRow);
      }
      if (found && timeField_p.isAttached()) {
        found = time == *timeField_p && *timeField_p == msFieldCols_p->time()(thisRow);
      }
      ...
      found = found && checkPhase && allEQ(phasePoly, msFieldCols_p->phaseDir()(thisRow));
      found = found && checkRef && allEQ(referencePoly, msFieldCols_p->referenceDir()(thisRow));
