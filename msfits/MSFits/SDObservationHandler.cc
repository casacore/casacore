...
void SDObservationHandler::attach(MeasurementSet &ms, Vector<Bool> &handledCols,
                                  const Record &row) {
  clearAll();
  initAll(ms, handledCols, row);
}
  ...
  if (msObs_p) {
    ...
    if (obsid_p.isAttached() && (*obsid_p).length() > 0) {
      if (!ns_obsidKey_p.isAttached()) {
        ...
        msObs_p->addColumn(
            ScalarColumnDesc<String>("NS_OBSID", "SDFITS OBSID keyword/column value"));
        nsObsIdCol_p.attach(*msObs_p, "NS_OBSID");
...
void SDObservationHandler::initAll(MeasurementSet &ms, Vector<Bool> &handledCols,
                                   const Record &row) {
