...
SDSourceHandler &SDSourceHandler::operator=(const SDSourceHandler &other) {
  if (this != &other) {
    ...
    nameKey_p.attachToRecord(index_p->accessKey(), MSSource::columnName(MSSource::NAME));
    codeKey_p.attachToRecord(index_p->accessKey(), MSSource::columnName(MSSource::CODE));
      ...
      while (!rowFound && whichOne < foundRows.nelements()) {
        ...
        if (rowFound && pulsarIdField_p.isAttached()) {
          if (msSourceCols_p->pulsarId().isNull())
            rowFound = !(*pulsarIdField_p >= 0);
          else
            rowFound = msSourceCols_p->pulsarId()(rownr) == *pulsarIdField_p;
        }
...
void SDSourceHandler::initAll(MeasurementSet &ms, Vector<Bool> &handledCols, const Record &row) {
