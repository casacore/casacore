...
struct casa_array_to_python {
  static boost::python::object makeobject(Array<T> const& arr) { return makePyArrayObject(arr); }
  static PyObject* convert(Array<T> const& c) { return boost::python::incref(makeobject(c).ptr()); }
};
