...
template <>
struct TypeConvTraits<signed char> {
...
template <typename T>
Array<T> ArrayCopy<T>::toArray(const IPosition& shape, void* data, bool copy) {
...
Array<Complex> ArrayCopy<Complex>::toArray(const IPosition& shape, void* data, bool copy) {
  if (!copy) {
    if (sizeof(Complex) == sizeof(TypeConvTraits<Complex>::python_type)) {
...
Array<DComplex> ArrayCopy<DComplex>::toArray(const IPosition& shape, void* data, bool copy) {
  if (!copy) {
    if (sizeof(DComplex) == sizeof(TypeConvTraits<DComplex>::python_type)) {
...
Array<String> ArrayCopy<String>::toArray(const IPosition& shape, void* data, bool) {
...
ValueHolder makeArray(PyObject* obj_ptr, Bool copyData) {
  ...
  if (!PyArray_ISCONTIGUOUS(po) || !PyArray_ISALIGNED(po) || PyArray_ISBYTESWAPPED(po)) {
