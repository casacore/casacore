...
Array<String> ArrayCopyUnicode_toArray(const IPosition& shape, void* data, size_t slen) {
  ...
  for (size_t i = 0; i < nr; ++i) {
    ...
    boost::python::object unicode(
        boost::python::handle<>(PyUnicode_DecodeUTF32(src, slen, NULL, NULL)));
    if (unicode) {
      boost::python::object utf8(boost::python::handle<>(PyUnicode_AsUTF8String(unicode.ptr())));
      if (utf8) to[i] = String(PyBytes_AS_STRING(utf8.ptr()));
    }
    src += slen;
  }
  return arr;
...
Bool PycArrayScalarCheck(PyObject* obj_ptr, int& type) {
  ...
  int types[ntypes] = {NPY_INT32,      NPY_INT64,  NPY_FLOAT32, NPY_FLOAT64, NPY_COMPLEX64,
                       NPY_COMPLEX128, NPY_UINT32, NPY_UINT64,  NPY_BOOL,    NPY_INT16,
                       NPY_UINT16,     NPY_INT8,   NPY_UINT8};
