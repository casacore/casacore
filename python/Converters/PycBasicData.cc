...
bool pyregistry::get(const std::string& name) { return _registry[name]; }
void pyregistry::set(const std::string& name) { _registry[name] = true; }
...
void convert_casa_iposition::reg() {
  ...
  if (!pyregistry::get(tname)) {
    ...
    from_python_sequence<casacore::IPosition, casa_reversed_variable_capacity_policy>();
  }
}
