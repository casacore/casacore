...
struct casa_string_to_python_str {
  ...
  static PyObject* convert(String const& s) { return boost::python::incref(makeobject(s).ptr()); }
};
...
struct casa_string_from_python_str {
  ...
  static void construct(PyObject* obj_ptr,
    ...
    if (PyUnicode_Check(obj_ptr)) {
      PyObject* temp_bytes =
          PyUnicode_AsEncodedString(obj_ptr, "UTF-8", "strict");  // Owned reference
...
struct std_vector_to_list {
  std_vector_to_list() {
    boost::python::to_python_converter<std::vector<T>, to_list<std::vector<T>>>();
  }
};
template <typename T>
struct casa_array_to_list {
  casa_array_to_list() {
    boost::python::to_python_converter<casacore::Array<T>, to_list<casacore::Array<T>>>();
  }
};
template <typename T>
struct casa_vector_to_list {
  casa_vector_to_list() {
    boost::python::to_python_converter<casacore::Vector<T>, to_list<casacore::Vector<T>>>();
  }
};
struct casa_iposition_to_list {
  casa_iposition_to_list() {
    boost::python::to_python_converter<casacore::IPosition, to_list<casacore::IPosition>>();
  }
};
...
struct from_python_sequence {
  ...
  static void* convertible(PyObject* obj_ptr) {
    ...
    if (PyBool_Check(obj_ptr) || PyLong_Check(obj_ptr) || PyFloat_Check(obj_ptr) ||
  ...
  static void construct(PyObject* obj_ptr,
    ...
    void* storage = ((rvalue_from_python_storage<ContainerType>*)data)->storage.bytes;
    new (storage) ContainerType();
    data->convertible = storage;
    ContainerType& result = *((ContainerType*)storage);
    if (PyBool_Check(obj_ptr) || PyLong_Check(obj_ptr) || PyFloat_Check(obj_ptr) ||
        PyComplex_Check(obj_ptr) || PyUnicode_Check(obj_ptr)
#ifndef IS_PY3K
        || PyString_Check(obj_ptr) || PyInt_Check(obj_ptr)
#endif
 ...
 private:
  ...
  static bool check_convertibility(PyObject* obj_ptr) {
    ...
    if (ConversionPolicy::check_convertibility_per_element()) {
      if (!ConversionPolicy::check_size(boost::type<ContainerType>(), obj_size)) return false;
...
inline void register_convert_casa_string() { convert_casa_string::reg(); }

// Register the IPosition conversion.
struct convert_casa_iposition {
  static void reg();
};
inline void register_convert_casa_iposition() { convert_casa_iposition::reg(); }
...
struct convert_std_vector {
  static void reg() {
    ...
    if (!pyregistry::get(tname)) {
      ...
      from_python_sequence<std::vector<T>, stl_variable_capacity_policy>();
    }
  }
};
template <typename T>
...
struct convert_casa_vector {
  static void reg() {
    ...
    if (!pyregistry::get(tname)) {
      ...
      from_python_sequence<casacore::Vector<T>, casa_variable_capacity_policy>();
    }
  }
};
template <typename T>
