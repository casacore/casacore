...
boost::python::object casa_value_to_python::makeobject(ValueHolder const& vh) {
  if (vh.isNull()) {
  ...
  switch (vh.dataType()) {
    ...
    default:
      throw AipsError("PycValueHolder: unknown casa data type " + ValueToString(vh.dataType()));
  }
}
...
#endif
        || PyLong_Check(obj_ptr) || PyFloat_Check(obj_ptr) || PyComplex_Check(obj_ptr) ||
        PyUnicode_Check(obj_ptr) || PyDict_Check(obj_ptr) || PyList_Check(obj_ptr) ||
        PyTuple_Check(obj_ptr) || PyIter_Check(obj_ptr) || PyRange_Check(obj_ptr) ||
...
void casa_value_from_python::construct(
  ...
  void* storage = ((rvalue_from_python_storage<ValueHolder>*)data)->storage.bytes;
...
ValueHolder casa_value_from_python::toVector(PyObject* obj_ptr) {
  ...
  switch (dt) {
    ...
    case TpDComplex:
      return ValueHolder(
          from_python_sequence<Vector<DComplex>, casa_variable_capacity_policy>::make_container(
              obj_ptr));
    case TpString:
      return ValueHolder(
          from_python_sequence<Vector<String>, casa_variable_capacity_policy>::make_container(
              obj_ptr));
    case TpOther:
...
DataType casa_value_from_python::checkDataType(PyObject* obj_ptr) {
  ...
  if (!(PyList_Check(obj_ptr) || PyTuple_Check(obj_ptr) || PyIter_Check(obj_ptr) ||
        PyRange_Check(obj_ptr) || PySequence_Check(obj_ptr))) {
    return TpOther;
  }
