...
void FitGaussian<T>::setFirstEstimate(const Matrix<T>& estimate) {
  if ((estimate.nrow() != itsNGaussians) || (estimate.ncolumn() != itsDimension * 3))
    throw(
        AipsError("FitGaussian<T>::setfirstestimate(const Matrix<T>& "
...
void FitGaussian<T>::setRetryFactors(const Matrix<T>& retryfactors) {
  if (retryfactors.ncolumn() != itsDimension * 3)
    throw(
        AipsError("FitGaussian<T>::setretryfactors(const Matrix<T>&"
...
Matrix<T> FitGaussian<T>::fit(const Matrix<T>& pos, const Vector<T>& f, const Vector<T>& sigma,
  ...
  if (pos.ncolumn() != itsDimension)
    throw(
        AipsError("FitGaussian<T>::fit(const Matrix<T>& pos, const"
      ...
      if (itsChisquare < 0) {
        ...
        for (uInt g = 0; g < itsNGaussians; g++) {
          if ((itsDimension == 1 && solution(g * ngpars + 2) < 0) ||
              ...
              (itsDimension == 3 && (solution(g * ngpars + 4) < 0 || solution(g * ngpars + 5) < 0 ||
                                     solution(g * ngpars + 6) < 0))) {
            fitfailure = 4;
            os << LogIO::DEBUG1 << "Unsuccessful - Negative axis widths not permissible."
               << LogIO::POST;
            break;
          }
        }
        ...
        if (!fitfailure) {
          ...
          if (itsRMS > maximumRMS) {
            ...
            os << LogIO::DEBUG1 << "Converged after " << fitter.currentIteration() << " iterations"
               << LogIO::POST;
          }
  ...
  } while ((fitfailure) && (attempt <= itsMaxRetries) && (timer.real() < itsMaxTime));
  ...
  if (itsSuccess) {
    if (fitfailure) {
      ...
      os << LogIO::DEBUG1 << "no fit satisfies RMS criterion; using best available fit"
         << LogIO::POST;
    }
