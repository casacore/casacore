...
GenericL2Fit<T> &GenericL2Fit<T>::operator=(const GenericL2Fit &other) {
  if (this != &other) {
    ...
    aCount_ai = other.aCount_ai;
    svd_p = other.svd_p;
    if (other.ptr_derive_p)
      ptr_derive_p = other.ptr_derive_p->clone();
    else
      ptr_derive_p = 0;
...
template <class T>
void GenericL2Fit<T>::setFunctionEx() {
...
Bool GenericL2Fit<T>::setConstraintEx(const uInt n,
  ...
  constrArg_p[n] = 0;
  constrArg_p[n] = new Vector<typename FunctionTraits<T>::BaseType>(x.copy());
...
typename FunctionTraits<T>::BaseType GenericL2Fit<T>::getVal_p(
  ...
  if (ptr_derive_p) {
    if (x.ndim() == 1) {
      valder_p =
          (*ptr_derive_p)(static_cast<const Vector<typename FunctionTraits<T>::BaseType> &>(x)[i]);
    } else {
  ...
  for (uInt i = 0; i < nrows; i++) {
    ...
    if (sigma) {
      if ((*sigma)[i] == typename FunctionTraits<T>::BaseType(0) ||
          (*sigma)[i] == typename FunctionTraits<T>::BaseType(-1))
        continue;
      sig = (*sigma)[i];
      if (!asweight_p) {
...
void GenericL2Fit<T>::buildConstraint() {
  ...
  for (uInt i = 0; i < constrFun_p.nelements(); ++i) {  // all constraints
    ...
    if (i < nConstraints())
      LSQFit::setConstraint(i, ceqit, b);
    else
      LSQFit::addConstraint(ceqit, b);
  }
}
