...
LSQFit::LSQFit(uInt nUnknowns, const LSQReal &, uInt nConstraints)
    : state_p(0),
      nun_p(nUnknowns),
      ncon_p(nConstraints),
      n_p(0),
...
LSQFit::LSQFit(uInt nUnknowns, const LSQComplex &, uInt nConstraints)
    : state_p(0),
      nun_p(2 * nUnknowns),
...
void LSQFit::clear() {
  if (piv_p)
    for (uInt *i = piv_p; i != piv_p + n_p; ++i) *i = i - piv_p;
...
void LSQFit::copy(const LSQFit &other, Bool all) {
  ...
  if (other.error_p) std::copy(other.error_p, other.error_p + N_ErrorField, error_p);
  if (other.constr_p) std::copy(other.constr_p, other.constr_p + ncon_p * nun_p, constr_p);
...
Bool LSQFit::solveItLoop(Double &fit, uInt &nRank, Bool doSVD) {
  if (!(state_p & NONLIN)) {  // first time through loop
    ...
    if (d0 > 0)
      fit = (error_p[SUMLL] - nar_p->error_p[SUMLL]) / d0;
    else
      fit = -1e-10;  // dummy
      ...
      for (uInt i1 = i; i1 < nIndex; ++i1) {
        if (nEqIndex[i1] < nun_p) {
          if (nEqIndex[i] <= nEqIndex[i1]) {
            ...
            norm_p->row(nEqIndex[i1])[nEqIndex[i]] += i3[i1];
        }
      }
    }
  }
...
const std::string &LSQFit::readyText() const {
  static std::string txt[LSQFit::N_ReadyCode] = {"Not ready",
...
void LSQFit::debugIt(uInt &nun, uInt &np, uInt &ncon, uInt &ner, uInt &rank, Double *&nEq,
                     Double *&known, Double *&constr, Double *&er, uInt *&piv, Double *&sEq,
