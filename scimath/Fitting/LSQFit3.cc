...
Bool LSQFit::fromRecord(String &error, const RecordInterface &in) {
  if (in.isDefined(recid) && in.type(in.idToNumber(RecordFieldId(recid))) == TpString &&
      ...
      in.isDefined(ncon) && in.type(in.idToNumber(RecordFieldId(ncon))) == TpInt &&
      in.isDefined(prec) && in.type(in.idToNumber(RecordFieldId(prec))) == TpDouble &&
      in.isDefined(startnon) && in.type(in.idToNumber(RecordFieldId(startnon))) == TpDouble &&
      in.isDefined(nonlin) && in.type(in.idToNumber(RecordFieldId(nonlin))) == TpDouble &&
      in.isDefined(rank) && in.type(in.idToNumber(RecordFieldId(rank))) == TpInt &&
      in.isDefined(nnc) && in.type(in.idToNumber(RecordFieldId(nnc))) == TpInt) {
    String rrecid;
