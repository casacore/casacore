...
int main(int argc, const char *argv[]) {
  ...
  switch (ft) {
    case '1': {
      ...
      dynamic_cast<CompoundFunction<AutoDiff<Double>> *>(gauss)->addFunction(g1);
      dynamic_cast<CompoundFunction<AutoDiff<Double>> *>(gauss)->addFunction(g2);
      dynamic_cast<CompoundFunction<AutoDiff<Double>> *>(gauss)->addFunction(p1);
    ...
    default: {
      ...
      dynamic_cast<CompiledFunction<AutoDiff<Double>> *>(gauss)->setFunction(
          "p6+p0*exp(-((x-p1)/p2)^2) + p3*exp(-((x-p4)/p5)^2)");
  ...
  try {
    ...
    // Show data
    cout << "# solutions, parameters, constraints: " << solution.nelements() << ", "
         << gauss->nparameters() << ", " << fitter.nConstraints() << endl;
