...
void checkLinearFit(LinearFitSVD<Double> &fitter) {
  ...
  // fit data to polynomial
  {
    ...
    for (uInt i = 0; i < combination.nparameters(); i++) {
      cout << "Actual Parameter " << actualParameters(i) << ", Computed Parameter " << solution(i)
           << endl;
    }
    // Print actual covariance and computed covariance
    for (uInt i = 0; i < combination.nparameters(); i++) {
      for (uInt j = 0; j < combination.nparameters(); j++) {
        cout << "Actual Covariance " << actualCovariance(i, j) << ", Computed Covariance "
             << covariance(i, j) << endl;
      }
    }
    cout << "actual ChiSquare " << actualChiSquare << " Computed ChiSquare " << fitter.chiSquare()
         << endl;
    ...
    AlwaysAssertExit(allNear(actualParameters, solution, 1.0e-5));
    // Compare actualCovariance with the covariance matrix
    AlwaysAssertExit(allNear(actualCovariance, covariance, 1.0e-5));
    // Compare actualChiSquare with the chiSquare value
    AlwaysAssertExit(near(actualChiSquare, fitter.chiSquare(), 1.0e-5));
    AlwaysAssertExit(near(actualChiSquare, sum(yres), 1.0e-5));
  ...
  // fitting polynomial to data with some coefficient being held fixed
  {
    ...
    for (uInt i = 0; i < solution.nelements(); i++) {
      if (i == 2)
        cout << "Fixed coefficient ";
      else
        cout << "Computed ";
      cout << solution(i) << " Std Dev " << sqrt(covariance(i, i)) << endl;
    }
    cout << "Solved for " << fitter.fittedNumber() << " parameters" << endl;
    AlwaysAssertExit(fitter.fittedNumber() == combination.parameters().nMaskedParameters());
    AlwaysAssertExit(fitter.fittedNumber() - fitter.getRank() == 0);
    cout << "Missing rank: " << fitter.fittedNumber() - fitter.getRank() << endl;
    ...
    for (uInt i = 0; i < solution.nelements(); i++) {
      ...
      AlwaysAssertExit(nearAbs(solution(i), poly[j], factor * sqrt(covariance(i, i))));
      j++;
    }
  }
  //************ test three ****************

  {
    ...
    // f(x,y) = a0 + a1*x+ a2*y + a3*x*x
    {
      ...
      cout << "Expect: " << 4 + 5 * z0[0] + 6 * sin(z0[1]) + 0.2 * z0[0] * z0[0] << endl;
...
void checkComplexLinearFit(LinearFitSVD<Complex> &fitter) {
  ...
  for (uInt i = 0; i < solution.nelements(); i++) {
    cout << "Expected: (" << poly[i].real() << "," << poly[i].imag() << ") ";
    cout << "Computed: (" << solution(i).real() << "," << solution(i).imag() << ") ";
  ...
  Double factor = 3;
  for (uInt i = 0; i < solution.nelements(); i++) {
    AlwaysAssertExit(nearAbs(abs(solution(i)), abs(poly[i]), factor * abs(sqrt(covariance(i, i)))));
  }
