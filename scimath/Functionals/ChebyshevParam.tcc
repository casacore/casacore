...
ChebyshevParam<T>::ChebyshevParam()
    : Function1D<T>(1),
      ...
      minx_p(T(-1)),
      maxx_p(T(1)),
      mode_p(ChebyshevEnums::CONSTANT) {}
...
ChebyshevParam<T>::ChebyshevParam(const T &min, const T &max,
                                  ChebyshevEnums::OutOfIntervalMode mode, const T &defval)
...
template <class T>
ChebyshevParam<T>::ChebyshevParam(const Vector<T> &coeffs, const T &min, const T &max,
                                  ChebyshevEnums::OutOfIntervalMode, const T &defval)
    : Function1D<T>(coeffs.nelements()),
      def_p(defval),
      minx_p(min),
      maxx_p(max),
      mode_p(ChebyshevEnums::CONSTANT) {
...
template <class T>
ChebyshevParam<T>::ChebyshevParam(const Vector<T> &coeffs, const RecordInterface &mode)
    : Function1D<T>(coeffs.nelements()),
      def_p(T(0)),
      minx_p(T(-1)),
      maxx_p(T(1)),
...
template <class T>
ChebyshevParam<T> &ChebyshevParam<T>::operator=(const ChebyshevParam<T> &other) {
  if (this != &other) {
    mode_p = other.mode_p;
    minx_p = other.minx_p;
    maxx_p = other.maxx_p;
...
void ChebyshevParam<T>::setCoefficients(const Vector<T> &coeffs) {
  if (coeffs.nelements() == 0) {
    throw AipsError(
        "ChebyshevParam<T>::setCoeffiecients(): "
        "empty Vector passed");
  }
...
template <class T>
void ChebyshevParam<T>::setCoefficient(const uInt which, const T &value) {
...
template <class T>
void ChebyshevParam<T>::derivativeCoeffs(Vector<T> &coeffs, const T &xmin, const T &xmax) {
  ...
  for (uInt i = 1; i < ce.nelements(); ++i) dce(i - 1) = T(2 * i) * ce(i) / (xmax - xmin);
...
void ChebyshevParamModeImpl<T>::setMode(const RecordInterface &in) {
  ...
  if (in.isDefined(String("interval"))) {
    ...
    if (in.type(in.idToNumber(fld)) == TpArrayDouble ||
        ...
        in.type(in.idToNumber(fld)) == TpArrayFloat || in.type(in.idToNumber(fld)) == TpArrayInt) {
      Vector<T> intv;
      in.get(fld, intv);
      if (intv(0) < intv(1))
  ...
  if (in.isDefined(String("default"))) {
    ...
    if (in.type(in.idToNumber(fld)) == TpDouble || in.type(in.idToNumber(fld)) == TpComplex ||
        in.type(in.idToNumber(fld)) == TpDComplex || in.type(in.idToNumber(fld)) == TpFloat ||
