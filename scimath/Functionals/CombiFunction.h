...
template <class T>
class CombiFunction : public CombiParam<T> {
 public:
  // # Constructors
...
template <class T>
class CombiFunction_PS<AutoDiff<T>> : public CombiParam<AutoDiff<T>> {
 public:
  // # Constructors
  //  The default constructor -- no functions, no parameters, nothing, the
  //  function operator returns a 0.
  ...
  // Make this object a (deep) copy of other.
  // <group>
  CombiFunction_PS(const CombiFunction_PS<AutoDiff<T>> &other) : CombiParam<AutoDiff<T>>(other) {}
  template <class W>
  CombiFunction_PS(const CombiFunction_PS<W> &other) : CombiParam<AutoDiff<T>>(other) {}
  // </group>
  // Make this object a (deep) copy of other.
  CombiFunction_PS<AutoDiff<T>> &operator=(const CombiFunction_PS<AutoDiff<T>> &other) {
    CombiParam<AutoDiff<T>>::operator=(other);
    return *this;
  }
  ...
  //  deleting the pointer.
  //  <group>
  virtual Function<AutoDiff<T>> *clone() const { return new CombiFunction_PS<AutoDiff<T>>(*this); }
