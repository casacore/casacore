...
T CompiledFunction<T>::eval(typename Function<T>::FunctionArg x) const {
  ...
  for (vector<FuncExprData::ExprOperator>::const_iterator pos =
    ...
    T t(0);
    if (pos->narg == 2 || (pos->code == FuncExprData::ATAN && pos->state.argcnt == 2)) {
      t = exec_p.back();
      exec_p.pop_back();
    }
    ...
    switch (pos->code) {
      ...
      case FuncExprData::TOIMAG:
        NumericTraits<T>::setValue(exec_p.back(), NumericTraits<T>::getValue(exec_p.back(), 0), 1);
        NumericTraits<T>::setValue(exec_p.back(), typename NumericTraits<T>::BaseType(0.0), 0);
        break;
      ...
      case FuncExprData::EXP2:
        exec_p.back() =
            exp(exec_p.back() * static_cast<typename FunctionTraits<T>::BaseType>(M_LN2));
        break;
      case FuncExprData::EXP10:
        exec_p.back() =
            exp(exec_p.back() * static_cast<typename FunctionTraits<T>::BaseType>(M_LN10));
        break;
      ...
      case FuncExprData::LOG2:
        exec_p.back() =
            log(exec_p.back()) / static_cast<typename FunctionTraits<T>::BaseType>(M_LN2);
        break;
      ...
      case FuncExprData::PI: {
        if (pos->state.argcnt == 0) {
          exec_p.push_back(T(static_cast<typename FunctionTraits<T>::BaseType>(M_PI)));
        } else {
          exec_p.back() *= static_cast<typename FunctionTraits<T>::BaseType>(M_PI);
        }
        break;
      }
      case FuncExprData::EE: {
        if (pos->state.argcnt == 0) {
          exec_p.push_back(T(static_cast<typename FunctionTraits<T>::BaseType>(M_E)));
        } else {
          exec_p.back() *= static_cast<typename FunctionTraits<T>::BaseType>(M_E);
        }
        break;
      }
