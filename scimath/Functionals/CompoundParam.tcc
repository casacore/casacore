...
operator=(const CompoundParam<T> &other) {
  if (this != &other) {
    ...
    paroff_p = Block<uint32_t>(other.paroff_p.nelements());
    funpar_p = Block<uint32_t>(other.funpar_p.nelements());
    locpar_p = Block<uint32_t>(other.locpar_p.nelements());
    for (uint32_t i=0; i<functionPtr_p.nelements(); ++i) {
      functionPtr_p[i] = other.functionPtr_p[i]->clone();
      paroff_p[i] = other.paroff_p[i];
    }
...
// Member functions
template <class T>
uint32_t CompoundParam<T>::addFunction(const Function<T> &newFunction) {
  ...
  for (uint32_t j=np; j<np+newFunction.nparameters(); ++j) {
