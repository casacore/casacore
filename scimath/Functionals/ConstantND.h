...
template <class T>
class ConstantND : public ConstantNDParam<T> {
 public:
  // # Constructors
  ...
  ConstantND<T> &operator=(const ConstantND<T> &other) {
    ConstantNDParam<T>::operator=(other);
    return *this;
  };
  // </group>
...
template <class T>
class ConstantND_PS<AutoDiff<T>> : public ConstantNDParam<AutoDiff<T>> {
 public:
  // # Construct
  ...
  template <class W>
  ConstantND_PS(const ConstantND_PS<W> &other) : ConstantNDParam<AutoDiff<T>>(other) {}
  ConstantND_PS<AutoDiff<T>> &operator=(const ConstantND_PS<AutoDiff<T>> &other) {
    ConstantNDParam<AutoDiff<T>>::operator=(other);
    return *this;
  };
  // </group>
  ...
  // deleting the pointer.
  // <group>
  virtual Function<AutoDiff<T>> *clone() const { return new ConstantND_PS<AutoDiff<T>>(*this); };
  virtual Function<typename FunctionTraits<AutoDiff<T>>::DiffType> *cloneAD() const {
    return new ConstantND<typename FunctionTraits<AutoDiff<T>>::DiffType>(*this);
  };
  virtual Function<typename FunctionTraits<AutoDiff<T>>::BaseType> *cloneNonAD() const {
    return new ConstantND<typename FunctionTraits<AutoDiff<T>>::BaseType>(*this);
  };
  // </group>
