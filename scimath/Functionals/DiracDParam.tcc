...
template <class T>
DiracDParam<T>::DiracDParam() : Function<T>(2) {
  param_p[HEIGHT] = T(1.0);
  param_p[CENTER] = T(0.0);
}

template <class T>
DiracDParam<T>::DiracDParam(const T &height) : Function<T>(2) {
  param_p[HEIGHT] = height;
  param_p[CENTER] = T(0.0);
}

template <class T>
DiracDParam<T>::DiracDParam(const T &height, const T &center) : Function<T>(2) {
  param_p[HEIGHT] = height;
  param_p[CENTER] = center;
}

template <class T>
DiracDParam<T>::DiracDParam(const DiracDParam<T> &other) : Function<T>(other) {}
