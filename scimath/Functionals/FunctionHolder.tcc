...
// # Operators
template <class T>
FunctionHolder<T> &FunctionHolder<T>::operator=(const FunctionHolder<T> &other) {
  if (this != &other) {
    if (other.hold_p) {
  ...
  if (hold_p && putType(error, out)) {
    ...
    if (nf_p == COMBINE || nf_p == COMPOUND) {
      ...
      if (nf_p == COMBINE) {
        ...
        x = dynamic_cast<const CompoundFunction<T> *>(hold_p.get())->nFunctions();
      }
      out.define("nfunc", x);
      Record func;
      for (Int i = 0; i < x; ++i) {
...
Bool FunctionHolder<T>::getType(String &error, Function<U> *&fn, const RecordInterface &in) {
  ...
  if (in.isDefined(String("mode")) && in.type(in.idToNumber(RecordFieldId("mode"))) == TpRecord) {
