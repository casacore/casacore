...
Record FunctionalProxy::fhd2rec() {
  Record rec;
  String err;
  if (!fhd_.toRecord(err, rec)) throw AipsError(err);
  return rec;
}
...
Record FunctionalProxy::fhdc2rec() {
  Record rec;
  String err;
  if (!fhdc_.toRecord(err, rec)) throw AipsError(err);
  return rec;
}
...
void FunctionalProxy::rec2fhdc(const Record& rec) {
  String err;
  if (!fhdc_.fromRecord(err, rec)) throw AipsError(err);
}
...
void FunctionalProxy::setmask(Int idx, Bool val) {
  ...
  if (idx < 0 || idx >= n) throw(AipsError("mask index out of bounds"));
