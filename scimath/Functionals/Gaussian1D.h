...
template <class T>
class Gaussian1D : public Gaussian1DParam<T> {
 public:
  // # Enumerations
  ...
  Gaussian1D<T> &operator=(const Gaussian1D<T> &other) {
    Gaussian1DParam<T>::operator=(other);
    return *this;
  }
...
template <class T>
class Gaussian1D_PS<AutoDiff<T>> : public Gaussian1DParam<AutoDiff<T>> {
 public:
  // # Constructors
  //  Constructs one dimensional Gaussians.
  //  <group>
  Gaussian1D_PS() : Gaussian1DParam<AutoDiff<T>>() {}
  explicit Gaussian1D_PS(const AutoDiff<T> &height) : Gaussian1DParam<AutoDiff<T>>(height) {}
  ...
  // Copy constructor (deep copy)
  // <group>
  Gaussian1D_PS(const Gaussian1D_PS &other) : Gaussian1DParam<AutoDiff<T>>(other) {}
  template <class W>
  Gaussian1D_PS(const Gaussian1D_PS<W> &other) : Gaussian1DParam<AutoDiff<T>>(other) {}
  // </group>

  // Copy assignment (deep copy)
  Gaussian1D_PS<AutoDiff<T>> &operator=(const Gaussian1D_PS<AutoDiff<T>> &other) {
    Gaussian1DParam<AutoDiff<T>>::operator=(other);
    return *this;
  }
  ...
  //  for deleting this pointer.
  //  <group>
  virtual Function<AutoDiff<T>> *clone() const { return new Gaussian1D<AutoDiff<T>>(*this); }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::DiffType> *cloneAD() const {
    return new Gaussian1D<typename FunctionTraits<AutoDiff<T>>::DiffType>(*this);
  }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::BaseType> *cloneNonAD() const {
    return new Gaussian1D<typename FunctionTraits<AutoDiff<T>>::BaseType>(*this);
  }
  // </group>
