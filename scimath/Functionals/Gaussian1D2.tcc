...
// # Operators
template <class T>
AutoDiff<T> Gaussian1D<AutoDiff<T>>::eval(typename Function<AutoDiff<T>>::FunctionArg x) const {
  AutoDiff<T> tmp;
  if (this->param_p[this->HEIGHT].nDerivatives() > 0)
    tmp = this->param_p[this->HEIGHT];
  else if (this->param_p[this->CENTER].nDerivatives() > 0)
    tmp = this->param_p[this->CENTER];
  else if (this->param_p[this->WIDTH].nDerivatives() > 0)
    tmp = this->param_p[this->WIDTH];
  T x_norm = (x[0] - this->param_p[this->CENTER].value()) / this->param_p[this->WIDTH].value() /
             this->fwhm2int.value();
  ...
  if (tmp.nDerivatives() > 0) {
    ...
    dev *= this->param_p[this->HEIGHT].value() * x_norm * T(2.0) /
    ...
    if (this->param_p.mask(this->WIDTH))
      tmp.deriv(this->WIDTH) = dev * x_norm * this->fwhm2int.value();
  }
  return tmp;
}
