...
template <class T>
class Gaussian2D : public Gaussian2DParam<T> {
 public:
  // # Enumerations
  ...
  Gaussian2D(const T &height, const T &xCenter, const T &yCenter, const T &majorAxis,
             const T &axialRatio, const T &pa)
      : Gaussian2DParam<T>(height, xCenter, yCenter, majorAxis, axialRatio, pa) {}
  // </group>
  ...
  Gaussian2D<T> &operator=(const Gaussian2D<T> &other) {
    Gaussian2DParam<T>::operator=(other);
    return *this;
  }
...
template <class T>
class Gaussian2D_PS<AutoDiff<T>> : public Gaussian2DParam<AutoDiff<T>> {
 public:
  // # Constructors
  //  Constructs two dimensional Gaussians.
  //  <group>
  Gaussian2D_PS() : Gaussian2DParam<AutoDiff<T>>() {}
  Gaussian2D_PS(const AutoDiff<T> &height, const Vector<AutoDiff<T>> &center,
                const Vector<AutoDiff<T>> &width, const AutoDiff<T> &pa)
      : Gaussian2DParam<AutoDiff<T>>(height, center, width, pa) {}
  Gaussian2D_PS(const AutoDiff<T> &height, const AutoDiff<T> &xCenter, const AutoDiff<T> &yCenter,
                const AutoDiff<T> &majorAxis, const AutoDiff<T> &axialRatio, const AutoDiff<T> &pa)
      : Gaussian2DParam<AutoDiff<T>>(height, xCenter, yCenter, majorAxis, axialRatio, pa) {}
  // </group>

  // Copy constructor (deep copy)
  // <group>
  Gaussian2D_PS(const Gaussian2D_PS &other) : Gaussian2DParam<AutoDiff<T>>(other) {}
  template <class W>
  Gaussian2D_PS(const Gaussian2D_PS<W> &other) : Gaussian2DParam<AutoDiff<T>>(other) {}
  // </group>

  // Copy assignment (deep copy)
  Gaussian2D_PS<AutoDiff<T>> &operator=(const Gaussian2D_PS<AutoDiff<T>> &other) {
    Gaussian2DParam<AutoDiff<T>>::operator=(other);
    return *this;
  }
  ...
  //  for deleting this pointer.
  //  <group>
  virtual Function<AutoDiff<T>> *clone() const { return new Gaussian2D<AutoDiff<T>>(*this); }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::DiffType> *cloneAD() const {
    return new Gaussian2D<typename FunctionTraits<AutoDiff<T>>::DiffType>(*this);
  }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::BaseType> *cloneNonAD() const {
    return new Gaussian2D<typename FunctionTraits<AutoDiff<T>>::BaseType>(*this);
  }
  // </group>
