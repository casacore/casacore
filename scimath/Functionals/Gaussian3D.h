...
template <class T>
class Gaussian3D : public Gaussian3DParam<T> {
 public:
  ...
  Gaussian3D(T &height, T &xCenter, T &yCenter, T &zCenter, T &xWidth, T &yWidth, T &zWidth,
             T &theta, T &phi);
  // </group>
...
template <class T>
class Gaussian3D_PS<AutoDiff<T>> : public Gaussian3DParam<AutoDiff<T>> {
 public:
  Gaussian3D_PS();
  Gaussian3D_PS(const AutoDiff<T> &height, const Vector<AutoDiff<T>> &center,
                const Vector<AutoDiff<T>> &width, const AutoDiff<T> &theta, const AutoDiff<T> &phi);
  Gaussian3D_PS(AutoDiff<T> &height, AutoDiff<T> &xCenter, AutoDiff<T> &yCenter,
                AutoDiff<T> &zCenter, AutoDiff<T> &xWidth, AutoDiff<T> &yWidth, AutoDiff<T> &zWidth,
                AutoDiff<T> &theta, AutoDiff<T> &phi);
  Gaussian3D_PS(const Gaussian3D_PS<AutoDiff<T>> &other);
  template <class W>
  Gaussian3D_PS(const Gaussian3D_PS<W> &other) : Gaussian3DParam<AutoDiff<T>>(other) {}
  ...
  virtual Function<typename FunctionTraits<AutoDiff<T>>::DiffType> *cloneAD() const {
    return new Gaussian3D<typename FunctionTraits<AutoDiff<T>>::DiffType>(*this);
  }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::BaseType> *cloneNonAD() const {
    return new Gaussian3D<typename FunctionTraits<AutoDiff<T>>::BaseType>(*this);
  }
