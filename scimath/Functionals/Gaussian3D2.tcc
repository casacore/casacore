...
template <class T>
Gaussian3D<AutoDiff<T>>::Gaussian3D() : Gaussian3DParam<AutoDiff<T>>() {}

template <class T>
Gaussian3D<AutoDiff<T>>::Gaussian3D(AutoDiff<T> &height, AutoDiff<T> &xCenter, AutoDiff<T> &yCenter,
                                    AutoDiff<T> &zCenter, AutoDiff<T> &xWidth, AutoDiff<T> &yWidth,
                                    AutoDiff<T> &zWidth, AutoDiff<T> &theta, AutoDiff<T> &phi)
    : Gaussian3DParam<AutoDiff<T>>(height, xCenter, yCenter, zCenter, xWidth, yWidth, zWidth, theta,
                                   phi) {}

template <class T>
Gaussian3D<AutoDiff<T>>::Gaussian3D(const AutoDiff<T> &height, const Vector<AutoDiff<T>> &center,
                                    const Vector<AutoDiff<T>> &width, const AutoDiff<T> &theta,
...
AutoDiff<T> Gaussian3D<AutoDiff<T>>::eval(typename Function<AutoDiff<T>>::FunctionArg x) const {
  ...
  if (tmp.nDerivatives() > 0) {
    ...
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::CX))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::CX) =
          tvalue *
          (cosTcosPV * xrowterm / Ax2 - sinTcosPV * yrowterm / Ay2 + sinPV * zrowterm / Az2);

    // derivative wrt Cy (mean)
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::CY))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::CY) =
          tvalue * (sinTV * xrowterm / Ax2 + cosTV * yrowterm / Ay2);

    // derivative wrt Cz (mean)
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::CZ))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::CZ) =
          tvalue *
          (-cosTsinPV * xrowterm / Ax2 + sinTsinPV * yrowterm / Ay2 + cosPV * zrowterm / Az2);

    // derivative wrt Ax
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::AX))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::AX) =
          tvalue * xwidthterm2 / this->param_p[Gaussian3DParam<AutoDiff<T>>::AX].value();

    // derivative wrt Ay
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::AY))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::AY) =
          tvalue * ywidthterm2 / this->param_p[Gaussian3DParam<AutoDiff<T>>::AY].value();

    // derivative wrt Az
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::AZ))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::AZ) =
          tvalue * zwidthterm2 / this->param_p[Gaussian3DParam<AutoDiff<T>>::AZ].value();

    // derivative wrt theta
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::THETA))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::THETA) =
          tvalue * (xrowterm * yrowterm / Ay2 - xrowterm * yrowterm / Ax2);

    // derivative wrt phi
    if (this->param_p.mask(Gaussian3DParam<AutoDiff<T>>::PHI))
      tmp.deriv(Gaussian3DParam<AutoDiff<T>>::PHI) =
          -tvalue * (xrowterm * (-Nx * cosTsinPV - Nz * cosTcosPV) / Ax2 +
