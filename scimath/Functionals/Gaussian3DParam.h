...
template <class Type>
class Gaussian3DParam : public Function<Type> {
 ...
 public:
  ...
  // <group>
  Gaussian3DParam();
  Gaussian3DParam(Type height, const Vector<Type> &center, const Vector<Type> &width, Type theta,
                  Type phi);
  Gaussian3DParam(Type &height, Type &xCenter, Type &yCenter, Type &zCenter, Type &xWidth,
                  Type &yWidth, Type &zWidth, Type &theta, Type &phi);
  // </group>
  ...
  Gaussian3DParam(const Gaussian3DParam<W> &other)
      : Function<Type>(other), fwhm2int(Type(1.0) / sqrt(log(Type(16.0)))) {
    settrigvals();
  }
  // </group>
