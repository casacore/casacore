...
template <class Type>
Gaussian3DParam<Type>::Gaussian3DParam() : Function<Type>(NPAR) {
  param_p[H] = Type(1.0);
  param_p[CX] = Type(0.0);
  param_p[CY] = Type(0.0);
  param_p[CZ] = Type(0.0);
...
template <class Type>
Gaussian3DParam<Type>::Gaussian3DParam(Type& height, Type& xCenter, Type& yCenter, Type& zCenter,
                                       Type& xWidth, Type& yWidth, Type& zWidth, Type& theta,
                                       Type& phi)
    : Function<Type>(NPAR) {
  param_p[H] = height;
  param_p[CX] = xCenter;
...
template <class Type>
Gaussian3DParam<Type>::Gaussian3DParam(Type /*height*/, const Vector<Type>& center,
                                       const Vector<Type>& width, Type T, Type P)
    : Function<Type>(NPAR) {
  fwhm2int = Type(1.0) / sqrt(log(Type(16.0)));
  setCenter(center);
  setWidth(width);
...
Type Gaussian3DParam<Type>::flux() const {
  return param_p[H] * param_p[AX] * param_p[AY] * param_p[AZ] * fwhm2int * fwhm2int * fwhm2int *
         Type(M_PI * sqrt(M_PI));
}

template <class Type>
void Gaussian3DParam<Type>::setFlux(const Type& flux) {
  param_p[H] = flux / (param_p[AX] * param_p[AY] * param_p[AZ] * fwhm2int * fwhm2int * fwhm2int *
                       Type(M_PI * sqrt(M_PI)));
}
...
void Gaussian3DParam<Type>::setCenter(const Vector<Type>& center) {
  if (center.nelements() != 3)
    throw(
        AipsError("Gaussian3D<Type>::setCenter(const Vector<Type>& center)"
...
void Gaussian3DParam<Type>::setWidth(const Vector<Type>& width) {
  if (width.nelements() != 3)
    throw(
        AipsError("Gaussian3DParam<Type>::setWidth"
...
void Gaussian3DParam<Type>::setXwidth(const Type& xwidth) {
  if (xwidth <= Type(0))
    throw(
        AipsError("Gaussian3DParam<Type>::setXwidth(const Type& xwidth)"
...
void Gaussian3DParam<Type>::setYwidth(const Type& ywidth) {
  if (ywidth <= Type(0))
    throw(
        AipsError("Gaussian3DParam<Type>::setYwidth(const Type& ywidth)"
                  " - width must be positive"));
  param_p[AY] = ywidth;
}
...
void Gaussian3DParam<Type>::setZwidth(const Type& zwidth) {
  if (zwidth <= Type(0))
    throw(
        AipsError("Gaussian3DParam<Type>::setZwidth(const Type& zwidth)"
                  " - width must be positive"));
  param_p[AZ] = zwidth;
}
...
void Gaussian3DParam<Type>::settheta(const Type& theta) {
  if (abs(theta) > Type(M_PI_4))
    throw(
        AipsError("Gaussian3DParam<Type>::settheta(const Type& theta)"
...
void Gaussian3DParam<Type>::setphi(const Type& phi) {
  if (abs(phi) > Type(M_PI_4))
    throw(
        AipsError("Gaussian3D<Type>::setphi(const Type& phi)"
