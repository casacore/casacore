...
GaussianNDParam<T>::GaussianNDParam(uInt nDim, const T &height, const Vector<T> &mean)
  ...
  if (mean.nelements() != itsDim) {
    throw(
        AipsError("GaussianNDParam<T>::GaussianNDParam(uInt nDim, "
                  "T height, "
...
GaussianNDParam<T>::GaussianNDParam(const GaussianNDParam<T> &other)
    : Function<T>(other), itsDim(other.itsDim), itsFlux2Hgt(other.itsFlux2Hgt) {}

template <class T>
GaussianNDParam<T> &GaussianNDParam<T>::operator=(const GaussianNDParam<T> &other) {
  if (this != &other) {
...
void GaussianNDParam<T>::setVariance(const Vector<T> &variance) {
  if (variance.nelements() != itsDim) {
    throw(
        AipsError("GaussianNDParam<T>::setVariance(const Vector<T> "
                  "&variance)"
                  " - variance must have nDim values."));
  }
...
void GaussianNDParam<T>::setCovariance(const Matrix<T> &covar) {
  ...
  if (locCovariance.shape() != IPosition(2, itsDim, itsDim)) {
    throw(
        AipsError("GaussianNDParam<T>::setCovariance("
                  "const Matrix<T> &covar)"
