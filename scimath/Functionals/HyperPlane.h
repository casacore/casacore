...
template <class T>
class HyperPlane : public HyperPlaneParam<T> {
 public:
  // # Constructors
  ...
  HyperPlane<T> &operator=(const HyperPlane<T> &other) {
    HyperPlaneParam<T>::operator=(other);
    return *this;
  }
  // </group>
...
template <class T>
class HyperPlane_PS<AutoDiff<T>> : public HyperPlaneParam<AutoDiff<T>> {
 public:
  // # Construct
  ...
  template <class W>
  HyperPlane_PS(const HyperPlane_PS<W> &other) : HyperPlaneParam<AutoDiff<T>>(other) {}
  HyperPlane_PS<AutoDiff<T>> &operator=(const HyperPlane_PS<AutoDiff<T>> &other) {
    HyperPlaneParam<AutoDiff<T>>::operator=(other);
    return *this;
  }
  // </group>
  ...
  // deleting the pointer.
  // <group>
  virtual Function<AutoDiff<T>> *clone() const { return new HyperPlane_PS<AutoDiff<T>>(*this); }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::DiffType> *cloneAD() const {
    return new HyperPlane<typename FunctionTraits<AutoDiff<T>>::DiffType>(*this);
  }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::BaseType> *cloneNonAD() const {
    return new HyperPlane<typename FunctionTraits<AutoDiff<T>>::BaseType>(*this);
  }
  // </group>
