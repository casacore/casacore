...
Interpolate1D<Domain, Range>::Interpolate1D() {}
...
template <class Domain, class Range>
Interpolate1D<Domain, Range>::Interpolate1D(const Interpolate1D<Domain, Range> &other)
...
template <class Domain, class Range>
Interpolate1D<Domain, Range> &Interpolate1D<Domain, Range>::operator=(
    const Interpolate1D<Domain, Range> &other) {
  if (this != &other) {
    curMethod = other.curMethod;
    nElements = other.nElements;
    xValues = other.xValues;
...
void Interpolate1D<Domain, Range>::setMethod(uInt newMethod) {
  ...
  if (newMethod == spline && curMethod != spline) {  // Calculate the y2Values
    ...
    for (i = 1; i < nElements - 1; i++) {
      ...
      r = (one / c) * (yValues[i + 1] - yValues[i]) - (one / a) * (yValues[i] - yValues[i - 1]);
      delta = a * t[i - 1];
      if (nearAbs(b, delta))
        throw(
            AipsError("Interpolate1D::setMethod"
...
template <class Domain, class Range>
Vector<Domain> Interpolate1D<Domain, Range>::getX() const {
  Vector<Domain> x(xValues.begin(), xValues.begin() + nElements);
  return x;
}

template <class Domain, class Range>
Vector<Range> Interpolate1D<Domain, Range>::getY() const {
