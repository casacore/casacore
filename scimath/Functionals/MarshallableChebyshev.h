...
template <class T>
class MarshallableChebyshev : public Chebyshev<T>, public FunctionMarshallable {
 private:
 ...
 public:
  ...
  // equal to zero.  The bounded domain is [T(-1), T(1)].  The
  // OutOfDomainMode is CONSTANT, and the default value is T(0).
  explicit MarshallableChebyshev(const uInt n) : Chebyshev<T>(n), FunctionMarshallable(FUNCTYPE) {}
  ...
  MarshallableChebyshev(
      const Vector<T> &coeffs, const T &min, const T &max,
      const typename ChebyshevEnums::OutOfIntervalMode mode = ChebyshevEnums::CONSTANT,
      const T &defval = T(0))
      : Chebyshev<T>(coeffs, min, max, mode, defval), FunctionMarshallable(FUNCTYPE) {}
  ...
  virtual Function<T> *clone() const { return new MarshallableChebyshev<T>(*this); }
};
