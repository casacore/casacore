...
template <class T>
const String MarshallableChebyshev<T>::modenames[] = {"default", "zeroth", "extrapolate", "cyclic",
                                                      "edge"};
...
template <class T>
const String MarshallableChebyshev<T>::FUNCFIELDS[] = {"coeffs", "mode", "def", "interval"};
...
template <class T>
MarshallableChebyshev<T>::MarshallableChebyshev(const Record& gr) throw(InvalidSerializationError)
  ...
  if (input.exists(FUNCFIELDS[MODE])) {
    ...
    if (i == ChebyshevEnums::NOutOfIntervalModes)
      throw InvalidSerializationError(String("Unrecognized mode: ") + modename);
    this->setOutOfIntervalMode(static_cast<ChebyshevEnums::OutOfIntervalMode>(i));
  }
