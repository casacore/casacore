...
template <class T>
class PoissonFunction : public PoissonParam<T> {
 public:
  // # Constructors
  ...
  explicit PoissonFunction(const T &lambda) : PoissonParam<T>(lambda) {}
  PoissonFunction(const T &lambda, const T &height) : PoissonParam<T>(lambda, height) {}
  // </group>
...
template <class T>
class PoissonFunction_PS<AutoDiff<T>> : public PoissonParam<AutoDiff<T>> {
 public:
  // # Constructors
  //  Constructs one dimensional Poisson.
  //  <group>
  PoissonFunction_PS() : PoissonParam<AutoDiff<T>>() {}
  explicit PoissonFunction_PS(const AutoDiff<T> &lambda) : PoissonParam<AutoDiff<T>>(lambda) {}
  ...
  // Copy constructor (deep copy)
  // <group>
  PoissonFunction_PS(const PoissonFunction_PS &other) : PoissonParam<AutoDiff<T>>(other) {}
  template <class W>
  PoissonFunction_PS(const PoissonFunction_PS<W> &other) : PoissonParam<AutoDiff<T>>(other) {}
  // </group>

  // Copy assignment (deep copy)
  PoissonFunction_PS<AutoDiff<T>> &operator=(const PoissonFunction_PS<AutoDiff<T>> &other) {
    PoissonFunction<AutoDiff<T>>::operator=(other);
    return *this;
  }
  ...
  //  for deleting this pointer.
  //  <group>
  virtual Function<AutoDiff<T>> *clone() const { return new PoissonFunction<AutoDiff<T>>(*this); }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::DiffType> *cloneAD() const {
    return new PoissonFunction<typename FunctionTraits<AutoDiff<T>>::DiffType>(*this);
  }
  virtual Function<typename FunctionTraits<AutoDiff<T>>::BaseType> *cloneNonAD() const {
    return new PoissonFunction<typename FunctionTraits<AutoDiff<T>>::BaseType>(*this);
  }
  // </group>
