...
// # Operators
template <class T>
AutoDiff<T> PoissonFunction<AutoDiff<T>>::eval(
    typename Function<AutoDiff<T>>::FunctionArg x) const {
  AutoDiff<T> tmp;
  ...
  tmp.value() =
      heightVal * (pow(lambdaVal, xVal) * exp(-1 * lambdaVal) / Combinatorics::factorial(xVal));
  ...
  if (tmp.nDerivatives() > 0) {
    ...
    if (param_p.mask(LAMBDA)) {
      tmp.deriv(LAMBDA) = heightVal * (xVal * pow(lambdaVal, xVal - 1) - pow(lambdaVal, xVal)) *
                          exp(-1 * lambdaVal) / Combinatorics::factorial(xVal);
    }
    if (param_p.mask(HEIGHT)) {
      tmp.deriv(HEIGHT) =
          pow(lambdaVal, xVal) * exp(-1 * lambdaVal) / Combinatorics::factorial(xVal);
    }
  }
