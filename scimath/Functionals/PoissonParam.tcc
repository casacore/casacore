...
template <class T>
PoissonParam<T>::PoissonParam() : Function<T>(2) {
  param_p[LAMBDA] = T(1.0);
  param_p[HEIGHT] = T(1.0);
}

template <class T>
PoissonParam<T>::PoissonParam(const T &lambda) : Function<T>(2) {
  param_p[LAMBDA] = lambda;
  param_p[HEIGHT] = T(1.0);
}

template <class T>
PoissonParam<T>::PoissonParam(const T &lambda, const T &height) : Function<T>(2) {
  param_p[LAMBDA] = lambda;
  param_p[HEIGHT] = height;
}

template <class T>
PoissonParam<T>::PoissonParam(const PoissonParam<T> &other) : Function<T>(other) {}
