...
// # Constructors
template <class T>
SimButterworthBandpass<T>::SimButterworthBandpass() : Function1D<T>(4), nl_p(0), nh_p(0) {
...
template <class T>
SimButterworthBandpass<T>::SimButterworthBandpass(const RecordInterface &gr, T mincut, T maxcut,
                                                  T center, T peak)
...
template <class T>
SimButterworthBandpass<T>::SimButterworthBandpass(const SimButterworthBandpass<T> &other)
...
// # Operators
template <class T>
SimButterworthBandpass<T> &SimButterworthBandpass<T>::operator=(
...
template <class T>
T SimButterworthBandpass<T>::eval(const typename FunctionTraits<T>::ArgType *x) const {
  // this does not reflect the true responses of Butterworth filters
  // calculate the low-pass portion
  T hp = T(1);
  if (x[0] > param_p[CENTER]) {
    hp = T(1) / sqrt(T(1) + pow((x[0] - param_p[CENTER]) / (param_p[MAXCUTOFF] - param_p[CENTER]),
                                T(2 * nh_p)));
  }
  // calculate the high-pass portion
  if (x[0] < param_p[CENTER]) {
    hp *= T(1) / sqrt(T(1) + pow((param_p[CENTER] - x[0]) / (param_p[MINCUTOFF] - param_p[CENTER]),
                                 T(2 * nl_p)));
  }
