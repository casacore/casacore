...
AutoDiff<Double> a0(const Vector<AutoDiff<Double>> &par, const AutoDiff<Double> x0) {
  return par(0) * exp(-(x0 - par(1)) * (x0 - par(1)) / par(2) / par(2));
}

AutoDiff<Double> a1(const Vector<AutoDiff<Double>> &par, const AutoDiff<Double> x0) {
  AutoDiff<Double> g = (x0 - par(1)) / par(2);
  return par(0) * exp(-g * g);
}

AutoDiff<Double> a2(const Vector<AutoDiff<Double>> &par, const AutoDiff<Double> x0) {
  AutoDiff<Double> y(x0);
  y -= par(1);
  y /= par(2);
  y *= y;
...
int main(int argc, const char *argv[]) {
  ...
  Gaussian1D<Double> g1d((Double(par(0))), (Double(par(1))),
  ...
  cout << "--------- Values original (OPTLIB  N=1000000/100000) ---------"
       << "\ng0:        3.91 real        3.91 user           0 system"
          ...
          "\na2:        6.72 real        6.32 user           0 system"
       << endl;
