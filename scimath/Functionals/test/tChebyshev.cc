...
int main() {
...
#endif
  AlwaysAssertExit(coeffs(0) == 0 && coeffs(1) == -6 && coeffs(2) == 0 && coeffs(3) == 8);
#ifdef DIAGNOSTICS
...
#endif
  AlwaysAssertExit(coeffs(0) == cheb.getCoefficient(0) && coeffs(1) == cheb.getCoefficient(1) &&
                   coeffs(2) == cheb.getCoefficient(2) && coeffs(3) == cheb.getCoefficient(3));
...
#endif
  AlwaysAssertExit(coeffs(0) == 1 && coeffs(1) == -4 && coeffs(2) == 4 && coeffs(3) == 8);
...
#ifdef DIAGNOSTICS
  cout << xmin - 1.3
       << ": cheb(-1.3)-cheb(2.7)=" << cheb(xmin - 1.3) - cheb(xmin - 1.3 + (xmax - xmin)) << endl;
#endif
  AlwaysAssertExit(nearAbs(cheb(xmin - 1.3), cheb(xmin - 1.3 + (xmax - xmin)), 1.0e-15));
#ifdef DIAGNOSTICS
  cout << xmax + 1.3
       << ": cheb(5.3)-cheb(1.3)=" << cheb(xmin + 1.3) - cheb(xmin + 1.3 - (xmax - xmin)) << endl;
#endif
  AlwaysAssertExit(nearAbs(cheb(xmax + 1.3), cheb(xmax + 1.3 - (xmax - xmin)), 1.0e-15));
  ...
  cout << "Results from setMode():" << endl
       ...
       << "  Range: " << cheb.getIntervalMin() << ", " << cheb.getIntervalMax() << endl
       << "  Default: " << cheb.getDefault() << endl;
#endif
  AlwaysAssertExit(cheb.getOutOfIntervalMode() == ChebyshevEnums::CYCLIC &&
                   cheb.getIntervalMin() == -10.0 && cheb.getIntervalMax() == 10.0 &&
                   cheb.getDefault() == 80.0);

  // test setMode() via constructor
  Chebyshev<Double> cheb2(2, rec);
  AlwaysAssertExit(cheb2.getOutOfIntervalMode() == ChebyshevEnums::CYCLIC &&
                   cheb2.getIntervalMin() == -10.0 && cheb2.getIntervalMax() == 10.0 &&
  ...
  try {
    ...
    AlwaysAssertExit(mode == String("zeroth") && tmp(0) == -15.0 && tmp(1) == 15.0 && def == 70.0);
