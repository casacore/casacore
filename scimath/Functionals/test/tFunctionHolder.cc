...
// Make near zero zero
Double Y(const Double in) { return (abs(in) < 1e-15 ? Double(0.0) : in); }
...
int main() {
  ...
  cout << "------------------------ Kaiser-Bessel --------------" << endl;
  {
    ...
    for (Double x = -1.2; x < 1.21; x += 0.2) cout << "x=" << Y(x) << ": " << Y(fn(x)) << endl;
    for (Double x = 6; x < 8.2; x += 1.0) cout << "x=" << Y(x) << ": " << Y(fn(x)) << endl;
    ...
    for (Double x = -1.2; x < 1.21; x += 0.2) cout << "x=" << Y(x) << ": " << Y(fnd(x)) << endl;
  }
  cout << "------------------------ sinc -----------------------" << endl;
  {
    ...
    for (Double x = -1.19999; x < 1.21; x += 0.2) cout << "x=" << Y(x) << ": " << Y(fnd(x)) << endl;
  }
  cout << "------------------------ Gaussian1D -----------------" << endl;
  {
    Gaussian1D<Double> fn;
    FunctionHolder<Double> fh(fn);
    for (Double x = -1.2; x < 1.21; x += 0.2)
      cout << "x=" << Y(x) << ": " << fh.asFunction()(x) << endl;
