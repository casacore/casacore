...
int main() {
  ...
  Gaussian3D<AutoDiff<Double>> gauss3dAD(heightAD, xCenAD, yCenAD, zCenAD, xWidthAD, yWidthAD,
                                         zWidthAD, thetaAD, phiAD);
  ...
  Gaussian3D<AutoDiffA<Double>> gauss3dADA(heightADA, xCenADA, yCenADA, zCenADA, xWidthADA,
                                           yWidthADA, zWidthADA, thetaADA, phiADA);
  //
  /*
    uInt npar = argc - 1;
    if (npar > 9) npar = 9;
  ...
  for (Double z = -0.25; z < 0.26; z += 0.25) {
    for (Double y = -1.0; y < 1.01; y += 0.25) {
      for (Double x = -1.0; x < 1.01; x += 0.25) {
        ...
        AlwaysAssertExit(near(gauss3dAD(x, y, z).value(), gauss3dADA(x, y, z).value()));
      }
      cout << endl;
    }
    cout << endl;
