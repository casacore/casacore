...
int main() {
  try {
    ...
    }
    {
      ...
      if ((allNear(gauss3D.variance(), variance, 1E-7)))
        cout << "Passed";
      else {
        cout << "Failed";
        failed = True;
    ...
    }
    {
      ...
      if ((allNear(gauss3D.covariance(), covariance, 1E-5)))
        cout << "Passed";
      else {
        cout << "Failed";
        failed = True;
            ...
            for (Float p = -5; p < 5; p += inc) {
              ...
              ev = height * exp(-0.5 *
                                (square(z(0) - mean(0)) * (1 - square(corr(2, 1))) / variance(0) +
                                 square(z(1) - mean(1)) * (1 - square(corr(2, 0))) / variance(1) +
                                 square(z(2) - mean(2)) * (1 - square(corr(1, 0))) / variance(2) -
