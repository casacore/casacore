...
template <class T>
AutoDiff<T>::AutoDiff() : val_p(T(0.0)), nd_p(0), grad_p(0) {}
...
AutoDiff<T>::AutoDiff(const AutoDiff<T> &other)
    : val_p(other.val_p), nd_p(other.nd_p), grad_p(other.nd_p) {
  grad_p = other.grad_p;
}

template <class T>
AutoDiff<T>::AutoDiff(const T &v, const Vector<T> &derivs)
    : val_p(v), nd_p(derivs.nelements()), grad_p(nd_p) {
  grad_p = derivs;
}
...
template <class T>
void AutoDiff<T>::derivatives(Vector<T> &res) const {
  res.resize(nd_p);
  res = grad_p;
}
