...
template <class FType>
Convolver<FType>::Convolver(const Convolver<FType>& other) {
...
template <class FType>
IPosition Convolver<FType>::defaultShape(const Array<FType>& psf) {
...
template <class FType>
IPosition Convolver<FType>::extractShape(IPosition& psfSize, const IPosition& imageSize) {
...
template <class FType>
void Convolver<FType>::makePsf(Array<FType>& psf) {
  validate();
...
void Convolver<FType>::linearConv(Array<FType>& result, const Array<FType>& model, Bool fullSize) {
  ...
  if (fullSize) resultSize.setFirst(imageSize + thePsfSize - 1);
...
template <class FType>
void Convolver<FType>::circularConv(Array<FType>& result, const Array<FType>& model) {
  // Check the dimensions of the model are compatible with the current psf
  validate();
  IPosition imageSize = extractShape(thePsfSize, model.shape());
