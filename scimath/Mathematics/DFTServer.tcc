...
//
{
  ...
  switch (Time.ndim()) {
    ...
    default:
      throw(
          DFTError("DFTServer constructor -- Error: time array should"
  ...
  switch (Freq.ndim()) {
    ...
    default:
      throw(
          DFTError("DFTServer constructor -- Error: Frequency array "
  ...
  if (dimTime != dimFreq)
    throw(
        DFTError("DFTServer::DFTServer - Error:  Time and Freq should"
  ...
  if (dimension > 3)
    throw(
        DFTError("DFTServer::dft: Error--cannot handle data points which "
...
DFTServer<T>::DFTServer(IPosition &shapeTime, IPosition &shapeFreq) {
  ...
  switch (shapeFreq.nelements()) {
    ...
    default:
      throw(
          DFTError("DFTServer constructor -- Error: frequency array should"
                   " be a vector or a matrix"));
      break;
  }
  ...
  if (dimTime != dimFreq)
    throw(
        DFTError("DFTServer constructor: Error--time and UV data must"
                 "have same dimension"));
...
Matrix<T> DFTServer<T>::getMatrix(Array<T> &data) {
  ...
  if (data.ndim() == 1) {
    if ((Shape(0) - 2) != dimension)
      throw(
          DFTError("DFTServer::getMatrix -- Error: data has "
                   " incorrect dimension"));
    Vector<T> vec;
    vec.reference(data);
    ...
    throw(
        DFTError("DFTServer::dft: Error--cannot handle time data "
...
//
{
  ...
  if (Time.ndim() == 1) Time = matIn.row(0);

  if (Freq.ndim() == 1) Freq = matOut.row(0);
}
  ...
  if (dir > 0) {  // DFT from time to frequency domain
    for (int j = 0; j <= numFreq - 1; j++) {
      ...
      for (int k = 0; k <= numTime - 1; k++) {
        ...
        sum_real += (Time(k, 0) * cos(phase) + Time(k, 1) * cos(phase + (M_PI_2)));
        sum_imag += (Time(k, 0) * sin(phase) + Time(k, 1) * sin(phase + (M_PI_2)));
      }
    ...
    for (int j = 0; j <= numTime - 1; j++) {
      ...
      for (int k = 0; k <= numFreq - 1; k++) {
        ...
        phase = (2.0 * M_PI) * phase;
        sum_real += (Freq(k, 0) * cos(phase) + Freq(k, 1) * cos(phase + (M_PI_2)));
        if (crFlag == 0) sum_imag += (Freq(k, 0) * sin(phase) + Freq(k, 1) * sin(phase + (M_PI_2)));
      }
