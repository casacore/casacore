...
class FFTWPlan {
 public:
  explicit FFTWPlan(fftw_plan plan) : itsPlan(plan) {}
  ~FFTWPlan() { fftw_destroy_plan(itsPlan); }
  fftw_plan getPlan() { return itsPlan; }
...
class FFTWPlanf {
 public:
  explicit FFTWPlanf(fftwf_plan plan) : itsPlan(plan) {}
  ~FFTWPlanf() { fftwf_destroy_plan(itsPlan); }
  fftwf_plan getPlan() { return itsPlan; }
...
FFTW::FFTW() : flags(FFTW_ESTIMATE) { initialize_fftw(); }
...
void FFTW::plan_r2c(const IPosition &size, float *in, std::complex<float> *out) {
  itsPlanR2Cf.reset(
      new FFTWPlanf(fftwf_plan_dft_r2c(size.nelements(), size.asStdVector().data(), in,
                                       reinterpret_cast<fftwf_complex *>(out), flags)));
}

void FFTW::plan_r2c(const IPosition &size, double *in, std::complex<double> *out) {
  itsPlanR2C.reset(new FFTWPlan(fftw_plan_dft_r2c(size.nelements(), size.asStdVector().data(), in,
                                                  reinterpret_cast<fftw_complex *>(out), flags)));
}

void FFTW::plan_c2r(const IPosition &size, std::complex<float> *in, float *out) {
  itsPlanC2Rf.reset(
      new FFTWPlanf(fftwf_plan_dft_c2r(size.nelements(), size.asStdVector().data(),
                                       reinterpret_cast<fftwf_complex *>(in), out, flags)));
}

void FFTW::plan_c2r(const IPosition &size, std::complex<double> *in, double *out) {
  itsPlanC2R.reset(
      new FFTWPlan(fftw_plan_dft_c2r(size.nelements(), size.asStdVector().data(),
                                     reinterpret_cast<fftw_complex *>(in), out, flags)));
}

void FFTW::plan_c2c_forward(const IPosition &size, std::complex<double> *in) {
  itsPlanC2CF.reset(new FFTWPlan(fftw_plan_dft(
      size.nelements(), size.asStdVector().data(), reinterpret_cast<fftw_complex *>(in),
      reinterpret_cast<fftw_complex *>(in), FFTW_FORWARD, flags)));
}

void FFTW::plan_c2c_forward(const IPosition &size, std::complex<float> *in) {
  itsPlanC2CFf.reset(new FFTWPlanf(fftwf_plan_dft(
      size.nelements(), size.asStdVector().data(), reinterpret_cast<fftwf_complex *>(in),
      reinterpret_cast<fftwf_complex *>(in), FFTW_FORWARD, flags)));
}

void FFTW::plan_c2c_backward(const IPosition &size, std::complex<double> *in) {
  itsPlanC2CB.reset(new FFTWPlan(fftw_plan_dft(
      size.nelements(), size.asStdVector().data(), reinterpret_cast<fftw_complex *>(in),
      reinterpret_cast<fftw_complex *>(in), FFTW_BACKWARD, flags)));
}

void FFTW::plan_c2c_backward(const IPosition &size, std::complex<float> *in) {
  itsPlanC2CBf.reset(new FFTWPlanf(fftwf_plan_dft(
      size.nelements(), size.asStdVector().data(), reinterpret_cast<fftwf_complex *>(in),
      reinterpret_cast<fftwf_complex *>(in), FFTW_BACKWARD, flags)));
}
...
FFTW::Plan FFTW::plan_redft00(const IPosition &size, float *in, float *out) {
  ...
  return Plan(new FFTWPlanf(fftwf_plan_r2r(size.nelements(), size.asStdVector().data(), in, out,
                                           kinds.data(), FFTW_ESTIMATE)));
}
...
FFTW::Plan FFTW::plan_redft00(const IPosition &size, double *in, double *out) {
  ...
  return Plan(new FFTWPlan(fftw_plan_r2r(size.nelements(), size.asStdVector().data(), in, out,
                                         kinds.data(), FFTW_ESTIMATE)));
}

void FFTW::Plan::Execute(float *in, float *out) { fftwf_execute_r2r(_planf->getPlan(), in, out); }

void FFTW::Plan::Execute(double *in, double *out) { fftw_execute_r2r(_plan->getPlan(), in, out); }
...
void FFTW::Plan::Execute(float *, float *) { throw std::runtime_error("FFTW not available"); }

void FFTW::Plan::Execute(double *, double *) { throw std::runtime_error("FFTW not available"); }

#endif

FFTW::Plan::Plan(Plan &&) = default;

FFTW::Plan::Plan(FFTWPlan *plan) : _plan(plan) {}

FFTW::Plan::Plan(FFTWPlanf *plan) : _planf(plan) {}
