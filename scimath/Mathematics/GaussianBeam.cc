...
GaussianBeam::GaussianBeam()
    : _major(Quantity(0, "arcsec")), _minor(Quantity(0, "arcsec")), _pa(Quantity(0, "deg")) {}

GaussianBeam::GaussianBeam(const Quantity& major, const Quantity& minor, const Quantity& pa) {
  setMajorMinor(major, minor);
  setPA(pa);
}

GaussianBeam::GaussianBeam(const Vector<Quantity>& parms) {
  if (parms.size() != 3) {
    throw AipsError(
...
GaussianBeam::GaussianBeam(const GaussianBeam& other)
    : _major(other._major), _minor(other._minor), _pa(other._pa) {}
...
void GaussianBeam::setMajorMinor(const Quantity& majAx, const Quantity& minAx) {
  static ostringstream oss;
  auto majVal = majAx.getValue();
  auto minVal = minAx.getValue();
  ThrowIf(isInf(majVal) || isInf(minVal) || isNaN(majVal) || isNaN(minVal),
          "Neither the major nor the minor axis value is permitted "
          "to be infinity or NaN");
  ThrowIf(majVal < 0, "Major axis cannot be less than zero.");
  ThrowIf(minVal < 0, "Minor axis cannot be less than zero.");
  ThrowIf(!majAx.isConform("rad"),
          "Major axis must have angular units (" + majAx.getUnit() + " is not).");
  ThrowIf(!minAx.isConform("rad"),
          "Major axis must have angular units (" + minAx.getUnit() + " is not).");
  ThrowIf(majAx < minAx, "Major axis must be greater or equal to minor axis");
  _major = majAx;
  _minor = minAx;
}
...
void GaussianBeam::setPA(const Quantity& pa, Bool unwrap) {
  ...
  ThrowIf(!pa.isConform("rad"),
          "Position angle must have angular units (" + pa.getUnit() + " is not).");
...
Double GaussianBeam::getArea(const Unit& unit) const {
  ...
  if (qunit.isConform("sr") || qunit.isConform("rad2")) {
    ...
    ostringstream oss;
    oss << className() << "::" << __FUNCTION__ << ": Unit " << unit.getName()
        << " is not a solid angle.";
...
GaussianBeam GaussianBeam::fromRecord(const Record& rec) {
  if (rec.nfields() != 3) {
...
void GaussianBeam::convert(const String& majUnit, const String& minUnit, const String& paUnit) {
