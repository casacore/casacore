...
void InterpolateArray1D<Domain, Range>::interpolate(Array<Range>& yout, Array<Bool>& youtFlags,
  ...
  interpolatePtr(youtPtrs, youtFlagPtrs, yStep, xout, xin, yinPtrs, yinFlagPtrs, method, goodIsTrue,
                 extrapolate);
...
void InterpolateArray1D<Domain, Range>::interpolatey(Cube<Range>& yout, Cube<Bool>& youtFlags,
  ...
  interpolateyPtr(youtPtrs, youtFlagPtrs, na, nb, nc, xout, xin, yinPtrs, yinFlagPtrs, method,
    ...
    case spline:  // natural cubic splines
    {
      ...
      for (Int j = 0; j < ny; j++) {
        ...
        if (nearAbs(xin[1], xin[0]))
          throw(
              AipsError("Interpolate1D::setMethod"
                        " data has repeated x values"));
        Domain a, b, delta;
        const Domain six = 6;
        const Float one = 1;
          ...
          if (nearAbs(xin[i + 1], xin[i]))
            throw(
                AipsError("Interpolate1D::setMethod"
                          " data has repeated x values"));
          c = (xin[i + 1] - xin[i]);
          r = (one / c) * (yin[i + 1][j] - yin[i][j]) - (one / a) * (yin[i][j] - yin[i - 1][j]);
          delta = a * t[i - 1];
          if (nearAbs(b, delta))
            throw(
                AipsError("Interpolate1D::setMethod"
    ...
    case spline:  // natural cubic splines
    {
      ...
      for (Int j = 0; j < ny; j++) {
        ...
        for (i = 0; i < Int(xout.nelements()); i++) {
          ...
          yout[i][j] = a * y1v + b * y2v + h * (a * a * a - a) * y1d + h * (b * b * b - b) * y2d;
          if (goodIsTrue)
            youtFlags[i][j] = (discard ? flag : f1 && f2);
          else
            youtFlags[i][j] = (discard ? flag : f1 || f2);
        }
      }
...
void InterpolateArray1D<Domain, Range>::interpolateyPtr(Block<Range*>& yout, Int na, Int nb, Int nc,
  ...
  switch (method) {
    ...
    {
      throw(AipsError(
          "Interpolate1DArray::interpolateyPtr(): method=nearestNeigbour is not implemented yet"));
      return;
    }
    case linear:  // Linear interpolation is the default
    {
    ...
    {
      throw(
          AipsError("Interpolate1DArray::interpolateyPtr(): method=cubic is not implemented yet"));
      return;
    }
    case spline:  // natural cubic splines
    {
