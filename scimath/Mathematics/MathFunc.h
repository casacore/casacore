...
class KB_Conv : public MathFunc<T> {
 public:
  // default constructor
  KB_Conv(T cut = MathFunc<T>::defcutoff(), T wparm = MathFunc<T>::defKBwidth(),
...
class Mod_KB_Conv : public MathFunc<T> {
 public:
  // default constructor
  Mod_KB_Conv(T cut = MathFunc<T>::defcutoff(), T wparm = MathFunc<T>::defKBwidth(),
              T kbparm = MathFunc<T>::defKBparm(), T gwparm = MathFunc<T>::defmodKBparm());
...
class ExpSincConv : public MathFunc<T> {
 public:
  ...
  // default constructor
  //
  ExpSincConv(T cut = MathFunc<T>::defcutoff(), T sincparm = MathFunc<T>::defSincparm(),
              T exppow = MathFunc<T>::defExpPower(), T expscale = MathFunc<T>::defExpScale());
  // copy constructor
