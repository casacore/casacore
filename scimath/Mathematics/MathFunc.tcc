...
MathFunc<T>::MathFunc(FUNCTYPE type) {
  switch (type) {
    ...
    default:
      throw(
          MathFuncError(" MathFunc::MathFunc: Invalid enumerated"
                        " type as argument"));
      break;
  }
}
...
MathFunc<T>::MathFunc(String& type, Vector<double>& args) {
  if (type == "MOD_KB")
    object = new Mod_KB_Conv<T>((T)args(0), (T)args(1), (T)args(2), (T)args(3));
    ...
    object = new ExpSincConv<T>((T)args(0), (T)args(1), (T)args(2), (T)args(3));
  else
    throw(
        MathFuncError(" MathFunc::MathFunc: Invalid String value"
                      " as argument"));
}
...
//
template <class T>
Unary<T>::Unary(const Unary<T>& other) : MathFunc<T>(), sup_width(other.sup_width) {
  // nothing
}
...
//
template <class T>
GaussianConv<T>::GaussianConv(T cut, T wparm)
    : MathFunc<T>(), sup_width(cut), fw2(wparm * wparm), ln16(4.0 * M_LN2) {
  // nothing
}
...
//
template <class T>
GaussianConv<T>::GaussianConv(const GaussianConv<T>& other)
    : MathFunc<T>(), sup_width(other.sup_width), fw2(other.fw2), ln16(4.0 * M_LN2) {
  // nothing
}
...
MathFunc<T>* GaussianConv<T>::clone() const {
  return new GaussianConv<T>(*this);
}  // use GaussianConv copy ctor
...
//
template <class T>
KB_Conv<T>::KB_Conv(T cut, T wparm, T KBparm)
    : MathFunc<T>(), kbparm(KBparm), fw(wparm), sup_width(cut) {
  // nothing
}
...
//
template <class T>
KB_Conv<T>::KB_Conv(const KB_Conv<T>& other)
    : MathFunc<T>(), kbparm(other.kbparm), fw(other.fw), sup_width(other.sup_width) {
  // nothing
}
...
MathFunc<T>* KB_Conv<T>::clone() const {
  return new KB_Conv<T>(*this);
}  // use KB_Conv copy constructor
...
//
template <class T>
Mod_KB_Conv<T>::Mod_KB_Conv(T cut, T wparm, T KBparm, T gwparm)
    : MathFunc<T>(),
      kbparm(KBparm),
      gw2(gwparm * gwparm),
      sup_width(cut),
...
//
template <class T>
Mod_KB_Conv<T>::Mod_KB_Conv(const Mod_KB_Conv<T>& other)
    : MathFunc<T>(),
      kbparm(other.kbparm),
      gw2(other.gw2),
      sup_width(other.sup_width),
...
MathFunc<T>* Mod_KB_Conv<T>::clone() const {
  return new Mod_KB_Conv<T>(*this);
}  // use Mod_KB_Conv copy ctor
...
//
template <class T>
Sinc_Conv<T>::Sinc_Conv(T cut, T sincparm) : MathFunc<T>(), Sinc_parm(sincparm), sup_width(cut) {
  // nothing
}
...
//
template <class T>
Sinc_Conv<T>::Sinc_Conv(const Sinc_Conv<T>& other)
    : MathFunc<T>(), Sinc_parm(other.Sinc_parm), sup_width(other.sup_width) {
  // nothing
}
...
MathFunc<T>* Sinc_Conv<T>::clone() const {
  return new Sinc_Conv<T>(*this);
}  // use Sinc_Conv copy ctor
...
//
template <class T>
Sph_Conv<T>::Sph_Conv(T cut, T Sphparm) : MathFunc<T>(), sup_width(cut), sphparm(Sphparm) {
  // nothing
}
...
//
template <class T>
Sph_Conv<T>::Sph_Conv(const Sph_Conv<T>& other)
    : MathFunc<T>(), sup_width(other.sup_width), sphparm(other.sphparm) {
  // nothing
}
...
MathFunc<T>* Sph_Conv<T>::clone() const {
  return new Sph_Conv<T>(*this);
}  // use Sph_Conv copy ctor
...
//
template <class T>
ExpSincConv<T>& ExpSincConv<T>::operator=(const ExpSincConv<T>& other) {
  if (this == &other) return *this;
  sup_width = other.sup_width;
  scale = other.scale;
  exponent = other.exponent;
...
MathFunc<T>* ExpSincConv<T>::clone() const {
  return new ExpSincConv<T>(*this);
}  // use ExpSincConv copy ctor
