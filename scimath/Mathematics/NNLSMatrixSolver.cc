...
extern "C" {
void nnls(FType *, int *, int *, int *, FType *, FType *, float *, FType *, FType *, int *, int *,
          int *);
}
...
// Copy Constructor
NNLSMatrixSolver::NNLSMatrixSolver(const NNLSMatrixSolver &other) : MatrixSolver(other) {}
...
NNLSMatrixSolver::NNLSMatrixSolver(const Matrix<FType> &A, const Vector<FType> &B)
    : MatrixSolver(A, B) {}
...
Bool NNLSMatrixSolver::solve()  // Solve AX=B for X
{
  ...
  nnls(a_data, &ndata, &ndata, &nflux, b_data, x_data, &rnorm, w, zz, index, &itmax, &mode);

  RVector = BVector - product(AMatrix, XVector);  // Update residual vector
  if (mode == 2) {
    ostringstream o;
    o << "dimensions set up incorrectly";
  ...
  if (mode == 3) {
    ostringstream o;
    o << "Exceeded number of iterations";
    message.priority(LogMessage::SEVERE);
    message.message(o.str());
    logSink().post(message);
    setSolved(False);
