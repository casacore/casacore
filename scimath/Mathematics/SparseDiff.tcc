...
template <class T>
SparseDiff<T>::SparseDiff() : rep_p(ObjectStack<SparseDiffRep<T>>::stack().get()) {}

template <class T>
SparseDiff<T>::SparseDiff(const T &v) : rep_p(ObjectStack<SparseDiffRep<T>>::stack().get()) {
  rep_p->val_p = v;
}
  ...
  if (grad().empty()) {
    ...
    for (typename vector<pair<uInt, T>>::iterator i = grad().begin(); i != grad().end(); ++i) {
      if (j == other.grad().end()) {
        ...
        if (value() != T(0)) tmp->grad_p.push_back(std::make_pair(j->first, j->second * value()));
        ++j;
        --i;
      }
    }
  ...
  if (grad().empty()) {
    ...
    for (typename vector<pair<uInt, T>>::iterator i = grad().begin(); i != grad().end(); ++i) {
      if (j == other.grad().end()) {
        ...
        if (value() != T(0))
          tmp->grad_p.push_back(std::make_pair(j->first, -j->second * value() / t));
        ++j;
        --i;
      }
    }
  ...
  if (grad().empty()) {
    ...
    for (typename vector<pair<uInt, T>>::iterator i = grad().begin(); i != grad().end(); ++i) {
      if (j == other.grad().end()) {
        ...
        if ((v = i->second + j->second) != T(0)) tmp->grad_p.push_back(std::make_pair(i->first, v));
        ++j;
      } else if (j->first > i->first) {
        tmp->grad_p.push_back(*i);
      } else {
