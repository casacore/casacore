...
class f<AutoDiffA<Double>> {
 public:
  AutoDiffA<Double> operator()(const AutoDiffA<Double>& x) {
    ...
    return AutoDiffA<Double>(
        a_p.value() * a_p.value() * a_p.value() * b_p.value() * b_p.value() * x.value(), dr);
  }
...
int main() {
  ...
  AutoDiff<AutoDiff<Double>> a4(AutoDiff<Double>(2), 1), b4(AutoDiff<Double>(3), 1),
      x4(AutoDiff<Double>(7, 1, 0), 1, 0);
  f<AutoDiff<AutoDiff<Double>>> f4;
  f4.set(a4, b4);
