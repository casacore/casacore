...
int main() {
  ...
  // test the constructors
  {
    ...
    if (z.value() != val || !allEQ(z.derivatives(), g)) {
      cerr << "AutoDiff<Float> z(val, g); failed z = " << z << " val = " << val << " g = " << g
           << endl;
      nerr++;
    }
  }
  ...
  // test the class member operators
  {
    ...
    z *= y;
    // verify result
    if (z.value() != (x.value() * y.value()) || z.derivatives()(0) != y.value() ||
    ...
    z /= y;
    // verify result
    if (z.value() != (x.value() / y.value()) || z.derivatives()(0) != (1.0 / y.value()) ||
    ...
    z += y;
    // verify result
    if (z.value() != (x.value() + y.value()) || z.derivatives()(0) != 1 ||
        z.derivatives()(1) != 1) {
      cerr << "+= operator failed" << endl;
      nerr++;
    }
  ...
  // AutoDIffMath tests
  {
    ...
    y = x - x;
    if (y.value() != 0.0 || !allEQ(y.derivatives(), Float(0.0))) {
      cerr << "operator-(const AutoDiff<T> &, const AutoDiff<T> &) failed" << endl;
      nerr++;
    }
    y = x * x;
    ...
    y = x / x;
    if (!near(y.value(), Float(1)) || !allNearAbs(y.derivatives(), Float(0.0), 1.0e-5)) {
      cerr << "operator/(const AutoDiff<T> &, const AutoDiff<T> &) failed" << endl;
      nerr++;
    }

    y = x + Float(1.0);
    if (y.value() != (x.value() + Float(1.0)) || !allEQ(y.derivatives(), x.derivatives())) {
    ...
    y = Float(1.0) - x;
    if (y.value() != (Float(1.0) - x.value()) || !allEQ(y.derivatives(), -x.derivatives())) {
      cerr << "operator-(const T&, const AutoDiff<T> &) failed" << endl;
      nerr++;
    }
    ...
    if (!near(y.value(), Float(2.0) / x.value()) ||
        !allNearAbs(y.derivatives(), -x.derivatives() * Float(2.0) / (x.value() * x.value()),
                    1.0e-5)) {
      cerr << "operator/(const T&, const AutoDiff<T> &) failed" << endl;
      nerr++;
    }
    ...
    if (y.value() != Float(acos(x.value())) ||
        !allEQ(y.derivatives(), -x.derivatives() / Float(sqrt(1.0 - x.value() * x.value())))) {
    ...
    if (!allNearAbs(y.value(), Float(atan(x.value())), 1.e-6) ||
        !allNearAbs(y.derivatives(), x.derivatives() / Float(1.0 + x.value() * x.value()), 1.e-6)) {
    ...
    if (y.value() != Float(atan2(w.value(), z.value())) ||
        !allEQ(y.derivatives(), (w.derivatives() / z.value() -
    ...
    if (y.value() != Float(pow(w.value(), z.value())) ||
        !allEQ(y.derivatives(),
               (Float(z.value() * pow(w.value(), z.value() - 1)) * w.derivatives() +
                Float(pow(w.value(), z.value()) * log(w.value())) * z.derivatives()))) {
    ...
    if (y.value() != Float(pow(x.value(), 2.5)) ||
        !allEQ(y.derivatives(), Float(2.5 * pow(x.value(), 1.5)) * x.derivatives())) {
    ...
    if (!allNearAbs(y.value(), Float(tan(x.value()))) ||
        !allNearAbs(y.derivatives(), x.derivatives() / Float(cos(x.value()) * cos(x.value())),
                    1.e-6)) {
    ...
    if (!allNearAbs(y.value(), Float(tanh(x.value()))) ||
        !allEQ(y.derivatives(), x.derivatives() / Float(cosh(x.value()) * cosh(x.value())))) {
      cerr << "sinh(const AutoDiff<T> &) failed" << endl;
      nerr++;
    }
  }
