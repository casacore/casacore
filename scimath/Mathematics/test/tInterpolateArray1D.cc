...
class TestLinearInterpolation1 {
 public:
  TestLinearInterpolation1() {
    ...
    cout << "--- equidistant output grid "
            "------------------------------------------------------------"
         << endl;
    cout << "--- change output grid width from identical to double width in steps of 1/iterations "
            "---"
         << endl;

    Double iterations =
        1000.;  // change output grid width from identical to double width in steps of 1/iterations
        ...
        for (uInt j = 0; j < 500; j++) {
          ...
          if (debug) {
            cout << it << " " << i << " " << j << " " << xin(j) << " " << xout(j) << " "
                 << yout(IPosition(2, i, j)) << " " << expect(IPosition(2, i, j)) << endl;
            cout << it << " flag " << i << " " << j << " " << youtFlags(IPosition(2, i, j)) << " "
...
class TestLinearInterpolation2 {
 public:
  TestLinearInterpolation2() {
    ...
    cout << "--- equidistant output grid, increasing values "
            "-----------------------------------------"
         << endl;
    cout << "--- change output grid width from identical to double width in steps of 1/iterations "
            "---"
    ...
    for (Int it = 0; it < (Int)iterations; it++) {
      ...
      for (uInt i = 0; i < 2; i++) {
        for (uInt j = 0; j < 500; j++) {
          Double diffr =
              yout(IPosition(2, i, j)).real() - xout(j) * expect(IPosition(2, i, j)).real();
          Double diffi =
              yout(IPosition(2, i, j)).imag() - xout(j) * expect(IPosition(2, i, j)).imag();
          if (debug) {
            cout << it << " " << i << " " << j << " " << xin(j) << " " << xout(j) << " "
                 << yout(IPosition(2, i, j)) << " " << xout(j) * expect(IPosition(2, i, j))
                 << " diffs: " << diffr << " " << diffi << endl;
            cout << it << " flag " << i << " " << j << " " << youtFlags(IPosition(2, i, j)) << " "
                 << expflags(IPosition(2, i, j)) << endl;
          }
...
class TestNearestInterpolation1 {
 public:
  TestNearestInterpolation1() {
    ...
    yatest +=
        static_cast<S>(0.4999);  // ensures middle value goes down (matches InterpolateArray1D)
    yatest = floor(yatest);
