...
int main() {
  ...
  // test the constructors
  {
    ...
    if (x.value() != 2.0 || x.nDerivatives() != 1 || x.derivative(0) != pair<uInt, Float>(1, 1)) {
    ...
    if (z.value() != val || z.nDerivatives() != 1 || z.derivative(0) != pair<uInt, Float>(2, 73.)) {
  ...
  // test the assignment operators
  {
    ...
    x = z;
    if (x.value() != y.value() || x.nDerivatives() != 2 || x.derivative(1) != z) {
    ...
    z0.push_back(z);
    x = z0;
    if (x.value() != y.value() || x.nDerivatives() != 2 || x.derivative(0) != z ||
        x.derivative(1) != z1) {
  ...
  // test the class member operators
  {
    ...
    z *= y;
    // verify result
    if (z.value() != (x.value() * y.value()) || z.derivative(0).second != y.value() ||
    ...
    z /= y;
    // verify result
    if (z.value() != (x.value() / y.value()) || z.derivative(0).second != (1.0 / y.value()) ||
    ...
    z += y;
    // verify result
    if (z.value() != (x.value() + y.value()) || z.derivative(0).second != 1 ||
  ...
  // AutoDIffMath tests
  {
    ...
    y = x + x;
    if (y.value() != (Float(2.0) * x.value()) || y.nDerivatives() != x.nDerivatives() ||
        y.derivative(0).second != Float(2.0) * x.derivative(0).second) {
      cerr << "operator+(const SparseDiff<T> &, const SparseDiff<T> &) failed" << endl;
      nerr++;
    }
    y = x - x;
    ...
    y = x * x;
    if (y.value() != (x.value() * x.value()) || y.nDerivatives() != x.nDerivatives() ||
        y.derivative(0).second != Float(2.0) * x.value() * x.derivative(0).second) {
      cerr << "operator*(const SparseDiff<T> &, const SparseDiff<T> &) failed" << endl;
      nerr++;
    }
    ...
    y = x + Float(1.0);
    if (y.value() != (x.value() + Float(1.0)) || y.nDerivatives() != x.nDerivatives() ||
    ...
    y = x * Float(2.0);
    if (y.value() != (x.value() * Float(2.0)) || y.nDerivatives() != x.nDerivatives() ||
    ...
    y = Float(1.0) - x;
    if (y.value() != (Float(1.0) - x.value()) || y.nDerivatives() != x.nDerivatives() ||
    ...
    if (y.value() != Float(acos(x.value())) ||
        !testDer(y, x, -Float(1.) / Float(sqrt(1.0 - x.value() * x.value())))) {
    ...
    if (!allNearAbs(y.value(), Float(atan(x.value())), 1.e-6) ||
        !testDer(y, x, Float(1.) / Float(1.0 + x.value() * x.value()))) {
      cerr << y.value() - Float(atan(x.value())) << endl;
      cerr << y.derivative(0).second - x.derivative(0).second / Float(1.0 + x.value() * x.value())
           << endl;
    ...
    if (y.value() != Float(atan2(w.value(), z.value())) ||
        !near(y.derivative(0).second,
              Float(1) / (Float(1) + w * w / z / z).value() * (w / z).derivative(0).second, 1e-5) ||
        !near(y.derivative(1).second,
              Float(1) / (Float(1) + w * w / z / z).value() * (w / z).derivative(1).second, 1e-5)) {
      cerr << "atan2(const SparseDiff<T> &, const SparseDiff<T> &g) failed" << endl;
      nerr++;
    }

    // cos(x) : derivative = -sin(x)
    y = cos(x);
    if (!nearAbs(y.value(), Float(cos(x.value()))) || !testDer(y, x, -Float(sin(x.value())))) {
    ...
    y = cosh(x);
    if (y.value() != Float(cosh(x.value())) || !testDer(y, x, Float(sinh(x.value())))) {
    ...
    y = exp(x);
    if (y.value() != Float(exp(x.value())) || !testDer(y, x, Float(exp(x.value())))) {
    ...
    y = log(x);
    if (y.value() != Float(log(x.value())) || !testDer(y, x, Float(1.) / x.value())) {
    ...
    if (y.value() != Float(log10(x.value())) ||
        !testDer(y, x, Float(1) / Float((x.value() * log(10.0))))) {
    ...
    if (!near(y.value(), Float(pow(w.value(), z.value())), 1E-7) ||
        !near(y.derivative(0).second,
              (Float(z.value() * pow(w.value(), z.value() - 1)) * w.derivative(0).second), 1E-7) ||
        !near(y.derivative(1).second,
              Float(pow(w.value(), z.value()) * log(w.value())) * z.derivative(0).second, 1E-7)) {
    ...
    if (y.value() != Float(pow(x.value(), 2.5)) ||
        !testDer(y, x, Float(2.5 * pow(x.value(), 1.5)))) {
    ...
    y = sin(x);
    if (!allNearAbs(y.value(), Float(sin(x.value()))) || !testDer(y, x, Float(cos(x.value())))) {
    ...
    y = sinh(x);
    if (!allNearAbs(y.value(), Float(sinh(x.value()))) || !testDer(y, x, Float(cosh(x.value())))) {
    ...
    if (!allNearAbs(y.value(), Float(sqrt(x.value()))) ||
        !testDer(y, x, Float(0.5 / sqrt(x.value())))) {
    ...
    if (!allNearAbs(y.value(), Float(tan(x.value()))) ||
        !testDer(y, x, Float(1) / Float(cos(x.value()) * cos(x.value())))) {
    ...
    if (!allNearAbs(y.value(), Float(tanh(x.value()))) ||
        !testDer(y, x, Float(1) / Float(cosh(x.value()) * cosh(x.value())))) {
      cerr << "sinh(const SparseDiff<T> &) failed" << endl;
      nerr++;
    }
  }
