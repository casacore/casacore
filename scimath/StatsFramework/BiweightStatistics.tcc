...
CASA_STATD
BiweightStatistics<CASA_STATP>::BiweightStatistics(const BiweightStatistics<CASA_STATP>& other)
    : ClassicalStatistics<CASA_STATP>(other),
      _c(other._c),
      _niter(other._niter),
...
CASA_STATD
void BiweightStatistics<CASA_STATP>::setStatsToCalculate(std::set<StatisticsData::STATS>& stats) {
...
void BiweightStatistics<CASA_STATP>::_computeLocationSums(
  ...
  if (chunk.weights) {
    ...
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _locationSums(sxw2, sw2, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
                  chunk.ranges->second);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
    // associated with it, and its stride is 1. No filtering of the data is
    // necessary.
    _locationSums(sxw2, sw2, dataIter, dataCount, chunk.dataStride);
  }
}
...
void BiweightStatistics<CASA_STATP>::_computeScaleSums(
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _scaleSums(sx_M2w4, ww_4u2, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
                   chunk.mask->second, chunk.ranges->first, chunk.ranges->second);
      } else {
        _scaleSums(sx_M2w4, ww_4u2, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
                   chunk.mask->second);
      ...
      _scaleSums(sx_M2w4, ww_4u2, dataIter, weightsIter, dataCount, chunk.dataStride,
                 chunk.ranges->first, chunk.ranges->second);
    } else {
      // has weights, but no mask nor ranges
      _scaleSums(sx_M2w4, ww_4u2, dataIter, weightsIter, dataCount, chunk.dataStride);
    ...
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _scaleSums(sx_M2w4, ww_4u2, dataIter, dataCount, chunk.dataStride, maskIter,
                 chunk.mask->second, chunk.ranges->first, chunk.ranges->second);
    } else {
      _scaleSums(sx_M2w4, ww_4u2, dataIter, dataCount, chunk.dataStride, maskIter,
                 chunk.mask->second);
    ...
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _scaleSums(sx_M2w4, ww_4u2, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
               chunk.ranges->second);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
    // associated with it, and its stride is 1. No filtering of the data is
    // necessary.
    _scaleSums(sx_M2w4, ww_4u2, dataIter, dataCount, chunk.dataStride);
  }
}
...
void BiweightStatistics<CASA_STATP>::_computeStats() {
  ...
  _npts = cs.getNPts();
  ThrowIf(_npts <= 1, "npts is " + ValueToString(_npts) +
  ...
  _range = std::pair<AccumType, AccumType>(_location - spread, _location + spread);
  if (_maxNiter >= 0) {
    ...
    for (_niter = 1; _niter <= _maxNiter; ++_niter) {
      ...
      _range = std::pair<AccumType, AccumType>(_location - spread, _location + spread);
      _doScale();
      if (abs(1 - _scale / prevScale) < epsilon || _niter == _maxNiter) {
        break;
      }
      spread = _c * _scale;
      _range = std::pair<AccumType, AccumType>(_location - spread, _location + spread);
