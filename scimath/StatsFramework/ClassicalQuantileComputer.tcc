...
CASA_STATD
ClassicalQuantileComputer<CASA_STATP>& ClassicalQuantileComputer<CASA_STATP>::operator=(
    const ClassicalQuantileComputer<CASA_STATP>& other) {
  if (this == &other) {
    return *this;
  }
...
CASA_STATD
StatisticsAlgorithmQuantileComputer<CASA_STATP>* ClassicalQuantileComputer<CASA_STATP>::clone()
    const {
  ...
  if (!median) {
    ...
    median.reset(new AccumType(
        indexToValue.size() == 1
            ...
            : (indexToValue[*indices.begin()] + indexToValue[*indices.rbegin()]) / AccumType(2)));
...
AccumType ClassicalQuantileComputer<CASA_STATP>::getMedianAndQuantiles(
  ...
  auto indexToValue =
      _indicesToValues(mynpts, mymin, mymax, binningThreshholdSizeBytes / sizeof(AccumType),
                       indices, persistSortedArray, nBins);
  if (!median) {
    median.reset(mynpts % 2 == 0 ? new AccumType((indexToValue[*medianIndices.begin()] +
...
CASA_STATD std::vector<std::vector<uInt64>> ClassicalQuantileComputer<CASA_STATP>::_binCounts(
  ...
  if (hist.size() > 1) {
    ...
    while (iDesc != eDesc) {
      if (iDesc != bDesc) {
        ThrowIf(iDesc->getMinHistLimit() <= prevDesc.getMinHistLimit(),
...
#pragma omp parallel for num_threads(nthreads)
  ...
  StatisticsUtilities<AccumType>::mergeResults(bins, sameVal, allSame, tBins, tSameVal, tAllSame,
                                               nThreadsMax);
  return bins;
}
...
void ClassicalQuantileComputer<CASA_STATP>::_computeBins(
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _findBins(bins, sameVal, allSame, dataIter, weightsIter, count, chunk.dataStride, maskIter,
                  chunk.mask->second, chunk.ranges->first, chunk.ranges->second, hist, maxLimit);
      } else {
        _findBins(bins, sameVal, allSame, dataIter, weightsIter, count, chunk.dataStride, maskIter,
                  chunk.mask->second, hist, maxLimit);
      }
    } else if (chunk.ranges) {
      _findBins(bins, sameVal, allSame, dataIter, weightsIter, count, chunk.dataStride,
                chunk.ranges->first, chunk.ranges->second, hist, maxLimit);
    } else {
      // has weights, but no mask nor ranges
      _findBins(bins, sameVal, allSame, dataIter, weightsIter, count, chunk.dataStride, hist,
                maxLimit);
    }
  } else if (chunk.mask) {
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _findBins(bins, sameVal, allSame, dataIter, count, chunk.dataStride, maskIter,
                chunk.mask->second, chunk.ranges->first, chunk.ranges->second, hist, maxLimit);
    } else {
      _findBins(bins, sameVal, allSame, dataIter, count, chunk.dataStride, maskIter,
                chunk.mask->second, hist, maxLimit);
    ...
    // associated with it
    _findBins(bins, sameVal, allSame, dataIter, count, chunk.dataStride, chunk.ranges->first,
    ...
    _findBins(bins, sameVal, allSame, dataIter, count, chunk.dataStride, hist, maxLimit);
  }
}
...
void ClassicalQuantileComputer<CASA_STATP>::_computeDataArray(
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _populateArray(ary, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
      ...
      _populateArray(ary, dataIter, weightsIter, dataCount, chunk.dataStride, chunk.ranges->first,
                     chunk.ranges->second);
    } else {
      // has weights, but no mask nor ranges
      _populateArray(ary, dataIter, weightsIter, dataCount, chunk.dataStride);
    }
  } else if (chunk.mask) {
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _populateArray(ary, dataIter, dataCount, chunk.dataStride, maskIter, chunk.mask->second,
    ...
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _populateArray(ary, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
                   chunk.ranges->second);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
    // associated with it, and its stride is 1. No filtering of the data is
    // necessary.
    _populateArray(ary, dataIter, dataCount, chunk.dataStride);
  }
}
...
void ClassicalQuantileComputer<CASA_STATP>::_computeDataArrays(
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _populateArrays(arys, currentCount, dataIter, weightsIter, dataCount, chunk.dataStride,
    ...
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _populateArrays(arys, currentCount, dataIter, dataCount, chunk.dataStride, maskIter,
                      chunk.mask->second, chunk.ranges->first, chunk.ranges->second, includeLimits,
                      maxCount);
    } else {
      _populateArrays(arys, currentCount, dataIter, dataCount, chunk.dataStride, maskIter,
                      chunk.mask->second, includeLimits, maxCount);
    }
  } else if (chunk.ranges) {
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _populateArrays(arys, currentCount, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
                    chunk.ranges->second, includeLimits, maxCount);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
    // associated with it, and its stride is 1. No filtering of the data is
    // necessary.
    _populateArrays(arys, currentCount, dataIter, dataCount, chunk.dataStride, includeLimits,
                    maxCount);
  }
}
...
ClassicalQuantileComputer<CASA_STATP>::_dataFromMultipleBins(
  ...
  for_each(dataIndices.cbegin(), dataIndices.cend(),
           [&iSameVal, &iDesc, &iCountSet, &vNewToOld, &vnpts, &vlimits, &binToHistogramMap,
            &vindices, &histToIdxValMap](const IndexSet& idxSet) {
  ...
  if (!vnpts.empty()) {
    auto dataFromBins = _dataFromSingleBins(vnpts, maxArraySize, vlimits, vindices, nBins);
    auto iNewToOld = vNewToOld.cbegin();
    auto iVLimits = vlimits.cbegin();
    for_each(dataFromBins.cbegin(), dataFromBins.cend(),
             [&iVLimits, &binToHistogramMap, &iNewToOld,
              &histToIdxValMap](const IndexValueMap& idxValMap) {
  ...
  for_each(hist.cbegin(), hist.cend(),
           [&ret, &histToIdxValMap](const StatsHistogram<AccumType>& myhist) {
...
ClassicalQuantileComputer<CASA_STATP>::_dataFromSingleBins(const BinCountArray& binNpts,
  ...
  if (totalPts <= maxArraySize) {
    ...
    for_each(dataArrays.cbegin(), dataArrays.cend(), [&iNpts](const DataArray& ary) {
      ThrowIf(ary.size() != *iNpts, "Logic Error: data array has " + ValueToString(ary.size()) +
...
std::map<uInt64, AccumType> ClassicalQuantileComputer<CASA_STATP>::_indicesToValues(
  ...
  return _dataFromSingleBins(vmynpts, maxArraySize, vlimits, vindices, nBins)[0];
}
...
Bool ClassicalQuantileComputer<CASA_STATP>::_valuesFromSortedArray(IndexValueMap& values,
  ...
  if (_doMedAbsDevMed && !this->_getSortedArray().empty()) {
    ...
    myArray = pSorted;
    StatisticsUtilities<AccumType>::convertToAbsDevMedArray(myArray, *this->_getMedian());
  }
  if (!_doMedAbsDevMed) {
