...
ClassicalStatistics<CASA_STATP>::ClassicalStatistics()
    : StatisticsAlgorithm<CASA_STATP>(),
      ...
      _qComputer(new ClassicalQuantileComputer<CASA_STATP>(&this->_getDataset())) {
  reset();
}
...
CASA_STATD
ClassicalStatistics<CASA_STATP>::ClassicalStatistics(const ClassicalStatistics<CASA_STATP>& cs)
    : StatisticsAlgorithm<CASA_STATP>(cs),
      _statsData(cs._statsData),
      _calculateAsAdded(cs._calculateAsAdded),
      _doMaxMin(cs._doMaxMin),
      _mustAccumulate(cs._mustAccumulate),
      _qComputer((ClassicalQuantileComputer<CASA_STATP>*)(cs._qComputer->clone())) {
  _qComputer->setDataset(&this->_getDataset());
}

CASA_STATD
ClassicalStatistics<CASA_STATP>& ClassicalStatistics<CASA_STATP>::operator=(
  ...
  _qComputer.reset((ClassicalQuantileComputer<CASA_STATP>*)(other._qComputer->clone()));
  // setting the dataset in the quantile calculator must be done explicitly
  _qComputer->setDataset(&this->_getDataset());
  return *this;
}

CASA_STATD StatisticsAlgorithm<CASA_STATP>* ClassicalStatistics<CASA_STATP>::clone() const {
...
void ClassicalStatistics<CASA_STATP>::_doNptsMinMax(uInt64& mynpts, AccumType& mymin,
  ...
  if (knownMin && knownMax) {
    ThrowIf(*knownMax < *knownMin, "Provided max " + ValueToString(*knownMax) +
                                       " is less than "
                                       "provided min " +
                                       ValueToString(*knownMin));
    mymin = *knownMin;
    mymax = *knownMax;
  }
  if (knownNpts) {
...
AccumType ClassicalStatistics<CASA_STATP>::getMedianAndQuantiles(
  ...
  _getStatsData().median.reset(new AccumType(
      _qComputer->getMedianAndQuantiles(quantiles, fractions, mynpts, mymin, mymax,
                                        binningThreshholdSizeBytes, persistSortedArray, nBins)));
  return *_getStatsData().median;
}

CASA_STATD
void ClassicalStatistics<CASA_STATP>::getMinMax(AccumType& mymin, AccumType& mymax) {
...
void ClassicalStatistics<CASA_STATP>::setCalculateAsAdded(Bool c) {
  ...
  ThrowIf(ds.iDataset() > 0, "Logic Error: " + String(__func__) +
                                 " cannot be called after the first dataset has been set");
  _calculateAsAdded = c;
}

CASA_STATD
void ClassicalStatistics<CASA_STATP>::setDataProvider(StatsDataProvider<CASA_STATP>* dataProvider) {
  ThrowIf(_calculateAsAdded,
...
CASA_STATD
void ClassicalStatistics<CASA_STATP>::setStatsToCalculate(std::set<StatisticsData::STATS>& stats) {
  const StatisticsDataset<CASA_STATP>& ds = this->_getDataset();
  ThrowIf(_calculateAsAdded && ds.iDataset() > 0,
          "Cannot set stats to be calculated after setting the first dataset "
          "when stats are to be calculated as data are added");
  _doMaxMin = stats.empty() || stats.find(StatisticsData::MAX) != stats.end() ||
...
CASA_STATD
LocationType ClassicalStatistics<CASA_STATP>::getStatisticIndex(StatisticsData::STATS stat) {
  ThrowIf(!(stat == StatisticsData::MAX || stat == StatisticsData::MIN),
  ...
  ThrowIf(!stats.empty() &&
              ((stat == StatisticsData::MAX && stats.find(StatisticsData::MAX) == stats.end()) ||
...
StatsData<AccumType> ClassicalStatistics<CASA_STATP>::_getInitialStats() const {
  static const auto stats = initializeStatsData<AccumType>();
  return stats;
}

CASA_STATD
AccumType ClassicalStatistics<CASA_STATP>::_getStatistic(StatisticsData::STATS stat) {
  switch (stat) {
    ...
    default:
      AccumType value;
      auto r = toRecord(_getStatistics());
      auto statString = StatisticsData::toString(stat);
      ThrowIf(!r.isDefined(statString), "Logic Error: stat " + statString +
                                            " is not defined. "
...
#pragma omp parallel for num_threads(nthreads)
  ...
  auto vstats = StatisticsUtilities<AccumType>::combine(xstats);
...
void ClassicalStatistics<CASA_STATP>::_computeStats(StatsData<AccumType>& stats, uInt64& ngood,
  ...
  if (chunk.weights) {
    ...
    if (chunk.mask) {
      ...
      _weightedStats(stats, location, dataIter, weightsIter, count, chunk.dataStride,
    ...
    // associated with it
    _unweightedStats(stats, ngood, location, dataIter, count, chunk.dataStride, chunk.ranges->first,
                     chunk.ranges->second);
  } else {
    ...
    // necessary.
    _unweightedStats(stats, ngood, location, dataIter, count, chunk.dataStride);
  }
  if (!chunk.weights) {
...
void ClassicalStatistics<CASA_STATP>::_accumNpts(uInt64& npts, const DataIterator& dataBegin,
  ...
  while (count < nr) {
    if (StatisticsUtilities<AccumType>::includeDatum(*datum, beginRange, endRange, isInclude)) {
      ++npts;
    }
...
CASA_STATD
void ClassicalStatistics<CASA_STATP>::_accumulate(StatsData<AccumType>& stats,
                                                  const AccumType& datum,
                                                  const LocationType& location) {
  if (_doMaxMin) {
    StatisticsUtilities<AccumType>::accumulate(stats.npts, stats.sum, stats.mean, stats.nvariance,
...
void ClassicalStatistics<CASA_STATP>::_accumulate(StatsData<AccumType>& stats,
  ...
  if (_doMaxMin) {
    StatisticsUtilities<AccumType>::waccumulate(
        stats.npts, stats.sumweights, stats.sum, stats.mean, stats.nvariance, stats.sumsq,
        *stats.min, *stats.max, stats.minpos, stats.maxpos, datum, weight, location);
  } else {
    StatisticsUtilities<AccumType>::waccumulate(stats.npts, stats.sumweights, stats.sum, stats.mean,
...
#pragma omp parallel for num_threads(nthreads)
  ...
  for (uInt i = 0; i < nThreadsMax; ++i) {
    ...
    if (tmax[idx8] && (!mymax || *tmax[idx8] > *mymax)) {
      mymax = tmax[idx8];
    }
  }
  ThrowIf(!mymax || !mymin, "No valid data found");
  datamin = *mymin;
  datamax = *mymax;
}
...
void ClassicalStatistics<CASA_STATP>::_computeMinMax(std::shared_ptr<AccumType>& mymax,
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _minMax(mymin, mymax, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
                chunk.mask->second, chunk.ranges->first, chunk.ranges->second);
      } else {
        _minMax(mymin, mymax, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
                chunk.mask->second);
      ...
      _minMax(mymin, mymax, dataIter, weightsIter, dataCount, chunk.dataStride, chunk.ranges->first,
              chunk.ranges->second);
    } else {
      // has weights, but no mask nor ranges
      _minMax(mymin, mymax, dataIter, weightsIter, dataCount, chunk.dataStride);
    }
  } else if (chunk.mask) {
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _minMax(mymin, mymax, dataIter, dataCount, chunk.dataStride, maskIter, chunk.mask->second,
              chunk.ranges->first, chunk.ranges->second);
    } else {
      _minMax(mymin, mymax, dataIter, dataCount, chunk.dataStride, maskIter, chunk.mask->second);
    }
    ...
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _minMax(mymin, mymax, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
            chunk.ranges->second);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
...
#pragma omp parallel for num_threads(nthreads)
  ...
  ThrowIf(!mymax || !mymin || myNpts == 0, "No valid data found");
  datamin = *mymin;
  datamax = *mymax;
  return myNpts;
}
...
void ClassicalStatistics<CASA_STATP>::_computeMinMaxNpts(
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _minMaxNpts(npts, mymin, mymax, dataIter, weightsIter, dataCount, chunk.dataStride,
                    maskIter, chunk.mask->second, chunk.ranges->first, chunk.ranges->second);
      } else {
        _minMaxNpts(npts, mymin, mymax, dataIter, weightsIter, dataCount, chunk.dataStride,
    ...
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _minMaxNpts(npts, mymin, mymax, dataIter, dataCount, chunk.dataStride, maskIter,
                  chunk.mask->second, chunk.ranges->first, chunk.ranges->second);
    } else {
      _minMaxNpts(npts, mymin, mymax, dataIter, dataCount, chunk.dataStride, maskIter,
                  chunk.mask->second);
    ...
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _minMaxNpts(npts, mymin, mymax, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
                chunk.ranges->second);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
    // associated with it. No filtering of the data is necessary.
...
void ClassicalStatistics<CASA_STATP>::_computeNpts(uInt64& npts, DataIterator dataIter,
  ...
  if (chunk.weights) {
    if (chunk.mask) {
      if (chunk.ranges) {
        _accumNpts(npts, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
                   chunk.mask->second, chunk.ranges->first, chunk.ranges->second);
      } else {
        _accumNpts(npts, dataIter, weightsIter, dataCount, chunk.dataStride, maskIter,
                   chunk.mask->second);
      ...
      _accumNpts(npts, dataIter, weightsIter, dataCount, chunk.dataStride, chunk.ranges->first,
                 chunk.ranges->second);
    } else {
      // has weights, but no mask nor ranges
      _accumNpts(npts, dataIter, weightsIter, dataCount, chunk.dataStride);
    }
  } else if (chunk.mask) {
    // this data set has no weights, but does have a mask
    if (chunk.ranges) {
      _accumNpts(npts, dataIter, dataCount, chunk.dataStride, maskIter, chunk.mask->second,
                 chunk.ranges->first, chunk.ranges->second);
    } else {
      _accumNpts(npts, dataIter, dataCount, chunk.dataStride, maskIter, chunk.mask->second);
    }
    ...
    // this data set has no weights no mask, but does have a set of ranges
    // associated with it
    _accumNpts(npts, dataIter, dataCount, chunk.dataStride, chunk.ranges->first,
               chunk.ranges->second);
  } else {
    // simplest case, this data set has no weights, no mask, nor any ranges
    // associated with it.
...
void ClassicalStatistics<CASA_STATP>::_updateDataProviderMaxMin(
  ...
  auto& ds = this->_getDataset();
  auto* dataProvider = ds.getDataProvider();
  if (!dataProvider) {
    return;
  }
  // if there is a data provider, and the max and/or min updated, we have to
  // update the data provider after each data set is processed, because the
  // LatticeStatsDataProvider currently requires that.
  auto& stats = _getStatsData();
  const auto same = &threadStats == &stats;
  const auto idataset = ds.iDataset();
  ...
  if (idataset == threadStats.minpos.first && (!stats.min || (*threadStats.min) < (*stats.min))) {
    if (!same) {
      // make a copy, do not assign one pointer to another
