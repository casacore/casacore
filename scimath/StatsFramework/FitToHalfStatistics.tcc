...
CASA_STATD
FitToHalfStatistics<CASA_STATP>::FitToHalfStatistics(FitToHalfStatisticsData::CENTER centerType,
...
CASA_STATD
FitToHalfStatistics<CASA_STATP>::FitToHalfStatistics(const FitToHalfStatistics<CASA_STATP>& other)
...
CASA_STATD
FitToHalfStatistics<CASA_STATP>& FitToHalfStatistics<CASA_STATP>::operator=(
...
AccumType FitToHalfStatistics<CASA_STATP>::getMedianAbsDevMed(std::shared_ptr<uInt64> knownNpts,
  ...
  if (!_getStatsData().medAbsDevMed) {
    ...
    // value
    getMedian();
    _getStatsData().medAbsDevMed.reset(
        new AccumType(ConstrainedRangeStatistics<CASA_STATP>::getMedianAbsDevMed(
            realNPts, knownMin, knownMax, binningThreshholdSizeBytes, persistSortedArray, nBins)));
  }
  return *_getStatsData().medAbsDevMed;
}

CASA_STATD
void FitToHalfStatistics<CASA_STATP>::_getMinMax(std::shared_ptr<AccumType>& realMin,
                                                 std::shared_ptr<AccumType>& realMax,
                                                 std::shared_ptr<AccumType> knownMin,
                                                 std::shared_ptr<AccumType> knownMax) {
...
CASA_STATD
void FitToHalfStatistics<CASA_STATP>::getMinMax(AccumType& mymin, AccumType& mymax) {
...
CASA_STATD
void FitToHalfStatistics<CASA_STATP>::_getRealMinMax(AccumType& realMin, AccumType& realMax) {
           ...
           [this, &actual, &knownNpts, &freal, &realPortionFractions, &actualToReal](Double q) {
             if (near(q, 0.5)) {
               ...
               if (isVirtualQ) {
                 ...
                 auto realIdx =
                     _useLower ? allNPts - (actualIdx + 1) : allNPts / 2 - (actualIdx + 1);
...
CASA_STATD
void FitToHalfStatistics<CASA_STATP>::setStatsToCalculate(std::set<StatisticsData::STATS>& stats) {
...
void FitToHalfStatistics<CASA_STATP>::_setRange() {
  ...
  if (_centerType == FitToHalfStatisticsData::CMEAN ||
...
#define _unweightedStatsCodeFH                                                                   \
  if (*datum >= _range->first && *datum <= _range->second) {                                     \
    StatisticsUtilities<AccumType>::accumulateSym(stats.npts, stats.nvariance, stats.sumsq,      \
                                                  *stats.min, *stats.max, stats.minpos,          \
...
#define _weightedStatsCodeFH                                                                \
  if (*datum >= _range->first && *datum <= _range->second) {                                \
    StatisticsUtilities<AccumType>::waccumulateSym(                                         \
        stats.npts, stats.sumweights, stats.nvariance, stats.sumsq, *stats.min, *stats.max, \
        stats.minpos, stats.maxpos, *datum, *weight, location, _centerValue);               \
  }
