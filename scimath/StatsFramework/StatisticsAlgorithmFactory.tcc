...
CASA_STATD
StatisticsAlgorithmFactory<CASA_STATP>::StatisticsAlgorithmFactory() { configureClassical(); }
...
void StatisticsAlgorithmFactory<CASA_STATP>::configureFitToHalf(
    FitToHalfStatisticsData::CENTER centerType, FitToHalfStatisticsData::USE_DATA useData,
...
void StatisticsAlgorithmFactory<CASA_STATP>::copy(
    StatisticsAlgorithmFactory<AccumType, DataIterator2, MaskIterator2, WeightsIterator2>& other)
    const {
...
StatisticsAlgorithmFactory<CASA_STATP>::createStatsAlgorithm() const {
  switch (_algorithm) {
    case StatisticsData::BIWEIGHT:
      return std::make_shared<BiweightStatistics<CASA_STATP>>(_biweightData.maxIter,
                                                              _biweightData.c);
    ...
    case StatisticsData::FITTOHALF: {
      return std::make_shared<FitToHalfStatistics<CASA_STATP>>(
          _fitToHalfData.center, _fitToHalfData.side, _fitToHalfData.centerValue);
    }
    case StatisticsData::CHAUVENETCRITERION: {
      return std::make_shared<ChauvenetCriterionStatistics<CASA_STATP>>(_chauvData.zScore,
                                                                        _chauvData.maxIter);
    }
    default:
      ThrowCc("Logic Error: Unhandled algorithm " + ValueToString(_algorithm));
  }
}

CASA_STATD StatisticsAlgorithmFactoryData::BiweightData
StatisticsAlgorithmFactory<CASA_STATP>::biweightData() const {
  ThrowIf(_algorithm != StatisticsData::BIWEIGHT,
...
StatisticsAlgorithmFactory<CASA_STATP>::fitToHalfData() const {
  ThrowIf(_algorithm != StatisticsData::FITTOHALF,
...
StatisticsAlgorithmFactory<CASA_STATP>::chauvenetData() const {
  ThrowIf(_algorithm != StatisticsData::CHAUVENETCRITERION,
  ...
  switch (algorithm) {
    ...
    case StatisticsData::FITTOHALF: {
      ...
      if (center == FitToHalfStatisticsData::CVALUE) {
        ThrowIf(!r.isDefined("center_value"), "field 'center_value' is not defined");
    ...
    default:
      ThrowCc("Logic Error: Unhandled algorithm " + ValueToString(algorithm));
  }
}
