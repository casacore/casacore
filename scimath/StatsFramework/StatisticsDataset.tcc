...
CASA_STATD StatisticsDataset<CASA_STATP>& StatisticsDataset<CASA_STATP>::operator=(
...
CASA_STATD void StatisticsDataset<CASA_STATP>::addData(const DataIterator& first, uInt nr,
  ...
  _counts.push_back(nrAccountsForStride    ? nr
                    : nr % dataStride == 0 ? nr / dataStride
                                           : nr / dataStride + 1);
...
CASA_STATD void StatisticsDataset<CASA_STATP>::addData(const DataIterator& first, uInt nr,
  ...
  for_each(dataRanges.cbegin(), dataRanges.cend(),
           [](const std::pair<AccumType, AccumType>& range) {
             ThrowIf(range.first > range.second,
...
void StatisticsDataset<CASA_STATP>::incrementThreadIters(DataIterator& dataIter,
  ...
  if (_chunk.mask) {
    std::advance(maskIter, nthreads * ClassicalStatisticsData::BLOCK_SIZE * _chunk.mask->second);
  }
  offset += increment;
}
...
void StatisticsDataset<CASA_STATP>::initThreadVars(uInt& nBlocks, uInt64& extra, uInt& nthreads,
  ...
  for (uInt tid = 0; tid < nthreads; ++tid) {
    ...
    if (_chunk.mask) {
      ...
      std::advance(maskIter[idx8], tid * ClassicalStatisticsData::BLOCK_SIZE * _chunk.mask->second);
    }
  }
}
