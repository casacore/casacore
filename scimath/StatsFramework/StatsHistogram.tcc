...
uInt StatsHistogram<AccumType>::getIndex(AccumType value) const {
  ...
  while (True) {
    ...
    if (value >= _maxBinLimits[testIdx - 1] && value < _maxBinLimits[testIdx]) {
      // bin found
      return testIdx;
    }
  ...
  os << "Guessed index " << idx << " with limits " << mymin << ", " << _maxBinLimits[idx] << endl;
  ThrowCc(os.str());
}

template <class AccumType>
const std::vector<AccumType>& StatsHistogram<AccumType>::getMaxBinLimits() const {
