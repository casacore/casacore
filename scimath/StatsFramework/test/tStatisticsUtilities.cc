...
int main() {
  try {
    ...
    for (uInt i = 0; i < 5; i++) {
      StatisticsUtilities<Double>::accumulate(npts, sum, mean, v[i]);
    }
    ...
    Double sumsq = 0;
    for (uInt i = 0; i < 5; i++) {
      StatisticsUtilities<Double>::accumulate(npts, sum, mean, nvariance, sumsq, v[i]);
    }
    AlwaysAssert(npts == 5, AipsError);
    AlwaysAssert(sum == 10, AipsError);
    AlwaysAssert(mean == 2, AipsError);
    ...
    uInt maxpos = 0;
    for (uInt i = 0; i < 5; i++) {
      StatisticsUtilities<Double>::accumulate(npts, sum, mean, nvariance, sumsq, datamin, datamax,
                                              minpos, maxpos, v[i], i);
    }
    AlwaysAssert(npts == 5, AipsError);
    AlwaysAssert(sum == 10, AipsError);
    ...
    for (uInt i = 0; i < 5; i++) {
      StatisticsUtilities<Double>::doMax(datamax, maxpos, i == 0, v[i], i);
      StatisticsUtilities<Double>::doMin(datamin, minpos, i == 0, v[i], i);
    }
    AlwaysAssert(datamin == 1, AipsError);
    AlwaysAssert(datamax == 3, AipsError);
    AlwaysAssert(minpos == 1, AipsError);
    ...
    Double wmean = 0;
    for (uInt i = 0; i < 5; i++) {
      StatisticsUtilities<Double>::waccumulate(npts, sumweights, wsum, wmean, v[i], w[i]);
    }
    AlwaysAssert(npts == 5, AipsError);
    ...
    Double wnvariance = 0;
    for (uInt i = 0; i < 5; i++) {
      StatisticsUtilities<Double>::waccumulate(npts, sumweights, wsum, wmean, wnvariance, wsumsq,
                                               v[i], w[i]);
    }
    AlwaysAssert(npts == 5, AipsError);
    AlwaysAssert(sumweights == 9, AipsError);
    ...
    Double center = 3;
    while (viter != vend) {
      StatisticsUtilities<Double>::accumulateSym(npts, nvariance, sumsq, *viter, center);
      ++viter;
    }
    ...
    viter = vbegin;
    while (viter != vend) {
      StatisticsUtilities<Double>::waccumulateSym(npts, sumweights, nvariance, sumsq, *viter,
                                                  *witer, center);
      ++viter;
      ++witer;
    }
    ...
    viter = vbegin;
    while (viter != vend) {
      StatisticsUtilities<Double>::accumulateSym(npts, nvariance, sumsq, datamin, datamax, minpos,
                                                 maxpos, *viter, count, center);
      ++viter;
      ++count;
    }
    AlwaysAssert(npts == 10, AipsError);
    ...
    count = 0;
    while (viter != vend) {
      StatisticsUtilities<Double>::waccumulateSym(npts, sumweights, nvariance, sumsq, datamin,
                                                  datamax, minpos, maxpos, *viter, *witer, count,
                                                  center);
      ++viter;
      ++witer;
      ++count;
    ...
    AlwaysAssert(maxpos == 3, AipsError);
    {
      ...
      Double d[] = {0.6, 2.7, 9.6, 5.1, 8.2, 2.3, 4.5, -5.6, 8.7, -3.2, -0.5, 3.2};
