...
class Compressor {
 public:
  Compressor(int compression_level) : compressor_(libdeflate_alloc_compressor(compression_level)) {
    if (!compressor_)
      throw std::runtime_error("Could not create compressor: bad settings or out of memory");
  }

  Compressor(int compression_level, const libdeflate_options& options)
      : compressor_(libdeflate_alloc_compressor_ex(compression_level, &options)) {
    if (!compressor_)
      throw std::runtime_error("Could not create compressor: bad settings or out of memory");
  }
  ...
  size_t Compress(std::span<const std::byte> input, std::span<std::byte> output) {
    return libdeflate_deflate_compress(compressor_, input.data(), input.size(), output.data(),
                                       output.size());
  }
...
class Decompressor {
 public:
  Decompressor() : decompressor_(libdeflate_alloc_decompressor()) {
    if (!decompressor_)
      throw std::runtime_error("Could not create decompressor: bad settings or out of memory");
  }
  ...
  size_t Decompress(std::span<const std::byte> input, std::span<std::byte> output) {
    ...
    const libdeflate_result result =
        libdeflate_deflate_decompress(decompressor_, input.data(), input.size(), output.data(),
                                      output.size(), &actual_out_nbytes_ret);
    switch (result) {
      ...
      case LIBDEFLATE_SHORT_OUTPUT:
        throw std::runtime_error("Could not decompress data: data shorter than expected");
      case LIBDEFLATE_INSUFFICIENT_SPACE:
        throw std::runtime_error("Could not decompress data: insufficient space in output buffer");
    }
