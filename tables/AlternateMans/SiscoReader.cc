...
void CopyResult(const std::vector<float>& real, const std::vector<float>& imaginary,
...
void SiscoReader::Open(std::span<std::byte> header_data) {
  ...
  if (!file_.good()) throw std::runtime_error("Could not open file " + filename_);
  ...
  if (!file_.good()) throw std::runtime_error("Failed to read header from " + filename_);
  ...
  for (size_t i = 0; i != n_threads; ++i) result_workers_.emplace_back([&]() { ResultLoop(); });
...
void SiscoReader::DecompressChunk() {
  ...
  if (read_lane_.read(chunk)) {
    ...
    const size_t n_values =
        chunk.decompress_buffer.size() / (stored_mantissa_size + kCompressedExponentSize);
...
void SiscoReader::Request(size_t baseline_index, size_t n_values) {
  const size_t stored_mantissa_size = (predict_level_ == -1) ? 3 : kCompressedMantissaSize;
  if (chunk_item_position_ * 2 * (stored_mantissa_size + kCompressedExponentSize) >=
      current_chunk_->decompress_buffer.size()) {
  ...
  const size_t exponents_start = (current_chunk_->decompress_buffer.size() /
                                  (stored_mantissa_size + kCompressedExponentSize)) *
                                 stored_mantissa_size;
...
void SiscoReader::ResultLoop() {
  ...
  while (request_queue_.PopIf(request, lock, condition)) {
    if (request.n_values != 0) {
      ...
      std::span real_mantissas(&request.chunk->read_buffer[request.mantissas_offset],
                               mantissas_size);
      std::span real_exponents(&request.chunk->decompress_buffer[request.exponents_offset],
                               exponents_size);
      Decompress2D(predict_level_, baseline.real_state_, real_mantissas, real_exponents,
                   result.real_data);

      std::span imaginary_mantissas(
          &request.chunk->read_buffer[request.mantissas_offset + mantissas_size], mantissas_size);
      std::span imaginary_exponents(
          &request.chunk->decompress_buffer[request.exponents_offset + exponents_size],
          exponents_size);
      Decompress2D(predict_level_, baseline.imaginary_state_, imaginary_mantissas,
                   imaginary_exponents, result.imaginary_data);
      ...
      // If the nr of decompressed chunks gets low, use this thread to decompress a chunk. This way,
      // threads are balanced between decompression and unpacking. This condition isn't "atomic",
      // i.e. all threads may simultaneously find that decompression is required. In that case, the
      // call will block and threads may become idle for some time; hence there is some balance
      // required between filling the queue, the nr of threads and the size of the lane.
    ...
    // The following needs to happen 'atomically'; otherwise the chunk gets removed and another
    // thread may still access the chunk. This is achieved by doing it while holding the lock.
    ...
    if (request.chunk->n_results_processed == request.chunk->n_results_in_chunk) {
      ...
      [[maybe_unused]] const bool is_removed =
          chunks_.remove_if([chunk](const Chunk& value) { return &value == chunk; });
      assert(is_removed);
    }
  }
}
...
size_t SiscoReader::ReadChunk(std::vector<std::byte>& buffer) {
  ...
  if (!file_.good()) throw std::runtime_error("Could not read from file " + filename_);
