...
SiscoStMan::SiscoStMan(const casacore::String & /*name*/, const casacore::Record &spec)
    : DataManager() {
  ...
  if (spec.isDefined(kDeflateLevelKey)) {
    ...
    if (deflate_level_ <= 0) throw std::runtime_error("Invalid value for " + kDeflateLevelKey);
  }
  if (spec.isDefined(kPredictLevelKey)) {
    predict_level_ = spec.asInt(kPredictLevelKey);
    if (predict_level_ < -1) throw std::runtime_error("Invalid value for " + kPredictLevelKey);
  }
  bool stokes_i = false;
  bool diagonal = false;
...
SiscoStMan::SiscoStMan(const SiscoStMan &source)
    : DataManager(),
      name_(source.name_),
      deflate_level_(source.deflate_level_),
      predict_level_(source.predict_level_),
      store_mode_(source.store_mode_) {}
...
void SiscoStMan::registerClass() { DataManager::registerCtor("SiscoStMan", Make); }

void SiscoStMan::create64(casacore::rownr_t) {}

casacore::rownr_t SiscoStMan::open64(casacore::rownr_t n_row, casacore::AipsIO &) { return n_row; }

casacore::DataManagerColumn *SiscoStMan::makeScalarColumn(const casacore::String & /*name*/,
                                                          int dataType,
                                                          const casacore::String &dataTypeID) {
  std::ostringstream s;
  s << "Can not create scalar columns with SiscoStMan! (requested datatype: '" << dataTypeID
    << "' (" << dataType << ")";
  throw std::runtime_error(s.str());
}

casacore::DataManagerColumn *SiscoStMan::makeDirArrColumn(const casacore::String &name,
                                                          int dataType,
                                                          const casacore::String &dataTypeID) {
...
casacore::rownr_t SiscoStMan::resync64(casacore::rownr_t nRow) { return nRow; }

void SiscoStMan::deleteManager() { unlink(fileName().c_str()); }

void SiscoStMan::prepare() {
  if (column_) column_->Prepare();
}
...
void SiscoStMan::addColumn(casacore::DataManagerColumn *) {
  throw std::runtime_error("Can't add generic columns to SiscoStMan");
}
