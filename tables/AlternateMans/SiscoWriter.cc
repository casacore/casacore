...
void Concatenate(std::vector<std::byte>& destination, const std::vector<std::byte>& mantissas,
                 const std::vector<std::byte>& exponents, size_t n_elements) {
  destination.reserve(n_elements * 2 * (kCompressedMantissaSize + kCompressedExponentSize));
  destination.assign(mantissas.begin(),
                     mantissas.begin() + n_elements * 2 * kCompressedMantissaSize);
  destination.insert(destination.end(), exponents.begin(),
                     exponents.begin() + n_elements * 2 * kCompressedExponentSize);

  // To test exponent compression:
  // destination.assign(exponents.begin(),exponents.begin() + n_elements * 2 *
...
void ConcatenateWithoutPrediction(std::vector<std::byte>& destination,
                                  const std::vector<std::byte>& mantissas,
                                  const std::vector<std::byte>& exponents, size_t n_elements) {
  // In the special case of no prediction, we know the mantissa is limited to 23 bits. Therefore,
  // the 24th bit is used for the sign, and the leading byte is removed. Although this doesn't
  // change the information in the stored data, it has a significant effect on the compression.
  destination.clear();
  ...
  for (size_t i = 0; i != n_values; ++i) {
    ...
    if (mantissas[i * 4 + 3] == std::byte(0)) value_with_sign = value_with_sign & std::byte(0x7F);
    destination.emplace_back(value_with_sign);
  }
  destination.insert(destination.end(), exponents.begin(),
                     exponents.begin() + n_values * kCompressedExponentSize);
}
}  // namespace
...
void SiscoWriter::Open(std::span<const std::byte> header_data) {
  file_ =
      std::ofstream(filename_, std::ios::trunc | std::ios::binary | std::ios::in | std::ios::out);
...
void SiscoWriter::Close() {
  ...
  for (std::thread& thread : compression_threads_) thread.join();
...
void SiscoWriter::Write(size_t baseline_index, std::span<const std::complex<float>> data) {
  ...
  if (chunk_write_position_ + row_size > write_chunk_->allocated_chunk_size) {
    ...
    if (chunk_write_position_ == 0) {
      ...
      write_chunk_->mantisas_data.resize(write_chunk_->allocated_chunk_size * 2 *
                                         kCompressedMantissaSize);
      write_chunk_->exponent_data.resize(write_chunk_->allocated_chunk_size * 2 *
                                         kCompressedExponentSize);
...
void SiscoWriter::NewChunk(std::unique_lock<std::mutex>& lock) {
  while (chunks_.size() >= kMaxChunks) chunk_finished_condition_.wait(lock);
...
void SiscoWriter::Preprocess(SiscoWriter::PreprocessingTask& task,
  ...
  if (task.mantisa_position) {
    ...
    Compress2D(predict_level_, baseline.imaginary_state_, task.imaginary_data, imaginary_mantissa,
               imaginary_exponent);
