...
UvwStMan::UvwStMan(const UvwStMan &source) : DataManager(), name_(source.name_) {}

UvwStMan::~UvwStMan() noexcept = default;

void UvwStMan::create64(rownr_t /*nRow*/) { file_ = UvwFile::CreateNew(fileName()); }
...
DataManagerColumn *UvwStMan::makeDirArrColumn(const String & /*name*/, int dataType,
                                              const String & /*dataTypeID*/) {
  if (dataType == TpDouble) {
    column_ = std::make_unique<UvwStManColumn>(file_);
    return column_.get();
  } else {
    throw std::runtime_error("Trying to create a Uvw column with wrong type");
  }
}
...
void UvwStMan::prepare() {
  if (column_) column_->Prepare(table());
}

void UvwStMan::addRow64(rownr_t) {}

void UvwStMan::removeRow64(rownr_t) {
  throw std::runtime_error("Can't remove rows from a UvwStMan");
}

void UvwStMan::addColumn(DataManagerColumn *) {
  throw std::runtime_error("Can't add generic columns to UvwStMan");
}
