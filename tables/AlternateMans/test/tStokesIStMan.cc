...
BOOST_AUTO_TEST_CASE(expand_from_stokes_i) {
  ...
  std::complex<float> test_data_b[12] = {
      {-3.7, 2.0}, {5.2, 0.0}, {std::numeric_limits<float>::quiet_NaN(), -2.1}, u, u, u, u, u, u, u,
      u,           u};
...
BOOST_AUTO_TEST_CASE(transform_to_stokes_i) {
  ...
  const bool test_data_a[data_a_size] = {false, false, false, false, true,  true,
                                         true,  true,  false, false, false, false};
  MorphingArray buffer_a;
  buffer_a.Resize<bool>(data_a_size);
  const bool* result_a =
      casacore::TransformToStokesI(test_data_a, buffer_a.Data<bool>(), data_a_size / 4);
  BOOST_CHECK(!result_a[0]);
  BOOST_CHECK(result_a[1]);
  BOOST_CHECK(!result_a[2]);

  constexpr size_t data_b_size = 16;
  const double test_data_b[data_b_size] = {3.0,
                                           0.0,
                                           0.0,
                                           3.0,
                                           20.0,
  ...
  const double* result_b =
      casacore::TransformToStokesI(test_data_b, buffer_b.Data<double>(), data_b_size / 4);
