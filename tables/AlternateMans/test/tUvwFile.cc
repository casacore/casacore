...
const std::array<std::size_t, 2> kBaselines[kNBaselines] = {{0, 0}, {0, 1}, {0, 2},
                                                            {1, 1}, {1, 2}, {2, 2}};
const std::array<std::size_t, 2> kBaselinesSwapped[kNBaselines] = {{0, 0}, {1, 0}, {2, 0},
                                                                   {1, 1}, {2, 1}, {2, 2}};
...
const std::array<std::size_t, 2> kBaselinesMissingAntenna[kNBaselines] = {{1, 1}, {1, 3}, {1, 4},
                                                                          {3, 3}, {3, 4}, {4, 4}};
}  // namespace
...
void WriteTwoTimesteps(UvwFile& file, const std::array<std::size_t, 2>* baselines,
                       bool with_autos) {
  uint64_t row = 0;
...
void ReadTwoTimesteps(UvwFile& file, const std::array<std::size_t, 2>* baselines, bool with_autos) {
  ...
  for (uint64_t i = 0; i != kNBaselines; ++i) {
    if (with_autos || baselines[i][0] != baselines[i][1]) {
      ...
      expected_uvw[1] *= 3.0;
      expected_uvw[2] *= 4.0;
      for (size_t z = 0; z != 3; ++z) BOOST_CHECK_CLOSE_FRACTION(expected_uvw[z], uvw[z], 1e-6);
      ++row;
    }
  }
  row = 0;
...
BOOST_AUTO_TEST_CASE(invalid_baselines) {
  ...
  BOOST_CHECK_THROW(file.WriteUvw(1, 2, 3, kUvwPerBaseline[1].data()), std::runtime_error);
  file.Close();
  unlink(kFilename.c_str());
}
...
BOOST_AUTO_TEST_CASE(same_positions) {
  ...
  for (uint64_t i = 0; i != kNBaselines; ++i) {
    ...
    for (size_t z = 0; z != 3; ++z) BOOST_CHECK_LT(std::fabs(uvw[z]), 1e-6);
  }
  file.Close();
  unlink(kFilename.c_str());
}
