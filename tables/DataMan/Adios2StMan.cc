...
Adios2StMan::Adios2StMan(MPI_Comm mpiComm, std::string engineType,
    ...
    : Adios2StMan(mpiComm, move(engineType), move(engineParams), move(transportParams),
                  move(operatorParams), {}) {}
...
Adios2StMan::Adios2StMan(MPI_Comm mpiComm, std::string engineType,
    ...
    : DataManager(),
      pimpl(std::unique_ptr<impl>(new impl(*this, &mpiComm, move(engineType), move(engineParams),
                                           move(transportParams), move(operatorParams),
                                           move(configFile)))) {}
#endif

Adios2StMan::Adios2StMan(std::string engineType, std::map<std::string, std::string> engineParams,
                         std::vector<std::map<std::string, std::string>> transportParams,
                         std::vector<std::map<std::string, std::string>> operatorParams)
...
Adios2StMan::Adios2StMan(std::string engineType, std::map<std::string, std::string> engineParams,
                         ...
                         std::string configFile)
    : DataManager(),
      pimpl(std::unique_ptr<impl>(new impl(*this, nullptr, move(engineType), move(engineParams),
                                           move(transportParams), move(operatorParams),
                                           move(configFile)))) {}

Adios2StMan::~Adios2StMan() = default;

DataManager *Adios2StMan::clone() const { return pimpl->clone(); }

String Adios2StMan::dataManagerType() const { return pimpl->dataManagerType(); }

String Adios2StMan::dataManagerName() const { return pimpl->dataManagerName(); }

void Adios2StMan::setDataManagerName(const String &aName) { pimpl->setDataManagerName(aName); }

void Adios2StMan::create64(rownr_t aNrRows) { pimpl->create64(aNrRows); }

rownr_t Adios2StMan::open64(rownr_t aRowNr, AipsIO &ios) { return pimpl->open64(aRowNr, ios); }
...
DataManagerColumn *Adios2StMan::makeDirArrColumn(const String &aName, int aDataType,
                                                 const String &aDataTypeID) {
  return pimpl->makeDirArrColumn(aName, aDataType, aDataTypeID);
}

DataManagerColumn *Adios2StMan::makeIndArrColumn(const String &aName, int aDataType,
                                                 const String &aDataTypeID) {
  return pimpl->makeIndArrColumn(aName, aDataType, aDataTypeID);
}

void Adios2StMan::deleteManager() { pimpl->deleteManager(); }

void Adios2StMan::addRow64(rownr_t aNrRows) { return pimpl->addRow64(aNrRows); }

DataManager *Adios2StMan::makeObject(const String &aDataManType, const Record &spec) {
  return impl::makeObject(aDataManType, spec);
}

Record Adios2StMan::dataManagerSpec() const { return pimpl->dataManagerSpec(); }

rownr_t Adios2StMan::getNrRows() { return pimpl->getNrRows(); }
  ...
  if (mpi_finalized) {
    ...
    if (!mpi_initialized) {
      ...
      if (provided != MPI_THREAD_MULTIPLE) {
        throw(
            std::runtime_error("Casacore is built with thread and MPI enabled, \
...
Adios2StMan::impl::impl(Adios2StMan &parent, void *mpiComm, std::string engineType,
                        std::map<std::string, std::string> engineParams,
                        std::vector<std::map<std::string, std::string>> transportParams,
                        std::vector<std::map<std::string, std::string>> operatorParams,
                        std::string configFile)
...
DataManager *Adios2StMan::impl::makeObject(const String &aDataManName, const Record &spec) {
  std::string configFile;
  std::string engine;
  ...
  Adios2StMan *dtman = new Adios2StMan(
      ...
      itsMpiComm,
#endif
      engine, engine_params, transport_params, operator_params, configFile);
...
DataManager *Adios2StMan::impl::clone() const {
  auto *stman = new Adios2StMan(
      ...
      itsAdiosOperatorParamsVec, itsAdiosConfigFile);
  stman->setDataManagerName(itsDataManName);
  return stman;
}

String Adios2StMan::impl::dataManagerType() const { return DATA_MANAGER_TYPE; }

void Adios2StMan::impl::addRow64(rownr_t aNrRows) { itsRows += aNrRows; }
...
DataManagerColumn *Adios2StMan::impl::makeScalarColumn(const String &name, int aDataType,
                                                       const String &dataTypeId) {
  return makeColumnCommon(name, aDataType, dataTypeId);
}

DataManagerColumn *Adios2StMan::impl::makeDirArrColumn(const String &name, int aDataType,
                                                       const String &dataTypeId) {
  return makeColumnCommon(name, aDataType, dataTypeId);
}

DataManagerColumn *Adios2StMan::impl::makeIndArrColumn(const String &name, int aDataType,
                                                       const String &dataTypeId) {
  return makeColumnCommon(name, aDataType, dataTypeId);
}

DataManagerColumn *Adios2StMan::impl::makeColumnCommon(const String &name, int aDataType,
                                                       const String & /*dataTypeId*/) {
...
void register_adios2stman() { DataManager::registerCtor("Adios2StMan", Adios2StMan::makeObject); }
}  // namespace casacore
