...
Adios2StManColumn::Adios2StManColumn(Adios2StMan::impl *aParent, int aDataType, String aColName,
...
void Adios2StManColumn::scalarColumnCellsVToSelection(const RefRows &rows) {
  ...
  if (!iter.pastEnd()) {
    throw std::runtime_error(
        "Adios2StManColumn::scalarColumnCellsVToSelection supports single slices");
  }
...
void Adios2StManColumn::columnSliceCellsVToSelection(const RefRows &rows, const Slicer &ns) {
  ...
  if (!iter.pastEnd()) {
    throw std::runtime_error(
        "Adios2StManColumn::columnSliceCellsVToSelection supports single slices");
  }
  columnSliceCellsVToSelection(row_start, row_end - row_start + 1, ns);
}

void Adios2StManColumn::columnSliceCellsVToSelection(rownr_t row_start, rownr_t row_count,
                                                     const Slicer &ns) {
  itsAdiosStart[0] = row_start;
...
Adios2StManColumn::ArrayColumnCellsVIter::ArrayColumnCellsVIter(Adios2StManColumn &column,
                                                                const RefRows &rownrs)
    : itsColumn(column), itsRowsSliceIter(rownrs) {
...
void Adios2StManColumn::getColumnSliceCellsV(const RefRows &rownrs, const Slicer &slicer,
                                             ArrayBase &data) {
  columnSliceCellsVToSelection(rownrs, slicer);
  fromAdios(&data);
}

void Adios2StManColumn::putColumnSliceCellsV(const RefRows &rownrs, const Slicer &slicer,
...
template <>
void Adios2StManColumnT<std::string>::create(std::shared_ptr<adios2::Engine> aAdiosEngine,
                                             char /*aOpenMode*/) {
...
void Adios2StManColumnString::getSliceV(rownr_t /*aRowNr*/, const Slicer & /*ns*/,
                                        ArrayBase & /*data*/) {
  throw std::runtime_error("Not implemented yet");
}

void Adios2StManColumnString::putSliceV(rownr_t /*aRowNr*/, const Slicer & /*ns*/,
                                        const ArrayBase & /*data*/) {
  throw std::runtime_error("Not implemented yet");
}
...
void Adios2StManColumnString::getColumnSliceCellsV(const RefRows & /*rownrs*/,
                                                   const Slicer & /*slicer*/,
                                                   ArrayBase & /*data*/) {
  throw std::runtime_error("Not implemented yet");
}

void Adios2StManColumnString::putColumnSliceCellsV(const RefRows & /*rownrs*/,
                                                   const Slicer & /*slicer*/,
                                                   const ArrayBase & /*data*/) {
  throw std::runtime_error("Not implemented yet");
}
