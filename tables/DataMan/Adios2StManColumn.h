...
class Adios2StManColumn : public StManColumnBase {
 public:
  Adios2StManColumn(Adios2StMan::impl *aParent, int aDataType, String aColName,
...
class Adios2StManColumnT : public Adios2StManColumn {
 public:
  ...
  void create(std::shared_ptr<adios2::Engine> aAdiosEngine, char aOpenMode) {
    ...
    if (!itsAdiosVariable && aOpenMode == 'w') {
      itsAdiosVariable =
          itsAdiosIO->DefineVariable<T>(itsColumnName, itsAdiosShape, itsAdiosStart, itsAdiosCount);
    }
  }
 ...
 private:
  ...
  void toAdios(const void *data, std::size_t offset) {
    ...
    if (!isShapeFixed) itsAdiosVariable.SetShape(itsAdiosShape);
...
class Adios2StManColumnString : public Adios2StManColumnT<std::string> {
 ...
 protected:
  ...
  void getColumnSliceCellsV(const RefRows & /*rownrs*/, const Slicer & /*slicer*/,
                            ArrayBase & /*data*/);
  void putColumnSliceCellsV(const RefRows & /*rownrs*/, const Slicer & /*slicer*/,
