...
DataManager* CompressComplex::clone() const { return new CompressComplex(*this); }

// # Return the type name of the engine (i.e. its class name).
String CompressComplex::dataManagerType() const { return className(); }
// # Return the class name.
// # Get the data type names using class ValType.
String CompressComplex::className() { return "CompressComplex"; }

String CompressComplex::dataManagerName() const { return virtualName(); }
...
void CompressComplex::getSlice(rownr_t rownr, const Slicer& slicer, Array<Complex>& array) {
  if (!array.shape().isEqual(buffer_p.shape())) {
    buffer_p.resize(array.shape());
  }
...
void CompressComplex::putSlice(rownr_t rownr, const Slicer& slicer, const Array<Complex>& array) {
...
void CompressComplex::getArrayColumnCells(const RefRows& rownrs, Array<Complex>& array) {
...
void CompressComplex::putArrayColumnCells(const RefRows& rownrs, const Array<Complex>& array) {
...
void CompressComplex::getColumnSliceCells(const RefRows& rownrs, const Slicer& slicer,
                                          Array<Complex>& array) {
  ArrayIterator<Complex> arrIter(array, array.ndim() - 1);
  RefRowsSliceIter rowsIter(rownrs);
  while (!rowsIter.pastEnd()) {
...
void CompressComplex::putColumnSliceCells(const RefRows& rownrs, const Slicer& slicer,
...
CompressComplexSD::CompressComplexSD(const Record& spec) : CompressComplex(spec) {
  if (spec.isDefined("SOURCENAME") && spec.isDefined("TARGETNAME")) {
    setNames(spec.asString("SOURCENAME"), spec.asString("TARGETNAME"));
    if (spec.isDefined("SCALE") && spec.isDefined("OFFSET")) {
      spec.get("SCALE", scale_p);
...
CompressComplexSD::CompressComplexSD(const CompressComplexSD& that) : CompressComplex(that) {}

CompressComplexSD::~CompressComplexSD() {}

// # Clone the engine object.
DataManager* CompressComplexSD::clone() const { return new CompressComplexSD(*this); }

// # Return the type name of the engine (i.e. its class name).
String CompressComplexSD::dataManagerType() const { return className(); }
// # Return the class name.
// # Get the data type names using class ValType.
String CompressComplexSD::className() { return "CompressComplexSD"; }

DataManager* CompressComplexSD::makeObject(const String&, const Record& spec) {
  return new CompressComplexSD(spec);
}
void CompressComplexSD::registerClass() { DataManager::registerCtor(className(), makeObject); }
