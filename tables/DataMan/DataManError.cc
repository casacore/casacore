...
DataManError::DataManError() : AipsError("Table DataManager error") { ; }
DataManError::DataManError(const String& str) : AipsError("Table DataManager error: " + str) { ; }
...
DataManUnknownCtor::DataManUnknownCtor(const String& msg) : DataManError(msg) {}
DataManUnknownCtor::~DataManUnknownCtor() noexcept {}

DataManInvDT::DataManInvDT() : DataManError("Invalid data type") { ; }
...
DataManInvOper::DataManInvOper() : DataManError("Invalid operation") { ; }
DataManInvOper::DataManInvOper(const String& s) : DataManError("Invalid operation: " + s) { ; }
DataManInvOper::~DataManInvOper() noexcept { ; }

DataManUnknownVirtualColumn::DataManUnknownVirtualColumn(const String& columnName,
                                                         const String& engineName)
    : DataManError("column " + columnName + " is unknown to virtual column engine " + engineName) {
  ;
}
DataManUnknownVirtualColumn::~DataManUnknownVirtualColumn() noexcept { ; }

TSMError::TSMError(const String& s) : DataManError("TiledStMan: " + s) { ; }
