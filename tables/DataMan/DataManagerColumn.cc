...
void DataManagerColumn::setShapeTiled(rownr_t rownr, const IPosition& shape, const IPosition&) {
...
void DataManagerColumn::putDComplex(rownr_t, const DComplex*) { throwPut(); }
void DataManagerColumn::putString(rownr_t, const String*) { throwPut(); }

void DataManagerColumn::getOther(rownr_t, void*) {
  throw(
      DataManInvOper("DataManagerColumn::getOther not allowed"
                     " in column " +
                     columnName()));
}
void DataManagerColumn::putOther(rownr_t, const void*) {
  throw(
      DataManInvOper("DataManagerColumn::putOther not allowed"
                     " in column " +
                     columnName()));
}
...
template <typename T>
void DataManagerColumn::PutCells(const RefRows& rownrs, const ArrayBase& arr) {
  const Vector<T>& vec = static_cast<const Vector<T>&>(arr);
  RefRowsSliceIter iter(rownrs);
  rownr_t i = 0;
  while (!iter.pastEnd()) {
...
void DataManagerColumn::getScalarColumnV(ArrayBase& arr) { getScalarColumnBase(arr); }
void DataManagerColumn::putScalarColumnV(const ArrayBase& arr) { putScalarColumnBase(arr); }
void DataManagerColumn::getScalarColumnCellsV(const RefRows& rows, ArrayBase& arr) {
  getScalarColumnCellsBase(rows, arr);
}
void DataManagerColumn::putScalarColumnCellsV(const RefRows& rows, const ArrayBase& arr) {
  putScalarColumnCellsBase(rows, arr);
}
void DataManagerColumn::getArrayV(rownr_t, ArrayBase&) {
  throw DataManError(
      "getArrayV not implemented"
      " for column " +
      columnName());
}
void DataManagerColumn::putArrayV(rownr_t, const ArrayBase&) {
  throw DataManError(
      "putArrayV not implemented"
      " for column " +
      columnName());
}
void DataManagerColumn::getArrayColumnV(ArrayBase& arr) { getArrayColumnBase(arr); }
void DataManagerColumn::putArrayColumnV(const ArrayBase& arr) { putArrayColumnBase(arr); }
void DataManagerColumn::getArrayColumnCellsV(const RefRows& rows, ArrayBase& arr) {
  getArrayColumnCellsBase(rows, arr);
}
void DataManagerColumn::putArrayColumnCellsV(const RefRows& rows, const ArrayBase& arr) {
...
void DataManagerColumn::putSliceV(rownr_t rownr, const Slicer& slicer, const ArrayBase& arr) {
...
void DataManagerColumn::getColumnSliceCellsV(const RefRows& rows, const Slicer& slicer,
                                             ArrayBase& arr) {
  getColumnSliceCellsBase(rows, slicer, arr);
}
void DataManagerColumn::putColumnSliceCellsV(const RefRows& rows, const Slicer& slicer,
                                             const ArrayBase& arr) {
...
void DataManagerColumn::getScalarColumnBase(ArrayBase& arr) {
  switch (dataType()) {
    ...
    default:
      throw(
          DataManInvOper("DataManagerColumn::getScalarColumnV not allowed"
                         " for column " +
                         columnName()));
  }
}
...
void DataManagerColumn::putScalarColumnBase(const ArrayBase& arr) {
  switch (dataType()) {
    ...
    default:
      throw(
          DataManInvOper("DataManagerColumn::putScalarColumnV not allowed"
                         " in column " +
                         columnName()));
  }
}

void DataManagerColumn::getScalarColumnCellsBase(const RefRows& rownrs, ArrayBase& arr) {
  switch (dataType()) {
    case TpBool:
    ...
    default:
      throw(
          DataManInvOper("DataManagerColumn::getScalarColumnCellsV not allowed"
                         " in column " +
                         columnName()));
  }
}

void DataManagerColumn::putScalarColumnCellsBase(const RefRows& rownrs, const ArrayBase& arr) {
  switch (dataType()) {
    case TpBool:
    ...
    default:
      throw(
          DataManInvOper("DataManagerColumn::putScalarColumnCellsV not allowed"
                         " in column " +
                         columnName()));
  }
}
...
void DataManagerColumn::putArrayColumnCellsBase(const RefRows& rows, const ArrayBase& arr) {
  std::unique_ptr<ArrayPositionIterator> iter = arr.makeIterator(arr.ndim() - 1);
  RefRowsSliceIter rowsIter(rows);
  while (!rowsIter.pastEnd()) {
    for (rownr_t row = rowsIter.sliceStart(); row <= rowsIter.sliceEnd();
...
void DataManagerColumn::getSliceArr(rownr_t row, const Slicer& section,
                                    std::shared_ptr<ArrayBase>& fullArr, ArrayBase& arr) {
...
void DataManagerColumn::putSliceArr(rownr_t row, const Slicer& section,
                                    std::shared_ptr<ArrayBase>& fullArr, const ArrayBase& arr) {
  IPosition shp = shape(row);
  if (shp.isEqual(arr.shape())) {
    putArrayV(row, arr);
  } else {
...
void DataManagerColumn::getSliceBase(rownr_t row, const Slicer& section, ArrayBase& arr) {
  std::shared_ptr<ArrayBase> fullArr(arr.makeArray());
  getSliceArr(row, section, fullArr, arr);
}
void DataManagerColumn::putSliceBase(rownr_t row, const Slicer& section, const ArrayBase& arr) {
  std::shared_ptr<ArrayBase> fullArr(arr.makeArray());
...
void DataManagerColumn::putColumnSliceBase(const Slicer& section, const ArrayBase& arr) {
  const IPosition& shp = arr.shape();
  rownr_t nr = shp[shp.size() - 1];
  std::shared_ptr<ArrayBase> fullArr(arr.makeArray());
  std::unique_ptr<ArrayPositionIterator> iter = arr.makeIterator(shp.size() - 1);
...
void DataManagerColumn::getColumnSliceCellsBase(const RefRows& rows, const Slicer& section,
                                                ArrayBase& arr) {
  std::shared_ptr<ArrayBase> fullArr(arr.makeArray());
  std::unique_ptr<ArrayPositionIterator> iter = arr.makeIterator(arr.ndim() - 1);
  RefRowsSliceIter rowsIter(rows);
...
void DataManagerColumn::putColumnSliceCellsBase(const RefRows& rows, const Slicer& section,
                                                const ArrayBase& arr) {
  std::shared_ptr<ArrayBase> fullArr(arr.makeArray());
  std::unique_ptr<ArrayPositionIterator> iter = arr.makeIterator(arr.ndim() - 1);
  RefRowsSliceIter rowsIter(rows);
