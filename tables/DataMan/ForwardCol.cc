...
ForwardColumnEngine::ForwardColumnEngine(const String& dataManagerName, const Record& spec)
    : refColumns_p(0), dataManName_p(dataManagerName) {
...
ForwardColumnEngine::ForwardColumnEngine(const Table& referencedTable,
    ...
    : refColumns_p(0), refTable_p(referencedTable), dataManName_p(dataManagerName) {}
...
SetupNewTable ForwardColumn::setupNewTable(const Table& table, const String& tableName,
...
void ForwardColumnEngine::addColumn(DataManagerColumn* colp) {
  ...
  throw DataManInternalError("ForwardColumnEngine::addColumn on column " + colp->columnName());
}
...
void ForwardColumnEngine::removeColumn(DataManagerColumn* colp) {
  ...
  throw DataManInternalError("ForwardColumnEngine::removeColumn on column " + colp->columnName());
}
...
DataManagerColumn* ForwardColumnEngine::makeScalarColumn(const String& name, int dataType,
                                                         const String& dataTypeId) {
  ForwardColumn* colp = new ForwardColumn(this, name, dataType, dataTypeId, refTable_p);
  addForwardColumn(colp);
  return colp;
}

DataManagerColumn* ForwardColumnEngine::makeIndArrColumn(const String& name, int dataType,
                                                         const String& dataTypeId) {
  return makeScalarColumn(name, dataType, dataTypeId);
}

void ForwardColumnEngine::create64(rownr_t) { baseCreate(); }
...
void ForwardColumnEngine::prepare() { basePrepare(); }
...
DataManager* ForwardColumnEngine::makeObject(const String& dataManagerName, const Record& spec) {
  DataManager* dmPtr = new ForwardColumnEngine(dataManagerName, spec);
  return dmPtr;
}
void ForwardColumnEngine::registerClass() { DataManager::registerCtor(className(), makeObject); }
String ForwardColumnEngine::dataManagerName() const { return dataManName_p; }
String ForwardColumnEngine::dataManagerType() const { return className(); }
String ForwardColumnEngine::className() { return "ForwardColumnEngine"; }
...
ForwardColumn::ForwardColumn(ForwardColumnEngine* enginePtr, const String& name, int dataType,
...
int ForwardColumn::dataType() const { return dataType_p; }
String ForwardColumn::dataTypeId() const { return dataTypeId_p; }

void ForwardColumn::fillTableName(const Table& thisTable, const Table& refTable) {
...
void ForwardColumn::basePrepare(const Table& thisTable, Bool writable) {
  ...
  String name = thisCol.keywordSet().asString("_ForwardColumn_TableName" + enginePtr_p->suffix());
  ...
  if (origCol.columnDesc() != thisCol.columnDesc()) {
    throw(DataManInvOper("ForwardColumn::prepare: ColumnDesc of " + colName_p +
                         " mismatches original"));
  }
...
void ForwardColumn::setShapeColumn(const IPosition& shape) {
  if (!refCol_p.isNull()) {
    if (shape != refCol_p.shapeColumn()) {
      throw(DataManInvOper("ForwardColumn::setShapeColumn: shape of column " + colName_p +
                           " mismatches forwarded column"));
    }
  }
}
...
void ForwardColumn::getScalarColumnV(ArrayBase& dataPtr) { colPtr_p->getScalarColumn(dataPtr); }

void ForwardColumn::getArrayColumnV(ArrayBase& dataPtr) { colPtr_p->getArrayColumn(dataPtr); }
...
void ForwardColumn::getColumnSliceCellsV(const RefRows& rownrs, const Slicer& ns,
                                         ArrayBase& dataPtr) {
...
void ForwardColumn::putSliceV(rownr_t rownr, const Slicer& ns, const ArrayBase& dataPtr) {
...
void ForwardColumn::putArrayColumnV(const ArrayBase& dataPtr) { colPtr_p->putArrayColumn(dataPtr); }

void ForwardColumn::putScalarColumnCellsV(const RefRows& rownrs, const ArrayBase& dataPtr) {
  colPtr_p->putScalarColumnCells(rownrs, dataPtr);
}

void ForwardColumn::putArrayColumnCellsV(const RefRows& rownrs, const ArrayBase& dataPtr) {
...
void ForwardColumn::putColumnSliceCellsV(const RefRows& rownrs, const Slicer& ns,
...
void ForwardColumn::getdouble(rownr_t rownr, double* dataPtr) { colPtr_p->get(rownr, dataPtr); }
void ForwardColumn::putdouble(rownr_t rownr, const double* dataPtr) {
  colPtr_p->put(rownr, dataPtr);
}

void ForwardColumn::getComplex(rownr_t rownr, Complex* dataPtr) { colPtr_p->get(rownr, dataPtr); }
void ForwardColumn::putComplex(rownr_t rownr, const Complex* dataPtr) {
  colPtr_p->put(rownr, dataPtr);
}

void ForwardColumn::getDComplex(rownr_t rownr, DComplex* dataPtr) { colPtr_p->get(rownr, dataPtr); }
void ForwardColumn::putDComplex(rownr_t rownr, const DComplex* dataPtr) {
  colPtr_p->put(rownr, dataPtr);
}

void ForwardColumn::getString(rownr_t rownr, String* dataPtr) { colPtr_p->get(rownr, dataPtr); }
void ForwardColumn::putString(rownr_t rownr, const String* dataPtr) {
  colPtr_p->put(rownr, dataPtr);
}

void ForwardColumn::getOther(rownr_t rownr, void* dataPtr) { colPtr_p->get(rownr, dataPtr); }
void ForwardColumn::putOther(rownr_t rownr, const void* dataPtr) { colPtr_p->put(rownr, dataPtr); }
