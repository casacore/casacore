...
DataManager* ISMBase::clone() const { return new ISMBase(*this); }

String ISMBase::dataManagerType() const { return "IncrementalStMan"; }
String ISMBase::dataManagerName() const { return dataManName_p; }
...
DataManagerColumn* ISMBase::makeScalarColumn(const String&, int dataType, const String&) {
...
DataManagerColumn* ISMBase::makeDirArrColumn(const String& name, int dataType,
                                             const String& dataTypeId) {
  return makeScalarColumn(name, dataType, dataTypeId);
}
DataManagerColumn* ISMBase::makeIndArrColumn(const String&, int dataType, const String&) {
  // # Extend colSet_p block if needed.
  if (ncolumn() >= colSet_p.nelements()) {
    colSet_p.resize(colSet_p.nelements() + 32);
  }
...
void ISMBase::makeCache() {
  if (cache_p == 0) {
    ...
    cache_p = new BucketCache(file_p, 512, bucketSize_p, nbucketInit_p, cacheSize_p, this,
                              ISMBucket::readCallBack, ISMBucket::writeCallBack,
...
void ISMBase::init() {
  ...
  if (checkBucketSize_p && bucketSize_p > 0) {
    ...
    if (bucketSize_p < headerSize + 2 * fixedSize) {
      throw(
          DataManError("IncrementalStMan: bucket too small "
                       "to hold 2 rows"));
    } else if (bucketSize_p < 32768) {
      if (bucketSize_p < headerSize + 10 * fixedSize) {
        throw(
            DataManError("IncrementalStMan: bucket < 32768 and "
  ...
  if (bucketSize_p >= 1 << 28) {
    throw DataManError(
        "IncrementalStMan: bucketSize exceeds 28 bits"
        " (>= 268435456)");
  }
}
