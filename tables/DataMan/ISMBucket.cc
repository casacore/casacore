...
void ISMBucket::replaceData(uInt& offset, const char* data, uInt newLeng, uInt oldLeng) {
  ...
  cout << "  replace at offset " << offset << ": oldleng=" << oldLeng << ", new=" << newLeng
       << endl;
#endif
  AlwaysAssert(dataLeng_p + newLeng - oldLeng + indexLeng_p <= stmanPtr_p->bucketSize(), AipsError);
  if (oldLeng == newLeng) {
...
void ISMBucket::write(char* bucketStorage) const {
  ...
  for (uInt i = 0; i < nrcol; i++) {
    ...
    if (use32) {
      ...
      offset += writeRownr(bucketStorage + offset, rowIndex_p[i]->storage(), nr);
    }
    offset += writeuInt(bucketStorage + offset, offIndex_p[i]->storage(), nr);
  }
...
void ISMBucket::read(const char* bucketStorage) {
  ...
  for (uInt i = 0; i < nrcol; i++) {
    ...
    if (use32) {
      ...
      offset += readRownr(rowIndex_p[i]->storage(), bucketStorage + offset, nr);
    }
    offset += readuInt(offIndex_p[i]->storage(), bucketStorage + offset, nr);
    indexLeng_p += nr * (uIntSize_p + rownrSize_p);
  }
}
