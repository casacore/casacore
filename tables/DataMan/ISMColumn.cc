...
void ISMColumn::remove(rownr_t bucketRownr, ISMBucket* bucket, rownr_t bucketNrrow,
  ...
  inx = bucket->getInterval(colnr_p, bucketRownr, bucketNrrow, stint, endint, offset);
#ifdef AIPS_TRACE
  cout << "remove column " << colnr_p << ", row " << bucketRownr << " (nrelem=" << nrelem_p
       << ", leng=" << fixedLength_p << ")" << endl;
  cout << "  Bucket start row " << bucketRownr << " (" << bucketNrrow << " rows)" << endl;
  cout << "  Interval " << stint << "-" << endint << " (row=" << bucketRownr
       << ", offset=" << offset << ")" << endl;
#endif
...
void ISMColumn::putDComplex(rownr_t rownr, const DComplex* value) { putValue(rownr, value); }
void ISMColumn::putString(rownr_t rownr, const String* value) { putValue(rownr, value); }
...
void ISMColumn::putValue(rownr_t rownr, const void* value) {
...
#ifdef AIPS_TRACE
  cout << "put column " << colnr_p << ", row " << rownr << " (nrelem=" << nrelem_p
       << ", leng=" << fixedLength_p << ")" << endl;
  ...
  if (afterLastRowPut) {
    ...
    if (bucketRownr == 0) {
      replaceData(bucket, bucketStartRow, bucketNrrow, bucketRownr, offIndex[inx], buffer, lenData);
    } else {
      if (equalPrev) {
#ifdef AIPS_TRACE
        cout << " equal prev";
  ...
  if (bucketRownr > stint && bucketRownr < endint) {
    ...
    addData(bucket, bucketStartRow, bucketNrrow, bucketRownr + 1, inx, buffer, lenData);
    handleCopy(rownr, buffer);
    putValue(rownr, value);
  } else {
    lenData = writeFunc_p(buffer, value, nrcopy_p);
    addData(bucket, bucketStartRow, bucketNrrow, bucketRownr, inx, buffer, lenData);
  }
}
...
void ISMColumn::putData(ISMBucket* bucket, rownr_t bucketStartRow, rownr_t bucketNrrow,
  ...
  if ((afterLastRow && bucketRownr == 0) || start == end) {
    ...
    addData(bucket, bucketStartRow, bucketNrrow, bucketRownr, inx, data, lenData, afterLastRow,
            canSplit);
  }
}
...
void ISMColumn::handleCopy(rownr_t rownr, const char*) {
  cout << "   handleCopy for row " << rownr << ", column " << colnr_p << endl;
#else
...
void ISMColumn::handleRemove(rownr_t rownr, const char*) {
  cout << "   handleRemove for row " << rownr << ", column " << colnr_p << endl;
#else
