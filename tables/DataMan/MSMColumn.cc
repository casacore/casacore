...
uInt MSMColumn::findExt(rownr_t index, Bool setCache) {
  ...
  if (i > Int(nrext_p)) {
    throw(indexError<rownr_t>(index,
                              "MSMColumn::findExt - "
                              "rownr " +
                                  String(std::to_string(index)) + " in column " + columnName() +
                                  " out of range"));
  }
  if (setCache) {
...
void MSMColumn::removeData(void* dp, rownr_t inx, rownr_t nrvalAfter) {
  ...
  if (byPtr_p) {
    objmove(static_cast<void**>(dp) + inx, static_cast<void**>(dp) + inx + 1, nrvalAfter - inx);
  } else if (dtype() == TpString) {
    objmove(static_cast<String*>(dp) + inx, static_cast<String*>(dp) + inx + 1, nrvalAfter - inx);
  } else {
    memmove(static_cast<char*>(dp) + inx * elemSize(),
            static_cast<char*>(dp) + (inx + 1) * elemSize(), (nrvalAfter - inx) * elemSize());
  }
}
