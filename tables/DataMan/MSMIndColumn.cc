...
void MSMIndColumn::setShapeColumn(const IPosition& shape) { fixedShape_p = shape; }
...
void MSMIndColumn::getArrayV(rownr_t rownr, ArrayBase& arr) {
  ...
  if (dtype() == TpString) {
    ...
    memcpy(static_cast<char*>(arrData), static_cast<const char*>(data->data()),
           elemSize() * arr.size());
  }
...
void MSMIndColumn::putArrayV(rownr_t rownr, const ArrayBase& arr) {
  ...
  if (dtype() == TpString) {
    ...
    memcpy(static_cast<char*>(data->data()), static_cast<const char*>(arrData),
           elemSize() * arr.size());
  }
...
void MSMIndColumn::getSliceV(rownr_t rownr, const Slicer& ns, ArrayBase& arr) {
  Data* data = getShape(rownr);  // # also checks if row contains data
...
void MSMIndColumn::putSliceV(rownr_t rownr, const Slicer& ns, const ArrayBase& arr) {
