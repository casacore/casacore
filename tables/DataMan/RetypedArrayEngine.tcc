...
template <class S, class T>
DataManager* RetypedArrayEngine<S, T>::makeObject(const String&, const Record& spec) {
...
void RetypedArrayEngine<S, T>::prepare() {
  ...
  if (isVirtualFixedShape_p) {
    BaseMappedArrayEngine<S, T>::setShapeColumn(shape_p.concatenate(virtualFixedShape_p));
  }
...
void RetypedArrayEngine<S, T>::create64(rownr_t initialNrrow) {
  ...
  thisCol.rwKeywordSet().define("_RetypedArrayEngine_Shape", shape_p.asVector());
  thisCol.rwKeywordSet().defineRecord("_RetypedArrayEngine_Record", record_p);
}
...
template <class S, class T>
IPosition RetypedArrayEngine<S, T>::getStoredShape(rownr_t rownr, const IPosition& virtualShape) {
...
template <class S, class T>
IPosition RetypedArrayEngine<S, T>::checkShape(const Array<S>& source, const Array<T>& target) {
  ...
  if (tShape.nelements() != shape_p.nelements() + sShape.nelements()) {
    throw(
        DataManInvOper("RetypedArrayEngine: stored/virtual"
...
template <class S, class T>
void RetypedArrayEngine<S, T>::mapOnGet(Array<S>& array, const Array<T>& target) {
...
template <class S, class T>
void RetypedArrayEngine<S, T>::mapOnPut(const Array<S>& array, Array<T>& target) {
  IPosition elemShape = checkShape(array, target);
