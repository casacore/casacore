...
DataManager* SSMBase::clone() const { return new SSMBase(*this); }

String SSMBase::dataManagerType() const { return "StandardStMan"; }

String SSMBase::dataManagerName() const { return itsDataManName; }
...
DataManagerColumn* SSMBase::makeScalarColumn(const String&, int aDataType, const String&) {
...
DataManagerColumn* SSMBase::makeDirArrColumn(const String&, int aDataType, const String&) {
  // # Extend itsPtrColumn block if needed.
  if (ncolumn() >= itsPtrColumn.nelements()) {
    itsPtrColumn.resize(itsPtrColumn.nelements() + 32);
  }
...
DataManagerColumn* SSMBase::makeIndArrColumn(const String&, int aDataType, const String&) {
  // # Extend itsPtrColumn block if needed.
  if (ncolumn() >= itsPtrColumn.nelements()) {
    itsPtrColumn.resize(itsPtrColumn.nelements() + 32);
  }
...
void SSMBase::makeCache() {
  if (itsCache == 0) {
    ...
    itsCache = new BucketCache(itsFile, 512, itsBucketSize, itsNrBuckets, itsCacheSize, this,
                               SSMBase::readCallBack, SSMBase::writeCallBack, SSMBase::initCallBack,
...
void SSMBase::readIndexBuckets() {
  ...
  for (uInt j = 0; j < itsNrIdxBuckets; j++) {
    ...
    if (itsIdxBucketOffset > 0) {
      AlwaysAssert(itsIdxBucketOffset + itsIndexLength <= itsBucketSize && itsNrIdxBuckets == 1,
                   AipsError);
...
void SSMBase::addColumn(DataManagerColumn* aColumn) {
  ...
  if (aBestFit != -1) {
    ...
    if (rowsPerBucket < 1) {
      ...
      throw DataManError(
          "StandardStMan::addColumn  bucketsize too small"
          " for adding column " +
          aColumn->columnName());
    }
