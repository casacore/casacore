...
SSMColumn::~SSMColumn() { delete[] static_cast<char*>(itsData); }
...
void SSMColumn::shiftRows(char* aValue, rownr_t aRowNr, rownr_t aSRow, rownr_t anERow) {
...
void SSMColumn::getString(rownr_t aRowNr, String* aValue) {
  if (itsMaxLen > 0) {
    ...
    rownr_t aStartRow;
    rownr_t anEndRow;
    char* buf = itsSSMPtr->find(aRowNr, itsColNr, aStartRow, anEndRow, columnName());
    itsReadFunc(sp, buf + (aRowNr - aStartRow) * itsExternalSizeBytes, itsNrCopy);
...
Char* SSMColumn::getRowValue(Int* data, rownr_t aRowNr) {
  ...
  itsReadFunc(data, aValue + (aRowNr - aStartRow) * itsExternalSizeBytes, itsNrCopy);
...
void SSMColumn::putfloat(rownr_t aRowNr, const float* aValue) {
  putValue(aRowNr, aValue);
  if (aRowNr >= columnCache().start() && aRowNr <= columnCache().end()) {
    static_cast<float*>(itsData)[aRowNr - columnCache().start()] = *aValue;
  }
}
void SSMColumn::putdouble(rownr_t aRowNr, const double* aValue) {
  putValue(aRowNr, aValue);
  if (aRowNr >= columnCache().start() && aRowNr <= columnCache().end()) {
    static_cast<double*>(itsData)[aRowNr - columnCache().start()] = *aValue;
  }
}
void SSMColumn::putComplex(rownr_t aRowNr, const Complex* aValue) {
  putValue(aRowNr, aValue);
  if (aRowNr >= columnCache().start() && aRowNr <= columnCache().end()) {
    static_cast<Complex*>(itsData)[aRowNr - columnCache().start()] = *aValue;
  }
}
...
void SSMColumn::putDComplex(rownr_t aRowNr, const DComplex* aValue) {
  putValue(aRowNr, aValue);
  if (aRowNr >= columnCache().start() && aRowNr <= columnCache().end()) {
    static_cast<DComplex*>(itsData)[aRowNr - columnCache().start()] = *aValue;
  }
}
...
void SSMColumn::putValue(rownr_t aRowNr, const void* aValue) {
  ...
  rownr_t anEndRow;
  char* aDummy = itsSSMPtr->find(aRowNr, itsColNr, aStartRow, anEndRow, columnName());
  itsWriteFunc(aDummy + (aRowNr - aStartRow) * itsExternalSizeBytes, aValue, itsNrCopy);
  itsSSMPtr->setBucketDirty();
}

void SSMColumn::putValueShortString(rownr_t aRowNr, const void* aValue, const String& string) {
  rownr_t aStartRow;
  rownr_t anEndRow;
  char* aDummy;
  ...
  itsWriteFunc(aDummy + (aRowNr - aStartRow) * itsExternalSizeBytes, aValue, itsNrCopy);
  memcpy(aDummy + (aRowNr - aStartRow) * itsExternalSizeBytes, string.c_str(), string.length());
  itsSSMPtr->setBucketDirty();
}
