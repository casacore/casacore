...
void SSMIndStringColumn::setShape(rownr_t aRowNr, const IPosition& aShape) {
  ...
  itsSSMPtr->getStringHandler()->putShape(buf[0], buf[1], buf[2], aShape);
  putValue(aRowNr, buf);
}
...
IPosition SSMIndStringColumn::shape(rownr_t aRowNr) {
  ...
  getRowValue(buf, aRowNr);
  if (buf[2] > 0) {
    itsSSMPtr->getStringHandler()->getShape(aShape, buf[0], buf[1], buf[2]);
  } else {
...
void SSMIndStringColumn::putArrayV(rownr_t aRowNr, const ArrayBase& aDataPtr) {
  if (itsShape.nelements() != 0) {
