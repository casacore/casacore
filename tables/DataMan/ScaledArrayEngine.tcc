...
template <class S, class T>
DataManager* ScaledArrayEngine<S, T>::makeObject(const String&, const Record& spec) {
...
void ScaledArrayEngine<S, T>::create64(rownr_t initialNrrow) {
  ...
  thisCol.rwKeywordSet().define("_ScaledArrayEngine_Scale", scale_p);
  thisCol.rwKeywordSet().define("_ScaledArrayEngine_Offset", offset_p);
  thisCol.rwKeywordSet().define("_ScaledArrayEngine_ScaleName", scaleName_p);
  thisCol.rwKeywordSet().define("_ScaledArrayEngine_OffsetName", offsetName_p);
  thisCol.rwKeywordSet().define("_ScaledArrayEngine_FixedScale", fixedScale_p);
  thisCol.rwKeywordSet().define("_ScaledArrayEngine_FixedOffset", fixedOffset_p);
}
...
template <class S, class T>
void ScaledArrayEngine<S, T>::scaleColumnOnGet(Array<S>& array, const Array<T>& target) {
...
template <class S, class T>
void ScaledArrayEngine<S, T>::scaleColumnOnPut(const Array<S>& array, Array<T>& target) {
  if (fixedScale_p && fixedOffset_p) {
...
template <class S, class T>
void ScaledArrayEngine<S, T>::getSlice(rownr_t rownr, const Slicer& slicer, Array<S>& array) {
  Array<T> target(array.shape());
  column().getSlice(rownr, slicer, target);
  scaleOnGet(getScale(rownr), getOffset(rownr), array, target);
}
template <class S, class T>
void ScaledArrayEngine<S, T>::putSlice(rownr_t rownr, const Slicer& slicer, const Array<S>& array) {
  Array<T> target(array.shape());
  scaleOnPut(getScale(rownr), getOffset(rownr), array, target);
...
template <class S, class T>
void ScaledArrayEngine<S, T>::getColumnSlice(const Slicer& slicer, Array<S>& array) {
  Array<T> target(array.shape());
  column().getColumn(slicer, target);
  scaleColumnOnGet(array, target);
}
template <class S, class T>
void ScaledArrayEngine<S, T>::putColumnSlice(const Slicer& slicer, const Array<S>& array) {
  Array<T> target(array.shape());
  scaleColumnOnPut(array, target);
