...
class ScaledComplexData : public BaseMappedArrayEngine<VirtualType, StoredType> {
 ...
 public:
  ...
  ScaledComplexData(const String& virtualColumnName, const String& storedColumnName,
                    VirtualType scale, VirtualType offset = 0);
  ...
  // <group>
  ScaledComplexData(const String& virtualColumnName, const String& storedColumnName,
                    const String& scaleColumnName, VirtualType offset = 0);
  ScaledComplexData(const String& virtualColumnName, const String& storedColumnName,
 ...
 private:
  ...
  void scaleOnGet(VirtualType scale, VirtualType offset, Array<VirtualType>& array,
  ...
  // It optimizes for scale=1 and/or offset=0.
  void scaleOnPut(VirtualType scale, VirtualType offset, const Array<VirtualType>& array,
  ...
  // Otherwise it iterates through the array and applies the scale
  // and offset per row.
  void scaleCellsOnGet(Array<VirtualType>& array, const Array<StoredType>& stored,
  ...
  // Otherwise it iterates through the array and applies the scale
  // and offset per row.
  void scaleCellsOnPut(const Array<VirtualType>& array, Array<StoredType>& stored,
                       const RefRows& rownrs);
