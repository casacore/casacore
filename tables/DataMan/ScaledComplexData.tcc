...
ScaledComplexData<S, T>::ScaledComplexData(const String& virtualColumnName,
                                           const String& storedColumnName, S scale, S offset)
...
ScaledComplexData<S, T>::ScaledComplexData(const String& virtualColumnName,
                                           ...
                                           const String& scaleColumnName, S offset)
    : BaseMappedArrayEngine<S, T>(virtualColumnName, storedColumnName),
...
template <class S, class T>
DataManager* ScaledComplexData<S, T>::makeObject(const String&, const Record& spec) {
...
template <class S, class T>
void ScaledComplexData<S, T>::create64(rownr_t initialNrrow) {
  ...
  thisCol.rwKeywordSet().define("_ScaledComplexData_Scale", scale_p);
  thisCol.rwKeywordSet().define("_ScaledComplexData_Offset", offset_p);
  thisCol.rwKeywordSet().define("_ScaledComplexData_ScaleName", scaleName_p);
  thisCol.rwKeywordSet().define("_ScaledComplexData_OffsetName", offsetName_p);
  thisCol.rwKeywordSet().define("_ScaledComplexData_FixedScale", fixedScale_p);
  thisCol.rwKeywordSet().define("_ScaledComplexData_FixedOffset", fixedOffset_p);
}
  ...
  if (offset == S(0.0, 0.0)) {
    ...
    if (scale == S(1.0, 1.0)) {
      ...
      while (ip < last) {
        ...
        *op = S(*ip * scale.real() + offset.real(), ip[1] * scale.imag() + offset.imag());
        ip++;
        ip++;
        op++;
      }
...
template <class S, class T>
void ScaledComplexData<S, T>::scaleColumnOnGet(Array<S>& array, const Array<T>& target) {
...
template <class S, class T>
void ScaledComplexData<S, T>::scaleColumnOnPut(const Array<S>& array, Array<T>& target) {
  if (fixedScale_p && fixedOffset_p) {
...
template <class S, class T>
void ScaledComplexData<S, T>::scaleCellsOnGet(Array<S>& array, const Array<T>& target,
...
template <class S, class T>
void ScaledComplexData<S, T>::scaleCellsOnPut(const Array<S>& array, Array<T>& target,
                                              const RefRows& rownrs) {
  if (fixedScale_p && fixedOffset_p) {
    scaleOnPut(scale_p, offset_p, array, target);
  } else {
...
template <class S, class T>
void ScaledComplexData<S, T>::getSlice(rownr_t rownr, const Slicer& slicer, Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
  column().getSlice(rownr, storedSlicer(slicer), target);
  scaleOnGet(getScale(rownr), getOffset(rownr), array, target);
}
template <class S, class T>
void ScaledComplexData<S, T>::putSlice(rownr_t rownr, const Slicer& slicer, const Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
  scaleOnPut(getScale(rownr), getOffset(rownr), array, target);
...
template <class S, class T>
void ScaledComplexData<S, T>::getArrayColumnCells(const RefRows& rownrs, Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
  column().getColumnCells(rownrs, target);
  scaleCellsOnGet(array, target, rownrs);
}
template <class S, class T>
void ScaledComplexData<S, T>::putArrayColumnCells(const RefRows& rownrs, const Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
...
template <class S, class T>
void ScaledComplexData<S, T>::getColumnSlice(const Slicer& slicer, Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
  column().getColumn(storedSlicer(slicer), target);
  scaleColumnOnGet(array, target);
}
template <class S, class T>
void ScaledComplexData<S, T>::putColumnSlice(const Slicer& slicer, const Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
  scaleColumnOnPut(array, target);
...
template <class S, class T>
void ScaledComplexData<S, T>::getColumnSliceCells(const RefRows& rownrs, const Slicer& slicer,
                                                  Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
...
template <class S, class T>
void ScaledComplexData<S, T>::putColumnSliceCells(const RefRows& rownrs, const Slicer& slicer,
                                                  const Array<S>& array) {
  Array<T> target(storedShape(array.shape()));
  scaleCellsOnPut(array, target, rownrs);
...
Slicer ScaledComplexData<S, T>::storedSlicer(const Slicer& virtualSlicer) const {
  return Slicer(IPosition(1, 0).concatenate(virtualSlicer.start()),
                ...
                IPosition(1, 1).concatenate(virtualSlicer.stride()), Slicer::endIsLast);
}
