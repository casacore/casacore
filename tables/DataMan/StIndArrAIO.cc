...
void StManColumnIndArrayAipsIO::reopenRW() { iosfile_p->reopenRW(); }
...
void StManColumnIndArrayAipsIO::putArrayV(rownr_t rownr, const ArrayBase& arr) {
  StIndArray* sia = getShape(rownr);
  sia->putArrayV(*iosfile_p, arr, dtype());
  stmanPtr_p->setHasPut();
}

void StManColumnIndArrayAipsIO::getSliceV(rownr_t rownr, const Slicer& ns, ArrayBase& arr) {
  StIndArray* sia = getShape(rownr);
  sia->getSliceV(*iosfile_p, ns, arr, dtype());
}

void StManColumnIndArrayAipsIO::putSliceV(rownr_t rownr, const Slicer& ns, const ArrayBase& arr) {
  StIndArray* sia = getShape(rownr);
