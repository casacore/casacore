...
void StManColumnAipsIO::getFile(rownr_t nrval, AipsIO& ios) {
  ...
  ios >> nr;
  if (nr != nrval) {
    throw(DataManInternalError("StManColumnAipsIO::getFile: mismatch in #values"));
  }
  deleteAll();
  if (nrval > 0) {
    resize(nrval);
...
DataManager* StManAipsIO::makeObject(const String& storageManagerName, const Record& spec) {
  StManAipsIO* smp = new StManAipsIO(storageManagerName, spec);
  return smp;
}

String StManAipsIO::dataManagerType() const { return "StManAipsIO"; }

DataManagerColumn* StManAipsIO::makeScalarColumn(const String& columnName, int dataType,
                                                 const String&) {
...
DataManagerColumn* StManAipsIO::makeDirArrColumn(const String& columnName, int dataType,
                                                 const String&) {
  // # Check if data type is not TpOther.
  throwDataTypeOther(columnName, dataType);
  // # Extend colSet_p block if needed.
...
DataManagerColumn* StManAipsIO::makeIndArrColumn(const String& columnName, int dataType,
                                                 const String&) {
  // # Check if data type is not TpOther.
  throwDataTypeOther(columnName, dataType);
  // # Extend colSet_p block if needed.
...
rownr_t StManAipsIO::resync64(rownr_t nrrow) {
  ...
  if (snr != sequenceNr() || nrc != ncolumn()) {
    throw(DataManInternalError("StManAipsIO::open: mismatch in seqnr,#col"));
  }
  if (nrrow != nrr) {
#if defined(TABLEREPAIR)
    cerr << "StManAipsIO::open: mismatch in #row (expected " << nrrow << ", found " << nrr << ")"
         << endl;
    cerr << "Remainder will be added or discarded" << endl;
    setHasPut();
#else
    throw(DataManInternalError("StManAipsIO::open: mismatch in #row; expected " +
                               std::to_string(nrrow) + ", found " + std::to_string(nrr)));
#endif
  }
  for (i = 0; i < ncolumn(); i++) {
    ios >> dt;
    if (dt != colSet_p[i]->dataType()) {
      throw(DataManInternalError("StManAipsIO::open: mismatch in data type"));
    }
  }
