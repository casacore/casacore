...
void TSMCoordColumn::setShape(rownr_t rownr, const IPosition& shape) {
  ...
  if (rec.isDefined(columnName())) {
    if (shape != rec.shape(columnName())) {
      throw(TSMError("setShape of coordinate column " + columnName() + " has already been done"));
    }
    return;
  }
  ...
  switch (dataType()) {
    ...
    case TpArrayDComplex:
      ...
      break;
    default:
      throw(DataManInvDT("Unknown data type for coordinate column " + columnName()));
  }
}
...
IPosition TSMCoordColumn::shape(rownr_t rownr) {
  ...
  if (!hypercube->valueRecord().isDefined(columnName())) {
    throw(DataManInvOper("TSMCoord: no array in row " + String(std::to_string(rownr)) +
...
void TSMCoordColumn::getfloat(rownr_t rownr, float* dataPtr) {
  ...
  RORecordFieldPtr<Array<float>> field(hypercube->valueRecord(), columnName());
...
void TSMCoordColumn::putfloat(rownr_t rownr, const float* dataPtr) {
  ...
  RecordFieldPtr<Array<float>> field(hypercube->rwValueRecord(), columnName());
...
void TSMCoordColumn::getArrayV(rownr_t rownr, ArrayBase& dataPtr) {
  ...
  switch (dtype()) {
    case TpInt:
      hypercube->valueRecord().toArray(columnName(), static_cast<Array<Int>&>(dataPtr));
      break;
    case TpUInt:
      hypercube->valueRecord().toArray(columnName(), static_cast<Array<uInt>&>(dataPtr));
      break;
    case TpFloat:
      hypercube->valueRecord().toArray(columnName(), static_cast<Array<float>&>(dataPtr));
      break;
    case TpDouble:
      hypercube->valueRecord().toArray(columnName(), static_cast<Array<double>&>(dataPtr));
      break;
    case TpComplex:
      hypercube->valueRecord().toArray(columnName(), static_cast<Array<Complex>&>(dataPtr));
      break;
    case TpDComplex:
      hypercube->valueRecord().toArray(columnName(), static_cast<Array<DComplex>&>(dataPtr));
      break;
    default:
...
void TSMCoordColumn::putArrayV(rownr_t rownr, const ArrayBase& dataPtr) {
  ...
  switch (dtype()) {
    case TpInt:
      ...
      break;
    case TpUInt:
      hypercube->rwValueRecord().define(columnName(), static_cast<const Array<uInt>&>(dataPtr));
      break;
    case TpFloat:
      hypercube->rwValueRecord().define(columnName(), static_cast<const Array<float>&>(dataPtr));
      break;
    case TpDouble:
      hypercube->rwValueRecord().define(columnName(), static_cast<const Array<double>&>(dataPtr));
      break;
    case TpComplex:
      hypercube->rwValueRecord().define(columnName(), static_cast<const Array<Complex>&>(dataPtr));
      break;
    case TpDComplex:
      hypercube->rwValueRecord().define(columnName(), static_cast<const Array<DComplex>&>(dataPtr));
      break;
    default:
