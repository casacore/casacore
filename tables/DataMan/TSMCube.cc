...
IPosition TSMCube::adjustTileShape(const IPosition& cubeShape, const IPosition& tileShape) const {
...
void TSMCube::setup() {
  ...
  bucketSize_p =
      stmanPtr_p->getLengthOffset(tileSize_p, externalOffset_p, localOffset_p, localTileLength_p);
...
void TSMCube::extendCoordinates(const Record& coordValues, const String& name, uInt length) {
  ...
  switch (stmanPtr_p->coordinateDataType(name)) {
    ...
    default:
      throw DataManInvDT("extendCoordinates in TSM " + stmanPtr_p->dataManagerName());
  }
}
...
char* TSMCube::readTile(const char* external) {
  ...
  stmanPtr_p->readTile(local, localOffset_p, external, externalOffset_p, tileSize_p);
  return local;
}
...
void TSMCube::writeTile(char* external, const char* local) {
  stmanPtr_p->writeTile(external, externalOffset_p, local, localOffset_p, tileSize_p);
}
...
uInt TSMCube::calcCacheSize(const IPosition& cubeShape, const IPosition& tileShape, Bool extensible,
  ...
  for (i = 0; i < nrdim; i++) {
    ...
    if (slice(axis) < 1 + end(axis) - start(axis)) {
      ...
      if (slice(axis) < tileShape(axis) || st != 0 || slice(axis) % tileShape(axis) != 0) {
        reused(i) = 1;
      }
    }
  }
  ...
  while (nrd > 0) {
    ...
    if (cacheSize == validateCacheSize(cacheSize, maxCacheSize, bucketSize)) {
      return cacheSize;
    }
    nrd = nr;
  }
...
void TSMCube::accessSection(const IPosition& start, const IPosition& end, char* section, uInt colnr,
  ...
  if (oneEntireTile) {
    ...
    if (writeFlag) {
      memcpy(dataArray + pixelOffset, section, tileSize_p * localPixelSize);
      cachePtr->setDirty();
    } else {
      memcpy(section, dataArray + pixelOffset, tileSize_p * localPixelSize);
    }
    return;
  }
  ...
  if (nOneLong >= nrdim_p - 1) {
    accessLine(section, pixelOffset, localPixelSize, writeFlag, cachePtr, startTile_p,
               endTile_p(lineIndex), startPixelInFirstTile_p, endPixelInLastTile_p(lineIndex),
               lineIndex);
    return;
  }
  ...
  IPosition tileIncr = expandedTilesPerDim_p.offsetIncrement(nrTileSection_p);
  ...
  while (True) {
    ...
    while (True) {
      ...
      if (writeFlag) {
        TSMCube_MoveData(dataArray + dataOffset, section + sectionOffset, localSize);
      } else {
        TSMCube_MoveData(section + sectionOffset, dataArray + dataOffset, localSize);
      }
...
void TSMCube::accessStrided(const IPosition& start, const IPosition& end, const IPosition& stride,
  ...
  while (True) {
    ...
    // Initialize the pixel position in the data and section.
    dataPos = startPixel;
    IPosition dataIncr = localPixelSize * expandedTileShape_p.offsetIncrement(nrPixel, stride);
