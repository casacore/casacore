...
TSMCubeBuff::TSMCubeBuff(TiledStMan* stman, AipsIO& ios) : TSMCube(stman, ios, True) {}
...
void TSMCubeBuff::makeCache() {
  ...
  if (!cache_p) {
    cache_p.reset(
        new BucketBuffered(filePtr_p->bucketFile(), fileOffset_p, bucketSize_p, nrTiles_p));
  }
}
...
void TSMCubeBuff::deleteCache() { cache_p.reset(); }

void TSMCubeBuff::setShape(const IPosition& cubeShape, const IPosition& tileShape) {
...
void TSMCubeBuff::accessSection(const IPosition& start, const IPosition& end, char* section,
  ...
  while (True) {
    ...
    IPosition dataIncr = dataPixelSize * expandedTileShape_p.offsetIncrement(dataLength);
