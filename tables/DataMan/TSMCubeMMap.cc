...
void TSMCubeMMap::accessSection(const IPosition& start, const IPosition& end, char* section,
  ...
  if (oneEntireTile) {
    ...
    if (writeFlag) {
      ...
      convertFunc(dataArray + tileOffset, section, tileSize_p * nrConvElem);
    } else {
      const char* dataArray = cachePtr->getBucket(tileNr);
      convertFunc(const_cast<char*>(section), dataArray + tileOffset, tileSize_p * nrConvElem);
    }
    return;
  }
    ...
    if (useBool) {
      ...
      while (True) {
        if (writeFlag) {
          ...
          Conversion::bitToBool(section + sectionOffset, dataArray, dataOffset, nrval);
        }
        dataOffset += dataSize;
