...
TSMDataColumn::TSMDataColumn(const TSMColumn& column) : TSMColumn(column) {
...
IPosition TSMDataColumn::shape(rownr_t rownr) {
  ...
  IPosition cubeShape = hypercube->cubeShape();
  if (cubeShape.nelements() == 0) {
    throw(DataManInvOper("TSM: no array in row " + String(std::to_string(rownr)) + " of column " +
...
void TSMDataColumn::accessCellSlice(rownr_t rownr, const Slicer& ns, const void* dataPtr,
  ...
  hypercube->accessStrided(start, end, stride, (char*)dataPtr, colnr_p, localPixelSize_p,
      ...
      if (doIt) {
        ...
        if (!isFixedShape()) {
          ...
          if (!cellShape.isEqual(hcShape)) {
            throw DataManError("getArrayColumnCells shape mismatch in column " + columnName());
          }
        }
      } else {
        end(lastAxis) = hcRowPos;
...
void TSMDataColumn::accessFullCells(TSMCube* hypercube, char* dataPtr, Bool writeFlag,
  ...
  hypercube->accessStrided(start, end, incr, dataPtr, colnr_p, localPixelSize_p, tilePixelSize_p,
                           writeFlag);
}
...
void TSMDataColumn::getColumnSliceV(const Slicer& ns, ArrayBase& dataPtr) {
  if (!stmanPtr_p->canAccessColumn()) {
...
void TSMDataColumn::putColumnSliceV(const Slicer& ns, const ArrayBase& dataPtr) {
  if (!stmanPtr_p->canAccessColumn()) {
...
void TSMDataColumn::getArrayColumnCellsV(const RefRows& rownrs, ArrayBase& dataPtr) {
...
void TSMDataColumn::putArrayColumnCellsV(const RefRows& rownrs, const ArrayBase& dataPtr) {
  // Only use optimized accessColumnCells for hypercubes where the rows
  // are mapped to a single axis.
  if (dataPtr.ndim() == stmanPtr_p->nrCoordVector() + 1) {
    Bool deleteIt;
...
void TSMDataColumn::getColumnSliceCellsV(const RefRows& rownrs, const Slicer& ns,
                                         ArrayBase& dataPtr) {
...
void TSMDataColumn::putColumnSliceCellsV(const RefRows& rownrs, const Slicer& ns,
