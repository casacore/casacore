...
DataManager* TiledColumnStMan::clone() const {
  TiledColumnStMan* smp = new TiledColumnStMan(hypercolumnName_p, tileShape_p, maximumCacheSize());
  return smp;
}

DataManager* TiledColumnStMan::makeObject(const String& group, const Record& spec) {
...
void TiledColumnStMan::create64(rownr_t nrrow) {
  ...
  if (nrdim_p != fixedCellShape_p.nelements() + 1) {
    throw(
        TSMError("TiledColumnStMan: hypercube dimensionality "
  ...
  cubeSet_p.resize(1);
  cubeSet_p[0] = makeTSMCube(fileSet_p[0], cubeShape, tileShape_p, emptyRecord);
  ...
  for (uInt i = 0; i < dataNames.nelements(); i++) {
    ...
    if (columnDesc.isScalar()) {
      ...
      if ((columnDesc.options() & ColumnDesc::FixedShape) != ColumnDesc::FixedShape) {
        throw(
            TSMError("TiledColumnStMan needs array column " + dataNames(i) + " to be FixedShape"));
      }
    }
  }
...
IPosition TiledColumnStMan::defaultTileShape() const { return tileShape_p; }
