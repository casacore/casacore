...
TiledDataStMan::TiledDataStMan(const String& hypercolumnName, const Record& spec)
    : TiledStMan(hypercolumnName, 0), nrrowLast_p(0) {
...
DataManager* TiledDataStMan::clone() const {
  TiledDataStMan* smp = new TiledDataStMan(hypercolumnName_p, maximumCacheSize());
  return smp;
}

DataManager* TiledDataStMan::makeObject(const String& group, const Record& spec) {
  TiledDataStMan* smp = new TiledDataStMan(group, spec);
  return smp;
}

String TiledDataStMan::dataManagerType() const { return "TiledDataStMan"; }
...
void TiledDataStMan::addHypercube(const IPosition& cubeShape, const IPosition& tileShape,
                                  const Record& values) {
