...
TiledDataStManAccessor::TiledDataStManAccessor(const Table& table, const String& dataManagerName)
  ...
  if (dmptr->dataManagerType() != dataMan.dataManagerType()) {
    throw(DataManError("Data manager " + dataManagerName + " has type " + dmptr->dataManagerType() +
                       "; expected " + dataMan.dataManagerType()));
  }
  if (!table.isWritable()) {
...
TiledDataStManAccessor::TiledDataStManAccessor(const TiledDataStManAccessor& that)
    : ROTiledStManAccessor(that), tiledDataManPtr_p(that.tiledDataManPtr_p) {}

TiledDataStManAccessor& TiledDataStManAccessor::operator=(const TiledDataStManAccessor& that) {
  ROTiledStManAccessor::operator=(that);
  tiledDataManPtr_p = that.tiledDataManPtr_p;
  return *this;
}

void TiledDataStManAccessor::addHypercube(const IPosition& cubeShape, const IPosition& tileShape,
                                          const Record& values) {
