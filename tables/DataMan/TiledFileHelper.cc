...
TiledFileHelper::TiledFileHelper(const String& fileName, const IPosition& shape, DataType dtype,
                                 const TSMOption& tsmOption, Bool writable, Bool bigEndian)
    : TiledStMan("TiledFileHelper", std::max(0, tsmOption.maxCacheSizeMB()) * 1024 * 1024) {
  ...
  switch (dtype) {
    ...
    case TpComplex:
      ...
      break;
    case TpDComplex:
      itsDesc.addColumn(ArrayColumnDesc<DComplex>("DATA", shape, ColumnDesc::FixedShape));
      break;
    default:
...
const TableDesc& TiledFileHelper::getDesc() const { return itsDesc; }

String TiledFileHelper::dataManagerType() const { return "TiledFileHelper"; }
