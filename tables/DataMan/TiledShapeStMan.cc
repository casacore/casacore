...
TiledShapeStMan::TiledShapeStMan(const String& hypercolumnName, const Record& spec)
    : TiledStMan(hypercolumnName, 0), nrUsedRowMap_p(0), lastHC_p(-1) {
...
DataManager* TiledShapeStMan::clone() const {
  TiledShapeStMan* smp =
      new TiledShapeStMan(hypercolumnName_p, defaultTileShape_p, maximumCacheSize());
  return smp;
}

DataManager* TiledShapeStMan::makeObject(const String& group, const Record& spec) {
  TiledShapeStMan* smp = new TiledShapeStMan(group, spec);
  return smp;
}

String TiledShapeStMan::dataManagerType() const { return "TiledShapeStMan"; }
...
TSMCube* TiledShapeStMan::singleHypercube() {
  if (nrUsedRowMap_p != 1 || rowMap_p[0] != nrrow_p - 1) {
    throw(TSMError("TiledShapeStMan: function on hypercolumn " + hypercolumnName_p +
                   " cannot be done "
...
void TiledShapeStMan::addHypercube(rownr_t rownr, const IPosition& cubeShape,
                                   const IPosition& tileShape) {
...
TSMCube* TiledShapeStMan::getHypercube(rownr_t rownr, IPosition& position) {
  ...
  if (position.nelements() > 0) {
    position(nrdim_p - 1) = posMap_p[lastHC_p] - (rowMap_p[lastHC_p] - rownr);
  }
  return hypercube;
}
