...
String TiledStMan::dataManagerName() const { return hypercolumnName_p; }
...
TSMCube* TiledStMan::makeTSMCube(TSMFile* file, const IPosition& cubeShape,
  ...
  if (tsmOption().option() == TSMOption::MMap) {
    ...
    hypercube = new TSMCubeMMap(this, file, cubeShape, tileShape, values, fileOffset);
  } else if (tsmOption().option() == TSMOption::Buffer) {
    // cout << "buffered TSM1" << endl;
    AlwaysAssert(file->bucketFile()->isBuffered(), AipsError);
    hypercube = new TSMCubeBuff(this, file, cubeShape, tileShape, values, fileOffset);
  } else {
    // cout << "caching TSM1" << endl;
    AlwaysAssert(file->bucketFile()->isCached(), AipsError);
    hypercube = new TSMCube(this, file, cubeShape, tileShape, values, fileOffset);
  }
  return hypercube;
}
...
DataManagerColumn* TiledStMan::makeScalarColumn(const String& columnName, int dataType,
                                                const String& dataTypeId) {
  return makeIndArrColumn(columnName, dataType, dataTypeId);
}
DataManagerColumn* TiledStMan::makeDirArrColumn(const String& columnName, int dataType,
                                                const String& dataTypeId) {
  return makeIndArrColumn(columnName, dataType, dataTypeId);
}
DataManagerColumn* TiledStMan::makeIndArrColumn(const String& columnName, int dataType,
                                                const String&) {
...
int TiledStMan::coordinateDataType(const String& columnName) const {
  ...
  throw(TSMError("coordinateDataType: column " + columnName + " is unknown"));
  return 0;
}
...
void TiledStMan::setup(Int extraNdim) {
  ...
  if (extraNdim < 0 || tableDesc.isHypercolumn(hypercolumnName_p)) {
    ...
    nrdim_p = tableDesc.hypercolumnDesc(hypercolumnName_p, dataNames, coordNames, idNames);
    ...
    for (uInt i = 0; i < ncolumn(); i++) {
      ...
      if (nd > 0) {
        if (ndim == 0) {
          ...
          throw TSMError("TiledStMan: dimensionality of column " + dataNames(i) +
                         " mismatches other columns");
        }
      }
    }
    if (ndim == 0) {
      throw TSMError("TiledStMan: unknown dimensionality for column " + dataNames(0));
    }
...
const TableDesc& TiledStMan::getDesc() const { return table().tableDesc(); }

void TiledStMan::setupCheck(const TableDesc&, const Vector<String>&) const {}

void TiledStMan::checkCubeShape(const TSMCube* hypercube, const IPosition& cubeShape) const {
  // Check if the dimensionalities are correct.
  if (cubeShape.nelements() != nrdim_p) {
    throw(TSMError("addHypercube dimensionality mismatch in " + hypercolumnName_p));
  }
    ...
    if (coordColSet_p[i] != 0) {
      ...
      if (shapeColumn.nelements() > 0) {
        if (shape(i) != shapeColumn(0)) {
          throw(TSMError("Mismatch in fixed shape of coordinate column " +
...
uInt TiledStMan::getBindings(const Vector<String>& columnNames, Block<TSMColumn*>& colSet,
  ...
  for (uInt i = 0; i < columnNames.nelements(); i++) {
    ...
    if (!found && mustExist) {
      throw(TSMError("TiledStMan column " + columnNames(i) + " is not bound"));
    }
  }
  return nrfound;
}
...
void TiledStMan::checkAddHypercube(const IPosition& cubeShape, const Record& values) const {
  ...
  if (getCubeIndex(values) >= 0) {
    throw(TSMError("addHypercube with already existing id values in " + hypercolumnName_p));
  }
}
...
void TiledStMan::checkCoordinates(const Block<TSMColumn*>& coordColSet, const IPosition& cubeShape,
    ...
    if (coordColSet[i] != 0) {
      ...
      if (values.isDefined(name)) {
        ...
        if (values.dataType(name) != dataType) {
          throw(TSMError("Data type mismatch for coordinate " + name));
        }
        IPosition shape = values.shape(name);
        if (shape.nelements() != 1) {
          throw(TSMError("Values of coordinate " + name + " do not form a vector"));
        }
...
uInt TiledStMan::headerFileGet(AipsIO& headerFile, rownr_t tabNrrow, Bool firstTime,
  ...
  if (seqnr != sequenceNr() || nrcol != ncolumn()) {
    ...
    // #if (sequenceNr() != 7) {
    throw(DataManInternalError("TiledStMan::headerFileGet: mismatch in seqnr,#col"));
    // #}
  }
  if (nrrow != nrrow_p) {
  ...
  if (nrrow < nrrow_p) {
    cubeSet_p[0]->extend(nrrow_p - nrrow, Record(), coordColSet_p[nrdim_p - 1]);
  }
  return version;
}
