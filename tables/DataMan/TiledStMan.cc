...
TiledStMan::TiledStMan (const String& hypercolumnName, uInt64 maximumCacheSize)
: DataManager       (),
...
{
    Record rec;
    rec.define ("ActualMaxCacheSize", Int64(maxCacheSize_p));
    return rec;
}
...
void TiledStMan::setProperties (const Record& rec)
{
    if (rec.isDefined("ActualMaxCacheSize")) {
        setMaximumCacheSize (rec.asInt64("ActualMaxCacheSize"));
    }
}
...
void TiledStMan::setMaximumCacheSize (uInt64 nbytes)
