...
template <class T>
VACEngine<T>::VACEngine(const String& sourceColumnName) : sourceName_p(sourceColumnName) {}

template <class T>
VACEngine<T>::VACEngine(const VACEngine<T>& that)
    : VirtualColumnEngine(), VirtualArrayColumn<T>(), sourceName_p(that.sourceName_p) {}
...
template <class T>
DataManagerColumn* VACEngine<T>::makeDirArrColumn(const String& columnName, int dataType,
                                                  const String& dataTypeId) {
  return makeIndArrColumn(columnName, dataType, dataTypeId);
}

template <class T>
DataManagerColumn* VACEngine<T>::makeIndArrColumn(const String& columnName, int, const String&) {
  if (sourceName_p.empty()) {
    sourceName_p = columnName;
  } else {
    if (columnName != sourceName_p) {
      throw(DataManInvOper("VACEngine with source column " + sourceName_p + " bound to column " +
                           columnName + "; must be the same"));
    }
  }
  return this;
}
