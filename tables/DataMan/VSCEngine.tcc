...
template <class T>
VSCEngine<T>::VSCEngine() : sourceName_p("") {}

template <class T>
VSCEngine<T>::VSCEngine(const String& sourceColumnName) : sourceName_p(sourceColumnName) {}

template <class T>
VSCEngine<T>::VSCEngine(const VSCEngine<T>& that)
    : VirtualColumnEngine(), VirtualScalarColumn<T>(), sourceName_p(that.sourceName_p) {}
...
template <class T>
DataManagerColumn* VSCEngine<T>::makeScalarColumn(const String& columnName, int, const String&) {
  if (sourceName_p.empty()) {
    sourceName_p = columnName;
  } else {
    if (columnName != sourceName_p) {
      throw(DataManInvOper("VSCEngine with source column " + sourceName_p + " bound to column " +
