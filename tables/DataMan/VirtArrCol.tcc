...
template <class T>
void VirtualArrayColumn<T>::getSliceV(rownr_t rownr, const Slicer& slicer, ArrayBase& array) {
  getSlice(rownr, slicer, static_cast<Array<T>&>(array));
}
template <class T>
void VirtualArrayColumn<T>::putSliceV(rownr_t rownr, const Slicer& slicer, const ArrayBase& array) {
...
template <class T>
void VirtualArrayColumn<T>::getArrayColumnCellsV(const RefRows& rownrs, ArrayBase& array) {
  getArrayColumnCells(rownrs, static_cast<Array<T>&>(array));
}
template <class T>
void VirtualArrayColumn<T>::putArrayColumnCellsV(const RefRows& rownrs, const ArrayBase& array) {
  putArrayColumnCells(rownrs, static_cast<const Array<T>&>(array));
}
template <class T>
void VirtualArrayColumn<T>::getColumnSliceV(const Slicer& slicer, ArrayBase& array) {
  getColumnSlice(slicer, static_cast<Array<T>&>(array));
}
template <class T>
void VirtualArrayColumn<T>::putColumnSliceV(const Slicer& slicer, const ArrayBase& array) {
  putColumnSlice(slicer, static_cast<const Array<T>&>(array));
}
template <class T>
void VirtualArrayColumn<T>::getColumnSliceCellsV(const RefRows& rownrs, const Slicer& slicer,
                                                 ArrayBase& array) {
  getColumnSliceCells(rownrs, slicer, static_cast<Array<T>&>(array));
}
template <class T>
void VirtualArrayColumn<T>::putColumnSliceCellsV(const RefRows& rownrs, const Slicer& slicer,
...
template <class T>
void VirtualArrayColumn<T>::getSlice(rownr_t rownr, const Slicer& slicer, Array<T>& arraySlice) {
...
template <class T>
void VirtualArrayColumn<T>::getColumnSlice(const Slicer& slicer, Array<T>& array) {
  getColumnSliceBase(slicer, array);
}
template <class T>
void VirtualArrayColumn<T>::putColumnSlice(const Slicer& slicer, const Array<T>& array) {
  putColumnSliceBase(slicer, array);
}

// # The default implementations of the Cells functions throw an exception.
template <class T>
void VirtualArrayColumn<T>::getArrayColumnCells(const RefRows& rownrs, Array<T>& value) {
  getArrayColumnCellsBase(rownrs, value);
}
template <class T>
void VirtualArrayColumn<T>::putArrayColumnCells(const RefRows& rownrs, const Array<T>& value) {
  putArrayColumnCellsBase(rownrs, value);
}
template <class T>
void VirtualArrayColumn<T>::getColumnSliceCells(const RefRows& rownrs, const Slicer& ns,
                                                Array<T>& value) {
  getColumnSliceCellsBase(rownrs, ns, value);
}
template <class T>
void VirtualArrayColumn<T>::putColumnSliceCells(const RefRows& rownrs, const Slicer& ns,
                                                const Array<T>& value) {
...
void VirtualArrayColumn<T>::putArray(rownr_t, const Array<T>&) {
  throw DataManInvOper(
      "VirtualArrayColumn::putArray not possible"
      " for column " +
      columnName());
}
