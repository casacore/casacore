...
DataManagerColumn* VirtualTaQLColumn::makeScalarColumn(const String& name, int dataType,
                                                       const String&) {
...
DataManagerColumn* VirtualTaQLColumn::makeIndArrColumn(const String& name, int dataType,
                                                       const String&) {
  AlwaysAssert(dataType != TpOther, AipsError);
  itsDataType = dataType;
  itsIsArray = True;
...
void VirtualTaQLColumn::prepare() {
  ...
  if (itsNode->isScalar() == itsIsArray) {
    throw DataManError(
        "VirtualTaQLColumn: "
        "expression and " +
        itsColumnName + " column type mismatch (not both scalar or array)");
  }
  ...
  if (itsNode->dataType() != exptype) {
    throw DataManError(
        ...
        itsColumnName + " data type mismatch");
  }
...
DataManager* VirtualTaQLColumn::makeObject(const String&, const Record& spec) {
  DataManager* dmPtr = new VirtualTaQLColumn(spec);
  return dmPtr;
}

void VirtualTaQLColumn::registerClass() { DataManager::registerCtor(className(), makeObject); }

String VirtualTaQLColumn::className() { return "VirtualTaQLColumn"; }
String VirtualTaQLColumn::dataManagerType() const { return className(); }
...
void VirtualTaQLColumn::getResult(rownr_t rownr) {
  ...
  if (!itsShape.empty() && !itsShape.isEqual(itsCurArray->shape())) {
    throw DataManError(
        "VirtualTaQLColumn::getResult - shape of result mismatches fixed "
        "shape of column " +
        columnName());
  }
}
...
void VirtualTaQLColumn::getScalarColumnCellsV(const RefRows& rownrs, ArrayBase& arr) {
  if (itsIsConst) {
    // Constant value, so fill the array with the same value.
    fillArray(arr);
  } else {
