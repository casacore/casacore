...
void RetypedArrayEx1::get(void*, Array<float>& out, const void* vin, const IPosition& shape) {
...
void* RetypedArrayEx2::newCopyInfo(const TableRecord& record, const IPosition& shape) {
...
RetypedArrayEx2::CopyInfo::~CopyInfo() { delete mask_p; }

void RetypedArrayEx2::CopyInfo::set(void* vout, const Array<DComplex>& in, const IPosition& shape) {
...
void RetypedArrayEx2::CopyInfo::get(Array<DComplex>& out, const void* vin, const IPosition& shape) {
...
void a(Bool doExcp) {
  ...
  td.addColumn(ArrayColumnDesc<float>("Data", IPosition(2, 2, 10), ColumnDesc::FixedShape));
  td.addColumn(ArrayColumnDesc<RetypedArrayEx1>(
      "colA", "", RetypedArrayEngine<RetypedArrayEx1, float>::className(), "", IPosition(1, 10),
  ...
  }
  {
    ...
    for (rownr_t i = 0; i < tab.nrow(); i++) {
      for (uInt j = 0; j < 10; j++) {
        ...
        resA(j) = RetypedArrayEx2(resD(0, j), resD(1, j), resD(2, j), resD(3, j));
      }
      colD.get(i, valD);
      colA.get(i, valA);
      if (!allEQ(valD, resD)) {
