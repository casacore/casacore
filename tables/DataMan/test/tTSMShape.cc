...
void check(const IPosition& shape, const IPosition& offsetIncr, const IPosition& stride,
           const char* message) {
    ...
    for (position(2) = 0; position(2) < stride(2); position(2)++) {
      for (position(1) = 0; position(1) < stride(1); position(1)++) {
        for (position(0) = 0; position(0) < stride(0); position(0)++) {
          ...
          if (pos != position) {
            cout << message << "invalid position " << pos << " at offset " << checkOffset << endl;
          }
...
int main() {
      ...
      for (position(1) = 0; position(1) < shape(1); position(1)++) {
        for (position(0) = 0; position(0) < shape(0); position(0)++) {
          ...
          if (pos != position + origin) {
            cout << "invalid position " << pos << " at offset " << checkOffset << endl;
          }
          checkOffset++;
        }
      }
...

  {
    ...
    if (incr(0) != 1 && incr(1) != 0 && incr(2) != 0 && incr(3) != 0) {
      cout << "Invalid incr(4,5,6,7): " << incr(0) << "," << incr(1) << "," << incr(2) << ","
           << incr(3) << endl;
    }
