...
void openPablo(char** argv) {
  ...
  if (EnvironmentVariable::isDefined("PABLOSTATS")) {
    ...
    if (pablostats.contains(lookfor) || pablostats == String("all")) {
      ...
      oss << pablostatsdir << "/" << myname.baseName() << "_node" << tracenode << ".PabloIO";
...
void makeCube(const char* argv[]) {
  ...
  for (i = 0; i < nrdim; i++) {
    ...
    istr >> cubeShape(i);
    if (cubeShape(i) <= 0) {
      throw AipsError("Arrayshape " + std::to_string(cubeShape(i)) + " must be > 0");
    }
  }
  if (tileV.nelements() != nrdim) {
    ...
    for (i = 0; i < nrdim; i++) {
      ...
      istr >> tileShape(i);
      if (tileShape(i) <= 0) {
        throw AipsError("Tileshape " + std::to_string(tileShape(i)) + " must be > 0");
      }
    }
  }
...
void getCube(Bool trav, Bool ask) {
  ...
  }
  {
    ...
    cout << "array x,y,z along tiles" << " (" << nr << " passes  " << nrsteps << ")" << endl;
    realtime = timer.real();
    timer.show("get      ");
    cout << "Throughput " << sizeMb / realtime << " Mb/sec" << endl;
    accessor.showCacheStatistics(cout);
  ...
  while (True) {
    ...
    accessor.setCacheSize(0, slice, getVec(nrdim, "window start: "),
                          getVec(nrdim, "window shape: "), getVec(nrdim, "axis path:    "));
    cout << "  result is:" << endl;
    accessor.showCacheStatistics(cout);
  }
}
