...
void writeFixed(const TSMOption& tsmOpt) {
  ...
  td.addColumn(ArrayColumnDesc<float>("Weight", IPosition(2, 16, 20), ColumnDesc::FixedShape));
  td.defineHypercolumn("TSMExample", 3, stringToVector("Data,Weight"),
  ...
  AlwaysAssertExit(accessor.hypercubeShape(2) == IPosition(3, 16, 20, table.nrow()));
  AlwaysAssertExit(accessor.tileShape(2) == IPosition(3, 5, 6, 1));
  AlwaysAssertExit(accessor.getHypercubeShape(0) == IPosition(3, 16, 20, table.nrow()));
...
void readTable(const TSMOption& tsmOpt, Bool readKeys) {
  ...
  if (readKeys) {
    AlwaysAssertExit(allEQ(accessor.getValueRecord(0).asArrayFloat("Freq"), freqValues));
    AlwaysAssertExit(allEQ(accessor.getValueRecord(0).asArrayFloat("Pol"), polValues));
    AlwaysAssertExit(allEQ(accessor.getValueRecord(0).asArrayFloat("Time"), time.getColumn()));
  }
    ...
    for (j = 0; j < 20; j++) {
      for (i = 0; i < 16; i++) {
        ...
        if (!allEQ(result, array)) {
          cout << "mismatch in getColumnSlice " << i << "," << j << endl;
        }
        array += float(1);
      }
    }
  ...
  // Get strided slices in the entire column.
  {
    ...
    for (j = 0; j < 20 / 2; j++) {
      for (i = 0; i < 16 / 2; i++) {
        data.getColumn(Slicer(IPosition(2, i, j), IPosition(2, 2, 2), IPosition(2, 16 / 2, 20 / 2)),
                       result);
        if (!allEQ(result, array)) {
          cout << "mismatch in getColumnSlice " << i << "," << j << endl;
        }
  ...
  // Get slices from each cell.
  {
    ...
    for (k = 0; k < table.nrow(); k++) {
      ...
      for (j = 0; j < 20 / 4; j++) {
        for (i = 0; i < 16 / 2; i++) {
          data.getSlice(k, Slicer(IPosition(2, i * 2, j * 4), IPosition(2, 2, 4)), result);
          if (!allEQ(result, array)) {
            cout << "mismatch in getSlice " << i << "," << j << "," << k << endl;
          }
          array += float(2);
        }
        array += float((4 - 1) * 16);
  ...
  // Get a strided slice from each cell.
  {
    ...
    for (k = 0; k < table.nrow(); k++) {
      ...
      for (j = 0; j < 20 / 5; j++) {
        for (i = 0; i < 16 / 8; i++) {
          data.getSlice(
              k, Slicer(IPosition(2, i, j), IPosition(2, 8, 5), IPosition(2, 16 / 8, 20 / 5)),
              result);
          if (!allEQ(result, array)) {
            cout << "mismatch in strided getSlice " << i << "," << j << "," << k << endl;
          }
          array += float(1);
        }
        array += float(16 - 16 / 8);
