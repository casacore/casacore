...
int main(int argc, const char* argv[]) {
  ...
  if (argc != 5) {
    cout << ">>>" << endl;
    cout << "tTiledDataStM_1 uses TiledDataStMan to store float arrays." << endl;
    cout << "It writes the data and reads them back per cell and "
            "per column-slice"
         << endl;
    cout << "and shows timing and cache statistics." << endl;
    cout << "Invoke as tTiledDataStM_1 arrayNdim cubeShape tileShape MaxCacheSize" << endl;
...
void a(const char* argum[]) {
  ...
  for (i = 0; i < cubeV.nelements(); i++) {
    ...
    istr >> cubeShape(i);
    if (cubeShape(i) <= 0) {
      throw AipsError("Cubeshape " + std::to_string(cubeShape(i)) + " must be > 0");
    }
  }
  ...
  td.addColumn(ArrayColumnDesc<float>("Data", cellShape, ColumnDesc::FixedShape));
  td.defineHypercolumn("TSMExample", cubeShape.nelements(), stringToVector("Data"));
