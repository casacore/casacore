...
void a() {
  ...
  td.addColumn(ArrayColumnDesc<float>("Pol", IPosition(1, 12), ColumnDesc::FixedShape));
  ...
  td.addColumn(ArrayColumnDesc<float>("Weight", IPosition(2, 12, 20), ColumnDesc::FixedShape));
  td.addColumn(ArrayColumnDesc<Bool>("Flag", IPosition(2, 12, 20), ColumnDesc::FixedShape));
  td.addColumn(ArrayColumnDesc<Complex>("OData", IPosition(2, 12, 20), ColumnDesc::FixedShape));
  td.defineHypercolumn("TSMExample", 4, stringToVector("Data,Weight,Flag"),
                       stringToVector("Pol,Freq,Baseline,Time"), stringToVector("Id"));
  td.defineHypercolumn("TSMExample2", 4, stringToVector("OData"));
  ...
  accessor.addHypercube(IPosition(4, 12, 20, 30, 42), IPosition(4, 4, 5, 6, 7), values);
  accessor2.addHypercube(IPosition(4, 12, 20, 30, 42), IPosition(4, 4, 5, 30, 42), Record());
  ...
  td1.defineHypercolumn("TSMExample1", 2, stringToVector("Data1"));
  // Create a storage manager for it.
  TiledDataStMan sm1("TSMExample1");
  table.addColumn(td1, sm1);
  TiledDataStManAccessor accessor1(table, "TSMExample1");

  accessor1.addHypercube(IPosition(2, 30, 42), IPosition(2, 9, 11), Record());
...
void b() {
  ...
  for (j = 0; j < 20 / 5; j++) {
    for (i = 0; i < 12 / 4; i++) {
      if (!allEQ(odata.getColumn(Slicer(IPosition(2, i * 4, j * 5), IPosition(2, 4, 5))),
