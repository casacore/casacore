...
int main() {
  ...
  // at offset 0.
  {
    ...
    try {
      ...
      AlwaysAssertExit(allEQ(arr, tfa.getFloat(Slicer(IPosition(2, 0, 0), shape))));
  ...
  // which is true for all data types except long.
  {
    ...
    try {
      ...
      TiledFileAccess tfa("tTiledFileAccess_tmp.dat", off2, shape, IPosition(2, 16, 1), TpFloat,
  ...
  // Check it after writing by iterating through the data.
  {
    ...
    try {
      ...
      for (Int i = 0; i < shape(0); i++) {
        ...
        AlwaysAssertExit(allEQ(arr(st, end) + DComplex(3, 5), tfal.getDComplex(Slicer(st, leng))));
      }
      cout << end << endl;
    } catch (std::exception& x) {
      cout << "Exception: " << x.what() << endl;
