...
void testSingle(const aocommon::UVector<unsigned int>& data, int bitCount) {
  ...
  aocommon::UVector<unsigned char> buffer(BytePacker::bufferSize(trimmedData.size(), bitCount), 0);
  ...
  for (size_t i = 0; i != trimmedData.size(); ++i) {
    BOOST_REQUIRE_MESSAGE(restoredData[i] == trimmedData[i],
                          "data[" << i << "] was incorrectly unpacked: was " << restoredData[i]
                                  << ", should be " << trimmedData[i]);
  }
}
...
void testCombinations(const aocommon::UVector<unsigned int>& data, int bitCount) {
  for (size_t dataSize = 1; dataSize != std::min<size_t>(32u, data.size()); ++dataSize) {
    aocommon::UVector<unsigned int> resizedData(data.begin(), data.begin() + dataSize);
  ...
  for (size_t dataSize = 1; dataSize != std::min<size_t>(32u, data.size() - 1); ++dataSize) {
    aocommon::UVector<unsigned int> resizedData(data.begin() + 1, data.begin() + dataSize + 1);
    testSingle(resizedData, bitCount);
  }
