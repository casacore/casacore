#ifndef AOCOMMON_UVECTOR_H_
#define AOCOMMON_UVECTOR_H_
...
class UVector : private Alloc {
 ...
 public:
  ...
  UVector(std::initializer_list<Tp> initlist, const allocator_type& allocator = Alloc())
    ...
    for (typename std::initializer_list<Tp>::const_iterator i = initlist.begin();
         i != initlist.end(); ++i) {
      *destIter = *i;
      ++destIter;
    }
  ...
  UVector& operator=(const UVector<Tp, Alloc>& other) {
    return assign_copy_from(
        other, typename std::allocator_traits<Alloc>::propagate_on_container_copy_assignment());
  }
  ...
  void push_back(std::initializer_list<Tp> initlist) {
    ...
    for (typename std::initializer_list<Tp>::iterator i = initlist.begin(); i != initlist.end();
         ++i) {
      *_end = *i;
      ++_end;
    }
 ...
 private:
  ...
  void construct_from_range(InputIterator first, InputIterator last, std::false_type) {
    construct_from_range<InputIterator>(
        first, last, typename std::iterator_traits<InputIterator>::iterator_category());
  }
  ...
  iterator insert_from_range(const_iterator position, InputIterator first, InputIterator last,
    ...
    return insert_from_range<InputIterator>(
        position, first, last, typename std::iterator_traits<InputIterator>::iterator_category());
  }
