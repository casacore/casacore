...
void LoggerHolder::append(const LoggerHolder& other) { itsRep->append(other); }

void LoggerHolder::reopenRW() { itsRep->reopenRW(); }
...
void LoggerHolder::flush() { itsRep->flush(); }

void LoggerHolder::resync() { itsRep->resync(); }

void LoggerHolder::removeParents() { itsRep->removeParents(); }
...
LoggerHolderRep::~LoggerHolderRep() { removeParents(); }
...
void LoggerHolderRep::append(const LoggerHolder& other) {
  ...
  for (LoggerHolder::const_iterator iter = other.begin(); iter != other.end(); iter++) {
    logsink.writeLocally(iter->time(), iter->message(), iter->priority(), iter->location(),
                         iter->objectID());
  }
}
