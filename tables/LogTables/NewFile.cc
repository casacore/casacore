...
Bool NewFile::valueOK(const String &value, String &error) const {
  LogOrigin OR("NewFile", "valueOK(const String &value, String &error) const", WHERE);
  LogMessage msg(OR);
  ...
  if (thefile.exists()) {
    ...
    if (remove == "yes") {
          ...
          if (!Table::isWritable(value)) {
            ...
            if (TableUtil::canDeleteTable(extra_error, value)) {
              try {
                ...
                removed = False;
                extra_error = String("Error deleting table ") + value + ":" + xxx.what();
              }
            }
          }
      ...
      if (!removed) {
        ...
        retval = True;
        msg.message(String("Removed file ") + value + " at users request")
            .line(__LINE__)
      ...
      retval = False;
      error = String("File ") + value + " exists, and the user does not want to remove it.";
    }
  } else {
    retval = True;
  }
