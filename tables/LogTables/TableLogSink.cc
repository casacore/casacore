...
String TableLogSink::localId() { return String("TableLogSink"); }

String TableLogSink::id() const { return String("TableLogSink"); }

TableLogSink::TableLogSink(LogMessage::Priority filter, const String& fileName)
    : LogSinkInterface(LogFilter(filter)) {
  init(fileName);
}

TableLogSink::TableLogSink(const LogFilterInterface& filter, const String& fileName)
...
void TableLogSink::init(const String& fileName) {
  ...
  if (fileName.empty()) {
    ...
    logMessage.priority(LogMessage::DEBUGGING)
        .line(__LINE__)
        .message(String("Opening existing file ") + fileName);
    LogSink::postGlobally(logMessage);
  } else if (Table::isReadable(fileName)) {
    // We can read it, but not write it!
    logMessage.priority(LogMessage::SEVERE)
        .line(__LINE__)
        .message(fileName + " exists, but is not writable");
    LogSink::postGloballyThenThrow(logMessage);
  } else {
    // Table does not exist - create
    logMessage.priority(LogMessage::DEBUGGING).line(__LINE__).message("Creating " + fileName);
    LogSink::postGlobally(logMessage);
...
TableLogSink::TableLogSink(const String& fileName) : LogSinkInterface() {
  LogMessage logMessage(LogOrigin("TableLogSink", "TableLogSink", WHERE));
  if (!Table::isReadable(fileName)) {
    ...
    logMessage.priority(LogMessage::DEBUGGING)
        .line(__LINE__)
        .message(String("Opening readonly ") + fileName);
    LogSink::postGlobally(logMessage);
  }
  attachCols();
}

TableLogSink::TableLogSink(const TableLogSink& other) : LogSinkInterface() { copy_other(other); }
...
TableLogSink::~TableLogSink() { TableLogSink::flush(); }
...
void TableLogSink::makeTable(SetupNewTable& setup) {
  ...
  log_table_p.tableInfo().readmeAddLine("Repository for software-generated logging messages");
}
...
LogSink TableLogSink::makeSink(const LogFilterInterface& filter, const String& fileName) {
  LogSinkInterface* sink = new TableLogSink(LogFilter(LogMessage::DEBUGGING), fileName);
