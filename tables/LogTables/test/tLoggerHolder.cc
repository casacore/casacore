...
void doIt(Bool tempClose) {
  ...
  AlwaysAssertExit(logger2.isTempClosed() == tempClose);
  nmsg = 0;
  for (LoggerHolder::const_iterator iter = sublogger.begin(); iter != sublogger.end(); iter++) {
    cout << iter->time() << ' ' << (*iter).message() << endl;
    nmsg++;
  }
  AlwaysAssertExit(nmsg == 1);
  nmsg = 0;
  for (LoggerHolder::const_iterator iter = sublogger1.begin(); iter != sublogger1.end(); iter++) {
    cout << iter->time() << ' ' << (*iter).message() << endl;
    nmsg++;
  }
