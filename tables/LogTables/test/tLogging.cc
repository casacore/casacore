...
void testLogFilter() {
  ...
  LogFilterTaql expr1("LOCATION=='::abc' && !(PRIORITY in ['INFO','DEBUGGING'])");
  LogFilterTaql expr2("LOCATION=='::abc' && PRIORITY in ['INFO','DEBUGGING']");
  LogFilterTaql expr(expr2);
  expr = expr1;
  AlwaysAssertExit(!expr.pass(LogMessage(String("abc"), LogMessage::NORMAL)));
  AlwaysAssertExit(!expr.pass(LogMessage(String("abc"), LogMessage::DEBUGGING)));
  AlwaysAssertExit(expr.pass(LogMessage(String("abc"), LogMessage::WARN)));
  AlwaysAssertExit(expr.pass(LogMessage(String("abc"), LogMessage::SEVERE)));
  AlwaysAssertExit(!expr.pass(LogMessage(String("abcd"), LogMessage::WARN)));
  AlwaysAssertExit(!expr.pass(LogMessage(String("abcd"), LogMessage::SEVERE)));
...
void testLogMessage() {
  ...
  // LogMessage &operator=(const LogMessage &other);
  {
    ...
    AlwaysAssertExit(m1.priority() == LogMessage::SEVERE && m1.message() == "");
    AlwaysAssertExit(m1.origin().functionName() == "test");
    LogMessage m2("message", LogOrigin("test"), LogMessage::SEVERE);
    AlwaysAssertExit(m2.priority() == LogMessage::SEVERE && m2.message() == "message" &&
                     m2.origin().functionName() == "test");
    LogMessage m3(m2);
    AlwaysAssertExit(m3.priority() == m2.priority() && m3.message() == m2.message() &&
                     m3.origin().functionName() == m2.origin().functionName());
    LogMessage m4;
    m4 = m2;
    AlwaysAssertExit(m4.priority() == m2.priority() && m4.message() == m2.message() &&
  ...
  // global operator<<
  {
    ...
    AlwaysAssertExit(now - m.messageTime() >= 0 && now - m.messageTime() < 0.1);
    m.line(100);
...
void testLogOrigin() {
  ...
  {
    LogOrigin empty;
    AlwaysAssertExit(empty.functionName() == "" && empty.className() == "" &&
                     empty.objectID().isNull() && empty.line() == 0 && empty.fileName() == "");
  }
  ...
  //           uInt lineNumber = 0);
  {
    ...
    AlwaysAssertExit(global.functionName() == "global" && global.className() == "" &&
                     global.objectID().isNull() && global.line() == 10 &&
  ...
  // 	      const char *fileName=0, uInt lineNumber = 0);
  {
    ...
    AlwaysAssertExit(member.functionName() == "member" && member.className() == "class" &&
                     member.objectID().isNull() && member.line() == 10 &&
  ...
  // global ostream &operator<<(ostream &os, const LogOrigin &origin);
  {
    ...
    AlwaysAssertExit(distributed.functionName() == "member" && distributed.className() == "class" &&
                     distributed.objectID() == id && distributed.line() == 10 &&
                     distributed.fileName() == "file" && distributed.fullName() == "class::member");
    LogOrigin t1(distributed), t2;
    t2 = distributed;
    AlwaysAssertExit(distributed.functionName() == t1.functionName() &&
                     distributed.className() == t1.className() &&
                     distributed.objectID() == t1.objectID() && distributed.line() == t1.line() &&
    ...
    AlwaysAssertExit(distributed.functionName() == t2.functionName() &&
                     ...
                     distributed.objectID() == t2.objectID() && distributed.line() == t2.line() &&
                     distributed.fileName() == t2.fileName());
    LogOrigin t3;
    t3.functionName(t1.functionName())
    ...
    AlwaysAssertExit(distributed.functionName() == t3.functionName() &&
                     ...
                     distributed.objectID() == t3.objectID() && distributed.line() == t3.line() &&
                     distributed.fileName() == t3.fileName());
    String s = t3.toString();
    AlwaysAssertExit(s.find("class") != std::string::npos &&
                     s.find("member") != std::string::npos && s.find("file") != std::string::npos);
...
void testLogSink() {
  ...
  LogSinkInterface* newGlobal = new TableLogSink(LogMessage::SEVERE, tableNames[1]);
  ...
  ScalarColumn<String> messageColumn(logTable, TableLogSink::columnName(TableLogSink::MESSAGE));
  ScalarColumn<String> messageColumn2(logTable2, TableLogSink::columnName(TableLogSink::MESSAGE));
  AlwaysAssertExit(messageColumn(0) == "test");
  // LogSink(const LogSink &other);
  LogSink sink4(sink3);
  // LogSink &operator=(const LogSink &other);
  LogSink sink5(LogMessage::SEVERE);
  sink5 = sink3;
  sink4.post(message);
  sink5.post(message);
  ...
  AlwaysAssertExit(dynamic_cast<const LogFilter&>(sink3.filter()).lowestPriority() ==
                   LogMessage::SEVERE);
...
void testLogAny(LogSink& sink) {
  LogIO lio(sink);
  lio << "test " << LogIO::WARN << "message" << LogIO::SEVERE << LogIO::POST;
  ...
  sink.localSink().writeLocally(sink.getTime(0), sink.getMessage(1), sink.getPriority(0),
                                sink.getLocation(0), sink.getObjectID(0));
