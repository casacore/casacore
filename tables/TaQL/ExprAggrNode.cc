...
TableExprFuncNode::NodeDataType TableExprAggrNode::checkOperands(Block<Int>& dtypeOper,
  ...
  if (ftype >= FirstAggrArrayFunc && ftype < LastAggrArrayFunc && nodes.size() > 0 &&
      nodes[0]->valueType() != VTArray) {
  ...
  for (const TENShPtr& n : nodes) {
    if (!TableExprNodeUtil::getAggrNodes(n.get()).empty()) {
      throw TableInvExpr(
          "The argument of an aggregate function cannot use "
  ...
  switch (ftype) {
    ...
    case ghistFUNC:
      ...
      if (nodes[1]->dataType() != NTInt) {
        throw TableInvExpr(
            "2nd argument of function GHIST "
            "has to be a constant integer scalar");
      }
      for (int i = 1; i < 4; ++i) {
        if (nodes[i]->valueType() != VTScalar || !nodes[i]->isConstant()) {
          throw TableInvExpr(
              "2nd, 3rd and 4th argument of function GHIST "
    ...
    case gfractileFUNC:
      ...
      if (nodes[1]->valueType() != VTScalar || !nodes[1]->isConstant()) {
        throw TableInvExpr(
            "2nd argument of function GFRACTILE "
    ...
    default:
      throw TableInvExpr("Unhandled aggregate function " + std::to_string(ftype));
  }
}
    ...
    switch (operands()[0]->dataType()) {
      case NTBool:
        switch (funcType()) {
          ...
          default:
            throw TableInvExpr("Aggregate function " + std::to_string(funcType()) +
      ...
      case NTDouble:
        switch (funcType()) {
          ...
          case gfractileFUNC:
            return new TableExprGroupFractileDouble(this, operands()[1]->getDouble(0));
          case ghistFUNC:
            return new TableExprGroupHistDouble(this, operands()[1]->getInt(0),
  ...
  switch (operands()[0]->dataType()) {
    ...
    case NTDouble:
      switch (funcType()) {
        ...
        case gfractileFUNC:
          return new TableExprGroupFractileArrayDouble(this, operands()[1]->getDouble(0));
        default:
          throw TableInvExpr("Aggregate function " + std::to_string(funcType()) +
