...
Int64 TableExprConeNode::getInt(const TableExprId& id) {
  switch (funcType()) {
    ...
    default:
      throw(
          TableInvExpr("TableExprConeNode::getInt, "
                       "unknown function"));
  }
  return 0;
}
...
TableExprNodeRep::NodeDataType TableExprConeNode::checkOperands(Block<Int>& dtypeOper,
  ...
  switch (fType) {
    ...
    default:
      throw(
          TableInvExpr("TableExprConeNode::checkOperands, "
...
MArray<Bool> TableExprConeNodeArray::getArrayBool(const TableExprId& id) {
  switch (funcType()) {
    case TableExprFuncNode::conesFUNC: {
      ...
      for (size_t j = 0; j < srcArr.nelements(); j += 2) {
        ...
        for (size_t i = 0; i < coneArr.nelements(); i += 3) {
          ...
          const double radius = cone[i + 2];
          *res++ =
              cos(radius) <= (sin(decCone) * sindec + cos(decCone) * cosdec * cos(raCone - ra));
        }
      }
      srcArr.freeStorage(src, deleteSrc);
      coneArr.freeStorage(cone, deleteCone);
    ...
    default:
      throw(
          TableInvExpr("TableExprConeNodeArray::getArrayBool, "
                       "unknown function"));
  }
}
...
MArray<Int64> TableExprConeNodeArray::getArrayInt(const TableExprId& id) {
  switch (funcType()) {
      ...
      for (size_t j = 0; j < srcArr.nelements(); j += 2) {
        ...
        *res = -1;
        for (size_t i = 0; i < coneArr.nelements(); i += 3) {
          ...
          const double radius = cone[i + 2];
          if (cos(radius) <=
              (sin(decCone) * sin(dec) + cos(decCone) * cos(dec) * cos(raCone - ra))) {
            *res = origin_p + i / 3;
            break;
          }
        }
    ...
    default:
      throw(
          TableInvExpr("TableExprConeNodeArray::getArrayInt, "
                       "unknown function"));
  }
}
