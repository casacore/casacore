...
void TableExprFuncNode::fillChildNodes(const vector<TENShPtr>& nodes, const Block<Int>& dtypeOper) {
  ...
  for (uInt i = 0; i < nodes.size(); i++) {
    if (i < dtypeOper.size() && dtypeOper[i] == NTDate) {
      ...
      } else if (nodes[i]->dataType() == NTDouble || nodes[i]->dataType() == NTInt) {
...
void TableExprFuncNode::tryToConst() {
  switch (funcType_p) {
    ...
    case isdefFUNC:
    case isnullFUNC:
      if (operands_p[0]->ndim() == 0 || operands_p[0]->shape().size() > 0) {
        exprtype_p = Constant;
      }
      break;
    case iscolFUNC:
...
Bool TableExprFuncNode::getBool(const TableExprId& id) {
  switch (funcType_p) {
    ...
    case iskeyFUNC: {
      ...
      try {
        if (columnName.empty()) {
          rec = TableExprNode::findLastKeyRec(table_p.keywordSet(), fieldNames, fullName);
        } else {
          TableColumn column(table_p, columnName);
          const TableRecord& colkeys = column.keywordSet();
    ...
    case near2FUNC:
      if (argDataType_p == NTDouble) {
        return near(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id), 1.0e-13);
      }
      return near(operands_p[0]->getDComplex(id), operands_p[1]->getDComplex(id), 1.0e-13);
    case near3FUNC:
      if (argDataType_p == NTDouble) {
        return near(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id),
                    operands_p[2]->getDouble(id));
      }
      return near(operands_p[0]->getDComplex(id), operands_p[1]->getDComplex(id),
                  operands_p[2]->getDouble(id));
    case nearabs2FUNC:
      if (argDataType_p == NTDouble) {
        return nearAbs(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id), 1.0e-13);
      }
      return nearAbs(operands_p[0]->getDComplex(id), operands_p[1]->getDComplex(id), 1.0e-13);
    case nearabs3FUNC:
      if (argDataType_p == NTDouble) {
        return nearAbs(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id),
                       operands_p[2]->getDouble(id));
      }
      return nearAbs(operands_p[0]->getDComplex(id), operands_p[1]->getDComplex(id),
                     operands_p[2]->getDouble(id));
    case iifFUNC:
      return operands_p[0]->getBool(id) ? operands_p[1]->getBool(id) : operands_p[2]->getBool(id);
    default:
      throw(
          TableInvExpr("TableExprFuncNode::getBool, "
                       "unknown function " +
...
Int64 TableExprFuncNode::getInt(const TableExprId& id) {
  switch (funcType_p) {
    ...
    case minFUNC:
      return std::min(operands_p[0]->getInt(id), operands_p[1]->getInt(id));
    case maxFUNC:
    ...
    case nelemFUNC:
      return (operands_p[0]->valueType() == VTScalar ? 1 : operands_p[0]->shape(id).product());
    case iifFUNC:
      return operands_p[0]->getBool(id) ? operands_p[1]->getInt(id) : operands_p[2]->getInt(id);
    default:
      throw(
          TableInvExpr("TableExprFuncNode::getInt, "
                       "unknown function " +
                       std::to_string(funcType_p)));
  }
  return 0;
...
Double TableExprFuncNode::getDouble(const TableExprId& id) {
  ...
  switch (funcType_p) {
    ...
    case minFUNC:
      return min(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id));
    case maxFUNC:
    ...
    case fmodFUNC:
      return fmod(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id));
    case mjdFUNC:
    ...
    case arrfractileFUNC:
      if (operands_p[0]->valueType() == VTArray) {
        return fractile(operands_p[0]->getArrayDouble(id), operands_p[1]->getDouble(id));
      }
      return operands_p[0]->getDouble(id);
    case iifFUNC:
      return operands_p[0]->getBool(id) ? operands_p[1]->getDouble(id)
                                        : operands_p[2]->getDouble(id);
    case angdistFUNC:
    case angdistxFUNC: {
      MArray<double> a1 = operands_p[0]->getArrayDouble(id);
      MArray<double> a2 = operands_p[1]->getArrayDouble(id);
      if (!(a1.size() == 2 && a1.array().contiguousStorage() && a2.size() == 2 &&
            a2.array().contiguousStorage())) {
        throw TableInvExpr(
            "Arguments of function ANGDIST[x] must have a "
...
DComplex TableExprFuncNode::getDComplex(const TableExprId& id) {
  ...
  switch (funcType_p) {
    ...
    case powFUNC:
      return pow(operands_p[0]->getDComplex(id), operands_p[1]->getDComplex(id));
    case squareFUNC: {
    ...
    case complexFUNC:
      ...
      return DComplex(operands_p[0]->getDouble(id), operands_p[1]->getDouble(id));
    case arrsumFUNC:
      if (operands_p[0]->valueType() == VTArray) {
    ...
    case arrmeanFUNC:
      ...
      return operands_p[0]->getDComplex(id);
    case iifFUNC:
      return operands_p[0]->getBool(id) ? operands_p[1]->getDComplex(id)
                                        : operands_p[2]->getDComplex(id);
    default:
      throw(
          TableInvExpr("TableExprFuncNode::getDComplex, "
                       "unknown function " +
                       std::to_string(funcType_p)));
  }
  return DComplex(0., 0.);
  ...
  switch (funcType_p) {
    ...
    case stringFUNC: {
      ...
      if (operands_p[0]->dataType() == NTBool) {
        ...
        return stringValue(operands_p[0]->getDouble(id), fmt, width, prec, getMVFormat(fmt),
                           operands_p[0]->unit());
      } else if (operands_p[0]->dataType() == NTComplex) {
        return stringValue(operands_p[0]->getDComplex(id), fmt, width, prec);
      } else if (operands_p[0]->dataType() == NTDate) {
        return stringValue(operands_p[0]->getDate(id), fmt, width, getMVFormat(fmt));
      }
    ...
    case iifFUNC:
      return operands_p[0]->getBool(id) ? operands_p[1]->getString(id)
                                        : operands_p[2]->getString(id);
    default:
      throw(
          TableInvExpr("TableExprFuncNode::getString, "
                       "unknown function " +
                       std::to_string(funcType_p)));
  }
  return "";
...
TaqlRegex TableExprFuncNode::getRegex(const TableExprId& id) {
  ...
  throw(
      TableInvExpr("TableExprFuncNode::getRegex, "
                   "unknown function " +
                   std::to_string(funcType_p)));
}
...
MVTime TableExprFuncNode::getDate(const TableExprId& id) {
  switch (funcType_p) {
    case datetimeFUNC: {
      ...
      throw(TableInvExpr("invalid date string " + operands_p[0]->getString(id)));
    }
    case mjdtodateFUNC:
    ...
    default:
      throw(
          TableInvExpr("TableExprFuncNode::getDate, "
                       "unknown function " +
                       std::to_string(funcType_p)));
  }
  return MVTime();
...
String TableExprFuncNode::stringValue(Double val, const String& fmt, Int width, Int prec,
  ...
  if (mvFormat.first >= 0) {
    ...
    return stringAngle(val, mvFormat.second, MVAngle::formatTypes(mvFormat.first));
  }
  return FormatString(fmt.c_str(), val);
}
...
String TableExprFuncNode::stringValue(const MVTime& val, const String& fmt, Int width,
  ...
  if (mvFormat.first >= 0) {
    return stringDT(val, mvFormat.second, MVTime::formatTypes(mvFormat.first));
  }
...
TableExprNodeRep::NodeDataType TableExprFuncNode::checkOperands(Block<Int>& dtypeOper,
  ...
  switch (fType) {
    ...
    case arrfractileFUNC:
      ...
      if (nodes[1]->valueType() != VTScalar) {
        throw TableInvExpr(
            "2nd argument of function FRACTILE "
            "has to be a scalar");
      }
      return checkDT(dtypeOper, NTReal, NTDouble, nodes);
    case arrsumFUNC:
    ...
    case angdistxFUNC:
      checkNumOfArg(2, 2, nodes);
      if (nodes[0]->valueType() != VTArray || nodes[1]->valueType() != VTArray) {
        throw TableInvExpr(
            "Arguments of function ANGDIST[x] "
            "have to be arrays");
      }
      if (nodes[0]->shape().product() != 2 || nodes[1]->shape().product() != 2) {
    ...
    case diagonalFUNC: {
      ...
      if (fType != arrayFUNC) {
        if (nodes[0]->valueType() != VTArray) {
          throw TableInvExpr("1st argument of function nr " + std::to_string(fType) +
                             " has to be an array");
        }
      }
      ...
      if (nodes[axarg]->dataType() != NTInt) {
        throw TableInvExpr(
            "The axes arguments of RUNNINGxxx, BOXEDxxx, "
            "or XXXs function " +
            std::to_string(fType) + " have to be integers");
      }
      ...
      if (nodes.size() == axarg + optarg + 1) {
        if (nodes[axarg + optarg]->dataType() != NTInt) {
          throw TableInvExpr(
              "3rd argument of function RESIZE"
              " has to be integer");
        }
      }
      return dtout;
  ...
  switch (fType) {
    ...
    case replaceFUNC:
      ...
      if (nodes[1]->valueType() != VTScalar ||
          (nodes[1]->dataType() != NTString && nodes[1]->dataType() != NTRegex)) {
        throw TableInvExpr(
            "2nd argument of function REPLACE "
            "has to be a string or regex");
      }
      if (nodes.size() == 3) {
        if (nodes[2]->valueType() != VTScalar || nodes[2]->dataType() != NTString) {
          throw TableInvExpr(
    ...
    case stringFUNC:
      if (checkNumOfArg(1, 2, nodes) == 2) {
        if ((nodes[1]->dataType() != NTString && nodes[1]->dataType() != NTDouble &&
             nodes[1]->dataType() != NTInt) ||
            nodes[1]->valueType() != VTScalar) {
          throw TableInvExpr(
              "2nd argument of function STRING "
    ...
    case nearabs3FUNC: {
      ...
      if (nodes[2]->valueType() != VTScalar) {
        throw TableInvExpr(
            "3rd argument of function NEAR and NEARABS "
            "has to be a scalar");
      }
      return checkDT(dtypeOper, NTNumeric, NTBool, nodes);
    }
  ...
  switch (fType) {
    ...
    default:
      throw(
          TableInvExpr("TableExprFuncNode::checkOperands, "
                       "function nr " +
                       std::to_string(fType) + " not contained in switch statement"));
  }
  return NTNumeric;
}
