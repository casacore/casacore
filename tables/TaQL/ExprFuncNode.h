...
class TableExprFuncNode : public TableExprNodeMulti {
 public:
  ...
  // <group>
  const std::vector<TENShPtr>& operands() const { return operands_p; }
  std::vector<TENShPtr>& rwOperands() { return operands_p; }
  FunctionType funcType() const { return funcType_p; }
  NodeDataType argDataType() const { return argDataType_p; }
  // </group>
