...
class TableExprFuncNodeArray : public TableExprNodeArray {
 public:
  ...
  TableExprFuncNode* getChild() { return &node_p; }
  const TableExprFuncNode* getChild() const { return &node_p; }
 ...
 protected:
  ...
  // <group>
  const std::vector<TENShPtr>& operands() const { return node_p.operands(); }
  std::vector<TENShPtr>& rwOperands() { return node_p.rwOperands(); }
  TableExprFuncNode::FunctionType funcType() const { return node_p.funcType(); }
  NodeDataType argDataType() const { return node_p.argDataType(); }
  // </group>
