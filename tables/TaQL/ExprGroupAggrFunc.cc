...
TableExprGroupCount::TableExprGroupCount(TableExprNodeRep* node)
  ...
  if (!itsColumn) {
    TableExprNodeColumn* col = dynamic_cast<TableExprNodeColumn*>(itsOperand);
    if (!col) {
...
TableExprGroupSumInt::TableExprGroupSumInt(TableExprNodeRep* node) : TableExprGroupFuncInt(node) {}
TableExprGroupSumInt::~TableExprGroupSumInt() {}
void TableExprGroupSumInt::apply(const TableExprId& id) { itsValue += itsOperand->getInt(id); }
...
void TableExprGroupProductInt::apply(const TableExprId& id) { itsValue *= itsOperand->getInt(id); }
