...
void TEGMeanAdd(const MArray<T>& src, Array<T>& dst, Array<Int64>& nr) {
  ...
  if (src.hasMask()) {
    ...
    typename Array<T>::const_iterator in = src.array().begin();
    for (typename Array<T>::contiter out = dst.cbegin(); out != dst.cend(); ++in, ++out, ++itn) {
      *out += *in;
      (*itn)++;
    }
  }
...
TableExprGroupMeanArrayDComplex::TableExprGroupMeanArrayDComplex(TableExprNodeRep* node)
    : TableExprGroupFuncDComplex(node), itsNr(0) {}
...
void TableExprGroupMinsArrayInt::finish() { TEGClearMasked(itsValue); }
...
void TableExprGroupMaxsArrayInt::finish() { TEGClearMasked(itsValue); }
...
void TableExprGroupProductsArrayInt::finish() { TEGClearMasked(itsValue); }
...
void TableExprGroupMinsArrayDouble::finish() { TEGClearMasked(itsValue); }
...
void TableExprGroupMaxsArrayDouble::finish() { TEGClearMasked(itsValue); }
...
void TableExprGroupProductsArrayDouble::finish() { TEGClearMasked(itsValue); }
...
void TableExprGroupProductsArrayDComplex::finish() { TEGClearMasked(itsValue); }
