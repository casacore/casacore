// # ExprMathNode.cc: Nodes representing scalar mathematical operators in table select expression
// tree # Copyright (C) 1994,1995,1996,1997,1998,1999,2000 # Associated Universities, Inc.
// Washington DC, USA.
// #
...
TableExprNodePlus::TableExprNodePlus(NodeDataType dt, const TableExprNodeRep& node)
...
TableExprNodeMinus::TableExprNodeMinus(NodeDataType dt, const TableExprNodeRep& node)
...
TableExprNodeTimes::TableExprNodeTimes(NodeDataType dt, const TableExprNodeRep& node)
...
TableExprNodeDivide::TableExprNodeDivide(NodeDataType dt, const TableExprNodeRep& node)
...
void TableExprNodeDivide::handleUnits() {
  if (lnode_p->unit().empty()) {
    ...
    if (rnode_p->isConstant() &&
        ...
        rnode_p->getDouble(0) == 86400. && lnode_p->unit().getName() == "s") {
      setUnit("d");
    } else {
      setUnit(lnode_p->unit());
    }
...
TableExprNodeModulo::TableExprNodeModulo(NodeDataType dt, const TableExprNodeRep& node)
    : TableExprNodeBinary(dt, node, OtModulo) {}
TableExprNodeModulo::~TableExprNodeModulo() {}
void TableExprNodeModulo::handleUnits() { TableExprNodeBinary::handleUnits(); }
