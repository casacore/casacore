...
TableExprNode::TableExprNode(const TENShPtr& node) : node_p(node) {}

TableExprNode::TableExprNode(const TableExprNode& node) : node_p(node.node_p) {}
...
TableExprNode TableExprNode::in(const TableExprNodeSet& set, const TaQLStyle& style) const {
...
TENShPtr TableExprNode::newIN(const TENShPtr& right, const TaQLStyle& style) const {
  ...
  if (vtRight == TableExprNodeRep::VTScalar) {
    ...
    TableExprNodeSet* set = dynamic_cast<TableExprNodeSet*>(right.get());
    if (set) {
      if (set->isSingle() && set->size() == 1 && !set->hasArrays()) {
        TENShPtr snode = (*set)[0]->start();
        return newEQ(snode);
      }
    } else {
      TableExprNodeArray* arr = dynamic_cast<TableExprNodeArray*>(right.get());
      if (arr) {
    ...
    throw(TableInvExpr("Right operand of IN has to be a scalar, array or set"));
  }
  ...
  if (dtype != rdtype) {
    if ((dtype == TableExprNodeRep::NTInt && rdtype == TableExprNodeRep::NTDouble) ||
        (dtype == TableExprNodeRep::NTDouble && rdtype == TableExprNodeRep::NTInt)) {
  ...
  TableExprNodeRep node(dtype, node_p->valueType(), TableExprNodeRep::OtIN, TableExprNodeRep::NoArr,
...
TableExprNode TableExprNode::operator+() const { return *this; }
...
TableExprNode TableExprNode::keyCol(const TableExprInfo& tabInfo, const String& name,
...
TableExprNode TableExprNode::newColumnNode(const TableExprInfo& tableInfo, const String& name,
                                           const Vector<String>& fieldNames) {
  ...
  if (fieldNames.size() > 0 && coldes.dataType() != TpRecord) {
    throw(TableInvExpr("Column " + name +
  ...
  if (coldes.isArray()) {
    ...
    if (coldes.dataType() == TpRecord) {
      throw(TableInvExpr("Sorry, column " + name +
                         " contains records, "
...
TableExprNode TableExprNode::newKeyConst(const TableRecord& keyset,
  ...
  switch (ks->dataType(fieldnr)) {
    ...
    case TpArrayBool:
      ...
      break;
    case TpArrayString:
      tsnptr = std::make_shared<TableExprNodeArrayConstString>(ks->asArrayString(name));
      break;
    case TpArrayComplex:
      tsnptr = std::make_shared<TableExprNodeArrayConstDComplex>(ks->asArrayComplex(name));
      break;
    case TpArrayDComplex:
      tsnptr = std::make_shared<TableExprNodeArrayConstDComplex>(ks->asArrayDComplex(name));
      break;
    case TpArrayUChar:
      tsnptr = std::make_shared<TableExprNodeArrayConstInt>(ks->asArrayuChar(name));
      break;
    case TpArrayShort:
      tsnptr = std::make_shared<TableExprNodeArrayConstInt>(ks->asArrayShort(name));
      break;
    case TpArrayInt:
      tsnptr = std::make_shared<TableExprNodeArrayConstInt>(ks->asArrayInt(name));
      break;
    case TpArrayUInt:
      tsnptr = std::make_shared<TableExprNodeArrayConstInt>(ks->asArrayuInt(name));
      break;
    case TpArrayInt64:
      tsnptr = std::make_shared<TableExprNodeArrayConstInt>(ks->asArrayInt64(name));
      break;
    case TpArrayFloat:
      tsnptr = std::make_shared<TableExprNodeArrayConstDouble>(ks->asArrayFloat(name));
      break;
    case TpArrayDouble:
      tsnptr = std::make_shared<TableExprNodeArrayConstDouble>(ks->asArrayDouble(name));
      break;
    case TpRecord:
      throw(TableInvExpr("Keyword " + fullName +
                         " contains records, "
    ...
    case TpTable:
      ...
      break;
    default:
      throw(TableInvExpr("keyword " + fullName + " has unknown data type"));
  }
  return tsnptr;
}
...
TableExprNode diagonal(const TableExprNode& array, const TableExprNode& firstAxis) {
  ...
  return TableExprNode::newFunctionNode(TableExprFuncNode::diagonalFUNC, array, set);
}
TableExprNode diagonal(const TableExprNode& array, const TableExprNode& firstAxis,
...
TableExprNode TableExprNode::newFunctionNode(TableExprFuncNode::FunctionType ftype,
...
TableExprNode TableExprNode::newFunctionNode(TableExprFuncNode::FunctionType ftype,
                                             const TableExprNode& node1, const TableExprNode& node2,
...
TableExprNode TableExprNode::newFunctionNode(TableExprFuncNode::FunctionType ftype,
                                             const TableExprNode& array, const TableExprNode& node,
                                             const TableExprNodeSet& axes) {
  TableExprNodeSet set;
  set.add(TableExprNodeSetElem(array));
  set.add(TableExprNodeSetElem(node));
...
TableExprNode TableExprNode::newFunctionNode(TableExprFuncNode::FunctionType ftype,
                                             ...
                                             const TableExprInfo& tabInfo, const TaQLStyle& style) {
  ...
  if (ftype >= TableExprFuncNode::FirstAggrFunc) {
    ...
    if (resVT == TableExprNodeRep::VTScalar) {
      fnode = std::make_shared<TableExprAggrNode>(ftype, resDT, resVT, set, par, dtypeOper);
    } else {
      fnode =
          std::make_shared<TableExprAggrNodeArray>(ftype, resDT, resVT, set, par, dtypeOper, style);
    }
  } else {
    resDT = TableExprFuncNode::checkOperands(dtypeOper, resVT, vtypeOper, ftype, par);
    if (resVT == TableExprNodeRep::VTScalar) {
      auto node =
          std::make_shared<TableExprFuncNode>(ftype, resDT, resVT, set, par, dtypeOper, tabInfo);
      fnode = node;
      ...
      fnode =
          std::make_shared<TableExprFuncNodeArray>(ftype, resDT, resVT, set, par, dtypeOper, style);
    }
  }
  return TableExprNodeRep::replaceConstNode(fnode);
}

TableExprNode TableExprNode::newUDFNode(const String& name, const TableExprNodeSet& set,
...
TableExprNode TableExprNode::newConeNode(TableExprFuncNode::FunctionType ftype,
  ...
  TENShPtr fnode;
  if (resVT == TableExprNodeRep::VTScalar) {
    fnode = std::make_shared<TableExprConeNode>(ftype, resDT, set, par, dtypeOper, origin);
  } else {
    fnode = std::make_shared<TableExprConeNodeArray>(ftype, resDT, set, par, dtypeOper, origin);
  }
  return TableExprNodeRep::replaceConstNode(fnode);
}
