...
TableExprNodeArray::TableExprNodeArray(const TableExprNodeRep& node, NodeDataType dtype,
                                       OperType otype)
    : TableExprNodeBinary(dtype, node, otype) {}
TableExprNodeArray::TableExprNodeArray(NodeDataType dtype, OperType otype, const IPosition& shape)
    : TableExprNodeBinary(dtype, VTArray, otype, Constant) {
  shape_p = shape;
  ndim_p = shape.size();
  if (ndim_p == 0) {
...
String TableExprNodeArray::getElemString(const TableExprId& id, const Slicer& slicer) {
  MArray<String> arr = getArrayString(id);
  return arr.array()(validateIndex(slicer.start(), arr.array()));
}
MVTime TableExprNodeArray::getElemDate(const TableExprId& id, const Slicer& slicer) {
...
MArray<DComplex> TableExprNodeArray::getSliceDComplex(const TableExprId& id, const Slicer& slicer) {
  MArray<DComplex> arr = getArrayDComplex(id);
  if (arr.isNull()) {
    return arr;
  }
...
MArray<String> TableExprNodeArray::getSliceString(const TableExprId& id, const Slicer& slicer) {
  MArray<String> arr = getArrayString(id);
  if (arr.isNull()) {
    return arr;
  }
...
MArray<MVTime> TableExprNodeArray::getSliceDate(const TableExprId& id, const Slicer& slicer) {
  MArray<MVTime> arr = getArrayDate(id);
  if (arr.isNull()) {
    return arr;
  }
...
Array<DComplex> TableExprNodeArray::getElemColumnDComplex(const Vector<rownr_t>&, const Slicer&) {
  TableExprNode::throwInvDT("(getElemColumnDComplex(Slicer) not implemented)");
  return Array<DComplex>();
}
Array<String> TableExprNodeArray::getElemColumnString(const Vector<rownr_t>&, const Slicer&) {
  TableExprNode::throwInvDT("(getElemColumnString(Slicer) not implemented)");
  return Array<String>();
}
...
TableExprNodeArrayColumn::TableExprNodeArrayColumn(const TableColumn& tablecol,
  ...
  switch (tabCol_p.columnDesc().dataType()) {
    ...
    case TpDouble:
      ...
      break;
    default:
      throw(TableInvExpr(tabCol_p.columnDesc().name(), "unknown data type"));
  }
  exprtype_p = Variable;
...
TableExprNodeArrayColumnBool::TableExprNodeArrayColumnBool(const TableColumn& col,
                                                           const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnuChar::TableExprNodeArrayColumnuChar(const TableColumn& col,
                                                             const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnShort::TableExprNodeArrayColumnShort(const TableColumn& col,
                                                             const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnuShort::TableExprNodeArrayColumnuShort(const TableColumn& col,
                                                               const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnInt::TableExprNodeArrayColumnInt(const TableColumn& col,
                                                         const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnuInt::TableExprNodeArrayColumnuInt(const TableColumn& col,
                                                           const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnInt64::TableExprNodeArrayColumnInt64(const TableColumn& col,
                                                             const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnFloat::TableExprNodeArrayColumnFloat(const TableColumn& col,
                                                             const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnDouble::TableExprNodeArrayColumnDouble(const TableColumn& col,
                                                               const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
TableExprNodeArrayColumnComplex::TableExprNodeArrayColumnComplex(const TableColumn& col,
                                                                 const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
DComplex TableExprNodeArrayColumnComplex::getElemDComplex(const TableExprId& id,
                                                          const Slicer& index) {
  Array<Complex> arr = col_p.getSlice(id.rownr(), index);
  return *arr.data();
}
MArray<DComplex> TableExprNodeArrayColumnComplex::getArrayDComplex(const TableExprId& id) {
  if (tabCol_p.isDefined(id.rownr())) {
...
MArray<DComplex> TableExprNodeArrayColumnComplex::getSliceDComplex(const TableExprId& id,
                                                                   const Slicer& index) {
  if (tabCol_p.isDefined(id.rownr())) {
    Array<Complex> arr = col_p.getSlice(id.rownr(), index);
    Array<DComplex> out(arr.shape());
...
Array<Complex> TableExprNodeArrayColumnComplex::getElemColumnComplex(const Vector<rownr_t>& rownrs,
                                                                     const Slicer& index) {
  return col_p.getColumnCells(rownrs, index);
}

TableExprNodeArrayColumnDComplex::TableExprNodeArrayColumnDComplex(const TableColumn& col,
                                                                   const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
DComplex TableExprNodeArrayColumnDComplex::getElemDComplex(const TableExprId& id,
                                                           const Slicer& index) {
  Array<DComplex> arr = col_p.getSlice(id.rownr(), index);
  return *arr.data();
}
MArray<DComplex> TableExprNodeArrayColumnDComplex::getArrayDComplex(const TableExprId& id) {
  if (tabCol_p.isDefined(id.rownr())) {
    return MArray<DComplex>(col_p(id.rownr()));
  }
  return MArray<DComplex>();
}
MArray<DComplex> TableExprNodeArrayColumnDComplex::getSliceDComplex(const TableExprId& id,
                                                                    const Slicer& index) {
  if (tabCol_p.isDefined(id.rownr())) {
...
TableExprNodeArrayColumnString::TableExprNodeArrayColumnString(const TableColumn& col,
                                                               const TableExprInfo& tableInfo)
    : TableExprNodeArrayColumn(col, tableInfo), col_p(col) {}
...
String TableExprNodeArrayColumnString::getElemString(const TableExprId& id, const Slicer& index) {
  Array<String> arr = col_p.getSlice(id.rownr(), index);
  return *arr.data();
}
MArray<String> TableExprNodeArrayColumnString::getArrayString(const TableExprId& id) {
  if (tabCol_p.isDefined(id.rownr())) {
    return MArray<String>(col_p(id.rownr()));
  }
  return MArray<String>();
}
MArray<String> TableExprNodeArrayColumnString::getSliceString(const TableExprId& id,
                                                              const Slicer& index) {
  if (tabCol_p.isDefined(id.rownr())) {
...
Array<String> TableExprNodeArrayColumnString::getElemColumnString(const Vector<rownr_t>& rownrs,
                                                                  const Slicer& index) {
  return col_p.getColumnCells(rownrs, index);
}
...
TableExprNodeIndex::TableExprNodeIndex(const TableExprNodeSet& indices, const TaQLStyle& style)
...
MArray<DComplex> TableExprNodeArrayPart::getArrayDComplex(const TableExprId& id) {
  DebugAssert(valueType() == VTArray, AipsError);
