...
TableExprNodeRecordField::TableExprNodeRecordField(DataType dtype, const Block<Int>& fieldNumbers)
...
Bool TableExprNodeRecordField::isDefined(const TableExprId& id) {
  ...
  switch (dtype_p) {
    case NTBool:
      return dtype == TpBool;
    case NTInt:
      return dtype == TpUChar || dtype == TpShort || dtype == TpInt || dtype == TpUInt ||
             dtype == TpInt64;
    case NTDouble:
      return dtype == TpUChar || dtype == TpShort || dtype == TpInt || dtype == TpUInt ||
             dtype == TpInt64 || dtype == TpFloat || dtype == TpDouble;
    case NTComplex:
      return dtype == TpUChar || dtype == TpShort || dtype == TpInt || dtype == TpUInt ||
             dtype == TpInt64 || dtype == TpFloat || dtype == TpDouble || dtype == TpComplex ||
             dtype == TpDComplex;
    case NTString:
...
const RecordInterface& TableExprNodeRecordField::getRecord(const TableExprId& id) const {
  const RecordInterface* recPtr = &(id.record());
  for (uInt i = 0; i < lastEntry_p; i++) {
    recPtr = &(recPtr->asRecord(fieldNrs_p[i]));
  }
...
TableExprNodeRecordFieldArray::TableExprNodeRecordFieldArray(DataType dtype,
...
const IPosition& TableExprNodeRecordFieldArray::getShape(const TableExprId& id) {
  varShape_p.resize(0);
  if (id.byData()) {
...
Bool TableExprNodeRecordFieldArray::isDefined(const TableExprId& id) {
  ...
  switch (dtype_p) {
    ...
    case NTInt:
      return dtype == TpArrayUChar || dtype == TpArrayShort || dtype == TpArrayInt ||
             dtype == TpArrayUInt || dtype == TpArrayInt64;
    case NTDouble:
      return dtype == TpArrayUChar || dtype == TpArrayShort || dtype == TpArrayInt ||
             dtype == TpArrayUInt || dtype == TpArrayInt64 || dtype == TpArrayFloat ||
             dtype == TpArrayDouble;
    case NTComplex:
      return dtype == TpArrayUChar || dtype == TpArrayShort || dtype == TpArrayInt ||
             dtype == TpArrayUInt || dtype == TpArrayInt64 || dtype == TpArrayFloat ||
             dtype == TpArrayDouble || dtype == TpArrayComplex || dtype == TpArrayDComplex;
    case NTString:
...
MArray<DComplex> TableExprNodeRecordFieldArray::getArrayDComplex(const TableExprId& id) {
  if (id.byData()) {
...
MArray<String> TableExprNodeRecordFieldArray::getArrayString(const TableExprId& id) {
  if (id.byData()) {
...
const RecordInterface& TableExprNodeRecordFieldArray::getRecord(const TableExprId& id) const {
  const RecordInterface* recPtr = &(id.record());
  for (uInt i = 0; i < lastEntry_p; i++) {
    recPtr = &(recPtr->asRecord(fieldNrs_p[i]));
  }
