...
void TableExprNodeRep::flattenTree(std::vector<TableExprNodeRep*>& nodes) { nodes.push_back(this); }
...
Array<Complex> TableExprNodeRep::getColumnComplex(const Vector<rownr_t>&) {
  TableExprNode::throwInvDT("(getColumnComplex not implemented)");
  return Array<Complex>();
}
Array<DComplex> TableExprNodeRep::getColumnDComplex(const Vector<rownr_t>& rownrs) {
  TableExprId id;
  rownr_t nrrow = rownrs.size();
...
Array<String> TableExprNodeRep::getColumnString(const Vector<rownr_t>& rownrs) {
  TableExprId id;
  rownr_t nrrow = rownrs.size();
...
TableExprNodeBinary::TableExprNodeBinary(NodeDataType tp, ValueType vtype, OperType optype,
                                         ExprType exprtype)
    : TableExprNodeRep(tp, vtype, optype, exprtype) {}

TableExprNodeBinary::TableExprNodeBinary(NodeDataType tp, const TableExprNodeRep& that,
                                         OperType oper)
...
TableExprNodeRep::NodeDataType TableExprNodeBinary::getDT(NodeDataType leftDtype,
                                                          NodeDataType rightDtype, OperType opt) {
  // Equal types is mostly fine.
  if (leftDtype == rightDtype) {
    if (leftDtype == NTBool || leftDtype == NTDouble || leftDtype == NTComplex ||
        leftDtype == NTString || (leftDtype == NTInt && opt != OtDivide)) {
      return leftDtype;
    }
  }
  ...
  TableExprNode::throwInvDT(
      "TableExprNodeBinary::getDT cannot combine "
      "arguments with data type " +
      typeString(leftDtype) + " and " + typeString(rightDtype));
  return NTComplex;  // compiler satisfaction
}

TableExprNodeRep TableExprNodeBinary::getCommonTypes(const TENShPtr& left, const TENShPtr& right,
                                                     OperType opt) {
...
const Unit& TableExprNodeBinary::makeEqualUnits(const TENShPtr& left, TENShPtr& right) {
...
void TableExprNodeBinary::adaptDataTypes() {
  ...
  if ((*otherNode)->dataType() == NTDouble) {
    ...
    newType = NTComplex;
    if (((*constNode)->dataType() != NTInt) && ((*constNode)->dataType() != NTDouble)) {
      return;
    }
  } else {
    return;
  ...
  if (vtype == VTScalar) {
    ...
    if (newType == NTDouble) {
      newNode = std::make_shared<TableExprNodeArrayConstDouble>((*constNode)->getArrayDouble(0));
    } else {
      newNode = std::make_shared<TableExprNodeArrayConstDComplex>((*constNode)->getArrayDouble(0));
    }
  }
...
TableExprNodeMulti::TableExprNodeMulti(NodeDataType tp, ValueType vtype, OperType oper,
...
TableExprNodeRep::NodeDataType TableExprNodeMulti::checkDT(Block<Int>& dtypeOper, NodeDataType dtIn,
  ...
  if (dtIn == NTAny) {
    if (dtOut != NTAny) {
      ...
      AlwaysAssert(dtOut != NTNumeric && dtOut != NTReal && dtOut != NTDouCom, AipsError);
      return dtOut;
    }
    ...
    for (uInt i = 0; i < nelem; i++) {
      ...
      if (nodes[i]->dataType() != dtIn) {
        if (dateConv && dtIn == NTDate) {
          if (nodes[i]->dataType() != NTString && nodes[i]->dataType() != NTDouble &&
          ...
          TableExprNode::throwInvDT("function argument is not " + typeString(dtIn));
        }
      }
    }
  }
