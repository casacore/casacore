...
class TaqlRegex {
 public:
  ...
  explicit TaqlRegex(const Regex& regex) : itsRegex(regex) {}
  ...
  const Regex& regex() const { return itsRegex; }
...
class TableExprNodeBinary : public TableExprNodeRep {
 public:
  ...
  static TableExprNodeRep getCommonTypes(const TENShPtr& left, const TENShPtr& right,
                                         OperType operType);
  ...
  // Get the child nodes.
  // <group>
  const TENShPtr& getLeftChild() const { return lnode_p; }
  const TENShPtr& getRightChild() const { return rnode_p; }
  // </group>
...
inline void TableExprNodeRep::setValueType(TableExprNodeRep::ValueType vtype) { vtype_p = vtype; }

// # Get the operator type of the node.
inline TableExprNodeRep::OperType TableExprNodeRep::operType() const { return optype_p; }
...
inline const Record& TableExprNodeRep::attributes() const { return attributes_p; }
