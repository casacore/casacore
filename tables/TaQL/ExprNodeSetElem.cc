...
TENShPtr TableExprNodeSetElemBase::evalExpr(const TENShPtr& expr, const TableExprId& id) const {
  if (!expr) {
    return TENShPtr();
  }
  TENShPtr res;
...
TableExprNodeSetElemSingle::TableExprNodeSetElemSingle(const TableExprNodeSetElemSingle& that,
                                                       const TENShPtr& value)
    : TableExprNodeSetElemBase(that.dataType()) {
  itsStart = value;
  setUnit(itsStart->unit());
}

TENSEBShPtr TableExprNodeSetElemSingle::evaluate(const TableExprId& id) const {
...
TableExprNodeSetElemDiscrete::TableExprNodeSetElemDiscrete(const TableExprNode& start,
  ...
  if ((dts != NTInt && dts != NTDouble && dts != NTDate) || dte != dts ||
      (dti != NTInt && dti != NTDouble)) {
    throw TableInvExpr(
        "start:end should have equal data types (Int, Double"
...
TableExprNodeSetElemDiscrete::TableExprNodeSetElemDiscrete(const TableExprNodeSetElemDiscrete& that,
                                                           const TENShPtr& start,
                                                           const TENShPtr& end,
                                                           const TENShPtr& incr)
    : TableExprNodeSetElemBase(that.dataType()), itsEndExcl(that.itsEndExcl) {
  itsStart = start;
  itsEnd = end;
  itsIncr = incr;
  setUnit(that.unit());
...
TENSEBShPtr TableExprNodeSetElemDiscrete::evaluate(const TableExprId& id) const {
  return TENSEBShPtr(new TableExprNodeSetElemDiscrete(*this, evalExpr(itsStart, id),
...
TENSEBShPtr TableExprNodeSetElemCont::evaluate(const TableExprId& id) const {
  return TENSEBShPtr(
...
TableExprNodeSetElem::TableExprNodeSetElem(const TENSEBShPtr& elem)
    : TableExprNodeRep(NTBool, VTSetElem, OtUndef, Constant), itsElem(elem) {
  init();
}
...
TableExprNodeSetElem::TableExprNodeSetElem(const TableExprNode& mid, const TableExprNode& width)
    : TableExprNodeRep(NTBool, VTSetElem, OtUndef, Constant),
