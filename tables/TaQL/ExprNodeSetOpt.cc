...
template <typename T>
TableExprNodeSetOptContSetBase<T>::TableExprNodeSetOptContSetBase(const TableExprNodeSet& orig,
                                                                  const std::vector<T>& starts,
                                                                  const std::vector<T>& ends)
    : TableExprNodeSetOptBase(orig), itsStarts(starts), itsEnds(ends) {
...
TENShPtr TableExprNodeSetOptContSetBase<T>::transform(const TableExprNodeSet& set, Bool combine) {
  ...
  if (!combine) {
    ...
    for (size_t i = 1; i < index.size(); i++) {
      ...
      if (st2 < endval ||
          (st2 == endval && (set[inx]->isLeftClosed() || set[index[i - 1]]->isRightClosed()))) {
...
TENShPtr TableExprNodeSetOptContSetBase<T>::createOptSet(const TableExprNodeSet& set,
  ...
  if (same) {
    ...
    optSet = new TableExprNodeSetOptContSetMixOC<T>(set, start, end, leftC, rightC);
  }
...
TableExprNodeSetOptContSet<T, LeftComp, RightComp>::TableExprNodeSetOptContSet(
    const TableExprNodeSet& orig, const std::vector<T>& starts, const std::vector<T>& ends,
    LeftComp leftCmp, RightComp rightCmp, const String& cmpType)
