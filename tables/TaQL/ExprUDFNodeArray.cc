...
TableExprUDFNodeArray::TableExprUDFNodeArray(const std::shared_ptr<UDFBase>& udf,
                                             const TableExprInfo& tabInfo, const TableExprNodeSet&)
    : TableExprNodeArray(udf->dataType(), OtFunc), itsTableInfo(tabInfo), itsUDF(udf) {
...
TableExprInfo TableExprUDFNodeArray::getTableInfo() const { return itsTableInfo; }

void TableExprUDFNodeArray::disableApplySelection() { itsUDF->disableApplySelection(); }
