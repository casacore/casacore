...
TableExprNodeUnit::TableExprNodeUnit(const TENShPtr& child, const Unit& unit)
...
Double TableExprNodeUnit::set(TableExprNodeRep& parent, const TENShPtr& child, const Unit& unit) {
  ...
  if (unit.empty()) {
    ...
    if (!child->unit().empty()) {
      ...
      if (!(type1 == type2 || (type1 == UnitVal::ANGLE && type2 == UnitVal::TIME) ||
            (type2 == UnitVal::ANGLE && type1 == UnitVal::TIME))) {
        throw TableInvExpr("Units " + unit.getName() + " and " + child->unit().getName() +
                           " do not conform");
      }
...
TENShPtr TableExprNodeUnit::useUnit(const TENShPtr& node, const Unit& unit) {
...
void TableExprNodeUnit::adaptUnit(TENShPtr& node, const Unit& unit) {
  // See if a conversion is needed.
  // If so, adapt the reference counts and replace it.
  node = useUnit(node, unit);
}

Unit TableExprNodeUnit::adaptUnits(TENShPtr& node1, TENShPtr& node2, TENShPtr& node3) {
...
TableExprNodeArrayUnit::TableExprNodeArrayUnit(const TENShPtr& child, const Unit& unit)
