...
class MArray : public MArrayBase {
 public:
  ...
  MArray(const Array<T>& array, const MArrayBase& marray)
      : MArrayBase(array, marray), itsArray(array) {}
  ...
  // <group>
  const Array<T>& array() const { return itsArray; }
  Array<T>& array() { return itsArray; }
  // </group>
  ...
  MArray<T> operator()(const IPosition& start, const IPosition& end, const IPosition& stride) {
    if (hasMask()) {
      return MArray<T>(itsArray(start, end, stride), mask()(start, end, stride));
    }
