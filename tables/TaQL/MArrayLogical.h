  ...
  if (left.isNull() || right.isNull()) {
    ...
    if (right.hasMask()) {
      return compareAllMasked(left.array().begin(), left.array().end(), right.array.begin(),
                              left.mask().begin(), right.mask().begin(), std::equal_to<T>());
    } else {
      return compareAllMasked(left.array().begin(), left.array().end(), right.array.begin(),
                              left.mask().begin(), std::equal_to<T>());
    }
...
size_t ntrue(const MArray<T>& a) {
  if (a.hasMask()) {
    return a.array().contiguousStorage() && a.mask().contiguousStorage()
               ? countNEMasked<T>(a.array().cbegin(), a.array().cend(), a.mask().cbegin(), T())
               : countNEMasked<T>(a.array().begin(), a.array().end(), a.mask().begin(), T());
  }
...
size_t nfalse(const MArray<T>& a) {
  if (a.hasMask()) {
    return a.array().contiguousStorage() && a.mask().contiguousStorage()
               ? countMasked<T>(a.array().cbegin(), a.array().cend(), a.mask().cbegin(), T())
               : countMasked<T>(a.array().begin(), a.array().end(), a.mask().begin(), T());
  }
