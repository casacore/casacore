...
template <typename T>
class MRmsFunc : public MArrayFunctorBase<T> {
 public:
  virtual ~MRmsFunc() {}
...
template <typename T>
inline MArray<T> partialArrayMath(const MArray<T>& a, const IPosition& collapseAxes,
...
void partialArrayMath(MArray<RES>& res, const MArray<T>& a, const IPosition& collapseAxes,
...
template <typename T>
inline MArray<T> boxedArrayMath(const MArray<T>& a, const IPosition& boxShape,
                                const MArrayFunctorBase<T>& funcObj) {
  MArray<T> res;
...
template <typename T, typename RES>
void boxedArrayMath(MArray<RES>& res, const MArray<T>& array, const IPosition& boxShape,
                    const MArrayFunctorBase<T, RES>& funcObj) {
...
MArray<T> operator+(const MArray<T>& left, const MArray<T>& right) {
  return (left.isNull() || right.isNull()
              ? MArray<T>()
              : MArray<T>(left.array() + right.array(), left.combineMask(right)));
}
...
MArray<T> atan2(const MArray<T>& left, const MArray<T>& right) {
  return (left.isNull() || right.isNull()
              ? MArray<T>()
              : MArray<T>(atan2(left.array(), right.array()), left.combineMask(right)));
}
...
MArray<T> pow(const MArray<T>& a, const MArray<T>& exp) {
  return (a.isNull() || exp.isNull() ? MArray<T>()
                                     : MArray<T>(pow(a.array(), exp.array()), a.combineMask(exp)));
}
...
MArray<T> min(const MArray<T>& left, const MArray<T>& right) {
  return (left.isNull() || right.isNull()
              ? MArray<T>()
              : MArray<T>(min(left.array(), right.array()), left.combineMask(right)));
}
...
MArray<T> max(const MArray<T>& left, const MArray<T>& right) {
  return (left.isNull() || right.isNull()
              ? MArray<T>()
              : MArray<T>(max(left.array(), right.array()), left.combineMask(right)));
}
...
MArray<T> fmod(const MArray<T>& left, const MArray<T>& right) {
  return (left.isNull() || right.isNull()
              ? MArray<T>()
              : MArray<T>(fmod(left.array(), right.array()), left.combineMask(right)));
}
...
MArray<T> floormod(const MArray<T>& left, const MArray<T>& right) {
  return (left.isNull() || right.isNull()
              ? MArray<T>()
              : MArray<T>(floormod(left.array(), right.array()), left.combineMask(right)));
}
...
T sum(const MArray<T>& a) {
  if (a.hasMask()) {
    return a.array().contiguousStorage() && a.mask().contiguousStorage()
               ? accumulateMasked<T>(a.array().cbegin(), a.array().cend(), a.mask().cbegin(), T(),
                                     std::plus<T>())
               : accumulateMasked<T>(a.array().begin(), a.array().end(), a.mask().begin(), T(),
                                     std::plus<T>());
  }
  return sum(a.array());
}
...
T product(const MArray<T>& a) {
  if (a.hasMask()) {
    return a.array().contiguousStorage() && a.mask().contiguousStorage()
               ? accumulateMasked<T>(a.array().cbegin(), a.array().cend(), a.mask().cbegin(),
                                     std::multiplies<T>())
               : accumulateMasked<T>(a.array().begin(), a.array().end(), a.mask().begin(),
                                     std::multiplies<T>());
  }
...
MArray<T> partialSumSqrs(const MArray<T>& a, const IPosition& collapseAxes) {
  if (a.isNull()) {
    ...
    return MArray<T>(partialArrayMath(a.array(), collapseAxes, SumSqrFunc<T>()));
  }
...
MArray<T> slidingSums(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, SumFunc<T>(), fillEdge));
  }
...
MArray<T> slidingSumSqrs(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, SumSqrFunc<T>(), fillEdge));
  }
...
MArray<T> slidingProducts(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, ProductFunc<T>(), fillEdge));
  }
...
MArray<T> slidingMins(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, MinFunc<T>(), fillEdge));
  }
...
MArray<T> slidingMaxs(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, MaxFunc<T>(), fillEdge));
  }
...
MArray<T> slidingMeans(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, MeanFunc<T>(), fillEdge));
  }
...
MArray<T> slidingVariances(const MArray<T>& a, const IPosition& halfBoxSize, uInt ddof,
  ...
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, VarianceFunc<T>(ddof), fillEdge));
  }
...
MArray<T> slidingStddevs(const MArray<T>& a, const IPosition& halfBoxSize, uInt ddof,
  ...
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, StddevFunc<T>(ddof), fillEdge));
  }
...
MArray<T> slidingAvdevs(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, AvdevFunc<T>(), fillEdge));
  }
...
MArray<T> slidingRmss(const MArray<T>& a, const IPosition& halfBoxSize, Bool fillEdge = True) {
  if (a.isNull()) {
    ...
    return MArray<T>(slidingArrayMath(a.array(), halfBoxSize, RmsFunc<T>(), fillEdge));
  }
