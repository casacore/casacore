...
void RecordGram::addToken(TableExprNode* ptr) { addToken(ptr, RecordGram::Node); }
void RecordGram::addToken(RecordGramVal* ptr) { addToken(ptr, RecordGram::Val); }
void RecordGram::addToken(TableExprNodeSet* ptr) { addToken(ptr, RecordGram::Set); }
void RecordGram::addToken(TableExprNodeSetElem* ptr) { addToken(ptr, RecordGram::Elem); }
...
void RecordGram::deleteTokenStorage() {
  for (std::map<void*, RecordGram::Token>::const_iterator iter = theirTokens.begin();
       iter != theirTokens.end(); ++iter) {
    switch (iter->second) {
...
Array<double> RecordGram::expr2ArrayDouble(const String& expr, const Record& vars,
                                           const String& unit) {
  String ex = expr;
  if (!unit.empty()) {
    // Convert to the given unit (e.g., 1.3 GHz to Hz).
...
Array<DComplex> RecordGram::expr2ArrayComplex(const String& expr, const Record& vars) {
  // Convert expression to tree.
  TableExprNode node(RecordGram::parse(vars, expr));
  // Evaluate.
...
Array<String> RecordGram::expr2ArrayString(const String& expr, const Record& vars) {
  String ex = expr;
  // Convert expression to tree.
  TableExprNode node(RecordGram::parse(vars, ex));
  // Evaluate.
...
Array<MVTime> RecordGram::expr2ArrayDate(const String& expr, const Record& vars) {
  String ex = expr;
  // Convert expression to tree.
  TableExprNode node(RecordGram::parse(vars, ex));
  // Evaluate.
...
TableExprNode RecordGram::doParse(const String& expression) {
  ...
  if (error) {
    throw AipsError(message + '\n' + "Scanned so far: " + command.substr(0, recordGramPosition()));
  }
  return result;
}
