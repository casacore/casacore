...
TaQLJoin::TaQLJoin(const TENShPtr& mainNode, const TENShPtr& joinNode,
    ...
    : itsMainNode(mainNode), itsJoinNode(joinNode), itsChildren(children) {
...
template <typename T>
std::shared_ptr<TaQLJoinBase> TaQLJoin::makeOptDiscrete(TableExprNodeRep& node,
  ...
  if (level == mainNodes.size() - 1) {
    ...
    for (size_t j = 1; j < rows.size(); ++j) {
      ...
      if (val2 == val) {
        ...
        vals.push_back(val);
        children.push_back(TaQLJoin::createRecursive(mainNodes, joinNodes, srows, level + 1));
        val = val2;
        srows.resize(0);
...
template <typename T>
std::shared_ptr<TaQLJoinBase> TaQLJoin::makeOptInterval(const TableExprNodeSet& set,
                                                        const std::vector<TableExprNode>& mainNodes,
                                                        const std::vector<TableExprNode>& joinNodes,
                                                        const std::vector<rownr_t>& rows,
                                                        size_t level) {
...
std::shared_ptr<TaQLJoinBase> TaQLJoin::createRecursive(const std::vector<TableExprNode>& mainNodes,
                                                        const std::vector<TableExprNode>& joinNodes,
                                                        const std::vector<rownr_t>& rows,
                                                        size_t level) {
  ...
  if (!set) {
    ...
    if (node->dataType() == TableExprNodeRep::NTInt &&
      ...
      throw TableInvExpr(
          "In a equality join condition only Int and String "
    ...
    if ((elem->dataType() == TableExprNodeRep::NTInt ||
      ...
      throw TableInvExpr(
          "In an interval join condition only Int, Double, Date "
...
TaQLJoinColumn::TaQLJoinColumn(const TENShPtr& columnNode, const TableParseJoin& join)
    : TableExprNodeRep(*columnNode), itsColumn(columnNode), itsJoin(join) {
  setUnit(columnNode->unit());
}

// Get the table info for this column.
TableExprInfo TaQLJoinColumn::getTableInfo() const { return itsColumn->getTableInfo(); }
...
TaQLJoinColumnBool::TaQLJoinColumnBool(const TENShPtr& columnNode, const TableParseJoin& join)
...
TaQLJoinColumnInt::TaQLJoinColumnInt(const TENShPtr& columnNode, const TableParseJoin& join)
    : TaQLJoinColumn(columnNode, join) {
  rownr_t nrow = itsColumn->getTableInfo().table().nrow();
  itsData.resize(nrow);
  for (rownr_t row = 0; row < nrow; ++row) {
...
TaQLJoinColumnDouble::TaQLJoinColumnDouble(const TENShPtr& columnNode, const TableParseJoin& join)
    : TaQLJoinColumn(columnNode, join) {
  rownr_t nrow = itsColumn->getTableInfo().table().nrow();
  itsData.resize(nrow);
  for (rownr_t row = 0; row < nrow; ++row) {
...
TaQLJoinColumnString::TaQLJoinColumnString(const TENShPtr& columnNode, const TableParseJoin& join)
    : TaQLJoinColumn(columnNode, join) {
  rownr_t nrow = itsColumn->getTableInfo().table().nrow();
  itsData.resize(nrow);
  for (rownr_t row = 0; row < nrow; ++row) {
...
TaQLJoinColumnDate::TaQLJoinColumnDate(const TENShPtr& columnNode, const TableParseJoin& join)
    : TaQLJoinColumn(columnNode, join) {
  rownr_t nrow = itsColumn->getTableInfo().table().nrow();
  itsData.resize(nrow);
  for (rownr_t row = 0; row < nrow; ++row) {
