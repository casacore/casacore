...
void TaQLRegexNodeRep::save(AipsIO& aio) const {
  aio << itsValue << itsCaseInsensitive << itsNegate << itsIgnoreBlanks << itsMaxDistance;
}
...
TaQLUnaryNodeRep::TaQLUnaryNodeRep(Type type, const TaQLNode& child)
    : TaQLNodeRep(TaQLNode_Unary), itsType(type), itsChild(child) {}
...
TaQLBinaryNodeRep::TaQLBinaryNodeRep(Type type, const TaQLNode& left, const TaQLNode& right)
    : TaQLNodeRep(TaQLNode_Binary), itsType(type), itsLeft(left), itsRight(right) {}
...
TaQLIndexNodeRep::TaQLIndexNodeRep(const TaQLNode& start, const TaQLNode& end, const TaQLNode& incr)
    : TaQLNodeRep(TaQLNode_Index), itsStart(start), itsEnd(end), itsIncr(incr) {}
...
TaQLJoinNodeRep::TaQLJoinNodeRep(const TaQLMultiNode& tables, const TaQLNode& condition)
    : TaQLNodeRep(TaQLNode_Join), itsTables(tables), itsCondition(condition) {}
...
TaQLKeyColNodeRep::TaQLKeyColNodeRep(const String& name, const String& nameMask)
    : TaQLNodeRep(TaQLNode_KeyCol), itsName(name), itsNameMask(nameMask) {}
...
void TaQLKeyColNodeRep::save(AipsIO& aio) const { aio << itsName << itsNameMask; }
...
TaQLTableNodeRep::TaQLTableNodeRep(const TaQLNode& table, const String& alias)
    : TaQLNodeRep(TaQLNode_Table), itsTable(table), itsAlias(alias) {}
...
TaQLColNodeRep::TaQLColNodeRep(const TaQLNode& expr, const String& name, const String& nameMask,
                               const String& dtype)
...
TaQLGroupNodeRep::TaQLGroupNodeRep(Type type, const TaQLMultiNode& nodes)
    : TaQLNodeRep(TaQLNode_Groupby), itsType(type), itsNodes(nodes) {}
...
TaQLSortKeyNodeRep::TaQLSortKeyNodeRep(Type type, const TaQLNode& child)
    : TaQLNodeRep(TaQLNode_SortKey), itsType(type), itsChild(child) {}
...
TaQLLimitOffNodeRep::TaQLLimitOffNodeRep(const TaQLNode& limit, const TaQLNode& offset)
    : TaQLNodeRep(TaQLNode_LimitOff), itsLimit(limit), itsOffset(offset) {}
...
TaQLGivingNodeRep::TaQLGivingNodeRep(const String& name, const TaQLMultiNode& type)
    : TaQLNodeRep(TaQLNode_Giving), itsName(name), itsType(type) {}
TaQLGivingNodeRep::TaQLGivingNodeRep(const TaQLMultiNode& exprlist)
    : TaQLNodeRep(TaQLNode_Giving), itsExprList(exprlist) {}
...
TaQLUpdExprNodeRep::TaQLUpdExprNodeRep(const String& name, const String& nameMask,
                                       const TaQLNode& expr)
    : TaQLNodeRep(TaQLNode_UpdExpr), itsName(name), itsNameMask(nameMask), itsExpr(expr) {}
TaQLUpdExprNodeRep::TaQLUpdExprNodeRep(const String& name, const String& nameMask,
                                       const TaQLMultiNode& indices, const TaQLNode& expr)
    : TaQLNodeRep(TaQLNode_UpdExpr),
      ...
      itsExpr(expr) {}
TaQLUpdExprNodeRep::TaQLUpdExprNodeRep(const String& name, const String& nameMask,
                                       const TaQLMultiNode& indices1, const TaQLMultiNode& indices2,
                                       const TaQLNode& expr)
    : TaQLNodeRep(TaQLNode_UpdExpr),
      itsName(name),
      itsNameMask(nameMask),
...
TaQLSelectNodeRep::TaQLSelectNodeRep(const TaQLNode& columns, const TaQLMultiNode& with,
                                     const TaQLNode& where, const TaQLNode& groupby,
                                     const TaQLNode& having, const TaQLNode& sort,
                                     const TaQLNode& limitoff, const TaQLNode& giving,
    ...
    : TaQLQueryNodeRep(TaQLNode_Select),
      ...
      itsDMInfo(dminfo) {}
TaQLSelectNodeRep::TaQLSelectNodeRep(const TaQLNode& columns, const TaQLMultiNode& with,
                                     const TaQLMultiNode& tables, const TaQLMultiNode& joins,
                                     const TaQLNode& where, const TaQLNode& groupby,
                                     const TaQLNode& having, const TaQLNode& sort,
                                     const TaQLNode& limitoff, const TaQLNode& giving,
                                     const TaQLMultiNode& dminfo)
...
TaQLNode TaQLSelectNodeRep::restore(AipsIO& aio) {
  ...
  std::unique_ptr<TaQLSelectNodeRep> node(new TaQLSelectNodeRep(
      columns, with, tables, joins, where, groupby, having, sort, limitoff, giving, dminfo));
  node->restoreSuper(aio);
  return node.release();
}

TaQLCountNodeRep::TaQLCountNodeRep(const TaQLMultiNode& with, const TaQLNode& columns,
...
TaQLNode TaQLCountNodeRep::restore(AipsIO& aio) {
  ...
  TaQLMultiNode tables = TaQLNode::restoreMultiNode(aio);
  TaQLNode where = TaQLNode::restoreNode(aio);
  std::unique_ptr<TaQLCountNodeRep> node(new TaQLCountNodeRep(with, columns, tables, where));
  node->restoreSuper(aio);
  return node.release();
}

TaQLUpdateNodeRep::TaQLUpdateNodeRep(const TaQLMultiNode& with, const TaQLMultiNode& tables,
                                     const TaQLMultiNode& update, const TaQLMultiNode& from,
                                     const TaQLNode& where, const TaQLNode& sort,
...
TaQLInsertNodeRep::TaQLInsertNodeRep(const TaQLMultiNode& with, const TaQLMultiNode& tables,
                                     const TaQLMultiNode& columns, const TaQLNode& values,
...
TaQLInsertNodeRep::TaQLInsertNodeRep(const TaQLMultiNode& with, const TaQLMultiNode& tables,
  ...
  for (uInt i = 0; i < nodes.size(); ++i) {
    ...
    if (rep->itsIndices1.isValid() || rep->itsIndices2.isValid()) {
      throw TableInvExpr(
          "Column indices or masks cannot be given in an "
          "INSERT command");
    }
...
TaQLDeleteNodeRep::TaQLDeleteNodeRep(const TaQLMultiNode& with, const TaQLMultiNode& tables,
                                     const TaQLNode& where, const TaQLNode& sort,
                                     const TaQLNode& limitoff)
...
TaQLCalcNodeRep::TaQLCalcNodeRep(const TaQLMultiNode& with, const TaQLMultiNode& tables,
                                 const TaQLNode& expr, const TaQLNode& where, const TaQLNode& sort,
                                 const TaQLNode& limitoff)
...
TaQLCreTabNodeRep::TaQLCreTabNodeRep(const TaQLMultiNode& with, const TaQLNode& giving,
                                     const TaQLMultiNode& likeDrop, const TaQLMultiNode& cols,
...
TaQLNode TaQLCreTabNodeRep::restore(AipsIO& aio) {
  ...
  std::unique_ptr<TaQLCreTabNodeRep> node(
      new TaQLCreTabNodeRep(with, giving, likeDrop, columns, limit, dminfo));
  node->restoreSuper(aio);
  return node.release();
}

TaQLColSpecNodeRep::TaQLColSpecNodeRep(const String& name, const String& likeCol,
...
TaQLRecFldNodeRep::TaQLRecFldNodeRep(const String& name, const TaQLNode& values,
                                     const String& dtype)
    : TaQLNodeRep(TaQLNode_RecFld),
      itsName(name),
      itsDtype(checkDataType(dtype)),
      itsValues(values) {}
TaQLRecFldNodeRep::TaQLRecFldNodeRep(const String& name, const TaQLRecFldNodeRep& node)
    : TaQLNodeRep(TaQLNode_RecFld),
      itsName(name),
      itsDtype(node.itsDtype),
      itsValues(node.itsValues) {}
TaQLRecFldNodeRep::TaQLRecFldNodeRep(const String& name, const String& fromName,
                                     const String& dtype)
    : TaQLNodeRep(TaQLNode_RecFld),
      itsName(name),
...
TaQLUnitNodeRep::TaQLUnitNodeRep(const String& unit, const TaQLNode& child)
    : TaQLNodeRep(TaQLNode_Unit), itsUnit(unit), itsChild(child) {}
...
TaQLAltTabNodeRep::TaQLAltTabNodeRep(const TaQLMultiNode& with, const TaQLNode& table,
...
TaQLAddColNodeRep::TaQLAddColNodeRep(const TaQLMultiNode& cols, const TaQLMultiNode& dminfo)
    : TaQLNodeRep(TaQLNode_AddCol), itsColumns(cols), itsDMInfo(dminfo) {}
...
TaQLSetKeyNodeRep::TaQLSetKeyNodeRep(const TaQLMultiNode& keyvals)
    : TaQLNodeRep(TaQLNode_SetKey), itsKeyVals(keyvals) {}
...
void TaQLSetKeyNodeRep::save(AipsIO& aio) const { itsKeyVals.saveNode(aio); }
...
TaQLAddRowNodeRep::TaQLAddRowNodeRep(const TaQLNode& nrow)
    : TaQLNodeRep(TaQLNode_AddRow), itsNRow(nrow) {}
...
void TaQLAddRowNodeRep::save(AipsIO& aio) const { itsNRow.saveNode(aio); }
TaQLNode TaQLAddRowNodeRep::restore(AipsIO& aio) {
  TaQLNode nrow = TaQLNode::restoreNode(aio);
  return new TaQLAddRowNodeRep(nrow);
}

TaQLConcTabNodeRep::TaQLConcTabNodeRep(const String& tableName, const TaQLMultiNode& tables,
...
TaQLShowNodeRep::TaQLShowNodeRep(const TaQLMultiNode& names)
    : TaQLNodeRep(TaQLNode_Show), itsNames(names) {}
...
void TaQLShowNodeRep::save(AipsIO& aio) const { itsNames.saveNode(aio); }
TaQLNode TaQLShowNodeRep::restore(AipsIO& aio) {
  TaQLMultiNode names = TaQLNode::restoreMultiNode(aio);
  return new TaQLShowNodeRep(names);
}

TaQLCopyColNodeRep::TaQLCopyColNodeRep(const TaQLMultiNode& names, const TaQLMultiNode& dminfo)
    : TaQLNodeRep(TaQLNode_CopyCol), itsNames(names), itsDMInfo(dminfo) {}
...
TaQLDropTabNodeRep::TaQLDropTabNodeRep(const TaQLMultiNode& with, const TaQLMultiNode& tables)
    : TaQLNodeRep(TaQLNode_DropTab), itsWith(with), itsTables(tables) {}
