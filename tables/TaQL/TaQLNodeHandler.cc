...
TaQLNodeHandler::~TaQLNodeHandler() { clearStack(); }
...
TableParseQuery* TaQLNodeHandler::pushStack(TableParseQuery::CommandType type) {
...
TaQLNodeResult TaQLNodeHandler::visitUnaryNode(const TaQLUnaryNodeRep& node) {
  ...
  TableExprNode exres(topStack()->doExists(notexists, node.style().doTiming()));
  popStack();
...
TaQLNodeResult TaQLNodeHandler::visitBinaryNode(const TaQLBinaryNodeRep& node) {
  ...
  if (node.itsType == TaQLBinaryNodeRep::B_INDEX) {
    ...
    return new TaQLNodeHRValue(TableParseQuery::handleSlice(left, right, node.itsRight.style()));
  }
...
TaQLNodeResult TaQLNodeHandler::visitFuncNode(const TaQLFuncNodeRep& node) {
  ...
  if (fncParts[fncParts.size() - 1] == "msid") {
    ...
    funcRes = topStack()->handleFunc(node.itsName, getHR(result).getExprSet(), node.style());
  }
  TaQLNodeHRValue* hrval = new TaQLNodeHRValue();
...
TableExprNode TaQLNodeHandler::handleIdFunc(const TaQLFuncNodeRep& node) {
  ...
  const TaQLKeyColNodeRep* colRep = dynamic_cast<const TaQLKeyColNodeRep*>(nodes[0].getRep());
  if (!colRep) {
  ...
  if (fncParts.size() + colParts.size() > 3) {
    throw TableInvExpr(
        "The MSID function or its column name can only "
...
TaQLNodeResult TaQLNodeHandler::visitRangeNode(const TaQLRangeNodeRep& node) {
  ...
  if (node.itsAsMidWidth) {
    elem = new TableExprNodeSetElem(getHR(start).getExpr(), getHR(end).getExpr());
  } else {
    if (start.isValid()) {
      if (end.isValid()) {
        elem = new TableExprNodeSetElem(node.itsLeftClosed, getHR(start).getExpr(),
                                        getHR(end).getExpr(), node.itsRightClosed);
      } else {
        elem = new TableExprNodeSetElem(node.itsLeftClosed, getHR(start).getExpr());
      }
    } else {
      elem = new TableExprNodeSetElem(getHR(end).getExpr(), node.itsRightClosed);
    }
  }
  hrval->setElem(elem);
...
TaQLNodeResult TaQLNodeHandler::visitColNode(const TaQLColNodeRep& node) {
  ...
  if (hrval->getExpr().isNull() && !node.itsNameMask.empty()) {
    throw TableInvExpr(
        "value AS (col,mask) can only be given if value "
        "is an expression");
  }
...
TaQLNodeResult TaQLNodeHandler::visitGroupNode(const TaQLGroupNodeRep& node) {
  ...
  topStack()->handleGroupby(outnodes, node.itsType == TaQLGroupNodeRep::Rollup);
  return TaQLNodeResult();
}
  ...
  if (node.itsIndices1.isValid()) {
    ...
    if (node.itsIndices2.isValid()) {
      ...
      topStack()->addUpdate(std::make_shared<TableParseUpdate>(
          node.itsName, node.itsNameMask, getHR(ires1).getExprSet(), getHR(ires2).getExprSet(),
          expr, node.itsIndices1.style()));
    } else {
      topStack()->addUpdate(std::make_shared<TableParseUpdate>(node.itsName, node.itsNameMask,
                                                               getHR(ires1).getExprSet(), expr,
                                                               node.itsIndices1.style()));
    }
  } else {
    topStack()->addUpdate(std::make_shared<TableParseUpdate>(node.itsName, node.itsNameMask, expr));
  }
  return TaQLNodeResult();
}
...
TaQLNodeResult TaQLNodeHandler::visitColSpecNode(const TaQLColSpecNodeRep& node) {
  ...
  topStack()->handleColSpec(node.itsName, node.itsLikeCol, node.itsDtype, spec,
                            node.style().isCOrder());
  return TaQLNodeResult();
}
...
Record TaQLNodeHandler::handleMultiRecFld(const TaQLNode& node) {
  ...
  for (uInt i = 0; i < vals.size(); ++i) {
    ...
    if (res.getValueHolder().dataType() == TpRecord) {
      ...
      TableParseUtil::setRecFld(rec, res.getString(), res.getDtype(), res.getValueHolder());
    }
  }
  return rec;
}
...
void TaQLNodeHandler::handleTableName(TaQLNodeHRValue* hrval, const TaQLNode& node) {
  ...
  if (tabnm->itsType == TaQLConstNodeRep::CTInt) {
    // A temptable has type Int, but the string can contain a subtable name.
...
TaQLNodeResult TaQLNodeHandler::visitConcTabNode(const TaQLConcTabNodeRep& node) {
  ...
  for (uInt i = 0; i < nodes.size(); ++i) {
    ...
    if (!tab.isNull()) {
      ...
      throw TableInvExpr("No matching tables found for $" + std::to_string(res.getInt()));
    } else {
...
TaQLNodeResult TaQLNodeHandler::visitCopyColNode(const TaQLCopyColNodeRep& node) {
  Record dminfo = handleMultiRecFld(node.itsDMInfo);
  ...
  for (uInt i = 0; i < names.size(); i += 2) {
    ...
    topStack()->addUpdate(
        std::make_shared<TableParseUpdate>(names[i], String(), tab.col(names[i + 1])));
  }
  // Execute it all.
...
void TaQLNodeHandler::handleInsCol(const TaQLMultiNode& node) {
  ...
  for (uInt i = 0; i < nodes.size(); ++i) {
    ...
    AlwaysAssert(nodes[i].nodeType() == TaQLNode_KeyCol, AipsError);
    TaQLKeyColNodeRep* colNode = (TaQLKeyColNodeRep*)(nodes[i].getRep());
    topStack()->handleColumn(-1, colNode->itsName, TableExprNode(), "", colNode->itsNameMask, "");
  }
}

void TaQLNodeHandler::handleInsVal(const TaQLNode& node) {
  AlwaysAssert(node.nodeType() == TaQLNode_Multi, AipsError);
  const TaQLMultiNodeRep& avals = dynamic_cast<const TaQLMultiNodeRep&>(*node.getRep());
  ...
  for (uInt i = 0; i < anodes.size(); ++i) {
    ...
    for (uInt j = 0; j < nodes.size(); ++j) {
      ...
      if (i == 0) {
        topStack()->addUpdate(std::make_shared<TableParseUpdate>("", "", expr));
      }
    }
  }
