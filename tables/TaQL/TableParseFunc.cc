...
TableExprNode TableParseFunc::makeFuncNode(TableParseQuery* tpq, const String& fname,
                                           const TableExprNodeSet& arguments,
                                           const Vector<int>& ignoreFuncs,
                                           const TableExprInfo& tabin, const TaQLStyle& style) {
  ...
  TableExprFuncNode::FunctionType ftype = findFunc(name, arguments.size(), ignoreFuncs);
          ...
          if (!axesIsArray) {
            ...
            for (uInt i = axarg; i < arguments.size(); i++) {
              ...
              if (!rep || !arg->isSingle() || rep->valueType() != TableExprNodeRep::VTScalar ||
                ...
                throw TableInvExpr("Axes/shape arguments " + std::to_string(i + 1) +
...
TableExprNode TableParseFunc::makeUDFNode(TableParseQuery* sel, const String& name,
                                          const TableExprNodeSet& arguments,
                                          const TableExprInfo& tabInfo, const TaQLStyle& style) {
  Vector<String> parts = stringToVector(name, '.');
  if (parts.size() == 1) {
    // No ., thus no UDF but a builtin function.
    throw TableInvExpr("TaQL function " + name +
                       " is unknown; "
...
TableExprFuncNode::FunctionType TableParseFunc::findFunc(const String& name, uInt narguments,
  ...
  } else if (funcName == "abs" || funcName == "amplitude" || funcName == "ampl") {
  ...
  } else if (funcName == "upcase" || funcName == "upper" || funcName == "toupper" ||
             funcName == "to_upper") {
    ftype = TableExprFuncNode::upcaseFUNC;
  } else if (funcName == "downcase" || funcName == "lower" || funcName == "tolower" ||
             funcName == "to_lower") {
