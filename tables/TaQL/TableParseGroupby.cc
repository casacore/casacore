...
void TableParseGroupby::findGroupAggr(const Block<TableExprNode>& columnNodes, Bool isSelect) {
  ...
  if (!itsHavingNode.isNull()) {
    if (nselAggr == 0 && itsGroupbyNodes.empty()) {
      throw TableInvExpr(
          "HAVING can only be used if GROUPBY is used or "
...
void TableParseGroupby::getAggrNodes(const TableExprNode& node,
  ...
  std::vector<TableExprNodeRep*> aggr = TableExprNodeUtil::getAggrNodes(node.getRep().get());
...
std::shared_ptr<TableExprGroupResult> TableParseGroupby::execGroupAggr(
  ...
  if ((itsGroupAggrUsed & ONLY_COUNTALL) != 0 && (itsGroupAggrUsed & GROUPBY) == 0) {
...
std::shared_ptr<TableExprGroupResult> TableParseGroupby::countAll(Vector<rownr_t>& rownrs) const {
