...
TableParseProject::TableParseProject(const TableParseTableList& tableList)
    : tableList_p(tableList), tableDesc_p(new TableDesc()), nrSelExprUsed_p(0) {}
...
Table TableParseProject::handleColumnFinish(Bool distinct, Bool hasResultSet,
  ...
  if (distinct && columnNames_p.empty()) {
    throw TableInvExpr(
        "SELECT DISTINCT can only be given with at least "
        "one column name");
  }
...
void TableParseProject::handleColSpec(const String& colName, const String& likeColName,
  ...
  for (uInt i = 0; i < spec.nfields(); i++) {
    ...
    if (name == "NDIM") {
      ...
      throw TableInvExpr(
          "TableParseProject::handleColSpec - "
          "column specification field name " +
          name + " is unknown");
    }
  }
...
Table TableParseProject::makeProjectExprTable(TableParseQuery& tpq) {
  ...
  for (uInt i = 0; i < columnExpr_p.size(); i++) {
    ...
    DataType dtype = makeDataType(columnExpr_p[i].dataType(), columnDtypes_p[i], columnNames_p[i]);
    addColumnDesc(td, dtype, newName, 0,
                  columnExpr_p[i].isScalar() ? -1 : 0,  // ndim
                  IPosition(), "", "", "", columnKeywords_p[i],
    ...
    if (!columnNameMasks_p[i].empty()) {
      addColumnDesc(td, TpBool, columnNameMasks_p[i], 0,
                    columnExpr_p[i].isScalar() ? -1 : 0,  // ndim
                    IPosition(), "", "", "", TableRecord(), Vector<String>(), Record());
    }
  }
  // Create the table.
  return tpq.createTable(td, 0, dminfo_p, std::vector<const Table*>(),
  ...
  if (!dtstr.empty()) {
    if (dtstr == "B") {
      if (dtype != TpOther && dtype != TpBool) {
        throw TableInvExpr("Expression of column " + colName + " does not have data type Bool");
      }
      return TpBool;
    }
    if (dtstr == "S") {
      if (dtype != TpOther && dtype != TpString) {
        throw TableInvExpr("Expression of column " + colName + " does not have data type String");
      }
      return TpString;
    }
    if (dtype == TpBool || dtype == TpString) {
      throw TableInvExpr("Expression of column " + colName + " does not have a numeric data type");
    }
    ...
    throw TableInvExpr("Datatype " + dtstr + " of column " + colName + " is invalid");
  }
  if (dtype == TpOther) {
...
void TableParseProject::addColumnDesc(TableDesc& td, DataType dtype, const String& colName,
  ...
  if (ndim < 0) {
    switch (dtype) {
      ...
      case TpComplex:
        ...
        break;
      case TpDComplex:
        td.addColumn(ScalarColumnDesc<DComplex>(colName, comment, dmType, dmGroup, options));
        break;
      case TpString:
    ...
    switch (dtype) {
      ...
      case TpComplex:
        td.addColumn(
            ArrayColumnDesc<Complex>(colName, comment, dmType, dmGroup, shape, options, ndim));
        break;
      case TpDComplex:
        td.addColumn(
            ArrayColumnDesc<DComplex>(colName, comment, dmType, dmGroup, shape, options, ndim));
        break;
      case TpString:
        td.addColumn(
  ...
  keys.merge(TableRecord(attributes), RecordInterface::OverwriteDuplicates);
...
std::pair<ColumnDesc, Record> TableParseProject::findColumnInfo(const String& colName,
  ...
  for (uInt i = 0; i < columnExpr_p.size(); i++) {
    if (!columnExpr_p[i].getRep()->isConstant()) {
      ...
      for (const Table& tab : tabs) {
        if (tab.nrow() != nrow) {
          throw TableInvExpr(
              "Nr of rows of tables used in select "
  ...
  if (!hasKey) {
    if (tryProj && shand.empty() && fieldNames.empty()) {
      ...
      if (found) {
        ...
        } else if (!columnOldNames_p.empty() && !columnOldNames_p[inx].empty()) {
    ...
    if (tab.tableDesc().isColumn(columnName) && tabPair.joinIndex() < 0) {
      if (firstColTable_p.isNull()) {
        ...
        if (tab.nrow() != firstColTable_p.nrow()) {
          throw TableInvExpr("Nr of rows (" + std::to_string(tab.nrow()) + ") in table column " +
                             std::string(name) + " differs from column " +
                             std::string(firstColName_p) + " (" +
    ...
    try {
      ...
      throw TableInvExpr(name + " is an unknown column (or keyword) in table " + tab.tableName());
    }
  }
