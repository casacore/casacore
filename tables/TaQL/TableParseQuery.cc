...
TableExprNode TableParseQuery::handleSlice(const TableExprNode& array,
  ...
  if (indices.dataType() == TableExprNodeRep::NTBool) {
    if (!(indices.isSingle() && indices.size() == 1 && indices.hasArrays())) {
      throw TableInvExpr(
          "Second argument of a masked array must be an array; maybe extra brackets are needed "
          "like [1,2][[T,F]]");
    }
...
TableExprNode TableParseQuery::handleFunc(const String& name, const TableExprNodeSet& arguments,
  ...
  TableExprNode node =
      TableParseFunc::makeFuncNode(this, name, arguments, ignoreFuncs, tableList_p.first(), style);
...
Table TableParseQuery::createTable(const TableDesc& td, Int64 nrow, const Record& dmInfo,
  ...
  if (parts.size() > 1) {
    return createSubTable(parts[parts.size() - 1], td, nrow, dmInfo, tempTables, stack);
  }
  // Create the table.
...
void TableParseQuery::handleHaving(const TableExprNode& node) { groupby_p.handleHaving(node); }
...
void TableParseQuery::handleDropTab(const std::vector<const Table*>& tempTables,
  ...
  for (TableParsePair& tab : tableList_p.fromTablesNC()) {
    ...
    if (parts.size() > 1) {
      ...
      Table parent(TableParseUtil::getTable(tab.tabnr(), tab.name().substr(0, sz), Table(),
                                            tempTables, stack));
...
void TableParseQuery::handleCreTab(const Record& dmInfo,
  ...
  DataManInfo::finalizeMerge(tableProject_p.tableDesc(), tableProject_p.dminfo());
  table_p =
      createTable(tableProject_p.tableDesc(), limit_p, tableProject_p.dminfo(), tempTables, stack);
}
...
void TableParseQuery::initDescriptions(const TableDesc& desc, const Record& dminfo) {
...
TableRecord& TableParseQuery::findKeyword(const String& name, String& keyName, Bool update) {
  ...
  if (columnName.empty()) {
    if (update) {
      rec = TableExprNode::findLastKeyRec(tab.rwKeywordSet(), fieldNames, fullName);
    } else {
      rec = TableExprNode::findLastKeyRec(tab.keywordSet(), fieldNames, fullName);
    }
  } else {
    if (update) {
      ...
      rec = TableExprNode::findLastKeyRec(TableColumn(tab, columnName).keywordSet(), fieldNames,
                                          fullName);
    }
  }
...
void TableParseQuery::handleSetKey(const String& name, const String& dtype,
...
void TableParseQuery::handleRenameKey(const String& oldName, const String& newName) {
  String keyName;
...
TableExprNode TableParseQuery::makeSubSet() const {
  ...
  if (resultSet_p->hasArrays()) {
    throw(
        TableInvExpr("Set in GIVING clause should contain scalar"
                     " elements"));
  }
...
void TableParseQuery::handleLimit(const TableExprNodeSetElem& expr) {
  ...
  if (expr.increment()) {
    ...
    if (stride_p <= 0) {
      throw TableInvExpr("in the LIMIT clause stride " + std::to_string(stride_p) +
                         " must be positive");
    }
  }
  if (expr.end()) {
...
void TableParseQuery::handleLimit(const TableExprNode& expr) { limit_p = evalIntScaExpr(expr); }

void TableParseQuery::handleOffset(const TableExprNode& expr) { offset_p = evalIntScaExpr(expr); }

void TableParseQuery::handleTableNoFrom() {
  if (limit_p < 0 || offset_p < 0 || endrow_p < 0) {
    throw TableInvExpr(
        "LIMIT and OFFSET values cannot be negative if no "
...
void TableParseQuery::handleInsert(TableParseQuery* sel) { insSel_p = sel; }
...
Table TableParseQuery::doFinish(Bool showTimings, Table& table,
  ...
  if (parts.size() > 1) {
    parent =
        TableParseUtil::openParentTable(resultName_p, parts[parts.size() - 1], tempTables, stack);
  ...
  if (resultType_p == 1) {
    ...
    if (resultType_p > 0) {
      ...
      table.rename(fullName, overwrite_p ? Table::New : Table::NewNoReplace);
      table.flush();
    }
  }
...
void TableParseQuery::handleGiving(const String& name, const Record& rec) {
  ...
  for (uInt i = 0; i < rec.nfields(); ++i) {
    ...
    if (done) {
      ...
      if (!ok) {
        throw TableParseError(
            "endian must have a string value "
      ...
      if (!ok) {
        throw TableParseError(
            "storage must have a string value "
  ...
  if (resultName_p.empty() && resultType_p > 2) {
    throw TableParseError(
        "output table name can only be omitted if "
...
void TableParseQuery::handleGiving(const TableExprNodeSet& set) {
  ...
  if (!tableProject_p.getColumnNames().empty()) {
    throw TableInvExpr(
        "Expressions can be given in SELECT or GIVING, "
        "not both");
  }
...
void TableParseQuery::execute(Bool showTimings, Bool setInGiving, Bool mustSelect, rownr_t maxRow,
  ...
  groupby_p.findGroupAggr(tableProject_p.getColumnExpr(), commandType_p == PSELECT);
  if (!groupby_p.isUsed()) {
  ...
  rownr_t nrmax = 0;
  if (endrow_p > 0 && sort_p.size() == 0 && !distinct_p && !groupby_p.isUsed()) {
    nrmax = endrow_p;
    if (doTracing) {
  ...
  if (groupby_p.isUsed() != 0) {
    ...
    if (doTracing) {
      ...
      cerr << "  applySelection called for " << applySelNodes_p.size() << " nodes" << endl;
    }
  }
  ...
  if (!distinct_p && (offset_p != 0 || limit_p != 0 || endrow_p != 0 || stride_p != 1)) {
  ...
  if (commandType_p == PUPDATE) {
    ...
    if (tableProject_p.getColumnNames().size() > 0) {
      ...
      if (doTracing) {
        cerr << "Final projection done of "
             << tableProject_p.getColumnNames().size() - tableProject_p.nColumnsPreCalc()
             << " columns resulting in " << resultTable.nrow() << " rows" << endl;
      }
    }
    ...
    if (distinct_p && (offset_p != 0 || limit_p != 0 || endrow_p != 0 || stride_p != 1)) {
      ...
      if (doTracing) {
        cerr << "LIMIT/OFFSET resulted in " << resultTable.nrow() << " rows" << endl;
      }
    }
...
String TableParseQuery::getTableStructure(const Vector<String>& parts, const TaQLStyle& style) {
  ...
  for (uInt i = 2; i < parts.size(); ++i) {
    ...
    if (opt == "dm") {
      ...
      throw TableInvExpr(parts[i] +
                         " is an unknown show table option; use: "
                         "dm col sort key tabkey colkey recur");
    }
  }
  std::ostringstream os;
  tableList_p.firstTable().showStructure(os, showdm, showcol, showsub, sortcol, style.isCOrder());
