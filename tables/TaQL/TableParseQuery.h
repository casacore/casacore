...
class TableParseQuery {
 public:
  ...
  CommandType commandType() const { return commandType_p; }

  // Return the expression node.
  TableExprNode getNode() const { return node_p; }

  // Return the table name list.
  TableParseTableList& tableList() { return tableList_p; }

  Table& projectExprTable() { return projectExprTable_p; }
  ...
  // 0 = no maximum.
  void execute(Bool showTimings, Bool setInGiving, Bool mustSelect, rownr_t maxRow,
  ...
  void handleCreTab(const Record& dmInfo, const std::vector<const Table*>& tempTables,
                    const std::vector<TableParseQuery*>& stack);
  ...
  void handleSort(const std::vector<TableParseSortKey>& sortList, Bool noDuplicates,
  ...
  const std::vector<TableParseJoin>& joins() const { return joins_p; }
  ...
  void addUpdate(const std::shared_ptr<TableParseUpdate>& upd) { update_p.push_back(upd); }

  // Set the insert expressions for all rows.
  void setInsertExprs(const std::vector<TableExprNode> exprs) { insertExprs_p = exprs; }

  // Replace the first table (used by CALC command).
  void replaceTable(const Table& table);

  // Set the DataManager info for a new table.
  void setDMInfo(const Record& dminfo) { tableProject_p.setDMInfo(dminfo); }

  // Get the projected column names.
  const Block<String>& getColumnNames() const { return tableProject_p.getColumnNames(); }

  // Get the resulting table.
  const Table& getTable() const { return table_p; }
  ...
  void addApplySelNode(const TableExprNode& node) { applySelNodes_p.push_back(node); }
 ...
 private:
  ...
  void doUpdate(Bool showTimings, const Table& origTable, Table& updTable,
  ...
  Table doFinish(Bool showTimings, Table& table, const std::vector<const Table*>& tempTables,
                 const std::vector<TableParseQuery*>& stack);
