...
class TableParsePair {
 public:
  ...
  // Get the shorthand.
  const String& shorthand() const { return shorthand_p; }
...
class TableParseTableList {
 public:
  ...
  std::vector<TableParsePair>& fromTablesNC() { return itsFromTables; }

  // Return the first FROM table (which is usually the table to operate on).
  TableExprInfo first() const { return itsFromTables.at(0).getTableInfo(); }

  // Return the first FROM table (which is usually the table to operate on).
  const Table& firstTable() const { return itsFromTables.at(0).table(); }
