...
TableParseUpdate::TableParseUpdate(const String& columnName, const String& columnNameMask,
                                   const TableExprNodeSet& indices, const TableExprNode& node,
...
TableParseUpdate::TableParseUpdate(const String& columnName, const String& columnNameMask,
                                   ...
                                   const TableExprNodeSet& indices2, const TableExprNode& node,
                                   const TaQLStyle& style)
    : columnName_p(columnName),
      columnNameMask_p(columnNameMask),
      maskFirst_p(False),
...
void TableParseUpdate::handleIndices(const TableExprNodeSet& indices, const TaQLStyle& style) {
  // Create a mask if a single bool element is given.
  if (indices.isSingle() && indices.size() == 1 && indices.dataType() == TableExprNodeRep::NTBool) {
    if (!mask_p.isNull()) {
      throw TableInvExpr(
          "A double indexed update array cannot contain "
          "two masks");
    }
...
template <typename TCOL, typename TNODE>
void TableParseUpdate::updateArray(rownr_t row, const TableExprId& rowid, const TableExprNode& node,
                                   const Array<TNODE>& res, ArrayColumn<TCOL>& col) {
...
template <typename TCOL, typename TNODE>
void TableParseUpdate::updateSlice(rownr_t row, const TableExprId& rowid, const TableExprNode& node,
                                   const Array<TNODE>& res, const Slicer& slice,
  ...
  if (col.isDefined(row)) {
    if (node.isScalar()) {
      ...
      if (slice.isFixed()) {
        ...
        arr.resize(slice.inferShapeFromSource(col.shape(row), blc, trc, inc));
      }
      arr = static_cast<TCOL>(val);
      ...
      if (acol.isDefined(row)) {
        ...
        if (node.isScalar()) {
          ...
          if (!aval.shape().isEqual(smask.shape())) {
            throw TableInvExpr("Array shapes in update of column " + col.columnDesc().name() +
                               " mismatch");
          }
...
void TableParseUpdate::checkMaskColumn(Bool hasMask, const ArrayColumn<Bool>& maskCol,
  ...
  if (!maskCol.isNull()) {
    if (!hasMask) {
      throw TableInvExpr(
          "No update mask column can be given for an "
...
void TableParseUpdate::updateColumn(TableColumn& col, ArrayColumn<Bool>& maskCol, rownr_t row,
  ...
  switch (node_p.getNodeRep()->dataType()) {
    ...
    case TableExprNodeRep::NTString:
      switch (col.columnDesc().dataType()) {
        case TpString:
          updateValue<String, String>(row, rowid, isScalarCol, node_p, mask.array(), col, slicerPtr,
                                      maskCol);
          break;
        default:
          throw TableInvExpr("Column " + columnName_p +
    ...
    case TableExprNodeRep::NTComplex:
      switch (col.columnDesc().dataType()) {
        case TpComplex:
          updateValue<Complex, DComplex>(row, rowid, isScalarCol, node_p, mask.array(), col,
                                         slicerPtr, maskCol);
          break;
        case TpDComplex:
          updateValue<DComplex, DComplex>(row, rowid, isScalarCol, node_p, mask.array(), col,
                                          slicerPtr, maskCol);
          break;
        default:
...
void TableParseUpdate::check(const Table& origTable, const Table& updTable) const {
  ...
  if (!tabdesc.isColumn(columnName_p)) {
    throw TableInvExpr("Update column " + columnName_p + " does not exist in table " +
                       updTable.tableName());
  }
  if (!updTable.isColumnWritable(columnName_p)) {
    throw TableInvExpr("Update column " + columnName_p + " is not writable in table " +
                       updTable.tableName());
  }
  ...
  if (!columnNameMask_p.empty()) {
    ...
    if (!updTable.isColumnWritable(columnNameMask_p)) {
      throw TableInvExpr("Update column " + columnNameMask_p + " is not writable in table " +
                         updTable.tableName());
    }
    ...
    if (coldescMask.dataType() != TpBool) {
      throw TableInvExpr("Update mask column " + columnNameMask_p + " must have data type Bool");
    }
  }
  ...
  if (isScalar && !node_p.isScalar()) {
    throw TableInvExpr(
        "An array value cannot be used in UPDATE of "
        " scalar element of column " +
        columnName_p + " in table " + updTable.tableName());
  }
}
