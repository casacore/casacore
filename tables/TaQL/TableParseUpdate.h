...
class TableParseUpdate {
 public:
  ...
  TableParseUpdate(const String& columnName, const String& columnNameMask,
                   const TableExprNodeSet& indices, const TableExprNode&, const TaQLStyle&);
  ...
  TableParseUpdate(const String& columnName, const String& columnNameMask,
                   const TableExprNodeSet& indices1, const TableExprNodeSet& indices2,
                   const TableExprNode&, const TaQLStyle&);
  ...
  void setNode(const TableExprNode& node) { node_p = node; }

  // Set the column name.
  void setColumnName(const String& name) { columnName_p = name; }

  // Set the column name for the mask.
  void setColumnNameMask(const String& name) { columnNameMask_p = name; }

  // Get the column name.
  const String& columnName() const { return columnName_p; }

  // Get the possible column name for the mask.
  const String& columnNameMask() const { return columnNameMask_p; }

  // Adapt the possible unit of the expression to the possible unit
  // of the column.
  void adaptUnit(const Unit& columnUnit) { node_p.adaptUnit(columnUnit); }
 ...
 private:
  ...
  void updateScalar(rownr_t row, const TableExprId& rowid, const TableExprNode& node,
                    TableColumn& col);
  template <typename TCOL, typename TNODE>
  void updateArray(rownr_t row, const TableExprId& rowid, const TableExprNode& node,
                   const Array<TNODE>& res, ArrayColumn<TCOL>& col);
  template <typename TCOL, typename TNODE>
  void updateSlice(rownr_t row, const TableExprId& rowid, const TableExprNode& node,
