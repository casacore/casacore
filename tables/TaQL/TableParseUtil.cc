...
Table getTable(Int tabnr, const String& name, const Table& ftab,
  ...
  if (name != "::") {
    if (subs.size() == 0 || (subs.size() > 1 && anyEQ(subs(Slice(1, subs.size() - 1)), String()))) {
  ...
  if (colDesc.isScalar()) {
    switch (colDesc.dataType()) {
      ...
      case TpString:
        ...
        break;
      default:
        throw(TableInvExpr("Nested query column " + colDesc.name() + " has unknown data type"));
    }
  } else {
    switch (colDesc.dataType()) {
      case TpBool:
