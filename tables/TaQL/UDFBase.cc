...
void UDFBase::init(const vector<TENShPtr>& operands, const TableExprInfo& tableInfo,
...
void UDFBase::setUnit(const String& unit) { itsUnit = unit; }
...
void UDFBase::registerUDF(const String& name, MakeUDFObject* func) {
  ...
  if (iter == theirRegistry.end()) {
    ...
    if (iter->second != func) {
      throw TableInvExpr("User defined TaQL function " + fname + " already exists");
    }
  }
