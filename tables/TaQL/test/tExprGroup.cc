...
void check(const TableExprNode& expr, const vector<Record>& recs, const DComplex& expVal,
           const String& str) {
  cout << "Test " << str << endl;
  // Get the aggregation node.
  TableExprAggrNode& aggr =
...
void doBool() {
  ...
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gallFUNC, expr), recs, allTrue(vecb),
        "all");
...
void doInt() {
  ...
  TableExprNode expr = makeRecordExpr(recs[0], "fld");
  // Create and check aggregation expressions.
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gminFUNC, expr), recs, min(veci),
        "minInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gmaxFUNC, expr), recs, max(veci),
        "maxInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gsumFUNC, expr), recs, sum(veci),
        "sumInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gproductFUNC, expr), recs, product(veci),
        "productInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gsumsqrFUNC, expr), recs,
        sum(veci * veci), "sumsqrInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gmeanFUNC, expr), recs, mean(vecd),
        "meanInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gvariance0FUNC, expr), recs,
        pvariance(vecd), "varianceInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gvariance1FUNC, expr), recs,
        variance(vecd), "varianceInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gstddev0FUNC, expr), recs, pstddev(vecd),
        "stddevInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gstddev1FUNC, expr), recs, stddev(vecd),
        "stddevInt");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::grmsFUNC, expr), recs, rms(vecd),
        "rmsInt");
  checkLazy(TableExprNode::newFunctionNode(TableExprFuncNode::gmedianFUNC, expr), recs,
            median(vecd), "medianInt");
  checkLazy(TableExprNode::newFunctionNode(TableExprFuncNode::gfractileFUNC, expr, 0.3), recs,
            fractile(vecd, 0.3), "fractileInt");
}
...
void doDouble() {
  ...
  TableExprNode expr = makeRecordExpr(recs[0], "fld");
  // Create and check aggregation expressions.
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gminFUNC, expr), recs, min(vecd),
        "minDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gmaxFUNC, expr), recs, max(vecd),
        "maxDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gsumFUNC, expr), recs, sum(vecd),
        "sumDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gproductFUNC, expr), recs, product(vecd),
        "productDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gsumsqrFUNC, expr), recs,
        sum(vecd * vecd), "sumsqrDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gmeanFUNC, expr), recs, mean(vecd),
        "meanDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gvariance0FUNC, expr), recs,
        pvariance(vecd), "varianceDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gvariance1FUNC, expr), recs,
        variance(vecd), "varianceDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gstddev0FUNC, expr), recs, pstddev(vecd),
        "stddevDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gstddev1FUNC, expr), recs, stddev(vecd),
        "stddevDouble");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::grmsFUNC, expr), recs, rms(vecd),
        "rmsDouble");
  checkLazy(TableExprNode::newFunctionNode(TableExprFuncNode::gmedianFUNC, expr), recs,
            median(vecd), "medianDouble");
  checkLazy(TableExprNode::newFunctionNode(TableExprFuncNode::gfractileFUNC, expr, 0.65), recs,
            fractile(vecd, 0.65), "fractileDouble");
}
...
void doDComplexArr() {
  ...
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gmeanFUNC, expr), recs, mean(vecd),
        "meanDComplex");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gvariance0FUNC, expr), recs,
        real(pvariance(vecd)), "varianceDComplex");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gvariance1FUNC, expr), recs,
        real(variance(vecd)), "varianceDComplex");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gstddev0FUNC, expr), recs,
        real(pstddev(vecd)), "stddevDComplex");
  check(TableExprNode::newFunctionNode(TableExprFuncNode::gstddev1FUNC, expr), recs,
        real(stddev(vecd)), "stddevDComplex");
}
