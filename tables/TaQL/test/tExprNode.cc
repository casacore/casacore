...
void checkScaDComplex(const String& str, TableExprId& exprid, const TableExprNode& expr,
...
void checkScaString(const String& str, TableExprId& exprid, const TableExprNode& expr,
                    const String& value) {
...
void checkScaDate(const String& str, TableExprId& exprid, const TableExprNode& expr,
                  const MVTime& value) {
...
void checkArrDComplex(const String& str, TableExprId& exprid, const TableExprNode& expr,
...
void checkArrString(const String& str, TableExprId& exprid, const TableExprNode& expr,
...
void doIt() {
  ...
  checkArrDComplex("+ az-cc", exprid, earrz1 + Complex(10, 20), arrz1 + DComplex(10, 20));
  ...
  checkArrBool("== az-cc", exprid, earrz1 == Complex(10, 20), arrz1 == DComplex(10, 20));
  ...
  checkScaBool("near3 sd-sd", exprid, near(esd1, esd2, 1.e-7), near(sd1, sd2, 1.e-7));
  checkScaBool("nearAbs3 sd-sd", exprid, nearAbs(esd1, esd2, 1.e-7), nearAbs(sd1, sd2, 1.e-7));
  ...
  checkArrBool("near ad-ad", exprid, near(earrd1, arrd1), near(arrd1, arrd1, 1.e-13));
  checkArrBool("nearAbs ad-ad", exprid, nearAbs(earrd1, arrd1), nearAbs(arrd1, arrd1, 1.e-13));
  checkArrBool("near3 ad-ad", exprid, near(earrd1, earrd2, 1.e-7), near(arrd1, arrd2, 1.e-7));
  checkArrBool("nearAbs3 ad-ad", exprid, nearAbs(earrd1, earrd2, 1.e-7),
               nearAbs(arrd1, arrd2, 1.e-7));
  checkArrBool("near ad-sd", exprid, near(earrd1, sd1), near(arrd1, sd1arr, 1.e-13));
  checkArrBool("nearAbs ad-sd", exprid, nearAbs(earrd1, sd1), nearAbs(arrd1, sd1arr, 1.e-13));
  checkArrBool("near3 ad-sd", exprid, near(earrd1, esd2, 1.e-7), near(sd2arr, arrd1, 1.e-7));
  checkArrBool("nearAbs3 ad-sd", exprid, nearAbs(earrd1, esd2, 1.e-7),
               nearAbs(sd2arr, arrd1, 1.e-7));
  checkArrBool("near sd-sd", exprid, near(sd1, earrd1), near(arrd1, sd1arr, 1.e-13));
  checkArrBool("nearAbs sd-ad", exprid, nearAbs(sd1, earrd1), nearAbs(arrd1, sd1arr, 1.e-13));
  checkArrBool("near3 sd-ad", exprid, near(esd2, earrd1, 1.e-7), near(sd2arr, arrd1, 1.e-7));
  ...
  checkScaBool("near3 sz-sz", exprid, near(esz1, esz2, 1.e-7), near(sz1, sz2, 1.e-7));
  checkScaBool("nearAbs3 sz-sz", exprid, nearAbs(esz1, esz2, 1.e-7), nearAbs(sz1, sz2, 1.e-7));
  ...
  checkArrBool("near az-az", exprid, near(earrz1, arrz1), near(arrz1, arrz1, 1.e-13));
  checkArrBool("nearAbs az-az", exprid, nearAbs(earrz1, arrz1), nearAbs(arrz1, arrz1, 1.e-13));
  checkArrBool("near3 az-az", exprid, near(earrz1, earrz2, 1.e-7), near(arrz1, arrz2, 1.e-7));
  checkArrBool("nearAbs3 az-az", exprid, nearAbs(earrz1, earrz2, 1.e-7),
               nearAbs(arrz1, arrz2, 1.e-7));
  checkArrBool("near az-sz", exprid, near(earrz1, sz1), near(arrz1, sz1arr, 1.e-13));
  checkArrBool("nearAbs az-sz", exprid, nearAbs(earrz1, sz1), nearAbs(arrz1, sz1arr, 1.e-13));
  checkArrBool("near3 az-sz", exprid, near(earrz1, esz2, 1.e-7), near(sz2arr, arrz1, 1.e-7));
  checkArrBool("nearAbs3 az-sz", exprid, nearAbs(earrz1, esz2, 1.e-7),
               nearAbs(sz2arr, arrz1, 1.e-7));
  checkArrBool("near sz-az", exprid, near(sz1, earrz1), near(arrz1, sz1arr, 1.e-13));
  checkArrBool("nearAbs sz-az", exprid, nearAbs(sz1, earrz1), nearAbs(arrz1, sz1arr, 1.e-13));
  checkArrBool("near3 sz-az", exprid, near(esz2, earrz1, 1.e-7), near(sz2arr, arrz1, 1.e-7));
  ...
  checkArrBool("near sd-az", exprid, near(esd2, arrz1), near(sdz2arr, arrz1, 1.e-13));
  ...
  checkArrBool("near ai-az", exprid, near(earri1, arrz1), near(arriz1, arrz1, 1.e-13));
  ...
  checkArrDComplex("pow ad-az", exprid, pow(earrd2, earrz1), pow(arrdz2, arrz1));
  ...
  checkArrDComplex("pow az-ad", exprid, pow(earrz2, earrd1), pow(arrz2, arrdz1));
  ...
  checkArrDComplex("complex ad-ad", exprid, formComplex(real(earrz1), imag(earrz1)), arrz1);
  ...
  checkArrDouble("atan2 ad-ad", exprid, atan2(earrd1, earrd2), atan2(arrd1, arrd2));
  checkArrDouble("atan2 ai-ad", exprid, atan2(earri1, earrd2), atan2(arrid1, arrd2));
  checkArrDouble("atan2 ad-sd", exprid, atan2(earrd2, esd1), atan2(arrd2, sd1arr));
  checkArrDouble("atan2 sd-ad", exprid, atan2(esd1, earrd2), atan2(sd1arr, arrd2));
  ...
  checkArrDComplex("min az-az", exprid, min(earrz2, earrz1), ::min(arrz2, arrz1));
  checkArrDComplex("min sz-az", exprid, min(esz2, earrz1), min(arrz1, sz2));
  checkArrDComplex("min az-sz", exprid, min(earrz1, esz2), ::min(arrz1, sz2arr));
  checkScaDComplex("min sd-sz", exprid, min(esd2, esz1), min(sdz2, sz1));
  checkArrDComplex("min ad-az", exprid, min(earrd2, earrz1), ::min(arrdz2, arrz1));
  checkArrDComplex("min sd-az", exprid, min(esd2, earrz1), min(arrz1, sdz2));
  checkArrDComplex("min ad-sz", exprid, min(earrd1, esz2), ::min(arrdz1, sz2arr));
  checkScaDComplex("min sz-sd", exprid, min(esz2, esd1), min(sz2, sdz1));
  checkArrDComplex("min az-ad", exprid, min(earrz2, earrd1), ::min(arrz2, arrdz1));
  ...
  checkArrDComplex("max az-az", exprid, max(earrz2, earrz1), ::max(arrz2, arrz1));
  checkArrDComplex("max sz-az", exprid, max(esz2, earrz1), max(arrz1, sz2));
  checkArrDComplex("max az-sz", exprid, max(earrz1, esz2), ::max(arrz1, sz2arr));
  ...
  checkArrDComplex("max ad-sz", exprid, max(earrd1, esz2), ::max(arrdz1, sz2arr));
  checkScaDComplex("max sz-sd", exprid, max(esz2, esd1), max(sz2, sdz1));
  checkArrDComplex("max az-ad", exprid, max(earrz2, earrd1), ::max(arrz2, arrdz1));
  ...
  checkArrString("substr(trim(as),-10,2)", exprid, substr(trim(earrs1), -10, 2),
                 Vector<String>(arrs1.size(), "a1"));
  checkArrString("substr(as,-100,-1)", exprid, substr(earrs1, -100, -1),
  ...
  checkScaString("replace(ss,ss2,ss)", exprid, replace(TableExprNode("abcdefab"), "ab", "xyz"),
                 "xyzcdefxyz");
  checkScaString("replace(ss,rg2)", exprid, replace(TableExprNode("abcdefab"), Regex("a.")),
                 "cdef");
  checkScaString("replace(ss,rg2,ss)", exprid,
                 replace(TableExprNode("abcdefab"), Regex("a."), "xaz"), "xazcdefxaz");
  checkScaString("replace(ss,rg1,ss)", exprid,
