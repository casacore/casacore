...
void doIt() {
  ...
  AlwaysAssertExit(!result);
  TableExprNode expr3(RecordGram::parse(rec, "sub1.sub2.fld1 == 1 && fld1 > 1"));
  ...
  TableExprNode expr4a(RecordGram::parse(rec, "isdefined (sub1.sub2.fld1)"));
  ...
  AlwaysAssertExit(result);
  TableExprNode expr5b(RecordGram::parse(rec, "nelements (shape (fld1)) == 0"));
  ...
  TableExprNode expr7a(RecordGram::parse(rec, "all(shape(arr1) in shape(arr1))"));
  expr7a.get(rec, result);
  AlwaysAssertExit(result);
  TableExprNode expr7b(RecordGram::parse(rec, "any(shape(arr1) in shape(arr1)-1)"));
  expr7b.get(rec, result);
  AlwaysAssertExit(!result);
  // Check if an array in a set works fine.
  TableExprNode expr7c(RecordGram::parse(rec, "all(shape(arr1) in [6,8,12])"));
  expr7c.get(rec, result);
  AlwaysAssertExit(result);
  TableExprNode expr7d(RecordGram::parse(rec, "any(shape(arr1) in [5,7,11])"));
  expr7d.get(rec, result);
  AlwaysAssertExit(!result);
  // Check if an array in a set works fine.
  TableExprNode expr7e(RecordGram::parse(rec, "all([6,8,12] in shape(arr1))"));
  expr7e.get(rec, result);
  AlwaysAssertExit(result);
  TableExprNode expr7f(RecordGram::parse(rec, "any([5,7,11] in shape(arr1))"));
  expr7f.get(rec, result);
  AlwaysAssertExit(!result);
  // Check if a set in a set works fine.
  TableExprNode expr7g(RecordGram::parse(rec, "all([6,8,12] in [6,8,12])"));
  expr7g.get(rec, result);
  AlwaysAssertExit(result);
  TableExprNode expr7h(RecordGram::parse(rec, "any([5,7,11] in [6,8,12])"));
...
void testExpr2() {
  ...
  AlwaysAssertExit(arrs.shape() == IPosition(1, 1) && arrs.data()[0] == "str");
  AlwaysAssertExit(arrm.shape() == IPosition(1, 1) &&
                   near(arrm.data()[0].second(), MVTime(Time(2017, 3, 12, 12, 34, 56.7)).second()));
  ...
  AlwaysAssertExit(arrd.shape() == IPosition(1, 2) && arrd.data()[0] == 0.1 &&
                   arrd.data()[1] == 1.2);
  AlwaysAssertExit(arrc.shape() == IPosition(1, 2) && arrc.data()[0] == DComplex(10, 0) &&
                   arrc.data()[1] == DComplex(0, 10));
  AlwaysAssertExit(arrs.shape() == IPosition(1, 2) && arrs.data()[0] == "str" &&
                   arrs.data()[1] == "xy");
  AlwaysAssertExit(
      arrm.shape() == IPosition(1, 2) &&
      near(arrm.data()[0].second(), MVTime(Time(2017, 3, 12, 12, 34, 56.7)).second()) &&
      near(arrm.data()[1].second(), MVTime(Time(2017, 3, 14, 12, 34, 56.7)).second()));
}
