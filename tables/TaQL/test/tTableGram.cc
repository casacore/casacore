...
void testUnit(const String& comm, double expResult, const String& expUnit) {
  ...
  if (!near(node.getDouble(0), expResult) || node.unit().getName() != expUnit) {
  ...
  for (uInt i = 0; i < colnam.nelements(); i++) {
    if (!tab.tableDesc().isColumn(colnam(i))) {
      ...
      if (!tableColumns[nrcol]->columnDesc().isScalar() &&
        ...
        cout << "Column " << colnam(i) << " contains scalars nor arrays" << endl;
...
void seltab(const String& str) {
  ...
  if (spos != String::npos) {
    ...
    if (epos == String::npos) {
      ...
      addCalc = !(s == "with" || s == "select" || s == "update" || s == "insert" || s == "calc" ||
                  s == "delete" || s == "count" || s == "create" || s == "createtable" ||
                  s == "drop" || s == "droptable" || s == "alter" || s == "altertable" ||
                  s == "using" || s == "usingstyle" || s == "time" || s == "show" || s == "help");
    }
  }
  String strc(str);
  if (addCalc) {
  ...
  if (result.isTable()) {
    ...
    cout << "    " << cmd << " result of " << tabp->nrow() << " rows" << endl;
