...
int main() {
  ...
  tableCommand(
      "CREATE TABLE tTableGramError_tmp.tab1 [cb bool, ci int, cd double [unit='Hz'], cs string, "
      "cc complex, ct date, cai int [shape=[3,4]]] limit 1");
  tableCommand(
      "CREATE TABLE tTableGramError_tmp.tab2 [cb bool, ci int, cd double, cs string, cc complex, "
      "ct date] limit 2");
  int n = 0;
  // Incomplete commands
  ...
  n += errCmd(
      "select ci from tTableGramError_tmp.tab1 where ci>max([select from tTableGramError_tmp.tab1 "
      "giving [cai]])");

  if (n > 0) {
    cerr << "***Error*** " << n << " commands succeeded, but were expected to fail." << endl;
    return 1;
  }
  return 0;
}
