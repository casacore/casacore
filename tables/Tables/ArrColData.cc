...
ArrayColumnData::ArrayColumnData(const ArrayColumnDescBase* cd, ColumnSet* csp)
  ...
  checkValueLength_p = (columnDesc().dataType() == TpString && columnDesc().maxLength() > 0);
}
...
void ArrayColumnData::createDataManagerColumn() {
  ...
  if ((colDescPtr_p->options() & ColumnDesc::Direct) == ColumnDesc::Direct) {
    dataColPtr_p = dataManPtr_p->createDirArrColumn(colDescPtr_p->name(), colDescPtr_p->dataType(),
                                                    colDescPtr_p->dataTypeId());
  } else {
    dataColPtr_p = dataManPtr_p->createIndArrColumn(colDescPtr_p->name(), colDescPtr_p->dataType(),
                                                    colDescPtr_p->dataTypeId());
  }
  // # Check if the shape is defined in case fixed.
  // # Pass it to the created data manager column.
  if ((colDescPtr_p->options() & ColumnDesc::FixedShape) == ColumnDesc::FixedShape) {
    if (!shapeColDef_p) {
      throw(
          TableInvOper("ArrayColumnData::createDataManagerColumn; "
...
void ArrayColumnData::setShapeColumn(const IPosition& shp) {
  if (shapeColDef_p) {
    if (shp != shapeCol_p) {
      throw(
          TableInvOper("ArrayColumnData: change in shape of FixedShape array"
                       " of column " +
                       colDescPtr_p->name()));
    }
  }
...
void ArrayColumnData::getSlice(rownr_t rownr, const Slicer& ns, ArrayBase& array) const {
  if (rtraceColumn_p) {
...
void ArrayColumnData::putArray(rownr_t rownr, const ArrayBase& array) {
  if (wtraceColumn_p) {
    TableTrace::trace(traceId(), columnDesc().name(), 'w', rownr, array.shape());
  }
...
void ArrayColumnData::putSlice(rownr_t rownr, const Slicer& ns, const ArrayBase& array) {
  if (wtraceColumn_p) {
    TableTrace::trace(traceId(), columnDesc().name(), 'w', rownr, array.shape(), ns.start(),
                      ns.end(), ns.stride());
  }
  if (checkValueLength_p) {
    checkValueLength(static_cast<const Array<String>*>(&array));
...
void ArrayColumnData::getArrayColumnCells(const RefRows& rownrs, ArrayBase& array) const {
  if (rtraceColumn_p) {
...
void ArrayColumnData::getColumnSlice(const Slicer& ns, ArrayBase& array) const {
  if (rtraceColumn_p) {
...
void ArrayColumnData::getColumnSliceCells(const RefRows& rownrs, const Slicer& ns,
...
void ArrayColumnData::putArrayColumn(const ArrayBase& array) {
  if (wtraceColumn_p) {
    TableTrace::trace(traceId(), columnDesc().name(), 'w', array.shape());
  }
  if (checkValueLength_p) {
    checkValueLength(static_cast<const Array<String>*>(&array));
  }
...
void ArrayColumnData::putArrayColumnCells(const RefRows& rownrs, const ArrayBase& array) {
  if (wtraceColumn_p) {
    TableTrace::trace(traceId(), columnDesc().name(), 'w', rownrs, array.shape());
  }
  if (checkValueLength_p) {
    checkValueLength(static_cast<const Array<String>*>(&array));
  }
...
void ArrayColumnData::putColumnSlice(const Slicer& ns, const ArrayBase& array) {
  if (wtraceColumn_p) {
    TableTrace::trace(traceId(), columnDesc().name(), 'w', array.shape(), ns.start(), ns.end(),
                      ns.stride());
  }
  if (checkValueLength_p) {
    checkValueLength(static_cast<const Array<String>*>(&array));
...
void ArrayColumnData::putColumnSliceCells(const RefRows& rownrs, const Slicer& ns,
                                          const ArrayBase& array) {
  if (wtraceColumn_p) {
    TableTrace::trace(traceId(), columnDesc().name(), 'w', rownrs, array.shape(), ns.start(),
                      ns.end(), ns.stride());
  }
  if (checkValueLength_p) {
    checkValueLength(static_cast<const Array<String>*>(&array));
