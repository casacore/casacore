...
ArrayColumnBase::ArrayColumnBase() : TableColumn() {}

ArrayColumnBase::ArrayColumnBase(const Table& tab, const String& columnName)
    : TableColumn(tab, columnName) {}

ArrayColumnBase::ArrayColumnBase(const TableColumn& column) : TableColumn(column) {}

ArrayColumnBase::ArrayColumnBase(const ArrayColumnBase& that) : TableColumn(that) {}

void ArrayColumnBase::reference(const ArrayColumnBase& that) { TableColumn::reference(that); }
...
void ArrayColumnBase::acbGetSlice(rownr_t rownr, const Slicer& arraySection, ArrayBase& arr,
  ...
  if (arraySection.isFixed()) {
    ...
    baseColPtr_p->getSlice(rownr, Slicer(blc, trc, inc, Slicer::endIsLast), arr);
  }
}
...
void ArrayColumnBase::acbGetColumnCells(const RefRows& rownrs, const Slicer& arraySection,
  ...
  if (nrrow > 0) {
    arrshp = arraySection.inferShapeFromSource(shape(rownrs.firstRow()), arrblc, arrtrc, arrinc);
  }
  // # Total shape is slice shape plus nr of table rows.
  arrshp.append(IPosition(1, nrrow));
  // Check array conformance and resize if needed and possible.
...
void ArrayColumnBase::setShape(rownr_t rownr, const IPosition& shape) {
  ...
  if (canChangeShape_p || !isDefined(rownr)) {
    ...
    if (!shape.isEqual(baseColPtr_p->shape(rownr))) {
      throw TableInvOper("ArrayColumn::setShape; shape cannot be changed for row " +
...
void ArrayColumnBase::setShape(rownr_t rownr, const IPosition& shape, const IPosition& tileShape) {
  checkWritable();
  TABLECOLUMNCHECKROW(rownr);
...
void ArrayColumnBase::acbPutSlice(rownr_t rownr, const Slicer& arraySection, const ArrayBase& arr) {
  checkWritable();
  TABLECOLUMNCHECKROW(rownr);
...
void ArrayColumnBase::acbPutSlice(rownr_t rownr, const Vector<Vector<Slice>>& arraySlices,
                                  const ArrayBase& arr) {
  checkWritable();
  TABLECOLUMNCHECKROW(rownr);
...
void ArrayColumnBase::acbPutColumn(const Slicer& arraySection, const ArrayBase& arr) {
  checkWritable();
  rownr_t nrrow = nrow();
  // # First check if number of rows matches.
...
void ArrayColumnBase::acbPutColumn(const Vector<Vector<Slice>>& arraySlices, const ArrayBase& arr) {
  checkWritable();
  rownr_t nrrow = nrow();
  // Get total shape.
  // Use shape of first row (if there) as overall array shape.
  ...
  if (!shp.isEqual(arr.shape())) {
    throw(TableArrayConformanceError("ArrayColumn::putColumn for column " +
...
void ArrayColumnBase::acbPutColumnRange(const Slicer& rowRange, const ArrayBase& arr) {
  rownr_t nrrow = nrow();
  IPosition shp, blc, trc, inc;
  shp = rowRange.inferShapeFromSource(IPosition(1, nrrow), blc, trc, inc);
  // # If the entire column is accessed, use that function.
...
void ArrayColumnBase::acbPutColumnCells(const RefRows& rownrs, const ArrayBase& arr) {
  checkWritable();
  // # First check if number of rows matches.
  rownr_t nrrow = rownrs.nrow();
  IPosition arrshp = arr.shape();
...
void ArrayColumnBase::acbPutColumnRange(const Slicer& rowRange, const Slicer& arraySection,
                                        const ArrayBase& arr) {
  rownr_t nrrow = nrow();
  IPosition shp, blc, trc, inc;
  shp = rowRange.inferShapeFromSource(IPosition(1, nrrow), blc, trc, inc);
...
void ArrayColumnBase::acbPutColumnCells(const RefRows& rownrs, const Slicer& arraySection,
                                        const ArrayBase& arr) {
  checkWritable();
  // # First check if number of rows matches.
  rownr_t nrrow = rownrs.nrow();
  ...
  if ((columnDesc().options() & ColumnDesc::FixedShape) == ColumnDesc::FixedShape) {
    ...
    arrshp2 = arraySection.inferShapeFromSource(shapeColumn(), arrblc, arrtrc, arrinc);
...
void ArrayColumnBase::acbPutColumnCells(const RefRows& rows,
  ...
  std::unique_ptr<ArrayPositionIterator> arrIter = source.makeIterator(source.ndim() - 1);
...
void ArrayColumnBase::acbGetColumnCells(const RefRows& rows, const ColumnSlicer& columnSlicer,
  ...
  RefRowsSliceIter rowIter(rows);
  std::unique_ptr<ArrayPositionIterator> arrIter = destination.makeIterator(destination.ndim() - 1);
  while (!rowIter.pastEnd()) {
    for (rownr_t rownr = rowIter.sliceStart(); rownr <= rowIter.sliceEnd();
         rownr += rowIter.sliceIncr()) {
...
void ArrayColumnBase::acbPutColumnCells(const RefRows& rows, const ColumnSlicer& columnSlicer,
                                        const ArrayBase& source) {
...
void ArrayColumnBase::acbPutColumn(const ArrayColumnBase& that) {
  ...
  rownr_t nrrow = nrow();
  if (nrrow != that.nrow()) {
    throw TableConformanceError("Nr of rows differs in ArrayColumn::putColumn for column " +
...
void ArrayColumnBase::handleSlices(const Vector<Vector<Slice>>& slices, BaseSlicesFunctor& functor,
                                   const Slicer& slicer, const ArrayBase& arr) const {
...
ColumnSlicer::ColumnSlicer(const IPosition& shape, const Vector<Slicer*>& dataSlicers,
                           const Vector<Slicer*>& destinationSlicers)
    : dataSlicers_p(dataSlicers), destinationSlicers_p(destinationSlicers), shape_p(shape) {
...
ColumnSlicer::~ColumnSlicer() { freeSlicers(); }
  ...
  for (uInt i = 0; i < dataSlicers_p.size(); i++) {
    if (dataSlicers_p[i]->length() != destinationSlicers_p[i]->length()) {
      return FormatString(
          ...
          i, dataSlicers_p[i]->length().toString().c_str(), i,
          destinationSlicers_p[i]->length().toString().c_str());
    }
  }
  return String();
}
