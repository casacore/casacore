...
class ArrayColumnBase : public TableColumn {
 public:
  ...
  // <group>
  void baseGet(rownr_t rownr, ArrayBase& array) const { baseColPtr_p->getArray(rownr, array); }
  ...
  // E.g., they are used internally in virtual column engines.
  // <group>
  void basePut(rownr_t rownr, const ArrayBase& array) { baseColPtr_p->putArray(rownr, array); }
  ...
  // <group>
  void acbPutColumnCells(const RefRows& rows, const Vector<Vector<Slice>>& arraySlices,
                         const ArrayBase& source);
  void acbPutColumnCells(const RefRows& rows, const ColumnSlicer& columnSlicer,
                         const ArrayBase& source);
  // </group>
  ...
  void handleSlices(const Vector<Vector<Slice>>& slices, BaseSlicesFunctor& functor,
                    const Slicer& slicer, const ArrayBase& array) const;
};
...
class ColumnSlicer {
 public:
  ...
  // the row axis.
  ColumnSlicer(const IPosition& shape, const Vector<Slicer*>& dataSlicers,
  ...
  const Vector<Slicer*>& getDataSlicers() const { return dataSlicers_p; }

  // Get the desintation slicers.
  const Vector<Slicer*>& getDestinationSlicers() const { return destinationSlicers_p; }
