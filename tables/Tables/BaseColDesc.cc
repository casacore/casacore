...
BaseColumnDesc::BaseColumnDesc(const String& name, const String& comment, const String& dataManType,
  ...
  if (dtype_p == TpOther) {
    if ((option_p & ColumnDesc::Undefined) == ColumnDesc::Undefined) {
      throw(TableInvColumnDesc(name, "Option Undefined only allowed for standard data types"));
    }
  }
...
BaseColumnDesc::~BaseColumnDesc() { delete keySetPtr_p; }
...
void BaseColumnDesc::setNdim(uInt ndim) {
  ...
  if (ndim > 0 && nrdim_p > 0) {
    throw(TableInvOper("setNdim(): dimensionality of column " + colName_p + " already defined"));
  }
  nrdim_p = ndim;
  if (nrdim_p <= 0) {
    nrdim_p = -1;
...
void BaseColumnDesc::setShape(const IPosition& shape) {
  if (!isArray()) {
    throw(TableInvOper("setShape: column " + colName_p + " is no array"));
  }
  if (shape_p.nelements() > 0) {
...
void BaseColumnDesc::getFile(AipsIO& ios, const TableAttr& parentAttr) {
  ...
  if (dtype != dtype_p) {
    throw(
        TableInternalError("BaseColumnDesc: data type read mismatch"
                           " for column " +
                           colName_p));
  }
  ios >> option_p;
