...
BaseColumn::BaseColumn(const BaseColumnDesc* cdp) : colDescPtr_p(cdp) {}
...
void BaseColumn::get(rownr_t, void*) const {
  throw(TableInvOper("get() not implemented for column " + colDesc_p.name() +
                     "; only valid for a scalar"));
}

void BaseColumn::getArray(rownr_t, ArrayBase&) const {
  throw(TableInvOper("getArray() not implemented for column " + colDesc_p.name() +
                     "; only valid for an array"));
}

void BaseColumn::getSlice(rownr_t, const Slicer&, ArrayBase&) const {
  throw(TableInvOper("getSlice() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::getScalarColumn(ArrayBase&) const {
  throw(TableInvOper("getScalarColumn() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for a scalar"));
}

void BaseColumn::getArrayColumn(ArrayBase&) const {
  throw(TableInvOper("getArrayColumn() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::getScalarColumnCells(const RefRows&, ArrayBase&) const {
  throw(TableInvOper("getScalarColumnCells() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for a scalar"));
}

void BaseColumn::getArrayColumnCells(const RefRows&, ArrayBase&) const {
  throw(TableInvOper("getArrayColumnCells() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::getColumnSliceCells(const RefRows&, const Slicer&, ArrayBase&) const {
  throw(TableInvOper("getColumnCells(Slicer&) not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::getColumnSlice(const Slicer&, ArrayBase&) const {
  throw(TableInvOper("getColumn(Slicer&) not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::put(rownr_t, const void*) {
  throw(TableInvOper("put() not implemented for column " + colDesc_p.name() +
                     "; only valid for a scalar"));
}

void BaseColumn::putArray(rownr_t, const ArrayBase&) {
  throw(TableInvOper("putArray() not implemented for column " + colDesc_p.name() +
                     "; only valid for an array"));
}

void BaseColumn::putSlice(rownr_t, const Slicer&, const ArrayBase&) {
  throw(TableInvOper("putSlice() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::putScalarColumn(const ArrayBase&) {
  throw(TableInvOper("putScalarColumn() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for a scalar"));
}

void BaseColumn::putArrayColumn(const ArrayBase&) {
  throw(TableInvOper("putArrayColumn() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::putScalarColumnCells(const RefRows&, const ArrayBase&) {
  throw(TableInvOper("putScalarColumnCells() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for a scalar"));
}

void BaseColumn::putArrayColumnCells(const RefRows&, const ArrayBase&) {
  throw(TableInvOper("putArrayColumnCells() not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::putColumnSlice(const Slicer&, const ArrayBase&) {
  throw(TableInvOper("putColumn(Slicer&) not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}

void BaseColumn::putColumnSliceCells(const RefRows&, const Slicer&, const ArrayBase&) {
  throw(TableInvOper("putColumnCells(Slicer&) not implemented for column " + colDescPtr_p->name() +
                     "; only valid for an array"));
}
...
void BaseColumn::getScalar(rownr_t rownr, void* value, const String& dataTypeId) const {
  if (!colDescPtr_p->isScalar()) {
    throwGetScalar();
  }
  if (colDescPtr_p->dataType() != TpOther || colDescPtr_p->dataTypeId() != dataTypeId) {
    throwGetType("void*");
  }
  get(rownr, value);
}
...
void BaseColumn::throwGetType(const String& type) const {
  throw(TableInvDT("invalid type promotion in getScalar(" + type + ") for column " +
                   colDescPtr_p->name() + " with type " +
                   ValType::getTypeStr(colDescPtr_p->dataType())));
}

void BaseColumn::throwPutType(const String& type) const {
  throw(TableInvDT("invalid type promotion in putScalar(" + type + ") for column " +
                   colDescPtr_p->name() + " with type " +
                   ValType::getTypeStr(colDescPtr_p->dataType())));
}
