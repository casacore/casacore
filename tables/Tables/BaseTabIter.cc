...
BaseTableIterator::BaseTableIterator(const std::shared_ptr<BaseTable>& btp,
  ...
  if (option == TableIterator::NoSort) {
    ...
    sortTab_p =
        btp->sort(keys, cmpObj_p, ord, sortopt, sortIterBoundaries_p, sortIterKeyIdxChange_p);
  }
...
void BaseTableIterator::copyState(const BaseTableIterator& other) {
  ...
  if (sortIterBoundaries_p) {
    ...
    std::advance(sortIterBoundariesIt_p,
                 std::distance(other.sortIterBoundaries_p->begin(), other.sortIterBoundariesIt_p));
  }
  if (sortIterKeyIdxChange_p) {
    sortIterKeyIdxChangeIt_p = sortIterKeyIdxChange_p->begin();
    std::advance(sortIterKeyIdxChangeIt_p, std::distance(other.sortIterKeyIdxChange_p->begin(),
