...
#ifdef USE_THREADS
    int provided;
    safe_mpi_invoke(MPI_Init_thread, "Error when initializing MPI", nullptr, nullptr,
...
Bool BaseTable::makeTableDir() {
  ...
  if (!openedForWrite()) {
#ifndef HAVE_MPI
    throw(TableInvOpt("BaseTable::makeTableDir", "must be Table::New, NewNoReplace or Update"));
#endif
  }
  ...
  if (file.exists()) {
    if (!file.isDirectory()) {
      throw(TableDuplFile(name_p, " (and is not a true table directory)"));
    }
    Directory tdir(file);
    if (!tdir.isEmpty()) {
...
void BaseTable::prepareCopyRename(const String& newName, int tableOption) const {
  // Options Delete and Old are wrong.
  if (tableOption == Table::Old || tableOption == Table::Delete) {
    throw(TableInvOpt("BaseTable::rename", "must be Table::New, NewNoReplace, Scratch or Update"));
  }
  ...
  if (fileNew.exists()) {
    if (!fileNew.isDirectory()) {
      throw(TableDuplFile(newName, " (and is not a true table directory)"));
    }
...
void BaseTable::rename(const String& newName, int tableOption) {
  ...
  if (madeDir_p) {
    File file(name_p);
    if (!file.isWritable()) {
      throw(TableInvOper("Table file " + name_p + " is readonly and cannot be renamed"));
    }
  }
...
void BaseTable::trueDeepCopy(const String& newName, const Record& dataManagerInfo,
  ...
  if (absNewName == name_p) {
    throw TableError("Table::deepCopy: new name equal to old name " + name_p);
  }
...
void BaseTable::copy(const String& newName, int tableOption) const {
  ...
  if (absNewName != name_p) {
    ...
    if (!madeDir_p) {
      throw(TableError("BaseTable::copy: no input table files exist for " + name_p));
    }
    // # Flush the data and subtables.
...
void BaseTable::addColumns(const TableDesc& desc, const Record& dmInfo, Bool addToParent) {
  ...
  if (rec.isDefined("TYPE") && rec.isDefined("NAME")) {
    ...
    throw TableError(
        "Invalid dmInfo record given in Table::addColumn"
        " for table " +
        name_p);
  }
}
...
Vector<rownr_t>& BaseTable::rowStorage() {
  throw(TableInvOper("rowStorage() not possible; table " + name_p + " is no RefTable"));
}
...

{
  ...
  return doSort(sortCol, cmpObj, order, option, sortIterBoundaries, sortIterKeyIdxChange);
}
...
std::shared_ptr<BaseTable> BaseTable::select(const TableExprNode& node, rownr_t maxRow,
                                             rownr_t offset) {
  ...
  if (node.dataType() != TpBool || !node.isScalar()) {
    throw(TableInvExpr("select expression result on table " + name_p + " is not Bool scalar"));
  }
...
std::shared_ptr<BaseTable> BaseTable::tabOr(BaseTable* that) {
  ...
  if (this->nrow() == this->root()->nrow() || that->nrow() == that->root()->nrow()) {
...
BaseTableIterator* BaseTable::makeIterator(const Block<String>& names,
  ...
  BaseTableIterator* bti = new BaseTableIterator(shared_from_this(), names, cmpObj, order, option,
...
void BaseTable::checkRowNumberThrow(rownr_t rownr) const {
  throw(TableError("TableColumn: row number " + std::to_string(rownr) + " exceeds #rows " +
                   std::to_string(nrrow_p + nrrowToAdd_p) + " in table " +
...
void BaseTable::showStructure(ostream& os, Bool showDataMans, Bool showColumns, Bool showSubTables,
  ...
  os << " (using " << dminfo.nfields() << " data managers)" << endl;
  ...
  if (!showDataMans) {
    ...
    for (uInt i = 0; i < dminfo.nfields(); ++i) {
      ...
      if (showColumns) {
        showColumnInfo(os, tdesc, maxl, dm.asArrayString("COLUMNS"), sortColumns, cOrder);
      }
    }
  }
    ...
    for (uInt i = 0; i < keywords.nfields(); ++i) {
      if (keywords.dataType(i) == TpTable) {
        ...
        if (tab.isSameRoot(Table(this))) {
          os << endl << "Subtable " << keywords.name(i) << " references the parent table!!" << endl;
        } else {
          tab.showStructure(os, showDataMans, showColumns, showSubTables, sortColumns, cOrder);
        }
      }
    }
  }
