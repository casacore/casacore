...
ColumnDescSet::ColumnDescSet() : colSeq_p(0) {}

ColumnDescSet::ColumnDescSet(const ColumnDescSet& that) : colSeq_p(0) { operator=(that); }
...
ColumnDesc& ColumnDescSet::addColumn(const ColumnDesc& cd, const String& newname) {
...
void ColumnDescSet::rename(const String& newname, const String& oldname) {
  if (!isDefined(oldname)) {
    throw(AipsError("TableDesc::renameColumn - old name " + oldname + " does not exist"));
  }
