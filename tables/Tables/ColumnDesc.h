...
class ColumnDesc {
 ...
 public:
  ...
  // <group>
  TableRecord& rwKeywordSet() { return colPtr_p->rwKeywordSet(); }
  const TableRecord& keywordSet() const { return colPtr_p->keywordSet(); }
  // </group>
  ...
  // contains arrays.
  DataType dataType() const { return colPtr_p->dataType(); }
  ...
  const String& dataTypeId() const { return colPtr_p->dataTypeId(); }

  // Get the type name of the default data manager.
  const String& dataManagerType() const { return colPtr_p->dataManagerType(); }

  // Get the type name of the default data manager
  // (allowing it to be changed).
  String& dataManagerType() { return colPtr_p->dataManagerType(); }

  // Get the data manager group.
  const String& dataManagerGroup() const { return colPtr_p->dataManagerGroup(); }
  ...
  String& comment() { return colPtr_p->comment(); }
  ...
  //    }
  // </srcblock>
  int options() const { return colPtr_p->options(); }
  ...
  const IPosition& shape() const { return colPtr_p->shape(); }
  ...
  // <group>
  const TableDesc* tableDesc() const { return colPtr_p->tableDesc(); }
  TableDesc* tableDesc() { return colPtr_p->tableDesc(); }
  // </group>
  ...
  void setName(const String& name) { colPtr_p->setName(name); }
  ...
  ConcatColumn* makeConcatColumn(ConcatTable* rtp) const { return colPtr_p->makeConcatColumn(rtp); }
 ...
 private:
  ...
  void checkAdd(const ColumnDescSet& cds) const { colPtr_p->checkAdd(cds); }
  ...
  // <group>
  void handleAdd(ColumnDescSet& cds) { colPtr_p->handleAdd(cds); }
  void handleRename(ColumnDescSet& cds, const String& oldName) {
    colPtr_p->handleRename(cds, oldName);
  }
  void handleRemove(ColumnDescSet& cds) { colPtr_p->handleRemove(cds); }
  // </group>
  ...
  PlainColumn* makeColumn(ColumnSet* csp) const { return colPtr_p->makeColumn(csp); }
