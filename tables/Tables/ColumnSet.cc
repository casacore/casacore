...
void ColumnSet::openMultiFile(uInt from, const Table& tab, ByteIO::OpenOption opt) {
  ...
  if (useMultiFile) {
    ...
    if (!multiFile_p) {
      if (storageOpt_p.option() == StorageOption::MultiFile) {
        multiFile_p =
            std::make_shared<MultiFile>(tab.tableName() + "/table.mf", opt,
                                        storageOpt_p.blockSize(), storageOpt_p.useODirect());
      } else {
        multiFile_p = std::make_shared<MultiHDF5>(tab.tableName() + "/table.mfh5", opt,
                                                  storageOpt_p.blockSize());
      }
    }
...
void ColumnSet::removeRow(rownr_t rownr) {
  if (!canRemoveRow()) {
    throw(TableInvOper("Rows cannot be removed from table " + baseTablePtr_p->tableName() +
                       "; its storage managers do not support it"));
  }
  if (rownr >= nrrow_p) {
    throw(TableInvOper("removeRow: rownr " + std::to_string(rownr) + " too high in table " +
...
void ColumnSet::addColumn(const ColumnDesc& columnDesc, Bool bigEndian, const TSMOption& tsmOption,
  ...
  dmptr = DataManager::getCtor(columnDesc.dataManagerType())(
      uniqueDataManagerName(columnDesc.dataManagerGroup()), Record());
...
void ColumnSet::addColumn(const ColumnDesc& columnDesc, const String& dataManager, Bool byName,
  ...
  if (dataManager.empty()) {
    throw(
        TableInvOper("Table::addColumn: no datamanager name/type given "
                     "when adding column " +
                     columnDesc.name() + " to table " + baseTablePtr_p->tableName()));
  }
  ...
  dmptr = DataManager::getCtor(dataManager)(uniqueDataManagerName(dataManager), Record());
  addColumn(columnDesc, *dmptr, bigEndian, tsmOption, tab);
  delete dmptr;
}

void ColumnSet::doAddColumn(const ColumnDesc& columnDesc, DataManager* dataManPtr) {
  if (!dataManPtr->canAddColumn()) {
    throw TableError("Table::addColumn - DataManager " + dataManPtr->dataManagerName() + " (" +
  ...
  for (uInt i = 0; i < columnNames.nelements(); i++) {
    ...
    if (dmCounts.at(dmPtr) >= 0 && !dmPtr->canRemoveColumn()) {
      throw TableInvOper("Table::removeColumn - column " + columnNames(i) +
                         " cannot be removed from table " + baseTablePtr_p->tableName());
    }
  }
  return dmCounts;
}
...
void ColumnSet::renameColumn(const String& newName, const String& oldName) {
  if (!tdescPtr_p->isColumn(oldName)) {
    throw(TableInvOper("Table::renameColumn; column " + oldName + " does not exist in table " +
...
DataManager* ColumnSet::findDataManager(const String& name, Bool byColumn) const {
  ...
  throw(
      TableInvOper("Data manager " + name + " is unknown in table " + baseTablePtr_p->tableName()));
}
...
rownr_t ColumnSet::getFile(AipsIO& ios, Table& tab, rownr_t nrrow, Bool bigEndian,
  ...
  openMultiFile(0, tab, tab.isWritable() ? ByteIO::Update : ByteIO::Old);
    ...
    if (thiscol->columnDesc() != othercol->columnDesc()) {
      throw(
          TableError("ColumnSet::syncColumns; another process "
                     ...
                     thiscol->columnDesc().name() + " in table " + baseTablePtr_p->tableName()));
    }
