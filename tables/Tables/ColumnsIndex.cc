...
ColumnsIndex::~ColumnsIndex() { deleteObjects(); }
...
void ColumnsIndex::addColumnToDesc(RecordDesc& description, const TableColumn& column) {
  ...
  if (!columnDesc.isScalar()) {
    throw(TableError("ColumnsIndex: column " + columnDesc.name() + " should be a scalar column"));
  }
...
void ColumnsIndex::fillRowNumbers(Vector<rownr_t>& rows, rownr_t start, rownr_t end) const {
