...
namespace casacore { //# NAMESPACE CASACORE - BEGIN
  ...
  uint32_t ConcatColumn::ndimColumn() const
  {
  ...
  uint32_t ConcatColumn::ndim (rownr_t rownr) const
  {
    uint32_t tableNr;
    rownr_t tabRownr;
    refTabPtr_p->rows().mapRownr (tableNr, tabRownr, rownr);
  ...
  void ConcatColumn::setMaximumCacheSize (uint32_t nbytes)
  {
				 ...
				 AccessRowsFunc* accessFunc) const
  {
    ...
    uint32_t rowAxis = arr.ndim() - 1;   // row axis in array
  ...
  void ConcatColumn::setColumnCache (uint32_t tableNr,
