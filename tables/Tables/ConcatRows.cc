...
ConcatRowsIter::ConcatRowsIter(const ConcatRows& rows)
    : itsRows(&rows), itsChunk(3), itsStart(0), itsEnd(rows.nrow()), itsIncr(1), itsTabNr(0) {
  itsPastEnd = (itsEnd == 0);
  itsChunk[0] = 0;
...
ConcatRowsIter::ConcatRowsIter(const ConcatRows& rows, rownr_t start, rownr_t end, rownr_t incr)
    : itsRows(&rows),
      itsChunk(3),
      itsStart(start),
...
void ConcatRowsIter::next() {
  if (!itsPastEnd) {
    if (itsTabNr + 1 >= itsRows->ntable() || (*itsRows)[itsTabNr] >= itsEnd) {
      ...
      itsChunk[1] = std::min((*itsRows)[itsTabNr + 1], itsEnd) - 1 - (*itsRows)[itsTabNr];
      ++itsTabNr;
    }
  }
}
