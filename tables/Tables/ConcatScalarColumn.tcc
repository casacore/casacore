...
template <typename T>
ConcatScalarColumn<T>::ConcatScalarColumn(const BaseColumnDesc* bcdp, ConcatTable* reftab)
...
template <typename T>
void ConcatScalarColumn<T>::getScalarColumnCells(const RefRows& rownrs, ArrayBase& arr) const {
  Vector<T>& vec = static_cast<Vector<T>&>(arr);
...
template <typename T>
void ConcatScalarColumn<T>::putScalarColumnCells(const RefRows& rownrs, const ArrayBase& arr) {
...
void ConcatScalarColumn<T>::fillSortKey(const Vector<T>* vecPtr, Sort& sortobj,
  ...
  sortobj.sortKey(datap, cmpObj, sizeof(T),
                  order == Sort::Descending ? Sort::Descending : Sort::Ascending);
