...
ConcatTable::ConcatTable(const Block<String>& tableNames, const Block<String>& subTables,
                         const String& subDirName, int option, const TableLock& lockOptions,
...
void ConcatTable::writeConcatTable(Bool) {
  ...
  if (changed_p) {
    ...
    for (uInt i = 0; i < tables_p.nelements(); ++i) {
      ...
      ios << Path::stripDirectory(tables_p[i].tableName(), tableName());
    }
...
int ConcatTable::tableType() const { return tables_p[0].tableType(); }

TableDesc ConcatTable::actualTableDesc() const { return *tdescPtr_p; }

Record ConcatTable::dataManagerInfo() const { return tables_p[0].dataManagerInfo(); }

// # Get the keyword set.
TableRecord& ConcatTable::keywordSet() { return keywordSet_p; }

// # Get the keyword set.
TableRecord& ConcatTable::rwKeywordSet() { return keywordSet_p; }
