...
void ReadAsciiTable::handleKeyset(Int lineSize, char* string1, char* first, char* second,
  ...
  while (True) {
    ...
    if (keyset.isDefined(keyName)) {
      logger << LogIO::WARN << "Keyword " << keyName << " skipped because defined twice in "
             << fileName << LogIO::POST;
    } else {
  ...
  if (keysets.isDefined(colName)) {
    logger << LogIO::WARN << "Keywordset of column " << colName
           << " skipped because defined twice in " << fileName << LogIO::POST;
  } else {
...
Int ReadAsciiTable::getTypeShape(const String& typestr, IPosition& shape, Int& type) {
  ...
  if (pos == 0) {
    throw AipsError("ReadAsciiTable: no type info in type string '" + typestr + "'");
  }
  ...
  for (uInt i = 0; i < vec.nelements(); i++) {
    ...
    if (shape(i) <= 0) {
      if (varAxis >= 0) {
        throw AipsError(
            "ReadAsciiTable: multiple variable axes in "
            "type string '" +
            typestr + "'");
      }
      varAxis = i;
...
Table ReadAsciiTable::makeTab(String& formatString, Table::TableType tableType,
  ...
  TableDesc td(tableproto, (tableproto.empty() ? TableDesc::Scratch : TableDesc::New));
  ifstream jFile;
  Path headerPath(headerfile);
  String hdrName = headerPath.expandedName();
  jFile.open(hdrName.c_str(), ios::in);
  if (!jFile) {
    throw AipsError("ReadAsciiTable: file " + hdrName + " not found or unreadable");
  }
  ...
  while (strncmp(string1, ".key", 4) == 0) {
    handleKeyset(lineSize, string1, first, second, keysets, logger, headerfile, jFile, lineNumber,
                 separator, testComment, commentMarker, firstHeaderLine, lastHeaderLine);
  }
  ...
  if (!autoHeader && !hdrGiven) {
    if (string1[0] == '\0') {
      throw AipsError("ReadAsciiTable: no COLUMN NAMES line in " + headerfile);
    }
    if (!getLine(jFile, lineNumber, string2, lineSize, testComment, commentMarker, firstHeaderLine,
                 lastHeaderLine)) {
      throw AipsError("ReadAsciiTable: no COLUMN TYPES line in " + headerfile);
    }
  }
  ...
  if (!oneFile) {
    ...
    if (!jFile) {
      throw AipsError("ReadAsciiTable: input file " + fileName + " not found or unreadable");
    }
    lineNumber = 0;
    if (autoHeader) {
      if (!getLine(jFile, lineNumber, string1, lineSize, testComment, commentMarker, firstLine,
  ...
  if (hdrGiven) {
    ...
    while (done1 >= 0) {
      ...
      if (done1 > 0 && done2 > 0) {
        ...
        throw AipsError(
            "ReadAsciiTable: mismatching COLUMN NAMES "
            "and TYPES lines in " +
            headerfile);
      }
    }
  ...
  for (Int i5 = 0; i5 < nrcol; i5++) {
    ...
    if (shapeOfColumn[i5].nelements() > 0) {
      ...
      switch (typeOfColumn[i5]) {
        ...
        case RATString:
          td.addColumn(ArrayColumnDesc<String>(nameOfColumn[i5], shape, option));
          break;
        case RATComX:
        case RATComZ:
          td.addColumn(ArrayColumnDesc<Complex>(nameOfColumn[i5], shape, option));
          break;
        case RATDComX:
        case RATDComZ:
          td.addColumn(ArrayColumnDesc<DComplex>(nameOfColumn[i5], shape, option));
          break;
      }
    } else {
      switch (typeOfColumn[i5]) {
  ...
  for (uInt i = 0; i < keysets.nfields(); i++) {
    ...
    if (colnm.empty()) {
      ...
      if (!tab.tableDesc().isColumn(colnm)) {
        logger << LogIO::WARN << "Keywordset of column " << colnm
               << " skipped because column is not defined in " << headerfile << LogIO::POST;
      } else {
  ...
  while (cont) {
    ...
    for (Int i6 = 0; i6 < nrcol; i6++) {
      if (shapeOfColumn[i6].nelements() > 0) {
        ...
        handleScalar(string1, lineSize, first, at1, separator, typeOfColumn[i6], tabcol[i6], rownr);
      }
    }
    rownr++;
    cont = getLine(jFile, lineNumber, string1, lineSize, testComment, commentMarker, firstLine,
...
Table readAsciiTable(String& formatString, Table::TableType tableType, const String& filein,
  ...
  return ReadAsciiTable::runt(formatString, tableType, filein, filein, tableproto, tablename,
                              autoHeader, autoShape, dumvec, dumvec, separator, commentMarkerRegex,
                              firstLine, lastLine);
}
