...
class RefRows {
 public:
  ...
  rownr_t nrows() const { return (itsNrows == 0 ? fillNrows() : itsNrows); }
  rownr_t nrow() const { return (itsNrows == 0 ? fillNrows() : itsNrows); }
  // </group>
  ...
  // contain slices.
  const Vector<rownr_t>& rowVector() const { return itsRows; }
...
class RefRowsSliceIter {
 public:
  ...
  // <group>
  rownr_t sliceStart() const { return itsStart; }
  rownr_t sliceEnd() const { return itsEnd; }
  rownr_t sliceIncr() const { return itsIncr; }
  // </group>
