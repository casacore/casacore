...
void RefTable::writeRefTable(Bool) {
  ...
  if (changed_p) {
    ...
    ios << Path::stripDirectory(baseTabPtr_p->tableName(), tableName());
    ios << nameMap_p;
...
void RefTable::getRef(AipsIO& ios, int opt, const TableLock& lockOptions,
  ...
  if (rootNrow > baseTabPtr_p->nrow()) {
    throw(TableInvOper("RefTable::getRef, #rows in referenced table decreased"));
  }
...
void RefTable::makeDesc(TableDesc& desc, const TableDesc& rootDesc,
                        std::map<String, String>& nameMap, Vector<String>& names) {
...
int RefTable::tableType() const { return baseTabPtr_p->tableType(); }
...
void RefTable::showStructureExtra(ostream& os) const {
  os << "out of " << baseTabPtr_p->tableName() << " (" << baseTabPtr_p->nrow() << " rows, "
     << baseTabPtr_p->tableDesc().ncolumn() << " columns)" << endl;
}

// # Get the keyword set.
TableRecord& RefTable::keywordSet() { return baseTabPtr_p->keywordSet(); }

// # Get the keyword set.
TableRecord& RefTable::rwKeywordSet() { return baseTabPtr_p->rwKeywordSet(); }
...
Vector<rownr_t>& RefTable::rowStorage() { return rowStorage_p; }
...
void RefTable::refAnd(rownr_t nr1, const rownr_t* inx1, rownr_t nr2, const rownr_t* inx2) {
...
void RefTable::refOr(rownr_t nr1, const rownr_t* inx1, rownr_t nr2, const rownr_t* inx2) {
...
void RefTable::refSub(rownr_t nr1, const rownr_t* inx1, rownr_t nr2, const rownr_t* inx2) {
...
void RefTable::refXor(rownr_t nr1, const rownr_t* inx1, rownr_t nr2, const rownr_t* inx2) {
  rownr_t allrow = nr1 + nr2;   // max #output rows
  rowStorage_p.resize(allrow);  // allocate output storage
  AlwaysAssert(rowStorage_p.contiguousStorage(), AipsError);
  rownr_t* rows = rowStorage_p.data();
