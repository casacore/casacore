...
void ColumnHolder::attach(const String &outCol, const String &inCol) {
  if (!out.tableDesc().isColumn(outCol) || !in.tableDesc().isColumn(inCol)) {
    throw(TableError("RowCopier: " + inCol + " or " + outCol + " is not a column"));
  }
  ...
  if (out.tableDesc()[outCol].isScalar() == in.tableDesc()[inCol].isScalar() &&
      out.tableDesc()[outCol].dataType() == in.tableDesc()[inCol].dataType()) {
    ...
    throw(TableError("RowCopier: " + inCol + " and " + outCol + " are not conformant"));
  }
}
...
RowCopier::RowCopier(Table &out, const Table &in, const Vector<String> &outNames,
