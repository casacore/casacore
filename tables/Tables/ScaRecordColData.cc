...
ScalarRecordColumnData::ScalarRecordColumnData(const ScalarRecordColumnDesc* cd, ColumnSet* csp)
    : PlainColumn(cd, csp) {}
...
void ScalarRecordColumnData::createDataManagerColumn() {
  ...
  dataColPtr_p = dataManPtr_p->createIndArrColumn(colDescPtr_p->name(), TpUChar, "");
}
...
void ScalarRecordColumnData::getScalarColumnCells(const RefRows& rownrs, ArrayBase& val) const {
  Vector<TableRecord>& vec = static_cast<Vector<TableRecord>&>(val);
...
void ScalarRecordColumnData::putScalarColumnCells(const RefRows& rownrs, const ArrayBase& val) {
  const Vector<TableRecord>& vec = static_cast<const Vector<TableRecord>&>(val);
  if (vec.nelements() != rownrs.nrow()) {
...
void ScalarRecordColumnData::allocIterBuf(void*&, void*&, std::shared_ptr<BaseCompare>&) {
  throw(
      TableError("Iterating on a column containing records "
                 "is not possible"));
}
