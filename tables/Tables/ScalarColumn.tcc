...
template <class T>
ScalarColumn<T>::ScalarColumn() : TableColumn() {}
...
template <class T>
ScalarColumn<T>::ScalarColumn(const TableColumn& column) : TableColumn(column) {
  checkDataType();
}

template <class T>
ScalarColumn<T>::ScalarColumn(const ScalarColumn<T>& that) : TableColumn(that) {}
...
template <class T>
void ScalarColumn<T>::put(rownr_t thisRownr, const ScalarColumn<T>& that, rownr_t thatRownr) {
...
template <class T>
void ScalarColumn<T>::putColumnRange(const Slicer& rowRange, const Vector<T>& vec) {
  rownr_t nrrow = nrow();
  IPosition shp, blc, trc, inc;
  shp = rowRange.inferShapeFromSource(IPosition(1, nrrow), blc, trc, inc);
  // # When the entire column is accessed, use that function.
...
template <class T>
void ScalarColumn<T>::putColumnCells(const RefRows& rownrs, const Vector<T>& vec) {
  checkWritable();
  // # Check the vector length.
  rownr_t nrrow = rownrs.nrow();
  if (vec.nelements() != nrrow) {
