...
class SetupNewTableRep {
 public:
  ...
  const String& name() const { return tabName_p; }

  // Get the table create option.
  int option() const { return option_p; }
  ...
  const std::shared_ptr<ColumnSet>& columnSetPtr() const { return colSetPtr_p; }

  // Get pointer to table description.
  // This function is used by PlainTable.
  const std::shared_ptr<TableDesc>& tableDescPtr() const { return tdescPtr_p; }

  // Set object to in use by a (Plain)Table object.
  // This function is used by PlainTable.
  void setInUse() { colSetPtr_p.reset(); }
...
class SetupNewTable {
 ...
 public:
  ...
  // Get the name of the table.
  const String& name() const { return newTable_p->name(); }

  // Get the table create option.
  int option() const { return newTable_p->option(); }
  ...
  // Create data managers and bind the columns using the specifications
  // in the given record (which is obtained using Table::dataManagerInfo()).
  void bindCreate(const Record& spec) { newTable_p->bindCreate(spec); }
 ...
 private:
  ...
  // Get pointer to column set.
  // This function is used by PlainTable.
  const std::shared_ptr<ColumnSet>& columnSetPtr() const { return newTable_p->columnSetPtr(); }

  // Get pointer to table description.
  // This function is used by PlainTable.
  const std::shared_ptr<TableDesc>& tableDescPtr() const { return newTable_p->tableDescPtr(); }

  // Set object to in use by a (Plain)Table object.
  // This function is used by PlainTable.
  void setInUse() { newTable_p->setInUse(); }

  // Make a data manager for all unbound columns.
  void handleUnbound() { newTable_p->handleUnbound(); }
};
