...
#define TVECMATHOPER(NAME, OP, OPA)                                                               \
  template <class T>                                                                              \
  TabVecRep<T>& aips_name2(tabVecRepvalr, NAME)(const TabVecRep<T>& tv, const T& val) {           \
  ...
  template <class T>                                                                              \
  TabVecRep<T>& aips_name2(tabVecRepvall, NAME)(const T& val, const TabVecRep<T>& tv) {           \
    rownr_t nr = tv.nelements();                                                                  \
    TabVecTemp<T>& vec = *(TabVecTemp<T>*)tv.newVec();                                            \
    T tmp;                                                                                        \
    for (rownr_t i = 0; i < nr; i++) {                                                            \
  ...
  template <class T>                                                                              \
  TabVecRep<T>& aips_name2(tabVecReptv, NAME)(const TabVecRep<T>& tvl, const TabVecRep<T>& tvr) { \
  ...
  template <class T>                                                                              \
  void aips_name2(tabVecReptvass, NAME)(TabVecRep<T> & tvl, const TabVecRep<T>& tvr) {            \
    rownr_t nr = tvr.nelements();                                                                 \
    tvl.validateConformance(nr);                                                                  \
    T tmp;                                                                                        \
    for (rownr_t i = 0; i < nr; i++) {                                                            \
...
void tabVecRepminmax(T& min, T& max, const TabVecRep<T>& tv) {
  ...
  if (nr == 0) {
    throw(
        ArrayError("void minMax(T& min, T& max, const TabVecRep<T>&) - "
...
#define TABVECFUNC2(NAME)                                                                       \
  template <class T>                                                                            \
  TabVecRep<T>& aips_name2(tabVecRep, NAME)(const TabVecRep<T>& tvl, const TabVecRep<T>& tvr) { \
    rownr_t nr = tvr.nelements();                                                               \
    tvl.validateConformance(nr);                                                                \
    TabVecTemp<T>& vec = *(TabVecTemp<T>*)tvl.newVec();                                         \
    T tmpl, tmpr;                                                                               \
...
T tabVecRepsum(const TabVecRep<T>& tv) {
  ...
  if (nr == 0) {
    throw(
        ArrayError("T sum(const TabVecRep<T>&) - "
                   "TabVecRep has no elements"));
  }
  T tmp, res;
  tv.getVal(0, res);
...
T tabVecRepproduct(const TabVecRep<T>& tv) {
  ...
  if (nr == 0) {
    throw(
        ArrayError("T product(const TabVecRep<T>&) - "
                   "TabVecRep has no elements"));
  }
  T tmp, res;
  tv.getVal(0, res);
...
template <class T>
TabVecRep<T>& tabVecRepcrossproduct(const TabVecRep<T>& tvl, const TabVecRep<T>& tvr) {
