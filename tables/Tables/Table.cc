...
Table::Table(SetupNewTable& newtab, Table::TableType type, rownr_t nrrow, Bool initialize,
  ...
  if (type == Table::Memory) {
    ...
    ptr = new PlainTable(newtab, nrrow, initialize, TableLock(), endianFormat, tsmOpt);
  }
  initBasePtr(ptr);
}
Table::Table(SetupNewTable& newtab, Table::TableType type, const TableLock& lockOptions,
  ...
  if (type == Table::Memory) {
    ...
    ptr = new PlainTable(newtab, nrrow, initialize, lockOptions, endianFormat, tsmOpt);
  }
  initBasePtr(ptr);
}
...
Table::Table(MPI_Comm mpiComm, SetupNewTable& newtab, rownr_t nrrow, Bool initialize,
    ...
    : baseTabPtr_p(0), lastModCounter_p(0) {
  initBasePtr(
      new PlainTable(mpiComm, newtab, nrrow, initialize, TableLock(), endianFormat, tsmOpt));
}
...
Table::Table(MPI_Comm mpiComm, SetupNewTable& newtab, Table::TableType type, rownr_t nrrow,
  ...
  if (type == Table::Memory) {
    ...
    ptr = new PlainTable(mpiComm, newtab, nrrow, initialize, TableLock(), endianFormat, tsmOpt);
  }
  initBasePtr(ptr);
}
...
Table::Table(MPI_Comm mpiComm, SetupNewTable& newtab, Table::TableType type,
  ...
  if (type == Table::Memory) {
    ...
    ptr = new PlainTable(mpiComm, newtab, nrrow, initialize, lockOptions, endianFormat, tsmOpt);
  }
  initBasePtr(ptr);
}
...
void Table::closeSubTables() const { return keywordSet().closeTables(); }
...
std::shared_ptr<BaseTable> Table::makeBaseTable(const String& name, const String& type,
  ...
  ios >> tp;
  if (tp == "PlainTable") {
    baseTabPtr = std::make_shared<PlainTable>(ios, version, name, type, nrrow, tableOption,
                                              lockOptions, tsmOpt, addToCache, locknr);
  } else if (tp == "RefTable") {
    baseTabPtr = std::make_shared<RefTable>(ios, name, nrrow, tableOption, lockOptions, tsmOpt);
  } else if (tp == "ConcatTable") {
    baseTabPtr = std::make_shared<ConcatTable>(ios, name, nrrow, tableOption, lockOptions, tsmOpt);
  } else {
...
BaseTable* Table::lookCache(const String& name, int tableOption, const TableLock& lockOptions) {
  return PlainTable::tableCache().lookCache(name, tableOption, lockOptions);
}
...
TableRecord& Table::rwKeywordSet() {
  if (!isWritable()) {
    throw(TableError("Table::rwKeywordSet cannot be used: table " + tableName() +
                     " is not writable"));
  }
...
TableExprNode Table::col(const String& columnName) const {
  ...
  return TableExprNode::newColumnNode(TableExprInfo(*this), columnName, fieldNames);
}
TableExprNode Table::col(const String& columnName, const Vector<String>& fieldNames) const {
  return TableExprNode::newColumnNode(TableExprInfo(*this), columnName, fieldNames);
}
...
Table Table::operator()(const TableExprNode& expr, rownr_t maxRow, rownr_t offset) const {
...
TableDesc Table::actualTableDesc() const { return baseTabPtr_p->actualTableDesc(); }

Record Table::dataManagerInfo() const { return baseTabPtr_p->dataManagerInfo(); }

// # Make the table file name.
String Table::fileName(const String& tableName) { return tableName + "/table.dat"; }
...
Table Table::openTable(const String& tableName, TableOption tabOpt, const TSMOption& tsmOpt) {
  return TableUtil::openTable(tableName, tabOpt, tsmOpt);
}
Table Table::openTable(const String& tableName, const TableLock& lockOptions, TableOption tabOpt,
                       const TSMOption& tsmOpt) {
...
TableInfo Table::tableInfo(const String& tableName) { return TableUtil::tableInfo(tableName); }
