...
Vector<String> TableCache::getLockedTables(FileLocker::LockType lockType, int lockOption) {
  std::lock_guard<std::mutex> sc(itsMutex);
  vector<String> names;
  for (const auto& x : tableMap_p) {
    PlainTable& table = *static_cast<PlainTable*>(x.second);
...
PlainTable* TableCache::lookCache(const String& name, int tableOption,
  ...
  if ((tableOption == cachedTableOption) ||
      ((cachedTableOption == Table::New || cachedTableOption == Table::NewNoReplace ||
        cachedTableOption == Table::Update) &&
       (tableOption == Table::Update || tableOption == Table::Old))) {
  ...
  throw(TableInvOper("Table " + name + " cannot be opened/created (already in cache)"));
}
