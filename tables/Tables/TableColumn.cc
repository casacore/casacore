...
TableColumn* TableColumn::clone() const { return new TableColumn(*this); }
...
const ColumnDesc& TableColumn::columnDesc() const { return baseColPtr_p->columnDesc(); }
...
void TableColumn::put(rownr_t thisRownr, const TableColumn& that, rownr_t thatRownr,
  ...
  if (columnDesc().isScalar()) {
    ...
    if (that.isDefined(thatRownr)) {
      ...
      } else if ((columnDesc().options() & ColumnDesc::FixedShape) != ColumnDesc::FixedShape) {
        baseColPtr_p->setShape(thisRownr, shape);
      }
      ValueHolder vh;
...
void TableColumn::throwNotWritable() const {
  throw TableError("Column " + columnDesc().name() + " in table " + baseTabPtr_p->tableName() +
                   " is not writable");
}
