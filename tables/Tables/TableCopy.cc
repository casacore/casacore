  ...
  for (uInt i = 0; i < outDesc.ncolumn(); i++) {
    ...
    if (out.isColumnWritable(i)) {
      ...
      if (inDesc.isColumn(name)) {
        ...
        copySubTables(outCol.rwKeywordSet(), inCol.keywordSet(), out.tableName(), out.tableType(),
                      in, noRows, omit);
      }
    }
  }
...
void TableCopy::cloneColumn(const Table& fromTable, const String& fromColumn, Table& toTable,
...
void TableCopy::doCloneColumn(const Table& fromTable, const String& fromColumn, Table& toTable,
