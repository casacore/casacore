...
template <typename T>
void TableCopy::cloneColumnTyped(const Table& fromTable, const String& fromColumn, Table& toTable,
                                 const String& newColumn, const String& dataManagerName,
                                 const Record& newdmInfo) {
  // Get existing column description.
  ColumnDesc cd(fromTable.tableDesc()[fromColumn]);
  if (cd.isScalar()) {
    ScalarColumnDesc<T> scd(newColumn, cd.comment(), cd.dataManagerType(), cd.dataManagerGroup(),
                            T(), cd.options());
    cd = ColumnDesc(scd);
  } else {
    ArrayColumnDesc<T> acd(newColumn, cd.comment(), cd.dataManagerType(), cd.dataManagerGroup(),
...
template <typename T>
void TableCopy::fillArrayColumn(Table& table, const String& column, const Array<T>& value) {
