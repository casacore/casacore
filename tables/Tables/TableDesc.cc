...
TableDesc::TableDesc() : option_p(Scratch) {
  init(TabPath());  // use default search path
}

TableDesc::TableDesc(const String& nam, TDOption opt) : name_p(nam), option_p(opt) {
  init(TabPath());  // use default search path
}

TableDesc::TableDesc(const String& nam, const String& version, TDOption opt)
    : name_p(nam), vers_p(version), option_p(opt) {
  init(TabPath());  // use default search path
}

TableDesc::TableDesc(const String& nam, const String& version, const TabPath& tdpath, TDOption opt)
    : name_p(nam), vers_p(version), option_p(opt) {
...
TableDesc::~TableDesc() {
  ...
  if (swwrite_p) {
    if (option_p == Update || option_p == New || option_p == NewNoReplace) {
...
void TableDesc::init(const TabPath& tdpath) {
  ...
  switch (option_p) {
    ...
    case TableDesc::Delete:
      ...
      break;
    default:
      throw(TableInvOpt("TableDesc", "must be Old, New, NewNoReplace, Scratch, Update or Delete"));
  }
...
// # Show the table.
void TableDesc::show() const { show(cout); }
...
// # Rename a column.
void TableDesc::renameColumn(const String& newname, const String& oldname) {
...
void TableDesc::defineHypercolumn(const String& hypercolumnName, uInt ndim,
  ...
  for (i = 0; i < ncoord; i++) {
    if (!coordColumnNames(i).empty()) {
      if (!isColumn(coordColumnNames(i))) {
        throwHypercolumn(hypercolumnName, "coordColumn " + coordColumnNames(i) + " does not exist");
      }
      const ColumnDesc& desc = columnDesc(coordColumnNames(i));
      int dataType = desc.dataType();
      if (dataType == TpString || dataType == TpArrayString || dataType == TpBool ||
          dataType == TpArrayBool || dataType == TpTable || dataType == TpRecord ||
          dataType == TpOther) {
        throwHypercolumn(hypercolumnName, "coordColumn " + coordColumnNames(i) + " is not numeric");
      }
      if (dataType == TpChar || dataType == TpArrayChar || dataType == TpUChar ||
          dataType == TpArrayUChar || dataType == TpShort || dataType == TpArrayShort ||
          dataType == TpUShort || dataType == TpArrayUShort || dataType == TpInt64 ||
          dataType == TpArrayInt64) {
        throwHypercolumn(hypercolumnName, "coordColumn " + coordColumnNames(i) +
                                              ": (u)Char, (u)Short and Int64 not supported");
      }
      if (desc.isArray()) {
        if (desc.ndim() != 1) {
          throwHypercolumn(hypercolumnName,
                           "coordColumn " + coordColumnNames(i) + " is not a scalar or vector");
        }
        lastCoordVec = i + 1;
      } else {
        if (firstCoordSca == 0) {
  ...
  if (firstCoordSca > 0 && lastCoordVec > firstCoordSca) {
    throwHypercolumn(hypercolumnName, "coordinate vectors have to describe the first axes");
  }
  ...
  for (i = 0; i < dataColumnNames.nelements(); i++) {
    ...
    if (desc.dataType() == TpString || desc.dataType() == TpArrayString ||
        desc.dataType() == TpTable || desc.dataType() == TpOther) {
      throwHypercolumn(hypercolumnName,
                       "dataColumn " + dataColumnNames(i) + " is not numeric or boolean");
    }
    if (desc.isArray() && desc.ndim() <= 0) {
      throwHypercolumn(hypercolumnName,
                       "the dimensionality of data column " + dataColumnNames(i) + " is undefined");
    }
    if (cellNdim == 0) {
      cellNdim = desc.ndim();
    }
  ...
  for (i = 0; i < idColumnNames.nelements(); i++) {
    if (!isColumn(idColumnNames(i))) {
      throwHypercolumn(hypercolumnName, "idColumn " + idColumnNames(i) + " does not exist");
    }
    const ColumnDesc& desc = columnDesc(idColumnNames(i));
    if (!desc.isScalar()) {
      throwHypercolumn(hypercolumnName, "idColumn " + idColumnNames(i) + " is not a scalar");
    }
    int dataType = desc.dataType();
    if (dataType == TpOther) {
      throwHypercolumn(hypercolumnName,
                       "idColumn " + idColumnNames(i) + " has no standard data type");
    }
    if (dataType == TpChar || dataType == TpUChar || dataType == TpShort || dataType == TpUShort ||
        dataType == TpInt64) {
  ...
  for (i = 0; i < nr; i++) {
    if (!names(i).empty()) {
      for (j = i + 1; j < nr; j++) {
        if (names(i) == names(j)) {
          throwHypercolumn(hypercolumnName, "column name " + names(i) + " is multiply used");
        }
      }
    }
  }
  ...
  if (nhyp > 0) {
    ...
    for (i = 0; i < nkey; i++) {
      if (privKey_p->type(i) == TpRecord) {
        const String& key = privKey_p->description().name(i);
        if (key.starts_with(theHyperPrefix)) {
          result(nhyp) = String(key).substr(theHyperPrefix.length());
          nhyp++;
        }
      }
    }
...
void TableDesc::removeHypercolumnDesc(const String& hypercolumnName) {
  if (!isHypercolumn(hypercolumnName)) {
    throw TableError("Hypercolumn " + hypercolumnName + " does not exist, thus cannot be removed");
  }
  // Remove all hypercolumns; start at the end to avoid invalid indices.
  privKey_p->removeField(theHyperPrefix + hypercolumnName);
}

void TableDesc::renameHypercolumn(const String& newHypercolumnName, const String& hypercolumnName) {
  if (!isHypercolumn(hypercolumnName)) {
    throw TableError("Hypercolumn " + hypercolumnName + " does not exist, thus cannot be renamed");
  }
