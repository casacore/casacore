...
TableError::TableError(Category c) : AipsError("Table error", c) {}
TableError::TableError(const String& str, Category c) : AipsError(str, c) {}
...
TableNoDatFile::TableNoDatFile(const String& filename, Category c)
    : TableError(filename.empty() ? String("No table name given at open")
                                  : "Table file " + filename + " does not exist",
                 c) {}
TableNoDatFile::~TableNoDatFile() noexcept {}

TableDescNoName::TableDescNoName(Category c) : TableError("No name for table description", c) {}
...
TableInvType::TableInvType(const String& tableName, const String& tpin, const String& tpfil,
                           Category c)
    : TableError(
          "Table file " + tableName + " is incorrect: Expected type " + tpin + ", found " + tpfil,
          c) {}
...
TableInvDT::TableInvDT(Category c) : TableError("Invalid Table data type", c) {}
...
TableInvOper::TableInvOper(Category c) : TableError("Invalid Table operation", c) {}
...
TableArrayConformanceError::TableArrayConformanceError(const String& s, const IPosition& shape,
                                                       const IPosition& exp, Category c)
    : TableError(std::string(s) + ": Table array conformance error (shape=" + shape.toString() +
...
TableInvSort::TableInvSort(Category c) : TableError("Invalid table sort", c) {}
...
TableParseError::TableParseError(const String& s, int pos, const String& token, Category c)
    : TableError("Error in TaQL command: " + s, c), itsPos(pos), itsToken(token) {}
TableParseError::~TableParseError() noexcept {}

TableGramError::TableGramError(int pos, const String& token, Category c)
    : TableError("parse error at or near position " + std::to_string(pos) + " '" +
                     std::string(token) + "'",
                 c),
      itsPos(pos),
      itsToken(token) {}
