...
void TableIterProxy::makeStepIter(const Table& tab, const Block<String>& columns,
  ...
  for (uInt i = 0; i < iterSteps.size(); ++i) {
    if (i < columns.size() && iterSteps[i] > 0) {
      ...
      if (!colDesc.isScalar()) {
        throw TableError(
            "Only scalar columns can be used in table "
            "iteration");
      }
      ...
      switch (dtype) {
        ...
        case TpString:
          ...
          break;
        default:
          throw TableError("No iteration step can be given for column " + columns[i]);
      }
    }
  }
  ...
  iter_p = TableIterator(sortab, columns, comps, orders, TableIterator::NoSort);
}

TableIterProxy::TableIterProxy(const TableIterProxy& that)
    : iter_p(that.iter_p), firstTime_p(that.firstTime_p) {}
