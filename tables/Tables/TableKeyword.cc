...
TableKeyword::TableKeyword(const String& tableDescName)
    : table_p(new Table), tableDescName_p(tableDescName) {}

TableKeyword::TableKeyword(const Table& table, const String& tableDescName)
    : table_p(new Table), attr_p(table), tableDescName_p(tableDescName) {
...
TableKeyword::~TableKeyword() { delete table_p; }
...
void TableKeyword::renameTable(const String& newParentName, const String& oldParentName) {
...
Table TableKeyword::table(const TableLock* lockOptions) const {
  ...
  // in an MS).
  return Table(attr_p.name(), lockOptions ? *lockOptions : attr_p.lockOptions(), option);
}

void TableKeyword::close() const { *table_p = Table(); }
