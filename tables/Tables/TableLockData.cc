...
void TableLockData::makeLock(const String& name, Bool create, FileLocker::LockType type,
  ...
  if (isPermanent()) {
    ...
    if (!itsLock->acquire(type, nattempts)) {
      throw(TableError("Permanent lock on table " + name + " could not be acquired (" +
                       itsLock->lastMessage() + ")"));
    }
  }
}
...
Bool TableLockData::acquire(MemoryIO* info, FileLocker::LockType type, uInt nattempts) {
  ...
  if (!status) {
    if (nattempts == 0) {
      throw(TableError("Error (" + itsLock->lastMessage() + ") when acquiring lock on " +
                       itsLock->name()));
    }
  }
  return status;
...
void TableLockData::release(Bool always) {
  ...
  if (always || !isPermanent()) {
    ...
    if (!itsLock->release(memIO)) {
      throw(TableError("Error (" + itsLock->lastMessage() + ") when releasing lock on " +
                       itsLock->name()));
    }
  }
}
