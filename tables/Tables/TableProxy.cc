...
TableProxy::TableProxy(const String& tableName, const Record& lockOptions, int option) {
  table_p =
      TableUtil::openTable(tableName, makeLockOptions(lockOptions), Table::TableOption(option));
}
...
TableProxy::TableProxy(const Vector<String>& tableNames,
                       const Vector<String>& concatenateSubTableNames, const Record& lockOptions,
                       int option) {
  ...
  std::copy(concatenateSubTableNames.begin(), concatenateSubTableNames.end(), subNames.begin());
...
TableProxy::TableProxy(const String& fileName, const String& headerName, const String& tableName,
  ...
  if (headerName.empty()) {
    if (columnNames.size() == 0 && dataTypes.size() == 0) {
      asciiFormat_p = readAsciiTable(fileName, "", tableName, autoHeader, sep, commentMarker,
                                     firstLine, lastLine, IPosition(autoShape));
    } else {
      asciiFormat_p = readAsciiTable(fileName, "", tableName, columnNames, dataTypes, sep,
                                     commentMarker, firstLine, lastLine);
    }
  } else {
    asciiFormat_p = readAsciiTable(headerName, fileName, "", tableName, sep, commentMarker,
                                   firstLine, lastLine);
  }
  // Open the table.
...
TableProxy::TableProxy(const TableProxy& that) : table_p(that.table_p) {}
...
String TableProxy::toAscii(const String& asciiFile, const String& headerFile,
  ...
  if (!ofs) {
    throw TableError("TableProxy::toAscii - error opening file '" + asciiFile + "'");
  }
  ...
  if (!headerFile.empty() && (headerFile != asciiFile)) {
    ...
    if (!ofs2) {
      throw TableError("TableProxy::toAscii - error opening file '" + headerFile + "'");
    }
  ...
  switch (vh.dataType()) {
    ...
    case TpArrayDComplex: {
      ...
      if (useBrackets) {
        ...
        for (Array<DComplex>::const_iterator iter = arr.begin(); iter != iterend; ++iter) {
          if (iter != arr.begin()) {
            os << sep;
          }
    ...
    case TpArrayString: {
      ...
      if (useBrackets) {
        ...
        for (Array<String>::const_iterator iter = arr.begin(); iter != iterend; ++iter) {
          if (iter != arr.begin()) {
            os << sep;
          }
          os << '"' << *iter << '"';
...
template <typename T>
void TableProxy::printArray(const Array<T>& arr, ostream& os, const String& sep) const {
  if (arr.empty()) {
    cout << "[]";
  } else {
...
TableProxy TableProxy::copy(const String& newTableName, Bool toMemory, Bool deepCopy,
  ...
  if (toMemory) {
    ...
    if (deepCopy || valueCopy) {
      table_p.deepCopy(newTableName, dminfo, Table::New, valueCopy, endOpt, noRows);
    } else {
...
Record TableProxy::getTableDesc(const TableDesc& tabdesc, Bool cOrder) {
  ...
  // Convert keywords
  rec.defineRecord("_keywords_", tabdesc.keywordSet().toRecord());
  // Convert private keywords
  rec.defineRecord("_private_keywords_", tabdesc.privateKeywordSet().toRecord());
...
String TableProxy::getAsciiFormat() const { return asciiFormat_p; }
...
void TableProxy::putKeywordSet(const String& columnName, const Record& valueSet) {
  TableRecord* keySet;
  if (columnName.empty()) {
    keySet = &(table_p.rwKeywordSet());
  } else {
...
void TableProxy::reopenRW() { table_p.reopenRW(); }

void TableProxy::resync() { table_p.resync(); }
...
void TableProxy::putTableInfo(const Record& value) {
  ...
  for (uInt i = 0; i < value.nfields(); i++) {
    ...
    if (value.name(i) == "type") {
      ...
      throw TableError("Name of field " + value.name(i) + " must be type, subType or readme");
    }
  }
...
void TableProxy::renameColumn(const String& nameOld, const String& nameNew) {
  ...
  for (uInt nrdone = 0, nrcols = 0; nrdone < gdesc.nfields(); ++nrdone) {
    ...
    if (isArray) {
      ...
      if (valtype == "boolean" || valtype == "bool") {
        ...
        tabdesc.addColumn(ScalarColumnDesc<DComplex>(name, comment, dmtype, dmgrp, option));
      } else if (valtype == "string") {
        tabdesc.addColumn(ScalarColumnDesc<String>(name, comment, dmtype, dmgrp, option));
...
Bool TableProxy::addArrayColumnDesc(TableDesc& tabdesc, const String& valtype, const String& name,
  ...
  if (valtype == "boolean" || valtype == "bool") {
    ...
    if (shp.nelements() > 0) {
      tabdesc.addColumn(ArrayColumnDesc<float>(name, comment, dmtype, dmgrp, shp, option));
    } else {
      tabdesc.addColumn(ArrayColumnDesc<float>(name, comment, dmtype, dmgrp, ndim, option));
    }
  } else if (valtype == "double") {
    if (shp.nelements() > 0) {
      tabdesc.addColumn(ArrayColumnDesc<double>(name, comment, dmtype, dmgrp, shp, option));
    } else {
      tabdesc.addColumn(ArrayColumnDesc<double>(name, comment, dmtype, dmgrp, ndim, option));
    }
  } else if (valtype == "complex") {
    if (shp.nelements() > 0) {
      tabdesc.addColumn(ArrayColumnDesc<Complex>(name, comment, dmtype, dmgrp, shp, option));
    } else {
      tabdesc.addColumn(ArrayColumnDesc<Complex>(name, comment, dmtype, dmgrp, ndim, option));
    }
  } else if (valtype == "dcomplex") {
    if (shp.nelements() > 0) {
      tabdesc.addColumn(ArrayColumnDesc<DComplex>(name, comment, dmtype, dmgrp, shp, option));
    } else {
      tabdesc.addColumn(ArrayColumnDesc<DComplex>(name, comment, dmtype, dmgrp, ndim, option));
    }
  } else if (valtype == "string") {
    if (shp.nelements() > 0) {
      tabdesc.addColumn(ArrayColumnDesc<String>(name, comment, dmtype, dmgrp, shp, option));
    } else {
      tabdesc.addColumn(ArrayColumnDesc<String>(name, comment, dmtype, dmgrp, ndim, option));
    }
  } else {
    ...
    switch (dtype) {
      ...
      case TpRecord: {
        ...
        if (isCell) {
          ...
          throw TableError(
              "TableProxy::getColumn not possible for a column"
              " containing records");
        }
      } break;
      default:
...
void TableProxy::getValueFromTable(const String& colName, Int64 rownr, Int64 nrow, Int64 incr,
  ...
  switch (vh.dataType()) {
    ...
    case TpArrayComplex: {
      ...
      if (dtype != TpComplex) {
        // Data has to be converted, so cannot be read directly into the array.
        arr = getValueFromTable(colName, rownr, nrow, incr, isCell).asArrayComplex();
      } else if (isScalar) {
        ScalarColumn<Complex> ac(table_p, colName);
    ...
    case TpArrayDComplex: {
      ...
      if (dtype != TpDComplex) {
        // Data has to be converted, so cannot be read directly into the array.
        arr = getValueFromTable(colName, rownr, nrow, incr, isCell).asArrayDComplex();
      } else if (isScalar) {
        ScalarColumn<DComplex> ac(table_p, colName);
...
ValueHolder TableProxy::getValueSliceFromTable(const String& colName, const Slicer& slicer,
  ...
  const ColumnDesc& cdesc = table_p.tableDesc().columnDesc(colName);
  if (!cdesc.isArray()) {
    throw TableError("TableProxy::getColumnSlice: column " + String(colName) +
                     " is not an array column");
  }
  // Exit immediately if no rows have to be done.
  if (nrow == 0) {
  ...
  switch (cdesc.dataType()) {
    case TpBool: {
      ...
      if (isCell) {
        ...
        return ValueHolder(ac.getColumnRange(Slice(rownr, nrow, incr), slicer));
      }
    } break;
    case TpUChar: {
      ArrayColumn<uChar> ac(table_p, colName);
...
void TableProxy::getValueSliceFromTable(const String& colName, const Slicer& slicer, Int64 rownr,
  ...
  switch (vh.dataType()) {
    ...
    case TpArrayComplex: {
      ...
      if (dtype != TpComplex) {
        // Data has to be converted; cannot be read directly into the array.
        arr = getValueSliceFromTable(colName, slicer, rownr, nrow, incr, isCell).asArrayComplex();
      } else {
        ArrayColumn<Complex> ac(table_p, colName);
        if (isCell) {
          ac.getSlice(rownr, slicer, arr);
    ...
    case TpArrayDComplex: {
      ...
      if (dtype != TpDComplex) {
        // Data has to be converted; cannot be read directly into the array.
        arr = getValueSliceFromTable(colName, slicer, rownr, nrow, incr, isCell).asArrayDComplex();
      } else {
        ArrayColumn<DComplex> ac(table_p, colName);
        if (isCell) {
          ac.getSlice(rownr, slicer, arr);
    ...
    switch (type) {
      case TpBool: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayBool());
        }
      } break;
      case TpUChar: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayuChar());
        }
      } break;
      case TpShort: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayShort());
        }
      } break;
      case TpUShort: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayuShort());
        }
      } break;
      case TpInt: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayInt());
        }
      } break;
      case TpUInt: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayuInt());
        }
      } break;
      case TpInt64: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayInt64());
        }
      } break;
      case TpFloat: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayFloat());
        }
      } break;
      case TpDouble: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayDouble());
        }
      } break;
      case TpComplex: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayComplex());
        }
      } break;
      case TpDComplex: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayDComplex());
        }
      } break;
      case TpString: {
        ...
        if (isCell) {
          ...
          col.putColumnRange(Slice(rownr, nrow, incr), value.asArrayString());
        }
      } break;
      case TpRecord: {
        ...
        if (isCell) {
          ...
          throw TableError(
              "TableProxy::putColumn not possible for a column"
              " containing records");
        }
      } break;
      default:
  ...
  switch (table_p.tableDesc().columnDesc(colName).dataType()) {
    case TpBool: {
      ...
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayBool());
      }
    } break;
    case TpUChar: {
      ArrayColumn<uChar> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayuChar());
      }
    } break;
    case TpShort: {
      ArrayColumn<Short> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayShort());
      }
    } break;
    case TpUShort: {
      ArrayColumn<uShort> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayuShort());
      }
    } break;
    case TpInt: {
      ArrayColumn<Int> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayInt());
      }
    } break;
    case TpUInt: {
      ArrayColumn<uInt> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayuInt());
      }
    } break;
    case TpInt64: {
      ArrayColumn<Int64> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayInt64());
      }
    } break;
    case TpFloat: {
      ArrayColumn<Float> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayFloat());
      }
    } break;
    case TpDouble: {
      ArrayColumn<Double> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayDouble());
      }
    } break;
    case TpComplex: {
      ArrayColumn<Complex> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayComplex());
      }
    } break;
    case TpDComplex: {
      ArrayColumn<DComplex> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayDComplex());
      }
    } break;
    case TpString: {
      ArrayColumn<String> col(table_p, colName);
      if (isCell) {
        ...
        col.putColumnRange(Slice(rownr, nrow, incr), slicer, value.asArrayString());
      }
    } break;
    default:
...
void TableProxy::findKeyId(RecordFieldId& fieldid, TableRecord*& keySet, const String& keyname,
  ...
  for (uInt i = 0; i < keys.nelements(); i++) {
    ...
    if (!keySet->isDefined(keys(i))) {
      if (mustExist) {
        if (column.empty()) {
          ...
          throw TableError("Keyword " + usedName + " in column " + column + " does not exist");
        }
      } else {
    ...
    if (i + 1 < keys.nelements()) {
      if (keySet->dataType(fieldid) != TpRecord) {
        if (column.empty()) {
          throw TableError("Table keyword " + usedName + " does not have a record-value");
        } else {
...
TableLock TableProxy::makeLockOptions(const Record& options) {
  ...
  if (str == "default") {
    ...
    throw TableError("'" + str +
                     "' is an unknown lock option; valid are "
...
Table::EndianFormat TableProxy::makeEndianFormat(const String& endianFormat) {
  ...
  if (!endianFormat.empty()) {
    ...
    if (str == "aipsrc") {
      ...
      throw TableError("endian format '" + endianFormat +
                       "' is incorrect; "
