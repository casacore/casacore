  ...
  if (trecp != 0) {
    ...
    for (uInt i = 0; i < n; i++) {
      ...
      if (dtype == TpRecord) {
        ...
        defineRecord(i, TableRecord(subrec), subrec.isFixed() ? Fixed : Variable);
      } else if (dtype == TpTable) {
        const TableKeyword& tabkey = *(const TableKeyword*)(other.get_pointer(i, dtype));
        defineTable(i, tabkey.table(), tabkey.isFixed() ? Fixed : Variable);
      } else {
...
RecordInterface* TableRecord::clone() const { return new TableRecord(*this); }
...
void TableRecord::makeUnique() { rwRef(); }
...
void TableRecord::defineRecord(const RecordFieldId& id, const RecordInterface& value,
                               RecordType type) {
...
void TableRecord::defineFromValueHolder(const RecordFieldId& fieldId, const ValueHolder& value) {
