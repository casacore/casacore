...
void TableRecordRep::defineDataField(Int whichField, DataType type, const void* value) {
  ...
  if (type == descDtype) {
    if (type == TpRecord) {
      *static_cast<TableRecord*>(data_p[whichField]) = *static_cast<const TableRecord*>(value);
    } else if (type == TpTable) {
      *static_cast<TableKeyword*>(data_p[whichField]) = *static_cast<const Table*>(value);
    } else {
    ...
    throw(
        AipsError("TableRecordRep::defineDataField - "
...
Bool TableRecordRep::conform(const TableRecordRep& other) const {
  ...
  for (Int i = 0; i < Int(nused_p); i++) {
    if (desc_p.type(i) == TpRecord) {
      ...
      const TableKeyword& thisKey = *static_cast<TableKeyword*>(const_cast<void*>(data_p[i]));
      if (thisKey.isFixed()) {
...
void* TableRecordRep::get_pointer(Int whichField, DataType type) const {
  ...
  if (!(isArray(type) && asScalar(type) == descDtype)) {
    throw(
        AipsError("TableRecordRep::get_pointer - "
                  "incorrect data type used for field " +
                  desc_p.name(whichField)));
  }
...
void TableRecordRep::print(std::ostream& os, Int maxNrValues, const String& indent) const {
  for (uInt i = 0; i < nused_p; i++) {
    ...
    if (desc_p.type(i) == TpRecord) {
      os << '{' << endl;
      static_cast<const TableRecord*>(data_p[i])->print(os, maxNrValues, indent + "  ");
      os << indent << '}' << endl;
    } else if (desc_p.type(i) == TpTable) {
      os << "Table " << static_cast<const TableKeyword*>(data_p[i])->tableName() << endl;
    } else {
...
void TableRecordRep::putData(AipsIO& os, const TableAttr& parentAttr) const {
  for (uInt i = 0; i < nused_p; i++) {
    if (desc_p.type(i) == TpRecord) {
      ...
      if (desc.nfields() == 0) {
        static_cast<TableRecord*>(const_cast<void*>(data_p[i]))->putRecord(os, parentAttr);
      } else {
...
void TableRecordRep::getData(AipsIO& os, uInt version, const TableAttr& parentAttr) {
  for (uInt i = 0; i < nused_p; i++) {
    ...
    if (type == TpRecord) {
      const RecordDesc& desc = desc_p.subRecord(i);
      if (desc.nfields() == 0) {
        static_cast<TableRecord*>(data_p[i])->getRecord(os, parentAttr);
      } else {
        static_cast<TableRecord*>(data_p[i])->getData(os, version, parentAttr);
      }
    } else if (type == TpTable) {
      String name;
      os >> name;
...
void TableRecordRep::getTableKeySet(AipsIO& os, uInt version, const TableAttr& parentAttr,
  ...
  if (type > 1) {
    ...
    for (i = 0; i < n; i++) {
      os >> key;   // keyword name
      os >> name;  // table name
      static_cast<TableKeyword*>(data_p[desc_p.fieldNumber(key)])->set(name, parentAttr);
    }
  }
