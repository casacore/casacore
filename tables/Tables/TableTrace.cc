...
int TableTrace::traceColumn(const ColumnDesc& cd) {
  ...
  if (theirOper > 0) {
    ...
    if ((cd.isScalar() && (theirColType & SCALAR) != 0) ||
        (cd.isArray() && (theirColType & ARRAY) != 0) || ((theirColType & RECORD) != 0)) {
      traceCol = theirOper;
    } else {
...
void TableTrace::writeSlice(const IPosition& blc, const IPosition& trc, const IPosition& inc) {
  *theirStream << ' ';
...
void TableTrace::trace(int tabid, const String& columnName, char oper, const RefRows& rownrs) {
  writeTraceFirst(tabid, columnName, oper);
  writeRefRows(rownrs);
  *theirStream << endl;
}

void TableTrace::trace(int tabid, const String& columnName, char oper, const IPosition& shape) {
  writeTraceFirst(tabid, columnName, oper);
  *theirStream << "* ";
...
void TableTrace::initTracing() {
  ...
  if (!fname.empty()) {
    ...
    *theirStream << "# time oper tabid name row(s) shape blc/trc/inc" << endl;
