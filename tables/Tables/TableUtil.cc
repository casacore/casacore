...
Table openTable(const String& tableName, Table::TableOption option, const TSMOption& tsmOption) {
  return openTable(tableName, TableLock(), option, tsmOption);
}

Table openTable(const String& tableName, const TableLock& lockOpt, Table::TableOption tabOpt,
  ...
  if (!t.first.keywordSet().isDefined(t.second)) {
    throw TableError("Table name " + tableName + " is invalid (" + "subtable " + t.second +
                     " is unknown)");
  }
...
Table createTable(const String& tableName, const TableDesc& desc, Table::TableOption tabOpt,
  ...
  if (!t.second.empty()) {
    return createSubTable(t.first, t.second, desc, tabOpt, storageOption, dmInfo, lockOptions,
                          nrrow, initialize, endian, tsmOpt);
  } else {
    // No subtable given, so create a main table.
    SetupNewTable newtab(tableName, desc, tableName.empty() ? Table::Scratch : tabOpt,
                         storageOption);
...
Table createSubTable(Table& parent, const String& subName, const TableDesc& desc,
  ...
  if (inx >= 0) {
    ...
    if (tabOpt == Table::NewNoReplace) {
      throw TableError("Subtable " + parent.tableName() + '/' + subName + " already exists");
    }
    // Remove the subtable.
...
void deleteTable(const String& tableName, Bool checkSubTables) {
  ...
  if (!canDeleteTable(message, tabName, checkSubTables)) {
    throw(TableError("Table " + tabName + " cannot be deleted: " + message));
  }
...
rownr_t getLayout(TableDesc& desc, const String& tableName) {
  ...
  if (tp == "PlainTable") {
    ...
    throw(TableInternalError("TableUtil::getLayout: unknown table kind " + tp));
  }
  ios.close();
  return nrow;
}
...
std::pair<Table, String> findParentTable(const String& fullName, const TableLock& lockOpt,
                                         Table::TableOption tabOpt, const TSMOption& tsmOpt) {
  Table tab;
  String lastPart, msg;
