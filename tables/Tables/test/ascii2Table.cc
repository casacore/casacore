...
void calc(Table& tab, const String& name) {
  ...
  if (cd.dataType() == TpBool || cd.dataType() == TpString || cd.dataType() == TpComplex ||
      cd.dataType() == TpDComplex || cd.ndim() != 0) {
  ...
  keys.define("Comment", "Mean, etc. of column " + name + " have been calculated");
}
