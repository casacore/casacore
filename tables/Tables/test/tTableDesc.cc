...
void a(Bool doExcp) {
  ...
  if (doExcp) {
    try {
      td.addColumn(ScalarColumnDesc<ExampleDesc>("af", ColumnDesc::Undefined));
    } catch (std::exception& x) {
      cout << x.what() << endl;  // undefined given
    }
  }
  ...
  if (doExcp) {
    try {
      ...
      cout << x.what() << endl;  // column is a scalar
    }
    try {
    ...
    try {
      ...
      cout << x.what() << endl;  // ndim already defined
    }
    try {
      td.rwColumnDesc("Arr3").setShape(IPosition());
    } catch (std::exception& x) {
      cout << x.what() << endl;  // shape already defined
    }
  }
  td.addColumn(SubTableDesc("sub1", "subtable by name", "tTableDesc_tmp_sub"));
  td.addColumn(SubTableDesc("sub2", "subtable copy", subtd));
  td.addColumn(SubTableDesc("sub3", "subtable pointer", &subtd));
  td.addColumn(SubTableDesc("sub4", "subtable by name", "tTableDesc_tmp_sub1"), "sub4Renamed");
  td.show();
  cout << td.keywordSet().isDefined("aa") << td.isColumn("ab") << td.keywordSet().isDefined("ab")
       << td.isColumn("aa") << endl;
  ...
  if (doExcp) {
    try {
      ...
      cout << x.what() << endl;  // subtable sub1 does not exist
    }
  }
...
void b(Bool doExcp) {
  ...
  if (doExcp) {
    try {
      ...
      cout << x.what() << endl;  // shape mimatches ndim
    }
  }
