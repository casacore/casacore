      ...
      if (opt == 8 || opt == 9) {
        ...
        if (!err) {
          ...
          if (opt == 9) {
            ...
            cols.put(rownr, "ARatherLongTestString" + std::to_string(val));
            ...
            if (rownr >= Int(tab.nrow())) {
              ...
              cout << "Row " << rownr << " has value " << col1(rownr) << ' ' << col2(rownr) << ' '
                   << col3(rownr) << ' ' << cols(rownr) << ' ' << freq(rownr)(IPosition(1, 0))
                   << '-' << freq(rownr)(IPosition(1, 24)) - 24 << ' '
                   << pol(rownr)(IPosition(1, 0)) << '-' << pol(rownr)(IPosition(1, 15)) - 15 << ' '
...
int main(int argc, const char* argv[]) {
  if (argc < 2) {
    cout << "Execute as: tTableLockSync 1 x  to create new table" << endl;
    cout << "            tTableLockSync 0 x  to update existing table" << endl;
