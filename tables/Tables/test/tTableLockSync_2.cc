...
void c(const TableLock& lockMode, uInt wait, uInt lastWait, Bool show) {
  ...
  while (True) {
    ...
    for (uInt rownr = oldNrrow; rownr < nrrow; rownr++) {
      ...
      result = col2(rownr);
      if (result != val + 1) {
        cout << "col2 in row " << rownr << " has value " << result << endl;
      }
      freq.get(rownr, freqValues);
      if (freqValues(IPosition(1, 0)) != val + 2 || freqValues(IPosition(1, 24)) != val + 2 + 24) {
        cout << "freq in row " << rownr << " has value " << freqValues(IPosition(1, 0)) << '-'
             << freqValues(IPosition(1, 24)) << endl;
      }
      pol.get(rownr, polValues);
      if (polValues(IPosition(1, 0)) != val + 3 || polValues(IPosition(1, 15)) != val + 3 + 15) {
        cout << "pol in row " << rownr << " has value " << polValues(IPosition(1, 0)) << '-'
      ...
      if (dataValues(IPosition(2, 0, 0)) != val + 4 ||
        ...
        cout << "data in row " << rownr << " has value " << dataValues(IPosition(2, 0, 0)) << '-'
...
int main(int argc, const char* argv[]) {
  if (argc < 6) {
    ...
    cout << "  tTableLockSync_2 option inspection_interval "
         ...
         << endl;
    cout << "    option   1=Permanent 2=PermanentWait 3=Auto 4=User" << endl;
