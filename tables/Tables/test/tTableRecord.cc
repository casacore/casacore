...
void check (const TableRecord&, int32_t intValue, uint32_t nrField);
...
void doIt (bool doExcp)
{
    ...
    //    RecordFieldPtr(TableRecord &record, uint32_t whichField);
    //    T &operator*()
    ...
    *stringField = "Hello";
    ...
    RecordFieldPtr<Array<int16_t> >    arrayshortField(record, 12);
    RecordFieldPtr<Array<int32_t> >      arrayintField(record, 13);
    RecordFieldPtr<Array<uint32_t> >     arrayuintField(record, 14);
    ...
    *subref = 9.0;
    ...
    // TableRecord& rwSubRecord (int32_t whichField);
    ...
    *arrayintField = -1234566;
    ...
    RecordFieldPtr<Array<int32_t> > savrfarray (savrec2, 13);
    ...
    *savrfarray = *savrf;
    ...
    //    RecordFieldPtr();
    //    void attachToRecord(TableRecord &record, uint32_t whichField);
...
void check (const TableRecord& record, int32_t intValue, uint32_t nrField)
{
    ...
    //    RORecordFieldPtr(TableRecord &record, uint32_t whichField);
    ...
    RORecordFieldPtr<Array<int16_t> >    arrayshortField(record, 12);
    RORecordFieldPtr<Array<int32_t> >      arrayintField(record, 13);
    RORecordFieldPtr<Array<uint32_t> >     arrayuintField(record, 14);
...
void testTable (bool doExcp)
{
    ...
    td1.addColumn (ScalarColumnDesc<int32_t> ("col1"));
    ...
    for(uint32_t i=0; i<td4.ncolumn(); ++i)
    {
    ...
    uint32_t dim = td4.hypercolumnDesc("hc1", dcresult, ccresult, icresult);
