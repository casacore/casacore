...
void testColons() {
  ...
  }
  {
    ...
    Table subtab4(TableUtil::createTable(mainName + "::SubTab2::SubSubTab4", td,
                                         Table::NewNoReplace, Table::Plain, StorageOption(),
                                         Record(), TableLock(), 10));
  }
  {
...
void testReplace() {
  // Create a subtable.
  {
    TableDesc td;
    td.addColumn(ScalarColumnDesc<String>("col2"));
    Table tab(TableUtil::createTable(mainName + "::SubTab2::SubSubTab6", td, Table::NewNoReplace));
    AlwaysAssertExit(tab.nrow() == 0);
    AlwaysAssertExit(tab.tableDesc().ncolumn() == 1);
  ...
  // Replace the subtable.
  {
    ...
    Table tab(TableUtil::createTable(mainName + "::SubTab2::SubSubTab6", td, Table::New));
    AlwaysAssertExit(tab.nrow() == 0);
...
void testLayoutInfo(const String& tableName) {
  ...
  TableDesc layout;
  cout << "TableUtil::getlayout #rows = " << TableUtil::getLayout(layout, tableName);
  layout.show(cout);
  cout << endl;
