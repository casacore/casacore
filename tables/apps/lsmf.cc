...
int main(int argc, char* argv[]) {
  try {
    ...
    for (vector<String>::const_iterator iter = fname.begin(); iter != fname.end(); ++iter) {
      if (iter->empty()) {
        ...
        if (!File(*iter).exists()) {
          ...
          std::shared_ptr<MultiFileBase> mfile(new MultiHDF5(*iter, ByteIO::Old));
          show(mfile, showbl, "MultiHDF5");
        } else {
          std::shared_ptr<MultiFileBase> mfile(new MultiFile(*iter, ByteIO::Old));
