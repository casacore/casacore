...
int main(int argc, char* argv[]) {
  try {
    ...
    inputs.create("headerfile", "", "Name of optional file containing headers", "string");
    inputs.create("autoheader", "F", "Determine header automatically?", "bool");
    inputs.create("autoshape", "[]", "Shape if all columns are treated as one array", "string");
    inputs.create("columnnames", "", "Comma separated names of the columns", "string");
    inputs.create("datatypes", "", "Comma separated data types of the columns", "string");
    inputs.create("sep", " ", "One character separator between values", "string");
    inputs.create("commentmarker", "", "Regex indicating comments", "string");
    inputs.create("firstline", "1", "First line to process", "int");
    inputs.create("lastline", "-1", "Last line to process (<0 is till end)", "int");
    ...
    TableProxy tp(in, hdrfile, out, autohdr, shp, sep, comm, first, last, cols, typs);
    cout << "Created table " << tp.table().tableName() << " with " << tp.table().nrow() << " rows"
         << endl;
