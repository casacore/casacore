...
uInt skipQuoted(const String& str, uInt st, uInt end) {
  ...
  throw AipsError("Unbalanced quoted string at position " + std::to_string(st) + " in " +
                  std::string(str));
}
...
vector<String> splitWS(const String& str) {
  ...
  if (qpos >= 0) {
    throw AipsError("Unbalanced quoted string at position " + std::to_string(qpos) + " in " +
                    std::string(str));
  }
  if (part.size() > 0) {
...
template <>
void showArray(const Array<MVTime>& arr, ostream& os) {
  if (arr.size() == 1) {
  ...
  if (expr.isScalar()) {
    switch (expr.getColumnDataType()) {
      ...
      case TpString:
        ...
        break;
      case TpQuantity: {
        AlwaysAssert(expr.getNodeRep()->dataType() == TableExprNodeRep::NTDate, AipsError);
        MVTime time;
        if (expr.nrow() != 1) os << '[';
...
void showParseError(const TableParseError& x) {
  ...
  if (isatty(2) && x.pos() >= 0) {
    ...
    cerr << msg.substr(0, errPos) << "\033[1;43m" << msg.substr(errPos, errLen);
...
Table taqlCommand(const Options& options, const String& varName, const String& command,
  ...
  if (spos != String::npos) {
    ...
    addComm = !(s == "with" || s == "select" || s == "update" || s == "insert" || s == "calc" ||
                s == "delete" || s == "count" || s == "create" || s == "createtable" ||
                s == "drop" || s == "droptable" || s == "alter" || s == "altertable" ||
                s == "using" || s == "usingstyle" || s == "time" || showHelp);
  }
  String strc(command);
  if (addComm) {
    strc = "SELECT " + command;
  ...
  if (result.isTable()) {
    tabp = result.table();
    if (printNRows) {
      os << "    " << cmd << " result of " << tabp.nrow() << " rows" << endl;
    }
    ...
    if ((cmd == "select" || cmd == "count") && colNames.size() > 0) {
      ...
      if (printSelect || (printAuto && maxNRows > 0)) {
        ...
        if (showSubset) {
          os << "  Note: only the first " << maxNRows << " rows are shown (out of " << tabp.nrow()
             << ')' << endl;
        }
      }
    }
...
void showVersion() { cerr << "taql version " << getVersionCASA() << endl; }

void showHelp() {
  cerr << endl;
  cerr << "TaQL is the query language for casacore tables and is described at" << endl;
  ...
  cerr << "  The default style is python; if no value is given after -s it defaults to glish"
       << endl;
  ...
  cerr << " -d separator           separator used between printed values (default a tab (\t))"
       << endl;
  ...
  cerr << "";
  cerr << "These options can also be given before any TaQL command. If followed by a command,"
       << endl;
...
void showTableInfo(const String& name, const Table& tab, const String& command, Int level,
  ...
  os << ' ' << command << endl;
  if (level >= 0) {
    os << "  " << tab.nrow() << " rows, " << tdesc.ncolumn() << " columns" << endl;
  }
  if (level > 0) {
...
void showTableMap(const TableMap& tables, ostream& os) {
  if (tables.empty()) {
    ...
    for (TableMap::const_iterator iter = tables.begin(); iter != tables.end(); ++iter) {
...
String substituteName(const String& name, const TableMap& tableMap, vector<const Table*>& tabs) {
